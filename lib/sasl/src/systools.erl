...
[`app(4)`](`p:kernel:app.md`), [`appup(4)`](appup.md), [`erl(1)`](`p:erts:erl_cmd.md`), [`rel(4)`](rel.md), `m:release_handler`, [`relup(4)`](relup.md), [`script(4)`](script.md)
""".
...
Generates a boot script `Name.script` and its binary version, the boot file `Name.boot`, unless the `{script_name, ScriptName}` option is given, in which case the names are `ScriptName.script` and `ScriptName.boot` The boot file specifies which code to be loaded and which applications to be started when the Erlang runtime system is started. See [`script(4)`](script.md).

The release resource file `Name.rel` is read to determine which applications are included in the release. Then the relevant application resource files `App.app` are read to determine which modules to be loaded, and if and how the applications are to be started. (Keys `modules` and `mod`, see [`app(4)`](`p:kernel:app.md`).
...
-doc """
Creates a release package file `Name.tar.gz`. This file must be uncompressed and unpacked on the target system using `m:release_handler` before the new release can be installed.

The release resource file `Name.rel` is read to determine which applications are included in the release. Then the relevant application resource files `App.app` are read to determine the version and modules of each application (keys `vsn` and `modules`, see [`app(4)`](`p:kernel:app.md`)).
...
If the `{extra_files, ExtraFiles}` option is given then the `ExtraFiles` are added to the tarball after everything else to be included has been added. The `ExtraFiles` list is a list of files or directories in the same format as the `add_type()` tuple for [erl_tar:add/3,4](`erl_tar:add/3`)
...
* __`include`__ - Each separate (variable) package is included in the main `ReleaseName.tar.gz` file. This is the default.

* __`ownfile`__ - Each separate (variable) package is generated as a separate file in the same directory as the `ReleaseName.tar.gz` file.

* __`omit`__ - No separate (variable) packages are generated. Applications that are found underneath a variable directory are ignored.
...
All checks with function [`make_script`](`make_script/1`) are performed before the release package is created. Options `src_tests` and `exref` are also valid here.

The return value and the handling of errors and warnings are the same as described for [`make_script`](`make_script/1`).
""".
...
A boot script generated using [`make_script`](`make_script/1`) is already transformed to the binary form.
""".
script2boot(File) ->
...
Generates a release upgrade file `relup` containing instructions for upgrading from or downgrading to one or more previous releases. The instructions are used by `m:release_handler` when installing a new version of a release in runtime.
...
If an upgrade includes a change from an emulator earlier than OTP R15 to OTP R15 or later, the warning `pre_R15_emulator_upgrade` is issued. For more information about this, see [Design Principles](`p:system:appup_cookbook.md`) in *System Documentation*.

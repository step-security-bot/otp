...
* __`{set_alarm, {AlarmId, AlarmDescr}}`__ - This event is generated by `alarm_handler:set_alarm({AlarmId, AlarmDecsr})`.

* __`{clear_alarm, AlarmId}`__ - This event is generated by `alarm_handler:clear_alarm(AlarmId)`.

The default simple handler is called `alarm_handler` and it can be exchanged by calling `gen_event:swap_handler/3` as `gen_event:swap_handler(alarm_handler, {alarm_handler, swap}, {NewHandler, Args})`. `NewHandler:init({Args, {alarm_handler, Alarms}})` is called. For more details, see `m:gen_event` in STDLIB.
...
%%-----------------------------------------------------------------
-doc "Returns a list of all active alarms. This function can only be used when the simple handler is installed.".
get_alarms() ->

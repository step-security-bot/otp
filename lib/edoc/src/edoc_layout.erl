...
-module(edoc_layout).
-moduledoc """
The standard HTML layout module for EDoc. See the `m:edoc` module for details on usage.
...
Options to the standard layout:
* __`{index_columns, integer()}`__ - Specifies the number of column pairs used for the function index tables. The default value is 1.

* __`{pretty_printer, atom()}`__ - Specifies how types and specifications are pretty printed. If the value `erl_pp` is specified the Erlang pretty printer (the module `erl_pp`) will be used. The default is to do no pretty printing which implies that lines can be very long.
...
* __`{sort_functions, boolean()}`__ - If `true`, the detailed function descriptions are listed by name, otherwise they are listed in the order of occurrence in the source file. The default value is `true`.

* __`{xml_export, Module::atom()}`__ - Specifies an [`xmerl`](`p:xmerl:index.html`) callback module to be used for exporting the documentation. See [`//xmerl/xmerl:export_simple/3`](`xmerl:export_simple/3`) for details.

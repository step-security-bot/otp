...
An implementation of an HTTP 1.1 compliant web server, as defined in [RFC 2616](http://www.ietf.org/rfc/rfc2616.txt). Provides web server start options, administrative functions, and an Erlang callback API.
...
A web server can be configured to start when starting the `Inets` application, or dynamically in runtime by calling the `Inets` application API `inets:start(httpd, ServiceConfig)` or `inets:start(httpd, ServiceConfig, How)`, see `m:inets`. The configuration options, also called properties, are as follows:
...
* __[](){: id=prop_proplist_file }
  \{proplist_file, path()\}__  
...
* __[](){: id=prop_doc_root }
  \{document_root, path()\}__  
  Defines the top directory for the documents that are available on the HTTP server.

[](){: id=props_comm }
*Communication Properties*

* __[](){: id=prop_bind_address }
  \{bind_address, ip_address() | hostname() | any\}__  
  Default is `any`

* __[](){: id=prop_server_name }
  \{server_name, string()\}__  
...
* __[](){: id=profile }
  \{profile, atom()\}__  
  Used together with [`bind_address`](`m:httpd#prop_bind_address`) and [`port`](`m:httpd#prop_port`) to uniquely identify a HTTP server. This can be useful in a virtualized environment, where there can be more that one server that has the same bind_address and port. If this property is not explicitly set, it is assumed that the [`bind_address`](`m:httpd#prop_bind_address`) and [`port`](`m:httpd#prop_port`) uniquely identifies the HTTP server.

* __[](){: id=prop_socket_type }
  \{socket_type, ip_comm | \{ip_comm, Config::proplist()\} | \{ssl, Config::proplist()\}\}__  
...
* __[](){: id=prop_ipfamily }
  \{ipfamily, inet | inet6\}__  
  Default is `inet`, legacy option `inet6fb4` no longer makes sense and will be translated to inet.

* __[](){: id=prop_minimum_bytes_per_second }
  \{minimum_bytes_per_second, integer()\}__  
...
[](){: id=props_api_modules }
*Erlang Web Server API Modules*

* __[](){: id=prop_modules }
  \{modules, \[atom()]\}__  
  Defines which modules the HTTP server uses when handling requests. Default is `[mod_alias, mod_auth, mod_esi, mod_actions, mod_cgi, mod_dir, mod_get, mod_head, mod_log, mod_disk_log]`. Notice that some `mod`\-modules are dependent on others, so the order cannot be entirely arbitrary. See the [Inets Web Server Modules](http_server.md) in the User's Guide for details.

[](){: id=props_limit }
*Limit properties*

* __[](){: id=prop_customize }
  \{customize, atom()\}__  
  A callback module to customize the inets HTTP servers behaviour see `m:httpd_custom_api`

* __[](){: id=prop_disable_chunked_encoding }
  \{disable_chunked_transfer_encoding_send, boolean()\}__  
  Allows you to disable chunked transfer-encoding when sending a response to an HTTP/1.1 client. Default is `false`.

* __[](){: id=prop_keep_alive }
  \{keep_alive, boolean()\}__  
...
* __[](){: id=prop_keep_alive_timeout }
  \{keep_alive_timeout, integer()\}__  
...
* __[](){: id=prop_max_uri }
  \{max_uri_size, integer()\}__  
  Limits the size of the HTTP request URI. Default is no limit.

* __[](){: id=prop_max_keep_alive_req }
  \{max_keep_alive_request, integer()\}__  
...
* __[](){: id=max_client_body_chunk }
  \{max_client_body_chunk, integer()\}__  
  Enforces chunking of a HTTP PUT or POST body data to be delivered to the mod_esi callback. Note this is not supported for mod_cgi. Default is no limit e.i the whole body is delivered as one entity, which could be very memory consuming. `m:mod_esi`.

[](){: id=props_admin }
*Administrative Properties*

* __[](){: id=prop_mime_types }
  \{mime_types, \[\{MimeType, Extension\}] | path()\}__  
...
* __[](){: id=prop_server_admin }
  \{server_admin, string()\}__  
  Defines the email-address of the server administrator to be included in any error messages returned by the server.

* __[](){: id=prop_server_tokens }
  \{server_tokens, none|prod|major|minor|minimal|os|full|\{private, string()\}\}__  
  ...
  * __`none`__ - "" % A Server: header will not be generated

  * __`prod`__ - "inets"

  * __`major`__ - "inets/5"

  * __`minor`__ - "inets/5.8"

  * __`minimal`__ - "inets/5.8.1"

  * __`os`__ - "inets/5.8.1 (unix)"

  * __`full`__ - "inets/5.8.1 (unix/linux) OTP/R15B"

  * __`{private, "foo/bar"}`__ - "foo/bar"

  By default, the value is as before, that is, `minimal`.

* __[](){: id=prop_logger }
  \{logger, Options::list()\}__  
  Currently only one option is supported:

  * __`{error, ServerID::atom()}`__ - Produces [logger events](`t:logger:log_event/0`) on logger [level error](`t:logger:level/0`) under the hierarchical logger [domain:](`t:logger:log_event/0`) `[otp, inets, httpd, ServerID, error]` The built in logger formatting function produces log entries from the error reports:
...
* __[](){: id=prop_log_format }
  \{log_format, common | combined\}__  
  ...
  * __`remotehost`__ - Remote.

  * __`rfc931`__ - The remote username of the client ([RFC 931](http://www.ietf.org/rfc/rfc931.txt)).
  ...
  * __`"request"`__ - The request line as it came from the client ([RFC 1945](http://www.ietf.org/rfc/rfc1945.txt)).
  ...
  * __`"referer"`__ - The URL the client was on before requesting the URL (if it could not be determined, a minus sign is placed in this field).

  * __`"user_agent"`__ - The software the client claims to be using (if it could not be determined, a minus sign is placed in this field).

  This affects the access logs written by `mod_log` and `mod_disk_log`.

* __[](){: id=prop_elog_format }
  \{error_log_format, pretty | compact\}__  
...
* __[](){: id=prop_script_alias }
  \{script_alias, \{Alias, RealName\}\}__  
...
* __[](){: id=prop_script_re_write }
  \{script_re_write, \{Re, Replacement\}\}__  
...
* __[](){: id=prop_script_nocache }
  \{script_nocache, boolean()\}__  
...
* __[](){: id=prop_script_timeout }
  \{script_timeout, integer()\}__  
...
* __[](){: id=prop_action }
  \{action, \{MimeType, CgiScript\}\} - requires mod_action__  
...
* __[](){: id=prop_script }
  \{script, \{Method, CgiScript\}\} - requires mod_action__  
  `Method = string()` and `CgiScript = string()`. `script` adds an action activating a CGI script whenever a file is requested using a certain HTTP method. The method is either GET or POST, as defined in [RFC 1945](http://www.ietf.org/rfc/rfc1945.txt). It propagates the URL and file path of the requested document using the standard CGI PATH_INFO and PATH_TRANSLATED environment variables.
...
* __[](){: id=prop_esi_alias }
  \{erl_script_alias, \{URLPath, \[AllowedModule]\}\}__  
...
* __[](){: id=prop_esi_nocache }
  \{erl_script_nocache, boolean()\}__  
...
* __[](){: id=prop_esi_timeout }
  \{erl_script_timeout, integer()\}__  
...
* __[](){: id=prop_dlog_format }
  \{disk_log_format, internal | external\}__  
...
* __[](){: id=prop_edlog }
  \{error_disk_log, path()\}__  
  Defines the filename of the (`m:disk_log`) error log file to be used to log server errors. If the filename does not begin with a slash (/), it is assumed to be relative to the `server_root`.

* __[](){: id=prop_edlog_size }
  \{error_disk_log_size, \{MaxBytes, MaxFiles\}\}__  
  `MaxBytes = integer()` and `MaxFiles = integer()`. Defines the properties of the (`m:disk_log`) error log file. This file is of type wrap log and max bytes is written to each file and max files is used before the first file is truncated and reused.

* __[](){: id=prop_sdlog }
  \{security_disk_log, path()\}__  
  Defines the filename of the (`m:disk_log`) access log file logging incoming security events, that is, authenticated requests. If the filename does not begin with a slash (/), it is assumed to be relative to the `server_root`.

* __[](){: id=prop_sdlog_size }
  \{security_disk_log_size, \{MaxBytes, MaxFiles\}\}__  
  `MaxBytes = integer()` and `MaxFiles = integer()`. Defines the properties of the `m:disk_log` access log file. This file is of type wrap log and max bytes is written to each file and max files is used before the first file is truncated and reused.

* __[](){: id=prop_tdlog }
  \{transfer_disk_log, path()\}__  
  Defines the filename of the (`m:disk_log`) access log file logging incoming requests. If the filename does not begin with a slash (/), it is assumed to be relative to the `server_root`.

* __[](){: id=prop_tdlog_size }
  \{transfer_disk_log_size, \{MaxBytes, MaxFiles\}\}__  
  `MaxBytes = integer()` and `MaxFiles = integer()`. Defines the properties of the `m:disk_log` access log file. This file is of type wrap log and max bytes is written to each file and max files is used before the first file is truncated and reused.
...
* __[](){: id=prop_allow_from }
  \{allow_from, all | \[RegxpHostString]\}__  
...
* __[](){: id=prop_deny_from }
  \{deny_from, all | \[RegxpHostString]\}__  
...
* __[](){: id=prop_auth_type }
  \{auth_type, plain | dets | mnesia\}__  
...
* __[](){: id=prop_auth_user_file }
  \{auth_user_file, path()\}__  
...
* __[](){: id=prop_auth_group_file }
  \{auth_group_file, path()\}__  
...
* __[](){: id=prop_auth_access_passwd }
  \{auth_access_password, string()\}__  
...
* __[](){: id=prop_max_retries }
  \{max_retries, integer()\}__  
...
* __[](){: id=prop_block_time }
  \{block_time, integer()\}__  
...
* __[](){: id=prop_fail_exp_time }
  \{fail_expire_time, integer()\}__  
...
* __[](){: id=prop_auth_timeout }
  \{auth_timeout, integer()\}__  
...
* __`data`__ - Type `[{InteractionKey,InteractionValue}]` is used to propagate data between modules. Depicted `interaction_data()` in function type declarations.

* __`socket_type`__ - `socket_type()` indicates whether it is an IP socket or an `ssl` socket.

* __`socket`__ - The socket, in format `ip_comm` or `ssl`, depending on `socket_type`.

* __`config_db`__ - The config file directives stored as key-value tuples in an ETS table. Depicted `config_db()` in function type declarations.

* __`method`__ - Type `"GET" | "POST" | "HEAD" | "TRACE"`, that is, the HTTP method.

* __`absolute_uri`__ - If the request is an HTTP/1.1 request, the URI can be in the absolute URI format. In that case, `httpd` saves the absolute URI in this field. An Example of an absolute URI is `"http://ServerName:Part/cgi-bin/find.pl?person=jocke"`

* __`request_uri`__ - The `Request-URI` as defined in [RFC 1945](http://www.ietf.org/rfc/rfc1945.txt), for example, `"/cgi-bin/find.pl?person=jocke"`.

* __`http_version`__ - The `HTTP` version of the request, that is, "HTTP/1.0", or "HTTP/1.1".

* __`request_line`__ - The `Request-Line` as defined in[RFC 1945](http://www.ietf.org/rfc/rfc1945.txt), for example, `"GET /cgi-bin/find.pl?person=jocke HTTP/1.0"`.

* __`parsed_header`__ - Type `[{HeaderKey,HeaderValue}]`. `parsed_header` contains all HTTP header fields from the HTTP request stored in a list as key-value tuples. See [RFC 2616](http://www.ietf.org/rfc/rfc2616.txt) for a listing of all header fields. For example, the date field is stored as `{"date","Wed, 15 Oct 1997 14:35:17 GMT"}`. RFC 2616 defines that HTTP is a case-insensitive protocol and the header fields can be in lower case or upper case. `httpd` ensures that all header field names are in lower case.

* __`entity_body`__ - The `entity-Body` as defined in [RFC 2616](http://www.ietf.org/rfc/rfc2616.txt), for example, data sent from a CGI script using the POST method.

* __`connection`__ - `true | false`. If set to `true`, the connection to the client is a persistent connection and is not closed when the request is served.
...
When a valid request reaches `httpd`, it calls `c:do/1` in each module, defined by the configuration option of `Module`. The function can generate data for other modules or a response that can be sent back to the client.

The field `data` in `ModData` is a list. This list is the list returned from the last call to `c:do/1`.

`Body` is the body of the HTTP response that is sent back to the client. An appropriate header is appended to the message. `StatusCode` is the status code of the response, see [RFC 2616](http://www.ietf.org/rfc/rfc2616.txt) for the appropriate values.

`Head` is a key value list of HTTP header fields. The server constructs an HTTP header from this data. See [RFC 2616](http://www.ietf.org/rfc/rfc2616.txt) for the appropriate value for each header field. If the client is an HTTP/1.0 client, the server filters the list so that only HTTP/1.0 header fields are sent back to the client.
...
When `httpd` is shut down, it tries to execute `c:remove/1` in each Erlang web server callback module. The programmer can use this function to clean up resources created in the store function.
""".
-doc(#{title => <<"ERLANG WEB SERVER API CALLBACK FUNCTIONS">>}).
...
-doc "`parse_query/1` parses incoming data to `erl` and `eval` scripts (see `m:mod_esi`) as defined in the standard URL format, that is, '+' becomes 'space' and decoding of hexadecimal characters (`%xx`).".
-doc(#{deprecated =>
           <<"httpd:parse_query/1 is deprecated; use uri_string:dissect_query/1 instead">>,
       title => <<"ERLANG WEB SERVER API HELP FUNCTIONS">>}).
...
> #### Note {: class=info }
> Pid is the pid returned from `inets:start/[2,3]`. Can also be retrieved form `inets:services/0` and `inets:services_info/0`, see `m:inets`.
""".
-doc(#{equiv => info/4}).
-doc(#{since => <<"OTP 18.0">>}).

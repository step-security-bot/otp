...
convert_chunk(What, Module, ModulePath) ->
    case code:get_doc(Module, #{ sources => [eep48] }) of
        {ok, #docs_v1{ format = <<"application/erlang+html">>,
            ...
            ModuleDocLine = case lists:member(Module, [init, asn1ct, erl_features, erl_lint]) of
...
generate_doc_attributes(D, Kind, Slogan, Meta, Files) ->
    DocString =
        case D of
            #{ <<"en">> := ErlangHtml } when not is_map_key(equiv, Meta) ->
                Prototype =
                    case is_map_key(spec, Meta) orelse Kind =/= function of
                        true ->
                            [];
                        false ->
                            [re:replace(Slogan,"\n\\s+"," ",[global,unicode]),"\n\n"]
...
%% preprocess_docs([{a,[{id,_Id}|_],[]} = A,{Tag,_,_} = C|T],D) when ?IS_HDR(Tag); Tag =:= dl ->
%%     [A | preprocess_docs([C|T], D)];
...
render_element({a, Ids, []}, _State, Pos, _Ind, _D) ->
    trimnl({[["[]()",ial([{id,Id}])] || {id,Id} <- Ids], Pos});
...
render_element({code,CodeAttr,[Content,{a,AAttr,AContent}|H]}, State, Pos, Ind, D) ->
    render_docs([{code,CodeAttr,[Content]},{a,AAttr,[{code,CodeAttr,AContent}]},
        ...
        maybe
            ...
            of
                ...
                false when length(Args) =:= 1,
                    ...
                    NameStr = io_lib:write_atom(Name),
                    {["`p:",NameStr,"_cmd.md`"], Pos + string:length(NameStr)};
                false ->
                    try
...
render_element({dl, [], [{dt,DTAttr,DTContent}, {dd,[],DDContent} | Content]}, State, Pos, Ind, D) ->
    ...
    {DDDocs, DDNewPos} = render_docs(DDContent, [li, dl | State], 0, Ind + 2, D),
    {Docs, NewPos} =
...
render_link(Docs, <<"https://erlang.org/doc/link/seeerl">>,
    ...
    ["[",Docs,"](`p:erl_docgen:doc_storage.md`)"];
render_link(Docs, <<"https://erlang.org/doc/link/seeerl">>,
            <<"edoc:edoc_cmd">>) ->
    ["[",Docs,"](edoc_cmd.md)"];
...
render_link(Docs, <<"https://erlang.org/doc/link/seeguide">>, Href) ->
    ...
    case string:lexemes(Href, ":#") of
        [App, <<"index">>] when App =:= CurrentApplication ->
            ["[", Docs, "](index.html)"];
        [App, <<"index">>] ->
            ["[", Docs, "](`p:",RemoveSystemApp(App),":index.html`)"];
        ...
        [App, Guide] ->
            ["[", Docs, "](`p:",RemoveSystemApp(App),":",string:lowercase(Guide),".md`)"];
        [App, Guide, Anchor] ->
            ["[", Docs, "](`p:",RemoveSystemApp(App),":",string:lowercase(Guide),".md#",
              render_link_anchor(Anchor),"`)"]
    end;
    ...
    case string:lexemes(Href, ":#") of
        ...
        [App, Guide, Anchor] when App =:= CurrentApplication ->
            ["[", Docs, "](",AddPostfix(Guide),".md#",
              render_link_anchor(Anchor),")"];
        [App, Guide] ->
            ["[", Docs, "](`p:",App,":",AddPostfix(Guide),".md`)"];
        [App, Guide, Anchor] ->
            ["[", Docs, "](`p:",App,":",AddPostfix(Guide),".md#",
              render_link_anchor(Anchor),"`)"]
    end;

...
The following section describes the mandatory and optional test suite functions that `Common Test` calls during test execution. For more details, see section [Writing Test Suites](write_test_chapter.md) in the User's Guide.
""".
%%------------------------------------------------------------------
...
-doc "The test suite information, as returned by [`Module:suite/0`](`c:suite/0`), [`Module:group/1`](`c:group/1`) and [`Module:Testcase/0`](`c:Testcase/0`).".
...
-doc """
[ct_test_def()](`t:ct_test_def/0`) = TestCase | \{group, GroupName\} | \{group, GroupName, Properties\} | \{group, GroupName, Properties, SubGroups\} | \{testcase, TestCase, TestCaseRepeatType\}  
TestCase = [ct_testname()](`t:ct_testname/0`)  
GroupName = [ct_groupname()](`t:ct_groupname/0`)  
...
TestCaseRepeatType = \[\{repeat, N\} | \{repeat_until_ok, N\} | \{repeat_until_fail, N\}]  
N = integer() | forever  
Reason = term()  

MANDATORY

Returns the list of all test cases and test case groups in the test suite module to be executed. This list also specifies the order the cases and groups are executed by `Common Test`. A test case is represented by an atom, the name of the test case function, or a `testcase` tuple indicating that the test case shall be repeated. A test case group is represented by a `group` tuple, where `GroupName`, an atom, is the name of the group (defined in [`Module:groups/0`](`c:groups/0`)). Execution properties for groups can also be specified, both for a top-level group and for any of its subgroups. Group execution properties specified here override properties in the group definition (see [`Module:groups/0`](`c:groups/0`)). (With value `default`, the group definition properties are used).

If `{skip, Reason}` is returned, all test cases in the module are skipped and `Reason` is printed on the HTML result page.

For details on groups, see section [Test Case Groups](write_test_chapter.md#test_case_groups) in the User's Guide.
""".
...
-doc """
[ct_group_def()](`t:ct_group_def/0`) = \{GroupName, Properties, GroupsAndTestCases\}  
GroupName = [ct_groupname()](`t:ct_groupname/0`)  
Properties = \[parallel | sequence | Shuffle | \{RepeatType, N\}]  
GroupsAndTestCases = \[Group | \{group, GroupName\} | TestCase | \{testcase, TestCase, TestCaseRepeatType\}]  
TestCase = [ct_testname()](`t:ct_testname/0`)  
Shuffle = shuffle | \{shuffle, Seed\}  
Seed = \{integer(), integer(), integer()\}  
RepeatType = repeat | repeat_until_all_ok | repeat_until_all_fail | repeat_until_any_ok | repeat_until_any_fail  
...
Defines test case groups. For details, see section [Test Case Groups](write_test_chapter.md#test_case_groups) in the User's Guide.
""".
-doc(#{title => <<"Callback Functions">>}).
-callback groups() ->
...
-doc """
[ct_info()](`t:ct_info/0`) = \{timetrap, Time\} | \{require, Required\} | \{require, Name, Required\} | \{userdata, UserData\} | \{silent_connections, Conns\} | \{stylesheet, CSSFile\} | \{ct_hooks, CTHs\} | \{ct_hooks_order, CTHOrder\}  
...
Tag `timetrap` sets the maximum time that each test case is allowed to execute (including [`Module:init_per_testcase/2`](`c:init_per_testcase/2`) and [`Module:end_per_testcase/2`](`c:end_per_testcase/2`)). If the timetrap time is exceeded, the test case fails with reason `timetrap_timeout`. A `TimeFunc` function can be used to set a new timetrap by returning a `TimeVal`. It can also be used to trigger a timetrap time-out by, at some point, returning a value other than a `TimeVal`. For details, see section [Timetrap Time-Outs](write_test_chapter.md#timetraps) in the User's Guide.

Tag `require` specifies configuration variables required by test cases (or configuration functions) in the suite. If the required configuration variables are not found in any of the configuration files, all test cases are skipped. For details about the `require` functionality, see function [`ct:require/1,2`](`ct:require/1`).

With `userdata`, the user can specify any test suite-related information, which can be read by calling `ct:userdata/2`.

Tag `ct_hooks` specifies the [Common Test Hooks](ct_hooks_chapter.md) to be run with this suite.

Other tuples than the ones defined are ignored.

For details about the test suite information function, see section [Test Suite Information Function](write_test_chapter.md#suite) in the User's Guide.
""".
-doc(#{title => <<"Callback Functions">>}).
-callback suite() ->
    [Info :: ct_info()].
...
-doc """
Config = NewConfig = SaveConfig = [ct_config()](`t:ct_config/0`)  
Reason = term()  

OPTIONAL; if this function is defined, then [`Module:end_per_suite/1`](`c:end_per_suite/1`) must also be defined.
...
For information on `save_config` and `skip_and_save`, see section [Saving Configuration Data](dependencies_chapter.md#save_config) in the User's Guide.
""".
-doc(#{title => <<"Callback Functions">>}).
...
-doc """
Config = SaveConfig = [ct_config()](`t:ct_config/0`)  

OPTIONAL; if this function is defined, then [`Module:init_per_suite/1`](`c:init_per_suite/1`) must also be defined.

This function is called as the last test case in the suite. It is meant to be used for cleaning up after [`Module:init_per_suite/1`](`c:init_per_suite/1`).

For information on `save_config`, see section [Saving Configuration Data](dependencies_chapter.md#save_config) in the User's Guide.
""".
-doc(#{title => <<"Callback Functions">>}).
...
-doc """
GroupName = [ct_groupname()](`t:ct_groupname/0`)  
[ct_info()](`t:ct_info/0`) = \{timetrap, Time\} | \{require, Required\} | \{require, Name, Required\} | \{userdata, UserData\} | \{silent_connections, Conns\} | \{stylesheet, CSSFile\} | \{ct_hooks, CTHs\}  
Time = TimeVal | TimeFunc  
TimeVal = MilliSec | \{seconds, integer()\} | \{minutes, integer()\} | \{hours, integer()\}  
TimeFunc = \{Mod, Func, Args\} | Fun  
MilliSec = integer()  
...
The test case group information function. It is supposed to return a list of tagged tuples that specify various properties related to the execution of a test case group (that is, its test cases and subgroups). Properties set by [`Module:group/1`](`c:group/1`) override properties with the same key that have been set previously by [`Module:suite/0`](`c:suite/0`).

Tag `timetrap` sets the maximum time that each test case is allowed to execute (including [`Module:init_per_testcase/2`](`c:init_per_testcase/2`) and [`Module:end_per_testcase/2`](`c:end_per_testcase/2`)). If the timetrap time is exceeded, the test case fails with reason `timetrap_timeout`. A `TimeFunc` function can be used to set a new timetrap by returning a `TimeVal`. It can also be used to trigger a timetrap time-out by, at some point, returning a value other than a `TimeVal`. For details, see section [Timetrap Time-Outs](write_test_chapter.md#timetraps) in the User's Guide.

Tag `require` specifies configuration variables required by test cases (or configuration functions) in the suite. If the required configuration variables are not found in any of the configuration files, all test cases in this group are skipped. For details about the `require` functionality, see function [`ct:require/1,2`](`ct:require/1`).
...
For details about the test case group information function, see section [Group Information Function](write_test_chapter.md#group_info) in the User's Guide.
""".
...
-doc """
GroupName = [ct_groupname()](`t:ct_groupname/0`)  
Config = NewConfig = [ct_config()](`t:ct_config/0`)  
Reason = term()  

OPTIONAL; if this function is defined, then [`Module:end_per_group/2`](`c:end_per_group/2`) must also be defined.
...
For information about test case groups, see section [Test Case Groups](write_test_chapter.md#test_case_groups) in the User's Guide.
""".
-doc(#{title => <<"Callback Functions">>}).
...
-doc """
GroupName = [ct_groupname()](`t:ct_groupname/0`)  
Config = [ct_config()](`t:ct_config/0`)  
Status = [ct_status()](`t:ct_status/0`)  

OPTIONAL; if this function is defined, then [`Module:init_per_group/2`](`c:init_per_group/2`) must also be defined.
...
For details about test case groups, see section [Test Case Groups](write_test_chapter.md#test_case_groups) in the User's Guide.
""".
-doc(#{title => <<"Callback Functions">>}).
...
OPTIONAL; if this function is defined, then [`Module:end_per_testcase/2`](`c:end_per_testcase/2`) must also be defined.
...
OPTIONAL; if this function is defined, then [`Module:init_per_testcase/2`](`c:init_per_testcase/2`) must also be defined.
...
The implementation of a test case. Call the functions to test and check the result. If something fails, ensure the function causes a runtime error or call [`ct:fail/1,2`](`ct:fail/1`) (which also causes the test case process to terminate).
...
For details about test case implementation, see section [Test Cases](write_test_chapter.md#test_cases) in the User's Guide.
...
The test case information function. It is supposed to return a list of tagged tuples that specify various properties related to the execution of this particular test case. Properties set by [`Module:Testcase/0`](`c:Testcase/0`) override properties set previously for the test case by [`Module:group/1`](`c:group/1`) or [`Module:suite/0`](`c:suite/0`).

Tag `timetrap` sets the maximum time that the test case is allowed to execute. If the timetrap time is exceeded, the test case fails with reason `timetrap_timeout`. [`Module:init_per_testcase/2`](`c:init_per_testcase/2`) and [`Module:end_per_testcase/2`](`c:end_per_testcase/2`) are included in the timetrap time. A `TimeFunc` function can be used to set a new timetrap by returning a `TimeVal`. It can also be used to trigger a timetrap time-out by, at some point, returning a value other than a `TimeVal`. For details, see section [Timetrap Time-Outs](write_test_chapter.md#timetraps) in the User's Guide.

Tag `require` specifies configuration variables that are required by the test case (or `c:init_per_testcase/2` or `c:end_per_testcase/2`). If the required configuration variables are not found in any of the configuration files, the test case is skipped. For details about the `require` functionality, see function [`ct:require/1,2`](`ct:require/1`).

If `timetrap` or `require` is not set, the default values specified by [`Module:suite/0`](`c:suite/0`) (or [`Module:group/1`](`c:group/1`)) are used.

With `userdata`, the user can specify any test case-related information that can be read by calling `ct:userdata/3`.

Other tuples than the ones defined are ignored.

For details about the test case information function, see section [Test Case Information Function](write_test_chapter.md#info_function) in the User's Guide.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP R14B">>}).

...
Call [`connect/1,2`](`connect/1`) to establish a connection to a server, then pass the returned handle to [`session/1-3`](`session/1`) to establish a NETCONF session on a new SSH channel. Each call to [`session/1-3`](`session/1`) establishes a new session on the same connection, and results in a hello message to the server.

Alternately, [`open/1,2`](`open/1`) can be used to establish a single session on a dedicated connection. (Or, equivalently, [`only_open/1,2`](`only_open/1`) followed by [`hello/1-3`](`hello/1`).)
...
The [`server_id()`](`t:server_id/0`) or an associated [`ct:target_name()`](`t:ct:target_name/0`) can then be passed to the aforementioned functions to use the referenced configuration.

[](){: id=Signaling }
*Signaling*

Protocol operations in the NETCONF protocol are realized as remote procedure calls (RPCs) from client to server and a corresponding reply from server to client. RPCs are sent using like-named functions (eg. [`edit_config/3-5`](`edit_config/3`) to send an edit-config RPC), with the server reply as return value. There are functions for each RPC defined in RFC 6241 and the create-subscription RPC from RFC 5277, all of which are wrappers on [`send_rpc/2,3`](`send_rpc/2`), that can be used to send an arbitrary RPC not defined in RFC 6241 or RFC 5277.
...
* __`raw`__ - The sent and received NETCONF data is logged to a separate text file "as is" without any formatting. A link to the file is added to the test case HTML log.

* __`pretty`__ - The sent and received NETCONF data is logged to a separate text file with XML data nicely indented. A link to the file is added to the test case HTML log.

* __`html (default)`__ - The sent and received NETCONF traffic is pretty printed directly in the test case HTML log.

* __`silent`__ - NETCONF traffic is not logged.
...
-doc "Handle to a connection to a NETCONF server as returned by [`connect/1,2`](`connect/1`), or to a session as returned by [`session/1-3`](`session/1`), [`open/1,2`](`open/1`), or [`only_open/1,2`](`only_open/1`).".
-opaque handle() :: pid().

-doc """
Options `host` and `port` specify the server endpoint to which to connect, and are passed directly to [`ssh:connect/4`](`ssh:connect/3`), as are arbitrary ssh options. Common options are `user`, `password` and `user_dir`.
...
-doc "Event notification messages sent as a result of calls to [`create_subscription/2,3`](`create_subscription/2`).".
-type notification() :: {notification, xml_attributes(), [simple_xml()]}.

-doc "".
-type stream_name() :: string().
-doc """
Stream information as returned by [`get_event_streams/1-3`](`get_event_streams/1`). See RFC 5277, "XML Schema for Event Notifications", for detail on the format of the string values.
""".
...
-doc "Representation of XML, as described in application [`xmerl`](`p:xmerl:index.html`).".
...
The opaque [`handle()`](`t:handle/0`) reference returned from this function is required as connection identifier when opening sessions over this connection, see [`session/1-3`](`session/1`).
""".
-doc(#{since => <<"OTP 20.0">>}).
...
If `KeyOrName` is a configured `t:server_id()` or a `target_name()` associated with such an Id, then the options for this server are fetched from the configuration file.
...
The opaque [`handle()`](`t:handle/0`) reference returned from this function can be used as connection identifier when opening sessions over this connection, see [`session/1-3`](`session/1`). However, if `KeyOrName` is a `target_name()`, that is, if the server is named through a call to `ct:require/2` or a `require` statement in the test suite, then this name can be used instead of [`handle()`](`t:handle/0`).
""".
-doc(#{since => <<"OTP 20.0">>}).
...
If there are open NETCONF sessions on the connection, these will be brutally aborted. To avoid this, close each session with [`close_session/1,2`](`close_session/1`)
""".
-doc(#{since => <<"OTP 20.0">>}).
...
The opaque [`handle()`](`t:handle/0`) reference returned from this function can be used as client identifier when calling any other function in this module. However, if `KeyOrName` is used and it is a `target_name()`, that is, if the server is named through a call to `ct:require/2` or a `require` statement in the test suite, then this name can be used instead of [`handle()`](`t:handle/0`).
""".
-doc(#{since => <<"OTP 20.0">>}).
...
If the server options are specified in a configuration file, or if a named client is needed for logging purposes (see section [Logging](`m:ct_netconfc#logging`) in this module), use `open/2` instead.

The opaque [`handle()`](`t:handle/0`) reference returned from this function is required as client identifier when calling any other function in this module.
""".
...
The opaque [`handle()`](`t:handle/0`) reference returned from this function can be used as client identifier when calling any other function in this module. However, if `KeyOrName` is a `target_name()`, that is, if the server is named through a call to `ct:require/2` or a `require` statement in the test suite, then this name can be used instead of [`handle()`](`t:handle/0`).
...
Operation [`kill_session/2,3`](`kill_session/2`) can be used to force the release of a lock owned by another NETCONF session. How this is achieved by the server side is implementation-specific.
""".
-doc(#{since => <<"OTP R15B02">>}).
...
If the client earlier has acquired a lock through [`lock/2,3`](`lock/2`), this operation releases the associated lock. To access another target than `running`, the server must support `:candidate` and/or `:startup`.
""".
-doc(#{since => <<"OTP R15B02">>}).
...
-doc "Executes an action. If the return type is void, `ok` is returned instead of `{ok,[simple_xml()]}`.".
-doc(#{since => <<"OTP R15B02">>}).
...
-doc """
Creates a subscription for event notifications by sending an RFC 5277 create-subscription RPC to the server. The calling process receives events as messages of type [`notification()`](`t:notification/0`).

From RFC 5722, 2.1 Subscribing to Receive Event Notifications:

* __`Stream`__ - Indicates which stream of event is of interest. If not present, events in the default NETCONF stream are sent.

* __`Filter`__ - Indicates which subset of all possible events is of interest. The parameter format is the same as that of the filter parameter in the NETCONF protocol operations. If not present, all events not precluded by other parameters are sent.

* __`StartTime`__ - Used to trigger the replay feature and indicate that the replay is to start at the time specified. If `StartTime` is not present, this is not a replay subscription. It is not valid to specify start times that are later than the current time. If `StartTime` is specified earlier than the log can support, the replay begins with the earliest available notification. This parameter is of type `dateTime` and compliant to RFC 3339. Implementations must support time zones.

* __`StopTime`__ - Used with the optional replay feature to indicate the newest notifications of interest. If `StopTime` is not present, the notifications continues until the subscription is terminated. Must be used with and be later than `StartTime`. Values of `StopTime` in the future are valid. This parameter is of type `dateTime` and compliant to RFC 3339. Implementations must support time zones.
...
If more complex filtering is needed, use [`ct_netconfc:get/2,3`](`get/2`) and specify the exact filter according to "XML Schema for Event Notifications" in RFC 5277.
""".
-doc(#{since => <<"OTP 20.0,OTP R15B02">>}).

...
Callback module for `m:ct_telnet`, for connecting to a Telnet server on a UNIX host.
...
To communicate through Telnet to the host specified by `HostNameOrIpAddress`, use the interface functions in `m:ct_telnet`, for example, `open(Name)` and `cmd(Name,Cmd)`.
...
The "keep alive" activity (that is, that `Common Test` sends NOP to the server every 10 seconds if the connection is idle) can be enabled or disabled for one particular connection as described here. It can be disabled for all connections using `telnet_settings` (see `m:ct_telnet`).

The `{port,PortNum}` tuple is optional and if omitted, default Telnet port 23 is used. Also the `keep_alive` tuple is optional, and the value default to `true` (enabled).

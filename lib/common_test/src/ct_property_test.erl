...
and the the property test module (in this example `ftp_simple_client_server.erl`) as almost a usual property testing module (More examples are in [the User's Guide](ct_property_test_chapter.md)):
...
This function investigates if support is available for either [QuickCheck](http://www.quviq.com), [PropEr](https://proper-testing.github.io) or [Triq](https://github.com/krestenkrab/triq) and compiles the properties with the first tool found. It is supposed to be called in the `init_per_suite/1` function in a CommonTest test suite.
...
%%%
-doc "Same as [`present_result(Module, Cmds, Triple, Config, [])`](`present_result/5`)".
-doc(#{since => <<"OTP 22.3">>}).
...
Config =  
  the Common Test [Config](`c:ct_suite:Testcase/1`) in test cases.  
...
See the [User's Guide](ct_property_test_chapter.md#stateful1) for an example of the usage and of the default printout.
...
* `CollectFun` is called with one argument: the `Cmds`. It should return a list of the values to be counted. The following pre-defined functions exist:
  * `ct_property_test:cmnd_names/1` returns a list of commands (function calls) generated in the `Cmnd` sequence, without Module, Arguments and other details.

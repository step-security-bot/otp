...
* Whatever added by [`init_per_suite/1`](`c:ct_suite:init_per_suite/1`) or [`init_per_testcase/2`](`c:ct_suite:init_per_testcase/2`) in the test suite.
""".
...
-doc "A name and association to configuration data introduced through a require statement, or a call to [`ct:require/2`](`require/2`), for example, `ct:require(mynodename,{node,[telnet]})`.".
...
-doc "Options that can be given to the `cth_conn_log` hook, which is used for logging of NETCONF and Telnet connections. See [ct_netconfc](`m:ct_netconfc#logging`) or [ct_telnet](`m:ct_telnet#logging`) for description and examples of how to use this hook.".
...
-doc """
Runs tests as specified by the combination of options in `Opts`. The options are the same as those used with program `ct_run`, see [Run Tests from Command Line](ct_run_cmd.md#ct_run) in the `ct_run` manual page.
...
`TestRunnerPid` is returned if `release_shell == true`. For details, see [`ct:break/1`](`break/1`).
...
If any functions (for example, Telnet or FTP) using "required configuration data" are to be called from the Erlang shell, configuration data must first be required with [`ct:require/2`](`require/2`).
...
See also [`ct:get_config/1`](`get_config/1`), [`ct:get_config/2`](`get_config/2`), [`ct:get_config/3`](`get_config/3`), [`ct:require/2`](`require/2`).
""".
...
-doc """
Checks if the required configuration is available and gives it a name. The semantics for `Required` is the same as in [`ct:require/1`](`require/1`) except that a list of `SubKey`s cannot be specified.

If the requested data is available, the subentry is associated with `Name` so that the value of the element can be read with [`ct:get_config/1,2`](`get_config/1`) provided `Name` is used instead of the whole `Required` term.
...
See also [`ct:get_config/1`](`get_config/1`), [`ct:get_config/2`](`get_config/2`), [`ct:get_config/3`](`get_config/3`), [`ct:require/1`](`require/1`).
""".
...
Returns the matching values or configuration elements, given a configuration variable key or its associated name (if one has been specified with [`ct:require/2`](`require/2`) or a `require` statement).
...
If a configuration variable key has been associated with a name (by [`ct:require/2`](`require/2`) or a `require` statement), the name can be used instead of the key to read the value:
...
See also [`ct:get_config/1`](`get_config/1`), [`ct:get_config/2`](`get_config/2`), [`ct:require/1`](`require/1`), [`ct:require/2`](`require/2`).
""".
...
-doc "Equivalent to [`ct:log(Category, Importance, Format, FormatArgs, Opts)`](`log/5`).".
...
For details on `Category`, `Importance` and the `no_css` option, see section [Logging - Categories and Verbosity Levels](write_test_chapter.md#logging) in the User's Guide.
...
-doc "Equivalent to [`ct:print(Category, Importance, Format, FormatArgs, [])`](`print/5`).".
-doc(#{since => <<"OTP R15B02">>}).
-spec print(X1, X2) -> ok
      when X1 :: Category | Importance | Format,
           X2 :: Format | FormatArgs,
...
-doc "Equivalent to [`ct:print(Category, Importance, Format, FormatArgs, Opts)`](`print/5`).".
...
For details on `Category` and `Importance`, see section [Logging - Categories and Verbosity Levels](write_test_chapter.md#logging) in the User's Guide.
""".
-doc(#{since => <<"OTP 19.2">>}).
...
-doc "Equivalent to [`ct:pal(Category, Importance, Format, FormatArgs, Opts)`](`pal/5`).".
...
-doc "Use this function to set, or modify, the verbosity level for a logging category. See the [User's Guide](write_test_chapter.md#logging) for details. Use the value `default` to set the general verbosity level.".
-doc(#{since => <<"OTP 19.1">>}).
...
-doc "This function returns the verbosity level for the specified logging category. See the [User's Guide](write_test_chapter.md#logging) for details. Use the value `default` to read the general verbosity level.".
-doc(#{since => <<"OTP 19.1">>}).
...
See also [`ct:capture_get/1`](`capture_get/1`), [`ct:capture_stop/0`](`capture_stop/0`).
""".
-doc(#{since => <<"OTP R15B">>}).
...
See also [`ct:capture_get/1`](`capture_get/1`), [`ct:capture_start/0`](`capture_start/0`).
""".
-doc(#{since => <<"OTP R15B">>}).
...
See also [`ct:capture_start/0`](`capture_start/0`), [`ct:capture_stop/0`](`capture_stop/0`), [`ct:log/3`](`log/3`).
""".
-doc(#{since => <<"OTP R15B">>}).
...
-doc "Terminates a test case with an error message specified by a format string and a list of values (used as arguments to `io_lib:format/2`).".
-doc(#{since => <<"OTP R15B">>}).
...
Arguments `Format` and `Args` are used in a call to `io_lib:format/2` to create the comment string. The behavior of `comment/2` is otherwise the same as function [`ct:comment/1`](`comment/1`).
""".
-doc(#{since => <<"OTP R15B">>}).
...
-doc "If the test is started with option `create_priv_dir` set to `manual_per_tc`, in order for the test case to use the private directory, it must first create it by calling this function.".
...
-doc "Performs command `listenv` on the specified Telnet connection and returns the result as a list of key-value pairs.".
...
-doc "Returns any data specified with tag `userdata` in the list of tuples returned from [`suite/0`](`c:ct_suite:suite/0`).".
...
-doc "Returns any data specified with tag `userdata` in the list of tuples returned from `Suite:group(GroupName)` or `Suite:Case()`.".
...
-doc "Returns status of ongoing test. The returned list contains information about which test case is executing (a list of cases when a parallel test case group is executing), as well as counters for successful, failed, skipped, and total test cases so far.".
...
For information about using encrypted configuration files when running tests, see section [Encrypted Configuration Files](config_file_chapter.md#encrypted_config_files) in the User's Guide.

For details on DES3 encryption/decryption, see application [`Crypto`](`p:crypto:index.html`).
""".
...
-doc "Decrypts `EncryptFileName`, previously generated with [`ct:encrypt_config_file/2,3`](`encrypt_config_file/2`). The original file contents is saved in the target file. The encryption key, a string, must be available in a text file named `.ct_config.crypt`, either in the current directory, or the home directory of the user (it is searched for in that order).".
...
-doc "Decrypts `EncryptFileName`, previously generated with [`ct:encrypt_config_file/2,3`](`encrypt_config_file/2`). The original file contents is saved in the target file. The key must have the same value as that used for encryption.".
...
-doc "Loads configuration variables using the specified callback module and configuration string. The callback module is to be either loaded or present in the code path. Loaded configuration variables can later be removed using function [`ct:remove_config/2`](`remove_config/2`).".
-doc(#{since => <<"OTP R14B">>}).
...
-doc "Removes configuration variables (together with their aliases) that were loaded with specified callback module and configuration string.".
-doc(#{since => <<"OTP R14B">>}).
...
-doc "This function, similar to `timer:sleep/1` in STDLIB, suspends the test case for a specified time. However, this function also multiplies `Time` with the `multiply_timetraps` value (if set) and under certain circumstances also scales up the time automatically if `scale_timetraps` is set to `true` (default is `false`).".
-doc(#{since => <<"OTP R14B">>}).
-spec sleep(Time) -> ok
...
-doc """
Cancels any active timetrap and pauses the execution of the current test case until the user calls function `continue/0`. The user can then interact with the Erlang node running the tests, for example, for debugging purposes or for manually executing a part of the test case. If a parallel group is executing, [`ct:break/2`](`break/2`) is to be called instead.

A cancelled timetrap is not automatically reactivated after the break, but must be started explicitly with [`ct:timetrap/1`](`timetrap/1`).

In order for the break/continue functionality to work, `Common Test` must release the shell process controlling `stdin`. This is done by setting start option `release_shell` to `true`. For details, see section [Running Tests from the Erlang Shell or from an Erlang Program](run_test_chapter.md#erlang_shell_or_program) in the User's Guide.
""".
-doc(#{since => <<"OTP R15B02">>}).
...
-doc """
Works the same way as [`ct:break/1`](`break/1`), only argument `TestCase` makes it possible to pause a test case executing in a parallel group. Function [`ct:continue/1`](`continue/1`) is to be used to resume execution of `TestCase`.
...
-doc "This function must be called to continue after a test case (not executing in a parallel group) has called function [`ct:break/1`](`break/1`).".
-doc(#{since => <<"OTP R15B02">>}).
-spec continue() -> ok.
continue() -> 
    test_server:continue().

-doc "This function must be called to continue after a test case has called [`ct:break/2`](`break/2`). If the paused test case, `TestCase`, executes in a parallel group, this function, rather than `continue/0`, must be used to let the test case proceed.".
-doc(#{since => <<"OTP R15B02">>}).

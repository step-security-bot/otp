...
When TLS performs certificate path validation according to [RFC 5280 ](http://www.ietf.org/rfc/rfc5280.txt)it should also perform CRL validation checks. To enable the CRL checks the application needs access to CRLs. A database of CRLs can be set up in many different ways. This module provides the behavior of the API needed to integrate an arbitrary CRL cache with the erlang ssl application. It is also used by the application itself to provide a simple default implementation of a CRL cache.
""".
...
-doc "For description see [X509 certificates records](`p:public_key:public_key_records.md`)".
...
-doc """
DistributionPoint = [dist_point()](`t:dist_point/0`)  
Issuer = [public_key:issuer_name()](`t:public_key:issuer_name/0`)  
DbHandle = [crl_cache_ref()](`t:crl_cache_ref/0`)  
CRLs = [[public_key:der_encoded()](`t:public_key:der_encoded/0`)]  
LoggerInfo = \{logger, [logger_info() ](`t:logger_info/0`)\}\}  
...
In an earlier version of this API, the `lookup` function received two arguments, omitting `Issuer`. For compatibility, this is still supported: if there is no `c:lookup/3` function in the callback module, `c:lookup/2` is called instead.
...
-callback lookup(dist_point(), issuer_name(), crl_cache_ref()) ->
    ...
    {{logger, logger_info()}, [public_key:der_encoded()]}.
-doc """
Issuer = [public_key:issuer_name()](`t:public_key:issuer_name/0`) | list()  
DbHandle = [cache_ref()](`t:crl_cache_ref/0`)  
LoggerInfo = \{logger, [logger_info() ](`t:logger_info/0`)\}  

Select the CRLs in the cache that are issued by `Issuer` unless the value is a list of so called general names, see [X509 certificates records](`p:public_key:public_key_records.md`), originating form `#'DistributionPoint'.cRLissuer` and representing different mechanism to obtain the CRLs. The cache callback needs to use the appropriate entry to retrieve the CRLs or return an empty list if it does not exist.
...
-doc """
DistributionPoint = [dist_point()](`t:dist_point/0`)  
CRL = [[public_key:der_encoded()](`t:public_key:der_encoded/0`)]  
FreshCRL = [[public_key:der_encoded()](`t:public_key:der_encoded/0`)]  
LoggerInfo = \{logger, [logger_info() ](`t:logger_info/0`)\}\}  

...
This module contains interface functions for the TLS/DTLS protocol. For detailed information about the supported standards see [ssl(6)](ssl_app.md).
...
For valid options, see the `m:inet`, `m:gen_tcp` and [gen_udp(3)](`m:gen_tcp`) manual pages in Kernel. Note that stream oriented options such as packet are only relevant for TLS and not DTLS
""".
-doc(#{title => <<"Types used in TLS/DTLS">>}).
-type socket_option()            :: gen_tcp:connect_option() | gen_tcp:listen_option() | gen_udp:option(). % exported
-doc "An opaque reference to the TLS/DTLS connection, may be used for equality matching.".
-doc(#{title => <<"Types used in TLS/DTLS">>}).
...
-doc """
Explicitly list acceptable signature schemes (algorithms) in the preferred order. Overrides the algorithms supplied in [`signature_algs`](`t:signature_algs/0`) option for certificates.
...
The client will send a `signature_algorithms_cert` extension (in the client hello message), if TLS version 1.2 (back-ported to TLS 1.2 in 24.1) or later is used, and the signature_algs_cert option is explicitly specified. By default, only the [signature_algs](`t:signature_algs/0`) extension is sent.

> #### Note {: class=info }
> Note that supported signature schemes for TLS-1.2 are [sign_scheme_legacy()](`t:sign_scheme_legacy/0`) and [rsassa_pss_scheme()](`t:rsassa_pss_scheme/0`)
""".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
...
-doc "Choose TLS or DTLS protocol for the transport layer security. Defaults to `tls`. For DTLS other transports than UDP are not yet supported.".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type protocol()                  :: tls | dtls.
-doc "Defaults to `full`. If hello is specified the handshake will pause after the hello message and give the user a possibility make decisions based on hello extensions before continuing or aborting the handshake by calling `handshake_continue/3` or `handshake_cancel/1`".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type handshake_completion()      :: hello | full.
-doc "The DER-encoded user certificate. Note that the cert option may also be a list of DER-encoded certificates where the first one is the user certificate, and the rest of the certificates constitutes the certificate chain. For maximum interoperability the certificates in the chain should be in the correct order, the chain will be sent as is to the peer. If chain certificates are not provided, certificates from [client_cacerts()](`t:client_cacerts/0`), [server_cacerts()](`t:server_cacerts/0`), or [client_cafile()](`t:client_cafile/0`), [server_cafile()](`t:server_cafile/0`) are used to construct the chain. If this option is supplied, it overrides option `certfile`.".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type cert()                      :: public_key:der_encoded().
-doc "Path to a file containing the user certificate on PEM format or possible several certificates where the first one is the user certificate and the rest of the certificates constitutes the certificate chain. For more details see [cert()](`t:cert/0`),".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type cert_pem()                  :: file:filename().
-doc "The DER-encoded user's private key or a map referring to a crypto engine and its key reference that optionally can be password protected, see also [crypto:engine_load/3 ](`crypto:engine_load/3`)and [Crypto's Users Guide](`p:crypto:engine_load.md`). If this option is supplied, it overrides option `keyfile`.".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
...
-doc "A certificate (or possibly a certificate and its chain) and its associated key on one of the possible formats. For the PEM file format there may also be a password associated with the file containg the key.".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
...
The function [ssl:cipher_suites/2 ](`cipher_suites/2`)can be used to find all cipher suites that are supported by default and all cipher suites that may be configured.

If you compose your own `t:cipher_suites()` make sure they are filtered for cryptolib support [ssl:filter_cipher_suites/2 ](`filter_cipher_suites/2`)Additionally the functions [ssl:append_cipher_suites/2 ](`append_cipher_suites/2`), [ssl:prepend_cipher_suites/2](`prepend_cipher_suites/2`), [ssl:suite_to_str/1](`suite_to_str/1`), [ssl:str_to_suite/1](`str_to_suite/1`), and [ssl:suite_to_openssl_str/1](`suite_to_openssl_str/1`) also exist to help creating customized cipher suite lists.
...
-doc "Specifies if to reject renegotiation attempt that does not live up to [RFC 5746](http://www.ietf.org/rfc/rfc5746.txt). By default `secure_renegotiate` is set to `true`, that is, secure renegotiation is enforced. If set to `false` secure renegotiation will still be used if possible, but it falls back to insecure renegotiation if the peer does not support [RFC 5746](http://www.ietf.org/rfc/rfc5746.txt).".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
...
* __`unknown_ca`__ - No trusted CA was found in the trusted store. The trusted CA is normally a so called ROOT CA, which is a self-signed certificate. Trust can be claimed for an intermediate CA (trusted anchor does not have to be self-signed according to X-509) by using option `partial_chain`.

* __`selfsigned_peer`__ - The chain consisted only of one self-signed certificate.

* __`PKIX X-509-path validation error`__ - For possible reasons, see `public_key:pkix_path_validation/3`
""".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type custom_verify()               ::  {Verifyfun :: fun(), InitialUserState :: any()}.
-doc """
Perform CRL (Certificate Revocation List) verification [(public_key:pkix_crls_validate/3)](`public_key:pkix_crls_validate/3`) on all the certificates during the path validation [(public_key:pkix_path_validation/3) ](`public_key:pkix_path_validation/3`)of the certificate chain. Defaults to `false`.

* __`peer`__ - check is only performed on the peer certificate.

* __`best_effort`__ - if certificate revocation status cannot be determined it will be accepted as valid.

The CA certificates specified for the connection will be used to construct the certificate chain validating the CRLs.

The CRLs will be fetched from a local or external cache. See `m:ssl_crl_cache_api`.
""".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type crl_check()                :: boolean() | peer | best_effort.
-doc """
Specify how to perform lookup and caching of certificate revocation lists. `Module` defaults to `m:ssl_crl_cache` with `DbHandle `being `internal` and an empty argument list.

There are two implementations available:

* __`ssl_crl_cache`__ - This module maintains a cache of CRLs. CRLs can be added to the cache using the function `ssl_crl_cache:insert/1`, and optionally automatically fetched through HTTP if the following argument is specified:

  * __`{http, timeout()}`__ - Enables fetching of CRLs specified as http URIs in[X509 certificate extensions](`p:public_key:public_key_records.md`). Requires the OTP inets application.

  

* __`ssl_crl_hash_dir`__ - This module makes use of a directory where CRLs are stored in files named by the hash of the issuer name.
  ...
  * __`{dir, string()}`__ - Specifies the directory in which the CRLs can be found.
""".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type crl_cache_opts()           :: {Module :: atom(),
                                     {DbHandle :: internal | term(),
                                      Args :: list()}}.
-doc "Integer (24 bits unsigned). Used to limit the size of valid TLS handshake packets to avoid DoS attacks. Defaults to 256*1024.".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type handshake_size()           :: integer().
-doc "When an integer-value is specified, `TLS/DTLS-connection` goes into hibernation after the specified number of milliseconds of inactivity, thus reducing its memory footprint. When `undefined` is specified (this is the default), the process never goes into hibernation.".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
...
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type root_fun()                 ::  fun().
-doc "TLS protocol versions supported by started clients and servers. This option overrides the application environment option `protocol_version` and `dtls_protocol_version`. If the environment option is not set, it defaults to all versions, supported by the SSL application. See also [ssl(6).](ssl_app.md)".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type protocol_versions()        ::  [protocol_version()].
-doc """
Explicitly list acceptable signature algorithms for certificates and handshake messages in the preferred order. The client will send its list as the client hello `signature_algorithm` extension introduced in TLS-1.2, see [Section 7.4.1.4.1 in RFC 5246](http://www.ietf.org/rfc/rfc5246.txt). Previously these algorithms where implicitly chosen and partly derived from the cipher suite.

In TLS-1.2 a somewhat more explicit negotiation is made possible using a list of \{[hash()](`t:hash/0`), [sign_algo()](`t:signature_algs/0`)\} pairs.

In TLS-1.3 these algorithm pairs are replaced by so called signature schemes [sign_scheme()](`t:sign_scheme/0`) and completely decoupled from the cipher suite.

Signature algorithms used for certificates may be overridden by the [signature schemes](`t:sign_schemes/0`) (algorithms) supplied by the `signature_algs_cert` option.
...
For Secure Remote Password (SRP), the fun is only used by the server to obtain parameters that it uses to generate its session keys. `DerivedKey` is to be derived according to [RFC 2945](http://tools.ietf.org/html/rfc2945#section/3) and [RFC 5054](http://tools.ietf.org/html/rfc5054#section-2.4): `crypto:sha([Salt, crypto:sha([Username, <<$:>>, Password])])`
""".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
...
-doc """
If set to `false`, TLS/DTLS Alert reports are not displayed. Deprecated in OTP 22, use \{log_level, [logging_level()](`t:logging_level/0`)\} instead.
""".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
-type log_alert()                :: boolean().
-doc "Specifies the log level for a TLS/DTLS connection. Alerts are logged on `notice` level, which is the default level. The level `debug` triggers verbose logging of TLS/DTLS protocol messages. See also [ssl(6)](ssl_app.md)".
-doc(#{title =>
           <<"TLS/DTLS OPTION DESCRIPTIONS - COMMON for SERVER and CLIENT">>}).
...
> #### Note {: class=info }
> This option is supported by TLS 1.3 and above. See also [SSL's Users Guide, Session Tickets and Session Resumption in TLS 1.3](using_ssl.md#session-tickets-and-session-resumption-in-tls-1-3)
""".
...
> #### Note {: class=info }
> OTP does not handle SRTP, so an external implementations of SRTP encoder/decoder and a packet demultiplexer are needed to make use of the `use_srtp` extension. See also [cb_info](`t:transport_option/0`) option.
...
-doc "Defaults to `verify_none` as additional options are needed to be able to perform the certificate verification. A warning will be emitted unless `verify_none` is explicitly configured. Usually the applications will want to configure `verify_peer` together with an appropriate `cacert` or `cacertfile` option. For example an `HTTPS` client would normally use the option `{cacerts, public_key:cacerts_get()}` (available since OTP-25) to access the CA certificates provided by the OS. Using verify_none means that all x509-certificate path validation errors will be ignored. See also option [verify_fun](`t:custom_verify/0`).".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
-type client_verify_type()       :: verify_type().
-doc "Reuses a specific session. The session should be referred by its session id if it is earlier saved with the option `{reuse_sessions, save}` since OTP-21.3 or explicitly specified by its session id and associated data since OTP-22.3. See also [SSL's Users Guide, Session Reuse pre TLS 1.3.](using_ssl.md#session-reuse-pre-tls-1-3)".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
-type client_reuse_session()     :: session_id() | {session_id(), SessionData::binary()}.
-doc "When `save` is specified a new connection will be negotiated and saved for later reuse. The session ID can be fetched with `connection_information/2` and used with the client option [reuse_session](`t:client_reuse_session/0`) The boolean value true specifies that if possible, automated session reuse will be performed. If a new session is created, and is unique in regard to previous stored sessions, it will be saved for possible later reuse. Since OTP-21.3.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
-type client_reuse_sessions()    :: boolean() | save.
-doc "If set to true, sends the certificate authorities extension in TLS-1.3 client hello. The default is false. Note that setting it to true may result in a big overhead if you have many trusted CA certificates. Since OTP-24.3.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
-type client_certificate_authorities()  :: boolean().
-doc "The DER-encoded trusted certificates. If this option is supplied it overrides option `cacertfile`.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
...
> #### Note {: class=info }
> When PEM caching is enabled, files provided with this option will be checked for updates at fixed time intervals specified by the [ssl_pem_cache_clean](ssl_app.md#configuration) environment parameter.

> #### Note {: class=info }
> Alternatively, CA certificates can be provided as a DER-encoded binary with [client_cacerts](`t:client_cacerts/0`) option.
""".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
...
-doc "Specifies the identity the client presents to the server. The matching secret is found by calling `user_lookup_fun`".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
...
-doc "Specifies the maximum fragment length the client is prepared to accept from the server. See [RFC 6066](http://www.ietf.org/rfc/rfc6066.txt)".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - CLIENT">>}).
...
> #### Note {: class=info }
> Alternatively, CA certificates can be provided as a DER-encoded binary with [server_cacerts](`t:server_cacerts/0`) option.
""".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
...
-doc "List of protocols to send to the client if the client indicates that it supports the Next Protocol extension. The client can select a protocol that is not on this list. The list of protocols must not contain an empty binary. If the server negotiates a Next Protocol, it can be accessed using the `negotiated_next_protocol/1` method.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
...
-doc "Client certificates are an optional part of the TLS protocol. A server only does x509-certificate path validation in mode `verify_peer`. By default the server is in `verify_none` mode an hence will not send an certificate request to the client. When using `verify_peer` you may also want to specify the options [fail_if_no_peer_cert](`t:fail_if_no_peer_cert/0`) and [certificate_authorities](`t:server_certificate_authorities/0`).".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
-type server_verify_type()       :: verify_type().
-doc "Used together with `{verify, verify_peer}` by an TLS/DTLS server. If set to `true`, the server fails if the client does not have a certificate to send, that is, sends an empty certificate. If set to `false`, it fails only if the client sends an invalid certificate (an empty certificate is considered valid). Defaults to false.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
-type fail_if_no_peer_cert()     :: boolean().
-doc "Enables the TLS/DTLS server to have a local policy for deciding if a session is to be reused or not. Meaningful only if `reuse_sessions` is set to `true`. `SuggestedSessionId` is a `t:binary()`, `PeerCert` is a DER-encoded certificate, `Compression` is an enumeration integer, and `CipherSuite` is of type `ciphersuite()`.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
-type server_reuse_session()     :: fun().
-doc "The boolean value true specifies that the server will agree to reuse sessions. Setting it to false will result in an empty session table, that is no sessions will be reused. See also option [reuse_session](`t:server_reuse_session/0`).".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
-type server_reuse_sessions()    :: boolean().
-doc "If the server receives a SNI (Server Name Indication) from the client matching a host listed in the `sni_hosts` option, the specific options for that host will override previously specified options. The option `sni_fun`, and `sni_hosts` are mutually exclusive.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
-type sni_hosts()                :: [{hostname(), [server_option() | common_option()]}].
-doc """
If the server receives a SNI (Server Name Indication) from the client, the given function will be called to retrieve [\[server_option()] ](`t:server_option/0`)for the indicated server. These options will be merged into predefined [\[server_option()] ](`t:server_option/0`)list. The function should be defined as: fun(ServerName :: string()) -> [\[server_option()] ](`t:server_option/0`)and can be specified as a fun or as named `fun module:function/1` The option `sni_fun`, and `sni_hosts` are mutually exclusive.
""".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
...
-doc "If true, use the server's preference for ECC curve selection. If false (the default), use the client's preference.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
-type honor_ecc_order()          :: boolean().
-doc "In protocols that support client-initiated renegotiation, the cost of resources of such an operation is higher for the server than the client. This can act as a vector for denial of service attacks. The SSL application already takes measures to counter-act such attempts, but client-initiated renegotiation can be strictly disabled by setting this option to `false`. The default value is `true`. Note that disabling renegotiation can result in long-lived connections becoming unusable due to limits on the number of messages the underlying cipher suite can encipher.".
-doc(#{title => <<"TLS/DTLS OPTION DESCRIPTIONS - SERVER">>}).
-type client_renegotiation()     :: boolean().
-doc """
Configures the seed used for the encryption of stateless session tickets. Allowed values are any randomly generated `t:binary()`. If this option is not configured, an encryption seed will be randomly generated.
...
If the option `active` is set to `once`, `true` or an integer value, the process owning the sslsocket will receive messages of type [active_msgs()](`t:active_msgs/0`)
""".
-doc(#{since => <<"OTP R14B">>}).
-doc(#{equiv => connect/4}).
...
When the option `verify` is set to `verify_peer` the check `public_key:pkix_verify_hostname/2` will be performed in addition to the usual x509-path validation checks. If the check fails the error \{bad_cert, hostname_check_failed\} will be propagated to the path validation fun [verify_fun](`t:custom_verify/0`), where it is possible to do customized checks by using the full possibilities of the `public_key:pkix_verify_hostname/3` API. When the option `server_name_indication` is provided, its value (the DNS name) will be used as `ReferenceID` to `public_key:pkix_verify_hostname/2`. When no `server_name_indication` option is given, the `Host` argument will be used as Server Name Indication extension. The `Host` argument will also be used for the `public_key:pkix_verify_hostname/2` check and if the `Host` argument is an `t:inet:ip_address()` the `ReferenceID` used for the check will be `{ip, Host}` otherwise `dns_id` will be assumed with a fallback to `ip` if that fails.
...
-doc """
Accepts an incoming connection request on a listen socket. `ListenSocket` must be a socket returned from `listen/2`. The socket returned is to be passed to [handshake/\[2,3]](`handshake/2`) to complete handshaking, that is, establishing the TLS/DTLS connection.
...
-doc """
If `Socket` is a ordinary `t:socket()`: upgrades a `gen_tcp`, or equivalent, socket to an SSL socket, that is, performs the TLS server-side handshake and returns a TLS socket.
...
If `Socket` is an [sslsocket() ](`t:sslsocket/0`): provides extra TLS/DTLS options to those specified in `listen/2` and then performs the TLS/DTLS handshake. Returns a new TLS/DTLS socket if the handshake is successful.
...
%%--------------------------------------------------------------------
-doc "Assigns a new controlling process to the SSL socket. A controlling process is the owner of an SSL socket, and receives all messages from the socket.".
...
In order to retrieve keylog and other secret information from a TLS 1.3 connection, [keep_secrets](`t:keep_secrets/0`) must be configured in advance and set to `true`.
...
%%--------------------------------------------------------------------
-doc "The peer certificate is returned as a DER-encoded binary. The certificate can be decoded with `public_key:pkix_decode_cert/2` Suggested further reading about certificates is [public_key User's Guide](`p:public_key:public_key_records.md`) and [ssl User's Guide](standards_compliance.md)".
...
%%--------------------------------------------------------------------
-doc "Same as `cipher_suites/2` but lists RFC or OpenSSL string names instead of [erl_cipher_suite()](`t:erl_cipher_suite/0`)".
-doc(#{since => <<"OTP 22.0">>}).
...
%%--------------------------------------------------------------------
-doc "Removes cipher suites if any of the filter functions returns false for any part of the cipher suite. If no filter function is supplied for some part the default behaviour regards it as if there was a filter function that returned true. For examples see [Customizing cipher suites ](using_ssl.md#customizing-cipher-suites)Additionally, this function also filters the cipher suites to exclude cipher suites not supported by the cryptolib used by the OTP crypto application. That is calling ssl:filter_cipher_suites(Suites, []) will be equivalent to only applying the filters for cryptolib support.".
-doc(#{since => <<"OTP 20.3">>}).
...
%%--------------------------------------------------------------------
-doc "Make `Preferred` suites become the most preferred suites that is put them at the head of the cipher suite list `Suites` after removing them from `Suites` if present. `Preferred` may be a list of cipher suites or a list of filters in which case the filters are use on `Suites` to extract the preferred cipher list.".
-doc(#{since => <<"OTP 20.3">>}).
...
%%--------------------------------------------------------------------
-doc "Make `Deferred` suites become the least preferred suites, that is put them at the end of the cipher suite list `Suites` after removing them from `Suites` if present. `Deferred` may be a list of cipher suites or a list of filters in which case the filters are use on `Suites` to extract the Deferred cipher list.".
-doc(#{since => <<"OTP 20.3">>}).
...
%%--------------------------------------------------------------------
-doc "Returns a list of supported ECCs. `eccs()` is equivalent to calling `eccs(Protocol)` with all supported protocols and then deduplicating the output.".
-doc(#{since => <<"OTP 19.2">>}).
...
* __`app_vsn`__ - The application version of the SSL application.

* __`supported`__ - TLS versions supported with current application environment and crypto library configuration. Overridden by a version option on [connect/\[2,3,4]](`connect/2`), `listen/2`, and [handshake/\[2,3]](`handshake/2`). For the negotiated TLS version, see [connection_information/1 ](`connection_information/1`).

* __`supported_dtls`__ - DTLS versions supported with current application environment and crypto library configuration. Overridden by a version option on [connect/\[2,3,4]](`connect/2`), `listen/2`, and [handshake/\[2,3]](`handshake/2`). For the negotiated DTLS version, see [connection_information/1 ](`connection_information/1`).

* __`available`__ - All TLS versions supported with the linked crypto library.

* __`available_dtls`__ - All DTLS versions supported with the linked crypto library.

* __`implemented`__ - All TLS versions supported by the SSL application if linked with a crypto library with the necessary support.

* __`implemented_dtls`__ - All DTLS versions supported by the SSL application if linked with a crypto library with the necessary support.
""".
-doc(#{since => <<"OTP R14B">>}).
...
%%---------------------------------------------------------------
-doc """
There are cryptographic limits on the amount of plaintext which can be safely encrypted under a given set of keys. If the amount of data surpasses those limits, a key update is triggered and a new set of keys are installed. See also the option [key_update_at](`t:key_update_at/0`).
...
%%--------------------------------------------------------------------
-doc "Uses the Pseudo-Random Function (PRF) of a TLS session to generate extra key material. It either takes user-generated values for `Secret` and `Seed` or atoms directing it to use a specific value from the session security parameters.".
...
%%--------------------------------------------------------------------
-doc "Converts [erl_cipher_suite()](`t:erl_cipher_suite/0`) to RFC name string.".
-doc(#{since => <<"OTP 21.0">>}).
...
%%--------------------------------------------------------------------
-doc """
Converts [erl_cipher_suite()](`t:erl_cipher_suite/0`) to OpenSSL name string.
...
%%
%%--------------------------------------------------------------------
-doc "Converts an RFC or OpenSSL name string to an [erl_cipher_suite()](`t:erl_cipher_suite/0`) Returns an error if the cipher suite is not supported or the name is not a valid cipher suite name.".
-doc(#{since => <<"OTP 22.0">>}).

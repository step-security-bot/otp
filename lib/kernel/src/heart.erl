...
An Erlang runtime system to be monitored by a heart program is to be started with command-line flag `-heart` (see also [`erl(1)`](`p:erts:erl_cmd.md`)). The `heart` process is then started automatically:
...
* __`ERL_CRASH_DUMP_SECONDS=0`__ - Suppresses the writing of a crash dump file entirely, thus rebooting the runtime system immediately. This is the same as not setting the environment variable.

* __`ERL_CRASH_DUMP_SECONDS=-1`__ - Setting the environment variable to a negative value does not reboot the runtime system until the crash dump file is completely written.

* __`ERL_CRASH_DUMP_SECONDS=S`__ - `heart` waits for `S` seconds to let the crash dump file be written. After `S` seconds, `heart` reboots the runtime system, whether the crash dump file is written or not.
...
-doc "Gets the temporary reboot command. If the command is cleared, the empty string is returned.".
...
-doc "Clears the temporary boot command. If the system terminates, the normal `HEART_COMMAND` is used to reboot.".
-spec clear_cmd() -> ok.
...
-doc "Get the validation callback. If the callback is cleared, `none` will be returned.".
-doc(#{since => <<"OTP 18.3">>}).
...
* __`check_schedulers`__ - If enabled, a signal will be sent to each scheduler to check its responsiveness. The system check occurs before any heartbeat sent to the port program. If any scheduler is not responsive enough the heart program will not receive its heartbeat and thus eventually terminate the node.
...
-doc "Returns `{ok, Options}` where `Options` is a list of current options enabled for heart. If the callback is cleared, `none` will be returned.".
-doc(#{since => <<"OTP 18.3">>}).

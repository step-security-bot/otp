...
server(info, {ReadHandle,eof}, State = #state{ read = ReadHandle }) ->
    ...
    keep_state_and_data;
server(info,{'EXIT',ReadHandle, _Reason}, #state{ read = ReadHandle }) ->
    keep_state_and_data;
...
server(info, {Requester, get_tty_state}, _State) ->
    Requester ! {self(), get_tty_state, prim_tty:isatty(stdout) },
    keep_state_and_data;
    ...
    case edlin:edit_line(unicode:characters_to_list(Cs), Cont) of
        ...
        {undefined,_Char,MoreCs,NewCont,Rs} ->
            {keep_state,
             ...
             {next_state, info, {ReadHandle,{data,MoreCs}}}};
...
switch_cmd(r, Gr0) ->
	...
	true ->
	    ...
	    {retry, [], Gr};
	false ->
	    {retry, [{put_chars,unicode,<<"Not alive\n">>}]}
    end;
switch_cmd({r, Node}, Gr) when is_atom(Node)->
    switch_cmd({r, Node, shell}, Gr);
switch_cmd({r,Node,Shell}, Gr0) when is_atom(Node),
                                            is_atom(Shell) ->
    Pid = group:start(self(), {Node,Shell,start,[]}),
    Gr = gr_add_cur(Gr0, Pid, {Node,Shell,start,[]}),
    {retry, [], Gr};
switch_cmd([{atom,_,q}], _Gr) ->

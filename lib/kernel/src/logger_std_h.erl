...
This is the standard handler for Logger. Multiple instances of this handler can be added to Logger, and each instance prints logs to [`standard_io`](`t:io:standard_io/0`), [`standard_error`](`t:io:standard_error/0`), or to file.

The handler has an overload protection mechanism that keeps the handler process and the Kernel application alive during high loads of log events. How overload protection works, and how to configure it, is described in the [`User's Guide` ](logger_chapter.md#overload_protection).

To add a new instance of the standard handler, use [`logger:add_handler/3` ](`logger:add_handler/3`). The handler configuration argument is a map which can contain general configuration parameters, as documented in the [`User's Guide` ](logger_chapter.md#handler_configuration), and handler specific parameters. The specific data is stored in a sub map with the key `config`, and can contain the following parameters:

* __`type = `{: id=type }[`io:standard_io()`](`t:io:standard_io/0`)` | `[`io:standard_error()`](`t:io:standard_error/0`)` | file | {device, `[`io:device()`](`t:io:device/0`)`}`__ - Specifies the log destination.

  The value is set when the handler is added, and it cannot be changed in runtime.

  Defaults to [`standard_io`](`t:io:standard_io/0`), unless parameter [`file`](`m:logger_std_h#file`) is given, in which case it defaults to `file`.

* __`file = `{: id=file }[`file:filename()`](`t:file:filename/0`)__ - This specifies the name of the log file when the handler is of type `file`.

  The value is set when the handler is added, and it cannot be changed in runtime.

  Defaults to the same name as the handler identity, in the current directory.

* __`modes = [`{: id=modes }[`file:mode()`](`t:file:mode/0`)`]`__ - This specifies the file modes to use when opening the log file, see `file:open/2`. If `modes` are not specified, the default list used is `[raw,append,delayed_write]`. If `modes` are specified, the list replaces the default modes list with the following adjustments:
...
* __`max_no_bytes = pos_integer() | infinity`{: id=max_no_bytes }__ - This parameter specifies if the log file should be rotated or not. The value `infinity` means the log file will grow indefinitely, while an integer value specifies at which file size (bytes) the file is rotated.

  Defaults to `infinity`.

* __`max_no_files = non_neg_integer()`{: id=max_no_files }__ - This parameter specifies the number of rotated log file archives to keep. This has meaning only if [`max_no_bytes`](`m:logger_std_h#max_no_bytes`) is set to an integer value.
...
* __`compress_on_rotate = boolean()`{: id=compress_on_rotate }__ - This parameter specifies if the rotated log file archives shall be compressed or not. If set to `true`, all archives are compressed with `gzip`, and renamed to `FileName.N.gz`

  `compress_on_rotate` has no meaning if [`max_no_bytes`](`m:logger_std_h#max_no_bytes`) has the value `infinity`.

  Defaults to `false`.

* __`file_check = non_neg_integer()`{: id=file_check }__ - When `logger_std_h` logs to a file, it reads the file information of the log file prior to each write operation. This is to make sure the file still exists and has the same inode as when it was opened. This implies some performance loss, but ensures that no log events are lost in the case when the file has been removed or renamed by an external actor.
...
* __`filesync_repeat_interval = pos_integer() | no_repeat`__ - This value, in milliseconds, specifies how often the handler does a file sync operation to write buffered data to disk. The handler attempts the operation repeatedly, but only performs a new sync if something has actually been logged.
...
Other configuration parameters exist, to be used for customizing the overload protection behaviour. The same parameters are used both in the standard handler and the disk_log handler, and are documented in the [`User's Guide` ](logger_chapter.md#overload_protection).
...
To set the default handler, that starts initially with the Kernel application, to log to file instead of [`standard_io`](`t:io:standard_io/0`), change the Kernel default logger configuration. Example:
...
An example of how to replace the standard handler with a disk_log handler at startup is found in the `m:logger_disk_log_h` manual.

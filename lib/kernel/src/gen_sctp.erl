...
This module provides functions for communicating with sockets using the SCTP protocol. The implementation assumes that the OS kernel supports SCTP [(RFC 2960)](http://www.rfc-archive.org/getrfc.php?rfc=2960) through the user-level [Sockets API Extensions](http://tools.ietf.org/html/draft-ietf-tsvwg-sctpsocket-13).
...
The set of admissible SCTP socket options is by construction orthogonal to the sets of TCP, UDP, and generic `inet` options. Only options listed here are allowed for SCTP sockets. Options can be set on the socket using [`open/1,2`](`open/1`) or `inet:setopts/2`, retrieved using `inet:getopts/2`. Options can be changed when calling [`connect/4,5`](`connect/4`).

[](){: id=option-binary }
[](){: id=option-list }
* __`{mode, list|binary}` or just `list` or `binary`__ - Determines the type of data returned from [`recv/1,2`](`recv/1`).

  [](){: id=option-active }

* __`{active, true|false|once|N}`__ - * If `false` (passive mode, the default), the caller must do an explicit [`recv`](`recv/1`) call to retrieve the available data from the socket.
  * If `true|once|N` (active modes) received data or events are sent to the owning process. See [`open/0..2`](`open/0`) for the message format.
...
* __`{tos, integer()}`__ - Sets the Type-Of-Service field on the IP datagrams that are sent, to the specified value. This effectively determines a prioritization policy for the outbound packets. The acceptable values are system-dependent.

* __`{priority, integer()}`__ - A protocol-independent equivalent of `tos` above. Setting priority implies setting `tos` as well.

* __`{dontroute, true|false}`__ - Defaults to `false`. If `true`, the kernel does not send packets through any gateway, only sends them to directly connected hosts.

* __`{reuseaddr, true|false}`__ - Defaults to `false`. If true, the local binding address `{IP,Port}` of the socket can be reused immediately. No waiting in state `CLOSE_WAIT` is performed (can be required for high-throughput servers).

* __`{sndbuf, integer()}`__ - The size, in bytes, of the OS kernel send buffer for this socket. Sending errors would occur for datagrams larger than `val(sndbuf)`. Setting this option also adjusts the size of the driver buffer (see `buffer` above).

* __`{recbuf, integer()}`__ - The size, in bytes, of the OS kernel receive buffer for this socket. Sending errors would occur for datagrams larger than `val(recbuf)`. Setting this option also adjusts the size of the driver buffer (see `buffer` above).

* __`{sctp_module, module()}`__ - Overrides which callback module is used. Defaults to `inet_sctp` for IPv4 and `inet6_sctp` for IPv6.

* __`{sctp_rtoinfo, #sctp_rtoinfo{}}`__  
  ```text
  #sctp_rtoinfo{
        assoc_id = assoc_id(),
        initial  = integer(),
  ...
  `assoc_id = 0` (default) indicates the whole endpoint. See [RFC 2960](http://www.rfc-archive.org/getrfc.php?rfc=2960) and [Sockets API Extensions for SCTP](http://tools.ietf.org/html/draft-ietf-tsvwg-sctpsocket-13) for the exact semantics of the field values.

* __`{sctp_associnfo, #sctp_assocparams{}}`__  
  ```text
  #sctp_assocparams{
        assoc_id                 = assoc_id(),
        asocmaxrxt               = integer(),
  ...
  `assoc_id = 0` (default) indicates the whole endpoint. See [Sockets API Extensions for SCTP](http://tools.ietf.org/html/draft-ietf-tsvwg-sctpsocket-13) for the discussion of their semantics. Rarely used.

* __`{sctp_initmsg, #sctp_initmsg{}}`__  
  ```text
  #sctp_initmsg{
  ...
  Determines the default parameters that this socket tries to negotiate with its peer while establishing an association with it. Is to be set after [`open/*`](`open/1`) but before the first [`connect/*`](`connect/4`). `#sctp_initmsg{}` can also be used as ancillary data with the first call of [`send/*`](`send/3`) to a new peer (when a new association is created).

  * __`num_ostreams`__ - Number of outbound streams

  * __`max_instreams`__ - Maximum number of inbound streams

  * __`max_attempts`__ - Maximum retransmissions while establishing an association

  * __`max_init_timeo`__ - Time-out, in milliseconds, for establishing an association

  

* __`{sctp_autoclose, integer() >= 0}`__ - Determines the time, in seconds, after which an idle association is automatically closed. `0` means that the association is never automatically closed.

* __`{sctp_nodelay, true|false}`__ - Turns on|off the Nagle algorithm for merging small packets into larger ones. This improves throughput at the expense of latency.

* __`{sctp_disable_fragments, true|false}`__ - If `true`, induces an error on an attempt to send a message larger than the current PMTU size (which would require fragmentation/reassembling). Notice that message fragmentation does not affect the logical atomicity of its delivery; this option is provided for performance reasons only.

* __`{sctp_i_want_mapped_v4_addr, true|false}`__ - Turns on|off automatic mapping of IPv4 addresses into IPv6 ones (if the socket address family is `AF_INET6`).

* __`{sctp_maxseg, integer()}`__ - Determines the maximum chunk size if message fragmentation is used. If `0`, the chunk size is limited by the Path MTU only.

* __`{sctp_primary_addr, #sctp_prim{}}`__  
  ```text
  #sctp_prim{
        assoc_id = assoc_id(),
        addr     = {IP, Port}
...
* __`{sctp_set_peer_primary_addr, #sctp_setpeerprim{}}`__  
  ```text
  #sctp_setpeerprim{
        assoc_id = assoc_id(),
        addr     = {IP, Port}
  ...
  [](){: id=option-sctp_adaptation_layer }

* __`{sctp_adaptation_layer, #sctp_setadaptation{}}`__  
  ```text
  #sctp_setadaptation{
        adaptation_ind = integer()
  }
  ```
  {: id=record-sctp_setadaptation }

  When set, requests that the local endpoint uses the value specified by `adaptation_ind` as the Adaptation Indication parameter for establishing new associations. For details, see [RFC 2960](http://www.rfc-archive.org/getrfc.php?rfc=2960) and [Sockets API Extensions for SCTP](http://tools.ietf.org/html/draft-ietf-tsvwg-sctpsocket-13).

* __`{sctp_peer_addr_params, #sctp_paddrparams{}}`__  
  ```text
  #sctp_paddrparams{
        assoc_id   = assoc_id(),
        address    = {IP, Port},
  ...
  * __`hbinterval`__ - Heartbeat interval, in milliseconds

  * __`pathmaxrxt`__ - Maximum number of retransmissions before this address is considered unreachable (and an alternative address is selected)

  * __`pathmtu`__ - Fixed Path MTU, if automatic discovery is disabled (see `flags` below)

  * __`sackdelay`__ - Delay, in milliseconds, for SAC messages (if the delay is enabled, see `flags` below)

  * __`flags`__ - The following flags are available:

    * __`hb_enable`__ - Enables heartbeat

    * __`hb_disable`__ - Disables heartbeat

    * __`hb_demand`__ - Initiates heartbeat immediately

    * __`pmtud_enable`__ - Enables automatic Path MTU discovery

    * __`pmtud_disable`__ - Disables automatic Path MTU discovery

    * __`sackdelay_enable`__ - Enables SAC delay

    * __`sackdelay_disable`__ - Disables SAC delay

    

  

* __`{sctp_default_send_param, #sctp_sndrcvinfo{}}`__  
  ```text
  #sctp_sndrcvinfo{
        stream     = integer(),
        ssn        = integer(),
  ...
  ```
  {: id=record-sctp_sndrcvinfo }

  `#sctp_sndrcvinfo{}` is used both in this socket option, and as ancillary data while sending or receiving SCTP messages. When set as an option, it provides default values for subsequent [`send`](`send/3`) calls on the association specified by `assoc_id`.

  `assoc_id = 0` (default) indicates the whole endpoint.

  The following fields typically must be specified by the sender:

  * __`sinfo_stream`__ - Stream number (0-base) within the association to send the messages through;

  * __`sinfo_flags`__ - The following flags are recognised:

    * __`unordered`__ - The message is to be sent unordered

    * __`addr_over`__ - The address specified in [`send`](`send/3`) overwrites the primary peer address

    * __`abort`__ - Aborts the current association without flushing any unsent data

    * __`eof`__ - Gracefully shuts down the current association, with flushing of unsent data

    Other fields are rarely used. For complete information, see [RFC 2960](http://www.rfc-archive.org/getrfc.php?rfc=2960) and [Sockets API Extensions for SCTP](http://tools.ietf.org/html/draft-ietf-tsvwg-sctpsocket-13).

  [](){: id=option-sctp_events }

* __`{sctp_events, #sctp_event_subscribe{}}`__  
  ```text
  ...
  ```
  {: id=record-sctp_event_subscribe }

  This option determines which [SCTP Events](`m:gen_sctp#sctp_events`) are to be received (through [`recv/*`](`recv/1`)) along with the data. The only exception is `data_io_event`, which enables or disables receiving of [`#sctp_sndrcvinfo{}`](`m:gen_sctp#record-sctp_sndrcvinfo`) ancillary data, not events. By default, all flags except `adaptation_layer_event` are enabled, although `sctp_data_io_event` and `association_event` are used by the driver itself and not exported to the user level.

* __`{sctp_delayed_ack_time, #sctp_assoc_value{}}`__  
  ```text
  #sctp_assoc_value{
        assoc_id    = assoc_id(),
        assoc_value = integer()
...
* __`{sctp_status, #sctp_status{}}`__  
  ...
  * __`sctp_state_empty`__ - Default. Means that no other state is active.

  * __`sctp_state_closed`__  

  * __`sctp_state_cookie_wait`__  

  * __`sctp_state_cookie_echoed`__  

  * __`sctp_state_established`__  

  * __`sctp_state_shutdown_pending`__  

  * __`sctp_state_shutdown_sent`__  

  * __`sctp_state_shutdown_received`__  

  * __`sctp_state_shutdown_ack_sent`__  

  Semantics of the other fields:

  * __`sstat_rwnd`__ - Current receiver window size of the association

  * __`sstat_unackdata`__ - Number of unacked data chunks

  * __`sstat_penddata`__ - Number of data chunks pending receipt

  * __`sstat_instrms`__ - Number of inbound streams

  * __`sstat_outstrms`__ - Number of outbound streams

  * __`sstat_fragmentation_point`__ - Message size at which SCTP fragmentation occurs

  * __`sstat_primary`__ - Information on the current primary peer address (see below for the format of `#sctp_paddrinfo{}`)

  [](){: id=option-sctp_get_peer_addr_info }

* __`{sctp_get_peer_addr_info, #sctp_paddrinfo{}}`__  
  ```text
  #sctp_paddrinfo{
        assoc_id  = assoc_id(),
        address   = {IP, Port},
...
`m:gen_tcp`, `m:gen_udp`, `m:inet`, [RFC 2960](http://www.rfc-archive.org/getrfc.php?rfc=2960) (Stream Control Transmission Protocol), [Sockets API Extensions for SCTP](http://tools.ietf.org/html/draft-ietf-tsvwg-sctpsocket-13)
""".
...
-doc "An option name or one of the [SCTP Socket Options](`m:gen_sctp#options`) used to get an option.".
-doc(#{title => <<"Exported data types">>}).
-type option_name() ::
...
-doc "One of the [SCTP Socket Options](`m:gen_sctp#options`) as returned when getting an option.".
-doc(#{title => <<"Exported data types">>}).
-type option_value() ::
        elementary_option() |
        record_option() |
...
* __`inet6`__ - Sets up the socket for IPv6.

* __`inet`__ - Sets up the socket for IPv4. This is the default.

A default set of socket [options](`m:gen_sctp#options`) is used. In particular, the socket is opened in [binary](`m:gen_sctp#option-binary`) and [passive](`m:gen_sctp#option-active`) mode, with SockType `seqpacket`, and with reasonably large [kernel](`m:inet#option-sndbuf`) and driver [buffers](`m:inet#option-buffer`).

If the socket is in [passive](`m:gen_sctp#option-active`) mode data can be received through the [`recv/1,2`](`recv/1`) calls.

If the socket is in [active](`m:gen_sctp#option-active`) mode data received data is delivered to the controlling process as messages:
...
> #### Note {: class=info }
> This message format unfortunately differs slightly from the [`gen_udp`](`gen_udp:open/1`) message format with ancillary data, and from the [`recv/1,2`](`recv/1`) return tuple format.
""".
...
-doc "Closes the socket and all associations on it. The unsent data is flushed as in `eof/2`. The `close/1` call is blocking or otherwise depending of the value of the [`linger`](`m:inet#option-linger`) socket [option](`m:gen_sctp#options`). If `close` does not linger or linger time-out expires, the call returns and the data is flushed in the background.".
...
The existing association argument `Assoc` can be either a [`#sctp_assoc_change{}`](`m:gen_sctp#record-sctp_assoc_change`) record as returned from, for example, [`recv/*`](`recv/2`), [`connect/*`](`connect/5`), or from a listening socket in active mode. It can also be just the field `assoc_id` integer from such a record.
""".
-doc(#{since => <<"OTP R15B">>}).
...
-doc """
[](){: id=connect-sockaddr4 }
This is conceptually the same as [`connect/5`](`m:gen_sctp#connect-addr-port5`), only with the difference that we use a socket address, [`socket:sockaddr_in()`](`t:socket:sockaddr_in/0`) or [`socket:sockaddr_in6()`](`t:socket:sockaddr_in6/0`) instead of an address (inet:ip_address() or inet:hostname()) and port-number.

[](){: id=connect-addr-port4 }
...
> Using a value of `Timeout` less than the maximum time taken by the OS to establish an association (around 4.5 minutes if the default values from [RFC 4960](https://tools.ietf.org/html/rfc4960) are used), can result in inconsistent or incorrect return values. This is especially relevant for associations sharing the same `Socket` (that is, source address and port), as the controlling process blocks until `connect/*` returns. [`connect_init/*`](`connect_init/4`) provides an alternative without this limitation.

[](){: id=record-sctp_assoc_change }
The result of `connect/*` is an `#sctp_assoc_change{}` event that contains, in particular, the new [Association ID](`t:assoc_id/0`):
...
* __`comm_up`__ - Association is successfully established. This indicates a successful completion of `connect`.

* __`cant_assoc`__ - The association cannot be established (`connect/*` failure).

Other states do not normally occur in the output from `connect/*`. Rather, they can occur in `#sctp_assoc_change{}` events received instead of data in [`recv/*`](`recv/1`) calls. All of them indicate losing the association because of various error conditions, and are listed here for the sake of completeness:
...
-doc """
[](){: id=connect_init-sockaddr4 }
This is conceptually the same as [`connect_init/5`](`m:gen_sctp#connect_init-addr-port5`), only with the difference that we use a socket address, [`socket:sockaddr_in()`](`t:socket:sockaddr_in/0`) or [`socket:sockaddr_in6()`](`t:socket:sockaddr_in6/0`) instead of an address (inet:ip_address() or inet:hostname()) and port-number.

[](){: id=connect_init-addr-port4 }
...
-doc """
[](){: id=connect_init-addr-port5 }
Initiates a new association for socket `Socket`, with the peer (SCTP server socket) specified by `Addr` and `Port`.

The fundamental difference between this API and `connect/*` is that the return value is that of the underlying OS `connect(2)` system call. If `ok` is returned, the result of the association establishment is received by the calling process as an [`#sctp_assoc_change{}`](`m:gen_sctp#record-sctp_assoc_change`) event. The calling process must be prepared to receive this, or poll for it using [`recv/*`](`recv/1`), depending on the value of the active option.

The parameters are as described in [`connect/*`](`connect/5`), except the `Timeout` value.
...
-doc "Same as `connectx_init(Socket, Addrs, Port, Opts, infinity)`.".
-doc(#{since => <<"OTP 25.0">>}).
...
This API is similar to `connect_init/*` except the underlying OS `m:sctp_connectx` system call is used.

If successful, the association ID is returned which will be received in a subsequent [`#sctp_assoc_change{}`](`m:gen_sctp#record-sctp_assoc_change`) event.
...
-doc "Gracefully terminates the association specified by `Assoc`, with flushing of all unsent data. The socket itself remains open. Other associations opened on this socket are still valid. The socket can be used in new associations.".
...
-doc "Abnormally terminates the association specified by `Assoc`, without flushing of unsent data. The socket itself remains open. Other associations opened on this socket are still valid, and the socket can be used in new associations.".
...
`AncData` is a list of ancillary data items that can be received along with the main `Data`. This list can be empty, or contain a single [`#sctp_sndrcvinfo{}`](`m:gen_sctp#record-sctp_sndrcvinfo`) record if receiving of such ancillary data is enabled (see option [`sctp_events`](`m:gen_sctp#option-sctp_events`)). It is enabled by default, as such ancillary data provides an easy way of determining the association and stream over which the message is received. (An alternative way is to get the association ID from `FromIP` and `FromPort` using socket option [`sctp_get_peer_addr_info`](`m:gen_sctp#option-sctp_get_peer_addr_info`), but this does still not produce the stream number).

`AncData` may also contain [ancillary data ](`t:inet:ancillary_data/0`)from the socket [options](`m:gen_sctp#options`) [`recvtos`](`m:inet#option-recvtos`), [`recvtclass`](`m:inet#option-recvtclass`) or [`recvttl`](`m:inet#option-recvttl`), if that is supported by the platform for the socket.

The `Data` received can be a `t:binary()` or a `t:list()` of bytes (integers in the range 0 through 255) depending on the socket mode, or an SCTP event.
...
* ```text
  ...
  * __`addr_unreachable`__  

  * __`addr_available`__  

  * __`addr_removed`__  

  * __`addr_added`__  

  * __`addr_made_prim`__  

  * __`addr_confirmed`__  

  In case of an error (for example, `addr_unreachable`), field `error` provides more diagnostics. In such cases, event `#sctp_paddr_change{}` is automatically converted into an `error` term returned by [`recv`](`recv/1`). The `error` field value can be converted into a string using `error_string/1`.
* ```text
  #sctp_send_failed{
        flags     = true | false,
        error     = integer(),
  ...
  * __`flags`__ - A Boolean specifying if the data has been transmitted over the wire.

  * __`error`__ - Provides extended diagnostics, use [`error_string/1`.](`error_string/1`)

  * __`info`__ - The original [`#sctp_sndrcvinfo{}`](`m:gen_sctp#record-sctp_sndrcvinfo`) record used in the failed [`send/*`.](`send/3`)

  * __`data`__ - The whole original data chunk attempted to be sent.

  In the current implementation of the Erlang/SCTP binding, this event is internally converted into an `error` term returned by [`recv/*`](`recv/1`).
* ```text
  #sctp_adaptation_event{
        adaptation_ind = integer(),
        assoc_id       = assoc_id()
  ...
  Delivered when a peer sends an adaptation layer indication parameter (configured through option [`sctp_adaptation_layer`](`m:gen_sctp#option-sctp_adaptation_layer`)). Notice that with the current implementation of the Erlang/SCTP binding, this event is disabled by default.
* ```text
  #sctp_pdapi_event{
  ...
  A partial delivery failure. In the current implementation of the Erlang/SCTP binding, this event is internally converted into an `error` term returned by [`recv/*`](`recv/1`).
""".
...
-doc "Translates an SCTP error number from, for example, `#sctp_remote_error{}` or `#sctp_send_failed{}` into an explanatory string, or one of the atoms `ok` for no error or `undefined` for an unrecognized error.".
...
-doc "Assigns a new controlling process `Pid` to `Socket`. Same implementation as `gen_udp:controlling_process/2`.".

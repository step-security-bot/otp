...
>
> The default is `Backend = inet` that selects the traditional `inet_drv.c` driver. The other choice is `Backend = socket` that selects the new `m:socket` module and its NIF implementation.
>
...
>
>   If a user calling [`gen_tcp:send/2`](`send/2`) with `inet_backend = inet`, tries to send more data than there is room for in the OS buffers, the "rest data" is buffered by the inet driver (and later sent in the background). The effect for the user is that the call is non-blocking.
>
...
>
>   An background send will detect a 'remote close' and (the inet driver will) mark the socket as 'closed'. No other action is taken. If the socket has `active` set to `false` (passive) at this point and no one is reading, this will not be noticed. But as soon as the socket is "activated" (`active` set to not `false`, `send/2` is called or [recv/2,3](`recv/2`) is called), an error message will be sent to the caller or (socket) owner: `{tcp_error, Socket, econnreset}`. Any data in the OS receive buffers will be lost\!
>
>   This behaviour is *not* replicated by the socket implementation. A send operation will detect a remote close and immediately return this to the caller, but do nothing else. A reader will therefore be able to extract any data from the OS buffers. If the socket is set to `active` not `false`, the data will be received as expected (`{tcp, ...}` and then a closed message (`{tcp_closed, ...}` will be received (not an error).
> * The option [show_econnreset](`m:inet#option-show_econnreset`) basically do *not* work as described when used with `inet_backend = socket`. The "issue" is that a remote close (as described above) *do* allow a reader to extract what is in the read buffers before a close is "delivered".
> * The option [nodelay](`m:inet#option-nodelay`) is a TCP specific option that is *not* compatible with `domain = local`.
>
...
>
>   Calling [gen_tcp:shutdown(Socket, write | read_write)](`shutdown/2`) on a socket created with `inet_backend = socket` will take *immediate* effect, unlike for a socket created with `inet_backend = inet`.
>
...
-type option() ::
	...
	{ipv6_v6only,     boolean()}.
-doc """
If the platform implements the IPv4 option `IP_PKTOPTIONS`, or the IPv6 option `IPV6_PKTOPTIONS` or `IPV6_2292PKTOPTIONS` for the socket this value is returned from `inet:getopts/2` when called with the option name [`pktoptions`](`t:option_name/0`).
...
-doc """
[](){: id=connect-sockaddr2 }
[](){: id=connect-sockaddr3 }
Connects to a server according to `SockAddr`. This is primarily intended for link local IPv6 addresses (which require the scope-id), [`socket:sockaddr_in6()`](`t:socket:sockaddr_in6/0`). But for completeness, we also support IPv4, [`socket:sockaddr_in()`](`t:socket:sockaddr_in/0`).

The [`options`](`m:gen_tcp#connect-options`) available are the same as for [`connect/3,4`](`m:gen_tcp#connect-port3`).
...
> #### Note {: class=info }
> The default values for options specified to `connect` can be affected by the Kernel configuration parameter `inet_default_connect_options`. For details, see `m:inet`.
""".
-doc(#{since => <<"OTP 24.3">>}).
-doc(#{equiv => connect/4}).
...
* __`{ip, Address}`__ - If the host has many network interfaces, this option specifies which one to use.

* __`{ifaddr, Address}`__ - Same as `{ip, Address}`. If the host has many network interfaces, this option specifies which one to use.

  However, if this instead is an [`socket:sockaddr_in()`](`t:socket:sockaddr_in/0`) or [`socket:sockaddr_in6()`](`t:socket:sockaddr_in6/0`) this takes precedence over any value previously set with the `ip` and `port` options. If these options (`ip` or/and `port`) however comes *after* this option, they may be used to *update* their corresponding fields of this options (for `ip`, the `addr` field, and for `port`, the `port` field).

* __`{fd, integer() >= 0}`__ - If a socket has somehow been connected without using `gen_tcp`, use this option to pass the file descriptor for it. If `{ip, Address}` and/or `{port, port_number()}` is combined with this option, the `fd` is bound to the specified interface and port before connecting. If these options are not specified, it is assumed that the `fd` is already bound appropriately.

* __`inet`__ - Sets up the socket for IPv4.

* __`inet6`__ - Sets up the socket for IPv6.

* __`local`__ - Sets up a Unix Domain Socket. See [`inet:local_address()`](`t:inet:local_address/0`)

* __`{port, Port}`__ - Specifies which local port number to use.

* __`{tcp_module, module()}`__ - Overrides which callback module is used. Defaults to `inet_tcp` for IPv4 and `inet6_tcp` for IPv6.

* __`Opt`__ - See `inet:setopts/2`.
...
* __`{backlog, B}`__ - `B` is an integer >= `0`. The backlog value defines the maximum length that the queue of pending connections can grow to. Defaults to `5`.

* __`inet6`__ - Sets up the socket for IPv6.

* __`inet`__ - Sets up the socket for IPv4.

* __`{fd, Fd}`__ - If a socket has somehow been connected without using `gen_tcp`, use this option to pass the file descriptor for it.

* __`{ip, Address}`__ - If the host has many network interfaces, this option specifies which one to listen on.
...
The returned socket `ListenSocket` should be used in calls to [`accept/1,2`](`accept/1`) to accept incoming connection requests.

> #### Note {: class=info }
> The default values for options specified to `listen` can be affected by the Kernel configuration parameter `inet_default_listen_options`. For details, see `m:inet`.
""".
...
* A POSIX error value if something else goes wrong, see `m:inet` for possible error values
...
1. Use [`gen_tcp:shutdown(Sock, write)`](`shutdown/2`) to signal that no more data is to be sent and wait for the read side of the socket to be closed.
1. Use the socket option [`{packet, N}`](`m:inet#packet`) (or something similar) to make it possible for the receiver to close the connection when it knowns it has received all the data.
""".
...
There is no `send` call with a time-out option, use socket option `send_timeout` if time-outs are desired. See section [Examples](`m:gen_tcp#examples`).

The return value `{error, {timeout, RestData}}` can only be returned when `inet_backend = socket`.

[](){: id=non_blocking_send }
> #### Note {: class=info }
> Non-blocking send.
>
...
-doc """
Receives a packet from a socket in *passive* mode. A closed socket is indicated by return value `{error, closed}`. If the socket is not in passive mode, the return value is `{error, einval}`.

...
-doc "All characters needs to be valid characters on the specific OS using [`file:native_name_encoding()`](`file:native_name_encoding/0`) encoding. Null characters (integer value zero) are not allowed.".
-type os_command() :: atom() | io_lib:chars().
-doc """
Options for [`os:cmd/2`](`cmd/2`)

* __`max_size`__ - The maximum size of the data returned by the `os:cmd/2` call. See the [`os:cmd/2`](`cmd/2`) documentation for more details.
""".
-type os_command_opts() :: #{ max_size => non_neg_integer() | infinity }.

-export_type([os_command/0, os_command_opts/0]).

-doc "A string containing valid characters on the specific OS for environment variable names using [`file:native_name_encoding()`](`file:native_name_encoding/0`) encoding. Null characters (integer value zero) are not allowed. On Unix, `=` characters are not allowed. On Windows, a `=` character is only allowed as the very first character in the string.".
-type env_var_name() :: nonempty_string().

-doc "A string containing valid characters on the specific OS for environment variable values using [`file:native_name_encoding()`](`file:native_name_encoding/0`) encoding. Null characters (integer value zero) are not allowed.".
-type env_var_value() :: string().

-doc "Assuming that environment variables has been correctly set, a strings containing valid characters on the specific OS for environment variable names and values using [`file:native_name_encoding()`](`file:native_name_encoding/0`) encoding. The first `=` characters appearing in the string separates environment variable name (on the left) from environment variable value (on the right).".
...
If Unicode filename encoding is in effect (see the [`erl` manual page](`p:erts:erl_cmd.md#file_name_encoding`)), the strings can contain characters with codepoints > 255.
""".
-doc(#{since => <<"OTP 24.0">>}).
...
If Unicode filename encoding is in effect (see the [`erl` manual page](`p:erts:erl_cmd.md#file_name_encoding`)), the strings `VarName` and `Value` can contain characters with codepoints > 255.
""".
...
-doc "Returns the current performance counter value in `perf_counter` [time unit](`m:erlang#type_time_unit`). This is a highly optimized call that might not be traceable.".
-doc(#{since => <<"OTP 19.0">>}).
...
-doc """
Returns the current [OS system time](`p:erts:time_correction.md#os_system_time`) in `native` [time unit](`m:erlang#type_time_unit`).
...
-doc """
Returns the current [OS system time](`p:erts:time_correction.md#os_system_time`) converted into the `Unit` passed as argument.

Calling `os:system_time(Unit)` is equivalent to [`erlang:convert_time_unit`](`erlang:convert_time_unit/3`)([`os:system_time()`](`system_time/0`)`, native, Unit)`.
...
If Unicode filename encoding is in effect (see the [`erl` manual page](`p:erts:erl_cmd.md#file_name_encoding`)), the string `VarName` can contain characters with codepoints > 255.
""".
...
* __`ignore`__ - This signal will be ignored.

* __`default`__ - This signal will use the default signal handler for the operating system.

* __`handle`__ - This signal will notify [`erl_signal_server`](kernel_app.md#erl_signal_server) when it is received by the Erlang runtime system.
""".
-doc(#{since => <<"OTP 20.0">>}).
...
> #### Note {: class=info }
> Think twice before using this function. Use module `m:filename` if you want to inspect or build filenames in a portable way. Avoid matching on atom `Osname`.
""".
...
* __`max_size`__ - The maximum size of the data returned by the `os:cmd` call. This option is a safety feature that should be used when the command executed can return a very large, possibly infinite, result.

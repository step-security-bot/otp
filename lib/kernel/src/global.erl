...
[](){: id=prevent_overlapping_partitions }
...
> #### Note {: class=info }
> None of the above services will be reliably delivered unless both of the kernel parameters [`connect_all`](kernel_app.md#connect_all) and [`prevent_overlapping_partitions`](kernel_app.md#prevent_overlapping_partitions) are enabled. Calls to the `global` API will, however, *not* fail even though one or both of them are disabled. You will just get unreliable results.
...
The global name server also performs the critical task of continuously monitoring changes in node configuration. If a node that runs a globally registered process goes down, the name is globally unregistered. To this end, the global name server subscribes to `nodeup` and `nodedown` messages sent from module `net_kernel`. Relevant Kernel application variables in this context are `net_setuptime`, `net_ticktime`, and `dist_auto_connect`. See also [`kernel(6)`](kernel_app.md#net_setuptime).
...
%% See OTP-3737.
-doc "Returns the pid with the globally registered name `Name`. Returns `undefined` if the name is not globally registered.".
...
Function `Resolve` has the same behavior as in [`register_name/2,3`](`register_name/2`).
""".
...
The disconnects will be made in such a way that only the current node will be removed from the cluster of `global` nodes. If [`prevent_overlapping_partitions`](`m:global#prevent_overlapping_partitions`) is enabled and you disconnect, from other nodes in the cluster of `global` nodes, by other means, `global` on the other nodes may partition the remaining nodes in order to ensure that no overlapping partitions appear. Even if `prevent_overlapping_partitions` is disabled, you should preferably use `global:disconnect/0` in order to remove current node from a cluster of `global` nodes, since you otherwise likely *will* create overlapping partitions which might [cause problems](`m:global#prevent_overlapping_partitions`).
...
If current node has been configured to be part of a [*global group*](`m:global_group`), only connected and/or synchronized nodes in that group are known to `global`, so `global:disconnect/0` will *only* disconnect from those nodes. If current node is *not* part of a *global group*, all [connected visible nodes](`erlang:nodes/0`) will be known to `global`, so `global:disconnect/0` will disconnect from all those nodes.

Note that information about connected nodes does not instantaneously reach `global`, so the caller might see a node part of the result returned by [`nodes()`](`erlang:nodes/0`) while it still is not known to `global`. The disconnect operation will, however, still not cause any overlapping partitions when `prevent_overlapping_partitions` is enabled. If `prevent_overlapping_partitions` is disabled, overlapping partitions might form in this case.

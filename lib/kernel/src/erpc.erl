...
> #### Note {: class=info }
> For some important information about distributed signals, see the [Blocking Signaling Over Distribution](`p:system:ref_man_processes.md#blocking-signaling-over-distribution`) section in the *Processes* chapter of the *Erlang Reference Manual*. Blocking signaling can, for example, cause timeouts in `erpc` to be significantly delayed.
""".
...
-doc """
The same as calling [`erpc:call(Node, erlang, apply, [Fun,[]], Timeout)`](`call/5`). May raise all the same exceptions as `call/5` plus an `{erpc, badarg}` `error` exception if `Fun` is not a fun of zero arity.
...
* __`throw`__ - The applied function called `throw(Value)` and did not catch this exception. The exception reason `Value` equals the argument passed to `throw/1`.

* __`exit`__ - Exception reason:

  * __`{exception, ExitReason}`__ - The applied function called `exit(ExitReason)` and did not catch this exception. The exit reason `ExitReason` equals the argument passed to `exit/1`.

  * __`{signal, ExitReason}`__ - The process that applied the function received an exit signal and terminated due to this signal. The process terminated with exit reason `ExitReason`.

  

* __`error`__ - Exception reason:

  * __`{exception, ErrorReason, StackTrace}`__ - A runtime error occurred which raised an error exception while applying the function, and the applied function did not catch the exception. The error reason `ErrorReason` indicates the type of error that occurred. `StackTrace` is formatted as when caught in a `try/catch` construct. The `StackTrace` is limited to the applied function and functions called by it.

  * __`{erpc, ERpcErrorReason}`__ - The `erpc` operation failed. The following `ERpcErrorReason`s are the most common ones:

    * __`badarg`__ - If any one of these are true:
    ...
    * __`noconnection`__ - The connection to `Node` was lost or could not be established. The function may or may not be applied.

    * __`system_limit`__ - The `erpc` operation failed due to some system limit being reached. This typically due to failure to create a process on the remote node `Node`, but can be other things as well.

    * __`timeout`__ - The `erpc` operation timed out. The function may or may not be applied.

    * __`notsup`__ - The remote node `Node` does not support this `erpc` operation.
...
-doc """
The same as calling [`erpc:send_request(Node, erlang, apply, [Fun, []])`](`send_request/4`).
...
The same as calling [`erpc:send_request(Node, erlang, apply, [Fun,[]]), Label, RequestIdCollection)`](`send_request/6`).
...
The same as calling [`erpc:reqids_add`](`reqids_add/3`)([`erpc:send_request`](`send_request/4`)`(Node, Module, Function, Args), Label, RequestIdCollection)`, but calling `send_request/6` is slightly more efficient.
...
-doc "The same as calling [`erpc:receive_response(RequestId, infinity)`](`receive_response/2`).".
-doc(#{since => <<"OTP 23.0">>}).
...
-doc """
Receive a response to a `call` request previously made by the calling process using `send_request/4`. `RequestId` should be the value returned from the previously made `send_request/4` call, and the corresponding response should not already have been received and handled to completion by `receive_response()`, [`check_response/4`](`check_response/2`), or [`wait_response/4`](`wait_response/2`).
...
`Label` is the label associated with the request identifier of the request that the response corresponds to. A request identifier is associated with a label when [adding a request identifier](`reqids_add/3`) in a [request identifier collection](`t:request_id_collection/0`), or when sending the request using `send_request/6`.
...
-doc "The same as calling [`erpc:wait_response(RequestId, 0)`](`wait_response/2`). That is, poll for a response message to a `call` request previously made by the calling process.".
-doc(#{since => <<"OTP 23.0">>}).
...
-doc "Saves `RequestId` and associates a `Label` with the request identifier by adding this information to `RequestIdCollection` and returning the resulting request identifier collection.".
-doc(#{since => <<"OTP 25.0">>}).
...
-doc "Returns a list of `{RequestId, Label}` tuples which corresponds to all request identifiers with their associated labels present in the `RequestIdCollection` collection.".
-doc(#{since => <<"OTP 25.0">>}).
...
-doc """
The same as calling [`erpc:multicall(Nodes, erlang, apply, [Fun,[]], Timeout)`](`multicall/5`). May raise all the same exceptions as `multicall/5` plus an `{erpc, badarg}` `error` exception if `Fun` is not a fun of zero arity.
...
* __`{ok, Result}`__ - The `call` operation for this specific node returned `Result`.

* __`{Class, ExceptionReason}`__ - The `call` operation for this specific node raised an exception of class `Class` with exception reason `ExceptionReason`. These correspond to the exceptions that `call/5` can raise.
...
-doc """
The same as calling [`erpc:multicast(Nodes,erlang,apply,[Fun,[]])`](`multicast/4`).

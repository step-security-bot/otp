...
It is also possible to call [`net_kernel:start(foobar, #{})`](`start/2`) directly from the normal Erlang shell prompt:
...
If the node is started with command-line flag `-sname`, the node name is `foobar@Host`, where `Host` is the short name of the host (not the fully qualified domain name). If started with flag `-name`, the node name is `foobar@Host`, where `Host` is the fully qualified domain name. For more information, see [`erl`](`p:erts:erl_cmd.md`).

Normally, connections are established automatically when another node is referenced. This functionality can be disabled by setting Kernel configuration parameter `dist_auto_connect` to `never`, see [`kernel(6)`](kernel_app.md). In this case, connections must be established explicitly by calling `connect_node/1`.

Which nodes that are allowed to communicate with each other is handled by the magic cookie system, see section [Distributed Erlang](`p:system:distributed.md`) in the Erlang Reference Manual.

> #### Warning {: class=warning }
> Starting a distributed node without also specifying [`-proto_dist inet_tls`](`p:erts:erl_cmd.md#proto_dist`) will expose the node to attacks that may give the attacker complete access to the node and in extension the cluster. When using un-secure distributed nodes, make sure that the network is configured to keep potential attackers out. See the [Using SSL for Erlang Distribution](`p:ssl:ssl_distribution.md`) User's Guide for details on how to setup a secure distributed node.
""".
...
* __`started => Started`__ - Valid values for `Started`:

  * __`no`__ - The distribution is not started. In this state none of the other keys below are present in the map.

  * __`static`__ - The distribution was started with command line arguments [`-name`](`p:erts:erl_cmd.md#name`) or [`-sname`](`p:erts:erl_cmd.md#sname`).

  * __`dynamic`__ - The distribution was started with [`net_kernel:start/1`](`start/1`) and can be stopped with [`net_kernel:stop/0`](`start/1`).

  

* __`name => Name`__ - The name of the node. Same as returned by `erlang:node/0` except when `name_type` is `dynamic` in which case `Name` may be `undefined` (instead of `nonode@nohost`).

* __`name_type => NameType`__ - Valid values for `NameType`:

  * __`static`__ - The node has a static node name set by the node itself.

  * __`dynamic`__ - The distribution was started in [dynamic node name](`p:system:distributed.md#dyn_node_name`) mode, and will get its node name assigned from the first node it connects to. If key `name` has value `undefined` that has not happened yet.

  

* __`name_domain => NameDomain`__ - Valid values for `NameDomain`:

  * __`shortnames`__ - The distribution was started to use node names with a short host portion (not fully qualified).

  * __`longnames`__ - The distribution was started to use node names with a long fully qualified host portion.
""".
-doc(#{since => <<"OTP 25.0">>}).
...
-doc "Turns a distributed node into a non-distributed node. For other nodes in the network, this is the same as the node going down. Only possible when the net kernel was started using `start/2`, otherwise `{error, not_allowed}` is returned. Returns `{error, not_found}` if the local node is not alive.".
...
-doc """
Sets `net_ticktime` (see [`kernel(6)`](kernel_app.md)) to `NetTicktime` seconds. `TransitionPeriod` defaults to `60`.

Some definitions:

* __Minimum transition traffic interval (`MTTI`)__ - `minimum(NetTicktime, PreviousNetTicktime)*1000 div 4` milliseconds.

* __Transition period__ - The time of the least number of consecutive `MTTI`s to cover `TransitionPeriod` seconds following the call to `set_net_ticktime/2` (that is, ((`TransitionPeriod*1000 - 1) div MTTI + 1)*MTTI` milliseconds).
...
* __`unchanged`__ - `net_ticktime` already has the value of `NetTicktime` and is left unchanged.

* __`change_initiated`__ - `net_kernel` initiated the change of `net_ticktime` to `NetTicktime` seconds.

* __`{ongoing_change_to, NewNetTicktime}`__ - The request is *ignored* because `net_kernel` is busy changing `net_ticktime` to `NewNetTicktime` seconds.
""".
...
-doc """
Returns currently used net tick time in seconds. For more information see the [`net_ticktime`](kernel_app.md#net_ticktime) `kernel(6)` parameter.

Defined return values (`Res`):

* __`NetTicktime`__ - `net_ticktime` is `NetTicktime` seconds.

* __`{ongoing_change_to, NetTicktime}`__ - `net_kernel` is currently changing `net_ticktime` to `NetTicktime` seconds.

* __`ignored`__ - The local node is not alive.
""".
...
* __`connection_id => boolean()`__ - If the value of the association equals `true`, a `connection_id => ConnectionId` association will be included in the `Info` map where `ConnectionId` is the connection identifier of the connection coming up or going down. For more info about this connection identifier see the documentation of [erlang:nodes/2](`m:erlang#connection_id`).

* __`node_type => NodeType`__ - Valid values for `NodeType`:

  * __`visible`__ - Subscribe to node status change messages for visible nodes only. The association `node_type => visible` will be included in the `Info` map.

  * __`hidden`__ - Subscribe to node status change messages for hidden nodes only. The association `node_type => hidden` will be included in the `Info` map.

  * __`all`__ - Subscribe to node status change messages for both visible and hidden nodes. The association `node_type => visible | hidden` will be included in the `Info` map.
...
* __`nodedown_reason => boolean()`__ - If the value of the association equals `true`, a `nodedown_reason => Reason` association will be included in the `Info` map for `nodedown` messages.
  ...
  * __`connection_setup_failed`__ - The connection setup failed (after `nodeup` messages were sent).

  * __`no_network`__ - No network is available.

  * __`net_kernel_terminated`__ - The `net_kernel` process terminated.

  * __`shutdown`__ - Unspecified connection shutdown.

  * __`connection_closed`__ - The connection was closed.

  * __`disconnect`__ - The connection was disconnected (forced from the current node).

  * __`net_tick_timeout`__ - Net tick time-out.

  * __`send_net_tick_failed`__ - Failed to send net tick over the connection.

  * __`get_status_failed`__ - Status information retrieval from the `Port` holding the connection failed.

  

When `Options` is a list, currently `ListOption` can be one of the following:

* __`connection_id`__ - A `{connection_id, ConnectionId}` tuple will be included in `Info` where `ConnectionId` is the connection identifier of the connection coming up or going down. For more info about this connection identifier see the documentation of [erlang:nodes/2](`m:erlang#connection_id`).

* __`{node_type, NodeType}`__ - Valid values for `NodeType`:

  * __`visible`__ - Subscribe to node status change messages for visible nodes only. The tuple `{node_type, visible}` will be included in the `Info` list.

  * __`hidden`__ - Subscribe to node status change messages for hidden nodes only. The tuple `{node_type, hidden}` will be included in the `Info` list.

  * __`all`__ - Subscribe to node status change messages for both visible and hidden nodes. The tuple `{node_type, visible | hidden}` will be included in the `Info` list.
...
* __`nodedown_reason`__ - The tuple `{nodedown_reason, Reason}` will be included in the `Info` list for `nodedown` messages.

  See the documentation of the [`nodedown_reason => boolean()`](`m:net_kernel#nodedown_reasons`) association above for information about possible `Reason` values.
...
-doc "Establishes a connection to `Node`. Returns `true` if a connection was established or was already established or if `Node` is the local node itself. Returns `false` if the connection attempt failed, and `ignored` if the local node is not alive.".
...
If `Name` is set to *`undefined`* the distribution will be started to request a dynamic node name from the first node it connects to. See [Dynamic Node Name](`p:system:distributed.md#dyn_node_name`). Setting `Name` to `undefined` implies options `dist_listen => false` and `hidden => true`.

Currently supported options:

* __`name_domain => NameDomain`__ - Determines the host name part of the node name. If `NameDomain` equals `longnames`, fully qualified domain names will be used which also is the default. If `NameDomain` equals `shortnames`, only the short name of the host will be used.

* __`net_ticktime => NetTickTime`__ - *Net tick time* to use in seconds. Defaults to the value of the [`net_ticktime`](kernel_app.md#net_ticktime) `kernel(6)` parameter. For more information about *net tick time*, see the `kernel` parameter. However, note that if the value of the `kernel` parameter is invalid, it will silently be replaced by a valid value, but if an invalid `NetTickTime` value is passed as option value to this function, the call will fail.

* __`net_tickintensity => NetTickIntensity`__ - *Net tick intensity* to use. Defaults to the value of the [`net_tickintensity`](kernel_app.md#net_tickintensity) `kernel(6)` parameter. For more information about *net tick intensity*, see the `kernel` parameter. However, note that if the value of the `kernel` parameter is invalid, it will silently be replaced by a valid value, but if an invalid `NetTickIntensity` value is passed as option value to this function, the call will fail.

* __`dist_listen => boolean()`__ - Enable or disable listening for incoming connections. Defaults to the value of the [`-dist_listen`](`p:erts:erl_cmd.md#dist_listen`) `erl` command line argument. Note that `dist_listen => false` implies `hidden => true`.

  If `undefined` has been passed as `Name`, the `dist_listen` option will be overridden with `dist_listen => false`.

* __`hidden => boolean()`__ - Enable or disable hidden node. Defaults to `true` if the [`-hidden`](`p:erts:erl_cmd.md#hidden`) `erl` command line argument has been passed; otherwise `false`.
...
* __`[Name]`__ - The same as `net_kernel:start([Name, longnames, 15000])`.

* __`[Name, NameDomain]`__ - The same as `net_kernel:start([Name, NameDomain, 15000])`.

* __`[Name, NameDomain, TickTime]`__ - The same as [`net_kernel:start(Name, #{name_domain => NameDomain, net_ticktime => ((TickTime*4-1) div 1000) + 1, net_tickintensity => 4})`](`start/2`). Note that `TickTime` is *not* the same as net tick time expressed in milliseconds. `TickTime` is the time between ticks when net tick intensity equals `4`.
""".
...
If `Node` is `new` the `Options` will then also be added to kernel configuration parameters [inet_dist_listen_options](kernel_app.md#inet_dist_listen_options) and [inet_dist_connect_options](kernel_app.md#inet_dist_connect_options).
...
If `Node` is a connected node the return value is the same as from [`inet:getopts(Sock, Options)`](`inet:getopts/2`) where `Sock` is the distribution socket for `Node`.

...
* __`{send, Serial, From, To, Message}`__ - Used when a process `From` with its trace token flag `send` set to `true` has sent information. `To` may be a process identifier, a registered name on a node represented as `{NameAtom, NodeAtom}`, or a node name represented as an atom. `From` may be a process identifier or a node name represented as an atom. `Message` contains the information passed along in this information transfer. If the transfer is done via message passing, it is the actual message.

* __`{'receive', Serial, From, To, Message}`__ - Used when a process `To` receives information with a trace token that has flag `'receive'` set to `true`. `To` may be a process identifier, or a node name represented as an atom. `From` may be a process identifier or a node name represented as an atom. `Message` contains the information passed along in this information transfer. If the transfer is done via message passing, it is the actual message.

* __`{print, Serial, From, _, Info}`__ - Used when a process `From` has called `seq_trace:print(Label, TraceInfo)` and has a trace token with flag `print` set to `true`, and `label` set to `Label`.
...
* __`set_token(label, Label)`__ - The `label` component is a term which identifies all events belonging to the same sequential trace. If several sequential traces can be active simultaneously, `label` is used to identify the separate traces. Default is 0.
...
* __`set_token(serial, SerialValue)`__ - `SerialValue = {Previous, Current}`. The `serial` component contains counters which enables the traced messages to be sorted, should never be set explicitly by the user as these counters are updated automatically. Default is `{0, 0}`.

* __`set_token(send, Bool)`__ - A trace token flag (`true | false`) which enables/disables tracing on information sending. Default is `false`.

* __`set_token('receive', Bool)`__ - A trace token flag (`true | false`) which enables/disables tracing on information reception. Default is `false`.

* __`set_token(print, Bool)`__ - A trace token flag (`true | false`) which enables/disables tracing on explicit calls to `seq_trace:print/1`. Default is `false`.

* __`set_token(timestamp, Bool)`__ - A trace token flag (`true | false`) which enables/disables a timestamp to be generated for each traced event. Default is `false`.

* __`set_token(strict_monotonic_timestamp, Bool)`__ - A trace token flag (`true | false`) which enables/disables a strict monotonic timestamp to be generated for each traced event. Default is `false`. Timestamps will consist of [Erlang monotonic time](`p:erts:time_correction.md#erlang_monotonic_time`) and a monotonically increasing integer. The time-stamp has the same format and value as produced by `{erlang:monotonic_time(nanosecond), erlang:unique_integer([monotonic])}`.

* __`set_token(monotonic_timestamp, Bool)`__ - A trace token flag (`true | false`) which enables/disables a strict monotonic timestamp to be generated for each traced event. Default is `false`. Timestamps will use [Erlang monotonic time](`p:erts:time_correction.md#erlang_monotonic_time`). The time-stamp has the same format and value as produced by `erlang:monotonic_time(nanosecond)`.
...
-doc "Returns the value of the trace token for the calling process. If `[]` is returned, it means that tracing is not active. Any other value returned is the value of an active trace token. The value returned can be used as input to the `set_token/1` function.".
...
-doc "Puts the Erlang term `TraceInfo` into the sequential trace output if the calling process currently is executing within a sequential trace and the `print` flag of the trace token is set.".
...
-doc "Same as `print/1` with the additional condition that `TraceInfo` is output only if `Label` is equal to the label component of the trace token.".
...
Sets the system tracer. The system tracer can be either a process, port or [tracer module](`m:erl_tracer`) denoted by `Tracer`. Returns the previous value (which can be `false` if no system tracer is active).

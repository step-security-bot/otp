...
>
> * The option [read_packets](`m:inet#option-read_packets`) is currently *ignored*.
...
-doc "The tuple with size 3 is *not* supported on all platforms. 'ifindex' defaults to zero (0) on platforms that supports the 3-tuple variant.".
...
* __`{udp_module, module()}`__ - Overrides which callback module is used. Defaults to `inet_udp` for IPv4 and `inet6_udp` for IPv6.

* __`{multicast_if, Address}`__ - Sets the local device for a multicast socket.

* __`{multicast_loop, true | false}`__ - When `true`, sent multicast packets are looped back to the local sockets.

* __`{multicast_ttl, Integer}`__ - Option `multicast_ttl` changes the time-to-live (TTL) for outgoing multicast datagrams to control the scope of the multicasts.

  Datagrams with a TTL of 1 are not forwarded beyond the local network. Defaults to `1`.

* __`{add_membership, {MultiAddress, InterfaceAddress}}`__ - Joins a multicast group.

* __`{drop_membership, {MultiAddress, InterfaceAddress}}`__ - Leaves a multicast group.
...
The message contains an `AncData` field if any of the socket [options](`t:option/0`) [`recvtos`](`m:inet#option-recvtos`), [`recvtclass`](`m:inet#option-recvtclass`) or [`recvttl`](`m:inet#option-recvttl`) are active, otherwise it does not.



If the socket is not in an active mode, data can be retrieved through the [`recv/2,3`](`recv/2`) calls. Notice that arriving UDP packets that are longer than the receive buffer option specifies can be truncated without warning.
...
-doc "Sends a packet on a connected socket (see [`connect/2`](`m:gen_udp#connect-sockaddr`) and [`connect/3`](`m:gen_udp#connect-addr-port`)).".
-doc(#{since => <<"OTP 24.3">>}).
...
This function is equivalent to [`send(Socket, Destination, [], Packet)`](`m:gen_udp#send-4-ancdata`).
""".
-doc(#{since => <<"OTP 22.1">>}).
...
This is a legacy clause mostly for `Destination = {local, Binary}` where `PortZero` is superfluous. It is equivalent to [`send(Socket, Destination, [], Packet)`](`m:gen_udp#send-4-ancdata`), the clause right above here.
""".
-doc(#{since => <<"OTP 22.1">>}).
...
Argument `Host` can be a hostname or a socket address, and `Port` can be a port number or a service name atom. These are resolved into a `Destination` and after that this function is equivalent to [`send(Socket, Destination, AncData, Packet)`](`m:gen_udp#send-4-ancdata`), read there about ancillary data.
""".
-doc(#{since => <<"OTP 22.1">>}).
...
If any of the socket [options](`t:option/0`) [`recvtos`](`m:inet#option-recvtos`), [`recvtclass`](`m:inet#option-recvtclass`) or [`recvttl`](`m:inet#option-recvttl`) are active, the `RecvData` tuple contains an `AncData` field, otherwise it does not.
""".
...
This means that it is not necessary to specify the destination address when sending a datagram. That is, we can use [`send/2`](`m:gen_udp#send-2`).

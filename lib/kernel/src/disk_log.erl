...
* __halt logs__ - Appends items to a single file, which size can be limited by the `disk_log` module.

* __wrap logs__ - Uses a sequence of wrap log files of limited size. As a wrap log file is filled up, further items are logged on to the next file in the sequence, starting all over with the first file when the last file is filled up.

* __rotate logs__ - Uses a sequence of rotate log files of limited size. As a log file is filled up, it is rotated and then compressed. There is one active log file and upto the configured number of compressed log files. Only externally formatted logs are supported. It follows the same naming convention as the handler logger_std_h for Logger. For more details about the naming convention check the file parameter for `open/1`.
...
The `blog()` functions can also be used for internally formatted logs, but in this case they must be called with binaries constructed with calls to [`term_to_binary/1`](`erlang:term_to_binary/1`). There is no check to ensure this, it is entirely the responsibility of the caller. If these functions are called with binaries that do not correspond to Erlang terms, the [`chunk/2,3`](`chunk/2`) and automatic repair functions fail. The corresponding terms (not the binaries) are returned when `chunk/2,3` is called.
...
The `disk_log` module does not report errors to the `m:error_logger` module. It is up to the caller to decide whether to employ the error logger. Function `format_error/1` can be used to produce readable messages from error replies. However, information events are sent to the error logger in two situations, namely when a log is repaired, or when a file is missing while reading chunks.
...
-doc "Chunk continuation returned by `chunk/2,3`, `bchunk/2,3`, or `chunk_step/3`.".
...
-type dlog_size() ::
          infinity |
          pos_integer() |
          {MaxNoBytes :: pos_integer(), MaxNoFiles :: pos_integer()}.
```
```erlang
-type open_error_rsn() ::
          no_such_log |
...
* __`{name, Log}`__ - Specifies the log name. This name must be passed on as a parameter in all subsequent logging operations. A name must always be supplied.

* __`{file, FileName}`__ - Specifies the name of the file to be used for logged terms. If this value is omitted and the log name is an atom or a string, the filename defaults to `lists:concat([Log, ".LOG"])` for halt logs.

  For wrap logs, this is the base name of the files. Each file in a wrap log is called `<FileName>.N`, where `N` is an integer. Each wrap log also has two files called `<FileName>.idx` and `<FileName>.siz`.

  For rotate logs, this is the name of the active log file. The compressed files are named as `<FileName>.N.gz`, where `N` is an integer and `<FileName>.0.gz` is the latest compressed log file. All the compressed files are renamed at each rotation so that the latest files have the smallest index. The maximum value for N is the value of `MaxNoFiles` minus 1.

* __`{linkto, LinkTo}`[](){: id=linkto }
__  
...
* __`{size, Size}`__ - Log size.
  ...
  For wrap and rotate logs, parameter `Size` can be a pair `{MaxNoBytes, MaxNoFiles}`. For wrap logs it can also be `infinity`. In the latter case, if the files of an existing wrap log with the same name can be found, the size is read from the existing wrap log, otherwise an error is returned.
  ...
  The first time an existing wrap log is opened, that is, when the disk log process is created, the value of the option `size` is allowed to differ from the current log size, and the size of the disk log is changed as per `change_size/2`.

  When opening an existing wrap log, it is not necessary to supply a value for option `size`, but if the log is already open, that is, the disk log process exists, the supplied value must equal the current log size, otherwise the tuple `{error, {size_mismatch, CurrentSize, NewSize}}` is returned.

  > #### Note {: class=info }
  > Before Erlang/OTP 24.0, the supplied value of option `size` was to be equal to the current log size when opening an existing wrap log for the first time, that is, when creating the disk log process.

  Rotate logs write at most `MaxNoBytes` bytes on the active log file and keep the latest `MaxNoFiles` compressed files. Regardless of `MaxNoBytes`, at least the header (if there is one) and one item are written on each rotate log file before rotation.

  When opening an already open halt log, option `size` is ignored.

* __`{notify, boolean()}`[](){: id=notify }
__  
  ...
  * __`{disk_log, Node, Log, {wrap, NoLostItems}}`__ - Sent when a wrap log has filled up one of its files and a new file is opened. `NoLostItems` is the number of previously logged items that were lost when truncating existing files.

  * __`{disk_log, Node, Log, {truncated, NoLostItems}}`__ - Sent when a log is truncated or reopened. For halt logs `NoLostItems` is the number of items written on the log since the disk log process was created. For wrap logs `NoLostItems` is the number of items on all wrap log files.

  * __`{disk_log, Node, Log, {read_only, Items}}`__ - Sent when an asynchronous log attempt is made to a log file opened in read-only mode. `Items` is the items from the log attempt.

  * __`{disk_log, Node, Log, {blocked_log, Items}}`__ - Sent when an asynchronous log attempt is made to a blocked log that does not queue log attempts. `Items` is the items from the log attempt.

  * __`{disk_log, Node, Log, {format_external, Items}}`__ - Sent when function `alog/2` or `alog_terms/2` is used for internally formatted logs. `Items` is the items from the log attempt.

  * __`{disk_log, Node, Log, full}`__ - Sent when an attempt to log items to a wrap log would write more bytes than the limit set by option `size`.

  * __`{disk_log, Node, Log, {error_status, Status}}`__ - Sent when the error status changes. The error status is defined by the outcome of the last attempt to log items to the log, or to truncate the log, or the last use of function `sync/1`, `inc_wrap_file/1`, or `change_size/2`. `Status` is either `ok` or `{error, Error}`, the former is the initial value.

  

* __`{head, Head}`__ - Specifies a header to be written first on the log file. If the log is a wrap or rotate log, the item `Head` is written first in each new file. `Head` is to be a term if the format is `internal`, otherwise an `t:iodata()`. Defaults to `none`, which means that no header is written first on the file.

* __`{head_func, {M,F,A}}`__ - Specifies a function to be called each time a new log file is opened. The call `M:F(A)` is assumed to return `{ok, Head}`. The item `Head` is written first in each file. `Head` is to be a term if the format is `internal`, otherwise an `t:iodata()`.

* __`{mode, Mode}`__ - Specifies if the log is to be opened in read-only or read-write mode. Defaults to `read_write`.

* __`{quiet, Boolean}`__ - Specifies if messages will be sent to `error_logger` on recoverable errors with the log files. Defaults to `false`.

`open/1` returns `{ok, Log}` if the log file is successfully opened. If the file is successfully repaired, the tuple `{repaired, Log, {recovered, Rec}, {badbytes, Bad}}` is returned, where `Rec` is the number of whole Erlang terms found in the file and `Bad` is the number of bytes in the file that are non-Erlang terms.
...
A log file can be opened more than once by giving different values to option `name` or by using the same file when opening a log on different nodes. It is up to the user of module `disk_log` to ensure that not more than one disk log process has write access to any file, otherwise the file can be corrupted.
...
Synchronously appends a term to a disk log. Returns `ok` or `{error, Reason}` when the term is written to disk. Terms are written by the ordinary `write()` function of the operating system. Hence, it is not guaranteed that the term is written to disk, it can linger in the operating system kernel for a while. To ensure that the item is written to disk, function `sync/1` must be called.

`log/2` is used for internally formatted logs, and `blog/2` for externally formatted logs. `blog/2` can also be used for internally formatted logs if the binary is constructed with a call to [`term_to_binary/1`](`erlang:term_to_binary/1`).
...
`log_terms/2` is used for internally formatted logs, and `blog_terms/2` for externally formatted logs. `blog_terms/2` can also be used for internally formatted logs if the binaries are constructed with calls to [`term_to_binary/1`](`erlang:term_to_binary/1`).
...
Asynchronously append an item to a disk log. `alog/2` is used for internally formatted logs and `balog/2` for externally formatted logs. `balog/2` can also be used for internally formatted logs if the binary is constructed with a call to [`term_to_binary/1`](`erlang:term_to_binary/1`).
...
Asynchronously append a list of items to a disk log. `alog_terms/2` is used for internally formatted logs and `balog_terms/2` for externally formatted logs. `balog_terms/2` can also be used for internally formatted logs if the binaries are constructed with calls to [`term_to_binary/1`](`erlang:term_to_binary/1`).
...
Removes all items from a disk log. If argument `Head` or `BHead` is specified, this item is written first in the newly truncated log, otherwise the header given to `open/1` is used. The header argument is used only once. Next time a wrap/rotate log file is opened, the header given to `open/1` is used.

`truncate/1` is used for both internally and externally formatted logs.

`truncate/2` is used for internally formatted logs, and `btruncate/2` for externally formatted logs.

Owners subscribing to notifications receive a `truncate` message.

If the attempt to truncate the log fails, the disk log process terminates with the EXIT message `{{failed,Reason},[{disk_log,Fun,Arity}]}`. Other processes having requests queued receive the message `{disk_log, Node, {error, disk_log_stopped}}`.
""".
...
Renames the log file to `File` and then recreates a new log file. If a wrap/rotate log exists, `File` is used as the base name of the renamed files. By default the header given to `open/1` is written first in the newly opened log file, but if argument `Head` or `BHead` is specified, this item is used instead. The header argument is used only once. Next time a wrap/rotate log file is opened, the header given to `open/1` is used.
...
For wrap logs, it forces the disk log to start logging to the next log file. It can be used, for example, with `change_size/2` to reduce the amount of disk space allocated by the disk log.
...
""".
-doc(#{deprecated =>
           <<"disk_log:inc_wrap_file/1 is deprecated and will be removed in OTP 28; use disk_log:next_file/1 instead">>}).
...
If the log size is decreased, for example, to save space, function `next_file/1`, can be used to force the log to wrap.
""".
...
-doc "Given the error returned by any function in this module, this function returns a descriptive string of the error in English. For file errors, function `format_error/1` in module [`file`](`file:format_error/1`) is called.".
...
-type dlog_info() ::
          ...
          {current_file, pos_integer()} |
          {no_overflows,
           {SinceLogWasOpened :: non_neg_integer(),
            SinceLastInfo :: non_neg_integer()}}.
```
...
* __`{name, Log}`__ - `Log` is the log name as specified by the `open/1` option `name`.

* __`{file, File}`__ - For halt logs `File` is the filename, and for wrap logs `File` is the base name.

* __`{type, Type}`__ - `Type` is the log type as specified by the `open/1` option `type`.

* __`{format, Format}`__ - `Format` is the log format as specified by the `open/1` option `format`.

* __`{size, Size}`__ - `Size` is the log size as specified by the `open/1` option `size`, or the size set by `change_size/2`. The value set by `change_size/2` is reflected immediately.

* __`{mode, Mode}`__ - `Mode` is the log mode as specified by the `open/1` option `mode`.

* __`{owners, [{pid(), Notify}]}`__ - `Notify` is the value set by the `open/1` option `notify` or function `change_notify/3` for the owners of the log.

* __`{users, Users}`__ - `Users` is the number of anonymous users of the log, see the `open/1` option [`linkto`](`m:disk_log#linkto`).

* __`{status, Status}`__ - `Status` is `ok` or `{blocked, QueueLogRecords}` as set by functions `block/1,2` and `unblock/1`.

* __`{node, Node}`__ - The information returned by the current invocation of function `info/1` is gathered from the disk log process running on `Node`.

The following pairs are returned for all logs opened in `read_write` mode:

* __`{head, Head}`__ - Depending on the value of the `open/1` options `head` and `head_func`, or set by function `change_header/2`, the value of `Head` is `none` (default), `{head, H}` (`head` option), or `{M,F,A}` (`head_func` option).

* __`{no_written_items, NoWrittenItems}`__ - `NoWrittenItems` is the number of items written to the log since the disk log process was created.

The following pair is returned for halt logs opened in `read_write` mode:

* __`{full, Full}`__ - `Full` is `true` or `false` depending on whether the halt log is full or not.

The following pairs are returned for wrap logs opened in `read_write` mode:

* __`{no_current_bytes, integer() >= 0}`__ - The number of bytes written to the current wrap log file.

* __`{no_current_items, integer() >= 0}`__ - The number of items written to the current wrap log file, header inclusive.

* __`{no_items, integer() >= 0}`__ - The total number of items in all wrap log files.

* __`{current_file, integer()}`__ - The ordinal for the current wrap log file in the range `1..MaxNoFiles`, where `MaxNoFiles` is specified by the `open/1` option `size` or set by `change_size/2`.

* __`{no_overflows, {SinceLogWasOpened, SinceLastInfo}}`__ - `SinceLogWasOpened` (`SinceLastInfo`) is the number of times a wrap log file has been filled up and a new one is opened or `inc_wrap_file/1` has been called since the disk log was last opened (`info/1` was last called). The first time `info/2` is called after a log was (re)opened or truncated, the two values are equal.
...
If atom `start` is specified as continuation, the first file of the wrap log is chosen as the new current file.

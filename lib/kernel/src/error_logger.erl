...
>
> See `m:logger` and the [Logging](logger_chapter.md) chapter in the User's Guide for more information.

The Erlang *error logger* is an event manager (see [OTP Design Principles](`p:system:design_principles.md`) and `m:gen_event`), registered as `error_logger`.

Error logger is no longer started by default, but is automatically started when an event handler is added with [`add_report_handler/1,2`](`add_report_handler/1`). The `error_logger` module is then also added as a handler to the new logger, causing log events to be forwarded from logger to error logger, and consequently to all installed error logger event handlers.
...
* __`{error, Gleader, {Pid, Format, Data}}`__ - Generated when `error_msg/1,2` or `format` is called.

* __`{error_report, Gleader, {Pid, std_error, Report}}`__ - Generated when `error_report/1` is called.

* __`{error_report, Gleader, {Pid, Type, Report}}`__ - Generated when `error_report/2` is called.

* __`{warning_msg, Gleader, {Pid, Format, Data}}`__ - Generated when `warning_msg/1,2` is called if warnings are set to be tagged as warnings.

* __`{warning_report, Gleader, {Pid, std_warning, Report}}`__ - Generated when `warning_report/1` is called if warnings are set to be tagged as warnings.

* __`{warning_report, Gleader, {Pid, Type, Report}}`__ - Generated when `warning_report/2` is called if warnings are set to be tagged as warnings.

* __`{info_msg, Gleader, {Pid, Format, Data}}`__ - Generated when `info_msg/1,2` is called.

* __`{info_report, Gleader, {Pid, std_info, Report}}`__ - Generated when `info_report/1` is called.

* __`{info_report, Gleader, {Pid, Type, Report}}`__ - Generated when `info_report/2` is called.
...
`m:gen_event`, `m:logger`, `m:log_mf_h`, [`kernel(6)`](kernel_app.md), [`sasl(6)`](`p:sasl:sasl_app.md`)
""".
...
These functions are kept for backwards compatibility and must not be used by new code. Use the [`?LOG_ERROR`](`m:logger#macros`) macro or [`logger:error/1,2,3`](`logger:error/1`) instead.
...
This functions is kept for backwards compatibility and must not be used by new code. Use the [`?LOG_ERROR`](`m:logger#macros`) macro or [`logger:error/1,2,3`](`logger:error/1`) instead.
...
This functions is kept for backwards compatibility and must not be used by new code. Use the [`?LOG_WARNING`](`m:logger#macros`) macro or [`logger:warning/1,2,3`](`logger:warning/1`) instead.
""".
...
These functions are kept for backwards compatibility and must not be used by new code. Use the [`?LOG_WARNING`](`m:logger#macros`) macro or [`logger:warning/1,2,3`](`logger:warning/1`) instead.
...
This functions is kept for backwards compatibility and must not be used by new code. Use the [`?LOG_INFO`](`m:logger#macros`) macro or [`logger:info/1,2,3`](`logger:info/1`) instead.
...
These functions are kept for backwards compatibility and must not be used by new code. Use the [`?LOG_INFO`](`m:logger#macros`) macro or [`logger:info/1,2,3`](`logger:info/1`) instead.
...
-doc """
Adds a new event handler to the error logger. The event handler must be implemented as a `gen_event` callback module, see `m:gen_event`.
...
The event handler must be able to handle the events in this module, see section [Events](`m:error_logger#events`).
...
-doc """
Deletes an event handler from the error logger by calling `gen_event:delete_handler(error_logger, Handler, [])`, see `m:gen_event`.
...
This function is useful as a shortcut during development and testing, but must not be used in a production system. See section [Logging](logger_chapter.md) in the Kernel User's Guide, and the `m:logger` manual page for information about how to configure Logger for live systems.

`Request` is one of the following:

* __`{open, Filename}`__ - Opens log file `Filename`. Returns `ok` if successful, or `{error, allready_have_logfile}` if logging to file is already enabled, or an error tuple if another error occurred (for example, if `Filename` cannot be opened). The file is opened with encoding UTF-8.

* __`close`__ - Closes the current log file. Returns `ok`, or `{error, module_not_found}`.

* __`filename`__ - Returns the name of the log file `Filename`, or `{error, no_log_file}` if logging to file is not enabled.
""".
...
This is done by manipulating the Logger configuration. The function is useful as a shortcut during development and testing, but must not be used in a production system. See section [Logging](logger_chapter.md) in the Kernel User's Guide, and the `m:logger` manual page for information about how to configure Logger for live systems.
""".
...
> #### Note {: class=info }
> The `error_logger_format_depth` variable is [deprecated](kernel_app.md#deprecated-configuration-parameters) since the [Logger API](`m:logger`) was introduced in Erlang/OTP 21.0. The variable, and this function, are kept for backwards compatibility since they still might be used by legacy report handlers.
""".
-doc(#{since => <<"OTP 20.0">>}).

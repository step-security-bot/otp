...
* __*Load and Unload on a "When Needed Basis"*__ - This (most common) scenario simply supports that each [user](`m:erl_ddll#users`) of the driver loads it when needed and unloads it when no longer needed. The driver is always reference counted and as long as a process keeping the driver loaded is still alive, the driver is present in the system.

  Each [user](`m:erl_ddll#users`) of the driver use *literally* the same pathname for the driver when demanding load, but the [users](`m:erl_ddll#users`) are not concerned with if the driver is already loaded from the file system or if the object code must be loaded from file system.

  The following two pairs of functions support this scenario:

  * __*load/2 and unload/1*__ - When using the `load/unload` interfaces, the driver is not unloaded until the *last port* using the driver is closed. Function `unload/1` can return immediately, as the [users](`m:erl_ddll#users`) have no interest in when the unloading occurs. The driver is unloaded when no one needs it any longer.
  ...
  * __*load_driver/2 and unload_driver/1*__ - These interfaces are intended to be used when it is considered an error that ports are open to a driver that no [user](`m:erl_ddll#users`) has loaded. The ports that are still open when the last [user](`m:erl_ddll#users`) calls `unload_driver/1` or when the last process having the driver loaded dies, are killed with reason `driver_unloaded`.

    The function names `load_driver` and `unload_driver` are kept for backward compatibility.

  

* __*Loading and Reloading for Code Replacement*__ - This scenario can occur if the driver code needs replacement during operation of the Erlang emulator. Implementing driver code replacement is a little more tedious than Beam code replacement, as one driver cannot be loaded as both "old" and "new" code. All [users](`m:erl_ddll#users`) of a driver must have it closed (no open ports) before the old code can be unloaded and the new code can be loaded.
  ...
  The preferred way to do driver code replacement is to let *one single process* keep track of the driver. When the process starts, the driver is loaded. When replacement is required, the driver is reloaded. Unload is probably never done, or done when the process exits. If more than one [user](`m:erl_ddll#users`) has a driver loaded when code replacement is demanded, the replacement cannot occur until the last "other" [user](`m:erl_ddll#users`) has unloaded the driver.

  Demanding reload when a reload is already in progress is always an error. Using the high-level functions, it is also an error to demand reloading when more than one [user](`m:erl_ddll#users`) has the driver loaded.

  To simplify driver replacement, avoid designing your system so that more than one [user](`m:erl_ddll#users`) has the driver loaded.
  ...
  * __*load/2 and reload/2*__ - This pair of functions is used when reloading is to be done after the last open port to the driver is closed.
  ...
  * __*load_driver/2 and reload_driver/2*__ - This pair of functions are used when open ports to the driver are to be killed with reason `driver_unloaded` to allow for new driver code to get loaded.

    However, if another process has the driver loaded, calling `reload_driver` returns error code `pending_process`. As stated earlier, the recommended design is to not allow other [users](`m:erl_ddll#users`) than the "driver reloader" to demand loading of the driver in question.

  

## See Also

[`erts:erl_driver(4)`](`p:erts:erl_driver.md`), [`erts:driver_entry(4)`](`p:erts:driver_entry.md`)
""".
...
The function throws a `badarg` exception if the parameter is not a `t:reference()`.
""".
...
* __`processes`__ - Returns all processes containing [users](`m:erl_ddll#users`) of the specific drivers as a list of tuples `{pid(),integer() >= 0}`, where `t:integer()` denotes the number of users in process `t:pid()`.

* __`driver_options`__ - Returns a list of the driver options provided when loading, and any options set by the driver during initialization. The only valid option is `kill_ports`.

* __`port_count`__ - Returns the number of ports (an `integer() >= 0`) using the driver.

* __`linked_in_driver`__ - Returns a `t:boolean()`, which is `true` if the driver is a statically linked-in one, otherwise `false`.

* __`permanent`__ - Returns a `t:boolean()`, which is `true` if the driver has made itself permanent (and is *not* a statically linked-in driver), otherwise `false`.

* __`awaiting_load`__ - Returns a list of all processes having monitors for `loading` active. Each process is returned as `{pid(),integer() >= 0}`, where `t:integer()` is the number of monitors held by process `t:pid()`.

* __`awaiting_unload`__ - Returns a list of all processes having monitors for `unloading` active. Each process is returned as `{pid(),integer() >= 0}`, where `t:integer()` is the number of monitors held by process `t:pid()`.
...
* __`Tag`__ - The monitor tag is always `driver`, as this function can only be used to create driver monitors. In the future, driver monitors will be integrated with process monitors, why this parameter has to be specified for consistence.

* __`Item`__ - Parameter `Item` specifies which driver to monitor (the driver name) and which state change to monitor. The parameter is a tuple of arity two whose first element is the driver name and second element is one of the following:

  * __`loaded`__ - Notifies when the driver is reloaded (or loaded if loading is underway). It only makes sense to monitor drivers that are in the process of being loaded or reloaded. A future driver name for loading cannot be monitored. That only results in a `DOWN` message sent immediately. Monitoring for loading is therefore most useful when triggered by function `try_load/3`, where the monitor is created *because* the driver is in such a pending state.

    Setting a driver monitor for `loading` eventually leads to one of the following messages being sent:

    * __`{'UP', reference(), driver, Name, loaded}`__ - This message is sent either immediately if the driver is already loaded and no reloading is pending, or when reloading is executed if reloading is pending.

      The [user](`m:erl_ddll#users`) is expected to know if reloading is demanded before creating a monitor for loading.

    * __`{'UP', reference(), driver, Name, permanent}`__ - This message is sent if reloading was expected, but the (old) driver made itself permanent before reloading. It is also sent if the driver was permanent or statically linked-in when trying to create the monitor.

    * __`{'DOWN', reference(), driver, Name, load_cancelled}`__ - This message arrives if reloading was underway, but the requesting [user](`m:erl_ddll#users`) cancelled it by dying or calling `try_unload/2` (or `unload/1`/`unload_driver/1`) again before it was reloaded.

    * __`{'DOWN', reference(), driver, Name, {load_failure, Failure}}`__ - This message arrives if reloading was underway but the loading for some reason failed. The `Failure` term is one of the errors that can be returned from `try_load/3`. The error term can be passed to `format_error/1` for translation into human readable form. Notice that the translation must be done in the same running Erlang virtual machine as the error was detected in.

    

  * __`unloaded`__ - Monitors when a driver gets unloaded. If one monitors a driver that is not present in the system, one immediately gets notified that the driver got unloaded. There is no guarantee that the driver was ever loaded.

    A driver monitor for unload eventually results in one of the following messages being sent:

    * __`{'DOWN', reference(), driver, Name, unloaded}`__ - The monitored driver instance is now unloaded. As the unload can be a result of a `reload/2` request, the driver can once again have been loaded when this message arrives.

    * __`{'UP', reference(), driver, Name, unload_cancelled}`__ - This message is sent if unloading was expected, but while the driver was waiting for all ports to get closed, a new [user](`m:erl_ddll#users`) of the driver appeared, and the unloading was cancelled.

      This message appears if `{ok, pending_driver}` was returned from `try_unload/2` for the last [user](`m:erl_ddll#users`) of the driver, and then `{ok, already_loaded}` is returned from a call to `try_load/3`.
    ...
    * __`{'UP', reference(), driver, Name, permanent}`__ - This message is sent if unloading was expected, but the driver made itself permanent before unloading. It is also sent if trying to monitor a permanent or statically linked-in driver.

    

  * __`unloaded_only`__ - A monitor created as `unloaded_only` behaves exactly as one created as `unloaded` except that the `{'UP', reference(), driver, Name, unload_cancelled}` message is never sent, but the monitor instead persists until the driver *really* gets unloaded.
...
* __`{ok, loaded}`__ - The driver was loaded and is immediately usable.

* __`{ok, already_loaded}`__ - The driver was already loaded by another process or is in use by a living port, or both. The load by you is registered and a corresponding `try_unload` is expected sometime in the future.

* __`{ok, pending_driver}`or `{ok, pending_driver, reference()}`__ - The load request is registered, but the loading is delayed because an earlier instance of the driver is still waiting to get unloaded (open ports use it). Still, unload is expected when you are done with the driver. This return value *mostly* occurs when options `{reload,pending_driver}` or `{reload,pending}` are used, but *can* occur when another [user](`m:erl_ddll#users`) is unloading a driver in parallel and driver option `kill_ports` is set. In other words, this return value always needs to be handled.

* __`{ok, pending_process}`or `{ok, pending_process, reference()}`__ - The load request is registered, but the loading is delayed because an earlier instance of the driver is still waiting to get unloaded by another [user](`m:erl_ddll#users`) (not only by a port, in which case `{ok,pending_driver}` would have been returned). Still, unload is expected when you are done with the driver. This return value *only* occurs when option `{reload,pending}` is used.
...
* __`Path`__ - The file system path to the directory where the driver object file is located. The filename of the object file (minus extension) must correspond to the driver name (used in parameter `Name`) and the driver must identify itself with the same name. `Path` can be provided as an *iolist()*, meaning it can be a list of other `t:iolist()`s, characters (8-bit integers), or binaries, all to be flattened into a sequence of characters.

  The (possibly flattened) `Path` parameter must be consistent throughout the system. A driver is to, by all [users](`m:erl_ddll#users`), be loaded using the same *literal* `Path`. The exception is when *reloading* is requested, in which case `Path` can be specified differently. Notice that all [users](`m:erl_ddll#users`) trying to load the driver later need to use the *new* `Path` if `Path` is changed using a `reload` option. This is yet another reason to have *only one loader* of a driver one wants to upgrade in a running system.

* __`Name`__ - This parameter is the name of the driver to be used in subsequent calls to function [`erlang:open_port`](`erlang:open_port/2`) in ERTS. The name can be specified as an `t:iolist()` or an `t:atom()`. The name specified when loading is used to find the object file (with the help of `Path` and the system-implied extension suffix, that is, `.so`). The name by which the driver identifies itself must also be consistent with this `Name` parameter, much as the module name of a Beam file much corresponds to its filename.

* __`OptionList`__ - Some options can be specified to control the loading operation. The options are specified as a list of two-tuples. The tuples have the following values and meanings:

  * __`{driver_options, DriverOptionList}`__ - This is to provide options that changes its general behavior and "sticks" to the driver throughout its lifespan.
    ...
    The only allowed driver option is `kill_ports`, which means that all ports opened to the driver are killed with exit reason `driver_unloaded` when no process any longer has the driver loaded. This situation arises either when the last [user](`m:erl_ddll#users`) calls `try_unload/2`, or when the last process having loaded the driver exits.

  * __`{monitor, MonitorOption}`__ - A `MonitorOption` tells `try_load/3` to trigger a driver monitor under certain conditions. When the monitor is triggered, the function returns a three-tuple `{ok, PendingStatus, reference()}`, where `t:reference()` is the monitor reference for the driver monitor.
  ...
  * __`{reload, ReloadOption}`__ - This option is used to *reload* a driver from disk, most often in a code upgrade scenario. Having a `reload` option also implies that parameter `Path` does *not* need to be consistent with earlier loads of the driver.

    To reload a driver, the process must have loaded the driver before, that is, there must be an active [user](`m:erl_ddll#users`) of the driver in the process.

    The `reload` option can be either of the following:

    * __`pending`__ - With the atom `pending`, reloading is requested for any driver and is effectuated when *all* ports opened to the driver are closed. The driver replacement in this case takes place regardless if there are still pending [users](`m:erl_ddll#users`) having the driver loaded.

      The option also triggers port-killing (if driver option `kill_ports` is used) although there are pending users, making it usable for forced driver replacement, but laying much responsibility on the driver [users](`m:erl_ddll#users`). The pending option is seldom used as one does not want other [users](`m:erl_ddll#users`) to have loaded the driver when code change is underway.

    * __`pending_driver`__ - This option is more useful. Here, reloading is queued if the driver is *not* loaded by any other [users](`m:erl_ddll#users`), but the driver has opened ports, in which case `{ok, pending_driver}` is returned (a `monitor` option is recommended).
...
* __`{error,linked_in_driver}`__ - The driver with the specified name is an Erlang statically linked-in driver, which cannot be manipulated with this API.

* __`{error,inconsistent}`__ - The driver is already loaded with other `DriverOptionList` or a different *literal* `Path` argument.

  This can occur even if a `reload` option is specified, if `DriverOptionList` differs from the current.

* __`{error, permanent}`__ - The driver has requested itself to be permanent, making it behave like an Erlang linked-in driver and can no longer be manipulated with this API.

* __`{error, pending_process}`__ - The driver is loaded by other [users](`m:erl_ddll#users`) when option `{reload, pending_driver}` was specified.

* __`{error, pending_reload}`__ - Driver reload is already requested by another [user](`m:erl_ddll#users`) when option `{reload, ReloadOption}` was specified.

* __`{error, not_loaded_by_this_process}`__ - Appears when option `reload` is specified. The driver `Name` is present in the system, but there is no [user](`m:erl_ddll#users`) of it in this process.

* __`{error, not_loaded}`__ - Appears when option `reload` is specified. The driver `Name` is not in the system. Only drivers loaded by this process can be reloaded.
...
-doc """
This is the low-level function to unload (or decrement reference counts of) a driver. It can be used to force port killing, in much the same way as the driver option `kill_ports` implicitly does. Also, it can trigger a monitor either because other [users](`m:erl_ddll#users`) still have the driver loaded or because open ports use the driver.

Unloading can be described as the process of telling the emulator that this particular part of the code in this particular process (that is, this [user](`m:erl_ddll#users`)) no longer needs the driver. That can, if there are no other users, trigger unloading of the driver, in which case the driver name disappears from the system and (if possible) the memory occupied by the driver executable code is reclaimed.

If the driver has option `kill_ports` set, or if `kill_ports` is specified as an option to this function, all pending ports using this driver are killed when unloading is done by the last [user](`m:erl_ddll#users`). If no port-killing is involved and there are open ports, the unloading is delayed until no more open ports use the driver. If, in this case, another [user](`m:erl_ddll#users`) (or even this user) loads the driver again before the driver is unloaded, the unloading never takes place.

To allow the [user](`m:erl_ddll#users`) to *request unloading* to wait for *actual unloading*, `monitor` triggers can be specified in much the same way as when loading. However, as [users](`m:erl_ddll#users`) of this function seldom are interested in more than decrementing the reference counts, monitoring is seldom needed.
...
* __`{ok, unloaded}`__ - The driver was immediately unloaded, meaning that the driver name is now free to use by other drivers and, if the underlying OS permits it, the memory occupied by the driver object code is now reclaimed.

  The driver can only be unloaded when there are no open ports using it and no more [users](`m:erl_ddll#users`) require it to be loaded.

* __`{ok, pending_driver}`or `{ok, pending_driver, reference()}`__ - Indicates that this call removed the last [user](`m:erl_ddll#users`) from the driver, but there are still open ports using it. When all ports are closed and no new [users](`m:erl_ddll#users`) have arrived, the driver is reloaded and the name and memory reclaimed.
...
* __`{ok, pending_process}`or `{ok, pending_process, reference()}`__ - The unload request is registered, but other [users](`m:erl_ddll#users`) still hold the driver. Notice that the term `pending_process` can refer to the running process; there can be more than one [user](`m:erl_ddll#users`) in the same process.

  This is a normal, healthy, return value if the call was just placed to inform the emulator that you have no further use of the driver. It is the most common return value in the most common [`scenario`](`m:erl_ddll#scenarios`) described in the introduction.

The function accepts the following parameters:

* __`Name`__ - `Name` is the name of the driver to be unloaded. The name can be specified as an `t:iolist()` or as an `t:atom()`.

* __`OptionList`__ - Argument `OptionList` can be used to specify certain behavior regarding ports and triggering monitors under certain conditions:

  * __`kill_ports`__ - Forces killing of all ports opened using this driver, with exit reason `driver_unloaded`, if you are the *last* [user](`m:erl_ddll#users`) of the driver.

    If other [users](`m:erl_ddll#users`) have the driver loaded, this option has no effect.

    To get the consistent behavior of killing ports when the last [user](`m:erl_ddll#users`) unloads, use driver option `kill_ports` when loading the driver instead.

  * __`{monitor, MonitorOption}`__ - Creates a driver monitor if the condition specified in `MonitorOption` is true. The valid options are:

    * __`pending_driver`__ - Creates a driver monitor if the return value is to be `{ok, pending_driver}`.

    * __`pending`__ - Creates a monitor if the return value is `{ok, pending_driver}` or `{ok, pending_process}`.
...
* __`{error, linked_in_driver}`__ - You were trying to unload an Erlang statically linked-in driver, which cannot be manipulated with this interface (and cannot be unloaded at all).

* __`{error, not_loaded}`__ - The driver `Name` is not present in the system.

* __`{error, not_loaded_by_this_process}`__ - The driver `Name` is present in the system, but there is no [user](`m:erl_ddll#users`) of it in this process.
...
* __`{error, permanent}`__ - The driver has made itself permanent, in which case it can no longer be manipulated by this interface (much like a statically linked-in driver).
...
The number of loads and unloads by different [users](`m:erl_ddll#users`) influences the loading and unloading of a driver file. The port killing therefore only occurs when the *last* [user](`m:erl_ddll#users`) unloads the driver, or when the last process having loaded the driver exits.
...
-doc """
Unloads, or at least dereferences the driver named `Name`. If the caller is the last [user](`m:erl_ddll#users`) of the driver, all remaining open ports using the driver are killed with reason `driver_unloaded` and the driver eventually gets unloaded.

If there are other [users](`m:erl_ddll#users`) of the driver, the reference counts of the driver is merely decreased, so that the caller is no longer considered a [user](`m:erl_ddll#users`). For use scenarios, see the [`description`](`m:erl_ddll#scenarios`) in the beginning of this module.
...
-doc """
Unloads, or at least dereferences the driver named `Name`. If the caller is the last [user](`m:erl_ddll#users`) of the driver, and no more open ports use the driver, the driver gets unloaded. Otherwise, unloading is delayed until all ports are closed and no [users](`m:erl_ddll#users`) remain.

If there are other [users](`m:erl_ddll#users`) of the driver, the reference counts of the driver is merely decreased, so that the caller is no longer considered a [user](`m:erl_ddll#users`) of the driver. For use scenarios, see the [`description`](`m:erl_ddll#scenarios`) in the beginning of this module.
...
-doc """
Reloads the driver named `Name` from a possibly different `Path` than previously used. This function is used in the code change [`scenario`](`m:erl_ddll#scenarios`) described in the introduction.

If there are other [users](`m:erl_ddll#users`) of this driver, the function returns `{error, pending_process}`, but if there are no other users, the function call hangs until all open ports are closed.
...
For more details, see [`scenarios`](`m:erl_ddll#scenarios`) in this module description and the function description for `reload/2`.
...
-doc "Returns a list of tuples `{DriverName, InfoList}`, where `InfoList` is the result of calling `info/1` for that `DriverName`. Only dynamically linked-in drivers are included in the list.".

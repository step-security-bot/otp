...
This module implements the main API for logging in Erlang/OTP. To create a log event, use the [API functions](`m:logger#logging_api`) or the log [macros](`m:logger#macros`), for example:
...
To configure the Logger backend, use [Kernel configuration parameters](kernel_app.md#logger) or [configuration functions](`m:logger#configuration_api`) in the Logger API.
...
If you want your systems logs to be printed to a file instead, you must configure the default handler to do so. The simplest way is to include the following in your [`sys.config`](config.md):
...
* how to configure Logger, see the [Configuration](logger_chapter.md#configuration) section in the User's Guide.
...
All macros expand to a call to Logger, where `Level` is taken from the macro name, or from the first argument in the case of the `?LOG` macro. Location data is added to the metadata as described under the [`metadata()`](`t:metadata/0`) type definition.
...
[`config(4)`](config.md), `m:erlang`, `m:io`, `m:logger_disk_log_h`, `m:logger_filters`, `m:logger_handler`, `m:logger_formatter`, `m:logger_std_h`, `m:unicode`
""".
...
-doc "A fun which converts a [`report()` ](`t:report/0`)to a format string and arguments, or directly to a string. See section [Log Message](logger_chapter.md#log_message) in the User's Guide for more information.".
...
* setting primary metadata with `set_primary_config/1` or through the kernel configuration parameter [logger_metadata](kernel_app.md#logger_metadata)
...
* __`domain`__ - The value associated with this key is used by filters for grouping log events originating from, for example, specific functional areas. See `logger_filters:domain/2` for a description of how this field can be used.

* __`report_cb`__ - If the log message is specified as a [`report()`](`t:report/0`), the `report_cb` key can be associated with a fun (report callback) that converts the report to a format string and arguments, or directly to a string. See the type definition of [`report_cb()`](`t:report_cb/0`), and section [Log Message](logger_chapter.md#log_message) in the User's Guide for more information about report callbacks.
""".
...
-type filter_id() :: atom().
-doc "A filter which can be installed as a handler filter, or as a primary filter in Logger.".
...
-doc "Configuration data for the formatter. See `m:logger_formatter` for an example of a formatter implementation.".
...
> #### Note {: class=info }
> DEPRECATED: Use [`logger_handler:id()`](`t:logger_handler:id/0`) instead.
""".
...
> #### Note {: class=info }
> DEPRECATED: Use [`logger_handler:config()`](`t:logger_handler:config/0`) instead.
""".
...
-doc "Equivalent to [`log(emergency,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Equivalent to [`log(alert,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Equivalent to [`log(critical,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Equivalent to [`log(error,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Equivalent to [`log(warning,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Equivalent to [`log(notice,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Equivalent to [`log(info,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Equivalent to [`log(debug,...)`](`log/2`).".
-doc(#{title => <<"Logging API functions">>,since => <<"OTP 21.0">>}).
...
-doc """
Create a log event at the given [log level](logger_chapter.md#log_level), with the given [message](logger_chapter.md#log_message) to be logged and [*metadata*](logger_chapter.md#metadata). Examples:
...
-doc """
Convert a log message on report form to `{Format, Args}`. This is the default report callback used by `m:logger_formatter` when no custom report callback is found. See section [Log Message](logger_chapter.md#log_message) in the Kernel User's Guide for information about report callbacks and valid forms of log messages.
...
-doc """
Return a timestamp that can be inserted as the `time` field in the meta data for a log event. It is produced with [`os:system_time(microsecond)`](`os:system_time/1`).
...
* __`t:log_event()`__ - The filter *passed*. The next primary filter, if any, is applied. If no more primary filters exist, the log event is forwarded to the handler part of Logger, where handler filters are applied.

* __`stop`__ - The filter *did not pass*, and the log event is immediately discarded.

* __`ignore`__ - The filter has no knowledge of the log event. The next primary filter, if any, is applied. If no more primary filters exist, the value of the primary `filter_default` configuration parameter specifies if the log event shall be discarded or forwarded to the handler part.

See section [Filters](logger_chapter.md#filters) in the User's Guide for more information about filters.
...
* __`t:log_event()`__ - The filter *passed*. The next handler filter, if any, is applied. If no more filters exist for this handler, the log event is forwarded to the handler callback.

* __`stop`__ - The filter *did not pass*, and the log event is immediately discarded.

* __`ignore`__ - The filter has no knowledge of the log event. The next handler filter, if any, is applied. If no more filters exist for this handler, the value of the `filter_default` configuration parameter for the handler specifies if the log event shall be discarded or forwarded to the handler callback.
...
-doc "Remove the filter identified by `FilterId` from the handler identified by `HandlerId`.".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Add or update primary configuration data for Logger. If the given `Key` already exists, its associated value will be changed to the given value. If it does not exist, it will be added.".
...
To modify the existing configuration, use `update_primary_config/1`, or, if a more complex merge is needed, read the current configuration with [`get_primary_config/0` ](`get_primary_config/0`), then do the merge before writing the new configuration back with this function.
...
See the definition of the [`logger_handler:config()`](`t:logger_handler:config/0`) type for more information about the different parameters.
""".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
To modify the existing configuration, use `update_handler_config/2`, or, if a more complex merge is needed, read the current configuration with [`get_handler_config/1` ](`get_handler_config/1`), then do the merge before writing the new configuration back with this function.
...
To modify the existing configuration, use `update_proxy_config/1`, or, if a more complex merge is needed, read the current configuration with [`get_proxy_config/0` ](`get_proxy_config/0`), then do the merge before writing the new configuration back with this function.

For more information about the proxy, see section [Logger Proxy](logger_chapter.md#proxy) in the Kernel User's Guide.
""".
...
To overwrite the existing configuration without any merge, use [`set_primary_config/1` ](`set_primary_config/1`).
""".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
To overwrite the existing configuration without any merge, use [`set_handler_config/2` ](`set_handler_config/2`).
""".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
To overwrite the existing configuration without any merge, use [`set_proxy_config/1` ](`set_proxy_config/1`).
...
To change the primary log level for Logger, use [`set_primary_config(level, Level)`](`set_primary_config/2`).

To change the log level for a handler, use [`set_handler_config(HandlerId, level, Level)` ](`set_handler_config/3`).
...
-doc "Remove module specific log settings. After this, the primary log level is used for the specified modules.".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Remove module specific log settings. After this, the primary log level is used for all modules.".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
This function is a convenience function that calls [logger:set_module_level/2](`set_module_level/2`) for each module associated with an application.
""".
...
This function is a utility function that calls [logger:unset_module_level/2](`unset_module_level/1`) for each module associated with an application.
""".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.1">>}).
...
-doc "Look up the current level for the given modules. Returns a list containing one `{Module,Level}` element for each of the given modules for which the module level was previously set with `set_module_level/2`.".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Look up all current module levels. Returns a list containing one `{Module,Level}` element for each module for which the module level was previously set with `set_module_level/2`.".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
%%%-----------------------------------------------------------------
%%% Misc
-doc "Compare the severity of two log levels. Returns `gt` if `Level1` is more severe than `Level2`, `lt` if `Level1` is less severe, and `eq` if the levels are equal.".
-doc(#{title => <<"Miscellaneous API functions">>,since => <<"OTP 21.0">>}).
...
-doc "Look up all current Logger configuration, including primary, handler, and proxy configuration, and module level settings.".
-doc(#{title => <<"Configuration API functions">>,since => <<"OTP 21.0">>}).
...
This reads the `logger` configuration parameter from the `my_app` application and starts the configured handlers. The contents of the configuration use the same rules as the [logger handler configuration](logger_chapter.md#handler-configuration).

...
-doc "Reads file `.hosts.erlang`, see section [Files](`m:net_adm#files`). Returns the hosts in this file as a list. Returns `{error, Reason}` if the file cannot be read or the Erlang terms on the file cannot be interpreted.".
...
-doc "Sets up a connection to `Node`. Returns `pong` if it is successful, otherwise `pang`.".
...
-doc "Returns the name of the local host. If Erlang was started with command-line flag `-name`, `Name` is the fully qualified name.".
...
-doc """
Similar to `epmd -names`, see [`erts:epmd(1)`](`p:erts:epmd_cmd.md`). `Host` defaults to the local host. Returns the names and associated port numbers of the Erlang nodes that `epmd` registered at the specified host. Returns `{error, address}` if `epmd` is not operational.
...
-doc "Returns the official name of `Host`, or `{error, Host}` if no such name is found. See also `m:inet`.".
...
Same as [`world/0,1`](`world/1`), but the hosts are specified as argument instead of being read from `.hosts.erlang`.
""".

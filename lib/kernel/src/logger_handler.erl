...
The behaviour module for logger handlers. A logger handler is a callback module that is called when a log event has passed all filters and is ready to be logged somewhere. For more information see [Handlers](logger_chapter.md#handlers) in the Users Guide.
...
Handler specific configuration data is inserted by the handler callback itself, in a sub structure associated with the field named `config`. See the `m:logger_std_h` and `m:logger_disk_log_h` manual pages for information about the specific configuration for these handlers.

See the [`logger_formatter(3)`](`t:logger_formatter:config/0`) manual page for information about the default configuration for this formatter.
""".
-type config() :: #{id => id(),
                    config => term(),
...
%%% Callbacks
-doc """
Config1 = Config2 = [config()](`t:config/0`)  
Reason = term()  
...
OldConfig = NewConfig = Config = [config()](`t:config/0`)  
Reason = term()  
...
-doc """
Config = FilteredConfig = [config()](`t:config/0`)  
...
-doc """
LogEvent = [logger:log_event()](`t:logger:log_event/0`)  
Config = [config()](`t:config/0`)  

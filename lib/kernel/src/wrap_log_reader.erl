...
This module makes it possible to read internally formatted wrap disk logs, see `m:disk_log`. `wrap_log_reader` does not interfere with `disk_log` activities; there is however a bug in this version of the `wrap_log_reader`, see section [Known Limitations](`m:wrap_log_reader#bugs`).

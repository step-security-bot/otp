...
Each Logger handler has a configured formatter specified as a module and a configuration term. The purpose of the formatter is to translate the log events to a final printable string ([`unicode:chardata()` ](`t:unicode:chardata/0`)) which can be written to the output device of the handler. See sections [Handlers](logger_chapter.md#handlers) and [Formatters](logger_chapter.md#formatters) in the Kernel User's Guide for more information.

`logger_formatter` is the default formatter used by Logger.

## See Also

`m:calendar`, `m:error_logger`, `m:io`, `m:io_lib`, `m:logger`, `m:maps`, [`sasl(6)`](`p:sasl:sasl_app.md`), `m:unicode`
""".
...
%%% Types
-doc """
The configuration term for `logger_formatter` is a [map](`m:maps`), and the following keys can be set as configuration parameters:

* __`chars_limit = integer() > 0 | unlimited`{: id=chars_limit }__ - A positive integer representing the value of the option with the same name to be used when calling `io_lib:format/3`. This value limits the total number of characters printed for each log event. Notice that this is a soft limit. For a hard truncation limit, see option `max_size`.

  Defaults to `unlimited`.

* __`depth = integer() > 0 | unlimited`{: id=depth }__ - A positive integer representing the maximum depth to which terms shall be printed by this formatter. Format strings passed to this formatter are rewritten. The format controls ~p and ~w are replaced with ~P and ~W, respectively, and the value is used as the depth parameter. For details, see [`io:format/2,3`](`io:format/2`) in STDLIB.

  Defaults to `unlimited`.

* __`legacy_header = boolean()`__ - If set to `true` a header field is added to logger_formatter's part of `Metadata`. The value of this field is a string similar to the header created by the old `m:error_logger` event handlers. It can be included in the log event by adding the list `[logger_formatter,header]` to the template. See the description of the [`template()`](`t:template/0`) type for more information.

  Defaults to `false`.

* __`max_size = integer() > 0 | unlimited`{: id=max_size }__ - A positive integer representing the absolute maximum size a string returned from this formatter can have. If the formatted string is longer, after possibly being limited by `chars_limit` or `depth`, it is truncated.

  Defaults to `unlimited`.

* __`report_cb = `[`logger:report_cb()`](`t:logger:report_cb/0`)__ - A report callback is used by the formatter to transform log messages on report form to a format string and arguments. The report callback can be specified in the metadata for the log event. If no report callback exists in metadata, `logger_formatter` will use `logger:format_report/1` as default callback.
...
* __`single_line = boolean()`__ - If set to `true`, each log event is printed as a single line. To achieve this, `logger_formatter` sets the field width to `0` for all `~p` and `~P` control sequences in the format a string (see `io:format/2`), and replaces all newlines in the message with `", "`. White spaces following directly after newlines are removed. Notice that newlines added by the `template` parameter are not replaced.

  Defaults to `true`.

* __`template = `{: id=template }[`template()`](`t:template/0`)__ - The template describes how the formatted string is composed by combining different data values from the log event. See the description of the [`template()`](`t:template/0`) type for more information about this.

* __`time_designator = byte()`__ - Timestamps are formatted according to RFC3339, and the time designator is the character used as date and time separator.

  Defaults to `$T`.

  The value of this parameter is used as the `time_designator` option to `calendar:system_time_to_rfc3339/2`.

* __`time_offset = integer() | [byte()]`__ - The time offset, either a string or an integer, to be used when formatting the timestamp.
  ...
  Defaults to an empty string, meaning that timestamps are displayed in local time. However, for backwards compatibility, if the SASL configuration parameter [`utc_log`](`p:sasl:sasl_app.md#utc_log`)`=true`, the default is changed to `"Z"`, meaning that timestamps are displayed in UTC.
...
* __`legacy_header = true, single_line = false`__ - Default template: `[[logger_formatter,header],"\n",msg,"\n"]`
  ...
  Notice that all eight levels can occur in the heading, not only `ERROR`, `WARNING` or `INFO` as `m:error_logger` produces. And microseconds are added at the end of the timestamp.

* __`legacy_header = true, single_line = true`__ - Default template: `[[logger_formatter,header],"\n",msg,"\n"]`
...
* __`legacy_header = false, single_line = true`__ - Default template: `[time," ",level,": ",msg,"\n"]`
...
* __`legacy_header = false, single_line = false`__ - Default template: `[time," ",level,":\n",msg,"\n"]`
...
%%% Callbacks
-doc """
FConfig = [formatter_config()](`t:logger:formatter_config/0`)  
Reason = term()  
...
See `m:logger_formatter` for an example implementation. `logger_formatter` is the default formatter used by Logger.
""".
...
-doc """
LogEvent = [log_event()](`t:logger:log_event/0`)  
FConfig = [formatter_config()](`t:logger:formatter_config/0`)  
FormattedLogEntry = [unicode:chardata()](`t:unicode:chardata/0`)  

This callback function is mandatory.

The function can be called by a log handler to convert a log event term to a printable string. The returned value can, for example, be printed as a log entry to the console or a file using [`io:put_chars/1,2`](`io:put_chars/1`).
...
* If the message is on report form, it is converted to `{Format,Args}` by calling the report callback. See section [Log Message](logger_chapter.md#log_message) in the Kernel User's Guide for more information about report callbacks and valid forms of log messages.
* The message size is limited according to the values of configuration parameters [`chars_limit`](`m:logger_formatter#chars_limit`) and [`depth`](`m:logger_formatter#depth`).
* The full log entry is composed according to the [`template`](`m:logger_formatter#template`).
* If the final string is too long, it is truncated according to the value of configuration parameter [`max_size`](`m:logger_formatter#max_size`).
""".
-doc(#{since => <<"OTP 21.0">>}).

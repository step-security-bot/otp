...
The code paths specified by `$OTPROOT`, `ERL_LIBS`, and boot scripts have their listings cached by default (except for ".") since OTP 26.0. The code server will lookup the contents in their directories once and avoid future file system traversals. Therefore modules added to such directories after the Erlang VM boots won't be picked up. You can disable this behaviour by setting `-cache_boot_paths false` or by calling `code:set_path(code:get_path())`.

The functions in this module and the command line options `-pa` and `-pz` are not cached by default. However, many of the functions that manipulate the code path accept the `cache` atom as an optional argument, which will enable caching on selected paths.
...
For more information about old and current code, and how to make a process switch from old to current code, see section Compilation and Code Loading in the [Erlang Reference Manual](`p:system:code_loading.md`).
...
* __`badfile`__ - The object code has an incorrect format or the module name in the object code is not the expected module name.

* __`nofile`__ - No file with object code was found.

* __`not_purged`__ - The object code could not be loaded because an old version of the code already existed.

* __`on_load_failure`__ - The module has an [\-on_load function](`p:system:code_loading.md#on_load`) that failed when it was called.

* __`sticky_directory`__ - The object code resides in a sticky directory.
""".
...
-doc "Returns the object code file extension corresponding to the Erlang machine used, namely `.beam`.".
...
Returns `{module, Module}` if successful, or `{error, Reason}` if loading fails. See [Error Reasons for Code-Loading Functions](`m:code#error_reasons`) for a description of the possible error reasons.
""".
...
-doc "Tries to load a module in the same way as `load_file/1`, unless the module is already loaded. However, in embedded mode it does not load a module that is not already loaded, but returns `{error, embedded}` instead. See [Error Reasons for Code-Loading Functions](`m:code#error_reasons`) for a description of other possible error reasons.".
...
Normally, `Loaded` is the absolute filename `Filename` from which the code is obtained. If the module is preloaded (see [`script(4)`](`p:sasl:script.md`)), `Loaded==preloaded`. If the module is Cover-compiled (see `m:cover`), `Loaded==cover_compiled`.
""".
...
-doc "Returns `true` if `Module` is the name of a module that has been loaded from a sticky directory (in other words: an attempt to reload the module will fail), or `false` if `Module` is not a loaded module or is not sticky.".
...
* __`{error, bad_directory}`__ - If any `Dir` is not a directory name
""".
-doc(#{since => <<"OTP 26.0">>}).
...
* __`false`__ - If the directory is not found

* __`{error, bad_name}`__ - If the argument is invalid
""".
...
* __`{error, bad_name}`__ - If `Name` is not found

* __`{error, bad_directory}`__ - If `Dir` does not exist

* __`{error, {badarg, [Name, Dir]}}`__ - If `Name` or `Dir` is invalid
""".
-doc(#{since => <<"OTP 26.0">>}).
...
Returns `ok` if successful, or `{error,[{Module,Reason}]}` if loading of some modules fails. See [Error Reasons for Code-Loading Functions](`m:code#error_reasons`) for a description of other possible error reasons.
""".
-doc(#{since => <<"OTP 19.0">>}).
...
* __`nofile`__ - No file with object code exists.

* __`on_load_not_allowed`__ - A module contains an [\-on_load function](`p:system:code_loading.md#on_load`).

* __`duplicated`__ - A module is included more than once in `Modules`.

* __`not_purged`__ - The object code cannot be loaded because an old version of the code already exists.

* __`sticky_directory`__ - The object code resides in a sticky directory.

* __`pending_on_load`__ - A previously loaded module contains an `-on_load` function that never finished.
...
-doc """
Prepares to load the modules in the list `Modules`. Finish the loading by calling [finish_loading(Prepared)](`finish_loading/1`).
...
-doc "Searches the code path for `Filename`, a file of arbitrary type. If found, the full name is returned. `non_existing` is returned if the file cannot be found. The function can be useful, for example, to locate application resource files.".
...
For more information about the documentation chunk see [Documentation Storage and Format](eep48_chapter.md) in Kernel's User's Guide.
""".
-doc(#{since => <<"OTP 23.0">>}).
...
-doc "Searches all directories in the code path for module names with identical names and writes a report to `stdout`.".
-spec clash() -> 'ok'.
...
* __`not_loaded`__ - If `Module` is not currently loaded.

* __`loaded`__ - If `Module` is loaded and the object file exists and contains the same code.

* __`removed`__ - If `Module` is loaded but no corresponding object file can be found in the code path.

* __`modified`__ - If `Module` is loaded but the object file contains code with a different MD5 checksum.
...
-doc "Returns the list of all currently loaded modules for which `module_status/1` returns `modified`. See also `all_loaded/0`.".
-doc(#{since => <<"OTP 20.0">>}).

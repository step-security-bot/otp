...
On operating systems with transparent naming (for example, all Unix systems except MacOS X), default is `utf8` if the terminal supports UTF-8, otherwise `latin1`. The default can be overridden using `+fnl` (to force `latin1` mode) or `+fnu` (to force `utf8` mode) when starting [`erl`](`p:erts:erl_cmd.md`).
...
When running in `utf8` mode, functions `list_dir/1` and `read_link/1` never return raw filenames. To return all filenames including raw filenames, use functions [`list_dir_all/1`](`m:file#list_dir_all`) and [`read_link_all/1`](`m:file#read_link_all`).

See also section [Notes About Raw Filenames](`p:stdlib:unicode_usage.md#notes-about-raw-filenames`) in the STDLIB User's Guide.
...
> #### Warning {: class=warning }
> If an error occurs when accessing an open file with module `m:io`, the process handling the file exits. The dead file process can hang if a process tries to access it later. This will be fixed in a future release.
...
%% data types
-doc "See also the documentation of the [`name_all()`](`t:name_all/0`) type.".
...
-doc "As returned by `open/2`; `t:pid()` is a process handling I/O-protocols.".
...
-doc "If VM is in Unicode filename mode, `t:string()` and `t:char()` are allowed to be > 255. See also the documentation of the [`name_all()`](`t:name_all/0`) type.".
-type name()      :: string() | atom() | deep_list().
-doc "If VM is in Unicode filename mode, characters are allowed to be > 255. `RawFilename` is a filename not subject to Unicode translation, meaning that it can contain characters not conforming to the Unicode encoding expected from the file system (that is, non-UTF-8 characters although the VM is started in Unicode filename mode). Null characters (integer value zero) are *not* allowed in filenames (not even at the end).".
-type name_all()  :: string() | atom() | deep_list() | (RawFilename :: binary()).
-doc "An atom that is named from the POSIX error codes used in Unix, and in the runtime libraries of most C compilers.".
-type posix() ::
        'eacces' | 'eagain' |
        'ebadf' | 'ebadmsg' | 'ebusy' |
...
-doc "Given the error reason returned by any function in this module, returns a descriptive string of the error in English.".
...
* __`{ok, Filename}`__ - If the file server of this node is not a slave, the file was opened by the file server of this node (this implies that `Pid` must be a local pid) and the file is not closed. `Filename` is the filename in flat string format.

* __`undefined`__ - In all other cases.

> #### Warning {: class=warning }
> This function is intended for debugging only.
""".
-doc(#{deprecated =>
           <<"file:pid2name/1 is deprecated and will be removed in OTP 27; this functionality is no longer supported">>}).
...
* __`eacces`__ - Missing read permission for one of the parents of the current directory.
""".
...
* __`enotsup`__ - The operating system has no concept of drives.

* __`eacces`__ - The drive does not exist.

* __`einval`__ - The format of `Drive` is invalid.
""".
...
The functions in the module `file` usually treat binaries as raw filenames, that is, they are passed "as is" even when the encoding of the binary does not agree with [`native_name_encoding()`](`native_name_encoding/0`). However, this function expects binaries to be encoded according to the value returned by `native_name_encoding()`.

Typical error reasons are:

* __`enoent`__ - The directory does not exist.

* __`enotdir`__ - A component of `Dir` is not a directory. On some platforms, `enoent` is returned.

* __`eacces`__ - Missing permission for the directory or one of its parents.

* __`badarg`__ - `Dir` has an improper type, such as tuple.

* __`no_translation`__ - `Dir` is a `t:binary()` with characters coded in ISO-latin-1 and the VM is operating with unicode filename encoding.
...
* __`enoent`__ - The file does not exist.

* __`eacces`__ - Missing permission for the file or one of its parents.

* __`eperm`__ - The file is a directory and the user is not superuser.

* __`enotdir`__ - A component of the filename is not a directory. On some platforms, `enoent` is returned instead.

* __`einval`__ - `Filename` has an improper type, such as tuple.
...
* __`eacces`__ - Missing read or write permissions for the parent directories of `Source` or `Destination`. On some platforms, this error is given if either `Source` or `Destination` is open.

* __`eexist`__ - `Destination` is not an empty directory. On some platforms, also given when `Source` and `Destination` are not of the same type.

* __`einval`__ - `Source` is a root directory, or `Destination` is a subdirectory of `Source`.

* __`eisdir`__ - `Destination` is a directory, but `Source` is not.

* __`enoent`__ - `Source` does not exist.

* __`enotdir`__ - `Source` is a directory, but `Destination` is not.

* __`exdev`__ - `Source` and `Destination` are on different file systems.
""".
...
* __`eacces`__ - Missing search or write permissions for the parent directories of `Dir`.

* __`eexist`__ - A file or directory named `Dir` exists already.

* __`enoent`__ - A component of `Dir` does not exist.

* __`enospc`__ - No space is left on the device.

* __`enotdir`__ - A component of `Dir` is not a directory. On some platforms, `enoent` is returned instead.
""".
...
* __`eexist`__ - The directory is not empty.

* __`enoent`__ - The directory does not exist.

* __`enotdir`__ - A component of `Dir` is not a directory. On some platforms, `enoent` is returned instead.

* __`einval`__ - Attempt to delete the current directory. On some platforms, `eacces` is returned instead.
""".
...
* __`ok`__ - The operation completed without errors.

* __`{error, posix()}`__ - An error occurred when accessing or deleting `File`. If some file or directory under `File` could not be deleted, `File` cannot be deleted as it is non-empty, and `{error, eexist}` is returned.
""".
-doc(#{since => <<"OTP 23.0">>}).
...
* __`local`__ - Returns local time.

* __`universal`__ - Returns universal time.

* __`posix`__ - Returns seconds since or before Unix time epoch, which is 1970-01-01 00:00 UTC.

Default is `{time, local}`.

If the option `raw` is set, the file server is not called and only information about local files is returned. Note that this will break this module's atomicity guarantees as it can race with a concurrent call to [`write_file_info/1,2` ](`write_file_info/2`).
...
* __`size = integer() >= 0`__ - Size of file in bytes.

* __`type = device | directory | other | regular`__ - The type of the file. Can also contain `symlink` when returned from [read_link_info/1,2](`read_link_info/1`).

* __`access = read | write | read_write | none`__ - The current system access to the file.

* __`atime = `[`date_time()`](`t:date_time/0`)` | integer() >= 0`__ - The last time the file was read.

* __`mtime = `[`date_time()`](`t:date_time/0`)` | integer() >= 0`__ - The last time the file was written.

* __`ctime = `[`date_time()`](`t:date_time/0`)` | integer() >=0`__ - The interpretation of this time field depends on the operating system. On Unix, it is the last time the file or the `inode` was changed. In Windows, it is the create time.

* __`mode = integer() >= 0`__ - The file permissions as the sum of the following bit values:

  * __`8#00400`__ - read permission: owner

  * __`8#00200`__ - write permission: owner

  * __`8#00100`__ - execute permission: owner

  * __`8#00040`__ - read permission: group

  * __`8#00020`__ - write permission: group

  * __`8#00010`__ - execute permission: group

  * __`8#00004`__ - read permission: other

  * __`8#00002`__ - write permission: other

  * __`8#00001`__ - execute permission: other

  * __`16#800`__ - set user id on execution

  * __`16#400`__ - set group id on execution

  On Unix platforms, other bits than those listed above may be set.

* __`links = integer() >= 0`__ - Number of links to the file (this is always 1 for file systems that have no concept of links).

* __`major_device = integer() >= 0`__ - Identifies the file system where the file is located. In Windows, the number indicates a drive as follows: 0 means A:, 1 means B:, and so on.

* __`minor_device = integer() >= 0`__ - Only valid for character devices on Unix. In all other cases, this field is zero.

* __`inode = integer() >= 0`__ - Gives the `inode` number. On non-Unix file systems, this field is zero.

* __`uid = integer() >= 0`__ - Indicates the owner of the file. On non-Unix file systems, this field is zero.

* __`gid = integer() >= 0`__ - Gives the group that the owner of the file belongs to. On non-Unix file systems, this field is zero.

Typical error reasons:

* __`eacces`__ - Missing search permission for one of the parent directories of the file.
...
-doc """
Works like [`read_file_info/1,2`](`read_file_info/2`) except that if `Name` is a symbolic link, information about the link is returned in the `file_info` record and the `type` field of the record is set to `symlink`.
...
* __`einval`__ - `Name` does not refer to a symbolic link or the name of the file that it refers to does not conform to the expected encoding.

* __`enoent`__ - The file does not exist.

* __`enotsup`__ - Symbolic links are not supported on this platform.
""".
...
* __`einval`__ - `Name` does not refer to a symbolic link.
...
* __`local`__ - Interprets the time set as local.

* __`universal`__ - Interprets it as universal time.

* __`posix`__ - Must be seconds since or before Unix time epoch, which is 1970-01-01 00:00 UTC.
...
* __`ctime = `[`date_time()`](`t:date_time/0`)` | integer() >= 0`__ - On Unix, any value specified for this field is ignored (the "ctime" for the file is set to the current time). On Windows, this field is the new creation time to set for the file.
...
* __`uid = integer() >= 0`__ - Indicates the file owner. Ignored for non-Unix file systems.

* __`gid = integer() >= 0`__ - Gives the group that the file owner belongs to. Ignored for non-Unix file systems.
...
* __`eacces`__ - Missing search or write permissions for `Dir` or one of its parent directories.

* __`enoent`__ - The directory does not exist.

* __`{no_translation, Filename}`__ - `Filename` is a `t:binary()` with characters coded in ISO Latin-1 and the VM was started with parameter `+fnue`.
""".
...
* __`eacces`__ - Missing permission for reading the file, or for searching one of the parent directories.

* __`eisdir`__ - The named file is a directory.

* __`enotdir`__ - A component of the filename is not a directory. On some platforms, `enoent` is returned instead.

* __`enomem`__ - There is not enough memory for the contents of the file.
""".
-doc(#{since => <<"OTP @OTP-18589@">>}).
...
* __`eacces`__ - Missing read or write permissions for the parent directories of `Existing` or `New`.

* __`eexist`__ - `New` already exists.

* __`enotsup`__ - Hard links are not supported on this platform.
""".
...
* __`eperm`__ - User does not have privileges to create symbolic links (`SeCreateSymbolicLinkPrivilege` on Windows).
""".
...
* __`enoent`__ - A component of the filename does not exist.

* __`enotdir`__ - A component of the filename is not a directory. On some platforms, `enoent` is returned instead.

* __`enospc`__ - No space is left on the device.

* __`eacces`__ - Missing permission for writing the file or searching one of the parent directories.
...
-doc "Same as `write_file/2`, but takes a third argument `Modes`, a list of possible modes, see `open/2`. The mode flags `binary` and `write` are implicit, so they are not to be used.".
...
* __`read`__ - The file, which must exist, is opened for reading.

* __`write`__ - The file is opened for writing. It is created if it does not exist. If the file exists and `write` is not combined with `read`, the file is truncated.

* __`append`__ - The file is opened for writing. It is created if it does not exist. Every write operation to a file opened with `append` takes place at the end of the file.

* __`exclusive`__ - The file is opened for writing. It is created if it does not exist. If the file exists, `{error, eexist}` is returned.
...
* __`read_ahead`__ - The same as `{read_ahead, Size}` with a reasonable default value for `Size` (roughly some 64 KB).

* __`compressed`__ - Makes it possible to read or write gzip compressed files. Option `compressed` must be combined with `read` or `write`, but not both. Notice that the file size obtained with `read_file_info/1` does probably not match the number of bytes that can be read from a compressed file.

* __`compressed_one`__ - Read one member of a gzip compressed file. Option `compressed_one` can only be combined with `read`.

* __`{encoding, Encoding}`__ - Makes the file perform automatic translation of characters to and from a specific (Unicode) encoding. Notice that the data supplied to `write/2` or returned by `read/2` still is byte-oriented; this option denotes only how data is stored in the disk file.

  Depending on the encoding, different methods of reading and writing data is preferred. The default encoding of `latin1` implies using this module (`file`) for reading and writing data as the interfaces provided here work with byte-oriented data. Using other (Unicode) encodings makes the `m:io` functions `get_chars`, `get_line`, and `put_chars` more suitable, as they can work with the full Unicode range.

  If data is sent to an `t:io_device()` in a format that cannot be converted to the specified encoding, or if data is read by a function that returns data in a format that cannot cope with the character range of the data, an error occurs and the file is closed.

  Allowed values for `Encoding`:

  * __`latin1`__ - The default encoding. Bytes supplied to the file, that is, `write/2` are written "as is" on the file. Likewise, bytes read from the file, that is, `read/2` are returned "as is". If module `m:io` is used for writing, the file can only cope with Unicode characters up to code point 255 (the ISO Latin-1 range).

  * __`unicode or utf8`__ - Characters are translated to and from UTF-8 encoding before they are written to or read from the file. A file opened in this way can be readable using function `read/2`, as long as no data stored on the file lies beyond the ISO Latin-1 range (0..255), but failure occurs if the data contains Unicode code points beyond that range. The file is best read with the functions in the Unicode aware module `m:io`.

    Bytes written to the file by any means are translated to UTF-8 encoding before being stored on the disk file.

  * __`utf16 or {utf16,big}`__ - Works like `unicode`, but translation is done to and from big endian UTF-16 instead of UTF-8.

  * __`{utf16,little}`__ - Works like `unicode`, but translation is done to and from little endian UTF-16 instead of UTF-8.

  * __`utf32 or {utf32,big}`__ - Works like `unicode`, but translation is done to and from big endian UTF-32 instead of UTF-8.

  * __`{utf32,little}`__ - Works like `unicode`, but translation is done to and from little endian UTF-32 instead of UTF-8.

  The Encoding can be changed for a file "on the fly" by using function `io:setopts/2`. So a file can be analyzed in latin1 encoding for, for example, a BOM, positioned beyond the BOM and then be set for the right encoding before further reading. For functions identifying BOMs, see module `m:unicode`.

  This option is not allowed on `raw` files.

* __`ram`__ - `File` must be `t:iodata()`. Returns an `t:fd()`, which lets module `file` operate on the data in-memory as if it is a file.

* __`sync`__ - On platforms supporting it, enables the POSIX `O_SYNC` synchronous I/O flag or its platform-dependent equivalent (for example, `FILE_FLAG_WRITE_THROUGH` on Windows) so that writes to the file block until the data is physically written to disk. However, be aware that the exact semantics of this flag differ from platform to platform. For example, none of Linux or Windows guarantees that all file metadata are also written before the call returns. For precise semantics, check the details of your platform documentation. On platforms with no support for POSIX `O_SYNC` or equivalent, use of the `sync` flag causes `open` to return `{error, enotsup}`.

* __`directory`__ - Allows `open` to work on directories.

Returns:

* __`{ok, IoDevice}`__ - The file is opened in the requested mode. `IoDevice` is a reference to the file.

* __`{error, Reason}`__ - The file cannot be opened.

`IoDevice` is really the pid of the process that handles the file. This process monitors the process that originally opened the file (the owner process). If the owner process terminates, the file is closed and the process itself terminates too. An `IoDevice` returned from this call can be used as an argument to the I/O functions (see `m:io`).
...
* __`eacces`__ - Missing permission for reading the file or searching one of the parent directories.

* __`eisdir`__ - The named file is a directory.

* __`enotdir`__ - A component of the filename is not a directory, or the filename itself is not a directory if `directory` mode was specified. On some platforms, `enoent` is returned instead.

* __`enospc`__ - There is no space left on the device (if `write` access was specified).
""".
...
* __`{ok, Data}`__ - If the file was opened in binary mode, the read bytes are returned in a binary, otherwise in a list. The list or binary is shorter than the number of bytes requested if end of file was reached.

* __`eof`__ - Returned if `Number>0` and end of file was reached before anything at all could be read.

* __`{error, Reason}`__ - An error occurred.

Typical error reasons:

* __`ebadf`__ - The file is not opened for reading.

* __`{no_translation, unicode, latin1}`__ - The file is opened with another `encoding` than `latin1` and the data in the file cannot be translated to the byte-oriented data that this function returns.
""".
...
* __`{ok, Data}`__ - One line from the file is returned, including the trailing LF, but with CRLF sequences replaced by a single LF (see above).

  If the file is opened in binary mode, the read bytes are returned in a binary, otherwise in a list.

* __`eof`__ - Returned if end of file was reached before anything at all could be read.
...
* __`{no_translation, unicode, latin1}`__ - The file is opened with another `encoding` than `latin1` and the data on the file cannot be translated to the byte-oriented data that this function returns.
""".
...
* __`enospc`__ - Not enough space left to write the file.
""".
...
* __`Offset`__ - The same as `{bof, Offset}`.

* __`{bof, Offset}`__ - Absolute offset.

* __`{cur, Offset}`__ - Offset from the current position.

* __`{eof, Offset}`__ - Offset from the end of file.

* __`bof | cur | eof`__ - The same as above with `Offset` 0.
...
* __`einval`__ - Either `Location` is illegal, or it is evaluated to a negative offset in the file. Notice that if the resulting position is a negative value, the result is an error, and after the call the file position is undefined.
""".
...
-doc "Truncates the file referenced by `IoDevice` at the current position. Returns `ok` if successful, otherwise `{error, Reason}`.".
...
* __`{ok, Terms}`__ - The file was successfully read.

* __`{error, atom()}`__ - An error occurred when opening the file or reading it. For a list of typical error codes, see `open/2`.

* __`{error, {Line, Mod, Term}}`__ - An error occurred when interpreting the Erlang terms in the file. To convert the three-element tuple to an English description of the error, use `format_error/1`.
...
The encoding of `Filename` can be set by a comment, as described in [`epp(3)`](`m:epp#encoding`).
""".
...
* __`{ok, Terms, FullName}`__ - The file is successfully read. `FullName` is the full name of the file.

* __`{error, enoent}`__ - The file cannot be found in any of the directories in `Path`.

* __`{error, atom()}`__ - An error occurred when opening the file or reading it. For a list of typical error codes, see `open/2`.

* __`{error, {Line, Mod, Term}}`__ - An error occurred when interpreting the Erlang terms in the file. Use `format_error/1` to convert the three-element tuple to an English description of the error.
...
* __`ok`__ - The file was read and evaluated.

* __`{error, atom()}`__ - An error occurred when opening the file or reading it. For a list of typical error codes, see `open/2`.

* __`{error, {Line, Mod, Term}}`__ - An error occurred when interpreting the Erlang expressions in the file. To convert the three-element tuple to an English description of the error, use `format_error/1`.
...
-doc "The same as `eval/1`, but the variable bindings `Bindings` are used in the evaluation. For information about the variable bindings, see `m:erl_eval`.".
...
* __`{ok, FullName}`__ - The file is read and evaluated. `FullName` is the full name of the file.

* __`{error, enoent}`__ - The file cannot be found in any of the directories in `Path`.

* __`{error, atom()}`__ - An error occurred when opening the file or reading it. For a list of typical error codes, see `open/2`.

* __`{error, {Line, Mod, Term}}`__ - An error occurred when interpreting the Erlang expressions in the file. Use `format_error/1` to convert the three-element tuple to an English description of the error.
...
* __`{ok, Value}`__ - The file is read and evaluated. `Value` is the value of the last expression.
...
-doc "The same as `script/1` but the variable bindings `Bindings` are used in the evaluation. See `m:erl_eval` about variable bindings.".
...
* __`{ok, Value, FullName}`__ - The file is read and evaluated. `FullName` is the full name of the file and `Value` the value of the last expression.
...
-doc "The same as `path_script/2` but the variable bindings `Bindings` are used in the evaluation. See `m:erl_eval` about variable bindings.".
...
* __`{ok, IoDevice, FullName}`__ - The file is opened in the requested mode. `IoDevice` is a reference to the file and `FullName` is the full name of the file.

* __`{error, enoent}`__ - The file cannot be found in any of the directories in `Path`.

* __`{error, atom()}`__ - The file cannot be opened.
""".
...
-doc "Changes the modification and last access times of a file. See `write_file_info/2`.".
...
The file used must be opened using the `raw` flag, and the process calling `sendfile` must be the controlling process of the socket. See `gen_tcp:controlling_process/2` or module [`socket`'s](`socket:setopt/3`) [level `otp` socket option ](`t:socket:otp_socket_option/0`)`controlling_process`.
...
* __`chunk_size`__ - The chunk size used by the Erlang fallback to send data. If using the fallback, set this to a value that comfortably fits in the systems memory. Default is 20 MB.
""".
-doc(#{since => <<"OTP R15B">>}).
...
%% sendfile/2
-doc "Sends the file `Filename` to `Socket`. Returns `{ok, BytesSent}` if successful, otherwise `{error, Reason}`.".
-doc(#{since => <<"OTP R15B">>}).

...
This is a handler for Logger that offers circular (wrapped) logs by using `m:disk_log`. Multiple instances of this handler can be added to Logger, and each instance prints to its own disk log file, created with the name and settings specified in the handler configuration.

The default standard handler, `m:logger_std_h`, can be replaced by a disk_log handler at startup of the Kernel application. See an example of this below.
...
* __`file`__ - This is the full name of the disk log file. The option corresponds to the `name` property in the [`dlog_option()`](`disk_log:open/1`) datatype.
...
* __`type`__ - This is the disk log type, `wrap` or `halt`. The option corresponds to the `type` property in the [`dlog_option()`](`disk_log:open/1`) datatype.

  The value is set when the handler is added, and it cannot be changed in runtime.

  Defaults to `wrap`.

* __`max_no_files`__ - This is the maximum number of files that disk_log uses for its circular logging. The option corresponds to the `MaxNoFiles` element in the `size` property in the [`dlog_option()`](`disk_log:open/1`) datatype.
...
* __`max_no_bytes`__ - This is the maximum number of bytes that is written to a log file before disk_log proceeds with the next file in order, or generates an error in case of a full halt log. The option corresponds to the `MaxNoBytes` element in the `size` property in the [`dlog_option()`](`disk_log:open/1`) datatype.

  The value is set when the handler is added, and it cannot be changed in runtime.

  Defaults to `1048576` bytes for a wrap log, and `infinity` for a halt log.

* __`filesync_repeat_interval`__ - This value, in milliseconds, specifies how often the handler does a disk_log sync operation to write buffered data to disk. The handler attempts the operation repeatedly, but only performs a new sync if something has actually been logged.

  Defaults to `5000` milliseconds.

  If `no_repeat` is set as value, the repeated sync operation is disabled. The user can also call the [`filesync/1` ](`filesync/1`)function to perform a disk_log sync.

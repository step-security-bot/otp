...
An application is defined by an *application specification*. The specification is normally located in an *application resource file* named `Application.app`, where `Application` is the application name. For details about the application specification, see [`app(4)`](app.md).
...
For details about applications and behaviours, see [OTP Design Principles](`p:system:design_principles.md`).

## See Also

[OTP Design Principles](`p:system:design_principles.md`), [kernel(6)](kernel_app.md), [app(4)](app.md)
""".
...
-doc """
StartType = [`start_type()`](`t:start_type/0`)  
StartArgs = term()  
Pid = pid()  
State = term()  
...
`StartType`{: id=start_type } defines the type of start:
...
The function is to return `{ok,Pid}` or `{ok,Pid,State}`, where `Pid` is the pid of the top supervisor and `State` is any term. If omitted, `State` defaults to `[]`. If the application is stopped later, `State` is passed to [`Module:prep_stop/1`](`c:prep_stop/1`).
""".
...
This function is called whenever an application has stopped. It is intended to be the opposite of [`Module:start/2`](`c:start/2`) and is to do any necessary cleaning up. The return value is ignored.

`State` is the return value of [`Module:prep_stop/1`](`c:prep_stop/1`), if such a function exists. Otherwise `State` is taken from the return value of [`Module:start/2`](`c:start/2`).
""".
-doc(#{title => <<"Callback Module">>}).
...
`State` is the state returned from [`Module:start/2`](`c:start/2`), or `[]` if no state was returned. `NewState` is any term and is passed to [`Module:stop/1`](`c:stop/1`).

The function is optional. If it is not defined, the processes are terminated and then [`Module:stop(State)`](`c:stop/1`) is called.
""".
-doc(#{title => <<"Callback Module">>}).
...
The application specification can also be specified directly as a tuple `AppSpec`, having the format and contents as described in [`app(4)`](app.md).
...
-doc "Unloads the application specification for `Application` from the application controller. It also unloads the application specifications for any included applications. Notice that the function does not purge the Erlang object code.".
...
-doc """
`Applications` is either an an `t:atom()` or a list of `t:atom()` representing multiple applications.

This function is equivalent to calling [`start/1,2`](`start/1`) repeatedly on all dependencies that are not yet started of each application. Optional dependencies will also be loaded and started if they are available.

The `Mode` argument controls if the applications should be started in `serial` mode (one at a time) or `concurrent` mode. In concurrent mode, a dependency graph is built and the leaves of the graph are started concurrently and recursively. In both modes, no assertion can be made about the order the applications are started. If not supplied, it defaults to `serial`.
...
The function reports `{error, {AppName,Reason}}` for errors, where `Reason` is any possible reason returned by [`start/1,2`](`start/1`) when starting a specific dependency.
...
The application master starts the application by calling the application callback function [`Module:start/2`](`c:start/2`) as defined by the application specification key `mod`.
...
* If a transient application terminates:
  * with `Reason == normal`, this is reported but no other applications are terminated.
...
-doc "Equivalent to [`start/1,2`](`start/1`) except it returns `ok` for already started applications.".
-doc(#{since => <<"OTP R16B01">>}).
...
-doc """
Takes over the distributed application `Application`, which executes at another node `Node`. At the current node, the application is restarted by calling [`Module:start({takeover,Node},StartArgs)`](`c:start/2`). `Module` and `StartArgs` are retrieved from the loaded application specification. The application at the other node is not stopped until the startup is completed, that is, when [`Module:start/2`](`c:start/2`) and any calls to [`Module:start_phase/3`](`c:start_phase/3`) have returned.
...
-doc """
Stops `Application`. The application master calls [`Module:prep_stop/1`](`c:prep_stop/1`), if such a function is defined, and then tells the top supervisor of the application to shut down (see `m:supervisor`). This means that the entire supervision tree, including included applications, is terminated in reversed start order. After the shutdown, the application master calls [`Module:stop/1`](`c:stop/1`). `Module` is the callback module as defined by the application specification key `mod`.
...
-doc "Returns a list with information about the applications, and included applications, which are loaded using `load/1,2`. `Application` is the application name. `Description` and `Vsn` are the values of their `description` and `vsn` application specification keys, respectively.".
...
-doc """
Sets the configuration `Config` for multiple applications. It is equivalent to calling `set_env/4` on each application individually, except it is more efficient. The given `Config` is validated before the configuration is set.
...
-doc "Works like `get_env/2` but returns value `Def` when configuration parameter `Par` does not exist.".
-doc(#{since => <<"OTP R16B">>}).
...
If the specified application does not exist or does not define a callback module, the function returns `undefined`.
""".
-doc(#{since => <<"OTP 26.0">>}).
...
For a description of `StartType`, see [`Module:start/2`](`m:application#start_type`).

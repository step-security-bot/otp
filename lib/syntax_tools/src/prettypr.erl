...
To get started, you should read about the [`document()`](`t:document/0`) data type; the main constructor functions: `text/1`, `above/2`, `beside/2`, `nest/2`, `sep/1`, and `par/2`; and the main layout function `format/3`.
...
Note that this is just a utility function, which does all the work of splitting the given string into words separated by whitespace and setting up a [`par`](`par/2`) with the proper indentation, containing a list of [`text`](`text/1`) elements.
...
-doc """
Yields the empty document, which has neither height nor width. (`empty` is thus different from an empty [`text`](`text/1`) string, which has zero width but height 1.)
...
-doc "Forces a line break at the end of the given document. This is a utility function; see `empty/0` for details.".
...
-doc "Indents a document a number of character positions to the right. Note that `N` may be negative, shifting the text to the left, or zero, in which case `D` is returned unchanged.".
...
-doc """
Computes a layout for a document and returns the corresponding text. See [`document()`](`t:document/0`) for further information. Throws `no_layout` if no layout could be selected.

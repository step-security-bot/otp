...
This class is used for a variety of menu-related events. Note that these do not include menu command events, which are handled using `m:wxCommandEvent` objects.

Events of this class are generated by both menus that are part of a `m:wxMenuBar`, attached to `m:wxFrame`, and popup menus shown by `wxWindow:popupMenu/4`. They are sent to the following objects until one of them handles the event: `-# The menu object itself, as returned by GetMenu(), if any. -# The wxMenuBar to which this menu is attached, if any. -# The window associated with the menu, e.g. the one calling PopupMenu() for the popup menus. -# The top level parent of that window if it's different from the window itself.`

This is similar to command events generated by the menu items, but, unlike them, `m:wxMenuEvent` are only sent to the window itself and its top level parent but not any intermediate windows in the hierarchy.

The default handler for `wxEVT_MENU_HIGHLIGHT` in `m:wxFrame` displays help text in the status bar, see `wxFrame:setStatusBarPane/2`.
...
wxWidgets docs: [wxMenuEvent](https://docs.wxwidgets.org/3.1/classwx_menu_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxMenuEventType`](`t:wxMenuEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
Note that the returned value can be NULL if the menu being opened doesn't have a corresponding `m:wxMenu`, e.g. this happens when opening the system menu in wxMSW port.

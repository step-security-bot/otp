...
See: [Overview events](https://docs.wxwidgets.org/3.1/overview_events.html#overview_events), `wxApp::IsActive` (not implemented in wx)

This class is derived (and can use functions) from: `m:wxEvent`

wxWidgets docs: [wxActivateEvent](https://docs.wxwidgets.org/3.1/classwx_activate_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxActivateEventType`](`t:wxActivateEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
-export([getActive/1]).

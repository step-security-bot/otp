...
wxWidgets docs: [wxGraphicsContext](https://docs.wxwidgets.org/3.1/classwx_graphics_context.html)
""".
-include("wxe.hrl").
...
Prefer to use the overload taking `wxGraphicsPenInfo` (not implemented in wx) unless you already have a `m:wxPen` as constructing one only to pass it to this method is wasteful.
""".
...
-doc "Fills the `widths` array with the widths from the beginning of `text` to the corresponding character of `text`.".

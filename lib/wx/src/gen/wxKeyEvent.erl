...
Notice that the first rule applies to all Unicode letters, not just the usual Latin-1 ones. However for non-Latin-1 letters only `getUnicodeKey/1` can be used to retrieve the key code as `getKeyCode/1` just returns `WXK_NONE` in this case.
...
wxWidgets docs: [wxKeyEvent](https://docs.wxwidgets.org/3.1/classwx_key_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxKeyEventType`](`t:wxKeyEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
The flags are platform-dependent and should only be used if the functionality provided by other `m:wxKeyEvent` methods is insufficient.

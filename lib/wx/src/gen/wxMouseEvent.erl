...
wxWidgets docs: [wxMouseEvent](https://docs.wxwidgets.org/3.1/classwx_mouse_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxMouseEventType`](`t:wxMouseEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
-doc "Returns the logical mouse position in pixels (i.e. translated according to the translation set for the DC, which usually indicates that the window has been scrolled).".
...
-doc "Returns true if the event was a mouse button event (not necessarily a button down event - that may be tested using `buttonDown/2`).".
...
-doc "Returns true if the system has been setup to do page scrolling with the mouse wheel instead of line scrolling.".

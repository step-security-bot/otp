...
`m:wxWindow` is the base class for all windows and represents any visible object on screen. All controls, top level windows and so on are windows. Sizers and device contexts are not, however, as they don't appear on screen themselves.
...
See: [Overview events](https://docs.wxwidgets.org/3.1/overview_events.html#overview_events), [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxwindow.html#wxwindowwxwindow">external documentation</a>.
-doc "Constructs a window, which can be a child of a frame, dialog or any other non-control window.".
...
The non-default constructor of `m:wxWindow` class does two things: it initializes the C++ object and it also creates the window object in the underlying graphical toolkit. The `create/4` method can be used to perform the second part later, while the default constructor can be used to perform the first part only.
...
Any application which captures the mouse in the beginning of some operation must handle `m:wxMouseCaptureLostEvent` and cancel this operation when it receives the event. The event handler must not recapture mouse.
...
Notice that this uses `m:wxClientDC` to draw on the window and the results of doing it while also drawing on `m:wxPaintDC` for this window are undefined. Hence this method shouldn't be used from EVT_PAINT handlers, just use `wxDC:clear/1` on the `m:wxPaintDC` you already use there instead.
""".
...
-doc """
This function simply generates a `m:wxCloseEvent` whose handler usually tries to close the window.
...
Remark: Note that this is a static function, so it can be called without needing a `m:wxWindow` pointer.
...
See: [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
This method is useful for visual appearance optimization (for example, it is a good idea to use it before doing many large text insertions in a row into a `m:wxTextCtrl` under wxGTK) but is not implemented on all platforms nor for all controls so it is mostly just a hint to wxWidgets and not a mandatory directive.
...
For example, for a static control, it will be the minimal size such that the control label is not truncated. For windows containing subwindows (typically `m:wxPanel`), the size returned by this function will be the same as the size the window would have had after calling `fit/1`.
...
See: `cacheBestSize/2`, [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
See: `hasCapture/1`, `captureMouse/1`, `releaseMouse/1`, `m:wxMouseCaptureLostEvent`, `m:wxMouseCaptureChangedEvent`
""".
...
See: `setDropTarget/2`, [Overview dnd](https://docs.wxwidgets.org/3.1/overview_dnd.html#overview_dnd)
""".
...
See: `setId/2`, [Overview windowids](https://docs.wxwidgets.org/3.1/overview_windowids.html#overview_windowids)
""".
...
See: `GetMaxClientSize()` (not implemented in wx), [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
See: `GetMinClientSize()` (not implemented in wx), [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
Returns the region specifying which parts of the window have been damaged. Should only be called within an `m:wxPaintEvent` handler.
...
See: `captureMouse/1`, `releaseMouse/1`, `m:wxMouseCaptureLostEvent`, `m:wxMouseCaptureChangedEvent`
""".
...
-doc """
Returns true if this window background is transparent (as, for example, for `m:wxStaticText`) and should show the parent window background.
...
-doc "Sends an `wxEVT_INIT_DIALOG` event, whose handler usually transfers data to the dialog via validators.".
...
Remark: Implementations of `setSize/6` can also implicitly implement the `move/4` function, which is defined in the base `m:wxWindow` class as the call:
...
Remark: Implementations of SetSize can also implicitly implement the `move/4` function, which is defined in the base `m:wxWindow` class as the call:
...
-doc "Same as `moveAfterInTabOrder/2` except that it inserts this window just before `win` instead of putting it right after it.".
...
Remark: You may wish to call this from a text control custom keypress handler to do the default navigation behaviour for the tab key, since the standard default behaviour for a multiline text control with the wxTE_PROCESS_TAB style is to insert a tab and not navigate to the next control. See also `m:wxNavigationKeyEvent` and HandleAsNavigationKey.
""".
...
See: `captureMouse/1`, `hasCapture/1`, `releaseMouse/1`, `m:wxMouseCaptureLostEvent`, `m:wxMouseCaptureChangedEvent`
""".
...
Remark: This function is currently only implemented under MSW and `m:wxTextCtrl` under wxGTK (it also works for `wxScrolled` (not implemented in wx) classes under all platforms).
...
See: `getBackgroundColour/1`, `setForegroundColour/2`, `getForegroundColour/1`, `clearBackground/1`, `refresh/2`, `m:wxEraseEvent`, `m:wxSystemSettings`
""".
...
-doc """
Used by `m:wxSizer` internally to notify the window about being managed by the given sizer.
...
See: `SetMaxClientSize()` (not implemented in wx), [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
See: `SetMinClientSize()` (not implemented in wx), [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
See: `getDropTarget/1`, [Overview dnd](https://docs.wxwidgets.org/3.1/overview_dnd.html#overview_dnd)
""".
...
See: `HasFocus()` (not implemented in wx), `m:wxFocusEvent`, `setFocus/1`, `wxPanel:setFocusIgnoringChildren/1`
""".
...
Please notice that the given font is not automatically used for `m:wxPaintDC` objects associated with this window, you need to call `wxDC:setFont/2` too. However this font is used by any standard controls for drawing their text as well as by `getTextExtent/3`.
...
See: `getId/1`, [Overview windowids](https://docs.wxwidgets.org/3.1/overview_windowids.html#overview_windowids)
""".
...
See: `setScrollbar/6`, `getScrollPos/2`, `getScrollThumb/2`, `m:wxScrollBar`, `wxScrolled` (not implemented in wx)
""".
...
See: `move/4`, [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
%% 
-doc """
Use of this function for windows which are not toplevel windows (such as `m:wxDialog` or `m:wxFrame`) is discouraged.

Please use `setMinSize/2` and `setMaxSize/2` instead.

See: `setSizeHints/4`, [Overview windowsizing](https://docs.wxwidgets.org/3.1/overview_windowsizing.html#overview_windowsizing)
""".
...
The base class version returns false, but this method is overridden in `m:wxControl` where it returns true.
""".
...
Notice that the default state of newly created top level windows is hidden (to allow you to create their contents without flicker) unlike for all the other, not derived from `m:wxTopLevelWindow`, windows that are by default created in the shown state.
...
The particular implementation depends on the window; for example a `m:wxToolBar` will send an update UI event for each toolbar button, and a `m:wxFrame` will send an update UI event for each menubar menu item.
...
See: `m:wxUpdateUIEvent`, `DoUpdateWindowUI()` (not implemented in wx), `OnInternalIdle()` (not implemented in wx)
""".
...
This function can be used to portably determine the number of physical pixels in a window of the given size, by multiplying the window size by the value returned from it. I.e. it returns the factor converting window coordinates to "content view" coordinates, where the view can be just a simple window displaying a `m:wxBitmap` or `m:wxGLCanvas` or any other kind of window rendering arbitrary "content" on screen.

...
This class represents the events generated by a control (typically a `m:wxTextCtrl` but other windows can generate these events as well) when its content gets copied or cut to, or pasted from the clipboard.
...
Note: These events are currently only generated by `m:wxTextCtrl` in wxGTK and wxOSX but are also generated by `m:wxComboBox` without wxCB_READONLY style in wxMSW.

See: `m:wxClipboard`

This class is derived (and can use functions) from: `m:wxCommandEvent` `m:wxEvent`

wxWidgets docs: [wxClipboardTextEvent](https://docs.wxwidgets.org/3.1/classwx_clipboard_text_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxClipboardTextEventType`](`t:wxClipboardTextEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
-export([]).

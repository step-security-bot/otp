...
See: `wxTimePickerCtrl` (not implemented in wx), `m:wxCalendarCtrl`, `m:wxDateEvent`

This class is derived (and can use functions) from: `m:wxPickerBase` `m:wxControl` `m:wxWindow` `m:wxEvtHandler`

wxWidgets docs: [wxDatePickerCtrl](https://docs.wxwidgets.org/3.1/classwx_date_picker_ctrl.html)
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxdatepickerctrl.html#wxdatepickerctrlwxdatepickerctrl">external documentation</a>.
-doc "Initializes the object and calls `Create()` (not implemented in wx) with all the parameters.".

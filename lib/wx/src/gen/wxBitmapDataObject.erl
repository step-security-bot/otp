...
See: [Overview dnd](https://docs.wxwidgets.org/3.1/overview_dnd.html#overview_dnd), `m:wxDataObject`, `wxDataObjectSimple` (not implemented in wx), `m:wxFileDataObject`, `m:wxTextDataObject`, `m:wxDataObject`

This class is derived (and can use functions) from: `m:wxDataObject`

wxWidgets docs: [wxBitmapDataObject](https://docs.wxwidgets.org/3.1/classwx_bitmap_data_object.html)
""".
-include("wxe.hrl").
...
You may wish to override this method when offering data on-demand, but this is not required by wxWidgets' internals. Use this method to get data in bitmap form from the `m:wxClipboard`.
""".

...
-doc """
[`glu:build1DMipmapLevels/9`](`build1DMipmapLevels/9`) builds a subset of prefiltered one-dimensional texture maps of decreasing resolutions called a mipmap. This is used for the antialiasing of texture mapped primitives.
...
-doc """
[`glu:build1DMipmaps/6`](`build1DMipmaps/6`) builds a series of prefiltered one-dimensional texture maps of decreasing resolutions called a mipmap. This is used for the antialiasing of texture mapped primitives.
...
-doc """
[`glu:build2DMipmapLevels/10`](`build2DMipmapLevels/10`) builds a subset of prefiltered two-dimensional texture maps of decreasing resolutions called a mipmap. This is used for the antialiasing of texture mapped primitives.
...
-doc """
[`glu:build2DMipmaps/7`](`build2DMipmaps/7`) builds a series of prefiltered two-dimensional texture maps of decreasing resolutions called a mipmap. This is used for the antialiasing of texture-mapped primitives.
...
-doc """
[`glu:build3DMipmapLevels/11`](`build3DMipmapLevels/11`) builds a subset of prefiltered three-dimensional texture maps of decreasing resolutions called a mipmap. This is used for the antialiasing of texture mapped primitives.
...
-doc """
[`glu:build3DMipmaps/8`](`build3DMipmaps/8`) builds a series of prefiltered three-dimensional texture maps of decreasing resolutions called a mipmap. This is used for the antialiasing of texture-mapped primitives.
...
-doc """
[`glu:checkExtension/2`](`checkExtension/2`) returns `?GLU_TRUE` if `ExtName` is supported otherwise `?GLU_FALSE` is returned.
...
-doc """
[`glu:cylinder/6`](`cylinder/6`) draws a cylinder oriented along the `z` axis. The base of the cylinder is placed at `z` = 0 and the top at z=height. Like a sphere, a cylinder is subdivided around the `z` axis into slices and along the `z` axis into stacks.
...
-doc """
[`glu:deleteQuadric/1`](`deleteQuadric/1`) destroys the quadrics object (created with [`glu:newQuadric/0`](`newQuadric/0`)) and frees any memory it uses. Once [`glu:deleteQuadric/1`](`deleteQuadric/1`) has been called, `Quad` cannot be used again.
...
-doc """
[`glu:errorString/1`](`errorString/1`) produces an error string from a GL or GLU error code. The string is in ISO Latin 1 format. For example, [`glu:errorString/1`](`errorString/1`)(`?GLU_OUT_OF_MEMORY`) returns the string `out of memory`.
...
-doc """
[`glu:getString/1`](`getString/1`) returns a pointer to a static string describing the GLU version or the GLU extensions that are supported.
...
-doc """
[`glu:lookAt/9`](`lookAt/9`) creates a viewing matrix derived from an eye point, a reference point indicating the center of the scene, and an `UP` vector.
...
-doc """
[`glu:newQuadric/0`](`newQuadric/0`) creates and returns a pointer to a new quadrics object. This object must be referred to when calling quadrics rendering and control functions. A return value of 0 means that there is not enough memory to allocate the object.
...
-doc """
[`glu:ortho2D/4`](`ortho2D/4`) sets up a two-dimensional orthographic viewing region. This is equivalent to calling `gl:ortho/6` with near=-1 and far=1.
...
-doc """
[`glu:perspective/4`](`perspective/4`) specifies a viewing frustum into the world coordinate system. In general, the aspect ratio in [`glu:perspective/4`](`perspective/4`) should match the aspect ratio of the associated viewport. For example, aspect=2.0 means the viewer's angle of view is twice as wide in `x` as it is in `y`. If the viewport is twice as wide as it is tall, it displays the image without distortion.
...
-doc """
[`glu:pickMatrix/5`](`pickMatrix/5`) creates a projection matrix that can be used to restrict drawing to a small region of the viewport. This is typically useful to determine what objects are being drawn near the cursor. Use [`glu:pickMatrix/5`](`pickMatrix/5`) to restrict drawing to a small region around the cursor. Then, enter selection mode (with `gl:renderMode/1`) and rerender the scene. All primitives that would have been drawn near the cursor are identified and stored in the selection buffer.
...
-doc """
[`glu:project/6`](`project/6`) transforms the specified object coordinates into window coordinates using `Model`, `Proj`, and `View`. The result is stored in `WinX`, `WinY`, and `WinZ`. A return value of `?GLU_TRUE` indicates success, a return value of `?GLU_FALSE` indicates failure.
...
-doc """
[`glu:quadricDrawStyle/2`](`quadricDrawStyle/2`) specifies the draw style for quadrics rendered with `Quad`. The legal values are as follows:
...
-doc """
[`glu:quadricNormals/2`](`quadricNormals/2`) specifies what kind of normals are desired for quadrics rendered with `Quad`. The legal values are as follows:
...
-doc """
[`glu:quadricOrientation/2`](`quadricOrientation/2`) specifies what kind of orientation is desired for quadrics rendered with `Quad`. The `Orientation` values are as follows:
...
-doc """
[`glu:quadricTexture/2`](`quadricTexture/2`) specifies if texture coordinates should be generated for quadrics rendered with `Quad`. If the value of `Texture` is `?GLU_TRUE`, then texture coordinates are generated, and if `Texture` is `?GLU_FALSE`, they are not. The initial value is `?GLU_FALSE`.
...
-doc """
[`glu:scaleImage/9`](`scaleImage/9`) scales a pixel image using the appropriate pixel store modes to unpack data from the source image and pack data into the destination image.
...
-doc """
[`glu:sphere/4`](`sphere/4`) draws a sphere of the given radius centered around the origin. The sphere is subdivided around the `z` axis into slices and along the `z` axis into stacks (similar to lines of longitude and latitude).
...
-doc """
[`glu:unProject/6`](`unProject/6`) maps the specified window coordinates into object coordinates using `Model`, `Proj`, and `View`. The result is stored in `ObjX`, `ObjY`, and `ObjZ`. A return value of `?GLU_TRUE` indicates success; a return value of `?GLU_FALSE` indicates failure.

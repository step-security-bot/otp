...
This class is responsible for actually drawing the cell in the grid. You may pass it to the `m:wxGridCellAttr` (below) to change the format of one given cell or to `wxGrid:setDefaultRenderer/2` to change the view of all cells. This is an abstract class, and you will normally use one of the predefined derived classes or derive your own class from it.

See: `wxGridCellAutoWrapStringRenderer` (not implemented in wx), `m:wxGridCellBoolRenderer`, `wxGridCellDateTimeRenderer` (not implemented in wx), `wxGridCellEnumRenderer` (not implemented in wx), `m:wxGridCellFloatRenderer`, `m:wxGridCellNumberRenderer`, `m:wxGridCellStringRenderer`

wxWidgets docs: [wxGridCellRenderer](https://docs.wxwidgets.org/3.1/classwx_grid_cell_renderer.html)
""".
-include("wxe.hrl").

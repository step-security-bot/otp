...
When wxWidgets needs to display an icon or a bitmap (e.g. in the standard file dialog), it does not use a hard-coded resource but asks `m:wxArtProvider` for it instead. This way users can plug in their own `m:wxArtProvider` class and easily replace standard art with their own version.

All that is needed is to derive a class from `m:wxArtProvider`, override either its `wxArtProvider::CreateBitmap()` (not implemented in wx) and/or its `wxArtProvider::CreateIconBundle()` (not implemented in wx) methods and register the provider with `wxArtProvider::Push()` (not implemented in wx):
...
Every bitmap and icon bundle are known to `m:wxArtProvider` under an unique ID that is used when requesting a resource from it. The ID is represented by the ?wxArtID type and can have one of these predefined values (you can see bitmaps represented by these constants in the page_samples_artprov):
...
The `client` is the entity that calls `m:wxArtProvider`'s `getBitmap/2` or `getIcon/2` function. It is represented by wxClientID type and can have one of these values:

Client ID serve as a hint to `m:wxArtProvider` that is supposed to help it to choose the best looking bitmap. For example it is often desirable to use slightly different icons in menus and toolbars even though they represent the same action (e.g. wxART_FILE_OPEN). Remember that this is really only a hint for `m:wxArtProvider` \- it is common that `getBitmap/2` returns identical bitmap for different client values\!

See: [Examples](https://docs.wxwidgets.org/3.1/page_samples.html#page_samples_artprov for an example of), `m:wxArtProvider`, usage; stock ID list

wxWidgets docs: [wxArtProvider](https://docs.wxwidgets.org/3.1/classwx_art_provider.html)
""".
-include("wxe.hrl").

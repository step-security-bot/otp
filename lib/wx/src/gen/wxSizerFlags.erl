...
Normally, when you add an item to a sizer via `wxSizer:add/4`, you have to specify a lot of flags and parameters which can be unwieldy. This is where `m:wxSizerFlags` comes in: it allows you to specify all parameters using the named methods instead. For example, instead of

you can now write

This is more readable and also allows you to create `m:wxSizerFlags` objects which can be reused for several sizer items.

Note that by specification, all methods of `m:wxSizerFlags` return the `m:wxSizerFlags` object itself to allowing chaining multiple methods calls like in the examples above.

See: `m:wxSizer`

wxWidgets docs: [wxSizerFlags](https://docs.wxwidgets.org/3.1/classwx_sizer_flags.html)
""".
-include("wxe.hrl").
...
-doc "Sets the `m:wxSizerFlags` to have a border with size as returned by `GetDefaultBorder()` (not implemented in wx).".
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxsizerflags.html#wxsizerflagsborder">external documentation</a>.
-doc """
Sets the `m:wxSizerFlags` to have a border of a number of pixels specified by `borderinpixels` with the directions specified by `direction`.

...
Objects of this class manage the print preview process. The object is passed a `m:wxPrintout` object, and the `m:wxPrintPreview` object itself is passed to a `m:wxPreviewFrame` object. Previewing is started by initializing and showing the preview frame. Unlike `wxPrinter:print/4`, flow of control returns to the application immediately after the frame is shown.

Note: The preview shown is only exact on Windows. On other platforms, the `m:wxDC` used for preview is different from what is used for printing and the results may be significantly different, depending on how is the output created. In particular, printing code relying on `wxDC:getTextExtent/3` heavily (for example, `m:wxHtmlEasyPrinting` and other wxHTML classes do) is affected. It is recommended to use native preview functionality on platforms that offer it (macOS, GTK+).

See: [Overview printing](https://docs.wxwidgets.org/3.1/overview_printing.html#overview_printing), `wxPrinterDC` (not implemented in wx), `m:wxPrintDialog`, `m:wxPrintout`, `m:wxPrinter`, `m:wxPreviewCanvas`, `m:wxPreviewControlBar`, `m:wxPreviewFrame`

wxWidgets docs: [wxPrintPreview](https://docs.wxwidgets.org/3.1/classwx_print_preview.html)
""".
-include("wxe.hrl").
...
Remark: Do not explicitly delete the printout objects once this constructor has been called, since they will be deleted in the `m:wxPrintPreview` destructor. The same does not apply to the `data` argument.
...
-doc "Gets the printout object to be used for printing from within the preview interface, or NULL if none exists.".
...
-doc """
Invokes the print process using the second `m:wxPrintout` object supplied in the `m:wxPrintPreview` constructor.

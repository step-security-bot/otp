...
It may be single line or multi-line. Notice that a lot of methods of the text controls are found in the base `wxTextEntry` (not implemented in wx) class which is a common base class for `m:wxTextCtrl` and other controls using a single line text entry field (e.g. `m:wxComboBox`).
...
To summarize: never use the indices returned by (multiline) `m:wxTextCtrl` as indices into the string it contains, but only as arguments to be passed back to the other `m:wxTextCtrl` methods. This problem doesn't arise for single-line platforms however where the indices in the control do correspond to the positions in the value string.
...
Note that even if your build of wxWidgets doesn't support this (the symbol `wxHAS_TEXT_WINDOW_STREAM` has value of 0 then) you can still use `m:wxTextCtrl` itself in a stream-like manner:

However the possibility to create a `std::ostream` associated with `m:wxTextCtrl` may be useful if you need to redirect the output of a function taking a `std::ostream` as parameter to a text control.
...
The following commands are processed by default event handlers in `m:wxTextCtrl`: `wxID_CUT`, `wxID_COPY`, `wxID_PASTE`, `wxID_UNDO`, `wxID_REDO`. The associated UI update events are also processed automatically, when the control has the focus.
...
wxWidgets docs: [wxTextCtrl](https://docs.wxwidgets.org/3.1/classwx_text_ctrl.html)

## Events

Event types emitted from this class: [`command_text_updated`](`m:wxCommandEvent`), [`command_text_enter`](`m:wxCommandEvent`), [`text_maxlen`](`m:wxCommandEvent`)
""".
-include("wxe.hrl").
...
However if the `style` parameter is the default `m:wxTextAttr`, then the default style is just reset (instead of being combined with the new style which wouldn't change it at all).

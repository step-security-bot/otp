...
A `m:wxDataObject` represents data that can be copied to or from the clipboard, or dragged and dropped. The important thing about `m:wxDataObject` is that this is a 'smart' piece of data unlike 'dumb' data containers such as memory buffers or files. Being 'smart' here means that the data object itself should know what data formats it supports and how to render itself in each of its supported formats.
...
So, you have several solutions when you need a `m:wxDataObject` class (and you need one as soon as you want to transfer data via the clipboard or drag and drop):
...
Each class derived directly from `m:wxDataObject` must override and implement all of its functions which are pure virtual in the base class. The data objects which only render their data or only set it (i.e. work in only one direction), should return 0 from `GetFormatCount()` (not implemented in wx).
...
wxWidgets docs: [wxDataObject](https://docs.wxwidgets.org/3.1/classwx_data_object.html)
""".
-include("wxe.hrl").
-export([]).

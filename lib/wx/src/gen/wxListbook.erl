...
`m:wxListbook` is a class similar to `m:wxNotebook` but which uses a `m:wxListCtrl` to show the labels instead of the tabs.

The underlying `m:wxListCtrl` displays page labels in a one-column report view by default. Calling wxBookCtrl::SetImageList will implicitly switch the control to use an icon view.

For usage documentation of this class, please refer to the base abstract class wxBookCtrl. You can also use the page_samples_notebook to see `m:wxListbook` in action.

Styles

This class supports the following styles:

See: ?wxBookCtrl, `m:wxNotebook`, [Examples](https://docs.wxwidgets.org/3.1/page_samples.html#page_samples_notebook)

This class is derived (and can use functions) from: `m:wxBookCtrlBase` `m:wxControl` `m:wxWindow` `m:wxEvtHandler`

wxWidgets docs: [wxListbook](https://docs.wxwidgets.org/3.1/classwx_listbook.html)

## Events

Event types emitted from this class: [`listbook_page_changed`](`m:wxBookCtrlEvent`), [`listbook_page_changing`](`m:wxBookCtrlEvent`)
""".
-include("wxe.hrl").
...
-doc "Create the list book control that has already been constructed with the default constructor.".

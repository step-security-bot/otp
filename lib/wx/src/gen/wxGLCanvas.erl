...
`m:wxGLCanvas` is a class for displaying OpenGL graphics. It is always used in conjunction with `m:wxGLContext` as the context can only be made current (i.e. active for the OpenGL commands) when it is associated to a `m:wxGLCanvas`.
...
Notice that versions of wxWidgets previous to 2.9 used to implicitly create a `m:wxGLContext` inside `m:wxGLCanvas` itself. This is still supported in the current version but is deprecated now and will be removed in the future, please update your code to create the rendering contexts explicitly.
...
See: `m:wxGLContext`, `wxGLAttributes` (not implemented in wx), `wxGLContextAttrs` (not implemented in wx)

This class is derived (and can use functions) from: `m:wxWindow` `m:wxEvtHandler`

wxWidgets docs: [wxGLCanvas](https://docs.wxwidgets.org/3.1/classwx_g_l_canvas.html)
""".
-include("wxe.hrl").

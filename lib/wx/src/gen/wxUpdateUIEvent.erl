...
Note that although events are sent in idle time, defining a `m:wxIdleEvent` handler for a window does not affect this because the events are sent from `wxWindow::OnInternalIdle` (not implemented in wx) which is always called in idle time.
...
wxWidgets docs: [wxUpdateUIEvent](https://docs.wxwidgets.org/3.1/classwx_update_u_i_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxUpdateUIEventType`](`t:wxUpdateUIEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
-doc "Specify how wxWidgets will send update events: to all windows, or only to those which specify that they will process the events.".

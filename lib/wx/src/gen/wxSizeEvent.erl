...
See: \{Width,Height\}, [Overview events](https://docs.wxwidgets.org/3.1/overview_events.html#overview_events)

This class is derived (and can use functions) from: `m:wxEvent`

wxWidgets docs: [wxSizeEvent](https://docs.wxwidgets.org/3.1/classwx_size_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxSizeEventType`](`t:wxSizeEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
This is the new total size of the window, i.e. the same size as would be returned by `wxWindow:getSize/1` if it were called now. Use `wxWindow:getClientSize/1` if you catch this event in a top level window such as `m:wxFrame` to find the size available for the window contents.
""".

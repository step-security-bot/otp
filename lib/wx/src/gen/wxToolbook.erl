...
`m:wxToolbook` is a class similar to `m:wxNotebook` but which uses a `m:wxToolBar` to show the labels instead of the tabs.

There is no documentation for this class yet but its usage is identical to `m:wxNotebook` (except for the features clearly related to tabs only), so please refer to that class documentation for now. You can also use the page_samples_notebook to see `m:wxToolbook` in action.

One feature of this class not supported by `m:wxBookCtrlBase` is the support for disabling some of the pages, see `EnablePage()` (not implemented in wx).

Styles

This class supports the following styles:

See: [Overview bookctrl](https://docs.wxwidgets.org/3.1/overview_bookctrl.html#overview_bookctrl), `m:wxBookCtrlBase`, `m:wxNotebook`, [Examples](https://docs.wxwidgets.org/3.1/page_samples.html#page_samples_notebook)

This class is derived (and can use functions) from: `m:wxBookCtrlBase` `m:wxControl` `m:wxWindow` `m:wxEvtHandler`

wxWidgets docs: [wxToolbook](https://docs.wxwidgets.org/3.1/classwx_toolbook.html)

## Events

Event types emitted from this class: [`toolbook_page_changed`](`m:wxBookCtrlEvent`), [`toolbook_page_changing`](`m:wxBookCtrlEvent`)
""".
-include("wxe.hrl").
...
-doc "Create the tool book control that has already been constructed with the default constructor.".

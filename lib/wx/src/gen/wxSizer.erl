...
See: [Overview sizer](https://docs.wxwidgets.org/3.1/overview_sizer.html#overview_sizer)
...
`m:wxSizer` itself is an abstract class, but the parameters are equivalent in the derived classes that you will instantiate to use it so they are described here:
""".
...
This is commonly done in the constructor of the window itself, see sample in the description of `m:wxBoxSizer`.
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxsizer.html#wxsizerlayout">external documentation</a>.
-doc "Call this to force layout of the children anew, e.g. after having added a child to or removed a child (window, other sizer or space) from the sizer while keeping the current dimension.".
...
%% 		 | {'userData', wx:wx_object()}.<br />
%% 
-doc "Same as `add/4`, but prepends the items to the beginning of the list of items (windows, subsizers or spaces) owned by this sizer.".
...
The detached child is deleted `only` if it is a sizer or a spacer (but not if it is a `m:wxWindow` because windows are owned by their parent window, not the sizer).
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxsizer.html#wxsizersetdimension">external documentation</a>.
-doc "Call this to force the sizer to take the given dimension and thus force the items owned by the sizer to resize themselves according to the rules defined by the parameter in the `add/4` and `prepend/4` methods.".
...
This only makes sense when `window` is actually a `m:wxTopLevelWindow` such as a `m:wxFrame` or a `m:wxDialog`, since SetSizeHints only has any effect in these classes. It does nothing in normal windows or controls.

This method is implicitly used by `wxWindow:setSizerAndFit/3` which is commonly invoked in the constructor of a toplevel window itself (see the sample in the description of `m:wxBoxSizer`) if the toplevel window is resizable.
""".

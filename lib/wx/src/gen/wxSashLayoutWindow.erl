...
`m:wxSashLayoutWindow` responds to OnCalculateLayout events generated by `m:wxLayoutAlgorithm`. It allows the application to use simple accessors to specify how the window should be laid out, rather than having to respond to events.

The fact that the class derives from `m:wxSashWindow` allows sashes to be used if required, to allow the windows to be user-resizable.
...
This class handles the EVT_QUERY_LAYOUT_INFO and EVT_CALCULATE_LAYOUT events for you. However, if you use sashes, see `m:wxSashWindow` for relevant event information. See also `m:wxLayoutAlgorithm` for information about the layout events.

See: `m:wxLayoutAlgorithm`, `m:wxSashWindow`, [Overview events](https://docs.wxwidgets.org/3.1/overview_events.html#overview_events)

This class is derived (and can use functions) from: `m:wxSashWindow` `m:wxWindow` `m:wxEvtHandler`

wxWidgets docs: [wxSashLayoutWindow](https://docs.wxwidgets.org/3.1/classwx_sash_layout_window.html)
""".
-include("wxe.hrl").
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxsashlayoutwindow.html#wxsashlayoutwindowwxsashlayoutwindow">external documentation</a>.
-doc "Constructs a sash layout window, which can be a child of a frame, dialog or any other non-control window.".
...
-doc "Initializes a sash layout window, which can be a child of a frame, dialog or any other non-control window.".
...
-doc "Returns the alignment of the window: one of wxLAYOUT_TOP, wxLAYOUT_LEFT, wxLAYOUT_RIGHT, wxLAYOUT_BOTTOM.".
...
-doc "Returns the orientation of the window: one of wxLAYOUT_HORIZONTAL, wxLAYOUT_VERTICAL.".

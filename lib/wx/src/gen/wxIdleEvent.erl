...
See: [Overview events](https://docs.wxwidgets.org/3.1/overview_events.html#overview_events), `m:wxUpdateUIEvent`, `wxWindow::OnInternalIdle` (not implemented in wx)

This class is derived (and can use functions) from: `m:wxEvent`

wxWidgets docs: [wxIdleEvent](https://docs.wxwidgets.org/3.1/classwx_idle_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxIdleEventType`](`t:wxIdleEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
-doc "Static function for specifying how wxWidgets will send idle events: to all windows, or only to those which specify that they will process the events.".

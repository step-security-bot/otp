...
Menu items may be either `normal` items, `check` items or `radio` items. Normal items don't have any special properties while the check items have a boolean flag associated to them and they show a checkmark in the menu when the flag is set. wxWidgets automatically toggles the flag value when the item is clicked and its value may be retrieved using either `isChecked/2` method of `m:wxMenu` or `m:wxMenuBar` itself or by using wxEvent::IsChecked when you get the menu notification for the item in question.
...
Finally, menu events can also be handled in the associated window, which is either the `m:wxFrame` associated with the menu bar this menu belongs to or the window for which `wxWindow:popupMenu/4` was called for the popup menus.

See overview_events_bind for how to bind event handlers to the various objects.

See: `m:wxMenuBar`, `wxWindow:popupMenu/4`, [Overview events](https://docs.wxwidgets.org/3.1/overview_events.html#overview_events), `wxFileHistory` (not implemented in wx)
...
See: `appendSeparator/1`, `appendCheckItem/4`, `appendRadioItem/4`, `AppendSubMenu()` (not implemented in wx), `insert/6`, `setLabel/3`, `getHelpString/2`, `setHelpString/3`, `m:wxMenuItem`
""".

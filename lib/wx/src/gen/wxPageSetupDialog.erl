...
When the dialog has been closed, you need to query the `m:wxPageSetupDialogData` object associated with the dialog.

Note that the OK and Cancel buttons do not destroy the dialog; this must be done by the application.

See: [Overview printing](https://docs.wxwidgets.org/3.1/overview_printing.html#overview_printing), `m:wxPrintDialog`, `m:wxPageSetupDialogData`

wxWidgets docs: [wxPageSetupDialog](https://docs.wxwidgets.org/3.1/classwx_page_setup_dialog.html)
""".
-include("wxe.hrl").
...
-doc "Shows the dialog, returning `wxID_OK` if the user pressed OK, and `wxID_CANCEL` otherwise.".

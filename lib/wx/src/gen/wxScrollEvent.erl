...
A scroll event holds information about events sent from stand-alone scrollbars (see `m:wxScrollBar`) and sliders (see `m:wxSlider`).

Note that scrolled windows send the `m:wxScrollWinEvent` which does not derive from `m:wxCommandEvent`, but from `m:wxEvent` directly - don't confuse these two kinds of events and use the event table macros mentioned below only for the scrollbar-like controls.
...
See: `m:wxScrollBar`, `m:wxSlider`, `m:wxSpinButton`, `m:wxScrollWinEvent`, [Overview events](https://docs.wxwidgets.org/3.1/overview_events.html#overview_events)

This class is derived (and can use functions) from: `m:wxCommandEvent` `m:wxEvent`

wxWidgets docs: [wxScrollEvent](https://docs.wxwidgets.org/3.1/classwx_scroll_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxScrollEventType`](`t:wxScrollEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").

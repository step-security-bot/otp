...
`m:wxSystemOptions` stores option/value pairs that wxWidgets itself or applications can use to alter behaviour at run-time. It can be used to optimize behaviour that doesn't deserve a distinct API, but is still important to be able to configure.
...
wxWidgets docs: [wxSystemOptions](https://docs.wxwidgets.org/3.1/classwx_system_options.html)
""".
-include("wxe.hrl").

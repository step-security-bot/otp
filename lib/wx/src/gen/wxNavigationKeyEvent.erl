...
This event is mainly used by wxWidgets implementations. A `m:wxNavigationKeyEvent` handler is automatically provided by wxWidgets when you enable keyboard navigation inside a window by inheriting it from wxNavigationEnabled<>.
...
wxWidgets docs: [wxNavigationKeyEvent](https://docs.wxwidgets.org/3.1/classwx_navigation_key_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxNavigationKeyEventType`](`t:wxNavigationKeyEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
-doc "Returns true if the navigation event represents a window change (for example, from Ctrl-Page Down in a notebook).".

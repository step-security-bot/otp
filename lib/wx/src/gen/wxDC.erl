...
A `m:wxDC` is a `"device context"` onto which graphics and text can be drawn. It is intended to represent different output devices and offers a common abstract API for drawing on any of them.

wxWidgets offers an alternative drawing API based on the modern drawing backends GDI+, CoreGraphics, Cairo and Direct2D. See `m:wxGraphicsContext`, `m:wxGraphicsRenderer` and related classes. There is also a `m:wxGCDC` linking the APIs by offering the `m:wxDC` API on top of a `m:wxGraphicsContext`.

`m:wxDC` is an abstract base class and cannot be created directly. Use `m:wxPaintDC`, `m:wxClientDC`, `m:wxWindowDC`, `m:wxScreenDC`, `m:wxMemoryDC` or `wxPrinterDC` (not implemented in wx). Notice that device contexts which are associated with windows (i.e. `m:wxClientDC`, `m:wxWindowDC` and `m:wxPaintDC`) use the window font and colours by default (starting with wxWidgets 2.9.0) but the other device context classes use system-default values so you always must set the appropriate fonts and colours before using them.
...
Beginning with wxWidgets 2.9.0 the entire `m:wxDC` code has been reorganized. All platform dependent code (actually all drawing code) has been moved into backend classes which derive from a common wxDCImpl class. The user-visible classes such as `m:wxClientDC` and `m:wxPaintDC` merely forward all calls to the backend implementation.
...
All `m:wxDC` functions use instead `logical` units, unless where explicitly stated. Logical units are arbitrary units mapped to device units using the current mapping mode (see `setMapMode/2`).
...
On some platforms (currently under MSW, GTK+ 3, macOS) `m:wxDC` has support for applying an arbitrary affine transformation matrix to its coordinate system (since 3.1.1 this feature is also supported by `m:wxGCDC` in all ports). Call `CanUseTransformMatrix()` (not implemented in wx) to check if this support is available and then call `SetTransformMatrix()` (not implemented in wx) if it is. If the transformation matrix is not supported, `SetTransformMatrix()` (not implemented in wx) always simply returns `false` and doesn't do anything.

This feature is only available when `wxUSE_DC_TRANSFORM_MATRIX` build option is enabled.

See: [Overview dc](https://docs.wxwidgets.org/3.1/overview_dc.html#overview_dc), `m:wxGraphicsContext`, `wxDCFontChanger` (not implemented in wx), `wxDCTextColourChanger` (not implemented in wx), `wxDCPenChanger` (not implemented in wx), `wxDCBrushChanger` (not implemented in wx), `wxDCClipper` (not implemented in wx)
...
See: `StretchBlit()` (not implemented in wx), `m:wxMemoryDC`, `m:wxBitmap`, `m:wxMask`
""".
...
-doc "Convert `device` X coordinate to logical coordinate, using the current mapping mode, user scale factor, device origin and axis orientation.".
...
-doc "Converts `device` Y coordinate to logical coordinate, using the current mapping mode, user scale factor, device origin and axis orientation.".
...
This method isn't available for `m:wxPostScriptDC` or `wxMetafileDC` (not implemented in wx) nor for any DC in wxOSX port and simply returns false there.

Note: Setting a pixel can be done using `drawPoint/2`.

Note: This method shouldn't be used with `m:wxPaintDC` as accessing the DC while drawing can result in unexpected results, notably in wxGTK.
""".
...
See: `m:wxFont`, `setFont/2`, `getPartialTextExtents/2`, `getMultiLineTextExtent/3`
""".
...
-doc "Converts logical X coordinate to device coordinate, using the current mapping mode, user scale factor, device origin and axis orientation.".
...
-doc "Converts logical Y coordinate to device coordinate, using the current mapping mode, user scale factor, device origin and axis orientation.".
...
If the argument is ?wxNullBrush (or another invalid brush; see `wxBrush:isOk/1`), the current brush is selected out of the device context (leaving `m:wxDC` without any valid brush), allowing the current brush to be destroyed safely.

See: `m:wxBrush`, `m:wxMemoryDC`, (for the interpretation of colours when drawing into a monochrome bitmap)
""".
...
If the argument is ?wxNullFont (or another invalid font; see `wxFont:isOk/1`), the current font is selected out of the device context (leaving `m:wxDC` without any valid font), allowing the current font to be destroyed safely.
...
If the argument is ?wxNullPen (or another invalid pen; see `wxPen:isOk/1`), the current pen is selected out of the device context (leaving `m:wxDC` without any valid pen), allowing the current pen to be destroyed safely.

See: `m:wxMemoryDC`, for the interpretation of colours when drawing into a monochrome bitmap
""".

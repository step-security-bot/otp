...
`m:wxLocale` class encapsulates all language-dependent settings and is a generalization of the C locale concept.
...
See: [Overview i18n](https://docs.wxwidgets.org/3.1/overview_i18n.html#overview_i18n), [Examples](https://docs.wxwidgets.org/3.1/page_samples.html#page_samples_internat), `wxXLocale` (not implemented in wx), `wxTranslations` (not implemented in wx)
...
-doc "This is the default constructor and it does nothing to initialize the object: `init/3` must be used to do that.".
-spec new() -> wxLocale().
new() ->
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxlocale.html#wxlocaleinit">external documentation</a>.
-doc "Deprecated: This form is deprecated, use the other one unless you know what you are doing.".
...
%% @doc See <a href="http://www.wxwidgets.org/manuals/2.8.12/wx_wxlocale.html#wxlocaleaddcatalog">external documentation</a>.
%%<br /> MsgIdLanguage = integer
-doc "Calls `wxTranslations::AddCatalog(const wxString&, wxLanguage, const wxString&)` (not implemented in wx).".
...
-doc "Returns the current short name for the locale (as given to the constructor or the `init/3` function).".
...
-doc "The destructor, like the constructor, also has global side effects: the previously set locale is restored and so the changes described in `init/3` documentation are rolled back.".

...
This class is derived (and can use functions) from: `m:wxBookCtrlEvent` `m:wxNotifyEvent` `m:wxCommandEvent` `m:wxEvent`

wxWidgets docs: [wxAuiNotebookEvent](https://docs.wxwidgets.org/3.1/classwx_aui_notebook_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxAuiNotebookEventType`](`t:wxAuiNotebookEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
-doc "Returns the page that was selected before the change, `wxNOT_FOUND` if none was selected.".

...
This documents the functions as a brief version of the complete [OpenGL reference pages.](https://www.khronos.org/registry/OpenGL-Refpages/)
""".
...
-doc """
[`gl:clearIndex/1`](`clearIndex/1`) specifies the index used by [`gl:clear/1`](`clear/1`) to clear the color index buffers. `C` is not clamped. Rather, `C` is converted to a fixed-point value with unspecified precision to the right of the binary point. The integer part of this value is then masked with 2 m-1, where m is the number of bits in a color index stored in the frame buffer.
...
-doc """
[`gl:clearColor/4`](`clearColor/4`) specifies the red, green, blue, and alpha values used by [`gl:clear/1`](`clear/1`) to clear the color buffers. Values specified by [`gl:clearColor/4`](`clearColor/4`) are clamped to the range \[0 1].
...
-doc """
In a scene composed entirely of opaque closed surfaces, back-facing polygons are never visible. Eliminating these invisible polygons has the obvious benefit of speeding up the rendering of the image. To enable and disable elimination of back-facing polygons, call [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) with argument `?GL_CULL_FACE`.
...
-doc """
[`gl:pointSize/1`](`pointSize/1`) specifies the rasterized diameter of points. If point size mode is disabled (see [`gl:enable/1`](`enable/1`) with parameter `?GL_PROGRAM_POINT_SIZE`), this value will be used to rasterize points. Otherwise, the value written to the shading language built-in variable gl_PointSize will be used.
...
-doc """
[`gl:polygonMode/2`](`polygonMode/2`) controls the interpretation of polygons for rasterization. `Face` describes which polygons `Mode` applies to: both front and back-facing polygons (`?GL_FRONT_AND_BACK`). The polygon mode affects only the final rasterization of polygons. In particular, a polygon's vertices are lit and the polygon is clipped and possibly culled before these modes are applied.
...
-doc """
Polygon stippling, like line stippling (see [`gl:lineStipple/2`](`lineStipple/2`)), masks out certain fragments produced by rasterization, creating a pattern. Stippling is independent of polygon antialiasing.
...
-doc """
[`gl:scissor/4`](`scissor/4`) defines a rectangle, called the scissor box, in window coordinates. The first two arguments, `X` and `Y`, specify the lower left corner of the box. `Width` and `Height` specify the width and height of the box.
...
-doc """
[`gl:getClipPlane/1`](`getClipPlane/1`) returns in `Equation` the four coefficients of the plane equation for `Plane`.
...
-doc """
When colors are written to the frame buffer, they are written into the color buffers specified by [`gl:drawBuffer/1`](`drawBuffer/1`). One of the following values can be used for default framebuffer:
...
-doc """
[`gl:pushAttrib/1`](`pushAttrib/1`) takes one argument, a mask that indicates which groups of state variables to save on the attribute stack. Symbolic constants are used to set bits in the mask. `Mask` is typically constructed by specifying the bitwise-or of several of these constants together. The special mask `?GL_ALL_ATTRIB_BITS` can be used to save all stackable states.
...
-doc """
[`gl:pushClientAttrib/1`](`pushClientAttrib/1`) takes one argument, a mask that indicates which groups of client-state variables to save on the client attribute stack. Symbolic constants are used to set bits in the mask. `Mask` is typically constructed by specifying the bitwise-or of several of these constants together. The special mask `?GL_CLIENT_ALL_ATTRIB_BITS` can be used to save all stackable client state.
...
-doc """
[`gl:renderMode/1`](`renderMode/1`) sets the rasterization mode. It takes one argument, `Mode`, which can assume one of three predefined values:
...
-doc """
[`gl:getError/0`](`getError/0`) returns the value of the error flag. Each detectable error is assigned a numeric code and symbolic name. When an error occurs, the error flag is set to the appropriate error code value. No other errors are recorded until [`gl:getError/0`](`getError/0`) is called, the error code is returned, and the flag is reset to `?GL_NO_ERROR`. If a call to [`gl:getError/0`](`getError/0`) returns `?GL_NO_ERROR`, there has been no detectable error since the last call to [`gl:getError/0`](`getError/0`), or since the GL was initialized.
...
-doc """
[`gl:finish/0`](`finish/0`) does not return until the effects of all previously called GL commands are complete. Such effects include all changes to GL state, all changes to connection state, and all changes to the frame buffer contents.
...
-doc """
[`gl:depthFunc/1`](`depthFunc/1`) specifies the function used to compare each incoming pixel depth value with the depth value present in the depth buffer. The comparison is performed only if depth testing is enabled. (See [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) of `?GL_DEPTH_TEST`.)
...
-doc """
[`gl:depthMask/1`](`depthMask/1`) specifies whether the depth buffer is enabled for writing. If `Flag` is `?GL_FALSE`, depth buffer writing is disabled. Otherwise, it is enabled. Initially, depth buffer writing is enabled.
...
-doc """
[`gl:clearAccum/4`](`clearAccum/4`) specifies the red, green, blue, and alpha values used by [`gl:clear/1`](`clear/1`) to clear the accumulation buffer.
...
-doc """
[`gl:matrixMode/1`](`matrixMode/1`) sets the current matrix mode. `Mode` can assume one of four values:
...
-doc """
[`gl:ortho/6`](`ortho/6`) describes a transformation that produces a parallel projection. The current matrix (see [`gl:matrixMode/1`](`matrixMode/1`)) is multiplied by this matrix and the result replaces the current matrix, as if [`gl:multMatrix()`](`multMatrixd/1`) were called with the following matrix as its argument:
...
-doc """
[`gl:frustum/6`](`frustum/6`) describes a perspective matrix that produces a perspective projection. The current matrix (see [`gl:matrixMode/1`](`matrixMode/1`)) is multiplied by this matrix and the result replaces the current matrix, as if [`gl:multMatrix()`](`multMatrixd/1`) were called with the following matrix as its argument:
...
-doc """
[`gl:viewport/4`](`viewport/4`) specifies the affine transformation of x and y from normalized device coordinates to window coordinates. Let (x nd y nd) be normalized device coordinates. Then the window coordinates (x w y w) are computed as follows:
...
-doc """
[`gl:loadIdentity/0`](`loadIdentity/0`) replaces the current matrix with the identity matrix. It is semantically equivalent to calling [`gl:loadMatrix()`](`loadMatrixd/1`) with the identity matrix
...
-doc """
[`gl:loadMatrix()`](`loadMatrixd/1`) replaces the current matrix with the one whose elements are specified by `M`. The current matrix is the projection matrix, modelview matrix, or texture matrix, depending on the current matrix mode (see [`gl:matrixMode/1`](`matrixMode/1`)).
...
-doc """
[`gl:multMatrix()`](`multMatrixd/1`) multiplies the current matrix with the one specified using `M`, and replaces the current matrix with the product.
...
-doc """
[`gl:scale()`](`scaled/3`) produces a nonuniform scaling along the `x`, `y`, and `z` axes. The three parameters indicate the desired scale factor along each of the three axes.
...
-doc """
[`gl:translate()`](`translated/3`) produces a translation by (x y z). The current matrix (see [`gl:matrixMode/1`](`matrixMode/1`)) is multiplied by this translation matrix, with the product replacing the current matrix, as if [`gl:multMatrix()`](`multMatrixd/1`) were called with the following matrix for its argument:
...
-doc """
[`gl:isList/1`](`isList/1`) returns `?GL_TRUE` if `List` is the name of a display list and returns `?GL_FALSE` if it is not, or if an error occurs.
...
-doc """
[`gl:deleteLists/2`](`deleteLists/2`) causes a contiguous group of display lists to be deleted. `List` is the name of the first display list to be deleted, and `Range` is the number of display lists to delete. All display lists d with list&lt;= d&lt;= list+range-1 are deleted.
...
-doc """
Display lists are groups of GL commands that have been stored for subsequent execution. Display lists are created with [`gl:newList/2`](`newList/2`). All subsequent commands are placed in the display list, in the order issued, until [`gl:endList/0`](`newList/2`) is called.
...
-doc """
[`gl:callList/1`](`callList/1`) causes the named display list to be executed. The commands saved in the display list are executed in order, just as if they were called without using a display list. If `List` has not been defined as a display list, [`gl:callList/1`](`callList/1`) is ignored.
...
-doc """
[`gl:callLists/1`](`callLists/1`) causes each display list in the list of names passed as `Lists` to be executed. As a result, the commands saved in each display list are executed in order, just as if they were called without using a display list. Names of display lists that have not been defined are ignored.
...
-doc """
[`gl:callLists/1`](`callLists/1`) specifies an array of offsets. Display-list names are generated by adding `Base` to each offset. Names that reference valid display lists are executed; the others are ignored.
...
-doc """
[`gl:vertex()`](`vertex2d/2`) commands are used within [`gl:'begin'/1`](`'begin'/1`)/[`gl:'end'/0`](`'begin'/1`) pairs to specify point, line, and polygon vertices. The current color, normal, texture coordinates, and fog coordinate are associated with the vertex when [`gl:vertex()`](`vertex2d/2`) is called.
...
indexsv({C}) ->  indexs(C).
-doc """
[`gl:index()`](`indexd/1`) updates the current (single-valued) color index. It takes one argument, the new value for the current color index.
...
-doc """
The GL stores both a current single-valued color index and a current four-valued RGBA color. [`gl:color()`](`color3b/3`) sets a new four-valued RGBA color. [`gl:color()`](`color3b/3`) has two major variants: [`gl:color3()`](`color3b/3`) and [`gl:color4()`](`color3b/3`). [`gl:color3()`](`color3b/3`) variants specify new red, green, and blue values explicitly and set the current alpha value to 1.0 (full intensity) implicitly. [`gl:color4()`](`color3b/3`) variants specify all four color components explicitly.
...
-doc """
[`gl:texCoord()`](`texCoord1d/1`) specifies texture coordinates in one, two, three, or four dimensions. [`gl:texCoord1()`](`texCoord1d/1`) sets the current texture coordinates to (s 0 0 1); a call to [`gl:texCoord2()`](`texCoord1d/1`) sets them to (s t 0 1). Similarly, [`gl:texCoord3()`](`texCoord1d/1`) specifies the texture coordinates as (s t r 1), and [`gl:texCoord4()`](`texCoord1d/1`) defines all four components explicitly as (s t r q).
...
-doc """
The GL maintains a 3D position in window coordinates. This position, called the raster position, is used to position pixel and bitmap write operations. It is maintained with subpixel accuracy. See [`gl:bitmap/7`](`bitmap/7`), [`gl:drawPixels/5`](`drawPixels/5`), and [`gl:copyPixels/5`](`copyPixels/5`).
...
-doc """
[`gl:rect()`](`rectd/4`) supports efficient specification of rectangles as two corner points. Each rectangle command takes four arguments, organized either as two consecutive pairs of (x y) coordinates or as two pointers to arrays, each containing an (x y) pair. The resulting rectangle is defined in the z=0 plane.
...
-doc """
[`gl:vertexPointer/4`](`vertexPointer/4`) specifies the location and data format of an array of vertex coordinates to use when rendering. `Size` specifies the number of coordinates per vertex, and must be 2, 3, or 4. `Type` specifies the data type of each coordinate, and `Stride` specifies the byte stride from one vertex to the next, allowing vertices and attributes to be packed into a single array or stored in separate arrays. (Single-array storage may be more efficient on some implementations; see [`gl:interleavedArrays/3`](`interleavedArrays/3`).)
...
-doc """
[`gl:normalPointer/3`](`normalPointer/3`) specifies the location and data format of an array of normals to use when rendering. `Type` specifies the data type of each normal coordinate, and `Stride` specifies the byte stride from one normal to the next, allowing vertices and attributes to be packed into a single array or stored in separate arrays. (Single-array storage may be more efficient on some implementations; see [`gl:interleavedArrays/3`](`interleavedArrays/3`).)
...
-doc """
[`gl:colorPointer/4`](`colorPointer/4`) specifies the location and data format of an array of color components to use when rendering. `Size` specifies the number of components per color, and must be 3 or 4. `Type` specifies the data type of each color component, and `Stride` specifies the byte stride from one color to the next, allowing vertices and attributes to be packed into a single array or stored in separate arrays. (Single-array storage may be more efficient on some implementations; see [`gl:interleavedArrays/3`](`interleavedArrays/3`).)
...
-doc """
[`gl:texCoordPointer/4`](`texCoordPointer/4`) specifies the location and data format of an array of texture coordinates to use when rendering. `Size` specifies the number of coordinates per texture coordinate set, and must be 1, 2, 3, or 4. `Type` specifies the data type of each texture coordinate, and `Stride` specifies the byte stride from one texture coordinate set to the next, allowing vertices and attributes to be packed into a single array or stored in separate arrays. (Single-array storage may be more efficient on some implementations; see [`gl:interleavedArrays/3`](`interleavedArrays/3`).)
...
-doc """
[`gl:edgeFlagPointer/2`](`edgeFlagPointer/2`) specifies the location and data format of an array of boolean edge flags to use when rendering. `Stride` specifies the byte stride from one edge flag to the next, allowing vertices and attributes to be packed into a single array or stored in separate arrays.
...
-doc """
[`gl:arrayElement/1`](`arrayElement/1`) commands are used within [`gl:'begin'/1`](`'begin'/1`)/[`gl:'end'/0`](`'begin'/1`) pairs to specify vertex and attribute data for point, line, and polygon primitives. If `?GL_VERTEX_ARRAY` is enabled when [`gl:arrayElement/1`](`arrayElement/1`) is called, a single vertex is drawn, using vertex and attribute data taken from location `I` of the enabled arrays. If `?GL_VERTEX_ARRAY` is not enabled, no drawing occurs but the attributes corresponding to the enabled arrays are modified.
...
-doc """
[`gl:drawArrays/3`](`drawArrays/3`) specifies multiple geometric primitives with very few subroutine calls. Instead of calling a GL procedure to pass each individual vertex, normal, texture coordinate, edge flag, or color, you can prespecify separate arrays of vertices, normals, and colors and use them to construct a sequence of primitives with a single call to [`gl:drawArrays/3`](`drawArrays/3`).
...
-doc """
[`gl:interleavedArrays/3`](`interleavedArrays/3`) lets you specify and enable individual color, normal, texture and vertex arrays whose elements are part of a larger aggregate array element. For some implementations, this is more efficient than specifying the arrays separately.
...
-doc """
[`gl:lightModel()`](`lightModelf/2`) sets the lighting model parameter. `Pname` names a parameter and `Params` gives the new value. There are three lighting model parameters:
...
-doc """
[`gl:material()`](`materialf/3`) assigns values to material parameters. There are two matched sets of material parameters. One, the `front-facing` set, is used to shade points, lines, bitmaps, and all polygons (when two-sided lighting is disabled), or just front-facing polygons (when two-sided lighting is enabled). The other set, `back-facing`, is used to shade back-facing polygons only when two-sided lighting is enabled. Refer to the [`gl:lightModel()`](`lightModelf/2`) reference page for details concerning one- and two-sided lighting calculations.
...
-doc """
[`gl:getMaterial()`](`getMaterialfv/2`) returns in `Params` the value or values of parameter `Pname` of material `Face`. Six parameters are defined:
...
-doc """
[`gl:colorMaterial/2`](`colorMaterial/2`) specifies which material parameters track the current color. When `?GL_COLOR_MATERIAL` is enabled, the material parameter or parameters specified by `Mode`, of the material or materials specified by `Face`, track the current color at all times.
...
-doc """
[`gl:pixelZoom/2`](`pixelZoom/2`) specifies values for the x and y zoom factors. During the execution of [`gl:drawPixels/5`](`drawPixels/5`) or [`gl:copyPixels/5`](`copyPixels/5`), if ( xr, yr) is the current raster position, and a given element is in the mth row and nth column of the pixel rectangle, then pixels whose centers are in the rectangle with corners at
...
-doc """
[`gl:drawPixels/5`](`drawPixels/5`) reads pixel data from memory and writes it into the frame buffer relative to the current raster position, provided that the raster position is valid. Use [`gl:rasterPos()`](`rasterPos2d/2`) or [`gl:windowPos()`](`windowPos2d/2`) to set the current raster position; use [`gl:get()`](`getBooleanv/1`) with argument `?GL_CURRENT_RASTER_POSITION_VALID` to determine if the specified raster position is valid, and [`gl:get()`](`getBooleanv/1`) with argument `?GL_CURRENT_RASTER_POSITION` to query the raster position.
...
-doc """
[`gl:clearStencil/1`](`clearStencil/1`) specifies the index used by [`gl:clear/1`](`clear/1`) to clear the stencil buffer. `S` is masked with 2 m-1, where m is the number of bits in the stencil buffer.
...
-doc """
[`gl:getTexGen()`](`getTexGendv/2`) returns in `Params` selected parameters of a texture coordinate generation function that was specified using [`gl:texGen()`](`texGend/3`). `Coord` names one of the (`s`, `t`, `r`, `q`) texture coordinates, using the symbolic constant `?GL_S`, `?GL_T`, `?GL_R`, or `?GL_Q`.
...
-doc """
[`gl:getTexEnv()`](`getTexEnvfv/2`) returns in `Params` selected values of a texture environment that was specified with [`gl:texEnv()`](`texEnvf/3`). `Target` specifies a texture environment.
...
-doc """
Texturing maps a portion of a specified texture image onto each graphical primitive for which texturing is enabled. To enable and disable one-dimensional texturing, call [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) with argument `?GL_TEXTURE_1D`.
...
-doc """
[`gl:genTextures/1`](`genTextures/1`) returns `N` texture names in `Textures`. There is no guarantee that the names form a contiguous set of integers; however, it is guaranteed that none of the returned names was in use immediately before the call to [`gl:genTextures/1`](`genTextures/1`).
...
-doc """
[`gl:deleteTextures/1`](`deleteTextures/1`) deletes `N` textures named by the elements of the array `Textures`. After a texture is deleted, it has no contents or dimensionality, and its name is free for reuse (for example by [`gl:genTextures/1`](`genTextures/1`)). If a texture that is currently bound is deleted, the binding reverts to 0 (the default texture).
...
-doc """
[`gl:prioritizeTextures/2`](`prioritizeTextures/2`) assigns the `N` texture priorities given in `Priorities` to the `N` textures named in `Textures`.
...
-doc """
[`gl:isTexture/1`](`isTexture/1`) returns `?GL_TRUE` if `Texture` is currently the name of a texture. If `Texture` is zero, or is a non-zero value that is not currently the name of a texture, or if an error occurs, [`gl:isTexture/1`](`isTexture/1`) returns `?GL_FALSE`.
...
-doc """
Texturing maps a portion of a specified texture image onto each graphical primitive for which texturing is enabled. To enable or disable one-dimensional texturing, call [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) with argument `?GL_TEXTURE_1D`.
...
-doc """
[`gl:copyTexImage1D/7`](`copyTexImage1D/7`) defines a one-dimensional texture image with pixels from the current `?GL_READ_BUFFER`.
...
-doc """
[`gl:copyTexImage2D/8`](`copyTexImage2D/8`) defines a two-dimensional texture image, or cube-map texture image with pixels from the current `?GL_READ_BUFFER`.
...
-doc """
[`gl:copyTexSubImage1D/6`](`copyTexSubImage1D/6`) and `glCopyTextureSubImage1D` replace a portion of a one-dimensional texture image with pixels from the current `?GL_READ_BUFFER` (rather than from main memory, as is the case for [`gl:texSubImage1D/7`](`texSubImage1D/7`)). For [`gl:copyTexSubImage1D/6`](`copyTexSubImage1D/6`), the texture object that is bound to `Target` will be used for the process. For `glCopyTextureSubImage1D`, `Texture` tells which texture object should be used for the purpose of the call.
...
-doc """
Evaluators provide a way to use polynomial or rational polynomial mapping to produce vertices, normals, texture coordinates, and colors. The values produced by an evaluator are sent to further stages of GL processing just as if they had been presented using [`gl:vertex()`](`vertex2d/2`), [`gl:normal()`](`normal3b/3`), [`gl:texCoord()`](`texCoord1d/1`), and [`gl:color()`](`color3b/3`) commands, except that the generated values do not update the current normal, texture coordinates, or color.
...
-doc """
`glMap1` and `glMap2` define evaluators. [`gl:getMap()`](`getMapdv/3`) returns evaluator parameters. `Target` chooses a map, `Query` selects a specific parameter, and `V` points to storage where the values will be returned.
...
-doc """
[`gl:evalCoord1()`](`evalCoord1d/1`) evaluates enabled one-dimensional maps at argument `U`. [`gl:evalCoord2()`](`evalCoord1d/1`) does the same for two-dimensional maps using two domain values, `U` and `V`. To define a map, call `glMap1` and `glMap2`; to enable and disable it, call [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`).
...
-doc """
[`gl:mapGrid()`](`mapGrid1d/3`) and [`gl:evalMesh()`](`evalMesh1/3`) are used together to efficiently generate and evaluate a series of evenly-spaced map domain values. [`gl:evalMesh()`](`evalMesh1/3`) steps through the integer domain of a one- or two-dimensional grid, whose range is the domain of the evaluation maps specified by `glMap1` and `glMap2`.
...
-doc """
Fog is initially disabled. While enabled, fog affects rasterized geometry, bitmaps, and pixel blocks, but not buffer clear operations. To enable and disable fog, call [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) with argument `?GL_FOG`.
...
-doc """
The name stack is used during selection mode to allow sets of rendering commands to be uniquely identified. It consists of an ordered set of unsigned integers. [`gl:initNames/0`](`initNames/0`) causes the name stack to be initialized to its default empty state.
...
-doc """
[`gl:drawRangeElements/6`](`drawRangeElements/6`) is a restricted form of [`gl:drawElements/4`](`drawElements/4`). `Mode`, and `Count` match the corresponding arguments to [`gl:drawElements/4`](`drawElements/4`), with the additional constraint that all values in the arrays `Count` must lie between `Start` and `End`, inclusive.
...
-doc """
Texturing maps a portion of a specified texture image onto each graphical primitive for which texturing is enabled. To enable and disable three-dimensional texturing, call [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) with argument `?GL_TEXTURE_3D`.
...
-doc """
[`gl:copyTexSubImage3D/9`](`copyTexSubImage3D/9`) and `glCopyTextureSubImage3D` functions replace a rectangular portion of a three-dimensional or two-dimensional array texture image with pixels from the current `?GL_READ_BUFFER` (rather than from main memory, as is the case for [`gl:texSubImage3D/11`](`texSubImage3D/11`)).
...
-doc """
[`gl:activeTexture/1`](`activeTexture/1`) selects which texture unit subsequent texture state calls will affect. The number of texture units an implementation supports is implementation dependent, but must be at least 80.
...
-doc """
[`gl:clientActiveTexture/1`](`clientActiveTexture/1`) selects the vertex array client state parameters to be modified by [`gl:texCoordPointer/4`](`texCoordPointer/4`), and enabled or disabled with [`gl:enableClientState/1`](`enableClientState/1`) or [`gl:disableClientState/1`](`enableClientState/1`), respectively, when called with a parameter of `?GL_TEXTURE_COORD_ARRAY`.
...
-doc """
[`gl:multiTexCoord()`](`multiTexCoord1d/2`) specifies texture coordinates in one, two, three, or four dimensions. [`gl:multiTexCoord1()`](`multiTexCoord1d/2`) sets the current texture coordinates to (s 0 0 1); a call to [`gl:multiTexCoord2()`](`multiTexCoord1d/2`) sets them to (s t 0 1). Similarly, [`gl:multiTexCoord3()`](`multiTexCoord1d/2`) specifies the texture coordinates as (s t r 1), and [`gl:multiTexCoord4()`](`multiTexCoord1d/2`) defines all four components explicitly as (s t r q).
...
-doc """
[`gl:loadTransposeMatrix()`](`loadTransposeMatrixf/1`) replaces the current matrix with the one whose elements are specified by `M`. The current matrix is the projection matrix, modelview matrix, or texture matrix, depending on the current matrix mode (see [`gl:matrixMode/1`](`matrixMode/1`)).
...
-doc """
[`gl:multTransposeMatrix()`](`multTransposeMatrixf/1`) multiplies the current matrix with the one specified using `M`, and replaces the current matrix with the product.
...
-doc """
[`gl:multiDrawArrays/3`](`multiDrawArrays/3`) specifies multiple sets of geometric primitives with very few subroutine calls. Instead of calling a GL procedure to pass each individual vertex, normal, texture coordinate, edge flag, or color, you can prespecify separate arrays of vertices, normals, and colors and use them to construct a sequence of primitives with a single call to [`gl:multiDrawArrays/3`](`multiDrawArrays/3`).
...
-doc """
[`gl:fogCoord()`](`fogCoordf/1`) specifies the fog coordinate that is associated with each vertex and the current raster position. The value specified is interpolated and used in computing the fog color (see [`gl:fog()`](`fogf/2`)).
...
-doc """
[`gl:secondaryColorPointer/4`](`secondaryColorPointer/4`) specifies the location and data format of an array of color components to use when rendering. `Size` specifies the number of components per color, and must be 3. `Type` specifies the data type of each color component, and `Stride` specifies the byte stride from one color to the next, allowing vertices and attributes to be packed into a single array or stored in separate arrays.
...
-doc """
The `?GL_BLEND_COLOR` may be used to calculate the source and destination blending factors. The color components are clamped to the range \[0 1] before being stored. See [`gl:blendFunc/2`](`blendFunc/2`) for a complete description of the blending operations. Initially the `?GL_BLEND_COLOR` is set to (0, 0, 0, 0).
...
-doc """
[`gl:genQueries/1`](`genQueries/1`) returns `N` query object names in `Ids`. There is no guarantee that the names form a contiguous set of integers; however, it is guaranteed that none of the returned names was in use immediately before the call to [`gl:genQueries/1`](`genQueries/1`).
...
-doc """
[`gl:deleteQueries/1`](`deleteQueries/1`) deletes `N` query objects named by the elements of the array `Ids`. After a query object is deleted, it has no contents, and its name is free for reuse (for example by [`gl:genQueries/1`](`genQueries/1`)).
...
-doc """
[`gl:isQuery/1`](`isQuery/1`) returns `?GL_TRUE` if `Id` is currently the name of a query object. If `Id` is zero, or is a non-zero value that is not currently the name of a query object, or if an error occurs, [`gl:isQuery/1`](`isQuery/1`) returns `?GL_FALSE`.
...
-doc """
[`gl:getQueryiv/2`](`getQueryiv/2`) returns in `Params` a selected parameter of the query object target specified by `Target`.
...
-doc """
These commands return a selected parameter of the query object specified by `Id`. [`gl:getQueryObject()`](`getQueryObjectiv/2`) returns in `Params` a selected parameter of the query object specified by `Id`. [`gl:getQueryBufferObject()`](`getQueryObjectiv/2`) returns in `Buffer` a selected parameter of the query object specified by `Id`, by writing it to `Buffer`'s data store at the byte offset specified by `Offset`.
...
-doc """
[`gl:bindBuffer/2`](`bindBuffer/2`) binds a buffer object to the specified buffer binding point. Calling [`gl:bindBuffer/2`](`bindBuffer/2`) with `Target` set to one of the accepted symbolic constants and `Buffer` set to the name of a buffer object binds that buffer object name to the target. If no buffer object with name `Buffer` exists, one is created with that name. When a buffer object is bound to a target, the previous binding for that target is automatically broken.
...
-doc """
[`gl:deleteBuffers/1`](`deleteBuffers/1`) deletes `N` buffer objects named by the elements of the array `Buffers`. After a buffer object is deleted, it has no contents, and its name is free for reuse (for example by [`gl:genBuffers/1`](`genBuffers/1`)). If a buffer object that is currently bound is deleted, the binding reverts to 0 (the absence of any buffer object).
...
-doc """
[`gl:genBuffers/1`](`genBuffers/1`) returns `N` buffer object names in `Buffers`. There is no guarantee that the names form a contiguous set of integers; however, it is guaranteed that none of the returned names was in use immediately before the call to [`gl:genBuffers/1`](`genBuffers/1`).
...
-doc """
[`gl:isBuffer/1`](`isBuffer/1`) returns `?GL_TRUE` if `Buffer` is currently the name of a buffer object. If `Buffer` is zero, or is a non-zero value that is not currently the name of a buffer object, or if an error occurs, [`gl:isBuffer/1`](`isBuffer/1`) returns `?GL_FALSE`.
...
-doc """
[`gl:bufferData/4`](`bufferData/4`) and `glNamedBufferData` create a new data store for a buffer object. In case of [`gl:bufferData/4`](`bufferData/4`), the buffer object currently bound to `Target` is used. For `glNamedBufferData`, a buffer object associated with ID specified by the caller in `Buffer` will be used instead.
...
-doc """
[`gl:getBufferParameteriv/2`](`getBufferParameteriv/2`) returns in `Data` a selected parameter of the buffer object specified by `Target`.
...
-doc """
[`gl:stencilMaskSeparate/2`](`stencilMaskSeparate/2`) controls the writing of individual bits in the stencil planes. The least significant n bits of `Mask`, where n is the number of bits in the stencil buffer, specify a mask. Where a 1 appears in the mask, it's possible to write to the corresponding bit in the stencil buffer. Where a 0 appears, the corresponding bit is write-protected. Initially, all bits are enabled for writing.
...
-doc """
In order to create a complete shader program, there must be a way to specify the list of things that will be linked together. Program objects provide this mechanism. Shaders that are to be linked together in a program object must first be attached to that program object. [`gl:attachShader/2`](`attachShader/2`) attaches the shader object specified by `Shader` to the program object specified by `Program`. This indicates that `Shader` will be included in link operations that will be performed on `Program`.
...
-doc """
[`gl:bindAttribLocation/3`](`bindAttribLocation/3`) is used to associate a user-defined attribute variable in the program object specified by `Program` with a generic vertex attribute index. The name of the user-defined attribute variable is passed as a null terminated string in `Name`. The generic vertex attribute index to be bound to this variable is specified by `Index`. When `Program` is made part of current state, values provided via the generic vertex attribute `Index` will modify the value of the user-defined attribute variable specified by `Name`.
...
-doc """
[`gl:compileShader/1`](`compileShader/1`) compiles the source code strings that have been stored in the shader object specified by `Shader`.
...
-doc """
[`gl:createProgram/0`](`createProgram/0`) creates an empty program object and returns a non-zero value by which it can be referenced. A program object is an object to which shader objects can be attached. This provides a mechanism to specify the shader objects that will be linked to create a program. It also provides a means for checking the compatibility of the shaders that will be used to create a program (for instance, checking the compatibility between a vertex shader and a fragment shader). When no longer needed as part of a program object, shader objects can be detached.
...
-doc """
[`gl:deleteProgram/1`](`deleteProgram/1`) frees the memory and invalidates the name associated with the program object specified by `Program.` This command effectively undoes the effects of a call to [`gl:createProgram/0`](`createProgram/0`).
...
-doc """
[`gl:deleteShader/1`](`deleteShader/1`) frees the memory and invalidates the name associated with the shader object specified by `Shader`. This command effectively undoes the effects of a call to [`gl:createShader/1`](`createShader/1`).
...
-doc """
[`gl:detachShader/2`](`detachShader/2`) detaches the shader object specified by `Shader` from the program object specified by `Program`. This command can be used to undo the effect of the command [`gl:attachShader/2`](`attachShader/2`).
...
-doc """
[`gl:enableVertexAttribArray/1`](`enableVertexAttribArray/1`) and [`gl:enableVertexArrayAttrib/2`](`disableVertexAttribArray/1`) enable the generic vertex attribute array specified by `Index`. [`gl:enableVertexAttribArray/1`](`enableVertexAttribArray/1`) uses currently bound vertex array object for the operation, whereas [`gl:enableVertexArrayAttrib/2`](`disableVertexAttribArray/1`) updates state of the vertex array object with ID `Vaobj`.
...
-doc """
[`gl:getProgram()`](`getProgramiv/2`) returns in `Params` the value of a parameter for a specific program object. The following parameters are defined:
...
-doc """
[`gl:getProgramInfoLog/2`](`getProgramInfoLog/2`) returns the information log for the specified program object. The information log for a program object is modified when the program object is linked or validated. The string that is returned will be null terminated.
...
-doc """
[`gl:getShader()`](`getShaderiv/2`) returns in `Params` the value of a parameter for a specific shader object. The following parameters are defined:
...
-doc """
[`gl:getShaderInfoLog/2`](`getShaderInfoLog/2`) returns the information log for the specified shader object. The information log for a shader object is modified when the shader is compiled. The string that is returned will be null terminated.
...
-doc """
[`gl:getShaderSource/2`](`getShaderSource/2`) returns the concatenation of the source code strings from the shader object specified by `Shader`. The source code strings for a shader object are the result of a previous call to [`gl:shaderSource/2`](`shaderSource/2`). The string returned by the function will be null terminated.
...
-doc """
[`gl:getVertexAttrib()`](`getVertexAttribdv/2`) returns in `Params` the value of a generic vertex attribute parameter. The generic vertex attribute to be queried is specified by `Index`, and the parameter to be queried is specified by `Pname`.
...
-doc """
[`gl:useProgram/1`](`useProgram/1`) installs the program object specified by `Program` as part of current rendering state. One or more executables are created in a program object by successfully attaching shader objects to it with [`gl:attachShader/2`](`attachShader/2`), successfully compiling the shader objects with [`gl:compileShader/1`](`compileShader/1`), and successfully linking the program object with [`gl:linkProgram/1`](`linkProgram/1`).
...
-doc """
[`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) enable and disable various capabilities. Use [`gl:isEnabled/1`](`isEnabled/1`) or [`gl:get()`](`getBooleanv/1`) to determine the current setting of any capability. The initial value for each capability with the exception of `?GL_DITHER` and `?GL_MULTISAMPLE` is `?GL_FALSE`. The initial value for `?GL_DITHER` and `?GL_MULTISAMPLE` is `?GL_TRUE`.
...
-doc """
[`gl:isEnabled/1`](`isEnabled/1`) returns `?GL_TRUE` if `Cap` is an enabled capability and returns `?GL_FALSE` otherwise. Boolean states that are indexed may be tested with [`gl:isEnabledi/2`](`isEnabled/1`). For [`gl:isEnabledi/2`](`isEnabled/1`), `Index` specifies the index of the capability to test. `Index` must be between zero and the count of indexed capabilities for `Cap`. Initially all capabilities except `?GL_DITHER` are disabled; `?GL_DITHER` is initially enabled.
...
-doc """
Transform feedback mode captures the values of varying variables written by the vertex shader (or, if active, the geometry shader). Transform feedback is said to be active after a call to [`gl:beginTransformFeedback/1`](`beginTransformFeedback/1`) until a subsequent call to [`gl:endTransformFeedback/0`](`beginTransformFeedback/1`). Transform feedback commands must be paired.
...
-doc """
[`gl:bindBufferRange/5`](`bindBufferRange/5`) binds a range the buffer object `Buffer` represented by `Offset` and `Size` to the binding point at index `Index` of the array of targets specified by `Target`. Each `Target` represents an indexed array of buffer binding points, as well as a single general binding point that can be used by other buffer manipulation functions such as [`gl:bindBuffer/2`](`bindBuffer/2`) or `glMapBuffer`. In addition to binding a range of `Buffer` to the indexed buffer binding target, [`gl:bindBufferRange/5`](`bindBufferRange/5`) also binds the range to the generic buffer binding point specified by `Target`.
...
-doc """
The names of the vertex or geometry shader outputs to be recorded in transform feedback mode are specified using [`gl:transformFeedbackVaryings/3`](`transformFeedbackVaryings/3`). When a geometry shader is active, transform feedback records the values of selected geometry shader output variables from the emitted vertices. Otherwise, the values of the selected vertex shader outputs are recorded.
...
-doc """
Information about the set of varying variables in a linked program that will be captured during transform feedback may be retrieved by calling [`gl:getTransformFeedbackVarying/3`](`getTransformFeedbackVarying/3`). [`gl:getTransformFeedbackVarying/3`](`getTransformFeedbackVarying/3`) provides information about the varying variable selected by `Index`. An `Index` of 0 selects the first varying variable specified in the `Varyings` array passed to [`gl:transformFeedbackVaryings/3`](`transformFeedbackVaryings/3`), and an `Index` of the value of `?GL_TRANSFORM_FEEDBACK_VARYINGS` minus one selects the last such variable.
...
-doc """
[`gl:getUniform()`](`getUniformfv/2`) and `glGetnUniform` return in `Params` the value(s) of the specified uniform variable. The type of the uniform variable specified by `Location` determines the number of values returned. If the uniform variable is defined in the shader as a boolean, int, or float, a single value will be returned. If it is defined as a vec2, ivec2, or bvec2, two values will be returned. If it is defined as a vec3, ivec3, or bvec3, three values will be returned, and so on. To query values stored in uniform variables declared as arrays, call [`gl:getUniform()`](`getUniformfv/2`) for each element of the array. To query values stored in uniform variables declared as structures, call [`gl:getUniform()`](`getUniformfv/2`) for each field in the structure. The values for uniform variables declared as a matrix will be returned in column major order.
...
-doc """
[`gl:bindFragDataLocation/3`](`bindFragDataLocation/3`) explicitly specifies the binding of the user-defined varying out variable `Name` to fragment shader color number `ColorNumber` for program `Program`. If `Name` was bound previously, its assigned binding is replaced with `ColorNumber`. `Name` must be a null-terminated string. `ColorNumber` must be less than `?GL_MAX_DRAW_BUFFERS`.
...
-doc """
These commands clear a specified buffer of a framebuffer to specified value(s). For [`gl:clearBuffer*()`](`clearBufferiv/3`), the framebuffer is the currently bound draw framebuffer object. For `glClearNamedFramebuffer*`, `Framebuffer` is zero, indicating the default draw framebuffer, or the name of a framebuffer object.
...
-doc """
[`gl:getString/1`](`getString/1`) returns a pointer to a static string describing some aspect of the current GL connection. `Name` can be one of the following:
...
-doc """
[`gl:isRenderbuffer/1`](`isRenderbuffer/1`) returns `?GL_TRUE` if `Renderbuffer` is currently the name of a renderbuffer object. If `Renderbuffer` is zero, or if `Renderbuffer` is not the name of a renderbuffer object, or if an error occurs, [`gl:isRenderbuffer/1`](`isRenderbuffer/1`) returns `?GL_FALSE`. If `Renderbuffer` is a name returned by [`gl:genRenderbuffers/1`](`genRenderbuffers/1`), by that has not yet been bound through a call to [`gl:bindRenderbuffer/2`](`bindRenderbuffer/2`) or [`gl:framebufferRenderbuffer/4`](`framebufferRenderbuffer/4`), then the name is not a renderbuffer object and [`gl:isRenderbuffer/1`](`isRenderbuffer/1`) returns `?GL_FALSE`.
...
-doc """
[`gl:bindRenderbuffer/2`](`bindRenderbuffer/2`) binds the renderbuffer object with name `Renderbuffer` to the renderbuffer target specified by `Target`. `Target` must be `?GL_RENDERBUFFER`. `Renderbuffer` is the name of a renderbuffer object previously returned from a call to [`gl:genRenderbuffers/1`](`genRenderbuffers/1`), or zero to break the existing binding of a renderbuffer object to `Target`.
...
-doc """
[`gl:genRenderbuffers/1`](`genRenderbuffers/1`) returns `N` renderbuffer object names in `Renderbuffers`. There is no guarantee that the names form a contiguous set of integers; however, it is guaranteed that none of the returned names was in use immediately before the call to [`gl:genRenderbuffers/1`](`genRenderbuffers/1`).
...
-doc """
[`gl:renderbufferStorage/4`](`renderbufferStorage/4`) is equivalent to calling [`gl:renderbufferStorageMultisample/5`](`renderbufferStorageMultisample/5`) with the `Samples` set to zero, and `glNamedRenderbufferStorage` is equivalent to calling `glNamedRenderbufferStorageMultisample` with the samples set to zero.
...
-doc """
[`gl:getRenderbufferParameteriv/2`](`getRenderbufferParameteriv/2`) and `glGetNamedRenderbufferParameteriv` query parameters of a specified renderbuffer object.
...
-doc """
[`gl:isFramebuffer/1`](`isFramebuffer/1`) returns `?GL_TRUE` if `Framebuffer` is currently the name of a framebuffer object. If `Framebuffer` is zero, or if `?framebuffer` is not the name of a framebuffer object, or if an error occurs, [`gl:isFramebuffer/1`](`isFramebuffer/1`) returns `?GL_FALSE`. If `Framebuffer` is a name returned by [`gl:genFramebuffers/1`](`genFramebuffers/1`), by that has not yet been bound through a call to [`gl:bindFramebuffer/2`](`bindFramebuffer/2`), then the name is not a framebuffer object and [`gl:isFramebuffer/1`](`isFramebuffer/1`) returns `?GL_FALSE`.
...
-doc """
[`gl:bindFramebuffer/2`](`bindFramebuffer/2`) binds the framebuffer object with name `Framebuffer` to the framebuffer target specified by `Target`. `Target` must be either `?GL_DRAW_FRAMEBUFFER`, `?GL_READ_FRAMEBUFFER` or `?GL_FRAMEBUFFER`. If a framebuffer object is bound to `?GL_DRAW_FRAMEBUFFER` or `?GL_READ_FRAMEBUFFER`, it becomes the target for rendering or readback operations, respectively, until it is deleted or another framebuffer is bound to the corresponding bind point. Calling [`gl:bindFramebuffer/2`](`bindFramebuffer/2`) with `Target` set to `?GL_FRAMEBUFFER` binds `Framebuffer` to both the read and draw framebuffer targets. `Framebuffer` is the name of a framebuffer object previously returned from a call to [`gl:genFramebuffers/1`](`genFramebuffers/1`), or zero to break the existing binding of a framebuffer object to `Target`.
...
-doc """
[`gl:genFramebuffers/1`](`genFramebuffers/1`) returns `N` framebuffer object names in `Ids`. There is no guarantee that the names form a contiguous set of integers; however, it is guaranteed that none of the returned names was in use immediately before the call to [`gl:genFramebuffers/1`](`genFramebuffers/1`).
...
-doc """
[`gl:checkFramebufferStatus/1`](`checkFramebufferStatus/1`) and `glCheckNamedFramebufferStatus` return the completeness status of a framebuffer object when treated as a read or draw framebuffer, depending on the value of `Target`.
...
-doc """
[`gl:framebufferRenderbuffer/4`](`framebufferRenderbuffer/4`) and `glNamedFramebufferRenderbuffer` attaches a renderbuffer as one of the logical buffers of the specified framebuffer object. Renderbuffers cannot be attached to the default draw and read framebuffer, so they are not valid targets of these commands.
...
-doc """
[`gl:getFramebufferAttachmentParameteriv/3`](`getFramebufferAttachmentParameteriv/3`) and `glGetNamedFramebufferAttachmentParameteriv` return parameters of attachments of a specified framebuffer object.
...
-doc """
[`gl:blitFramebuffer/10`](`blitFramebuffer/10`) and `glBlitNamedFramebuffer` transfer a rectangle of pixel values from one region of a read framebuffer to another region of a draw framebuffer.
...
-doc """
[`gl:renderbufferStorageMultisample/5`](`renderbufferStorageMultisample/5`) and `glNamedRenderbufferStorageMultisample` establish the data storage, format, dimensions and number of samples of a renderbuffer object's image.
...
-doc """
[`gl:bindVertexArray/1`](`bindVertexArray/1`) binds the vertex array object with name `Array`. `Array` is the name of a vertex array object previously returned from a call to [`gl:genVertexArrays/1`](`genVertexArrays/1`), or zero to break the existing vertex array object binding.
...
-doc """
[`gl:genVertexArrays/1`](`genVertexArrays/1`) returns `N` vertex array object names in `Arrays`. There is no guarantee that the names form a contiguous set of integers; however, it is guaranteed that none of the returned names was in use immediately before the call to [`gl:genVertexArrays/1`](`genVertexArrays/1`).
...
-doc """
[`gl:isVertexArray/1`](`isVertexArray/1`) returns `?GL_TRUE` if `Array` is currently the name of a vertex array object. If `Array` is zero, or if `Array` is not the name of a vertex array object, or if an error occurs, [`gl:isVertexArray/1`](`isVertexArray/1`) returns `?GL_FALSE`. If `Array` is a name returned by [`gl:genVertexArrays/1`](`genVertexArrays/1`), by that has not yet been bound through a call to [`gl:bindVertexArray/1`](`bindVertexArray/1`), then the name is not a vertex array object and [`gl:isVertexArray/1`](`isVertexArray/1`) returns `?GL_FALSE`.
...
-doc """
[`gl:primitiveRestartIndex/1`](`primitiveRestartIndex/1`) specifies a vertex array element that is treated specially when primitive restarting is enabled. This is known as the primitive restart index.
...
-doc """
[`gl:getUniformIndices/2`](`getUniformIndices/2`) retrieves the indices of a number of uniforms within `Program`.
...
-doc """
[`gl:getUniformBlockIndex/2`](`getUniformBlockIndex/2`) retrieves the index of a uniform block within `Program`.
...
-doc """
[`gl:getActiveUniformBlockiv/4`](`getActiveUniformBlockiv/4`) retrieves information about an active uniform block within `Program`.
...
-doc """
[`gl:getActiveUniformBlockName/3`](`getActiveUniformBlockName/3`) retrieves the name of the active uniform block at `UniformBlockIndex` within `Program`.
...
-doc """
Binding points for active uniform blocks are assigned using [`gl:uniformBlockBinding/3`](`uniformBlockBinding/3`). Each of a program's active uniform blocks has a corresponding uniform buffer binding point. `Program` is the name of a program object for which the command [`gl:linkProgram/1`](`linkProgram/1`) has been issued in the past.
...
-doc """
[`gl:drawElementsInstancedBaseVertex/6`](`drawElementsInstancedBaseVertex/6`) behaves identically to [`gl:drawElementsInstanced/5`](`drawElementsInstanced/5`) except that the `i`th element transferred by the corresponding draw call will be taken from element `Indices`\[i] + `Basevertex` of each enabled array. If the resulting value is larger than the maximum value representable by `Type`, it is as if the calculation were upconverted to 32-bit unsigned integers (with wrapping on overflow conditions). The operation is undefined if the sum would be negative.
...
-doc """
[`gl:fenceSync/2`](`fenceSync/2`) creates a new fence sync object, inserts a fence command into the GL command stream and associates it with that sync object, and returns a non-zero name corresponding to the sync object.
...
-doc """
[`gl:isSync/1`](`isSync/1`) returns `?GL_TRUE` if `Sync` is currently the name of a sync object. If `Sync` is not the name of a sync object, or if an error occurs, [`gl:isSync/1`](`isSync/1`) returns `?GL_FALSE`. Note that zero is not the name of a sync object.
...
-doc """
[`gl:deleteSync/1`](`deleteSync/1`) deletes the sync object specified by `Sync`. If the fence command corresponding to the specified sync object has completed, or if no [`gl:waitSync/3`](`waitSync/3`) or [`gl:clientWaitSync/3`](`clientWaitSync/3`) commands are blocking on `Sync`, the object is deleted immediately. Otherwise, `Sync` is flagged for deletion and will be deleted when it is no longer associated with any fence command and is no longer blocking any [`gl:waitSync/3`](`waitSync/3`) or [`gl:clientWaitSync/3`](`clientWaitSync/3`) command. In either case, after [`gl:deleteSync/1`](`deleteSync/1`) returns, the name `Sync` is invalid and can no longer be used to refer to the sync object.
...
-doc """
[`gl:clientWaitSync/3`](`clientWaitSync/3`) causes the client to block and wait for the sync object specified by `Sync` to become signaled. If `Sync` is signaled when [`gl:clientWaitSync/3`](`clientWaitSync/3`) is called, [`gl:clientWaitSync/3`](`clientWaitSync/3`) returns immediately, otherwise it will block and wait for up to `Timeout` nanoseconds for `Sync` to become signaled.
...
-doc """
[`gl:waitSync/3`](`waitSync/3`) causes the GL server to block and wait until `Sync` becomes signaled. `Sync` is the name of an existing sync object upon which to wait. `Flags` and `Timeout` are currently not used and must be set to zero and the special value `?GL_TIMEOUT_IGNORED`, respectively

`Flags` and `Timeout` are placeholders for anticipated future extensions of sync object capabilities. They must have these reserved values in order that existing code calling [`gl:waitSync/3`](`waitSync/3`) operate properly in the presence of such extensions.
...
-doc """
[`gl:getSynciv/3`](`getSynciv/3`) retrieves properties of a sync object. `Sync` specifies the name of the sync object whose properties to retrieve.
...
-doc """
[`gl:texImage2DMultisample/6`](`texImage2DMultisample/6`) establishes the data storage, format, dimensions and number of samples of a multisample texture's image.
...
-doc """
[`gl:texImage3DMultisample/7`](`texImage3DMultisample/7`) establishes the data storage, format, dimensions and number of samples of a multisample texture's image.
...
-doc """
[`gl:sampleMaski/2`](`sampleMaski/2`) sets one 32-bit sub-word of the multi-word sample mask, `?GL_SAMPLE_MASK_VALUE`.
...
-doc """
[`gl:bindFragDataLocationIndexed/4`](`bindFragDataLocationIndexed/4`) specifies that the varying out variable `Name` in `Program` should be bound to fragment color `ColorNumber` when the program is next linked. `Index` may be zero or one to specify that the color be used as either the first or second color input to the blend equation, respectively.
...
-doc """
[`gl:getFragDataIndex/2`](`getFragDataIndex/2`) returns the index of the fragment color to which the variable `Name` was bound when the program object `Program` was last linked. If `Name` is not a varying out variable of `Program`, or if an error occurs, -1 will be returned.
...
-doc """
[`gl:genSamplers/1`](`genSamplers/1`) returns `N` sampler object names in `Samplers`. There is no guarantee that the names form a contiguous set of integers; however, it is guaranteed that none of the returned names was in use immediately before the call to [`gl:genSamplers/1`](`genSamplers/1`).
...
-doc """
[`gl:isSampler/1`](`isSampler/1`) returns `?GL_TRUE` if `Id` is currently the name of a sampler object. If `Id` is zero, or is a non-zero value that is not currently the name of a sampler object, or if an error occurs, [`gl:isSampler/1`](`isSampler/1`) returns `?GL_FALSE`.
...
-doc """
[`gl:bindSampler/2`](`bindSampler/2`) binds `Sampler` to the texture unit at index `Unit`. `Sampler` must be zero or the name of a sampler object previously returned from a call to [`gl:genSamplers/1`](`genSamplers/1`). `Unit` must be less than the value of `?GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS`.
...
-doc """
[`gl:samplerParameter()`](`samplerParameteri/3`) assigns the value or values in `Params` to the sampler parameter specified as `Pname`. `Sampler` specifies the sampler object to be modified, and must be the name of a sampler object previously returned from a call to [`gl:genSamplers/1`](`genSamplers/1`). The following symbols are accepted in `Pname`:
...
-doc """
[`gl:getSamplerParameter()`](`getSamplerParameteriv/2`) returns in `Params` the value or values of the sampler parameter specified as `Pname`. `Sampler` defines the target sampler, and must be the name of an existing sampler object, returned from a previous call to [`gl:genSamplers/1`](`genSamplers/1`). `Pname` accepts the same symbols as [`gl:samplerParameter()`](`samplerParameteri/3`), with the same interpretations:
...
-doc """
[`gl:queryCounter/2`](`queryCounter/2`) causes the GL to record the current time into the query object named `Id`. `Target` must be `?GL_TIMESTAMP`. The time is recorded after all previous commands on the GL client and server state and the framebuffer have been fully realized. When the time is recorded, the query result for that object is marked available. [`gl:queryCounter/2`](`queryCounter/2`) timer queries can be used within a [`gl:beginQuery/2`](`beginQuery/2`) / [`gl:endQuery/1`](`beginQuery/2`) block where the target is `?GL_TIME_ELAPSED` and it does not affect the result of that query object.
...
-doc """
The blend equations determine how a new pixel (the ''source'' color) is combined with a pixel already in the framebuffer (the ''destination'' color). This function sets both the RGB blend equation and the alpha blend equation to a single equation. [`gl:blendEquationi/2`](`blendEquation/1`) specifies the blend equation for a single draw buffer whereas [`gl:blendEquation/1`](`blendEquation/1`) sets the blend equation for all draw buffers.
...
-doc """
The blend equations determines how a new pixel (the ''source'' color) is combined with a pixel already in the framebuffer (the ''destination'' color). These functions specify one blend equation for the RGB-color components and one blend equation for the alpha component. [`gl:blendEquationSeparatei/3`](`blendEquationSeparate/2`) specifies the blend equations for a single draw buffer whereas [`gl:blendEquationSeparate/2`](`blendEquationSeparate/2`) sets the blend equations for all draw buffers.
...
-doc """
Pixels can be drawn using a function that blends the incoming (source) RGBA values with the RGBA values that are already in the frame buffer (the destination values). Blending is initially disabled. Use [`gl:enable/1`](`enable/1`) and [`gl:disable/1`](`enable/1`) with argument `?GL_BLEND` to enable and disable blending.
...
-doc """
[`gl:drawArraysIndirect/2`](`drawArraysIndirect/2`) specifies multiple geometric primitives with very few subroutine calls. [`gl:drawArraysIndirect/2`](`drawArraysIndirect/2`) behaves similarly to [`gl:drawArraysInstancedBaseInstance/5`](`drawArraysInstancedBaseInstance/5`), execept that the parameters to [`gl:drawArraysInstancedBaseInstance/5`](`drawArraysInstancedBaseInstance/5`) are stored in memory at the address given by `Indirect`.
...
-doc """
[`gl:drawElementsIndirect/3`](`drawElementsIndirect/3`) specifies multiple indexed geometric primitives with very few subroutine calls. [`gl:drawElementsIndirect/3`](`drawElementsIndirect/3`) behaves similarly to [`gl:drawElementsInstancedBaseVertexBaseInstance/7`](`drawElementsInstancedBaseVertexBaseInstance/7`), execpt that the parameters to [`gl:drawElementsInstancedBaseVertexBaseInstance/7`](`drawElementsInstancedBaseVertexBaseInstance/7`) are stored in memory at the address given by `Indirect`.
...
-doc """
[`gl:getSubroutineUniformLocation/3`](`getSubroutineUniformLocation/3`) returns the location of the subroutine uniform variable `Name` in the shader stage of type `Shadertype` attached to `Program`, with behavior otherwise identical to [`gl:getUniformLocation/2`](`getUniformLocation/2`).
...
-doc """
[`gl:deleteTransformFeedbacks/1`](`deleteTransformFeedbacks/1`) deletes the `N` transform feedback objects whose names are stored in the array `Ids`. Unused names in `Ids` are ignored, as is the name zero. After a transform feedback object is deleted, its name is again unused and it has no contents. If an active transform feedback object is deleted, its name immediately becomes unused, but the underlying object is not deleted until it is no longer active.
...
-doc """
[`gl:genTransformFeedbacks/1`](`genTransformFeedbacks/1`) returns `N` previously unused transform feedback object names in `Ids`. These names are marked as used, for the purposes of [`gl:genTransformFeedbacks/1`](`genTransformFeedbacks/1`) only, but they acquire transform feedback state only when they are first bound.
...
-doc """
[`gl:pauseTransformFeedback/0`](`pauseTransformFeedback/0`) pauses transform feedback operations on the currently active transform feedback object. When transform feedback operations are paused, transform feedback is still considered active and changing most transform feedback state related to the object results in an error. However, a new transform feedback object may be bound while transform feedback is paused.
...
-doc """
[`gl:resumeTransformFeedback/0`](`resumeTransformFeedback/0`) resumes transform feedback operations on the currently active transform feedback object. When transform feedback operations are paused, transform feedback is still considered active and changing most transform feedback state related to the object results in an error. However, a new transform feedback object may be bound while transform feedback is paused.
...
-doc """
[`gl:beginQueryIndexed/3`](`beginQueryIndexed/3`) and [`gl:endQueryIndexed/2`](`beginQueryIndexed/3`) delimit the boundaries of a query object. `Query` must be a name previously returned from a call to [`gl:genQueries/1`](`genQueries/1`). If a query object with name `Id` does not yet exist it is created with the type determined by `Target`. `Target` must be one of `?GL_SAMPLES_PASSED`, `?GL_ANY_SAMPLES_PASSED`, `?GL_PRIMITIVES_GENERATED`, `?GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN`, or `?GL_TIME_ELAPSED`. The behavior of the query object depends on its type and is as follows.
...
-doc """
[`gl:getQueryIndexediv/3`](`getQueryIndexediv/3`) returns in `Params` a selected parameter of the indexed query object target specified by `Target` and `Index`. `Index` specifies the index of the query object target and must be between zero and a target-specific maxiumum.
...
-doc """
[`gl:shaderBinary/3`](`shaderBinary/3`) loads pre-compiled shader binary code into the `Count` shader objects whose handles are given in `Shaders`. `Binary` points to `Length` bytes of binary shader code stored in client memory. `BinaryFormat` specifies the format of the pre-compiled code.
...
-doc """
[`gl:getShaderPrecisionFormat/2`](`getShaderPrecisionFormat/2`) retrieves the numeric range and precision for the implementation's representation of quantities in different numeric formats in specified shader type. `ShaderType` specifies the type of shader for which the numeric precision and range is to be retrieved and must be one of `?GL_VERTEX_SHADER` or `?GL_FRAGMENT_SHADER`. `PrecisionType` specifies the numeric format to query and must be one of `?GL_LOW_FLOAT`, `?GL_MEDIUM_FLOAT``?GL_HIGH_FLOAT`, `?GL_LOW_INT`, `?GL_MEDIUM_INT`, or `?GL_HIGH_INT`.
...
-doc """
[`gl:clearDepth/1`](`clearDepth/1`) specifies the depth value used by [`gl:clear/1`](`clear/1`) to clear the depth buffer. Values specified by [`gl:clearDepth/1`](`clearDepth/1`) are clamped to the range \[0 1].
...
-doc """
[`gl:getProgramBinary/2`](`getProgramBinary/2`) returns a binary representation of the compiled and linked executable for `Program` into the array of bytes whose address is specified in `Binary`. The maximum number of bytes that may be written into `Binary` is specified by `BufSize`. If the program binary is greater in size than `BufSize` bytes, then an error is generated, otherwise the actual number of bytes written into `Binary` is returned in the variable whose address is given by `Length`. If `Length` is `?NULL`, then no length is returned.
...
-doc """
[`gl:programBinary/3`](`programBinary/3`) loads a program object with a program binary previously returned from [`gl:getProgramBinary/2`](`getProgramBinary/2`). `BinaryFormat` and `Binary` must be those returned by a previous call to [`gl:getProgramBinary/2`](`getProgramBinary/2`), and `Length` must be the length returned by [`gl:getProgramBinary/2`](`getProgramBinary/2`), or by [`gl:getProgram()`](`getProgramiv/2`) when called with `Pname` set to `?GL_PROGRAM_BINARY_LENGTH`. If these conditions are not met, loading the program binary will fail and `Program`'s `?GL_LINK_STATUS` will be set to `?GL_FALSE`.
...
-doc """
[`gl:programParameter()`](`programParameteri/3`) specifies a new value for the parameter nameed by `Pname` for the program object `Program`.
...
-doc """
[`gl:createShaderProgram()`](`createShaderProgramv/2`) creates a program object containing compiled and linked shaders for a single stage specified by `Type`. `Strings` refers to an array of `Count` strings from which to create the shader executables.
...
-doc """
[`gl:genProgramPipelines/1`](`genProgramPipelines/1`) returns `N` previously unused program pipeline object names in `Pipelines`. These names are marked as used, for the purposes of [`gl:genProgramPipelines/1`](`genProgramPipelines/1`) only, but they acquire program pipeline state only when they are first bound.
...
-doc """
[`gl:getProgramPipelineiv/2`](`getProgramPipelineiv/2`) retrieves the value of a property of the program pipeline object `Pipeline`. `Pname` specifies the name of the parameter whose value to retrieve. The value of the parameter is written to the variable whose address is given by `Params`.
...
-doc """
[`gl:programUniform()`](`programUniform1i/3`) modifies the value of a uniform variable or a uniform variable array. The location of the uniform variable to be modified is specified by `Location`, which should be a value returned by [`gl:getUniformLocation/2`](`getUniformLocation/2`). [`gl:programUniform()`](`programUniform1i/3`) operates on the program object specified by `Program`.
...
-doc """
[`gl:validateProgramPipeline/1`](`validateProgramPipeline/1`) instructs the implementation to validate the shader executables contained in `Pipeline` against the current GL state. The implementation may use this as an opportunity to perform any internal shader modifications that may be required to ensure correct operation of the installed shaders given the current GL state.
...
-doc """
The [`gl:vertexAttrib()`](`vertexAttrib1d/2`) family of entry points allows an application to pass generic vertex attributes in numbered locations.
...
-doc """
[`gl:copyImageSubData/15`](`copyImageSubData/15`) may be used to copy data from one image (i.e. texture or renderbuffer) to another. [`gl:copyImageSubData/15`](`copyImageSubData/15`) does not perform general-purpose conversions such as scaling, resizing, blending, color-space, or format conversions. It should be considered to operate in a manner similar to a CPU memcpy. CopyImageSubData can copy between images with different internal formats, provided the formats are compatible.
...
-doc """
[`gl:framebufferParameteri/3`](`framebufferParameteri/3`) and `glNamedFramebufferParameteri` modify the value of the parameter named `Pname` in the specified framebuffer object. There are no modifiable parameters of the default draw and read framebuffer, so they are not valid targets of these commands.
...
-doc """
[`gl:getFramebufferParameteriv/2`](`getFramebufferParameteriv/2`) and `glGetNamedFramebufferParameteriv` query parameters of a specified framebuffer object.
...
-doc """
[`gl:invalidateTexSubImage/8`](`invalidateTexSubImage/8`) invalidates all of a texture image. `Texture` and `Level` indicated which texture image is being invalidated. After this command, data in the texture image has undefined values.
...
-doc """
[`gl:invalidateBufferData/1`](`invalidateBufferData/1`) invalidates all of the content of the data store of a buffer object. After invalidation, the content of the buffer's data store becomes undefined.
...
-doc """
[`gl:invalidateFramebuffer/2`](`invalidateFramebuffer/2`) and `glInvalidateNamedFramebufferData` invalidate the entire contents of a specified set of attachments of a framebuffer.
...
-doc """
[`gl:invalidateSubFramebuffer/6`](`invalidateSubFramebuffer/6`) and `glInvalidateNamedFramebufferSubData` invalidate the contents of a specified region of a specified set of attachments of a framebuffer.
...
-doc """
[`gl:getProgramInterfaceiv/3`](`getProgramInterfaceiv/3`) queries the property of the interface identifed by `ProgramInterface` in `Program`, the property name of which is given by `Pname`.
...
-doc """
[`gl:getProgramResourceIndex/3`](`getProgramResourceIndex/3`) returns the unsigned integer index assigned to a resource named `Name` in the interface type `ProgramInterface` of program object `Program`.
...
-doc """
[`gl:getProgramResourceName/4`](`getProgramResourceName/4`) retrieves the name string assigned to the single active resource with an index of `Index` in the interface `ProgramInterface` of program object `Program`. `Index` must be less than the number of entries in the active resource list for `ProgramInterface`.
...
-doc """
[`gl:getProgramResourceLocationIndex/3`](`getProgramResourceLocationIndex/3`) returns the fragment color index assigned to the variable named `Name` in interface `ProgramInterface` of program object `Program`. `Program` must be the name of a program that has been linked successfully. `ProgramInterface` must be `?GL_PROGRAM_OUTPUT`.
...
-doc """
[`gl:vertexAttribBinding/2`](`vertexAttribBinding/2`) and [`gl:vertexArrayAttribBinding/3`](`vertexAttribBinding/2`) establishes an association between the generic vertex attribute of a vertex array object whose index is given by `Attribindex`, and a vertex buffer binding whose index is given by `Bindingindex`. For [`gl:vertexAttribBinding/2`](`vertexAttribBinding/2`), the vertex array object affected is that currently bound. For [`gl:vertexArrayAttribBinding/3`](`vertexAttribBinding/2`), `Vaobj` is the name of the vertex array object.
...
-doc """
[`gl:debugMessageControl/5`](`debugMessageControl/5`) controls the reporting of debug messages generated by a debug context. The parameters `Source`, `Type` and `Severity` form a filter to select messages from the pool of potential messages generated by the GL.
...
-doc """
[`gl:getDebugMessageLog/2`](`getDebugMessageLog/2`) retrieves messages from the debug message log. A maximum of `Count` messages are retrieved from the log. If `Sources` is not NULL then the source of each message is written into up to `Count` elements of the array. If `Types` is not NULL then the type of each message is written into up to `Count` elements of the array. If `Id` is not NULL then the identifier of each message is written into up to `Count` elements of the array. If `Severities` is not NULL then the severity of each message is written into up to `Count` elements of the array. If `Lengths` is not NULL then the length of each message is written into up to `Count` elements of the array.
...
-doc """
[`gl:pushDebugGroup/4`](`pushDebugGroup/4`) pushes a debug group described by the string `Message` into the command stream. The value of `Id` specifies the ID of messages generated. The parameter `Length` contains the number of characters in `Message`. If `Length` is negative, it is implied that `Message` contains a null terminated string. The message has the specified `Source` and `Id`, the `Type``?GL_DEBUG_TYPE_PUSH_GROUP`, and `Severity``?GL_DEBUG_SEVERITY_NOTIFICATION`. The GL will put a new debug group on top of the debug group stack which inherits the control of the volume of debug output of the debug group previously residing on the top of the debug group stack. Because debug groups are strictly hierarchical, any additional control of the debug output volume will only apply within the active debug group and the debug groups pushed on top of the active debug group.
...
-doc """
[`gl:objectPtrLabel/3`](`objectPtrLabel/3`) labels the sync object identified by `Ptr`.
...
-doc """
[`gl:clearTexImage/5`](`clearTexImage/5`) fills all an image contained in a texture with an application supplied value. `Texture` must be the name of an existing texture. Further, `Texture` may not be the name of a buffer texture, nor may its internal format be compressed.
...
-doc """
[`gl:clearTexSubImage/11`](`clearTexSubImage/11`) fills all or part of an image contained in a texture with an application supplied value. `Texture` must be the name of an existing texture. Further, `Texture` may not be the name of a buffer texture, nor may its internal format be compressed.
...
-doc """
[`gl:bindTextures/2`](`bindTextures/2`) binds an array of existing texture objects to a specified number of consecutive texture units. `Count` specifies the number of texture objects whose names are stored in the array `Textures`. That number of texture names are read from the array and bound to the `Count` consecutive texture units starting from `First`. The target, or type of texture is deduced from the texture object and each texture is bound to the corresponding target of the texture unit. If the name zero appears in the `Textures` array, any existing binding to any target of the texture unit is reset and the default texture for that target is bound in its place. Any non-zero entry in `Textures` must be the name of an existing texture object. If `Textures` is `?NULL` then it is as if an appropriately sized array containing only zeros had been specified.
...
-doc """
[`gl:clipControl/2`](`clipControl/2`) controls the clipping volume behavior and the clip coordinate to window coordinate transformation behavior.
...
-doc """
[`gl:createTransformFeedbacks/1`](`createTransformFeedbacks/1`) returns `N` previously unused transform feedback object names in `Ids`, each representing a new transform feedback object initialized to the default state.
...
-doc """
[`gl:transformFeedbackBufferBase/3`](`transformFeedbackBufferBase/3`) binds the buffer object `Buffer` to the binding point at index `Index` of the transform feedback object `Xfb`.
...
-doc """
[`gl:transformFeedbackBufferRange/5`](`transformFeedbackBufferRange/5`) binds a range of the buffer object `Buffer` represented by `Offset` and `Size` to the binding point at index `Index` of the transform feedback object `Xfb`.
...
-doc """
[`gl:createBuffers/1`](`createBuffers/1`) returns `N` previously unused buffer names in `Buffers`, each representing a new buffer object initialized as if it had been bound to an unspecified target.
...
-doc """
[`gl:flushMappedBufferRange/3`](`flushMappedBufferRange/3`) indicates that modifications have been made to a range of a mapped buffer object. The buffer object must previously have been mapped with the `?GL_MAP_FLUSH_EXPLICIT_BIT` flag.
...
-doc """
[`gl:createFramebuffers/1`](`createFramebuffers/1`) returns `N` previously unused framebuffer names in `Framebuffers`, each representing a new framebuffer object initialized to the default state.
...
-doc """
[`gl:createRenderbuffers/1`](`createRenderbuffers/1`) returns `N` previously unused renderbuffer object names in `Renderbuffers`, each representing a new renderbuffer object initialized to the default state.
...
-doc """
[`gl:createTextures/2`](`createTextures/2`) returns `N` previously unused texture names in `Textures`, each representing a new texture object of the dimensionality and type specified by `Target` and initialized to the default values for that texture type.
...
-doc """
[`gl:texBuffer/3`](`texBuffer/3`) and [`gl:textureBuffer/3`](`texBuffer/3`) attaches the data store of a specified buffer object to a specified texture object, and specify the storage format for the texture image found in the buffer object. The texture object must be a buffer texture.
...
-doc """
[`gl:texBufferRange/5`](`texBufferRange/5`) and [`gl:textureBufferRange/5`](`texBufferRange/5`) attach a range of the data store of a specified buffer object to a specified texture object, and specify the storage format for the texture image found in the buffer object. The texture object must be a buffer texture.
...
-doc """
[`gl:generateMipmap/1`](`generateMipmap/1`) and [`gl:generateTextureMipmap/1`](`generateMipmap/1`) generates mipmaps for the specified texture object. For [`gl:generateMipmap/1`](`generateMipmap/1`), the texture object that is bound to `Target`. For [`gl:generateTextureMipmap/1`](`generateMipmap/1`), `Texture` is the name of the texture object.
...
-doc """
[`gl:bindTextureUnit/2`](`bindTextureUnit/2`) binds an existing texture object to the texture unit numbered `Unit`.
...
-doc """
[`gl:createVertexArrays/1`](`createVertexArrays/1`) returns `N` previously unused vertex array object names in `Arrays`, each representing a new vertex array object initialized to the default state.
...
-doc """
[`gl:vertexArrayElementBuffer/2`](`vertexArrayElementBuffer/2`) binds a buffer object with id `Buffer` to the element array buffer bind point of a vertex array object with id `Vaobj`. If `Buffer` is zero, any existing element array buffer binding to `Vaobj` is removed.
...
-doc """
[`gl:createSamplers/1`](`createSamplers/1`) returns `N` previously unused sampler names in `Samplers`, each representing a new sampler object initialized to the default state.
...
-doc """
[`gl:createProgramPipelines/1`](`createProgramPipelines/1`) returns `N` previously unused program pipeline names in `Pipelines`, each representing a new program pipeline object initialized to the default state.
...
-doc """
[`gl:createQueries/2`](`createQueries/2`) returns `N` previously unused query object names in `Ids`, each representing a new query object with the specified `Target`.
...
-doc """
[`gl:colorTable/6`](`colorTable/6`) may be used in two ways: to test the actual size and color resolution of a lookup table given a particular set of parameters, or to load the contents of a color lookup table. Use the targets `?GL_PROXY_*` for the first case and the other targets for the second case.
...
-doc """
[`gl:copyColorTable/5`](`copyColorTable/5`) loads a color table with pixels from the current `?GL_READ_BUFFER` (rather than from main memory, as is the case for [`gl:colorTable/6`](`colorTable/6`)).
...
-doc """
[`gl:getColorTable/4`](`getColorTable/4`) returns in `Table` the contents of the color table specified by `Target`. No pixel transfer operations are performed, but pixel storage modes that are applicable to [`gl:readPixels/7`](`readPixels/7`) are performed.
...
-doc """
[`gl:convolutionFilter1D/6`](`convolutionFilter1D/6`) builds a one-dimensional convolution filter kernel from an array of pixels.
...
-doc """
[`gl:convolutionFilter2D/7`](`convolutionFilter2D/7`) builds a two-dimensional convolution filter kernel from an array of pixels.
...
-doc """
[`gl:convolutionParameter()`](`convolutionParameterf/3`) sets the value of a convolution parameter.
...
-doc """
[`gl:copyConvolutionFilter1D/5`](`copyConvolutionFilter1D/5`) defines a one-dimensional convolution filter kernel with pixels from the current `?GL_READ_BUFFER` (rather than from main memory, as is the case for [`gl:convolutionFilter1D/6`](`convolutionFilter1D/6`)).
...
-doc """
[`gl:copyConvolutionFilter2D/6`](`copyConvolutionFilter2D/6`) defines a two-dimensional convolution filter kernel with pixels from the current `?GL_READ_BUFFER` (rather than from main memory, as is the case for [`gl:convolutionFilter2D/7`](`convolutionFilter2D/7`)).
...
-doc """
[`gl:getConvolutionParameter()`](`getConvolutionParameterfv/2`) retrieves convolution parameters. `Target` determines which convolution filter is queried. `Pname` determines which parameter is returned:
...
-doc """
[`gl:separableFilter2D/8`](`separableFilter2D/8`) builds a two-dimensional separable convolution filter kernel from two arrays of pixels.
...
-doc """
[`gl:getHistogram/5`](`getHistogram/5`) returns the current histogram table as a one-dimensional image with the same width as the histogram. No pixel transfer operations are performed on this image, but pixel storage modes that are applicable to 1D images are honored.
...
-doc """
[`gl:getMinmaxParameter()`](`getMinmaxParameterfv/2`) retrieves parameters for the current minmax table by setting `Pname` to one of the following values:
...
-doc """
[`gl:resetHistogram/1`](`resetHistogram/1`) resets all the elements of the current histogram table to zero.
...
-doc """
[`gl:resetMinmax/1`](`resetMinmax/1`) resets the elements of the current minmax table to their initial values: the \`\`maximum'' element receives the minimum possible component values, and the \`\`minimum'' element receives the maximum possible component values.

...
An instance of a `m:wxGLContext` represents the state of an OpenGL state machine and the connection between OpenGL and the system.
...
OpenGL core profile specification defines several flags at context creation that determine not only the type of context but also some features. Some of these flags can be set in the list of attributes used at `m:wxGLCanvas` ctor. But since wxWidgets 3.1.0 it is strongly encouraged to use the new mechanism: setting the context attributes with a `wxGLContextAttrs` (not implemented in wx) object and the canvas attributes with a `wxGLAttributes` (not implemented in wx) object.

The best way of knowing if your OpenGL environment supports a specific type of context is creating a `m:wxGLContext` instance and checking `isOK/1`. If it returns false, then simply delete that instance and create a new one with other attributes.
...
See: `m:wxGLCanvas`, `wxGLContextAttrs` (not implemented in wx), `wxGLAttributes` (not implemented in wx)

wxWidgets docs: [wxGLContext](https://docs.wxwidgets.org/3.1/classwx_g_l_context.html)
""".
-include("wxe.hrl").
...
-doc """
Makes the OpenGL state that is represented by this rendering context current with the `m:wxGLCanvas` `win`.

Note: `win` can be a different `m:wxGLCanvas` window than the one that was passed to the constructor of this rendering context. If `RC` is an object of type `m:wxGLContext`, the statements `"RC.SetCurrent(win);"` and `"win.SetCurrent(RC);"` are equivalent, see `wxGLCanvas:setCurrent/2`.
""".
...
If this function returns false then the `m:wxGLContext` object is useless and should be deleted and recreated with different attributes.

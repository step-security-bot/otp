...
It contains a `m:wxPrintData` member which is used to hold basic printer configuration data (as opposed to the user-interface configuration settings stored by `m:wxPageSetupDialogData`).

See: [Overview printing](https://docs.wxwidgets.org/3.1/overview_printing.html#overview_printing), `m:wxPageSetupDialog`

wxWidgets docs: [wxPageSetupDialogData](https://docs.wxwidgets.org/3.1/classwx_page_setup_dialog_data.html)
""".
-include("wxe.hrl").
...
-doc "Returns true if the page setup dialog will take its minimum margin values from the currently selected printer properties (Windows only).".
...
-doc "Returns true if the dialog will simply return default printer information (such as orientation) instead of showing a dialog (Windows only).".
...
-doc "Pass true if the dialog will simply return default printer information (such as orientation) instead of showing a dialog (Windows only).".
...
If a corresponding paper id is found, it will be set in the internal `m:wxPrintData` object, otherwise the paper size overrides the paper id.
""".

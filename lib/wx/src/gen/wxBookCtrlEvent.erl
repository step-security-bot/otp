...
See: `m:wxNotebook`, `m:wxListbook`, `m:wxChoicebook`, `m:wxTreebook`, `m:wxToolbook`, `m:wxAuiNotebook`

This class is derived (and can use functions) from: `m:wxNotifyEvent` `m:wxCommandEvent` `m:wxEvent`

wxWidgets docs: [wxBookCtrlEvent](https://docs.wxwidgets.org/3.1/classwx_book_ctrl_event.html)
""".
-include("wxe.hrl").

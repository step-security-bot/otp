...
`m:wxAuiManager` manages the panes associated with it for a particular `m:wxFrame`, using a pane's `m:wxAuiPaneInfo` information to determine each pane's docking and floating behaviour.

`m:wxAuiManager` uses wxWidgets' sizer mechanism to plan the layout of each frame. It uses a replaceable dock art class to do all drawing, so all drawing is localized in one area, and may be customized depending on an application's specific needs.

`m:wxAuiManager` works as follows: the programmer adds panes to the class, or makes changes to existing pane properties (dock position, floating state, show state, etc.). To apply these changes, `m:wxAuiManager`'s `update/1` function is called. This batch processing can be used to avoid flicker, by modifying more than one pane at a time, and then "committing" all of the changes at once by calling `update/1`.
...
See: [Overview aui](https://docs.wxwidgets.org/3.1/overview_aui.html#overview_aui), `m:wxAuiNotebook`, `m:wxAuiDockArt`, `m:wxAuiPaneInfo`

This class is derived (and can use functions) from: `m:wxEvtHandler`

wxWidgets docs: [wxAuiManager](https://docs.wxwidgets.org/3.1/classwx_aui_manager.html)

## Events

Event types emitted from this class: [`aui_pane_button`](`m:wxAuiManagerEvent`), [`aui_pane_close`](`m:wxAuiManagerEvent`), [`aui_pane_maximize`](`m:wxAuiManagerEvent`), [`aui_pane_restore`](`m:wxAuiManagerEvent`), [`aui_pane_activated`](`m:wxAuiManagerEvent`), [`aui_render`](`m:wxAuiManagerEvent`)
""".
-include("wxe.hrl").
...
The `window` parameter should specify any child window or sub-child window of the frame or window managed by `m:wxAuiManager`.
...
%% 
-doc """
`getPane/2` is used to lookup a `m:wxAuiPaneInfo` object either by window pointer or by pane name, which acts as a unique id for a window pane.
...
-doc """
Instructs `m:wxAuiManager` to use art provider specified by parameter `art_provider` for all drawing calls.

This allows pluggable look-and-feel features. The previous art provider object, if any, will be deleted by `m:wxAuiManager`.
...
`m:wxAuiManager` by default limits the size of any new dock to 1/3 of the window size. For horizontal docks, this would be 1/3 of the window height. For vertical docks, 1/3 of the width.
...
`update/1` must be invoked after `addPane/4` or `insertPane/4` are called in order to "realize" or "commit" the changes. In addition, any number of changes may be made to `m:wxAuiPaneInfo` structures (retrieved with `getPane/2`), but to realize the changes, `update/1` must be called. This construction allows pane flicker to be avoided by updating the whole layout at one time.
""".

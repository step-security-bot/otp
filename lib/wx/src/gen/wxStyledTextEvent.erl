...
wxWidgets docs: [wxStyledTextEvent](https://docs.wxwidgets.org/3.1/classwx_styled_text_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxStyledTextEventType`](`t:wxStyledTextEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
Many of the `m:wxStyledTextCtrl` methods such as `wxStyledTextCtrl:insertText/3` and `wxStyledTextCtrl:paste/1` have an event number associated with them. This method returns that number while a macro is being recorded so that the macro can be played back later.

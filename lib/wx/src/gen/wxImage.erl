...
A `m:wxImage` cannot (currently) be drawn directly to a `m:wxDC`. Instead, a platform-specific `m:wxBitmap` object must be created from it using the wxBitmap::wxBitmap(wxImage,int depth) constructor. This bitmap can then be drawn in a device context, using `wxDC:drawBitmap/4`.

More on the difference between `m:wxImage` and `m:wxBitmap`: `m:wxImage` is just a buffer of RGB bytes with an optional buffer for the alpha bytes. It is all generic, platform independent and image file format independent code. It includes generic code for scaling, resizing, clipping, and other manipulations of the image data. OTOH, `m:wxBitmap` is intended to be a wrapper of whatever is the native image format that is quickest/easiest to draw to a DC or to be the target of the drawing operations performed on a `m:wxMemoryDC`. By splitting the responsibilities between wxImage/wxBitmap like this then it's easier to use generic code shared by all platforms and image types for generic operations and platform specific code where performance or compatibility is needed.

One colour value of the image may be used as a mask colour which will lead to the automatic creation of a `m:wxMask` object associated to the bitmap object.

Alpha channel support

Starting from wxWidgets 2.5.0 `m:wxImage` supports alpha channel data, that is in addition to a byte for the red, green and blue colour components for each pixel it also stores a byte representing the pixel opacity.
...
See: `m:wxBitmap`, ?wxInitAllImageHandlers(), `wxPixelData` (not implemented in wx)
...
If `static_data` is false then the `m:wxImage` will take ownership of the data and free it afterwards. For this, it has to be allocated with `malloc`.
""".
...
Some of the `m:wxImage` handlers have been modified to set the palette if one exists in the image file (usually 256 or less colour images in GIF or PNG format).
""".
...
Adds standard image format handlers. It only install wxBMPHandler for the time being, which is used by `m:wxBitmap`.
...
-doc "Returns true if the given pixel is transparent, i.e. either has the mask colour if this image has a mask or if this image has alpha channel and alpha value of this pixel is strictly less than `threshold`.".
...
-doc "Rotates the hue of each pixel in the image by `angle`, which is a double in the range of -1.0 to +1.0, where -1.0 corresponds to -360 degrees and +1.0 corresponds to +360 degrees.".
...
This is also useful for scaling bitmaps in general as the only other way to scale bitmaps is to blit a `m:wxMemoryDC` into another `m:wxMemoryDC`.
...
If the pointer is not NULL, it must have one byte for each image pixel and be allocated with malloc(). `m:wxImage` takes ownership of the pointer and will free it unless `static_data` parameter is set to true - in this case the caller should do it.
""".
...
If `static_data` is false, after this call the pointer to the data is owned by the `m:wxImage` object, that will be responsible for deleting it. Do not pass to this function a pointer obtained through `getData/1`.
""".
...
The palette may be used when converting `m:wxImage` to `m:wxBitmap` (MSW only at present) or in file save operations (none as yet).
""".

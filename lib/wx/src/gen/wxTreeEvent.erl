...
To process input from a tree control, use these event handler macros to direct input to member functions that take a `m:wxTreeEvent` argument.

See: `m:wxTreeCtrl`

This class is derived (and can use functions) from: `m:wxNotifyEvent` `m:wxCommandEvent` `m:wxEvent`

wxWidgets docs: [wxTreeEvent](https://docs.wxwidgets.org/3.1/classwx_tree_event.html)

## Events

Use `wxEvtHandler:connect/3` with [`wxTreeEventType`](`t:wxTreeEventType/0`) to subscribe to events of this type.
""".
-include("wxe.hrl").
...
-doc "Returns the old item index (valid for `EVT_TREE_SEL_CHANGING` and `EVT_TREE_SEL_CHANGED` events).".
...
In both cases the position is in client coordinates - i.e. relative to the `m:wxTreeCtrl` window (so that you can pass it directly to e.g. `wxWindow:popupMenu/4`).
""".

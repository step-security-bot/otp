...
`m:wxStyledTextCtrl` is a 1 to 1 mapping of "raw" scintilla interface, whose documentation can be found in the Scintilla website ([http://www.scintilla.org/](http://www.scintilla.org/)).
...
Links for quick access to the various categories of `m:wxStyledTextCtrl` functions:

See: `m:wxStyledTextEvent`

This class is derived (and can use functions) from: `m:wxControl` `m:wxWindow` `m:wxEvtHandler`

wxWidgets docs: [wxStyledTextCtrl](https://docs.wxwidgets.org/3.1/classwx_styled_text_ctrl.html)
""".
-include("wxe.hrl").
...
-doc "Remember the current position in the undo history as the position at which the document was saved.".
...
-doc "Are white space characters currently visible? Returns one of wxSTC_WS_* constants.".
...
-doc "Find the position from a point within the window but return wxSTC_INVALID_POSITION if not close to text.".
...
-doc "Retrieve the current end of line mode - one of wxSTC_EOL_CRLF, wxSTC_EOL_CR, or wxSTC_EOL_LF.".
...
-doc "Change style from current styling position for length characters to a style and move the current styling position to after this newly styled segment.".
...
-doc "If drawing is buffered then each line of text is drawn into a bitmap buffer before drawing it to the screen to avoid flicker.".
...
-doc "Set the foreground colour of the main and additional selections and whether to use this setting.".
...
-doc "Set the background colour of the main and additional selections and whether to use this setting.".
...
-doc "Should the auto-completion list be cancelled if the user backspaces to a position before where the box was created.".
...
-doc "Define a set of characters that when typed will cause the autocompletion to choose the selected item.".
...
-doc "Set whether or not autocompletion deletes any word characters after the inserted text upon completion.".
...
-doc "Retrieve whether or not autocompletion deletes any word characters after the inserted text upon completion.".
...
-doc "Indentation will only use space characters if useTabs is false, otherwise it will use a combination of tabs and spaces.".
...
-doc "Sets the position that starts the target which is used for updating the document without affecting the scroll position.".
...
-doc "Sets the position that ends the target which is used for updating the document without affecting the scroll position.".
...
-doc "Split the lines in the target into lines that are less wide than pixelWidth where possible.".
...
-doc "Like HomeExtend but when word-wrap is enabled extends first to start of display line HomeDisplayExtend, then to start of document line HomeExtend.".
...
-doc "Like LineEnd but when word-wrap is enabled goes first to end of display line LineEndDisplay, then to start of document line LineEnd.".
...
-doc "Like LineEndExtend but when word-wrap is enabled extends first to end of display line LineEndDisplayExtend, then to start of document line LineEndExtend.".
...
-doc "Like VCHome but when word-wrap is enabled goes first to start of display line VCHomeDisplay, then behaves like VCHome.".
...
-doc "Like VCHomeExtend but when word-wrap is enabled extends first to start of display line VCHomeDisplayExtend, then behaves like VCHomeExtend.".
...
-doc "Sets the cursor to one of the wxSTC_CURSOR* values.".
...
-doc "Change the way control characters are displayed: If symbol is < 32, keep the drawn way, else, use the given character.".
...
-doc "Set the selection mode to stream (wxSTC_SEL_STREAM) or rectangular (wxSTC_SEL_RECTANGLE/wxSTC_SEL_THIN) or by lines (wxSTC_SEL_LINES).".
...
-doc "Move caret to first position on line, extending rectangular selection to new caret position.".
...
-doc "Move caret to last position on line, extending rectangular selection to new caret position.".
...
-doc "Move caret to top of page, or one page up if already at top of page, extending selection to new caret position.".
...
-doc "Move caret to bottom of page, or one page down if already at bottom of page, extending selection to new caret position.".
...
-doc "Move caret left one word, position cursor at end of word, extending selection to new caret position.".
...
-doc "Move caret right one word, position cursor at end of word, extending selection to new caret position.".
...
-doc "Set style size, face, bold, italic, and underline attributes from a `m:wxFont`'s attributes.".
...
-doc "Perform one of the operations defined by the wxSTC_CMD_* constants.".

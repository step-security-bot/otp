...
By default, a `m:wxAuiNotebook` uses an instance of this class called `wxAuiDefaultTabArt` (not implemented in wx) which provides bitmap art and a colour scheme that is adapted to the major platforms' look. You can either derive from that class to alter its behaviour or write a completely new tab art class.
...
wxWidgets docs: [wxAuiTabArt](https://docs.wxwidgets.org/3.1/classwx_aui_tab_art.html)
""".
-include("wxe.hrl").

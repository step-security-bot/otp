...
This module implements a text based interface to the [`trace/3`](`erlang:trace/3`) and the [`trace_pattern/2`](`erlang:trace_pattern/2`) BIFs. It makes it possible to trace functions, processes, ports and messages.
...
For more examples of how to use `dbg` from the Erlang shell, see the [simple example](`m:dbg#simple_example`) section.

The utilities are also suitable to use in system testing on large systems, where other tools have too much impact on the system performance. Some primitive support for sequential tracing is also included, see the [advanced topics](`m:dbg#advanced`) section.
...
`n` stands for *n*ode. The `dbg` server keeps a list of nodes where tracing should be performed. Whenever a `tp/2` call or a `p/2` call is made, it is executed for all nodes in this list including the local node (except for `p/2` with a specific `t:pid()` or `t:port()` as first argument, in which case the command is executed only on the node where the designated process or port resides).

This function adds a remote node (`Nodename`) to the list of nodes where tracing is performed. It starts a tracer process on the remote node, which will send all trace messages to the tracer process on the local node (via the Erlang distribution). If no tracer process is running on the local node, the error reason `no_local_tracer` is returned. The tracer process on the local node must be started with the [`tracer/0/2`](`tracer/2`) function.
...
`cn` stands for *c*lear *n*ode. Clears a node from the list of traced nodes. Subsequent calls to `tp/2` and `p/2` will not consider that node, but tracing already activated on the node will continue to be in effect.
...
%% List traced nodes
%%
-doc "`ln` stands for *l*ist *n*odes. Shows the list of traced nodes on the console.".
ln() ->
    lists:foreach(fun(X) ->
                          io:format("~p~n",[X])
                  end,
...
`tp` stands for *t*race *p*attern. This function enables call trace for one or more functions. All exported functions matching the `{Module, Function, Arity}` argument will be concerned, but the `match_spec()` may further narrow down the set of function calls generating trace messages.

For a description of the `match_spec()` syntax, please turn to the *User's guide* part of the online documentation for the runtime system (*erts*). The chapter [*Match Specifications in Erlang*](`p:erts:match_spec.md`) explains the general match specification "language". The most common generic match specifications used can be found as `Built-inAlias`', see `ltp/0` below for details.
...
`tpl` stands for *t*race *p*attern *l*ocal. This function works as `tp/2`, but enables tracing for local calls (and local functions) as well as for global calls (and functions).
""".
...
`tpe` stands for *t*race *p*attern *e*vent. This function associates a match specification with trace event `send` or `'receive'`. By default all executed `send` and `'receive'` events are traced if enabled for a process. A match specification can be used to filter traced events based on sender, receiver and/or message content.

For a description of the `match_spec()` syntax, please turn to the *User's guide* part of the online documentation for the runtime system (*erts*). The chapter [*Match Specifications in Erlang*](`p:erts:match_spec.md`) explains the general match specification "language".
...
`ctp` stands for *c*lear *t*race *p*attern. This function disables call tracing on the specified functions. The semantics of the parameter is the same as for the corresponding function specification in `tp/2` or `tpl/2`. Both local and global call trace is disabled.
...
`ctpl` stands for *c*lear *t*race *p*attern *l*ocal. This function works as `ctp/1`, but only disables tracing set up with `tpl/2` (not with `tp/2`).
""".
...
`ctpg` stands for *c*lear *t*race *p*attern *g*lobal. This function works as `ctp/1`, but only disables tracing set up with `tp/2` (not with `tpl/2`).
""".
...
`ctpe` stands for *c*lear *t*race *p*attern *e*vent. This function clears match specifications for the specified trace event (`send` or `'receive'`). It will revert back to the default behavior of tracing all triggered events.
...
%%
-doc """
`ltp` stands for *l*ist *t*race *p*atterns. Use this function to recall all match specifications previously used in the session (i. e. previously saved during calls to `tp/2`, and built-in match specifications. This is very useful, as a complicated match_spec can be quite awkward to write. Note that the match specifications are lost if `stop/0` is called.
...
`wtp` stands for *w*rite *t*race *p*atterns. This function will save all match specifications saved during the session (during calls to `tp/2`) and built-in match specifications in a text file with the name designated by `Name`. The format of the file is textual, why it can be edited with an ordinary text editor, and then restored with `rtp/1`.
...
if `Type` is `module`, then the second parameter should be either a tuple describing the `m:erl_tracer` module to be used for tracing and the state to be used for that tracer module or a fun returning the same tuple.
...
-doc "Equivalent to `flush_trace_port(node())`.".
flush_trace_port() ->
    trace_port_control(flush).
-doc "Equivalent to `trace_port_control(Nodename,flush)`.".
flush_trace_port(Node) ->
    trace_port_control(Node, flush).

-doc "Equivalent to `trace_port_control(node(),Operation)`.".
...
* __`flush`__ - This function is used to flush the internal buffers held by a trace port driver. Currently only the file trace driver supports this operation. Returns `ok`.

* __`get_listen_port`__ - Returns `{ok, IpPort}` where `IpPort` is the IP port number used by the driver listen socket. Only the ip trace driver supports this operation.
""".
trace_port_control(Node, flush) ->
...
`p` stands for *p*rocess. Traces `Item` in accordance to the value specified by `Flags`. The variation of `Item` is listed below:

* __`t:pid()` or `t:port()`__ - The corresponding process or port is traced. The process or port may be a remote process or port (on another Erlang node). The node must be in the list of traced nodes (see `n/1` and `tracer/3`).

* __`all`__ - All processes and ports in the system as well as all processes and ports created hereafter are to be traced.

* __`processes`__ - All processes in the system as well as all processes created hereafter are to be traced.

* __`ports`__ - All ports in the system as well as all ports created hereafter are to be traced.

* __`new`__ - All processes and ports created after the call is are to be traced.

* __`new_processes`__ - All processes created after the call is are to be traced.

* __`new_ports`__ - All ports created after the call is are to be traced.

* __`existing`__ - All existing processes and ports are traced.

* __`existing_processes`__ - All existing processes are traced.

* __`existing_ports`__ - All existing ports are traced.

* __`t:atom()`__ - The process or port with the corresponding registered name is traced. The process or port may be a remote process (on another Erlang node). The node must be added with the `n/1` or `tracer/3` function.

* __`t:integer()`__ - The process `<0.Item.0>` is traced.

* __`{X, Y, Z}`__ - The process `<X.Y.Z>` is traced.

* __`t:string()`__ - If the `Item` is a string "<X.Y.Z>" as returned from [`pid_to_list/1`](`erlang:pid_to_list/1`), the process `<X.Y.Z>` is traced.
...
* __`s (send)`__ - Traces the messages the process or port sends.

* __`r (receive)`__ - Traces the messages the process or port receives.

* __`m (messages)`__ - Traces the messages the process or port receives and sends.

* __`c (call)`__ - Traces global function calls for the process according to the trace patterns set in the system (see tp/2).

* __`p (procs)`__ - Traces process related events to the process.

* __`ports`__ - Traces port related events to the port.

* __`sos (set on spawn)`__ - Lets all processes created by the traced process inherit the trace flags of the traced process.

* __`sol (set on link)`__ - Lets another process, `P2`, inherit the trace flags of the traced process whenever the traced process links to `P2`.

* __`sofs (set on first spawn)`__ - This is the same as `sos`, but only for the first process spawned by the traced process.

* __`sofl (set on first link)`__ - This is the same as `sol`, but only for the first call to [`link/1`](`erlang:link/1`) by the traced process.

* __`all`__ - Sets all flags except `silent`.

* __`clear`__ - Clears all flags.
...
-doc "`i` stands for *i*nformation. Displays information about all traced processes and ports.".
i() -> req(i).
	
-doc "Equivalent to `c(Mod, Fun, Args, all)`.".
c(M, F, A) ->
    c(M, F, A, all).
-doc "`c` stands for *c*all. Evaluates the expression `apply(Mod, Fun, Args)` with the trace flags in `Flags` set. This is a convenient way to trace processes from the Erlang shell.".
...
-doc "Stops the `dbg` server, clears all trace flags for all processes, clears all trace patterns for all functions, clears trace patterns for send/receive, shuts down all trace clients, and closes all trace ports.".
stop() ->
    {ok, _} = ctp(),
    {ok, _} = ctpe('receive'),
    {ok, _} = ctpe('send'),
...
-doc "Equivalent to `get_tracer(node())`.".
get_tracer() ->
    req({get_tracer,node()}).
-doc """
Nodename = atom()  
...
-doc "`h` stands for *h*elp. Gives a list of items for brief online help.".
h() ->
    help_display(
      [
...
`h` stands for *h*elp. Gives a brief help text for functions in the dbg module. The available items can be listed with `dbg:h/0`.
""".
h(p) ->

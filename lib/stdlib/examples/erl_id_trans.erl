...
record_defs([{record_field,Anno,{atom,La,A},Val0}|Is]) ->
    Val1 = expr(Val0),
    [{record_field,Anno,{atom,La,A},Val1}|record_defs(Is)];
record_defs([{record_field,Anno,{atom,La,A}}|Is]) ->
    [{record_field,Anno,{atom,La,A}}|record_defs(Is)];
record_defs([{typed_record_field,{record_field,Anno,{atom,La,A},Val0},Type}|
             Is]) ->
    Val1 = expr(Val0),
    Type1 = type(Type),
    [{typed_record_field,{record_field,Anno,{atom,La,A},Val1},Type1}|
     record_defs(Is)];
record_defs([{typed_record_field,{record_field,Anno,{atom,La,A}},Type}|Is]) ->
    Type1 = type(Type),
    [{typed_record_field,{record_field,Anno,{atom,La,A}},Type1}|
     record_defs(Is)];
record_defs([]) -> [].
...
guard_test(Expr={call,Anno,{atom,La,F},As0}) ->
    case erl_internal:type_test(F, length(As0)) of
	true -> 
	    As1 = gexpr_list(As0),
	    {call,Anno,{atom,La,F},As1};
	_ ->
	    gexpr(Expr)
    end;
guard_test(Any) ->
...
gexpr({call,Anno,{remote,La,{atom,Lb,erlang},{atom,Lc,F}},As0}) ->
    case erl_internal:guard_bif(F, length(As0)) or
	 erl_internal:arith_op(F, length(As0)) or 
	 erl_internal:comp_op(F, length(As0)) or
	 erl_internal:bool_op(F, length(As0)) of
	true -> As1 = gexpr_list(As0),
		{call,Anno,{remote,La,{atom,Lb,erlang},{atom,Lc,F}},As1}
    end;
gexpr({bin,Anno,Fs}) ->
    Fs2 = pattern_grp(Fs),
    {bin,Anno,Fs2};
...
function_type({type,Anno,'fun',[{type,Lt,product,As},B]}) ->
    As1 = type_list(As),
    B1 = type(B),
    {type,Anno,'fun',[{type,Lt,product,As1},B1]}.
...
constraint({type,Anno,constraint,[{atom,L,A},[V,T]]}) ->
    V1 = type(V),
    T1 = type(T),
    {type,Anno,constraint,[{atom,L,A},[V1,T1]]}.
...
type({type,Anno,map,Ps}) ->
    ...
    {type,Anno,map,Ps1};
type({type,Anno,record,[{atom,La,N}|Fs]}) ->
    Fs1 = field_types(Fs),
    {type,Anno,record,[{atom,La,N}|Fs1]};
...
field_types([{type,Anno,field_type,[{atom,La,A},T]}|Fs]) ->
    T1 = type(T),
    [{type,Anno,field_type,[{atom,La,A},T1]}|field_types(Fs)];
field_types([]) -> [].

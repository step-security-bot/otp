...
Functions used for implementation of processes are also expected to understand system messages, such as debug messages and code change. These functions must be used to implement the use of system messages for a process; either directly, or through standard behaviors, such as `m:gen_server`.
...
[](){: id=dbg_opt }
The functions make references to a debug structure. The debug structure is a list of `t:dbg_opt()`, which is an internal data type used by function `handle_system_msg/6`. No debugging is performed if it is an empty list.
...
-type name()         :: pid() | atom()
                      ...
                      | {'via', module(), term()}.
-doc """
* __`{in,Msg}`__ - Is produced by `gen_server` and `gen_event` when the message `Msg` arrives.

* __`{in,Msg,State}`__ - Is produced by `gen_statem` when the message `Msg` arrives in state `State`.

  For `gen_statem` the `Msg` term is an `{EventType,EventContent}` tuple.

* __`{out,Msg,To}`__ - Is produced by `gen_statem` when the reply `Msg` is sent back to `To` by returning a `{reply,To,Msg}` action from the callback module.

  `To` is of the same type as the first argument to `gen_statem:reply/2`.

* __`{out,Msg,To,State}`__ - Is produced by `gen_server` when the reply `Msg` is sent back to `To` by returning a `{reply,...}` tuple from the callback module.

  `To` is of the same type as the first argument to `gen_server:reply/2`.

  `State` is the new server state.

* __`{noreply,State}`__ - Is produced by `gen_server` when a `{noreply,...}` tuple is returned from the callback module.

  `State` is the new server state.

* __`{continue,Continuation}`__ - Is produced by `gen_server` when a `{continue,Continuation}` tuple is returned from the callback module.

* __`{postpone,Event,State,NextState}`__ - Is produced by `gen_statem` when the message `Event` is postponed in state `State`. `NextState` is the new state.

  `Event` is an `{EventType,EventContent}` tuple.

* __`{consume,Event,State,NextState}`__ - Is produced by `gen_statem` when the message `Event` is consumed in state `State`. `NextState` is the new state.

  `Event` is an `{EventType,EventContent}` tuple.

* __`{start_timer,Action,State}`__ - Is produced by `gen_statem` when the action `Action` starts a timer in state `State`.

* __`{insert_timeout,Event,State}`__ - Is produced by `gen_statem` when a timeout zero action inserts event `Event` in state `State`.

  `Event` is an `{EventType,EventContent}` tuple.

* __`{enter,Module,State}`__ - Is produced by `gen_statem` when module `Module` enters the first state `State`.

* __`{module,Module,State}`__ - Is produced by `gen_statem` when setting module `Module` in state `State`.

* __`{terminate,Reason,State}`__ - Is produced by `gen_statem` when it terminates with reason `Reason` in state `State`.
""".
...
-doc "Suspends the process. When the process is suspended, it only responds to other system messages, but not other messages.".
...
* A `m:gen_statem` process returns information, such as its current state name and state data.
* A `m:gen_event` process returns information about each of its registered handlers.

Callback modules for `gen_server`, `gen_statem`, and `gen_event` can also change the value of `Misc` by exporting a function `format_status/2`, which contributes module-specific information. For details, see `c:gen_server:format_status/2`, `c:gen_statem:format_status/2`, and `c:gen_event:format_status/2`.
""".
...
* For a `m:gen_server` process, the returned `State` is the state of the callback module.
* For a `m:gen_statem` process, `State` is the tuple `{CurrentState,CurrentData}`.
* For a `m:gen_event` process, `State` is a list of tuples, where each tuple corresponds to an event handler registered in the process and contains `{Module, Id, HandlerState}`, as follows:

  * __`Module`__ - The module name of the event handler.

  * __`Id`__ - The ID of the handler (which is `false` if it was registered without an ID).

  * __`HandlerState`__ - The state of the handler.

  

If the callback module exports a function `c:system_get_state/1`, it is called in the target process to get its state. Its argument is the same as the `Misc` value returned by [`get_status/1,2`](`get_status/1`), and function [`Module:system_get_state/1`](`c:system_get_state/1`) is expected to extract the state of the callback module from it. Function `c:system_get_state/1` must return `{ok, State}`, where `State` is the state of the callback module.

If the callback module does not export a `c:system_get_state/1` function, `get_state/1,2` assumes that the `Misc` value is the state of the callback module and returns it directly instead.

If the callback module's `c:system_get_state/1` function crashes or throws an exception, the caller exits with error `{callback_failed, {Module, system_get_state}, {Class, Reason}}`, where `Module` is the name of the callback module and `Class` and `Reason` indicate details of the exception.

Function `c:system_get_state/1` is primarily useful for user-defined behaviors and modules that implement OTP [special processes](`m:sys#process-implementation-functions`). The `gen_server`, `gen_statem`, and `gen_event` OTP behavior modules export this function, so callback modules for those behaviors need not to supply their own.
...
* For a `m:gen_server` process, `State` is the state of the callback module and `NewState` is a new instance of that state.
* For a `m:gen_statem` process, `State` is the tuple `{CurrentState,CurrentData}`, and `NewState` is a similar tuple, which can contain a new current state, new state data, or both.
* For a `m:gen_event` process, `State` is the tuple `{Module, Id, HandlerState}` as follows:
...
If the callback module exports a `c:system_replace_state/2` function, it is called in the target process to replace its state using `StateFun`. Its two arguments are `StateFun` and `Misc`, where `Misc` is the same as the `Misc` value returned by [`get_status/1,2`](`get_status/1`). A `c:system_replace_state/2` function is expected to return `{ok, NewState, NewMisc}`, where `NewState` is the new state of the callback module, obtained by calling `StateFun`, and `NewMisc` is a possibly new value used to replace the original `Misc` (required as `Misc` often contains the state of the callback module within it).

If the callback module does not export a `c:system_replace_state/2` function, [`replace_state/2,3`](`replace_state/2`) assumes that `Misc` is the state of the callback module, passes it to `StateFun` and uses the return value as both the new state and as the new value of `Misc`.

If the callback module's function `c:system_replace_state/2` crashes or throws an exception, the caller exits with error `{callback_failed, {Module, system_replace_state}, {Class, Reason}}`, where `Module` is the name of the callback module and `Class` and `Reason` indicate details of the exception. If the callback module does not provide a `c:system_replace_state/2` function and `StateFun` crashes or throws an exception, the caller exits with error `{callback_failed, StateFun, {Class, Reason}}`.

Function `c:system_replace_state/2` is primarily useful for user-defined behaviors and modules that implement OTP [special processes](`m:sys#process-implementation-functions`). The OTP behavior modules `gen_server`, `gen_statem`, and `gen_event` export this function, so callback modules for those behaviors need not to supply their own.
""".
-doc(#{since => <<"OTP R16B01">>}).
...
-doc "Tells the process to change code. The process must be suspended to handle this message. Argument `Extra` is reserved for each process to use as its own. Function [`Module:system_code_change/4`](`c:system_code_change/4`) is called. `OldVsn` is the old version of the `Module`.".
...
-doc "Orders the process to terminate with the specified `Reason`. The termination is done asynchronously, so it is not guaranteed that the process is terminated when the function returns.".
-doc(#{since => <<"OTP 18.0">>}).
...
If `Flag` is `print`, the logged events are printed to [`standard_io`](`t:io:standard_io/0`).

The events are formatted with a function that is defined by the process that generated the event (with a call to [`handle_debug/4`)](`handle_debug/4`).
""".
...
-doc "Prints all system events on [`standard_io`](`t:io:standard_io/0`). The events are formatted with a function that is defined by the process that generated the event (with a call to `handle_debug/4`).".
...
-doc "Enables or disables the logging of all system events in text format to the file. The events are formatted with a function that is defined by the process that generated the event (with a call to `handle_debug/4`). The file is opened with encoding UTF-8.".
...
-doc "Enables or disables the collection of statistics. If `Flag` is `get`, the statistical collection is returned.".
...
-doc "Turns off all debugging for the process. This includes functions that are installed explicitly with function [`install/2,3`](`install/2`), for example, triggers.".
...
-doc "Removes an installed debug function from the process. `Func` or `FuncId` must be the same as previously installed.".
...
* [`Module:system_continue(Parent, NDebug, Misc)`](`c:system_continue/3`), where the process continues the execution.
* [`Module:system_terminate(Reason, Parent, Debug, Misc)`](`c:system_terminate/4`), if the process is to terminate.

`Module` must export the following:

* `c:system_continue/3`
* `c:system_terminate/4`
* `c:system_code_change/4`
* `c:system_get_state/1`
* `c:system_replace_state/2`

Argument `Misc` can be used to save internal data in a process, for example, its state. It is sent to [`Module:system_continue/3`](`c:system_continue/3`) or [`Module:system_terminate/4`](`c:system_terminate/4`).
""".
-doc(#{title => <<"Process Implementation Functions">>}).
...
-doc(#{deprecated =>
       ...
       title => <<"Process Implementation Functions">>}).
...
-doc "Prints the logged system events in the debug structure, using `FormFunc` as defined when the event was generated by a call to `handle_debug/4`.".
-doc(#{title => <<"Process Implementation Functions">>}).
...
-doc "Returns the logged system events in the debug structure, that is the last argument to `handle_debug/4`.".
-doc(#{title => <<"Process Implementation Functions">>,
       since => <<"OTP-22.0">>}).
...
-doc "Can be used by a process that initiates a debug structure from a list of options. The values of argument `Opt` are the same as for the corresponding functions.".
-doc(#{title => <<"Process Implementation Functions">>}).

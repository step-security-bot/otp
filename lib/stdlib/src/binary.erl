...
-doc "Opaque data type representing a compiled search pattern. Guaranteed to be a `t:tuple()` to allow programs to distinguish it from non-precompiled search patterns.".
-opaque cp() :: {'am' | 'bm', reference()}.
-doc """
A representation of a part (or range) in a binary. `Start` is a zero-based offset into a `t:binary()` and `Length` is the length of that part. As input to functions in this module, a reverse part specification is allowed, constructed with a negative `Length`, so that the part of the binary begins at `Start` \+ `Length` and is -`Length` long. This is useful for referencing the last `N` bytes of a binary as `{size(Binary), -N}`. The functions in this module always return `t:part()`s with positive `Length`.
""".
...
-doc "Returns the byte at position `Pos` (zero-based) in binary `Subject` as an integer. If `Pos` >= `byte_size(Subject)`, a `badarg` exception is raised.".
-doc(#{since => <<"OTP R14B">>}).
...
Converts `Subject` to a list of `t:byte()`s, each representing the value of one byte. `PosLen` or alternatively `Pos` and `Len` denote which part of the `Subject` binary to convert. By default, the entire `Subject` binary is converted.
...
-doc """
Builds an internal structure representing a compilation of a search pattern, later to be used in functions `match/3`, `matches/3`, `split/3`, or `replace/4`. The `t:cp()` returned is guaranteed to be a `t:tuple()` to allow programs to distinguish it from non-precompiled search patterns.
...
-doc """
Creates a binary with the content of `Subject` duplicated `N` times. The default for `N` is `1`.
...
-doc """
Converts the binary digit representation, in big endian or little endian, of a positive integer in `Subject` to an Erlang `t:integer()`. The default `Endianness` is `big`.
...
-doc "Returns the first byte of binary `Subject` as an integer. If the size of `Subject` is zero, a `badarg` exception is raised.".
-doc(#{since => <<"OTP R14B">>}).
...
-doc "Returns the last byte of binary `Subject` as an integer. If the size of `Subject` is zero, a `badarg` exception is raised.".
-doc(#{since => <<"OTP R14B">>}).
...
* __\{scope, \{Start, Length\}\}__ - Only the specified part is searched. Return values still have offsets from the beginning of `Subject`. A negative `Length` is allowed as described in section Data Types in this manual.
...
> #### Note {: class=info }
> `part/2` and `part/3` are also available in the `m:erlang` module under the names `binary_part/2` and `binary_part/3`. Those BIFs are allowed in guard tests.
...
In this example, we chose to copy the binary content before inserting it in `t:gb_sets:set()` if it references a binary more than twice the data size we want to keep. Of course, different rules apply when copying to different programs.
...
* __\{scope, part()\}__ - Works as in `match/3` and `matches/3`. Notice that this only defines the scope of the search for matching strings, it does not cut the binary before splitting. The bytes before and after the scope are kept in the result. See the example below.

* __trim__ - Removes trailing empty parts of the result (as does `trim` in `re:split/3`.

* __trim_all__ - Removes all empty parts of the result.

* __global__ - Repeats the split until `Subject` is exhausted. Conceptually option `global` makes split work on the positions returned by `matches/3`, while it normally works on the position returned by `match/3`.
...
-doc """
Constructs a new binary by replacing the parts in `Subject` matching `Pattern` with `Replacement` if given as a literal `t:binary()` or with the result of applying `Replacement` to a matching subpart if given as a `fun`.

If `Replacement` is given as a `t:binary()` and the matching subpart of `Subject` giving raise to the replacement is to be inserted in the result, option `{insert_replaced, InsPos}` inserts the matching part into `Replacement` at the specified position (or positions) before inserting `Replacement` into `Subject`. If `Replacement` is given as a `fun` instead, this option is ignored.

If any position specified in `InsPos` > size of the replacement binary, a `badarg` exception is raised.

Options `global` and `{scope, part()}` work as for `split/3`. The return type is always a `t:binary()`.

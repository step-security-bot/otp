...
The Erlang code preprocessor includes functions that are used by the `m:compile` module to preprocess macros and include files before the parsing takes place.

The Erlang source file *encoding*{: id=encoding } is selected by a comment in one of the first two lines of the source file. The first string matching the regular expression `coding\s*[:=]\s*([-a-zA-Z0-9])+` selects the encoding. If the matching string is not a valid encoding, it is ignored. The valid encodings are `Latin-1` and `UTF-8`, where the case of the characters can be chosen freely.
...
-doc "Equivalent to `epp:open([{name, FileName}, {includes, IncludePath}, {macros, PredefMacros}])`.".
...
The option `location` is forwarded to the Erlang token scanner, see [`erl_scan:tokens/3,4`](`erl_scan:tokens/3`).

The `{compiler_internal,term()}` option is forwarded to the Erlang token scanner, see [`{compiler_internal,term()}`](`m:erl_scan#compiler_interal`).
""".
-doc(#{since => <<"OTP 17.0">>}).
-spec open(Options) ->
...
-doc "Returns the raw tokens of the next Erlang form from the opened Erlang source file. A tuple `{eof, Line}` is returned at the end of the file. The first form corresponds to an implicit attribute `-file(File,1).`, where `File` is the file name.".
...
-doc "Returns the next Erlang form from the opened Erlang source file. Tuple `{eof, Location}` is returned at the end of the file. The first form corresponds to an implicit attribute `-file(File,1).`, where `File` is the file name.".
...
-doc "Takes an `ErrorDescriptor` and returns a string that describes the error or warning. This function is usually called implicitly when processing an `ErrorInfo` structure (see section [Error Information](`m:epp#errorinfo`)).".
...
-doc "Equivalent to `epp:parse_file(FileName, [{includes, IncludePath}, {macros, PredefMacros}])`.".
...
-doc "Returns a string representation of an encoding. The string is recognized by [`read_encoding/1,2`](`read_encoding/1`), [`read_encoding_from_binary/1,2`](`read_encoding_from_binary/1`), and [`set_encoding/1,2`](`set_encoding/1`) as a valid encoding.".
-doc(#{since => <<"OTP R16B">>}).
...
-doc """
Read the [encoding](`m:epp#encoding`) from a file. Returns the read encoding, or `none` if no valid encoding is found.
...
-doc """
Reads the [encoding](`m:epp#encoding`) from an I/O device and sets the encoding of the device accordingly. The position of the I/O device referenced by `File` is not affected. If no valid encoding can be read from the I/O device, the encoding of the I/O device is set to the default encoding.
...
-doc """
Reads the [encoding](`m:epp#encoding`) from an I/O device and sets the encoding of the device accordingly. The position of the I/O device referenced by `File` is not affected. If no valid encoding can be read from the I/O device, the encoding of the I/O device is set to the [encoding](`m:epp#encoding`) specified by `Default`.
...
-doc """
Read the [encoding](`m:epp#encoding`) from a binary. Returns the read encoding, or `none` if no valid encoding is found.

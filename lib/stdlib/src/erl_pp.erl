...
-doc """
Optional argument `HookFunction`{: id=hook_function }, shown in the functions described in this module, defines a function that is called when an unknown form occurs where there is to be a valid expression. If `HookFunction` is equal to `none`, there is no hook function.
...
-doc "Pretty prints a `Form`, which is an abstract form of a type that is returned by `erl_parse:parse_form/1`.".
...
-doc "Same as [`form/1,2`](`form/1`), but only for the sequence of expressions in `Expressions`.".
...
-doc "Prints one expression. It is useful for implementing hooks (see section [Known Limitations](`m:erl_pp#knownlimitations`)).".

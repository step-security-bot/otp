...
A string in this module is represented by [`unicode:chardata()`](`t:unicode:chardata/0`), that is, a list of codepoints, binaries with UTF-8-encoded codepoints (*UTF-8 binaries*), or a mix of the two.
...
This module has been reworked in Erlang/OTP 20 to handle [`unicode:chardata()`](`t:unicode:chardata/0`) and operate on grapheme clusters. The [`old functions`](`m:string#obsolete-api-functions`) that only work on Latin-1 lists as input are still available but should not be used, they will be deprecated in a future release.
...
If `IgnoreCase` is `true` the function does [`casefold`ing](`casefold/1`) on the fly before the equality test.

If `Norm` is not `none` the function applies normalization on the fly before the equality test. There are four available normalization forms: [`nfc`](`unicode:characters_to_nfc_list/1`), [`nfd`](`unicode:characters_to_nfd_list/1`), [`nfkc`](`unicode:characters_to_nfkc_list/1`), and [`nfkd`](`unicode:characters_to_nfkd_list/1`).
...
Default `Characters` is the set of nonbreakable whitespace codepoints, defined as Pattern_White_Space in [Unicode Standard Annex #31](http://unicode.org/reports/tr31/). `By default, Dir` is `both`.
...
This function is [obsolete](`m:string#obsolete-api-functions`). Use `[String1, String2]` as `Data` argument, and call `unicode:characters_to_list/2` or `unicode:characters_to_binary/2` to flatten the output.
""".
-doc(#{title => <<"Obsolete API functions">>}).
...
This function is [obsolete](`m:string#obsolete-api-functions`). Use `lists:duplicate/2`.
""".
-doc(#{title => <<"Obsolete API functions">>}).
...
-doc """
Returns the word in position `Number` of `String`. Words are separated by blanks or `Character`s.

This function is [obsolete](`m:string#obsolete-api-functions`). Use `nth_lexeme/3`.
...
-doc """
Returns `String` with the length adjusted in accordance with `Number`. The left margin is fixed. If `length(String)` < `Number`, then `String` is padded with blanks or `Character`s.

This function is [obsolete](`m:string#obsolete-api-functions`). Use `pad/2` or `pad/3`.
...
-doc """
Returns `String` with the length adjusted in accordance with `Number`. The right margin is fixed. If the length of `(String)` < `Number`, then `String` is padded with blanks or `Character`s.
...
This function is [obsolete](`m:string#obsolete-api-functions`) use `lowercase/1`, `uppercase/1`, `titlecase/1` or `casefold/1`.
""".
-doc(#{title => <<"Obsolete API functions">>}).
...
This function is [obsolete](`m:string#obsolete-api-functions`). Use `lists:join/2`.

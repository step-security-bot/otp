...
Whenever an *ordering function*{: id=ordering_function } `F` is expected as argument, it is assumed that the following properties hold of `F` for all x, y, and z:
...
* __`fail`__ - The call will fail if the given lists are not of equal length. This is the default.

* __`trim`__ - Surplus elements from the longer list will be ignored.
...
* __`{pad, Defaults}`__ - The shorter list will be padded to the length of the longer list, using the respective elements from the given `Defaults` tuple.
...
-doc "Returns a list containing the sorted elements of `List1`, according to the [ordering function](`m:lists#ordering_function`) `Fun`. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`.".
...
-doc "Returns the sorted list formed by merging `List1` and `List2`. Both `List1` and `List2` must be sorted according to the [ordering function](`m:lists#ordering_function`) `Fun` before evaluating this function. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`. When two elements compare equal, the element from `List1` is picked before the element from `List2`.".
...
-doc "Returns a list containing the sorted elements of `List1` where all except the first element of the elements comparing equal according to the [ordering function](`m:lists#ordering_function`) `Fun` have been deleted. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`.".
...
-doc "Returns the sorted list formed by merging `List1` and `List2`. Both `List1` and `List2` must be sorted according to the [ordering function](`m:lists#ordering_function`) `Fun` and contain no duplicates before evaluating this function. `Fun(A, B)` is to return `true` if `A` compares less than or equal to `B` in the ordering, otherwise `false`. When two elements compare equal, the element from `List1` is picked and the one from `List2` is deleted.".

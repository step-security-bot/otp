...
This module provides the same interface as the `m:ordsets` module but with an undefined representation. One difference is that while this module considers two elements as different if they do not match (`=:=`), `ordsets` considers two elements as different if and only if they do not compare equal (`==`).
...
The following functions in this module also exist and provide the same functionality in the `m:gb_sets` and `m:ordsets` modules. That is, by only changing the module name for each call, you can try out different set representations.
...
-doc "Returns `true` if `Set` appears to be a set of elements, otherwise `false`. Note that the test is shallow and will return `true` for any term that coincides with the possible representations of a set. See also note on [data types](`p:system:data_types.md#no_user_types`).".
...
-doc "Returns the elements of `Set` as a list. The order of the returned elements is undefined.".
...
-doc "Folds `Function` over every element in `Set` and returns the final value of the accumulator. The evaluation order is undefined.".

...
This module provides functions for pretty-printing errors and exceptions. It is used by both the `m:shell` and by `m:proc_lib` to print exceptions.

It is possible for the module raising an error to provide additional information by calling [`error/3`](`erlang:error/3`) with extra error information. More details about this mechanism is described in [EEP-54](https://www.erlang.org/erlang-enhancement-proposals/eep-0054.html).
...
-doc """
Reason = term()  
StackTrace = [erlang:stacktrace()](`t:erlang:stacktrace/0`)  
ArgumentPosition = pos_integer()  
ErrorDescription =  
  #\{ ArgumentPosition => [unicode:chardata()](`t:unicode:chardata/0`),  
     general => [unicode:chardata()](`t:unicode:chardata/0`),  
     reason => [unicode:chardata()](`t:unicode:chardata/0`) \}  
...
* __`ArgumentPosition = pos_integer()`__ - The position of the argument that caused the error starting at 1.

* __`general`__ - An error that is not associated with any argument caused the error.

* __`reason`__ - If the `Reason` should be printed differently than the default way.

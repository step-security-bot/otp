...
The data representing `sofs` as used by this module is to be regarded as opaque by other modules. In abstract terms, the representation is a composite type of existing Erlang terms. See note on [data types](`p:system:data_types.md#no_user_types`). Any code assuming knowledge of the format is running on thin ice.
...
* The *empty set* contains no elements.
  ...
  Set A is *equal*{: id=equal } to set B if they contain the same elements, which is denoted A = B. Two ordered sets are equal if they contain the same number of elements and have equal elements at each coordinate.

  Set B is a *subset*{: id=subset } of set A if A contains all elements that B contains.

  The *union*{: id=union } of two sets A and B is the smallest set that contains all elements of A and all elements of B.

  The *intersection*{: id=intersection } of two sets A and B is the set that contains all elements of A that belong to B.

  Two sets are *disjoint*{: id=disjoint } if their intersection is the empty set.

  The *difference*{: id=difference } of two sets A and B is the set that contains all elements of A that do not belong to B.

  The *symmetric difference*{: id=symmetric_difference } of two sets is the set that contains those element that belong to either of the two sets, but not both.

  The *union*{: id=union_n } of a collection of sets is the smallest set that contains all the elements that belong to at least one set of the collection.

  The *intersection*{: id=intersection_n } of a non-empty collection of sets is the set that contains all elements that belong to every set of the collection.
* The *Cartesian product*{: id=Cartesian_product } of two sets X and Y, denoted X × Y, is the set \{a : a = (x, y) for some x in X and for some y in Y\}.

  A *relation*{: id=relation } is a subset of X × Y. Let R be a relation. The fact that (x, y) belongs to R is written as x R y. As relations are sets, the definitions of the last item (subset, union, and so on) apply to relations as well.

  The *domain*{: id=domain } of R is the set \{x : x R y for some y in Y\}.

  The *range*{: id=range } of R is the set \{y : x R y for some x in X\}.

  The *converse*{: id=converse } of R is the set \{a : a = (y, x) for some (x, y) in R\}.

  If A is a subset of X, the *image*{: id=image } of A under R is the set \{y : x R y for some x in A\}. If B is a subset of Y, the *inverse image*{: id=inverse_image } of B is the set \{x : x R y for some y in B\}.

  If R is a relation from X to Y, and S is a relation from Y to Z, the *relative product*{: id=relative_product } of R and S is the relation T from X to Z defined so that x T z if and only if there exists an element y in Y such that x R y and y S z.

  The *restriction*{: id=restriction } of R to A is the set S defined so that x S y if and only if there exists an element x in A such that x R y.

  If S is a restriction of R to A, then R is an *extension*{: id=extension } of S to X.

  If X = Y, then R is called a relation *in* X.

  The *field*{: id=field } of a relation R in X is the union of the domain of R and the range of R.

  If R is a relation in X, and if S is defined so that x S y if x R y and not x = y, then S is the *strict*{: id=strict_relation } relation corresponding to R. Conversely, if S is a relation in X, and if R is defined so that x R y if x S y or x = y, then R is the *weak*{: id=weak_relation } relation corresponding to S.
...
* A *function*{: id=function } F is a relation, a subset of X × Y, such that the domain of F is equal to X and such that for every x in X there is a unique element y in Y with (x, y) in F. The latter condition can be formulated as follows: if x F y and x F z, then y = z. In this module, it is not required that the domain of F is equal to X for a relation to be considered a function.
  ...
  If the converse of a function F is a function F', then F' is called the *inverse*{: id=inverse } of F.

  The relative product of two functions F1 and F2 is called the *composite*{: id=composite } of F1 and F2 if the range of F1 is a subset of the domain of F2.
* Sometimes, when the range of a function is more important than the function itself, the function is called a *family*.
  ...
  When the indexed set is a set of subsets of a set X, we call x a *family of subsets*{: id=family } of X.
...
* A *partition*{: id=partition } of a set X is a collection S of non-empty subsets of X whose union is X and whose elements are pairwise disjoint.

  A relation in a set is an *equivalence relation* if it is reflexive, symmetric, and transitive.

  If R is an equivalence relation in X, and x is an element of X, the *equivalence class*{: id=equivalence_class } of x with respect to R is the set of all those elements y of X for which x R y holds. The equivalence classes constitute a partitioning of X. Conversely, if C is a partition of X, the relation that holds for any two elements of X if they belong to the same equivalence class, is an equivalence relation induced by the partition C.

  If R is an equivalence relation in X, the *canonical map*{: id=canonical_map } is the function that maps every element of X onto its equivalence class.
* [](){: id=binary_relation }
  Relations as defined above (as sets of ordered pairs) are from now on referred to as *binary relations*.

  We call a set of ordered sets (x\[1], ..., x\[n]) an *(n-ary) relation*{: id=n_ary_relation }, and say that the relation is a subset of the [](){: id=Cartesian_product_tuple }
  Cartesian product X\[1] × ... × X\[n], where x\[i] is an element of X\[i], 1 <= i <= n.

  The *projection*{: id=projection } of an n-ary relation R onto coordinate i is the set \{x\[i] : (x\[1], ..., x\[i], ..., x\[n]) in R for some x\[j] in X\[j], 1 <= j <= n and not i = j\}. The projections of a binary relation R onto the first and second coordinates are the domain and the range of R, respectively.

  The relative product of binary relations can be generalized to n-ary relations as follows. Let TR be an ordered set (R\[1], ..., R\[n]) of binary relations from X to Y\[i] and S a binary relation from (Y\[1] × ... × Y\[n]) to Z. The *relative product*{: id=tuple_relative_product } of TR and S is the binary relation T from X to Z defined so that x T z if and only if there exists an element y\[i] in Y\[i] for each 1 <= i <= n such that x R\[i] y\[i] and (y\[1], ..., y\[n]) S z. Now let TR be a an ordered set (R\[1], ..., R\[n]) of binary relations from X\[i] to Y\[i] and S a subset of X\[1] × ... × X\[n]. The *multiple relative product*{: id=multiple_relative_product } of TR and S is defined to be the set \{z : z = ((x\[1], ..., x\[n]), (y\[1],...,y\[n])) for some (x\[1], ..., x\[n]) in S and for some (x\[i], y\[i]) in R\[i], 1 <= i <= n\}.

  The *natural join*{: id=natural_join } of an n-ary relation R and an m-ary relation S on coordinate i and j is defined to be the set \{z : z = (x\[1], ..., x\[n],  y\[1], ..., y\[j-1], y\[j+1], ..., y\[m]) for some (x\[1], ..., x\[n]) in R and for some (y\[1], ..., y\[m]) in S such that x\[i] = y\[j]\}.
* [](){: id=sets_definition }
  ...
  * For every term T, if X is the empty list or a non-empty sorted list \[X[1], ..., X\[n]] without duplicates such that (T, X\[i]) belongs to Sets for every 1 <= i <= n, then (\[T], X) belongs to Sets (*typed unordered sets*).
  ...
  An *external set*{: id=external_set } is an element of the range of Sets.

  A *type*{: id=type } is an element of the domain of Sets.

  If S is an element (T, X) of Sets, then T is a *valid type*{: id=valid_type } of X, T is the type of S, and X is the external set of S. `from_term/2` creates a set from a type and an Erlang term turned into an external set.
...
A few functions of this module (`drestriction/3`, `family_projection/2`, `partition/2`, `partition_family/2`, `projection/2`, `restriction/3`, `substitution/2`) accept an Erlang function as a means to modify each element of a given unordered set. [](){: id=set_fun }
Such a function, called SetFun in the following, can be specified as a functional object (fun), a tuple `{external, Fun}`, or an integer:
...
-doc """
[](){: id=from_term }
Creates an element of [Sets](`m:sofs#sets_definition`) by traversing term `Term`, sorting lists, removing duplicates, and deriving or verifying a [valid type](`m:sofs#valid_type`) for the so obtained external set. An explicitly specified [type](`m:sofs#type`) `Type` can be used to limit the depth of the traversal; an atomic type stops the traversal, as shown by the following example where `"foo"` and `{"foo"}` are left unmodified:
...
Other functions that create sets are `from_external/2` and `from_sets/1`. Special cases of `from_term/2` are [`a_function/1,2`](`a_function/1`), `empty_set/0`, [`family/1,2`](`family/1`), [`relation/1,2`](`relation/1`), and [`set/1,2`](`set/1`).
""".
...
-doc "Creates a set from the [external set](`m:sofs#external_set`) `ExternalSet` and the [type](`m:sofs#type`) `Type`. It is assumed that `Type` is a [valid type](`m:sofs#valid_type`) of `ExternalSet`.".
...
-doc "Returns the [untyped empty set](`m:sofs#sets_definition`). `empty_set()` is equivalent to `from_term([], ['_'])`.".
...
-doc "Creates an [unordered set](`m:sofs#sets_definition`). `set(L, T)` is equivalent to `from_term(L, T)`, if the result is an unordered set. If no [type](`m:sofs#type`) is explicitly specified, `[atom]` is used as the set type.".
...
-doc """
Returns the [unordered set](`m:sofs#sets_definition`) containing the sets of list `ListOfSets`.
...
Returns the [ordered set](`m:sofs#sets_definition`) containing the sets of the non-empty tuple `TupleOfSets`.
""".
...
-doc "Creates a [relation](`m:sofs#relation`). `relation(R, T)` is equivalent to `from_term(R, T)`, if T is a [type](`m:sofs#type`) and the result is a relation. If `Type` is an integer N, then `[{atom, ..., atom}])`, where the tuple size is N, is used as type of the relation. If no type is explicitly specified, the size of the first tuple of `Tuples` is used if there is such a tuple. `relation([])` is equivalent to `relation([], 2)`.".
...
-doc "Creates a [function](`m:sofs#function`). `a_function(F, T)` is equivalent to `from_term(F, T)` if the result is a function. If no [type](`m:sofs#type`) is explicitly specified, `[{atom, atom}]` is used as the function type.".
...
-doc "Creates a [family of subsets](`m:sofs#family`). `family(F, T)` is equivalent to `from_term(F, T)` if the result is a family. If no [type](`m:sofs#type`) is explicitly specified, `[{atom, [atom]}]` is used as the family type.".
...
-doc "Returns the [external set](`m:sofs#external_set`) of an atomic, ordered, or unordered set.".
...
-doc """
Returns the set containing every element of `Set1` for which `Fun` returns `true`. If `Fun` is a tuple `{external, Fun2}`, `Fun2` is applied to the [external set](`m:sofs#external_set`) of each element, otherwise `Fun` is applied to each element.
...
-doc """
Returns the [symmetric difference](`m:sofs#symmetric_difference`) (or the Boolean sum) of `Set1` and `Set2`.
...
-doc """
Returns the [Cartesian product](`m:sofs#cartesian_product`) of `Set1` and `Set2`.
...
-doc """
Returns the [Cartesian product](`m:sofs#cartesian_product_tuple`) of the non-empty tuple of sets `TupleOfSets`. If (x\[1], ..., x\[n]) is an element of the n-ary relation `Relation`, then x\[i] is drawn from element i of `TupleOfSets`.
...
-doc """
Creates the [function](`m:sofs#function`) that maps each element of set `Set` onto `AnySet`.
...
-doc """
Returns `true` if `AnySet1` and `AnySet2` are [equal](`m:sofs#equal`), otherwise `false`. The following example shows that `==/2` is used when comparing sets for equality:
...
-doc "Returns `true` if `Set1` is a [subset](`m:sofs#subset`) of `Set2`, otherwise `false`.".
...
-doc "Returns `true` if `Term` appears to be an [unordered set](`m:sofs#sets_definition`), an ordered set, or an atomic set, otherwise `false`. Note that this function will return `true` for any term that coincides with the representation of a `sofs` set. See also note on [data types](`p:system:data_types.md#no_user_types`).".
...
-doc "Returns `true` if `AnySet` appears to be an [unordered set](`m:sofs#sets_definition`), and `false` if `AnySet` is an ordered set or an atomic set or any other term. Note that the test is shallow and this function will return `true` for any term that coincides with the representation of an unordered set. See also note on [data types](`p:system:data_types.md#no_user_types`).".
...
-doc "Returns `true` if `Set1` and `Set2` are [disjoint](`m:sofs#disjoint`), otherwise `false`.".
...
-doc """
Returns the [intersection](`m:sofs#intersection_n`) of the set of sets `SetOfSets`.
...
-doc """
Returns the binary relation containing the elements (E, Set) such that Set belongs to `SetOfSets` and E belongs to Set. If `SetOfSets` is a [partition](`m:sofs#partition`) of a set X and R is the equivalence relation in X induced by `SetOfSets`, then the returned relation is the [canonical map](`m:sofs#canonical_map`) from X onto the equivalence classes with respect to R.
...
-doc """
Returns [family](`m:sofs#family`) `Family` such that the index set is equal to the [domain](`m:sofs#domain`) of the binary relation `BinRel`, and `Family`\[i] is the [image](`m:sofs#image`) of the set of i under `BinRel`.
...
-doc """
If `ListOfBinRels` is a non-empty list \[R[1], ..., R\[n]] of binary relations and `BinRel1` is a binary relation, then `BinRel2` is the [relative product](`m:sofs#tuple_relative_product`) of the ordered set (R\[i], ..., R\[n]) and `BinRel1`.

If `BinRel1` is omitted, the relation of equality between the elements of the [Cartesian product](`m:sofs#cartesian_product_tuple`) of the ranges of R\[i], range R\[1] × ... × range R\[n], is used instead (intuitively, nothing is "lost").
...
Returns the [relative product](`m:sofs#relative_product`) of the binary relations `BinRel1` and `BinRel2`.
""".
...
-doc """
Returns the [relative product](`m:sofs#relative_product`) of the [converse](`m:sofs#converse`) of the binary relation `BinRel1` and the binary relation `BinRel2`.
...
-doc """
Returns the [image](`m:sofs#image`) of set `Set1` under the binary relation `BinRel`.
...
-doc """
Returns the [inverse image](`m:sofs#inverse_image`) of `Set1` under the binary relation `BinRel`.
...
-doc """
Returns the [strict relation](`m:sofs#strict_relation`) corresponding to the binary relation `BinRel1`.
...
-doc """
Returns a subset S of the [weak relation](`m:sofs#weak_relation`) W corresponding to the binary relation `BinRel1`. Let F be the [field](`m:sofs#field`) of `BinRel1`. The subset S is defined so that x S y if x W y for some x in F and for some y in F.
...
-doc """
Returns the [extension](`m:sofs#extension`) of `BinRel1` such that for each element E in `Set` that does not belong to the [domain](`m:sofs#domain`) of `BinRel1`, `BinRel2` contains the pair (E, `AnySet`).
...
-doc "Returns `true` if the binary relation `BinRel` is a [function](`m:sofs#function`) or the untyped empty set, otherwise `false`.".
...
-doc """
Returns the [restriction](`m:sofs#restriction`) of the binary relation `BinRel1` to `Set`.
...
-doc """
Returns the difference between the binary relation `BinRel1` and the [restriction](`m:sofs#restriction`) of `BinRel1` to `Set`.
...
-doc """
Returns the [composite](`m:sofs#composite`) of the functions `Function1` and `Function2`.
...
If `SetFun` is a number i >= 1 and `Set1` is a relation, then the returned set is the [projection](`m:sofs#projection`) of `Set1` onto coordinate i.
...
Let `SetOfSets` be a set of sets and `BinRel` a binary relation. The function that maps each element `Set` of `SetOfSets` onto the [image](`m:sofs#image`) of `Set` under `BinRel` is returned by the following function:
...
-doc """
Returns the [partition](`m:sofs#partition`) of the union of the set of sets `SetOfSets` such that two elements are considered equal if they belong to the same elements of `SetOfSets`.
...
-doc """
Returns the [partition](`m:sofs#partition`) of `Set` such that two elements are considered equal if the results of applying `SetFun` are equal.
...
-doc """
Returns a pair of sets that, regarded as constituting a set, forms a [partition](`m:sofs#partition`) of `Set1`. If the result of applying `SetFun` to an element of `Set1` gives an element in `Set2`, the element belongs to `Set3`, otherwise the element belongs to `Set4`.
...
-doc """
If `TupleOfBinRels` is a non-empty tuple \{R\[1], ..., R\[n]\} of binary relations and `BinRel1` is a binary relation, then `BinRel2` is the [multiple relative product](`m:sofs#multiple_relative_product`) of the ordered set (R\[i], ..., R\[n]) and `BinRel1`.
...
-doc """
Returns the [natural join](`m:sofs#natural_join`) of the relations `Relation1` and `Relation2` on coordinates `I` and `J`.
...
-doc """
If `Family` is a [family](`m:sofs#family`), then `BinRel` is the binary relation containing all pairs (i, x) such that i belongs to the index set of `Family` and x belongs to `Family`\[i].
...
-doc """
If `Family1` is a [family](`m:sofs#family`), then `Family2` is the [restriction](`m:sofs#restriction`) of `Family1` to those elements i of the index set for which `Fun` applied to `Family1`\[i] returns `true`. If `Fun` is a tuple `{external, Fun2}`, then `Fun2` is applied to the [external set](`m:sofs#external_set`) of `Family1`\[i], otherwise `Fun` is applied to `Family1`\[i].
...
-doc """
If `Family1` is a [family](`m:sofs#family`) and `Family1`\[i] is a set of sets for each i in the index set of `Family1`, then `Family2` is the family with the same index set as `Family1` such that `Family2`\[i] is the [union](`m:sofs#union_n`) of `Family1`\[i].
...
-doc """
If `Family1` is a [family](`m:sofs#family`) and `Family1`\[i] is a set of sets for every i in the index set of `Family1`, then `Family2` is the family with the same index set as `Family1` such that `Family2`\[i] is the [intersection](`m:sofs#intersection_n`) of `Family1`\[i].
...
-doc """
If `Family1` is a [family](`m:sofs#family`) and `Family1`\[i] is a binary relation for every i in the index set of `Family1`, then `Family2` is the family with the same index set as `Family1` such that `Family2`\[i] is the [domain](`m:sofs#domain`) of `Family1[i]`.
...
-doc """
If `Family1` is a [family](`m:sofs#family`) and `Family1`\[i] is a binary relation for every i in the index set of `Family1`, then `Family2` is the family with the same index set as `Family1` such that `Family2`\[i] is the [range](`m:sofs#range`) of `Family1`\[i].
...
-doc """
If `Family1` is a [family](`m:sofs#family`) and `Family1`\[i] is a binary relation for every i in the index set of `Family1`, then `Family2` is the family with the same index set as `Family1` such that `Family2`\[i] is the [field](`m:sofs#field`) of `Family1`\[i].
...
-doc """
If `Family1` and `Family2` are [families](`m:sofs#family`), then `Family3` is the family such that the index set is the union of `Family1`:s and `Family2`:s index sets, and `Family3`\[i] is the union of `Family1`\[i] and `Family2`\[i] if both map i, otherwise `Family1`\[i] or `Family2`\[i].
...
-doc """
If `Family1` and `Family2` are [families](`m:sofs#family`), then `Family3` is the family such that the index set is the intersection of `Family1`:s and `Family2`:s index sets, and `Family3`\[i] is the intersection of `Family1`\[i] and `Family2`\[i].
...
-doc """
If `Family1` and `Family2` are [families](`m:sofs#family`), then `Family3` is the family such that the index set is equal to the index set of `Family1`, and `Family3`\[i] is the difference between `Family1`\[i] and `Family2`\[i] if `Family2` maps i, otherwise `Family1[i]`.
...
-doc """
Returns [family](`m:sofs#family`) `Family` where the indexed set is a [partition](`m:sofs#partition`) of `Set` such that two elements are considered equal if the results of applying `SetFun` are the same value i. This i is the index that `Family` maps onto the [equivalence class](`m:sofs#equivalence_class`).
...
-doc """
If `Family1` is a [family](`m:sofs#family`), then `Family2` is the family with the same index set as `Family1` such that `Family2`\[i] is the result of calling `SetFun` with `Family1`\[i] as argument.
...
-doc """
Creates a directed graph from [family](`m:sofs#family`) `Family`. For each pair (a, \{b\[1], ..., b\[n]\}) of `Family`, vertex a and the edges (a, b\[i]) for 1 <= i <= n are added to a newly created directed graph.

...
This module contains functions for parsing and handling URIs ([RFC 3986](https://www.ietf.org/rfc/rfc3986.txt)) and form-urlencoded query strings ([HTML 5.2](https://www.w3.org/TR/html52/)).

Parsing and serializing non-UTF-8 form-urlencoded query strings are also supported ([HTML 5.0](https://www.w3.org/TR/html50/)).
...
Functions with `t:uri_string()` argument accept lists, binaries and mixed lists (lists with binary elements) as input type. All of the functions but `transcode/2` expects input as lists of unicode codepoints, UTF-8 encoded binaries and UTF-8 percent-encoded URI parts ("%C3%B6" corresponds to the unicode character "ö").
...
In case of lists there is only percent-encoding. In binaries, however, both binary encoding and percent-encoding shall be considered. `transcode/2` provides the means to convert between the supported encodings, it takes a `t:uri_string()` and a list of options specifying inbound and outbound encodings.

[RFC 3986](https://www.ietf.org/rfc/rfc3986.txt) does not mandate any specific character encoding and it is usually defined by the protocol or surrounding text. This library takes the same assumption, binary and percent-encoding are handled as one configuration unit, they cannot be set to different values.
...
%%   %x96    `    grave / accent
%%-------------------------------------------------------------------------
-doc "List of unicode codepoints, a UTF-8 encoded binary, or a mix of the two, representing an [RFC 3986](https://www.ietf.org/rfc/rfc3986.txt) compliant URI (*percent-encoded form*). A URI is a sequence of characters from a very limited set: the letters of the basic Latin alphabet, digits, and a few special characters.".
...
%%-------------------------------------------------------------------------
-doc """
Transforms an `URI` into a normalized form using Syntax-Based Normalization as defined by [RFC 3986](https://www.ietf.org/rfc/rfc3986.txt).
...
%%-------------------------------------------------------------------------
-doc """
Parses an [RFC 3986](https://www.ietf.org/rfc/rfc3986.txt) compliant `t:uri_string()` into a `t:uri_map()`, that holds the parsed components of the `URI`. If parsing fails, an error tuple is returned.
...
%%-------------------------------------------------------------------------
-doc """
Creates an [RFC 3986](https://www.ietf.org/rfc/rfc3986.txt) compliant `URIString` (percent-encoded), based on the components of `URIMap`. If the `URIMap` is invalid, an error tuple is returned.
...
%% Misc
%%-------------------------------------------------------------------------
-doc "This is a utility function meant to be used in the shell for printing the allowed characters in each major URI component, and also in the most important characters sets. Please note that this function does not replace the ABNF rules defined by the standards, these character sets are derived directly from those aformentioned rules. For more information see the [Uniform Resource Identifiers](uri_string_usage.md#percent_encoding) chapter in stdlib's Users Guide.".
-doc(#{since => <<"OTP 23.2">>}).
...
-doc """
Decodes all percent-encoded triplets in the input that can be both a `t:uri_string()` and a `t:uri_map()`. Note, that this function performs raw decoding and it shall be used on already parsed URI components. Applying this function directly on a standard URI can effectively change it.
...
Unreserved characters defined in [RFC 3986](https://www.ietf.org/rfc/rfc3986.txt) are not quoted.
...
%%-------------------------------------------------------------------------
-doc """
Composes a form-urlencoded `QueryString` based on a `QueryList`, a list of non-percent-encoded key-value pairs. Form-urlencoding is defined in section 4.10.21.6 of the [HTML 5.2](https://www.w3.org/TR/html52/) specification and in section 4.10.22.6 of the [HTML 5.0](https://www.w3.org/TR/html50/) specification for non-UTF-8 encodings.
...
%%-------------------------------------------------------------------------
-doc """
Dissects an urlencoded `QueryString` and returns a `QueryList`, a list of non-percent-encoded key-value pairs. Form-urlencoding is defined in section 4.10.21.6 of the [HTML 5.2](https://www.w3.org/TR/html52/) specification and in section 4.10.22.6 of the [HTML 5.0](https://www.w3.org/TR/html50/) specification for non-UTF-8 encodings.

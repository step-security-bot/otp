...
This module provides algorithms based on depth-first traversal of directed graphs. For basic functions on directed graphs, see the `m:digraph` module.
...
* Digraphs can be annotated with more information. Such information can be attached to the vertices and to the edges of the digraph. An annotated digraph is called a *labeled digraph*, and the information attached to a vertex or an edge is called a *label*{: id=label }.
...
* A *depth-first traversal*{: id=depth_first_traversal } of a directed digraph can be viewed as a process that visits all vertices of the digraph. Initially, all vertices are marked as unvisited. The traversal starts with an arbitrarily chosen vertex, which is marked as visited, and follows an edge to an unmarked vertex, marking that vertex. The search then proceeds from that vertex in the same fashion, until there is no edge leading to an unvisited vertex. At that point the process backtracks, and the traversal continues as long as there are unexamined edges. If unvisited vertices remain when all edges from the first vertex have been examined, some so far unvisited vertex is chosen, and the process is repeated.
* A *partial ordering*{: id=partial_ordering } of a set S is a transitive, antisymmetric, and reflexive relation between the objects of S.
* The problem of *topological sorting*{: id=topsort } is to find a total ordering of S that is a superset of the partial ordering. A digraph G = (V, E) is equivalent to a relation E on V (we neglect that the version of directed graphs provided by the `digraph` module allows multiple edges between vertices). If the digraph has no cycles of length two or more, the reflexive and transitive closure of E is a partial ordering.
* A *subgraph*{: id=subgraph } G' of G is a digraph whose vertices and edges form subsets of the vertices and edges of G.
* G' is *maximal* with respect to a property P if all other subgraphs that include the vertices of G' do not have property P.
* A *strongly connected component*{: id=strong_components } is a maximal subgraph such that there is a path between each pair of vertices.
* A *connected component*{: id=components } is a maximal subgraph such that there is a path between each pair of vertices, considering all edges undirected.
* An *arborescence*{: id=arborescence } is an acyclic digraph with a vertex V, the *root*{: id=root }, such that there is a unique path from V to every other vertex of G.
* A *tree*{: id=tree } is an acyclic non-empty digraph such that there is a unique path between every pair of vertices, considering all edges undirected.
...
-doc "Returns a list of [connected components](`m:digraph_utils#components`). Each component is represented by its vertices. The order of the vertices and the order of the components are arbitrary. Each vertex of digraph `Digraph` occurs in exactly one component.".
...
-doc "Returns a list of [strongly connected components](`m:digraph_utils#strong_components`). Each strongly component is represented by its vertices. The order of the vertices and the order of the components are arbitrary. Each vertex of digraph `Digraph` occurs in exactly one strong component.".
...
-doc "Returns a list of [strongly connected components](`m:digraph_utils#strong_components`). Each strongly component is represented by its vertices. The order of the vertices and the order of the components are arbitrary. Only vertices that are included in some [cycle](`m:digraph_utils#cycle`) in `Digraph` are returned, otherwise the returned list is equal to that returned by `strong_components/1`.".
...
-doc "Returns an unsorted list of digraph vertices such that for each vertex in the list, there is a [path](`m:digraph_utils#path`) in `Digraph` from some vertex of `Vertices` to the vertex. In particular, as paths can have length zero, the vertices of `Vertices` are included in the returned list.".
...
-doc "Returns an unsorted list of digraph vertices such that for each vertex in the list, there is a [path](`m:digraph_utils#path`) in `Digraph` of length one or more from some vertex of `Vertices` to the vertex. As a consequence, only those vertices of `Vertices` that are included in some [cycle](`m:digraph_utils#cycle`) are returned.".
...
-doc "Returns an unsorted list of digraph vertices such that for each vertex in the list, there is a [path](`m:digraph_utils#path`) from the vertex to some vertex of `Vertices`. In particular, as paths can have length zero, the vertices of `Vertices` are included in the returned list.".
...
-doc "Returns an unsorted list of digraph vertices such that for each vertex in the list, there is a [path](`m:digraph_utils#path`) of length one or more from the vertex to some vertex of `Vertices`. Therefore only those vertices of `Vertices` that are included in some [cycle](`m:digraph_utils#cycle`) are returned.".
...
-doc "Returns a [topological ordering](`m:digraph_utils#topsort`) of the vertices of digraph `Digraph` if such an ordering exists, otherwise `false`. For each vertex in the returned list, no [out-neighbors](`m:digraph_utils#out_neighbour`) occur earlier in the list.".
...
-doc "Returns `true` if and only if digraph `Digraph` is [acyclic](`m:digraph_utils#acyclic_digraph`).".
...
-doc "Returns `{yes, Root}` if `Root` is the [root](`m:digraph_utils#root`) of the arborescence `Digraph`, otherwise `no`.".
...
-doc "Returns `true` if and only if digraph `Digraph` is an [arborescence](`m:digraph_utils#arborescence`).".
...
-doc "Returns `true` if and only if digraph `Digraph` is a [tree](`m:digraph_utils#tree`).".
...
-doc "Returns a list of all vertices of `Digraph` that are included in some [loop](`m:digraph_utils#loop`).".
...
-doc """
Creates a maximal [subgraph](`m:digraph_utils#subgraph`) of `Digraph` having as vertices those vertices of `Digraph` that are mentioned in `Vertices`.
...
If the value of option `keep_labels` is `true`, which is the default, the [labels](`m:digraph_utils#label`) of vertices and edges of `Digraph` are used for the subgraph as well. If the value is `false`, default label `[]` is used for the vertices and edges of the subgroup.
...
-doc """
Creates a digraph where the vertices are the [strongly connected components](`m:digraph_utils#strong_components`) of `Digraph` as returned by `strong_components/1`. If X and Y are two different strongly connected components, and vertices x and y exist in X and Y, respectively, such that there is an edge [emanating](`m:digraph_utils#emanate`) from x and [incident](`m:digraph_utils#incident`) on y, then an edge emanating from X and incident on Y is created.

The created digraph has the same type as `Digraph`. All vertices and edges have the default [label](`m:digraph_utils#label`) `[]`.

Each [cycle](`m:digraph_utils#cycle`) is included in some strongly connected component, which implies that a [topological ordering](`m:digraph_utils#topsort`) of the created digraph always exists.
""".
...
-doc "Returns all vertices of digraph `Digraph`. The order is given by a [depth-first traversal](`m:digraph_utils#depth_first_traversal`) of the digraph, collecting visited vertices in preorder.".
...
-doc "Returns all vertices of digraph `Digraph`. The order is given by a [depth-first traversal](`m:digraph_utils#depth_first_traversal`) of the digraph, collecting visited vertices in postorder. More precisely, the vertices visited while searching from an arbitrarily chosen vertex are collected in postorder, and all those collected vertices are placed before the subsequently visited vertices.".

...
This module provides an expand_fun for the erlang shell [`expand/1,2`](`expand/1`). It is possible to override this expand_fun [`io:setopts/1,2`](`io:setopts/1`).
""".
...
By setting the `{hide, result}` or `{hide, title}` options you may hide suggestions. Sometimes the title isn't useful and just produces text noise, in the example above the `t:any()` result is part of a section with title `Types`. Hiding results is currently not in use, but the idea is that a section can be selected in the expand area and all the other section entries should be collapsed.

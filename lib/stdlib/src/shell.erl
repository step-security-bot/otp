...
The shell is a user interface program for entering expression sequences. The expressions are evaluated and a value is returned. The shell provides an Emacs like set of shortcuts for editing the text of the current line. See [tty - A Command-Line Interface](`p:erts:tty.md`) in the ERTS User's Guide for a list of all available shortcuts. You may also change the shortcuts to suit your preferences more, see [edlin - line editor in the shell](`m:edlin`).

A history mechanism saves previous commands and their values, which can then be incorporated in later commands. How many commands and results to save can be determined by the user, either interactively, by calling `history/1` and `results/1`, or by setting the application configuration parameters [`shell_history_length`](stdlib_app.md#shell_history_length) and [`shell_saved_results`](stdlib_app.md#shell_saved_results) for the STDLIB application. The shell history can be saved to disk by setting the application configuration parameter [`shell_history`](`p:kernel:kernel_app.md#shell_history`) for the Kernel application.
...
The commands below are the built-in shell commands that are always available. In most system the commands listed in the `m:c` module are also available in the shell.

* __`b()`__ - Prints the current variable bindings.

* __`f()`__ - Removes all variable bindings.

* __`f(X)`__ - Removes the binding of variable `X`.
...
* __`h()`__ - Prints the history list.

* __`history(N)`__ - Sets the number of previous commands to keep in the history list to `N`. The previous number is returned. Defaults to 20.

* __`results(N)`__ - Sets the number of results from previous commands to keep in the history list to `N`. The previous number is returned. Defaults to 20.

* __`e(N)`__ - Repeats command `N`, if `N` is positive. If it is negative, the `N`th previous command is repeated (that is, `e(-1)` repeats the previous command).

* __`v(N)`__ - Uses the return value of command `N` in the current command, if `N` is positive. If it is negative, the return value of the `N`th previous command is used (that is, `v(-1)` uses the value of the previous command).

* __`help()`__ - Evaluates `shell_default:help()`.

* __`h(Module, Function)`__ - Print the documentation for `Module:Function` in the shell if available.

* __`ht(Module, Type)`__ - Print the documentation for `Module:Type` in the shell if available.

* __`hcb(Module, Callback)`__ - Print the documentation for `Module:Callback` in the shell if available.

* __`c(Mod)`__ - Evaluates `shell_default:c(Mod)`. This compiles and loads the module `Mod` and purges old versions of the code, if necessary. `Mod` can be either a module name or a a source file path, with or without `.erl` extension.

* __`catch_exception(Bool)`__ - Sets the exception handling of the evaluator process. The previous exception handling is returned. The default (`false`) is to kill the evaluator process when an exception occurs, which causes the shell to create a new evaluator process. When the exception handling is set to `true`, the evaluator process lives on. This means, for example, that ports and ETS tables as well as processes linked to the evaluator process survive the exception.

* __`rd(RecordName, RecordDefinition)`__ - Defines a record in the shell. `RecordName` is an atom and `RecordDefinition` lists the field names and the default values. Usually record definitions are made known to the shell by use of the `rr/1,2,3` commands described below, but sometimes it is handy to define records on the fly.

* __`rf()`__ - Removes all record definitions, then reads record definitions from the modules `shell_default` and `user_default` (if loaded). Returns the names of the records defined.

* __`rf(RecordNames)`__ - Removes selected record definitions. `RecordNames` is a record name or a list of record names. To remove all record definitions, use `'_'`.

* __`rl()`__ - Prints all record definitions.

* __`rl(RecordNames)`__ - Prints selected record definitions. `RecordNames` is a record name or a list of record names.

* __`rp(Term)`__ - Prints a term using the record definitions known to the shell. All of `Term` is printed; the depth is not limited as is the case when a return value is printed.

* __`rr(Module)`__ - Reads record definitions from a module's BEAM file. If there are no record definitions in the BEAM file, the source file is located and read instead. Returns the names of the record definitions read. `Module` is an atom.

* __`rr(Wildcard)`__ - Reads record definitions from files. Existing definitions of any of the record names read are replaced. `Wildcard` is a wildcard string as defined in `m:filelib`, but not an atom.

* __`rr(WildcardOrModule, RecordNames)`__ - Reads record definitions from files but discards record names not mentioned in `RecordNames` (a record name or a list of record names).

* __`rr(WildcardOrModule, RecordNames, Options)`__ - Reads record definitions from files. The compiler options `{i, Dir}`, `{d, Macro}`, and `{d, Macro, Value}` are recognized and used for setting up the include path and macro definitions. To read all record definitions, use `'_'` as value of `RecordNames`.
...
Command 3 builds the tuple `Descriptor`, evaluating the BIF [`list_to_atom/1` ](`erlang:list_to_atom/1`).
...
* __`c [nn]`__ - Connects to job number `<nn>` or the current job. The standard shell is resumed. Operations that use standard I/O by the current job are interleaved with user inputs to the shell.

* __`i [nn]`__ - Stops the current evaluator process for job number `nn` or the current job, but does not kill the shell process. So, any variable bindings and the process dictionary are preserved and the job can be connected again. This command can be used to interrupt an endless loop.

* __`k [nn]`__ - Kills job number `nn` or the current job. All spawned processes in the job are killed, provided they have not evaluated the `group_leader/1` BIF and are located on the local machine. Processes spawned on remote nodes are not killed.

* __`j`__ - Lists all jobs. A list of all known jobs is printed. The current job name is prefixed with '*'.

* __`s`__ - Starts a new job. This is assigned the new index `[nn]`, which can be used in references.

* __`s [shell]`__ - Starts a new job. This is assigned the new index `[nn]`, which can be used in references. If optional argument `shell` is specified, it is assumed to be a module that implements an alternative shell.

* __`r [node]`__ - Starts a remote job on `node`. This is used in distributed Erlang to allow a shell running on one node to control a number of applications running on a network of nodes. If optional argument `shell` is specified, it is assumed to be a module that implements an alternative shell.

* __`q`__ - Quits Erlang. Notice that this option is disabled if Erlang is started with the ignore break, `+Bi`, system flag (which can be useful, for example when running a restricted shell, see the next section).

* __`?`__ - Displays the help message above.
...
If you want an Erlang node to have a remote job active from the start (rather than the default local job), start Erlang with flag [`-remsh`](`p:erts:erl_cmd.md#remsh`), for example, `erl -remsh other_node@other_host`
...
These callback functions are called from local and non-local evaluation function handlers, described in the `m:erl_eval` manual page. (Arguments in `ArgList` are evaluated before the callback functions are called.)
...
-doc "Starts the interactive shell if it has not already been started. It can be used to programatically start the shell from an escript or when erl is started with the -noinput or -noshell flags.".
-doc(#{since => <<"OTP 26.0">>}).
-spec start_interactive() -> ok | {error, already_started}.
start_interactive() ->
    user_drv:start_shell().
-doc """
Starts the interactive shell if it has not already been started. It can be used to programatically start the shell from an [`escript`](`p:erts:escript_cmd.md`) or when [`erl`](`p:erts:erl_cmd.md`) is started with the [`-noinput`](`p:erts:erl_cmd.md#noinput`) or [`-noshell`](`p:erts:erl_cmd.md#noshell`) flags. The following options are allowed:

* __noshell__ - Starts the interactive shell as if [`-noshell`](`p:erts:erl_cmd.md#noshell`) was given to [`erl`](`p:erts:erl_cmd.md`). This is only useful when erl is started with [`-noinput`](`p:erts:erl_cmd.md#noinput`) and the system want to read input data.

* __[mfa()](`t:erlang:mfa/0`)__ - Starts the interactive shell using [`mfa()`](`t:erlang:mfa/0`) as the default shell.

* __\{[node()](`t:erlang:node/0`), [mfa()](`t:erlang:mfa/0`)\}__ - Starts the interactive shell using [`mfa()`](`t:erlang:mfa/0`) on [`node()`](`t:erlang:node/0`) as the default shell.

* __\{remote, [`string()`](`t:erlang:string/0`)\}__ - Starts the interactive shell using as if [`-remsh`](`p:erts:erl_cmd.md#remsh`) was given to [`erl`](`p:erts:erl_cmd.md`).

* __\{remote, [`string()`](`t:erlang:string/0`), [`mfa()`](`t:erlang:mfa/0`)\}__ - Starts the interactive shell using as if [`-remsh`](`p:erts:erl_cmd.md#remsh`) was given to [`erl`](`p:erts:erl_cmd.md`) but with an alternative shell implementation.

On error this function will return:

* __already_started__ - if an interactive shell is already started.

* __noconnection__ - if a remote shell was requested but it could not be connected to.

* __badfile | nofile | on_load_failure__ - if a remote shell was requested with a custom [mfa()](`t:erlang:mfa/0`), but the module could not be loaded. See [Error Reasons for Code-Loading Functions](`m:code#error_reasons`) for a description of the error reasons.
""".
-doc(#{since => <<"OTP 26.0">>}).
...
-doc "Returns the current shell process on the node where the calling process' group_leader is located. If that node has no shell this function will return undefined.".
-doc(#{since => <<"OTP 26.0">>}).
...
-doc "Exits a restricted shell and starts a normal shell. The function is meant to be called from the shell.".
...
-doc "Sets the number of previous commands to keep in the history list to `N`. The previous number is returned. Defaults to 20.".
...
-doc "Sets the number of results from previous commands to keep in the history list to `N`. The previous number is returned. Defaults to 20.".
...
-doc "Sets the shell prompt function to `PromptFunc`. The previous prompt function is returned.".

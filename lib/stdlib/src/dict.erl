...
This module provides the same interface as the `m:orddict` module. One difference is that while this module considers two keys as different if they do not match (`=:=`), `orddict` considers two keys as different if and only if they do not compare equal (`==`).
...
-doc "This function returns value from dictionary and a new dictionary without this value. Returns `error` if the key is not present in the dictionary.".
-doc(#{since => <<"OTP 20.0">>}).
...
-doc "Stores a `Key`-`Value` pair in dictionary `Dict2`. If `Key` already exists in `Dict1`, the associated value is replaced by `Value`.".
...
-doc "Updates a value in a dictionary by calling `Fun` on the value to get a new value. An exception is generated if `Key` is not present in the dictionary.".
...
-doc "Calls `Fun` on successive keys and values of dictionary `Dict` together with an extra argument `Acc` (short for accumulator). `Fun` must return a new accumulator that is passed to the next call. `Acc0` is returned if the dictionary is empty. The evaluation order is undefined.".
...
-doc "Calls `Fun` on successive keys and values of dictionary `Dict1` to return a new value for each key. The evaluation order is undefined.".
...
-doc "`Dict2` is a dictionary of all keys and values in `Dict1` for which `Pred(Key, Value)` is `true`.".

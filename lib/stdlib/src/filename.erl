...
-doc "Same as `absname/1`, except that the directory to which the filename is to be made relative is specified in argument `Dir`.".
...
-doc "Joins two filename components with directory separators. Equivalent to `join([Name1, Name2])`.".
...
* __`absolute`__ - The path name refers to a specific file on a specific volume.

  Unix example: `/usr/local/bin`

  Windows example: `D:/usr/local/bin`

* __`relative`__ - The path name is relative to the current working directory on the current volume.

  Example: `foo/bar, ../src`

* __`volumerelative`__ - The path name is relative to the current working directory on a specified volume, or it is a specific file on the current working volume.
...
-doc "Converts a possibly deep list filename consisting of characters and atoms into the corresponding flat string filename.".
...
Equivalent to [basedir(PathType, Application, #\{\})](`m:filename#basedir_3_1`) or [basedir(PathsType, Application, #\{\})](`m:filename#basedir_3_2`).
""".
-doc(#{since => <<"OTP 19.0">>}).

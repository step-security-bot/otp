...
As all operations performed by Dets are disk operations, it is important to realize that a single look-up operation involves a series of disk seek and read operations. The Dets functions are therefore much slower than the corresponding `m:ets` functions, although Dets exports a similar interface.
...
-doc "Match specifications, see section [Match Specification in Erlang](`p:erts:match_spec.md`) in ERTS User's Guide and the `m:ms_transform` module.".
...
-doc "Deletes all objects from a table in almost constant time. However, if the table if fixed, `delete_all_objects(T)` is equivalent to `match_delete(T, '_')`.".
...
-doc "Deletes all instances of a specified object from a table. If a table is of type `bag` or `duplicate_bag`, this function can be used to delete only some of the objects with a specified key.".
...
There are two reasons why `first/1` and `next/2` are not to be used: they are not efficient, and they prevent the use of key `'$end_of_table'`, as this atom is used to indicate the end of the table. If possible, use functions [`match`](`match/1`), [`match_object`](`match_object/1`), and [`select`](`select/1`) for traversing tables.
""".
...
-doc "Calls `Function` on successive elements of table `Name` together with an extra argument `AccIn`. The table elements are traversed in unspecified order. `Function` must return a new accumulator that is passed to the next call. `Acc0` is returned if the table is empty.".
...
-doc "Deletes all objects of table `Name` and then inserts all the objects of the ETS table `EtsTab`. The objects are inserted in unspecified order. As `ets:safe_fixtable/2` is called, the ETS table must be public or owned by the calling process.".
...
* `{filename, `[`file:name()`](`t:file:name/0`)`}` \- The name of the file where objects are stored.
* `{keypos, `[`keypos()`](`t:keypos/0`)`}` \- The key position.
* `{size, integer() >= 0}` \- The number of objects stored in the table.
* `{type, `[`type()`](`t:type/0`)`}` \- The table type.
""".
...
* `{access, `[`access()`](`t:access/0`)`}` \- The access mode.
* `{auto_save, `[`auto_save()`](`t:auto_save/0`)`}` \- The autosave interval.
* `{bchunk_format, binary()}` \- An opaque binary describing the format of the objects returned by `bchunk/2`. The binary can be used as argument to `is_compatible_bchunk_format/2`.
...
* `{no_slots, {Min, Used, Max}}` \- The number of slots of the table. `Min` is the minimum number of slots, `Used` is the number of currently used slots, and `Max` is the maximum number of slots.
...
* `{safe_fixed_monotonic_time, SafeFixed}` \- If the table is fixed, `SafeFixed` is a tuple `{FixedAtTime, [{Pid,RefCount}]}`. `FixedAtTime` is the time when the table was first fixed, and `Pid` is the pid of the process that fixes the table `RefCount` times. There can be any number of processes in the list. If the table is not fixed, `SafeFixed` is the atom `false`.

  `FixedAtTime` corresponds to the result returned by `erlang:monotonic_time/0` at the time of fixation. The use of `safe_fixed_monotonic_time` is [time warp safe](`p:erts:time_correction.md#time_warp_safe_code`).
* `{safe_fixed, SafeFixed}` \- The same as `{safe_fixed_monotonic_time, SafeFixed}` except the format and value of `FixedAtTime`.

  `FixedAtTime` corresponds to the result returned by `erlang:timestamp/0` at the time of fixation. Notice that when the system uses single or multi [time warp modes](`p:erts:time_correction.md#time_warp_modes`), this can produce strange results. This is because the use of `safe_fixed` is not [time warp safe](`p:erts:time_correction.md#time_warp_safe_code`). Time warp safe code must use `safe_fixed_monotonic_time` instead.
""".
...
-doc "Inserts one or more objects into the table `Name`. If there already exists an object with a key matching the key of some of the given objects and the table type is `set`, the old object will be replaced.".
...
-doc "Inserts one or more objects into table `Name`. If there already exists some object with a key matching the key of any of the specified objects, the table is not updated and `false` is returned. Otherwise the objects are inserted and `true` returned.".
...
-doc "Returns `true` if it would be possible to initialize table `Name`, using `init_table/3` with option `{format, bchunk}`, with objects read with `bchunk/2` from some table `T`, such that calling `info(T, bchunk_format)` returns `BchunkFormat`.".
...
-doc "Works like `lookup/2`, but does not return the objects. Returns `true` if one or more table elements has key `Key`, otherwise `false`.".
...
-doc "Opens an existing table. If the table is not properly closed, it is repaired. The returned reference is to be used as the table name. This function is most useful for debugging purposes.".
...
* `{access, `[`access()`](`t:access/0`)`}` \- Existing tables can be opened in read-only mode. A table that is opened in read-only mode is not subjected to the automatic file reparation algorithm if it is later opened after a crash. Defaults to `read_write`.
* `{auto_save, `[`auto_save()`](`t:auto_save/0`)`}` \- The autosave interval. If the interval is an integer `Time`, the table is flushed to disk whenever it is not accessed for `Time` milliseconds. A table that has been flushed requires no reparation when reopened after an uncontrolled emulator halt. If the interval is the atom `infinity`, autosave is disabled. Defaults to 180000 (3 minutes).
* `{estimated_no_objects, `[`no_slots()`](`t:no_slots/0`)`}` \- Equivalent to option `min_no_slots`.
* `{file, `[`file:name()`](`t:file:name/0`)`}` \- The name of the file to be opened. Defaults to the table name.
* `{max_no_slots, `[`no_slots()`](`t:no_slots/0`)`}` \- The maximum number of slots to be used. Defaults to 32 M, which is the maximal value. Notice that a higher value can increase the table fragmentation, and a smaller value can decrease the fragmentation, at the expense of execution time.
* `{min_no_slots, `[`no_slots()`](`t:no_slots/0`)`}` \- Application performance can be enhanced with this flag by specifying, when the table is created, the estimated number of different keys to be stored in the table. Defaults to 256, which is the minimum value.
* `{keypos, `[`keypos()`](`t:keypos/0`)`}` \- The position of the element of each object to be used as key. Defaults to 1. The ability to explicitly state the key position is most convenient when we want to store Erlang records in which the first position of the record is the name of the record type.
...
It is not guaranteed that calls to `first/1`, `next/2`, or select and match functions work as expected even if the table is fixed; the limited support for concurrency provided by the `m:ets` module is not yet provided by Dets. Fixing a table currently only disables resizing of the hash list of the table.
...
-doc """
Returns the results of applying match specification `MatchSpec` to all or some objects stored in table `Name`. The order of the objects is not specified. For a description of match specifications, see the [ERTS User's Guide](`p:erts:match_spec.md`).
...
-doc """
Returns the results of applying match specification `MatchSpec` to some or all objects stored in table `Name`. The order of the objects is not specified. For a description of match specifications, see the [ERTS User's Guide](`p:erts:match_spec.md`).
...
-doc """
Deletes each object from table `Name` such that applying match specification `MatchSpec` to the object returns value `true`. For a description of match specifications, see the [ERTS User's Guide](`p:erts:match_spec.md`). Returns the number of deleted objects.
...
-doc "The objects of a table are distributed among slots, starting with slot `0` and ending with slot `n`. Returns the list of objects associated with slot `I`. If `I` > `n`, `'$end_of_table'` is returned.".
...
-doc """
Returns a Query List Comprehension (QLC) query handle. The `m:qlc` module provides a query language aimed mainly for Mnesia, but ETS tables, Dets tables, and lists are also recognized by `qlc` as sources of data. Calling [`dets:table/1,2`](`table/1`) is the means to make Dets table `Name` usable to `qlc`.

When there are only simple restrictions on the key position, `qlc` uses [`dets:lookup/2`](`lookup/2`) to look up the keys. When that is not possible, the whole table is traversed. Option `traverse` determines how this is done:

* `first_next` \- The table is traversed one key at a time by calling `dets:first/1` and `dets:next/2`.
* `select` \- The table is traversed by calling [`dets:select/3`](`select/3`) and [`dets:select/1`](`select/1`). Option `n_objects` determines the number of objects returned (the third argument of `select/3`). The match specification (the second argument of `select/3`) is assembled by `qlc`:
...
* `{select, `[match_spec()](`t:match_spec/0`)`}` \- As for `select`, the table is traversed by calling `dets:select/3` and `dets:select/1`. The difference is that the match specification is specified explicitly. This is how to state match specifications that cannot easily be expressed within the syntax provided by `qlc`.
...
-doc "Inserts the objects of the Dets table `Name` into the ETS table `EtsTab`. The order in which the objects are inserted is not specified. The existing objects of the ETS table are kept unless overwritten.".
...
* __`continue`__ - Continue to perform the traversal. For example, the following function can be used to print the contents of a table:

  ```text
  fun(X) -> io:format("~p~n", [X]), continue end.
  ```

* __`{continue, Val}`__ - Continue the traversal and accumulate `Val`. The following function is supplied to collect all objects of a table in a list:

  ```text
  fun(X) -> {continue, X} end.
  ```

* __`{done, Value}`__ - Terminate the traversal and return `[Value | Acc]`.

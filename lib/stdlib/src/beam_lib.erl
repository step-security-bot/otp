...
Option `debug_info` can be specified to the Compiler (see [`compile(3)`](`m:compile#debug_info`)) to have debug information, such as [Erlang Abstract Format](`p:erts:absform.md`), stored in the `debug_info` chunk. Tools such as Debugger and Xref require the debug information to be included.
...
1. Use Compiler option `{debug_info_key,Key}`, see [`compile(3)`](`m:compile#debug_info_key`) and function `crypto_key_fun/1` to register a fun that returns the key whenever `beam_lib` must decrypt the debug information.
...
1. Store the key in a text file named `.erlang.crypt`.
  ...
  In this case, Compiler option `encrypt_debug_info` can be used, see [`compile(3)`](`m:compile#encrypt_debug_info`).

## .erlang.crypt

`beam_lib` searches for `.erlang.crypt` in the current directory, then the [user's home directory](`m:init#home`) and then [`filename:basedir(user_config, "erlang")`](`m:filename#user_config`). If the file is found and contains a key, `beam_lib` implicitly creates a crypto key fun and registers it.
...
-doc "Each of the functions described below accept either the filename (as a string) or a binary containing the BEAM module.".
...
-doc "The list of attributes is sorted on `Attribute` (in `t:attrib_entry()`) and each attribute name occurs once in the list. The attribute values occur in the same order as in the file. The lists of functions are also sorted.".
...
* __`{file, Filename} | {binary, Binary}`__ - The name (string) of the BEAM file, or the binary from which the information was extracted.

* __`{module, Module}`__ - The name (atom) of the module.

* __`{chunks, [{ChunkId, Pos, Size}]}`__ - For each chunk, the identifier (string) and the position and size of the chunk data, in bytes.
""".
...
-doc "Reads chunk data for selected chunks references. The order of the returned list of chunk data is determined by the order of the list of chunks references.".
...
-doc "Compares the BEAM files in two directories as `cmp_dirs/2`, but the names of files that exist in only one directory or are different are presented on standard output.".
...
-doc "Removes all chunks from a BEAM file except those used by the loader. In particular, the debug information (chunk `debug_info` and `abstract_code`) is removed.".
-spec strip(Beam1) ->
...
-doc "Removes all chunks from a BEAM file except those used by the loader or mentioned in `AdditionalChunks`. In particular, the debug information (chunk `debug_info` and `abstract_code`) is removed.".
-doc(#{since => <<"OTP 22.0">>}).
...
-doc "Removes all chunks except those used by the loader from BEAM files. In particular, the debug information (chunk `debug_info` and `abstract_code`) is removed. The returned list contains one element for each specified filename, in the same order as in `Files`.".
...
-doc "Removes all chunks except those used by the loader or mentioned in `AdditionalChunks`. In particular, the debug information (chunk `debug_info` and `abstract_code`) is removed. The returned list contains one element for each specified filename, in the same order as in `Files`.".
-doc(#{since => <<"OTP 22.0">>}).
...
-doc "Removes all chunks except those used by the loader from the BEAM files of a release. `Dir` is to be the installation root directory. For example, the current OTP release can be stripped with the call `beam_lib:strip_release(code:root_dir())`.".
...
-doc "Removes all chunks except those used by the loader or mentioned in `AdditionalChunks`. `Dir` is to be the installation root directory. For example, the current OTP release can be stripped with the call `beam_lib:strip_release(code:root_dir())`.".
-doc(#{since => <<"OTP 22.0">>}).
...
-doc "Calculates an MD5 redundancy check for the code of the module (compilation date and other attributes are not included).".
-spec md5(Beam) ->
...
-doc "For a specified error returned by any function in this module, this function returns a descriptive string of the error in English. For file errors, function [`file:format_error(Posix)`](`file:format_error/1`) is to be called.".

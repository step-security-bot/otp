...
This module provides the same interface as the `m:sets` module but with a defined representation. One difference is that while `sets` considers two elements as different if they do not match (`=:=`), this module considers two elements as different if and only if they do not compare equal (`==`).

See the [Compatibility Section in the `sets(3)` module](`m:sets#compatibility`) for more information about the compatibility of the different implementations of sets in the Standard Library.
...
-doc "Returns `true` if `Ordset` is an ordered set of elements, otherwise `false`. This function will return `true` for any ordered list, even when not constructed by the functions in this module.".
...
%%  otherwise 'false'.
-doc "Returns `true` if `Ordset1` and `Ordset2` are equal, that is when every element of one set is also a member of the respective other set, otherwise `false`.".
...
-doc "Returns `true` if `Ordset1` and `Ordset2` are disjoint (have no elements in common), otherwise `false`.".
...
-doc "Returns `true` when every element of `Ordset1` is also a member of `Ordset2`, otherwise `false`.".
...
-doc "Folds `Function` over every element in `Ordset` and returns the final value of the accumulator.".

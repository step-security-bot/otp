...
The data representing a queue as used by this module is to be regarded as opaque by other modules. In abstract terms, the representation is a composite type of existing Erlang terms. See note on [data types](`p:system:data_types.md#no_user_types`). Any code assuming knowledge of the format is running on thin ice.
...
%% O(1)
-doc "Tests if `Term` is a queue and returns `true` if so, otherwise `false`. Note that the test will return `true` for a term coinciding with the representation of a queue, even when not constructed by thus module. See also note on [data types](`p:system:data_types.md#no_user_types`).".
-doc(#{title => <<"Original API">>}).
...
%%
%% O(length(L))
-doc "Returns a queue containing the items in `L` in the same order; the head item of the list becomes the front item of the queue.".
-doc(#{title => <<"Original API">>}).

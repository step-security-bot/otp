...
This module can be used to render function and type documentation to be printed in a shell. This is the module that is used to render the docs accessed through the shell through [`c:h/1,2,3`](`c:h/1`). Example:
...
This module formats and renders EEP-48 documentation of the format `application/erlang+html`. For more information about this format see [Documentation Storage](`p:erl_docgen:doc_storage.md`) in Erl_Docgen's User's Guide. It can also render any other format of "text" type, although those will be rendered as is.
""".
...
-doc "The record holding EEP-48 documentation for a module. You can use `code:get_doc/1` to fetch this information from a module.".
-type docs_v1() :: #docs_v1{}.
-doc """
The configuration of how the documentation should be rendered.

* __encoding__ - Configure the encoding that should be used by the renderer for graphical details such as bullet-points. By default `shell_docs` uses the value returned by [`io:getopts()`](`io:getopts/0`).

* __ansi__ - Configure whether [ansi escape codes](https://en.wikipedia.org/wiki/ANSI_escape_code) should be used to render graphical details such as bold and underscore. By default `shell_docs` will try to determine if the receiving shell supports ansi escape codes. It is possible to override the automated check by setting the kernel configuration parameter `shell_docs_ansi` to a `t:boolean()` value.

* __columns__ - Configure how wide the target documentation should be rendered. By default `shell_docs` used the value returned by [`io:columns()`](`io:columns/0`).
""".
...
-doc "This function can be used to find out which tags are supported by `application/erlang+html` documentation.".
-doc(#{since => <<"OTP 24.0">>}).
-spec supported_tags() -> [chunk_element_type()].
supported_tags() ->
    ?ALL_ELEMENTS.

-doc "This function can be used to do a basic validation of the doc content of `application/erlang+html` format.".
-doc(#{since => <<"OTP 23.0">>}).
...
-doc "This function can be used to do whitespace normalization of `application/erlang+html` documentation.".
-doc(#{since => <<"OTP 23.0">>}).

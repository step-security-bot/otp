...
-export_type([docs_v1/0, chunk_elements/0, chunk_element/0, chunk_element_attrs/0,
              chunk_element_attr/0, chunk_element_type/0]).
...
render_docs(DocContents, #config{ type = man } = Config) ->
    shell_docs_man:render_docs(DocContents, Config);
render_docs(DocContents, #config{ type = markdown } = Config) ->
    shell_docs_markdown:render_docs(DocContents, Config);
render_docs(DocContents, Config) when is_map(Config) ->
    render_docs(DocContents, init_config(DocContents, Config)).
render_docs(DocContents, DocsV1, Config) when is_map(Config) ->
    render_docs(DocContents, init_config(DocsV1, Config)).
...
init_config(D, Config) ->
    ...
    #config{ docs = D,
             type = maps:get(type, Config, #config{}#config.type),

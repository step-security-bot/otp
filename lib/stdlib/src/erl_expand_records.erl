...
Section [The Abstract Format](`p:erts:absform.md`) in ERTS User's Guide.
""".
...
-doc "Expands all records in a module to use explicit tuple operations and adds explicit module names to calls to BIFs and imported functions. The returned module has no references to records, attributes, or code.".

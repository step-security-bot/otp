...
Every time a random number is requested, a state is used to calculate it, and a new state is produced. The state can either be implicit (kept in the process dictionary) or be an explicit argument and return value. In this implementation, the state (the type `t:ran()`) consists of a tuple of three integers.

> #### Note {: class=info }
> This random number generator is not cryptographically strong. If a strong cryptographic random number generator is needed, use one of functions in the `m:crypto` module, for example, [`crypto:strong_rand_bytes/1`](`m:crypto`).
...
-doc(#{deprecated =>
           <<"random:seed0/0 is deprecated; use the 'rand' module instead">>}).
-spec seed0() -> ran().
...
-doc "Seeds random number generation with default (fixed) values in the process dictionary and returns the old state.".
-doc(#{deprecated =>
           <<"random:seed/0 is deprecated; use the 'rand' module instead">>}).
-spec seed() -> ran().
...
-doc "`seed({A1, A2, A3})` is equivalent to `seed(A1, A2, A3)`.".
-doc(#{deprecated =>
           <<"random:seed/1 is deprecated; use the 'rand' module instead">>}).
...
-doc "Returns a random float uniformly distributed between `0.0` and `1.0`, updating the state in the process dictionary.".
-doc(#{deprecated =>
...
-doc "Returns, for a specified integer `N >= 1`, a random integer uniformly distributed between `1` and `N`, updating the state in the process dictionary.".
-doc(#{deprecated =>
...
-doc "Returns, for a specified state, a random float uniformly distributed between `0.0` and `1.0`, and a new state.".
-doc(#{deprecated =>
           <<"random:uniform_s/1 is deprecated; use the 'rand' module instead">>}).
...
-doc "Returns, for a specified integer `N >= 1` and a state, a random integer uniformly distributed between `1` and `N`, and a new state.".
-doc(#{deprecated =>
           <<"random:uniform_s/2 is deprecated; use the 'rand' module instead">>}).

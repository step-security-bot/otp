...
`argparse` is designed to work with [`escript`](`p:erts:escript_cmd.md`). The example below is a fully functioning Erlang program accepting two command line arguments and printing their product.
...
-doc "User-defined help template to print in the command usage. First element of a tuple must be a string. It is printed as a part of the usage header. Second element of the tuple can be either a string printed as-is, a list containing strings, `type` and `default` atoms, or a user-defined function that must return a string.".
-type argument_help() :: {
...
-doc """
Argument specification. Defines a single named argument that is returned in the [`argument map`](`t:arg_map/0`). The only required field is `name`, all other fields have defaults.
...
* __`name`__ - Sets the argument name in the parsed argument map. If `help` is not defined, name is also used to generate the default usage message.

* __`short`__ - Defines a short (single character) form of an optional argument.
...
* __`long`__ - Defines a long form of an optional argument.
...
* __`required`__ - Forces the parser to expect the argument to be present in the command line. By default, all positional argument are required, and all options are not.

* __`default`__ - Specifies the default value to put in the parsed argument map if the value is not supplied in the command line.
...
* __`type`__ - Defines type conversion and validation routine. The default is `string`, assuming no conversion.

* __`nargs`__ - Defines the number of following arguments to consume from the command line. By default, the parser consumes the next argument and converts it into an Erlang term according to the specified type.

  * __`t:pos_integer()`__ - Consume exactly this number of positional arguments, fail if there is not enough. Value in the argument map contains a list of exactly this length. Example, defining a positional argument expecting 3 integer values:
  ...
  * __`list`__ - Consume all following arguments until hitting the next option (starting with an option prefix). May result in an empty list added to the arguments map.
  ...
  * __`nonempty_list`__ - Same as `list`, but expects at least one argument. Returns an error if the following command line argument is an option switch (starting with the prefix).

  * __`'maybe'`__ - Consumes the next argument from the command line, if it does not start with an option prefix. Otherwise, adds a default value to the arguments map.
  ...
  * __`{'maybe', term()}`__ - Consumes the next argument from the command line, if it does not start with an option prefix. Otherwise, adds a specified Erlang term to the arguments map.

  * __`all`__ - Fold all remaining command line arguments into a list, ignoring any option prefixes or switches. Useful for proxying arguments into another command line utility.
...
* __`action`__ - Defines an action to take when the argument is found in the command line. The default action is `store`.

  * __`store`__ - Store the value in the arguments map. Overwrites the value previously written.
  ...
  * __`{store, term()}`__ - Stores the specified term instead of reading the value from the command line.
  ...
  * __`append`__ - Appends the repeating occurrences of the argument instead of overwriting.
  ...
  * __`{append, term()}`__ - Same as `append`, but instead of consuming the argument from the command line, appends a provided `t:term()`.

  * __`count`__ - Puts a counter as a value in the arguments map. Useful for implementing verbosity option:
  ...
  * __`extend`__ - Works as `append`, but flattens the resulting list. Valid only for `nargs` set to `list`, `nonempty_list`, `all` or `t:pos_integer()`.
...
* __`help`__ - Specifies help/usage text for the argument. `argparse` provides automatic generation based on the argument name, type and default value, but for better usability it is recommended to have a proper description. Setting this field to `hidden` suppresses usage output for this argument.
""".
-type argument() :: #{
...
-doc "Arguments map is the map of argument names to the values extracted from the command line. It is passed to the matching command handler. If an argument is omitted, but has the default value is specified, it is added to the map. When no default value specified, and argument is not present in the command line, corresponding key is not present in the resulting map.".
-type arg_map() :: #{argument_name() => term()}.
%% Arguments map: argument name to a term, produced by parser. Supplied to the command handler

-doc """
Command handler specification. Called by [`run/3` ](`run/3`)upon successful parser return.

* __`fun((arg_map()) -> term())`__ - Function accepting [`argument map`](`t:arg_map/0`). See the basic example in the [Quick Start](`m:argparse#quick-start`) section.

* __`{Module :: module(), Function :: atom()}`__ - Function named `Function`, exported from `Module`, accepting [`argument map`](`t:arg_map/0`).

* __`{fun(() -> term()), Default :: term()}`__ - Function accepting as many arguments as there are in the `arguments` list for this command. Arguments missing from the parsed map are replaced with the `Default`. Convenient way to expose existing functions.
...
* __`{Module :: module(), Function :: atom(), Default :: term()}`__ - Function named `Function`, exported from `Module`, accepting as many arguments as defined for this command. Arguments missing from the parsed map are replaced with the `Default`. Effectively, just a different syntax to the same functionality as demonstrated in the code above.
""".
-type handler() ::
...
* __usage__ - Formatted command line usage text, e.g. `rm [-rf] <directory>`.

* __commands__ - Expanded list of sub-commands.

* __arguments__ - Detailed description of positional arguments.

* __options__ - Detailed description of optional arguments.
""".
...
* __`commands`__ - Maps of nested commands. Keys must be strings, matching command line input. Basic utilities do not need to specify any nested commands.

* __`arguments`__ - List of arguments accepted by this command, and all nested commands in the hierarchy.

* __`help`__ - Specifies help/usage text for this command. Pass `hidden` to remove this command from the usage output.

* __`handler`__ - Specifies a callback function to call by `run/3` when the parser is successful.
""".
-type command() :: #{
...
-doc "Path to the nested command. First element is always the `progname`, subsequent elements are nested command names.".
...
-doc """
Returned from [`parse/2,3`](`parse/3`) when the user input cannot be parsed according to the command specification.
...
* __`prefixes`__ - Changes the option prefix (the default is `-`).

* __`default`__ - Specifies the default value for all optional arguments. When this field is set, resulting argument map will contain all argument names. Useful for easy pattern matching on the argument map in the handler function.

* __`progname`__ - Specifies the program (root command) name. Returned as the first element of the command path, and printed in help/usage text. It is recommended to have this value set, otherwise the default one is determined with `init:get_argument(progname)` and is often set to `erl` instead of the actual script name.

* __`command`__ - Specifies the path to the nested command for `help/2`. Useful to limit output for complex utilities with multiple commands, and used by the default error handling logic.

* __`columns`__ - Specifies the help/usage text width (characters) for `help/2`. Default value is 80.
""".
-type parser_options() :: #{
...
-doc "Returned from [`parse/2,3`](`parse/3`). Contains arguments extracted from the command line, path to the nested command (if any), and a (potentially nested) command specification that was considered when the parser finished successfully. It is expected that the command contains a handler definition, that will be called passing the argument map.".
-type parse_result() ::
...
%%  definition.
-doc """
Parses command line arguments according to the command specification. Raises an exception if the command specification is not valid. Use [`erl_error:format_exception/3,4` ](`erl_error:format_exception/3`)to see a friendlier message. Invalid command line input does not raise an exception, but makes `parse/2,3` to return a tuple [`{error, parser_error()}`](`t:parser_error/0`).
...
-doc "Generates help/usage information text for the command supplied, or any nested command when `command` option is specified. Does not provide localisaton. Expects `progname` to be set, otherwise defaults to return value of `init:get_argument(progname)`.".
-doc(#{since => <<"OTP 26.0">>}).
...
> This function is designed to work as an entry point to a standalone [`escript`](`p:erts:escript_cmd.md`). Therefore, it halts the emulator for any error detected. Do not use this function through remote procedure call, or it may result in an unexpected shutdown of a remote node.
""".
-doc(#{since => <<"OTP 26.0">>}).
...
-doc "Generates human-readable text for [`parser error`](`t:parser_error/0`). Does not include help/usage information, and does not provide localisation.".
-doc(#{since => <<"OTP 26.0">>}).

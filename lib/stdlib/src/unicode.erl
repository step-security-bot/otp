...
-doc "A `t:binary()` with characters encoded in the UTF-8 coding standard.".
...
-doc "A `t:binary()` with characters coded in a user-specified Unicode encoding other than UTF-8 (that is, UTF-16 or UTF-32).".
...
-doc "A `t:binary()` with characters coded in ISO Latin-1.".
-type latin1_binary() :: binary().
-doc "An `t:integer()` representing a valid ISO Latin-1 character (0-255).".
-type latin1_char() :: byte().
-doc "Same as `t:iodata()`.".
-type latin1_chardata() :: latin1_charlist() | latin1_binary().
-doc "Same as `t:iolist()`.".
...
-doc "Same as `characters_to_binary(Data, InEncoding, unicode)`.".
...
If `InEncoding` is `latin1`, parameter `Data` corresponds to the `t:iodata()` type, but for `unicode`, parameter `Data` can contain integers > 255 (Unicode characters beyond the ISO Latin-1 range), which makes it invalid as `t:iodata()`.
...
-doc "Same as `characters_to_list(Data, unicode)`.".
...
-doc "Same as `characters_to_binary(Data, unicode, unicode)`.".
...
* __`unicode`__ - An alias for `utf8`, as this is the preferred encoding for Unicode characters in binaries.

* __`utf16`__ - An alias for `{utf16,big}`.

* __`utf32`__ - An alias for `{utf32,big}`.

The atoms `big` and `little` denote big- or little-endian encoding.

Errors and exceptions occur as in `characters_to_list/2`, but the second element in tuple `error` or `incomplete` is a `t:binary()` and not a `t:list()`.
""".

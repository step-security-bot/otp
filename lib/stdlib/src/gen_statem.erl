...
`gen_statem` provides a generic state machine behaviour that for new code replaces its predecessor `m:gen_fsm` since Erlang/OTP 20.0. The `gen_fsm` behaviour remains in OTP "as is".

> #### Note {: class=info }
> If you are new to `gen_statem` and want an overview of concepts and operation the section [`gen_statem` Behaviour ](`p:system:statem.md`)located in the User's Guide [OTP Design Principles ](`p:system:index.html`)is recommended to read before this reference manual, possibly after the Description section you are reading here.

This reference manual contains type descriptions generated from types in the `gen_statem` source code, so they are correct. However, the generated descriptions also reflect the type hierarchy, which sometimes makes it hard to get a good overview. If so, see the section [`gen_statem` Behaviour ](`p:system:statem.md`)in the [OTP Design Principles ](`p:system:index.html`)User's Guide.

> #### Note {: class=info }
> * This behavior appeared in Erlang/OTP 19.0.
> * In OTP 19.1 a backwards incompatible change of the return tuple from [`Module:init/1`](`c:init/1`) was made and the mandatory callback function [`Module:callback_mode/0` ](`c:callback_mode/0`)was introduced.
> * In OTP 20.0 [generic time-outs ](`t:generic_timeout/0`)were added.
> * In OTP 22.1 time-out content [`update` ](`t:timeout_update_action/0`)and explicit time-out [`cancel` ](`t:timeout_cancel_action/0`)were added.
> * In OTP 22.3 the possibility to change the callback module with actions [`change_callback_module`](`t:action/0`), [`push_callback_module`](`t:action/0`) and [`pop_callback_module`](`t:action/0`), was added.

`gen_statem` has got the same features that `m:gen_fsm` had and adds some really useful:
...
Two [*callback modes*](`t:callback_mode/0`) are supported:

* One for finite-state machines (`m:gen_fsm` like), which requires the state to be an atom and uses that state as the name of the current callback function.
* One that allows the state to be any term and that uses one callback function for all states.

The callback model(s) for `gen_statem` differs from the one for `m:gen_fsm`, but it is still fairly easy to [rewrite from ](`m:gen_fsm#migration to gen_statem`)`gen_fsm` to `gen_statem`.

A generic state machine server process (`gen_statem`) implemented using this module has a standard set of interface functions and includes functionality for tracing and error reporting. It also fits into an OTP supervision tree. For more information, see [OTP Design Principles](`p:system:statem.md`).
...
Events are of different [types](`t:event_type/0`), so the callback functions can know the origin of an event and how to respond.

If a callback function fails or returns a bad value, the `gen_statem` terminates, unless otherwise stated. However, an exception of class [`throw`](`erlang:throw/1`) is not regarded as an error but as a valid return from all callback functions.

[](){: id="state callback" }
The *state callback* for a specific [state](`t:state/0`) in a `gen_statem` is the callback function that is called for all events in this state. It is selected depending on which [*callback mode*](`t:callback_mode/0`) that the callback module defines with the callback function [`Module:callback_mode/0`](`c:callback_mode/0`).

When the [*callback mode*](`t:callback_mode/0`) is `state_functions`, the state must be an atom and is used as the *state callback* name; see [`Module:StateName/3`](`c:StateName/3`). This co-locates all code for a specific state in one function as the `gen_statem` engine branches depending on state name. Note the fact that the callback function [`Module:terminate/3`](`c:terminate/3`) makes the state name `terminate` unusable in this mode.

When the [*callback mode*](`t:callback_mode/0`) is `handle_event_function`, the state can be any term and the *state callback* name is [`Module:handle_event/4`](`c:handle_event/4`). This makes it easy to branch depending on state or event as you desire. Be careful about which events you handle in which states so that you do not accidentally postpone an event forever creating an infinite busy loop.

When `gen_statem` receives a process message it is converted into an event and the [*state callback*](`m:gen_statem#state callback`) is called with the event as two arguments: type and content. When the [*state callback*](`m:gen_statem#state callback`) has processed the event it returns to `gen_statem` which does a *state transition*. If this *state transition* is to a different state, that is: `NextState =/= State`, it is a *state change*.

The [*state callback*](`m:gen_statem#state callback`) may return [*transition actions*](`t:action/0`) for `gen_statem` to execute during the *state transition*, for example to reply to a [`gen_statem:call/2,3`](`call/2`).
...
The [*state callback*](`m:gen_statem#state callback`) can insert events using the [*transition actions*](`t:action/0`) `next_event` and such an event is inserted in the event queue as the next to call the [*state callback*](`m:gen_statem#state callback`) with. That is, as if it is the oldest incoming event. A dedicated [`event_type()`](`t:event_type/0`) `internal` can be used for such events making them impossible to mistake for external events.
...
The `gen_statem` engine can automatically make a specialized call to the [*state callback*](`m:gen_statem#state callback`) whenever a new state is entered; see [`state_enter()`](`t:state_enter/0`). This is for writing code common to all state entries. Another way to do it is to explicitly insert an event at the *state transition*, and/or to use a dedicated *state transition* function, but that is something you will have to remember at every *state transition* to the state(s) that need it.
...
For the details of a *state transition*, see type [`transition_option()`](`t:transition_option/0`).

A `gen_statem` handles system messages as described in `m:sys`. The `sys` module can be used for debugging a `gen_statem`.

Notice that a `gen_statem` does not trap exit signals automatically, this must be explicitly initiated in the callback module (by calling [`process_flag(trap_exit, true)`](`erlang:process_flag/2`).
...
The `gen_statem` process can go into hibernation; see `proc_lib:hibernate/3`. It is done when a [*state callback*](`m:gen_statem#state callback`) or [`Module:init/1`](`c:init/1`) specifies `hibernate` in the returned [`Actions`](`t:action/0`) list. This feature can be useful to reclaim process heap memory while the server is expected to be idle for a long time. However, use this feature with care, as hibernation can be too costly to use after every event; see `erlang:hibernate/3`.

There is also a server start option [`{hibernate_after, Timeout}` ](`t:enter_loop_opt/0`)for [`start/3,4`](`start/3`), [`start_monitor/3,4`](`start_monitor/3`), [`start_link/3,4`](`start_link/3`) or [`enter_loop/4,5,6`](`enter_loop/4`), that may be used to automatically hibernate the server.

If the `gen_statem` process terminates, e.g. as a result of a function in the callback module returning `{stop,Reason}`, an exit signal with this `Reason` is sent to linked processes and ports. See [Processes](`p:system:ref_man_processes.md#errors`) in the Reference Manual for details regarding error handling using exit signals.

> #### Note {: class=info }
> For some important information about distributed signals, see the [*Blocking Signaling Over Distribution*](`p:system:ref_man_processes.md#blocking-signaling-over-distribution`) section in the *Processes* chapter of the *Erlang Reference Manual*. Blocking signaling can, for example, cause call timeouts in `gen_statem` to be significantly delayed.

## Example

The following example shows a simple pushbutton model for a toggling pushbutton implemented with [*callback mode*](`t:callback_mode/0`) `state_functions`. You can push the button and it replies if it went on or off, and you can ask for a count of how many times it has been pushed to switch on.
...
To compare styles, here follows the same example using [*callback mode*](`t:callback_mode/0`) `handle_event_function`, or rather the code to replace after function `c:init/1` of the `pushbutton.erl` example file above:
...
`m:gen_event`, `m:gen_fsm`, `m:gen_server`, `m:proc_lib`, `m:supervisor`, `m:sys`.
""".
...
-doc "Destination to use when replying through, for example, the [`action()`](`t:action/0`) `{reply,From,Reply}` to a process that has called the `gen_statem` server using `call/2`.".
-type from() ::
...
-doc "If the [*callback mode*](`t:callback_mode/0`) is `handle_event_function`, the state can be any term. After a *state change* (`NextState =/= State`), all postponed events are retried.".
-type state() ::
	state_name() | % For StateName/3 callback functions
	term(). % For handle_event/4 callback function

-doc "If the [*callback mode*](`t:callback_mode/0`) is `state_functions`, the state must be an atom. After a *state change* (`NextState =/= State`), all postponed events are retried. Note that the state `terminate` is not possible to use since it would collide with the optional callback function [`Module:terminate/3`](`c:terminate/3`).".
-type state_name() :: atom().

-doc "A term in which the state machine implementation is to store any server data it needs. The difference between this and the [`state()`](`t:state/0`) itself is that a change in this data does not cause postponed events to be retried. Hence, if a change in this data would change the set of events that are handled, then that data item is to be made a part of the state.".
-type data() :: term().

-doc """
There are 3 categories of events: [external](`t:external_event_type/0`), [timeout](`t:timeout_event_type/0`), and `internal`.

`internal` events can only be generated by the state machine itself through the *transition action* [`next_event`](`t:action/0`).
""".
-type event_type() ::
        external_event_type() | timeout_event_type() | 'internal'.
-doc "External events are of 3 types: `{call,From}`, `cast`, or `info`. Type `call` originates from the API functions `call/2` and `send_request/2`. For calls, the event contains whom to reply to. Type `cast` originates from the API function `cast/2`. Type `info` originates from regular process messages sent to the `gen_statem`.".
-type external_event_type() ::
        {'call',From :: from()} | 'cast' | 'info'.
-doc "There are 3 types of time-out events that the state machine can generate for itself with the corresponding [timeout_action()](`t:timeout_action/0`)s.".
...
See [`event_type`](`t:event_type/0`) that describes the origins of the different event types, which is also where the event content comes from.
""".
-type event_content() :: term().

-doc "This is the return type from [`Module:callback_mode/0`](`c:callback_mode/0`) and selects [*callback mode*](`t:callback_mode/0`) and whether to do [*state enter calls*](`t:state_enter/0`), or not.".
-type callback_mode_result() ::
	callback_mode() | [callback_mode() | state_enter()].
-doc """
The *callback mode* is selected with the return value from [`Module:callback_mode/0`](`c:callback_mode/0`):

* __`state_functions`__ - The state must be of type [`state_name()`](`t:state_name/0`) and one callback function per state, that is, [`Module:StateName/3`](`c:StateName/3`), is used.

* __`handle_event_function`__ - The state can be any term and the callback function [`Module:handle_event/4`](`c:handle_event/4`) is used for all states.

The function [`Module:callback_mode/0`](`c:callback_mode/0`) is called when starting the `gen_statem`, after code change and after changing the callback module with any of the actions [`change_callback_module`](`t:action/0`), [`push_callback_module`](`t:action/0`) or [`pop_callback_module`](`t:action/0`). The result is cached for subsequent calls to [state callbacks](`m:gen_statem#state callback`).
""".
-type callback_mode() :: 'state_functions' | 'handle_event_function'.
-doc """
Whether the state machine should use *state enter calls* or not is selected when starting the `gen_statem` and after code change using the return value from [`Module:callback_mode/0`](`c:callback_mode/0`).

If [`Module:callback_mode/0`](`c:callback_mode/0`) returns a list containing `state_enter`, the `gen_statem` engine will, at every *state change*, call the [state callback](`m:gen_statem#state callback`) with arguments `(enter, OldState, Data)` or `(enter, OldState, State, Data)`, depending on the [*callback mode*](`t:callback_mode/0`). This may look like an event but is really a call performed after the previous [*state callback*](`m:gen_statem#state callback`) returned and before any event is delivered to the new [*state callback*](`m:gen_statem#state callback`). See [`Module:StateName/3`](`c:StateName/3`) and [`Module:handle_event/4`](`c:handle_event/4`). Such a call can be repeated by returning a [`repeat_state` ](`t:state_callback_result/1`)or [`repeat_state_and_data` ](`t:state_callback_result/1`)tuple from the *state callback*.

If [`Module:callback_mode/0`](`c:callback_mode/0`) does not return such a list, no *state enter calls* are done.

If [`Module:code_change/4`](`c:code_change/4`) should transform the state, it is regarded as a state rename and not a *state change*, which will not cause a *state enter call*.
...
-doc """
Transition options can be set by [actions](`t:action/0`) and modify the *state transition*. The *state transition* takes place when the [*state callback*](`m:gen_statem#state callback`) has processed an event and returns. Here are the sequence of steps for a *state transition*:

1. All returned [actions](`t:action/0`) are processed in order of appearance. In this step all replies generated by any [`reply_action()`](`t:reply_action/0`) are sent. Other actions set `t:transition_option()`s that come into play in subsequent steps.
1. If [*state enter calls* ](`t:state_enter/0`)are used, and either it is the initial state or one of the callback results [`repeat_state_and_data` ](`t:state_callback_result/1`)or [`repeat_state_and_data` ](`t:state_callback_result/1`)is used the `gen_statem` engine calls the current state callback with arguments [`(enter, State, Data)`](`t:state_enter/0`) or [`(enter, State, State, Data)`](`t:state_enter/0`) (depending on [*callback mode*](`t:callback_mode/0`)) and when it returns starts again from the top of this sequence.

  If [*state enter calls* ](`t:state_enter/0`)are used, and the state changes the `gen_statem` engine calls the new state callback with arguments [`(enter, OldState, Data)`](`t:state_enter/0`) or [`(enter, OldState, State, Data)`](`t:state_enter/0`) (depending on [*callback mode*](`t:callback_mode/0`)) and when it returns starts again from the top of this sequence.
1. If [`postpone()`](`t:postpone/0`) is `true`, the current event is postponed.
1. If this is a *state change*, the queue of incoming events is reset to start with the oldest postponed.
1. All events stored with [`action()`](`t:action/0`) `next_event` are inserted to be processed before previously queued events.
1. Time-out timers [`event_timeout()`](`t:event_timeout/0`), [`generic_timeout()`](`t:generic_timeout/0`) and [`state_timeout()`](`t:state_timeout/0`) are handled. Time-outs with zero time are guaranteed to be delivered to the state machine before any external not yet received event so if there is such a time-out requested, the corresponding time-out zero event is enqueued as the newest received event; that is after already queued events such as inserted and postponed events.

  Any event cancels an [`event_timeout()`](`t:event_timeout/0`) so a zero time event time-out is only generated if the event queue is empty.

  A *state change* cancels a [`state_timeout()`](`t:state_timeout/0`) and any new transition option of this type belongs to the new state, that is; a [`state_timeout()`](`t:state_timeout/0`) applies to the state the state machine enters.
1. If there are enqueued events the [*state callback*](`m:gen_statem#state callback`) for the possibly new state is called with the oldest enqueued event, and we start again from the top of this sequence.
1. Otherwise the `gen_statem` goes into `receive` or hibernation (if [`hibernate()`](`t:hibernate/0`) is `true`) to wait for the next message. In hibernation the next non-system event awakens the `gen_statem`, or rather the next incoming message awakens the `gen_statem`, but if it is a system event it goes right back into hibernation. When a new message arrives the [*state callback*](`m:gen_statem#state callback`) is called with the corresponding event, and we start again from the top of this sequence.
""".
-type transition_option() ::
	postpone() | hibernate() |
	event_timeout() | generic_timeout() | state_timeout().
-doc "If `true`, postpones the current event and retries it after a *state change* (`NextState =/= State`).".
-type postpone() ::
...
> #### Note {: class=info }
> If there are enqueued events to process when hibernation is requested, this is optimized by not hibernating but instead calling [`erlang:garbage_collect/0` ](`erlang:garbage_collect/0`)to simulate that the `gen_statem` entered hibernation and immediately got awakened by an enqueued event.
""".
-type hibernate() ::
	%% If 'true' hibernate the server instead of going into receive
	boolean().
-doc """
Starts a timer set by [`enter_action()`](`t:enter_action/0`) `timeout`. When the timer expires an event of [`event_type()`](`t:event_type/0`) `timeout` will be generated. See `erlang:start_timer/4` for how `Time` and [`Options`](`t:timeout_option/0`) are interpreted. Future `erlang:start_timer/4` `Options` will not necessarily be supported.
...
-doc """
Starts a timer set by [`enter_action()`](`t:enter_action/0`) `{timeout,Name}`. When the timer expires an event of [`event_type()`](`t:event_type/0`) `{timeout,Name}` will be generated. See `erlang:start_timer/4` for how `Time` and [`Options`](`t:timeout_option/0`) are interpreted. Future `erlang:start_timer/4` `Options` will not necessarily be supported.
...
-type generic_timeout() ::
	...
	Time :: timeout() | integer().
-doc """
Starts a timer set by [`enter_action()`](`t:enter_action/0`) `state_timeout`. When the timer expires an event of [`event_type()`](`t:event_type/0`) `state_timeout` will be generated. See `erlang:start_timer/4` for how `Time` and [`Options`](`t:timeout_option/0`) are interpreted. Future `erlang:start_timer/4` `Options` will not necessarily be supported.
...
-type state_timeout() ::
	...
	Time :: timeout() | integer().
-doc "If `Abs` is `true` an absolute timer is started, and if it is `false` a relative, which is the default. See [`erlang:start_timer/4` ](`erlang:start_timer/4`)for details.".
-type timeout_option() :: {abs,Abs :: boolean()}.

-doc """
These *transition actions* can be invoked by returning them from the [*state callback*](`m:gen_statem#state callback`) when it is called with an [event](`t:event_type/0`), from [`Module:init/1`](`c:init/1`) or by giving them to [`enter_loop/5,6`](`enter_loop/5`).

Actions are executed in the containing list order.

Actions that set [transition options ](`t:transition_option/0`)override any previous of the same type, so the last in the containing list wins. For example, the last [`postpone()`](`t:postpone/0`) overrides any previous `t:postpone()` in the list.

* __`postpone`__ - Sets the [`transition_option()` ](`t:transition_option/0`)[`postpone()`](`t:postpone/0`) for this *state transition*. This action is ignored when returned from [`Module:init/1`](`c:init/1`) or given to [`enter_loop/5,6`](`enter_loop/5`), as there is no event to postpone in those cases.

* __`next_event`__ - This action does not set any [`transition_option()` ](`t:transition_option/0`)but instead stores the specified `EventType` and `EventContent` for insertion after all actions have been executed.
  ...
  An event of type [`internal`](`t:event_type/0`) is to be used when you want to reliably distinguish an event inserted this way from any external event.

* __`change_callback_module`__ - Changes the callback module to `NewModule` which will be used when calling all subsequent [state callbacks](`m:gen_statem#state callback`).

  The `gen_statem` engine will find out the [*callback mode* ](`t:callback_mode/0`)of `NewModule` by calling [`NewModule:callback_mode/0` ](`c:callback_mode/0`)before the next [state callback](`m:gen_statem#state callback`).

  Changing the callback module does not affect the *state transition* in any way, it only changes which module that handles the events. Be aware that all relevant callback functions in `NewModule` such as the [state callback](`m:gen_statem#state callback`), [`NewModule:code_change/4`](`c:code_change/4`), [`NewModule:format_status/1` ](`c:format_status/1`)and [`NewModule:terminate/3` ](`c:terminate/3`)must be able to handle the state and data from the old module.

* __`push_callback_module`__ - Pushes the current callback module to the top of an internal stack of callback modules and changes the callback module to `NewModule`. Otherwise like `{change_callback_module, NewModule}` above.

* __`pop_callback_module`__ - Pops the top module from the internal stack of callback modules and changes the callback module to be the popped module. If the stack is empty the server fails. Otherwise like `{change_callback_module, NewModule}` above.
""".
-type action() ::
	...
	enter_action().
-doc """
These *transition actions* can be invoked by returning them from the [*state callback*](`m:gen_statem#state callback`), from [`Module:init/1`](`c:init/1`) or by giving them to [`enter_loop/5,6`](`enter_loop/5`).

Actions are executed in the containing list order.

Actions that set [transition options](`t:transition_option/0`) override any previous of the same type, so the last in the containing list wins. For example, the last [`event_timeout()`](`t:event_timeout/0`) overrides any previous `t:event_timeout()` in the list.

* __`hibernate`__ - Sets the [`transition_option()`](`t:transition_option/0`) [`hibernate()`](`t:hibernate/0`) for this *state transition*.
""".
-type enter_action() ::
...
* __`Time`__ - Short for `{timeout,Time,Time}`, that is, the time-out message is the time-out time. This form exists to make the [*state callback*](`m:gen_statem#state callback`) return value `{next_state,NextState,NewData,Time}` allowed like for `gen_fsm`.

* __`timeout`__ - Sets the [`transition_option()`](`t:transition_option/0`) [`event_timeout()`](`t:event_timeout/0`) to `Time` with `EventContent` and time-out options [`Options`](`t:timeout_option/0`).

* __`{timeout,Name}`__ - Sets the [`transition_option()`](`t:transition_option/0`) [`generic_timeout()`](`t:generic_timeout/0`) to `Time` for `Name` with `EventContent` and time-out options [`Options`](`t:timeout_option/0`).

* __`state_timeout`__ - Sets the [`transition_option()`](`t:transition_option/0`) [`state_timeout()`](`t:state_timeout/0`) to `Time` with `EventContent` and time-out options [`Options`](`t:timeout_option/0`).
""".
-type timeout_action() ::
...
-doc "This is a shorter and clearer form of [timeout_action() ](`t:timeout_action/0`)with `Time = infinity` which cancels a time-out.".
-type timeout_cancel_action() ::
        {'timeout', 'cancel'} |
        {{'timeout', Name :: term()}, 'cancel'} |
        {'state_timeout', 'cancel'}.
-doc """
Updates a time-out with a new `EventContent`. See [timeout_action() ](`t:timeout_action/0`)for how to start a time-out.
...
-doc """
This *transition action* can be invoked by returning it from the [*state callback*](`m:gen_statem#state callback`), from [`Module:init/1`](`c:init/1`) or by giving it to [`enter_loop/5,6`](`enter_loop/5`).

It does not set any [`transition_option()` ](`t:transition_option/0`)but instead replies to a caller waiting for a reply in `call/2`. `From` must be the term from argument [`{call,From}`](`t:event_type/0`) in a call to a [*state callback*](`m:gen_statem#state callback`).

Note that using this action from [`Module:init/1`](`c:init/1`) or [`enter_loop/5,6`](`enter_loop/5`) would be weird on the border of witchcraft since there has been no earlier call to a [*state callback*](`m:gen_statem#state callback`) in this server.
""".
-type reply_action() ::
...
-doc """
For a succesful initialization, `State` is the initial [`state()`](`t:state/0`) and `Data` the initial server [`data()`](`t:data/0`) of the `gen_statem`.

The [`Actions`](`t:action/0`) are executed when entering the first [state](`t:state/0`) just as for a [*state callback*](`m:gen_statem#state callback`), except that the action `postpone` is forced to `false` since there is no event to postpone.

For an unsuccesful initialization, `{stop,Reason}`, `{error,Reason}` or `ignore` should be used; see [`start_link/3,4`](`start_link/3`).
...
%%
-doc """
`State` is the current state and it cannot be changed since the state callback was called with a [*state enter call*](`t:state_enter/0`).

* __`next_state`__ - The `gen_statem` does a state transition to `State`, which has to be the current state, sets `NewData`, and executes all `Actions`.
""".
...
-doc """
`StateType` is [`state_name()`](`t:state_name/0`) if [*callback mode*](`t:callback_mode/0`) is `state_functions`, or [`state()`](`t:state/0`) if [*callback mode*](`t:callback_mode/0`) is `handle_event_function`.

* __`next_state`__ - The `gen_statem` does a *state transition* to `NextState` (which can be the same as the current state), sets `NewData`, and executes all `Actions`. If `NextState =/= CurrentState` the *state transition* is a *state change*.
""".
...
-doc """
`ActionType` is [`enter_action()`](`t:enter_action/0`) if the state callback was called with a [*state enter call*](`t:state_enter/0`) and [`action()`](`t:action/0`) if the state callback was called with an event.

* __`keep_state`__ - The same as `{next_state,CurrentState,NewData,Actions}`.

* __`keep_state_and_data`__ - The same as `{keep_state,CurrentData,Actions}`.

* __`repeat_state`__ - If the `gen_statem` runs with [*state enter calls*](`t:state_enter/0`), the *state enter call* is repeated, see type [`transition_option()`](`t:transition_option/0`), other than that `repeat_state` is the same as `keep_state`.

* __`repeat_state_and_data`__ - The same as `{repeat_state,CurrentData,Actions}`.

* __`stop`__ - Terminates the `gen_statem` by calling [`Module:terminate/3`](`c:terminate/3`) with `Reason` and `NewData`, if specified. An exit signal with this reason is sent to linked processes and ports. The default `Reason` is `normal`.

* __`stop_and_reply`__ - Sends all `Replies`, then terminates the `gen_statem` by calling [`Module:terminate/3`](`c:terminate/3`) with `Reason` and `NewData`, if specified. An exit signal with this reason is sent to linked processes and ports.
...
-doc "An opaque collection of request identifiers ([`request_id()`](`t:request_id/0`)) where each request identifier can be associated with a label chosen by the user. For more information see `reqids_new/0`.".
...
* __`0..4294967295`__ - Timeout relative to current time in milliseconds.

* __`infinity`__ - Infinite timeout. That is, the operation will never time out.

* __`{abs, Timeout}`__ - An absolute [Erlang monotonic time](`erlang:monotonic_time/1`) timeout in milliseconds. That is, the operation will time out when [`erlang:monotonic_time(millisecond)`](`erlang:monotonic_time/1`) returns a value larger than or equal to `Timeout`. `Timeout` is not allowed to identify a time further into the future than `4294967295` milliseconds. Identifying the timeout using an absolute timeout value is especially handy when you have a deadline for responses corresponding to a complete collection of requests ([`request_id_collection()`](`t:request_id_collection/0`)) , since you do not have to recalculate the relative time until the deadline over and over again.
""".
...
-doc """
Args = term()  
Result(StateType) = [init_result(StateType)](`t:init_result/1`)  

[](){: id=Module:init-1 }
Whenever a `gen_statem` is started using [`start_link/3,4`](`start_link/3`), [`start_monitor/3,4`](`start_monitor/3`), or [`start/3,4`](`start/3`), this function is called by the new process to initialize the implementation state and server data.

`Args` is the `Args` argument provided to that start function.

> #### Note {: class=info }
> Note that if the `gen_statem` is started through `m:proc_lib` and [`enter_loop/4-6`](`enter_loop/4`), this callback will never be called. Since this callback is not optional it can in that case be implemented as:
>
> ```text
...
-doc """
CallbackMode = [callback_mode()](`t:callback_mode/0`) | [ [callback_mode()](`t:callback_mode/0`) | [state_enter()](`t:state_enter/0`) ]  

This function is called by a `gen_statem` when it needs to find out the [*callback mode*](`t:callback_mode/0`) of the callback module. The value is cached by `gen_statem` for efficiency reasons, so this function is only called once after server start, after code change, and after changing the callback module, but before the first [*state callback*](`m:gen_statem#state callback`) in the current callback module's code version is called. More occasions may be added in future versions of `gen_statem`.

Server start happens either when [`Module:init/1`](`c:init/1`) returns or when [`enter_loop/4-6`](`enter_loop/4`) is called. Code change happens when [`Module:code_change/4`](`c:code_change/4`) returns. A change of the callback module happens when a [*state callback*](`m:gen_statem#state callback`) returns any of the actions [`change_callback_module`](`t:action/0`), [`push_callback_module`](`t:action/0`) or [`pop_callback_module`](`t:action/0`).

The `CallbackMode` is either just [`callback_mode()`](`t:callback_mode/0`) or a list containing [`callback_mode()`](`t:callback_mode/0`) and possibly the atom [`state_enter`](`t:state_enter/0`).
...
-doc """
EventType = [event_type()](`t:event_type/0`)  
EventContent = term()  
State = [state()](`t:state/0`)  
Data = NewData = [data()](`t:data/0`)  
StateEnterResult(StateName) = [state_enter_result(StateName)](`t:state_enter_result/1`)  
StateFunctionResult = [event_handler_result](`t:event_handler_result/1`)([state_name()](`t:state_name/0`))  
StateEnterResult(State) = [state_enter_result(State)](`t:state_enter_result/1`)  
HandleEventResult = [event_handler_result](`t:event_handler_result/1`)([state()](`t:state/0`))  

Whenever a `gen_statem` receives an event from `call/2`, `cast/2`, or as a normal process message, one of these functions is called. If [*callback mode*](`t:callback_mode/0`) is `state_functions`, `Module:StateName/3` is called, and if it is `handle_event_function`, [`Module:handle_event/4`](`c:handle_event/4`) is called.

If `EventType` is [`{call,From}`](`t:event_type/0`), the caller waits for a reply. The reply can be sent from this or from any other [*state callback*](`m:gen_statem#state callback`) by returning with `{reply,From,Reply}` in [`Actions`](`t:action/0`), in [`Replies`](`t:reply_action/0`), or by calling [`reply(From, Reply)`](`reply/2`).
...
For options that can be set and actions that can be done by `gen_statem` after returning from this function, see [`action()`](`t:action/0`).

When the `gen_statem` runs with [*state enter calls*](`t:state_enter/0`), these functions are also called with arguments `(enter, OldState, ...)` during every *state change*. In this case there are some restrictions on the [actions](`t:enter_action/0`) that may be returned: [`postpone()`](`t:postpone/0`) is not allowed since a *state enter call* is not an event so there is no event to postpone, and [`{next_event,_,_}`](`t:action/0`) is not allowed since using *state enter calls* should not affect how events are consumed and produced. You may also not change states from this call. Should you return `{next_state,NextState, ...}` with `NextState =/= State` the `gen_statem` crashes. Note that it is actually allowed to use `{repeat_state, NewData, ...}` although it makes little sense since you immediately will be called again with a new *state enter call* making this just a weird way of looping, and there are better ways to loop in Erlang. If you do not update `NewData` and have some loop termination condition, or if you use `{repeat_state_and_data, _}` or `repeat_state_and_data` you have an infinite loop\! You are advised to use `{keep_state,...}`, `{keep_state_and_data,_}` or `keep_state_and_data` since changing states from a *state enter call* is not possible anyway.

Note the fact that you can use [`throw`](`erlang:throw/1`) to return the result, which can be useful. For example to bail out with `throw(keep_state_and_data)` from deep within complex code that cannot return `{next_state,State,Data}` because `State` or `Data` is no longer in scope.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
This function is called by a `gen_statem` when it is about to terminate. It is to be the opposite of [`Module:init/1`](`c:init/1`) and do any necessary cleaning up. When it returns, the `gen_statem` terminates with `Reason`. The return value is ignored.

`Reason` is a term denoting the stop reason and [`State`](`t:state/0`) is the internal state of the `gen_statem`.

`Reason` depends on why the `gen_statem` is terminating. If it is because another callback function has returned, a stop tuple `{stop,Reason}` in [`Actions`](`t:action/0`), `Reason` has the value specified in that tuple. If it is because of a failure, `Reason` is the error reason.
...
Notice that for any other reason than `normal`, `shutdown`, or `{shutdown,Term}`, the `gen_statem` is assumed to terminate because of an error and an error report is issued using `m:logger`.

When the `gen_statem` process exits, an exit signal with the same reason is sent to linked processes and ports.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback terminate(
...
OldState = NewState = [state()](`t:state/0`)  
OldData = NewData = [data()](`t:data/0`)  
Reason = term()  

> #### Note {: class=info }
> This callback is optional, so callback modules need not export it. If a release upgrade/downgrade with `Change = {advanced,Extra}` specified in the `.appup` file is made when `c:code_change/4` is not implemented the process will crash with exit reason `undef`.

This function is called by a `gen_statem` when it is to update its internal state during a release upgrade/downgrade, that is, when the instruction `{update,Module,Change,...}`, where `Change = {advanced,Extra}`, is specified in the [`appup`](`p:sasl:appup.md`) file. For more information, see [OTP Design Principles](`p:system:release_handling.md#instr`).
...
> This callback is deprecated, in new code use `c:format_status/1`. If a `c:format_status/1` callback exists, then this function will never be called.
...
>
> If this callback is exported but fails, to hide possibly sensitive data, the default function will instead return `{State,Info}`, where `Info` says nothing but the fact that `c:format_status/2` has crashed.

This function is called by a `gen_statem` process when any of the following apply:

* One of [`sys:get_status/1,2`](`sys:get_status/1`) is invoked to get the `gen_statem` status. `Opt` is set to the atom `normal` for this case.
* The `gen_statem` terminates abnormally and logs an error. `Opt` is set to the atom `terminate` for this case.

This function is useful for changing the form and appearance of the `gen_statem` status for these cases. A callback module wishing to change the [`sys:get_status/1,2`](`sys:get_status/1`) return value and how its status appears in termination error logs exports an instance of `c:format_status/2`, which returns a term describing the current status of the `gen_statem`.
...
The function is to return `Status`, a term that contains the appropriate details of the current state and status of the `gen_statem`. There are no restrictions on the form `Status` can take, but for the [`sys:get_status/1,2`](`sys:get_status/1`) case (when `Opt` is `normal`), the recommended form for the `Status` value is `[{data, [{"State", Term}]}]`, where `Term` provides relevant details of the `gen_statem` state. Following this recommendation is not required, but it makes the callback module status consistent with the rest of the [`sys:get_status/1,2`](`sys:get_status/1`) return value.
...
* __`state`__ - The current state of the `gen_statem` process.

* __`data`__ - The state data of the the `gen_statem` process.

* __`reason`__ - The reason that caused the state machine to terminate.

* __`queue`__ - The event queue of the `gen_statem` process.

* __`postponed`__ - The [postponed](`t:postpone/0`) events queue of the `gen_statem` process.

* __`timeouts`__ - The active [time-outs](`t:timeout_action/0`) of the `gen_statem` process.

* __`log`__ - The [sys log](`sys:log/2`) of the server.
...
%% often condensed way.
-doc """
Status = [format_status()](`t:format_status/0`)  
NewStatus = [format_status()](`t:format_status/0`)  
...
-doc "Name specification to use when starting a `gen_statem` server. See `start_link/3` and [`server_ref()`](`t:server_ref/0`) below.".
...
-doc """
Server specification to use when addressing a `gen_statem` server. See `call/2` and [`server_name()`](`t:server_name/0`) above.

It can be:

* __`pid() | LocalName`__ - The `gen_statem` is locally registered.

* __`{Name,Node}`__ - The `gen_statem` is locally registered on another node.

* __`{global,GlobalName}`__ - The `gen_statem` is globally registered in `m:global`.

* __`{via,RegMod,ViaName}`__ - The `gen_statem` is registered in an alternative process registry. The registry callback module `RegMod` is to export functions `register_name/2`, `unregister_name/1`, `whereis_name/1`, and `send/2`, which are to behave like the corresponding functions in `m:global`. Thus, `{via,global,GlobalName}` is the same as `{global,GlobalName}`.
""".
...
-doc "Options that can be used when starting a `gen_statem` server through, for example, `start_link/3`.".
...
%%
-doc """
Options that can be used when starting a `gen_statem` server through, [`enter_loop/4-6`](`enter_loop/4`).

* __`hibernate_after`__ - `HibernateAfterTimeout` specifies that the `gen_statem` process awaits any message for `HibernateAfterTimeout` milliseconds and if no message is received, the process goes into hibernation automatically (by calling `proc_lib:hibernate/3`).

* __`debug`__ - For every entry in `Dbgs`, the corresponding function in `m:sys` is called.
""".
-type enter_loop_opt() :: % Some gen:option()s works for enter_loop/*
	{'hibernate_after', HibernateAfterTimeout :: timeout()}
      | {'debug', Dbgs :: [sys:debug_option()]}.

-doc "Return value from the [`start/3,4`](`start/3`) and [`start_link/3,4`](`start_link/3`) functions.".
...
%%
-doc """
Creates a standalone `gen_statem` process according to OTP design principles (using `m:proc_lib` primitives). As it does not get linked to the calling process, this start function cannot be used by a supervisor to start a child.

For a description of arguments and return values, see [`start_link/3,4`](`start_link/3`).
""".
-doc(#{since => <<"OTP 19.0">>}).
-spec start(
...
%%
-doc """
Creates a `gen_statem` process according to OTP design principles (using `m:proc_lib` primitives) that is spawned as linked to the calling process. This is essential when the `gen_statem` must be part of a supervision tree so it gets linked to its supervisor.

The `gen_statem` process calls [`Module:init/1`](`c:init/1`) to initialize the server. To ensure a synchronized startup procedure, `start_link/3,4` does not return until [`Module:init/1`](`c:init/1`) has returned or failed.

`ServerName` specifies the [`server_name()`](`t:server_name/0`) to register for the `gen_statem` process. If the `gen_statem` process is started with `start_link/3`, no `ServerName` is provided and the `gen_statem` process is not registered.

`Module` is the name of the callback module.

`Args` is an arbitrary term that is passed as the argument to [`Module:init/1`](`c:init/1`).

* If option [`{timeout,Time}` ](`t:start_opt/0`)is present in `Opts`, the `gen_statem` process is allowed to spend `Time` milliseconds initializing or it is terminated and the start function returns [`{error,timeout}`](`t:start_ret/0`).
* If option [`{hibernate_after,HibernateAfterTimeout}` ](`t:enter_loop_opt/0`)is present, the `gen_statem` process awaits any message for `HibernateAfterTimeout` milliseconds and if no message is received, the process goes into hibernation automatically (by calling `proc_lib:hibernate/3`).
* If option [`{debug,Dbgs}` ](`t:enter_loop_opt/0`)is present in `Opts`, debugging through `m:sys` is activated.
* If option [`{spawn_opt,SpawnOpts}` ](`t:start_opt/0`)is present in `Opts`, `SpawnOpts` is passed as option list to `erlang:spawn_opt/2`, which is used to spawn the `gen_statem` process.

> #### Note {: class=info }
> Using spawn option `monitor` is not allowed, it causes this function to fail with reason `badarg`.

If the `gen_statem` process is successfully created and initialized, this function returns [`{ok,Pid}`](`t:start_ret/0`), where `Pid` is the `t:pid()` of the `gen_statem` process. If a process with the specified `ServerName` exists already, this function returns [`{error,{already_started,OtherPid}}`](`t:start_ret/0`), where `OtherPid` is the `t:pid()` of that process, and the `gen_statem` process exits with reason `normal` before calling [`Module:init/1`](`c:init/1`).

If [`Module:init/1`](`c:init/1`) does not return within the [start timeout](`t:start_opt/0`), the `gen_statem` process is killed with [`exit(_, kill)`](`erlang:exit/2`), and this function returns [`{error,timeout}`](`t:start_ret/0`).

This function returns [`{error,Reason}`](`t:start_ret/0`) if [`Module:init/1`](`c:init/1`) returns [`{stop,Reason}`](`t:init_result/1`) or [`{error,Reason}`](`t:init_result/1`), or fails with reason `Reason`. This function returns [`ignore`](`t:start_ret/0`) if [`Module:init/1`](`c:init/1`) returns [`ignore`](`t:init_result/1`). In these cases the `gen_statem` process exits with reason `Reason`, except when [`Module:init/1`](`c:init/1`) returns `ignore` or `{error,_}`; then the `gen_statem` process exits with reason `normal`.
...
> #### Warning {: class=warning }
> Before OTP 26.0, if the started `gen_statem` process returned e.g. `{stop,Reason}` from [`Module:init/1`](`c:init/1`), this function could return `{error,Reason}` *before* the started `gen_statem` process had terminated so starting again might fail because VM resources such as the registered name was not yet unregistered, and an `'EXIT'` message could arrive later to the process calling this function.
>
> But if the started `gen_statem` process instead failed during [`Module:init/1`](`c:init/1`), a process link `{'EXIT',Pid,Reason}` message caused this function to return `{error,Reason}` so the `'EXIT'` message had been consumed and the started `gen_statem` process had terminated.
>
...
The difference between returning `{stop,_}` and `{error,_}` from [`Module:init/1`](`c:init/1`), is that `{error,_}` results in a graceful ("silent") termination since the `gen_statem` process exits with reason `normal`.
""".
-doc(#{since => <<"OTP 19.0">>}).
-spec start_link(
	ServerName :: server_name(),
...
%%
-doc """
Creates a standalone `gen_statem` process according to OTP design principles (using `m:proc_lib` primitives) and atomically sets up a monitor to the newly created process. As it does not get linked to the calling process, this start function cannot be used by a supervisor to start a child.
...
%%
-doc """
Orders the `gen_statem` [`ServerRef`](`t:server_ref/0`) to exit with the specified `Reason` and waits for it to terminate. The `gen_statem` calls [`Module:terminate/3`](`c:terminate/3`) before exiting.

This function returns `ok` if the server terminates with the expected reason. Any other reason than `normal`, `shutdown`, or `{shutdown,Term}` causes an error report to be issued through `m:logger`. An exit signal with the same reason is sent to linked processes and ports. The default `Reason` is `normal`.
...
-doc "Sends an asynchronous event to the `gen_statem` [`ServerRef`](`t:server_ref/0`) and returns `ok` immediately, ignoring if the destination node or `gen_statem` does not exist. The `gen_statem` calls the [*state callback*](`m:gen_statem#state callback`) with [`event_type()`](`t:event_type/0`) `cast` and event content `Msg`.".
-doc(#{since => <<"OTP 19.0">>}).
...
%%
-doc """
Makes a synchronous call to the `gen_statem` [`ServerRef`](`t:server_ref/0`) by sending a request and waiting until its reply arrives. The `gen_statem` calls the [*state callback*](`m:gen_statem#state callback`) with [`event_type()`](`t:event_type/0`) `{call,From}` and event content `Request`.

A `Reply` is generated when a [*state callback*](`m:gen_statem#state callback`) returns with `{reply,From,Reply}` as one [`action()`](`t:action/0`), and that `Reply` becomes the return value of this function.
...
Previous issue with late replies that could occur when having network issues or using `dirty_timeout` is now prevented by use of [*process aliases*](`p:system:ref_man_processes.md#process-aliases`). `{clean_timeout, T}` and `{dirty_timeout, T}` therefore no longer serves any purpose and will work the same as `Timeout` while all of them also being equally efficient.

The call can also fail, for example, if the `gen_statem` dies before or during this function call.

When this call fails it [exits](`erlang:exit/1`) the calling process. The exit term is on the form `{Reason, Location}` where `Location = {gen_statem,call,ArgList}`. See [`gen_server:call/3` ](`gen_server:call/3`)that has a description of relevant values for the `Reason` in the exit term.
""".
-doc(#{since => <<"OTP 19.0">>}).
-spec call(
	ServerRef :: server_ref(),
...
The call `gen_statem:wait_response(gen_statem:send_request(ServerRef,Request), Timeout)` can be seen as equivalent to [`gen_statem:call(Server,Request,Timeout)`](`call/3`), ignoring the error handling.

The `gen_statem` calls the [*state callback*](`m:gen_statem#state callback`) with [`event_type()`](`t:event_type/0`) `{call,From}` and event content `Request`.

A `Reply` is generated when a [*state callback*](`m:gen_statem#state callback`) returns with `{reply,From,Reply}` as one [`action()`](`t:action/0`), and that `Reply` becomes the return value of [`receive_response/1,2`](`receive_response/2`), [`wait_response/1,2`](`wait_response/2`), or `check_response/2` function.
""".
-doc(#{since => <<"OTP 23.0">>}).
...
The same as calling [`gen_statem:reqids_add`](`reqids_add/3`)([`statem:send_request`](`send_request/2`)`(ServerRef, Request), Label, ReqIdCollection)`, but calling `send_request/4` is slightly more efficient.
""".
-doc(#{since => <<"OTP 25.0">>}).
...
-doc "The same as calling [`gen_statem:receive_response(ReqId, infinity)`](`receive_response/2`).".
-doc(#{since => <<"OTP 23.0">>}).
...
The return value `Reply` is generated when a [*state callback*](`m:gen_statem#state callback`) returns with `{reply,From,Reply}` as one [`action()`](`t:action/0`), and that `Reply` becomes the return value of this function.
...
The `Label` in the response equals the `Label` associated with the request identifier that the response corresponds to. The `Label` of a request identifier is associated when [saving the request id](`reqids_add/3`) in a request identifier collection, or when sending the request using `send_request/4`.
...
The `Label` in the response equals the `Label` associated with the request identifier that the response corresponds to. The `Label` of a request identifier is associated when [adding the request id](`reqids_add/3`) in a request identifier collection, or when sending the request using `send_request/4`.
...
-doc "Saves `ReqId` and associates a `Label` with the request identifier by adding this information to `ReqIdCollection` and returning the resulting request identifier collection.".
-doc(#{since => <<"OTP 25.0">>}).
...
-doc "Returns a list of `{ReqId, Label}` tuples which corresponds to all request identifiers with their associated labels present in the `ReqIdCollection` collection.".
-doc(#{since => <<"OTP 25.0">>}).
...
-doc """
This function can be used by a `gen_statem` to explicitly send a reply to a process that waits in `call/2` when the reply cannot be defined in the return value of a [*state callback*](`m:gen_statem#state callback`).

`From` must be the term from argument [`{call,From}`](`t:event_type/0`) to the [*state callback*](`m:gen_statem#state callback`). A reply or multiple replies canalso be sent using one or several [`reply_action()`](`t:reply_action/0`)s from a [*state callback*](`m:gen_statem#state callback`).
...
-doc "The same as `enter_loop/6` with `Actions = []` except that no [`server_name()`](`t:server_name/0`) must have been registered. This creates an anonymous server.".
-doc(#{since => <<"OTP 19.1">>}).
-spec enter_loop(
...
%%
-doc """
If `Server_or_Actions` is a `t:list()`, the same as `enter_loop/6` except that no [`server_name()`](`t:server_name/0`) must have been registered and `Actions = Server_or_Actions`. This creates an anonymous server.
...
%%
-doc """
Makes the calling process become a `gen_statem`. Does not return, instead the calling process enters the `gen_statem` receive loop and becomes a `gen_statem` server. The process *must* have been started using one of the start functions in `m:proc_lib`. The user is responsible for any initialization of the process, including registering a name for it.

This function is useful when a more complex initialization procedure is needed than the `gen_statem` behavior provides.

`Module`, `Opts` have the same meaning as when calling [`start[_link|_monitor]/3,4`](`start_link/3`).

If `Server` is `self()` an anonymous server is created just as when using [`start[_link|_monitor]/3`](`start_link/3`). If `Server` is a [`server_name()`](`t:server_name/0`) a named server is created just as when using [`start[_link|_monitor]/4`](`start_link/4`). However, the [`server_name()`](`t:server_name/0`) name must have been registered accordingly *before* this function is called.

`State`, `Data`, and `Actions` have the same meanings as in the return value of [`Module:init/1`](`c:init/1`). Also, the callback module does not need to export a [`Module:init/1`](`c:init/1`) function.

The function fails if the calling process was not started by a `m:proc_lib` start function, or if it is not registered according to [`server_name()`](`t:server_name/0`).
""".
-doc(#{since => <<"OTP 19.0">>}).
-spec enter_loop(
	Module :: module(), Opts :: [enter_loop_opt()],

...
* `simple_one_for_one` \- A simplified `one_for_one` supervisor, where all child processes are dynamically added instances of the same process type, that is, running the same code.
  ...
  Function `terminate_child/2` can be used for children under `simple_one_for_one` supervisors by specifying the child's `t:pid()` as the second argument. If instead the child specification identifier is used, `terminate_child/2` return `{error,simple_one_for_one}`.
...
[](){: id=auto_shutdown }
A supervisor can be configured to automatically shut itself down with exit reason `shutdown` when [significant children](`m:supervisor#significant_child`) terminate with the `auto_shutdown` key in the above map:
...
For more information, see the section [Automatic Shutdown](`p:system:sup_princ.md#automatic-shutdown`) in Supervisor Behavior in OTP Design Principles.
...
The old tuple format is kept for backwards compatibility, see [child_spec()](`t:child_spec/0`), but the map is preferred.
...
* [](){: id=significant_child }
  `significant` defines if a child is considered significant for [automatic self-shutdown](`m:supervisor#auto_shutdown`) of the supervisor.

  Setting this option to `true` when the [restart type](`m:supervisor#restart`) is `permanent` is invalid. Also, it is considered invalid to start children with this option set to `true` in a supervisor when the [`auto_shutdown`](`m:supervisor#auto_shutdown`) supervisor flag is set to `never`.
...
-doc "Not a `t:pid()`.".
-type child_id()      :: term().
-doc "Value `undefined` for `A` (the argument list) is only to be used internally in `supervisor`. If the restart type of the child is `temporary`, the process is never to be restarted and therefore there is no need to store the real argument list. Value `undefined` is then stored instead.".
...
-type sup_ref()       :: (Name :: atom())
                       ...
                       | {'via', Module :: module(), Name :: any()}
                       | pid().
-doc "The tuple format is kept for backward compatibility only. A map is preferred; see more details [above](`m:supervisor#child_spec`).".
...
-doc "The tuple format is kept for backward compatibility only. A map is preferred; see more details [above](`m:supervisor#sup_flags`).".
...
 SupFlags = [`sup_flags()`](`t:sup_flags/0`)  
 ChildSpec = [`child_spec()`](`t:child_spec/0`)  

Whenever a supervisor is started using [`start_link/2,3`](`start_link/2`), this function is called by the new process to find out about restart strategy, maximum restart intensity, and child specifications.

`Args` is the `Args` argument provided to the start function.

`SupFlags` is the supervisor flags defining the restart strategy and maximum restart intensity for the supervisor. `[ChildSpec]` is a list of valid child specifications defining which child processes the supervisor must start and monitor. See the discussion in section [`Supervision Principles`](`m:supervisor#supervision_princ`) earlier.
...
Notice that this function can also be called as a part of a code upgrade procedure. Therefore, the function is not to have any side effects. For more information about code upgrade of supervisors, see section [Changing a Supervisor](`p:system:appup_cookbook.md#sup`) in OTP Design Principles.
""".
...
The created supervisor process calls [`Module:init/1`](`c:init/1`) to find out about restart strategy, maximum restart intensity, and child processes. To ensure a synchronized startup procedure, `start_link/2,3` does not return until [`Module:init/1`](`c:init/1`) has returned and all child processes have been started.
...
* If `SupName={via,Module,Name}`, the supervisor is registered as `Name` using the registry represented by `Module`. The `Module` callback must export the functions `register_name/2`, `unregister_name/1`, and `send/2`, which must behave like the corresponding functions in `m:global`. Thus, `{via,global,Name}` is a valid reference.
...
* If [`Module:init/1`](`c:init/1`) returns `ignore`, this function returns `ignore` as well, and the supervisor terminates with reason `normal`.
* If [`Module:init/1`](`c:init/1`) fails or returns an incorrect value, this function returns `{error,Term}`, where `Term` is a term with information about the error, and the supervisor terminates with reason `Term`.
...
`SupRef`{: id=SupRef } can be any of the following:
...
If the supervisor is `simple_one_for_one`, `Id` must be the `t:pid()` of the child process. If the specified process is alive, but is not a child of the specified supervisor, the function returns `{error,not_found}`. If the child specification identifier is specified instead of a `t:pid()`, the function returns `{error,simple_one_for_one}`.
...
-doc """
Returns a property list (see `m:proplists`) containing the counts for each of the following elements of the supervisor's child specifications and managed processes:
...
If the optional `AutoShutdown` argument is given and not `undefined`, also checks if the child specifications are allowed for the given [auto_shutdown](`m:supervisor#auto_shutdown`) option.
""".
-doc(#{since => <<"OTP 24.0">>}).

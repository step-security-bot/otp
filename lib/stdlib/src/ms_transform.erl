...
[](){: id=top }
This module provides the parse transformation that makes calls to `m:ets` and `dbg:fun2ms/1` translate into literal match specifications. It also provides the back end for the same functions when called from the Erlang shell.
...
As everyone trying to use [`ets:select/2`](`ets:select/1`) or `m:dbg` seems to end up reading this manual page, this description is an introduction to the concept of match specifications.
...
This example illustrates the most used calls in match specifications for `dbg`. The other, more esoteric, calls are listed and explained in [Match specifications in Erlang](`p:erts:match_spec.md`) in ERTS User's Guide, as they are beyond the scope of this description.
...
For more information about match specifications, see the [Match specifications in Erlang](`p:erts:match_spec.md`) in ERTS User's Guide.
""".
...
-doc "Takes an error code returned by one of the other functions in the module and creates a textual description of the error.".
...
For information about how to use this parse transformation, see `m:ets` and `dbg:fun2ms/1`.

For a description of match specifications, see section [Match Specification in Erlang](`p:erts:match_spec.md`) in ERTS User's Guide.
""".

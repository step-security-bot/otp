...
This module provides a pseudo random number generator. The module contains a number of algorithms. The uniform distribution algorithms are based on the [Xoroshiro and Xorshift algorithms ](http://xorshift.di.unimi.it)by Sebastiano Vigna. The normal distribution algorithm uses the [Ziggurat Method by Marsaglia and Tsang ](http://www.jstatsoft.org/v05/i08)on top of the uniform distribution algorithm.
...
At the end of this module documentation there are also some [niche algorithms ](`m:rand#niche-algorithms-api`)to be used without this module's normal [plug-in framework API ](`m:rand#plug-in-framework-api`)that may be useful for special purposes like short generation time when quality is not essential, for seeding other generators, and such.

[](){: id=algorithms }
The following algorithms are provided:

* __`exsss`__(Since OTP 22.0)  
  Xorshift116**, 58 bits precision and period of 2^116-1

  Jump function: equivalent to 2^64 calls

  This is the Xorshift116 generator combined with the StarStar scrambler from the 2018 paper by David Blackman and Sebastiano Vigna: [Scrambled Linear Pseudorandom Number Generators](http://vigna.di.unimi.it/ftp/papers/ScrambledLinear.pdf)
  ...
  Alas, this combination is about 10% slower than `exrop`, but is despite that the [*default algorithm* ](`m:rand#default-algorithm`)thanks to its statistical qualities.

* __`exro928ss`__(Since OTP 22.0)  
  Xoroshiro928**, 58 bits precision and a period of 2^928-1

  Jump function: equivalent to 2^512 calls

  This is a 58 bit version of Xoroshiro1024**, from the 2018 paper by David Blackman and Sebastiano Vigna: [Scrambled Linear Pseudorandom Number Generators ](http://vigna.di.unimi.it/ftp/papers/ScrambledLinear.pdf)that on a 64 bit Erlang system executes only about 40% slower than the [*default* `exsss` *algorithm* ](`m:rand#default-algorithm`)but with much longer period and better statistical properties, but on the flip side a larger state.

  Many thanks to Sebastiano Vigna for his help with the 58 bit adaption.

* __`exrop`__(Since OTP 20.0)  
  Xoroshiro116+, 58 bits precision and period of 2^116-1

  Jump function: equivalent to 2^64 calls

* __`exs1024s`__(Since OTP 20.0)  
  Xorshift1024*, 64 bits precision and a period of 2^1024-1

  Jump function: equivalent to 2^512 calls

* __`exsp`__(Since OTP 20.0)  
  Xorshift116+, 58 bits precision and period of 2^116-1

  Jump function: equivalent to 2^64 calls

  This is a corrected version of the previous [*default algorithm*, ](`m:rand#default-algorithm`)that now has been superseded by Xoroshiro116+ (`exrop`). Since there is no native 58 bit rotate instruction this algorithm executes a little (say < 15%) faster than `exrop`. See the [algorithms' homepage](http://xorshift.di.unimi.it).

[](){: id=default-algorithm }
The current *default algorithm* is [`exsss` (Xorshift116**). ](`m:rand#algorithms`)If a specific algorithm is required, ensure to always use `seed/1` to initialize the state.
...
If a process calls `uniform/0`, `uniform/1` or `uniform_real/0` without setting a seed first, `seed/1` is called automatically with the [*default algorithm* ](`m:rand#default-algorithm`)and creates a non-constant seed.

The functions with explicit state never use the process dictionary.

*Examples:*

Simple use; creates and seeds the [*default algorithm* ](`m:rand#default-algorithm`)with a non-constant seed if not already done:
...
For all these generators except `exro928ss` and `exsss` the lowest bit(s) has got a slightly less random behaviour than all other bits. 1 bit for `exrop` (and `exsp`), and 3 bits for `exs1024s`. See for example the explanation in the [Xoroshiro128+ ](http://xoroshiro.di.unimi.it/xoroshiro128plus.c)generator source code:
...
-doc "Returns the random number generator state in an external format. To be used with `seed/1`.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 18.0">>}).
...
-doc """
Seeds random number generation with the specifed algorithm and time-dependent data if `AlgOrStateOrExpState` is an algorithm. `Alg = default` is an alias for the [*default algorithm*.](`m:rand#default-algorithm`)
...
-doc "Seeds random number generation with the specified algorithm and integers in the process dictionary and returns the state. `Alg = default` is an alias for the [*default algorithm*.](`m:rand#default-algorithm`)".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 18.0,OTP 24.0">>}).
...
-doc "Seeds random number generation with the specified algorithm and integers and returns the state. `Alg = default` is an alias for the [*default algorithm*.](`m:rand#default-algorithm`)".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 18.0,OTP 24.0">>}).
...
-doc "Returns, for a specified integer `N >= 1`, a random integer uniformly distributed in the value range `1 =< X =< N` and updates the state in the process dictionary.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 18.0">>}).
...
-doc "Returns, for a specified integer `N >= 1` and a state, a random integer uniformly distributed in the value range `1 =< X =< N` and a new state.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 18.0">>}).
...
-doc "Returns, for a specified integer `N >= 0`, a `t:binary()` with that number of random bytes. Generates as many random numbers as required using the selected algorithm to compose the binary, and updates the state in the process dictionary accordingly.".
...
-doc "Returns, for a specified integer `N >= 0` and a state, a `t:binary()` with that number of random bytes, and a new state. Generates as many random numbers as required using the selected algorithm to compose the binary, and the new state.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 24.0">>}).
...
-doc "Returns a standard normal deviate float (that is, the mean is 0 and the standard deviation is 1) and updates the state in the process dictionary.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 18.0">>}).
-spec normal() -> float().
normal() ->
...
-doc "Returns a normal N(Mean, Variance) deviate float and updates the state in the process dictionary.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 20.0">>}).
...
-doc "Returns, for a specified state, a standard normal deviate float (that is, the mean is 0 and the standard deviation is 1) and a new state.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 18.0">>}).
...
-doc "Returns, for a specified state, a normal N(Mean, Variance) deviate float and a new state.".
-doc(#{title => <<"Plug-in framework API">>,since => <<"OTP 20.0">>}).
...
This is an API function into the internal implementation of the [`exsp`](`m:rand#algorithms`) algorithm that enables using it without the overhead of the plug-in framework, which might be useful for time critial applications. On a typical 64 bit Erlang VM this approach executes in just above 30% (1/3) of the time for the default algorithm through this module's normal plug-in framework.

To seed this generator use [`{_, AlgState} = rand:seed_s(exsp)` ](`seed_s/1`)or [`{_, AlgState} = rand:seed_s(exsp, Seed)` ](`seed_s/1`)with a specific `Seed`.

> #### Note {: class=info }
> This function offers no help in generating a number on a selected range, nor in generating a floating point number. It is easy to accidentally mess up the fairly good statistical properties of this generator when doing either. See the recepies at the start of this [Niche algorithms API ](`m:rand#niche-algorithms-api`)description. Note also the caveat about weak low bits that this generator suffers from. The generator is exported in this form primarily for performance.
""".
...
The quality of the output value improves much by using a scrambler instead of just taking the low bits. Function [`mwc59_value32` ](`mwc59_value32/1`)is a fast scrambler that returns a decent 32-bit number. The slightly slower [`mwc59_value` ](`mwc59_value/1`)scrambler returns 59 bits of very good quality, and [`mwc59_float`](`mwc59_float/1`) returns a `t:float()` of very good quality.

The low bits of the base generator are surprisingly good, so the lowest 16 bits actually pass fairly strict PRNG tests, despite the generator's weaknesses that lie in the high bits of the 32-bit MWC "digit". It is recommended to use `rem` on the the generator state, or bit mask extracting the lowest bits to produce numbers in a range 16 bits or less. See the recepies at the start of this [Niche algorithms API ](`m:rand#niche-algorithms-api`)description.

On a typical 64 bit Erlang VM this generator executes in below 8% (1/13) of the time for the default algorithm in the [plug-in framework API ](`m:rand#plug-in-framework-api`)of this module. With the [`mwc59_value32` ](`mwc59_value32/1`)scrambler the total time becomes 16% (1/6), and with [`mwc59_value` ](`mwc59_value/1`)it becomes 20% (1/5) of the time for the default algorithm. With [`mwc59_float`](`mwc59_float/1`) the total time is 60% of the time for the default algorithm generating a `t:float()`.
...
-doc """
Returns a 59-bit value `V` from a generator state `CX`. The generator state is scrambled using an 4-bit followed by a 27-bit xorshift, which masks the statistical imperfecions of the base generator [`mwc59`](`mwc59/1`) enough that all 59 bits are of very good quality.

Be careful to not accidentaly create a bignum when handling the value `V`.

It is in general general better to use the high bits from this scrambler than the low. See the recepies at the start of this [Niche algorithms API ](`m:rand#niche-algorithms-api`)description.
...
-doc "Returns the generator value `V` from a generator state `CX`, as a `t:float()`. The generator state is scrambled as with [`mwc59_value/1` ](`mwc59_value/1`)before converted to a `t:float()`.".
-doc(#{title => <<"Niche algorithms API">>,since => <<"OTP 25.0">>}).
...
Without `S`, the generator state is created as for [`seed_s(atom())`](`seed_s/1`).
""".
-doc(#{title => <<"Niche algorithms API">>,since => <<"OTP 25.0">>}).

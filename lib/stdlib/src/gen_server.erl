...
A `gen_server` process handles system messages as described in `m:sys`. The `sys` module can be used for debugging a `gen_server` process.
...
If the `gen_server` process needs to perform an action immediately after initialization or to break the execution of a callback into multiple steps, it can return `{continue,Continue}` in place of the time-out or hibernation value, which will immediately invoke the `c:handle_continue/2` callback.

If the `gen_server` process terminates, e.g. as a result of a function in the callback module returning `{stop,Reason,NewState}`, an exit signal with this `Reason` is sent to linked processes and ports. See [Processes](`p:system:ref_man_processes.md#errors`) in the Reference Manual for details regarding error handling using exit signals.
...
  | ignore  
 State = term()  
 Timeout = [timeout()](`t:erlang:timeout/0`)  
 Reason = term()  

Whenever a `gen_server` process is started using [`start/3,4`](`start/3`), [`start_monitor/3,4`](`start_monitor/3`), or [`start_link/3,4`](`start_link/3`), this function is called by the new process to initialize.
...
* __`{ok,_,Timeout}`  
  `{ok,_,hibernate}`  
  `{ok,_,{continue,Continue}}`__  
  See the corresponding return values from [`Module:handle_call/3` ](`c:handle_call/3`)for a description of this tuple member.
...
See function [`start_link/3,4`](`start_link/3`)'s return value [`start_ret()`](`t:start_ret/0`) in these different cases.
""".
...
Whenever a `gen_server` process receives a request sent using [`call/2,3`](`call/2`) or [`multi_call/2,3,4`](`multi_call/2`), this function is called to handle the request.
...
* __`{reply,Reply,NewState}`  
  `{reply,Reply,NewState,_}`__  
...
* __`{noreply,NewState}`  
  `{noreply,NewState,_}`__  
  The `gen_server` process continues executing with the possibly updated internal state `NewState`.

  A reply to the client request has to be created by calling [`reply(From, Reply)`](`reply/2`), either in this or in a later callback.

* __`{reply,_,_,Timeout}`  
  `{noreply,_,Timeout}`__  
  If an integer `Timeout` is provided, a time-out occurs unless a request or a message is received within that many milliseconds. A time-out is represented by the atom `timeout` to be handled by the [`Module:handle_info/2` ](`c:handle_info/2`)callback function. `Timeout =:= infinity` can be used to wait indefinitely, which is the same as returning a value without a `Timeout` member.

* __`{reply,_,_,hibernate}`  
  `{noreply,_,hibernate}`__  
  The process goes into hibernation waiting for the next message to arrive (by calling `proc_lib:hibernate/3`).

* __`{reply,_,_,{continue,Continue}}`  
  `{noreply,_,{continue,Continue}}`__  
  The process will execute the [`Module:handle_continue/2` ](`c:handle_continue/2`)callback function, with `Continue` as the first argument.

* __`{stop,Reason,NewState}`  
  `{stop,Reason,Reply,NewState}`__  
  The `gen_server` process will call [`Module:terminate(Reason,NewState)` ](`c:terminate/2`)and then terminate.
...
Whenever a `gen_server` process receives a request sent using `cast/2` or [`abcast/2,3`](`abcast/2`), this function is called to handle the request.

For a description of the arguments and possible return values, see [`Module:handle_call/3`](`c:handle_call/3`).
""".
-doc(#{title => <<"Callback Functions">>}).
...
For a description of the other arguments and possible return values, see [`Module:handle_call/3`](`c:handle_call/3`).
""".
-doc(#{title => <<"Callback Functions">>}).
...
This function is called by a `gen_server` process whenever a previous callback returns one of the tuples containing `{continue, Continue}`. `c:handle_continue/2` is invoked immediately after the previous callback, which makes it useful for performing work after initialization or for splitting the work in a callback in multiple steps, updating the process state along the way.
...
This function is called by a `gen_server` process when it is about to terminate. It is to be the opposite of [`Module:init/1`](`c:init/1`) and do any necessary cleaning up. When it returns, the `gen_server` process terminates with `Reason`. The return value is ignored.
...
Notice that for any other reason than `normal`, `shutdown`, or `{shutdown,Term}`, see `stop/3`, the `gen_server` process is assumed to terminate because of an error, and an error report is issued using `m:logger`.
...
> #### Note {: class=info }
> This callback is optional, so callback modules need not export it. If a release upgrade/downgrade with `Change={advanced,Extra}` specified in the `appup` file is made when `c:code_change/3` isn't implemented the process will crash with an `undef` exit reason.

This function is called by a `gen_server` process when it is to update its internal state during a release upgrade/downgrade, that is, when the instruction `{update,Module,Change,...}`, where `Change={advanced,Extra}`, is specifed in the `appup` file. For more information, see section [Release Handling Instructions](`p:system:release_handling.md#instr`) in OTP Design Principles.
...
* __`state`__ - The internal state of the `gen_server` process.

* __`message`__ - The message that caused the server to terminate.

* __`reason`__ - The reason that caused the server to terminate.
...
> This callback is optional, so callback modules need not export it. The `gen_server` module provides a default implementation of this function that returns the callback module state.
>
> If this callback is exported but fails, to hide possibly sensitive data, the default function will instead return the fact that `c:format_status/1` has crashed.

This function is called by a `gen_server` process in the following situations:

* [`sys:get_status/1,2`](`sys:get_status/1`) is invoked to get the `gen_server` status.
* The `gen_server` process terminates abnormally and logs an error.

This callback is used to limit the status of the process returned by [`sys:get_status/1,2`](`sys:get_status/1`) or sent to `m:logger`.
...
-doc "Destination, given to the `gen_server` as the first argument to the callback function [`Module:handle_call/3`](`c:handle_call/3`), to be used by the when replying through `reply/2` (instead of through the callback function's return value) to the process `Client` that has called the `gen_server` using [`call/2,3`](`call/2`). `Tag` is a term that is unique for this call/request instance.".
...
-doc """
Name specification to use when starting a `gen_server`. See functions [`start/3,4`](`start/3`), [`start_link/3,4`](`start_link/3`), [`start_monitor/3,4`](`start_monitor/3`), [`enter_loop/3,4,5`](`enter_loop/3`), and the type [`server_ref()`](`t:server_ref/0`) below.

* __`{local,LocalName}`__ - Register the `gen_server` locally as `LocalName` using [`register/2`](`erlang:register/2`).

* __`{global,GlobalName}`__ - Register the `gen_server` process id globally as `GlobalName` using `global:register_name/2`.

* __`{via,RegMod,ViaName}`__ - Register the `gen_server` process with the registry represented by `RegMod`. The `RegMod` callback is to export the functions `register_name/2`, `unregister_name/1`, `whereis_name/1`, and `send/2`, which are to behave like the corresponding functions in `m:global`. Thus, `{via,global,GlobalName}` is a valid reference equivalent to `{global,GlobalName}`.
""".
...
-doc """
Server specification to use when addressing a `gen_server`. See [`call/2,3`](`call/2`), `cast/2`, `send_request/2`, `check_response/2`, `wait_response/2`, [`stop/2,3`](`stop/1`) and the type [`server_name()`](`t:server_name/0`) above.

It can be:

* __`t:pid()`__ - The `gen_server`'s process identifier.

* __`LocalName`__ - The `gen_server` is locally registered as `LocalName` with [`register/2`](`erlang:register/2`).

* __`{Name,Node}`__ - The `gen_server` is locally registered on another node.

* __`{global,GlobalName}`__ - The `gen_server` is globally registered in `m:global`.

* __`{via,RegMod,ViaName}`__ - The `gen_server` is registered in an alternative process registry. The registry callback module `RegMod` is to export functions `register_name/2`, `unregister_name/1`, `whereis_name/1`, and `send/2`, which are to behave like the corresponding functions in `m:global`. Thus, `{via,global,GlobalName}` is the same as `{global,GlobalName}`.
""".
...
-doc """
Options that can be used when starting a `gen_server` server through, for example, [`start_link/3,4`](`start_link/3`).

* __`{timeout,Timeout}`__ - How many milliseconds the `gen_server` process is allowed to spend initializing or it is terminated and the start function returns `{error,timeout}`.

* __`{spawn_opt,SpawnOptions}`__ - The `SpawnOptions` option list is passed to the function used to spawn the `gen_server`; see [`spawn_opt/2`](`erlang:spawn_opt/2`).

  > #### Note {: class=info }
  > Using spawn option `monitor` is not allowed, it causes a `badarg` failure.

* __`t:enter_loop_opt()`__ - See the type [`enter_loop_opt()`](`t:enter_loop_opt/0`) below for more start options that are also allowed by [`enter_loop/3,4,5`](`enter_loop/3`).
""".
...
%%
-doc """
Options that can be used when starting a `gen_server` server through [`enter_loop/3-5`](`enter_loop/3`) or the start functions such as [`start_link/3,4`](`start_link/3`).

* __`{hibernate_after,HibernateAfterTimeout}`__ - Specifies that the `gen_server` process awaits any message for `HibernateAfterTimeout` milliseconds and if no message is received, the process goes into hibernation automatically (by calling `proc_lib:hibernate/3`).

* __`{debug,Dbgs}`__ - For every entry in `Dbgs`, the corresponding function in `m:sys` is called.
""".
...
-doc """
Return value from the [`start/3,4`](`start/3`) and [`start_link/3,4`](`start_link/3`) functions.

* __`{ok,Pid}`__ - The `gen_server` process was succesfully created and initialized, with the process identifier `Pid`.

* __`{error,{already_started,OtherPid}}`__ - A process with the specified `ServerName` exists already with the process identifier `OtherPid`. This `gen_server` was not started, or rather exited with reason `normal` before calling [`Module:init/1`](`c:init/1`).

* __`{error,timeout}`__ - The `gen_server` process failed to initialize since [`Module:init/1`](`c:init/1`) did not return within the [start timeout](`t:start_opt/0`). The `gen_server` process was killed with [`exit(_, kill)`](`erlang:exit/2`).

* __`ignore`__ - The `gen_server` process failed to initialize since [`Module:init/1`](`c:init/1`) returned `ignore`.

* __`{error,Reason}`__ - The `gen_server` process failed to initialize since [`Module:init/1`](`c:init/1`) returned `{stop,Reason}`, `{error,Reason}`, or it failed with reason `Reason`.

See [`Module:init/1`](`c:init/1`) about the exit reason for the `gen_server` process when it fails to initialize.
""".
...
-doc "Return value from the [`start_monitor/3,4`](`start_monitor/3`) functions. The same as type [`start_ret()`](`t:start_ret/0`) except that for a succesful start it returns both the process identifier `Pid` and a [`monitor/2,3`](`erlang:monitor/2`) `t:reference()` `MonRef`.".
...
The `gen_server` process calls [`Module:init/1`](`c:init/1`) to initialize. To ensure a synchronized startup procedure, `start_link/3,4` does not return until [`Module:init/1`](`c:init/1`) has returned or failed.

Using the argument `ServerName` creates a `gen_server` with a registered name. See type [`server_name()`](`t:server_name/0`) for different name registrations. If no `ServerName` is provided, the `gen_server` process is not registered.

`Module` is the name of the callback module.

`Args` is any term that is passed as the argument to [`Module:init/1`](`c:init/1`).

See type [`start_opt()`](`t:start_opt/0`) for `Options` when starting the `gen_server` process.

See type [`start_ret()`](`t:start_ret/0`) for a description this function's return values.
...
> #### Warning {: class=warning }
> Before OTP 26.0, if the started `gen_server` process returned e.g. `{stop,Reason}` from [`Module:init/1`](`c:init/1`), this function could return `{error,Reason}` *before* the started `gen_statem` process had terminated so starting again might fail because VM resources such as the registered name was not yet unregistered. An `'EXIT'` message could arrive later to the process calling this function.
>
> But if the started `gen_server` process instead failed during [`Module:init/1`](`c:init/1`), a process link `{'EXIT',Pid,Reason}` message caused this function to return `{error,Reason}` so the `'EXIT'` message had been consumed and the started `gen_statem` process had terminated.
>
...
The difference between returning `{stop,_}` and `{error,_}` from [`Module:init/1`](`c:init/1`), is that `{error,_}` results in a graceful ("silent") termination since the `gen_server` process exits with reason `normal`.
""".
-spec start_link(
	ServerName :: server_name(),
	Module     :: module(),
...
Other than that see [`start_link/3,4`](`start_link/3`). Note that the return value for a successful start differs in that it returns a monitor `reference`. See type [`start_mon_ret()`](`t:start_mon_ret/0`).
...
-doc """
Orders the generic server specified by `ServerRef` to exit with the specified `Reason`, default 'normal', and waits for it to terminate. The `gen_server` process calls [`Module:terminate/2` ](`c:terminate/2`)before exiting.

The function returns `ok` if the server terminates with the expected reason. Any other reason than `normal`, `shutdown`, or `{shutdown,Term}` causes an error report to be issued using `m:logger`. An exit signal with the same reason is sent to linked processes and ports.
...
-doc """
Makes a synchronous call to the `ServerRef` of the `gen_server` process by sending a request and waiting until a reply arrives or a time-out occurs. The `gen_server` process calls [`Module:handle_call/3` ](`c:handle_call/3`)to handle the request.

See also `ServerRef`'s type [`server_ref()`](`t:server_ref/0`).

`Request` is any term that is passed as the first argument to [`Module:handle_call/3`](`c:handle_call/3`).
...
The return value `Reply` is passed from the return value of [`Module:handle_call/3`](`c:handle_call/3`).
...
* __`timeout`__ - The call was aborted after waiting `Timeout` milliseconds for a reply, as described above.

* __`noproc`__ - The `ServerRef` refers to a server by name (it is not a `t:pid()`) and looking up the server process failed, or the `t:pid()` was already terminated.

* __`{nodedown,Node}`__ - The `ServerRef` refers to a server on the remote node `Node` and the connection to that node failed.

* __`calling_self`__ - A call to `self()` would hang indefinitely.
...
* __`_OtherTerm`__ - The server process exited during the call, with reason `Reason`. Either by returning `{stop,Reason,_}` from one of its callbacks (without replying to this call), by raising an exception, or due to getting an exit signal it did not trap.
""".
-spec call(
        ServerRef :: server_ref(),
        Request   :: term(),
...
The call `gen_server:receive_response(gen_server:send_request(ServerRef, Request), Timeout)` can be seen as equivalent to [`gen_server:call(ServerRef, Request, Timeout)`](`call/3`), ignoring the error handling.

The `gen_server` process calls [`Module:handle_call/3` ](`c:handle_call/3`)to handle the request.

See the type [`server_ref()`](`t:server_ref/0`) for the possible values for `ServerRef`.
...
The same as calling [`gen_server:reqids_add`](`reqids_add/3`)([`gen_server:send_request`](`send_request/2`)`(ServerRef, Request), Label, ReqIdCollection)`, but calling `send_request/4` is slightly more efficient.
""".
-doc(#{since => <<"OTP 25.0">>}).
...
-doc """
Sends an asynchronous request to the `ServerRef` of the `gen_server` process and returns `ok` immediately, ignoring if the destination node or `gen_server` process does not exist. The `gen_server` process calls [`Module:handle_cast/2` ](`c:handle_cast/2`)to handle the request.

See also `ServerRef`'s type [`server_ref()`](`t:server_ref/0`).

`Request` is any term that is passed as the first argument to [`Module:handle_cast/2`](`c:handle_cast/2`).
""".
-spec cast(
        ServerRef :: server_ref(),
        Request   :: term()
...
-doc """
This function can be used by a `gen_server` process to explicitly send a reply to a client that called [`call/2,3`](`call/2`) or [`multi_call/2,3,4`](`multi_call/2`), when the reply cannot be passed in the return value of [`Module:handle_call/3`](`c:handle_call/3`).
...
-doc """
Sends an asynchronous request to the `gen_server` processes locally registered as `Name` at the specified nodes. The function returns immediately and ignores nodes that do not exist, or where the `gen_server` `Name` does not exist. The `gen_server` processes call [`Module:handle_cast/2` ](`c:handle_cast/2`)to handle the request.
...
-doc """
Makes a synchronous call to all `gen_server` processes locally registered as `Name` at the specified nodes, by first sending the request to the nodes, and then waiting for the replies. The `gen_server` processes on the nodes call [`Module:handle_call/3` ](`c:handle_call/3`)to handle the request.
...
When a reply `Reply` is received from the `gen_server` process at a node `Node`, `{Node,Reply}` is added to `Replies`. `Reply` is passed from the return value of [`Module:handle_call/3`](`c:handle_call/3`).
...
`Module`, `Options`, and `ServerName` have the same meanings as when calling [`start[_link|_monitor]/3,4` ](`start_link/3`)or it can be `self()` for an anonymous server, which is the same as calling an `enter_loop/3,4` function without a `ServerName` argument. However, if `ServerName` is specified (and not as `self()`), the process must have been registered accordingly *before* this function is called.

`State`, `Timeout`, `Hibernate` and `Cont` have the same meanings as in the return value of [`Module:init/1`](`c:init/1`), which is *not* called when `enter_loop/3,4,5` is used. Note that to adhere to the [gen_server Behaviour ](`p:system:gen_server_concepts.md`)such a callback function needs to be defined, and it might as well be the one used when starting the `gen_server` process through `proc_lib`, and then be the one that calls `enter_loop/3,4,5`. But if such a [`Module:init/1`](`c:init/1`) function in for example error cases cannot call `enter_loop/3,4,5`, it should return a value that follows the type specification for [`Module:init/1`](`c:init/1`) such as `ignore`, although that value will be lost when returning to the spawning function.

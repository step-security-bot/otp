...
When a process that is started using `proc_lib` terminates abnormally (that is, with another exit reason than `normal`, `shutdown`, or `{shutdown,Term}`), a *crash report* is generated, which is written to terminal by the default logger handler setup by Kernel. For more information about how crash reports were logged prior to Erlang/OTP 21.0, see [SASL Error Logging](`p:sasl:error_logging.md`) in the SASL User's Guide.
...
%%
-doc "A restricted set of [spawn options](`t:spawn_option/0`). Most notably `monitor` is *not* part of these options.".
...
-doc "Spawns a new process and initializes it as described in the beginning of this manual page. The process is spawned using the [`spawn`](`erlang:spawn/1`) BIFs.".
...
-doc "Spawns a new process and initializes it as described in the beginning of this manual page. The process is spawned using the [`spawn_link`](`erlang:spawn_link/1`) BIFs.".
...
-doc "Spawns a new process and initializes it as described in the beginning of this manual page. The process is spawned using the [`erlang:spawn_opt`](`erlang:spawn_opt/2`) BIFs.".
...
-doc """
This function does the same as (and does call) the [`hibernate/3`](`erlang:hibernate/3`) BIF, but ensures that exception handling and logging continues to work as expected when the process wakes up.
...
To indicate a succesful start, the started process *must* call [`init_ack(Parent, Ret)`](`init_ack/2`) where `Parent` is the process that evaluates this function, or [`init_ack(Ret)`](`init_ack/1`). `Ret` is then returned by this function.

If the process fails to start, it *must* fail; preferably by calling [`init_fail(Parent, Ret, Exception)` ](`init_fail/3`)where `Parent` is the process that evaluates this function, or [`init_fail(Ret, Exception)`](`init_fail/2`). `Ret` is then returned by this function, and the started process fails with `Exception`.
...
Argument `SpawnOpts`, if specified, is passed as the last argument to the [`spawn_opt/2,3,4,5`](`erlang:spawn_opt/2`) BIF.

> #### Note {: class=info }
> Using spawn option `monitor` is not allowed. It causes the function to fail with reason `badarg`.
>
> Using spawn option `link` will set a link to the spawned process, just like [start_link/3,4,5](`start_link/3`).
""".
...
Besides setting a link on the spawned process this function behaves like [start/3,4,5](`start/3`).
...
Besides setting a monitor on the spawned process this function behaves like [start/3,4,5](`start/3`).
...
-doc """
This function must only be used by a process that has been started by a [`start[_link|_monitor]/3,4,5`](`start/3`) function. It tells `Parent` that the process has initialized itself and started.

Function `init_ack/1` uses the parent value previously stored by the start function used.

If neither this function nor [`init_fail/2,3`](`init_fail/2`) is called by the started process, the start function returns an error tuple when the started process exits, or when the start function time-out (if used) has passed, see [`start/3,4,5`](`start/3`).

> #### Warning {: class=warning }
> Do not use this function to return an error indicating that the process start failed. When doing so the start function can return before the failing process has exited, which may block VM resources required for a new start attempt to succeed. Use [`init_fail/2,3`](`init_fail/2`) for that purpose.
...
-doc """
Parent = [pid()](`t:erlang:pid/0`)  
Ret = [term()](`t:erlang:term/0`)  
Exception = \{Class, Reason\} | \{Class, Reason, Stacktrace\}  

This function must only be used by a process that has been started by a [`start[_link|_monitor]/3,4,5`](`start/3`) function. It tells `Parent` that the process has failed to initialize, and immediately raises an exception according to `Exception`. The start function then returns `Ret`.
...
> #### Warning {: class=warning }
> Do not consider catching the exception from this function. That would defeat its purpose. A process started by a [`start[_link|_monitor]/3,4,5`](`start/3`) function should end in a value (that will be ignored) or an exception that will be handled by this module. See [Description](`m:proc_lib`).

If neither this function nor [`init_ack/1,2`](`init_ack/1`) is called by the started process, the start function returns an error tuple when the started process exits, or when the start function time-out (if used) has passed, see [`start/3,4,5`](`start/3`).
...
-doc """
> #### Note {: class=info }
> This function is deprecated in the sense that the `error_logger` is no longer the preferred interface for logging in Erlang/OTP. A new [logging API](`p:kernel:logger_chapter.md`) was added in Erlang/OTP 21.0, but legacy `error_logger` handlers can still be used. New Logger handlers do not need to use this function, since the formatting callback (`report_cb`) is included as metadata in the log event.

This function can be used by a user-defined legacy `error_logger` event handler to format a crash report. The crash report is sent using `m:logger`, and the event to be handled is of the format `{error_report, GL, {Pid, crash_report, CrashReport}}`, where `GL` is the group leader pid of process `Pid` that sent the crash report.
""".
-doc(#{since => <<"OTP R16B">>}).
...
The implementation of this function is based on the `terminate` system message, and requires that the process handles system messages correctly. For information about system messages, see `m:sys` and section [sys and proc_lib](`p:system:spec_proc.md`) in OTP Design Principles.
""".
-doc(#{since => <<"OTP 18.0">>}).

...
This module provides a query interface to [Mnesia](`m:mnesia`), [ETS](`m:ets`), [Dets](`m:dets`), and other data structures that provide an iterator style traversal of objects.

## Overview

This module provides a query interface to *QLC tables*. Typical QLC tables are Mnesia, ETS, and Dets tables. Support is also provided for user-defined tables, see section [Implementing a QLC Table](`m:qlc#implementing_a_qlc_table`). [](){: id=query_list_comprehension }
A *query* is expressed using *Query List Comprehensions* (QLCs). The answers to a query are determined by data in QLC tables that fulfill the constraints expressed by the QLCs of the query. QLCs are similar to ordinary list comprehensions as described in [Erlang Reference Manual](`p:system:expressions.md#lcs`) and [Programming Examples](`p:system:list_comprehensions.md`), except that variables introduced in patterns cannot be used in list expressions. In the absence of optimizations and options such as `cache` and `unique` (see section [Common Options](`m:qlc#common_options`), every QLC free of QLC tables evaluates to the same list of answers as the identical ordinary list comprehension.

While ordinary list comprehensions evaluate to lists, calling [`q/1,2`](`q/1`) returns a *query handle*{: id=query_handle }. To obtain all the answers to a query, [`eval/1,2`](`eval/1`) is to be called with the query handle as first argument. Query handles are essentially functional objects (funs) created in the module calling `q/1,2`. As the funs refer to the module code, be careful not to keep query handles too long if the module code is to be replaced. Code replacement is described in section [Compilation and Code Loading](`p:system:code_loading.md`) in the Erlang Reference Manual. The list of answers can also be traversed in chunks by use of a *query cursor*{: id=query_cursor }. Query cursors are created by calling [`cursor/1,2`](`cursor/1`) with a query handle as first argument. Query cursors are essentially Erlang processes. One answer at a time is sent from the query cursor process to the process that created the cursor.
...
`Expression` (the *template*) is any Erlang expression. Qualifiers are either *filters* or *generators*. Filters are Erlang expressions returning `t:boolean()`. Generators have the form `Pattern <- ListExpression`, where `ListExpression` is an expression evaluating to a query handle or a list. Query handles are returned from [`append/1,2`](`append/1`), [`keysort/2,3`](`keysort/2`), [`q/1,2`](`q/1`), [`sort/1,2`](`sort/1`), [`string_to_handle/1,2,3`](`string_to_handle/1`), and `table/2`.
...
* `{max_list_size, MaxListSize}`{: id=max_list_size }, where `MaxListSize` is the size in bytes of terms on the external format. If the accumulated size of collected objects exceeds `MaxListSize`, the objects are written onto a temporary file. This option is used by option `{cache, list}` and by the merge join method. Defaults to 512*1024 bytes.
* `{tmpdir_usage, TmpFileUsage}` determines the action taken when `qlc` is about to create temporary files on the directory set by option `tmpdir`. If the value is `not_allowed`, an error tuple is returned, otherwise temporary files are created as needed. Default is `allowed`, which means that no further action is taken. The values `info_msg`, `warning_msg`, and `error_msg` mean that the function with the corresponding name in module `m:error_logger` is called for printing some information (currently the stacktrace).
...
As mentioned earlier, queries are expressed in the list comprehension syntax as described in section [Expressions](`p:system:expressions.md`) in Erlang Reference Manual. In the following, some familiarity with list comprehensions is assumed. The examples in section [List Comprehensions](`p:system:list_comprehensions.md`) in Programming Examples can get you started. Notice that list comprehensions do not add any computational power to the language; anything that can be done with list comprehensions can also be done without them. But they add syntax for expressing simple search problems, which is compact and clear once you get used to it.

Many list comprehension expressions can be evaluated by the `qlc` module. Exceptions are expressions, such that variables introduced in patterns (or filters) are used in some generator later in the list comprehension. As an example, consider an implementation of `lists:append(L)`: `[X ||Y <- L, X <- Y]`. `Y` is introduced in the first generator and used in the second. The ordinary list comprehension is normally to be preferred when there is a choice as to which to use. One difference is that [`eval/1,2`](`eval/1`) collects answers in a list that is finally reversed, while list comprehensions collect answers on the stack that is finally unwound.

What the `qlc` module primarily adds to list comprehensions is that data can be read from QLC tables in small chunks. A QLC table is created by calling [`qlc:table/2`](`table/2`). Usually `qlc:table/2` is not called directly from the query but through an interface function of some data structure. Erlang/OTP includes a few examples of such functions: [`mnesia:table/1,2`](`mnesia:table/1`), [`ets:table/1,2`](`ets:table/1`), and [`dets:table/1,2`](`dets:table/1`). For a given data structure, many functions can create QLC tables, but common for these functions is that they return a query handle created by [`qlc:table/2`](`table/2`). Using the QLC tables provided by Erlang/OTP is usually probably sufficient, but for the more advanced user section [Implementing a QLC Table](`m:qlc#implementing_a_qlc_table`) describes the implementation of a function calling `qlc:table/2`.

Besides `qlc:table/2`, other functions return query handles. They are used more seldom than tables, but are sometimes useful. [`qlc:append/1,2`](`append/1`) traverses objects from many tables or lists after each other. If, for example, you want to traverse all answers to a query `QH` and then finish off by a term `{finished}`, you can do that by calling `qlc:append(QH, [{finished}])`. `append/2` first returns all objects of `QH`, then `{finished}`. If a tuple `{finished}` exists among the answers to `QH`, it is returned twice from `append/2`.
...
As an example of how to use function `table/2`, the implementation of a QLC table for the `m:gb_trees` module is given:
...
`m:dets`, `m:erl_eval`, `m:erlang`, `m:error_logger`, `m:ets`, `m:file`, `m:file_sorter`, `m:mnesia`, `m:shell`, [Erlang Reference Manual](`p:system:index.html`), [Programming Examples](`p:system:index.html`)
""".
...
-doc "".
-type(answer() :: term()).
-doc "Parse trees for Erlang expression, see section [The Abstract Format](`p:erts:absform.md`) in the ERTS User's Guide.".
-type(abstract_expr() :: erl_parse:abstract_expr()).
-doc "Match specification, see section [Match Specifications in Erlang](`p:erts:match_spec.md`) in the ERTS User's Guide and `m:ms_transform`.".
...
-doc "Returns a query handle. When evaluating query handle `QH`, all answers to the first query handle in `QHL` are returned, followed by all answers to the remaining query handles in `QHL`.".
...
-doc """
Creates a query cursor and makes the calling process the owner of the cursor. The cursor is to be used as argument to [`next_answers/1,2`](`next_answers/1`) and (eventually) `delete_cursor/1`. Calls `erlang:spawn_opt/2` to spawn and link to a process that evaluates the query handle. The value of option `spawn_options` is used as last argument when calling `spawn_opt/2`. Defaults to `[link]`.
...
-doc "Returns a descriptive string in English of an error tuple returned by some of the functions of the `qlc` module or the parse transform. This function is mainly used by the compiler invoking the parse transform.".
...
When calling `qlc:q/1,2` from the Erlang shell, the parse transform is automatically called. When this occurs, the fun substituted for the QLC is not compiled but is evaluated by `m:erl_eval`. This is also true when expressions are evaluated by `file:eval/1,2` or in the debugger.
...
[`sort/1,2`](`sort/1`) and [`keysort/2,3`](`keysort/2`) can also be used for caching answers and for removing duplicates. When sorting answers are cached in a list, possibly stored on a temporary file, and no ETS tables are used.

Sometimes (see `table/2`) traversal of tables can be done by looking up key values, which is assumed to be fast. Under certain (rare) circumstances there can be too many key values to look up. [](){: id=max_lookup }
Option `{max_lookup, MaxLookup}` can then be used to limit the number of lookups: if more than `MaxLookup` lookups would be required, no lookups are done but the table is traversed instead. Defaults to `infinity`, which means that there is no limit on the number of keys to look up.

*Example:*

In the following example, using the `gb_table` module from section [Implementing a QLC Table](`m:qlc#implementing_a_qlc_table`), there are six keys to look up: `{1,a}`, `{1,b}`, `{1,c}`, `{2,a}`, `{2,b}`, and `{2,c}`. The reason is that the two elements of key `{X, Y}` are compared separately.
...
-doc """
A string version of [`q/1,2`](`q/1`). When the query handle is evaluated, the fun created by the parse transform is interpreted by `m:erl_eval`. The query string is to be one single QLC terminated by a period.
...
> #### Note {: class=info }
> Query handles created this way may have worse performance than when created directly via [`q/1,2`](`q/1`).
""".
...
* Callback function `TraverseFun` is used for traversing the table. It is to return a list of objects terminated by either `[]` or a nullary fun to be used for traversing the not yet traversed objects of the table. Any other return value is immediately returned as value of the query evaluation. Unary `TraverseFun`s are to accept a match specification as argument. The match specification is created by the parse transform by analyzing the pattern of the generator calling `qlc:table/2` and filters using variables introduced in the pattern. If the parse transform cannot find a match specification equivalent to the pattern and filters, `TraverseFun` is called with a match specification returning every object.
  ...
  * Other modules can provide a nullary `TraverseFun`. An example is [`gb_table:table/1`](`m:qlc#gb_table`) in section [Implementing a QLC Table](`m:qlc#implementing_a_qlc_table`).
* Unary callback function `PreFun` is called once before the table is read for the first time. If the call fails, the query evaluation fails.
  ...
  * The value of `parent_value` is the value returned by `ParentFun`, or `undefined` if there is no `ParentFun`. `ParentFun` is called once just before the call of `PreFun` in the context of the process calling [`eval/1,2`](`eval/1`), [`fold/3,4`](`fold/3`), or [`cursor/1,2`](`cursor/1`).
...
* Unary callback function `InfoFun` is to return information about the table. `undefined` is to be returned if the value of some tag is unknown:
  ...
  * __`indices`__ - Returns a list of indexed positions, a list of positive integers.

  * __`is_unique_objects`__ - Returns `true` if the objects returned by `TraverseFun` are unique.

  * __`keypos`__ - Returns the position of the table key, a positive integer.

  * __`is_sorted_key`__ - Returns `true` if the objects returned by `TraverseFun` are sorted on the key.

  * __`num_of_objects`__ - Returns the number of objects in the table, a non-negative integer.

  
* Unary callback function `FormatFun` is used by [`info/1,2`](`info/1`) for displaying the call that created the query handle of the table. Defaults to `undefined`, which means that `info/1,2` displays a call to `'$MOD':'$FUN'/0`. It is up to `FormatFun` to present the selected objects of the table in a suitable way. However, if a character list is chosen for presentation, it must be an Erlang expression that can be scanned and parsed (a trailing dot is added by `info/1,2` though).
  ...
  * __`{lookup, Position, Keys, NElements, DepthFun}`.__ - `LookupFun` is used for looking up objects in the table.

  * __`{match_spec, MatchExpression}`__ - No way of finding all possible answers by looking up keys was found, but the filters could be transformed into a match specification. All answers are found by calling `TraverseFun(MatchExpression)`.

  * __`{all, NElements, DepthFun}`__ - No optimization was found. A match specification matching all objects is used if `TraverseFun` is unary.
...
For the various options recognized by `table/1,2` in respective module, see [`ets(3)`](`ets:table/1`), [`dets(3)`](`dets:table/1`), and [`mnesia(3)`](`mnesia:table/1`).
""".

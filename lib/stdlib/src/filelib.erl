...
* __?__ - Matches one character.

* __*__ - Matches any number of characters up to the end of the filename, the next dot, or the next slash.

* __\**__ - Two adjacent `*` used as a single pattern match all files and zero or more directories and subdirectories.

* __\[Character1,Character2,...]__ - Matches any of the characters listed. Two characters separated by a hyphen match a range of characters. Example: `[A-Z]` matches any uppercase letter.

* __\{Item,...\}__ - Alternation. Matches one of the alternatives.
...
-doc "Same as `wildcard/1`, except that `Cwd` is used instead of the working directory.".
...
-doc """
Folds function `Fun` over all (regular) files `F` in directory `Dir` whose basename (for example, just `"baz.erl"` in `"foo/bar/baz.erl"`) matches the regular expression `RegExp` (for a description of the allowed regular expressions, see the `m:re` module). If `Recursive` is `true`, all subdirectories to `Dir` are processed. The regular expression matching is only done on the filename without the directory part.
...
-doc "Returns the date and time the specified file or directory was last modified, or `0` if the file does not exist.".
...
If `Rules` is left out or is an empty list, the default system rules are used. See also the Kernel application parameter [`source_search_rules`](`p:kernel:kernel_app.md#source_search_rules`).
""".
-doc(#{since => <<"OTP 20.0">>}).
...
-doc "Equivalent to `find_source(Base, Dir)`, where `Dir` is `filename:dirname(FilePath)` and `Base` is `filename:basename(FilePath)`.".
-doc(#{since => <<"OTP 20.0">>}).

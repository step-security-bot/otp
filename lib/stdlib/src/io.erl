...
All functions in this module have an optional parameter [`IoDevice`](`t:device/0`). If included, it must be the pid of a process that handles the I/O protocols. Normally, it is a `IoDevice` returned by `file:open/2`. If no [`IoDevice`](`t:device/0`) is given, [`standard_io`](`t:standard_io/0`) is used.

For a description of the I/O protocols, see section [The Erlang I/O Protocol](io_protocol.md) in the User's Guide.

> #### Warning {: class=warning }
> As from Erlang/OTP R13A, data supplied to function `put_chars/2` is to be in the [`unicode:chardata()`](`t:unicode:chardata/0`) format. This means that programs supplying binaries to this function must convert them to UTF-8 before trying to output the data on an I/O device.
>
> If an I/O device is set in binary mode, functions [`get_chars/2,3`](`get_chars/2`) and [`get_line/1,2`](`get_line/1`) can return binaries instead of lists. The binaries are, as from Erlang/OTP R13A, encoded in UTF-8.
>
> To work with binaries in ISO Latin-1 encoding, use the `m:file` module instead.
>
> For conversion functions between character encodings, see the `m:unicode` module.
...
By default all I/O sent to `standard_io` will en up in the [`user`](`t:user/0`) I/O device of the node that spawned the calling process.

`standard_io` is an alias for [`group_leader/0`](`erlang:group_leader/0`), so in order to change where the default input/output requests are sent you can change the group leader for the current process using [`group_leader(NewGroupLeader, self())`](`erlang:group_leader/2`).
""".
...
See [The Interactive Shell](unicode_usage.md#the-interactive-shell) and [Escripts and non-interactive I/O](unicode_usage.md#escripts-and-non-interactive-i-o) in the Using Unicode In Erlang User's Guide for details on how Unicode is handled by `user`.
""".
-type user() :: user.
-doc "An I/O device, either `standard_io`, `standard_error`, `user`, a registered name, or a pid handling I/O protocols (returned from `file:open/2`).".
...
-doc "Writes the characters of `CharData` to the I/O server (`IoDevice`). If you want to write latin1 encoded bytes to the I/O server you should use `file:write/2` instead.".
...
-doc "Retrieves the number of columns of the `IoDevice` (that is, the width of a terminal). The function succeeds for terminal devices and returns `{error, enotsup}` for all other I/O devices.".
...
-doc "Retrieves the number of rows of `IoDevice` (that is, the height of a terminal). The function only succeeds for terminal devices, for all other I/O devices the function returns `{error, enotsup}`.".
...
* __`Data`__ - The input characters. If the I/O device supports Unicode, the data can represent codepoints > 255 (the `latin1` range). If the I/O server is set to deliver binaries, they are encoded in UTF-8 (regardless of whether the I/O device supports Unicode). If you want the data to be returned as a latin1 encoded binary you should use `file:read/2` instead.

* __`eof`__ - End of file was encountered.

* __`{error, ErrorDescription}`__ - Other (rare) error condition, such as `{error, estale}` if reading from an NFS file system.
""".
...
* __`Data`__ - The characters in the line terminated by a line feed (or end of file). If the I/O device supports Unicode, the data can represent codepoints > 255 (the `latin1` range). If the I/O server is set to deliver binaries, they are encoded in UTF-8 (regardless of if the I/O device supports Unicode). If you want the data to be returned as a latin1 encoded binary you should use `file:read_line/1` instead.
...
The `terminal` option is read only and indicates whether the output stream is a terminal or not. When it is a terminal, most systems that Erlang runs on allows the use of [ANSI escape codes](https://en.wikipedia.org/wiki/ANSI_escape_code) to control what the terminal outputs.
...
* __`binary`, `list`, or `{binary, boolean()}`__ - If set in binary mode (`binary` or `{binary, true}`), the I/O server sends binary data (encoded in UTF-8) as answers to the `get_line`, `get_chars`, and, if possible, `get_until` requests (for details, see section [The Erlang I/O Protocol](io_protocol.md)) in the User's Guide). The immediate effect is that [`get_chars/2,3`](`get_chars/2`) and [`get_line/1,2`](`get_line/1`) return UTF-8 binaries instead of lists of characters for the affected I/O device.
...
* __`{echo, boolean()}`__ - Denotes if the terminal is to echo input. Only supported for the standard shell I/O server (`group.erl`)

* __`{expand_fun, expand_fun()}`__ - Provides a function for tab-completion (expansion) like the Erlang shell. This function is called when the user presses the *Tab* key. The expansion is active when calling line-reading functions, such as [`get_line/1,2`](`get_line/1`).

  The function is called with the current line, up to the cursor, as a reversed string. It is to return a three-tuple: `{yes|no, string(), list()}`. The first element gives a beep if `no`, otherwise the expansion is silent; the second is a string that will be entered at the cursor position; the third is a list of possible expansions. If this list is not empty, it is printed below the current input line. The list of possible expansions can be formatted in different ways to make more advanced expansion suggestions more readable to the user, see `edlin_expand:expand/2` for documentation of that.
...
* __`{encoding, latin1 | unicode}`__ - Specifies how characters are input or output from or to the I/O device, implying that, for example, a terminal is set to handle Unicode input and output or a file is set to handle UTF-8 data encoding.
  ...
  > #### Note {: class=info }
  > Prior to OTP 26.0, when Erlang was started with the `-oldshell` or `-noshell` flags (for example, in an `escript`), the default encoding for [`standard_io`](`t:standard_io/0`) was set to `latin1`, meaning that any characters > codepoint 255 were escaped and that input was expected to be plain 8-bit ISO Latin-1. As of OTP 26.0, [`standard_io`](`t:standard_io/0`) always defaults to `unicode` if its supported, otherwise `latin1`.
  >
  > If you want to send raw bytes on [`standard_io`](`t:standard_io/0`), you now always need to explicitly set the encoding to `latin1`; otherwise, code points 128-255 will be converted to UTF-8. This is best done by setting the kernel configuration parameter [standard_io_encoding](`p:kernel:kernel_app.md#standard_io_encoding`) to `latin1`.
...
* __`{encoding, utf8 | utf16 | utf32 | {utf16,big} | {utf16,little} | {utf32,big} | {utf32,little}}`__ - For disk files, the encoding can be set to various UTF variants. This has the effect that data is expected to be read as the specified encoding from the file, and the data is written in the specified encoding to the disk file.
...
* __`{ok, Term}`__ - The parsing was successful.

* __`eof`__ - End of file was encountered.

* __`{error, ErrorInfo}`__ - The parsing failed.
...
* __`{ok, Term, EndLocation}`__ - The parsing was successful.

* __`{eof, EndLocation}`__ - End of file was encountered.

* __`{error, ErrorInfo, ErrorLocation}`__ - The parsing failed.
...
* `Mod` is the control sequence modifier. This is one or more characters that change the interpretation of `Data`.
  ...
  * __`t`__ - For Unicode translation.

  * __`l`__ - For stopping `p` and `P` from detecting printable characters.

  * __`k`__ - For use with `p`, `P`, `w`, and `W` to format maps in map-key `ordered` order (see [maps:iterator_order()](`t:maps:iterator_order/0`)).

  * __`K`__ - Similar to `k`, for formatting maps in map-key order, but takes an extra argument that specifies the [maps:iterator_order()](`t:maps:iterator_order/0`).
...
* __`~`__ - Character `~` is written.

* __`c`__ - The argument is a number that is interpreted as an ASCII code. The precision is the number of times the character is printed and defaults to the field width, which in turn defaults to 1. Example:
...
* __`f`__ - The argument is a float that is written as `[-]ddd.ddd`, where the precision is the number of digits after the decimal point. The default precision is 6 and it cannot be < 1.

* __`e`__ - The argument is a float that is written as `[-]d.ddde+-ddd`, where the precision is the number of digits written. The default precision is 6 and it cannot be < 2.

* __`g`__ - The argument is a float that is written as `f`, if it is >= 0.1 and < 10000.0. Otherwise, it is written in the `e` format. The precision is the number of significant digits. It defaults to 6 and is not to be < 2. If the absolute value of the float does not allow it to be written in the `f` format with the desired number of significant digits, it is also written in the `e` format.

* __`s`__ - Prints the argument with the string syntax. The argument is, if no Unicode translation modifier is present, an `t:iolist()`, a `t:binary()`, or an `t:atom()`. If the Unicode translation modifier (`t`) is in effect, the argument is `t:unicode:chardata()`, meaning that binaries are in UTF-8. The characters are printed without quotes. The string is first truncated by the specified precision and then padded and justified to the specified field width. The default precision is the field width.
...
* __`p`__ - Writes the data with standard syntax in the same way as `~w`, but breaks terms whose printed representation is longer than one line into many lines and indents each line sensibly. Left-justification is not supported. It also tries to detect flat lists of printable characters and output these as strings. For example:
  ...
  The field width specifies the maximum line length. It defaults to 80. The precision specifies the initial indentation of the term. It defaults to the number of characters printed on this line in the *same* call to `write/1` or [`format/1,2,3`](`format/1`). For example, using `T` above:
...
* __`W`__ - Writes data in the same way as `~w`, but takes an extra argument that is the maximum depth to which terms are printed. Anything below this depth is replaced with `...`. For example, using `T` above:
...
* __`P`__ - Writes data in the same way as `~p`, but takes an extra argument that is the maximum depth to which terms are printed. Anything below this depth is replaced with `...`, for example:
...
* __`B`__ - Writes an integer in base 2-36, the default base is 10. A leading dash is printed for negative integers.
...
* __`X`__ - Like `B`, but takes an extra argument that is a prefix to insert before the number, but after the leading dash, if any.
...
* __`#`__ - Like `B`, but prints the number with an Erlang style `#`\-separated base prefix. Example:
...
* __`b`__ - Like `B`, but prints lowercase letters.

* __`x`__ - Like `X`, but prints lowercase letters.

* __`+`__ - Like `#`, but prints lowercase letters.

* __`n`__ - Writes a new line.

* __`i`__ - Ignores the next term.

The function returns:

* __`ok`__ - The formatting succeeded.
...
* Control sequences, which have the general format `~*FMC`, where:
  ...
  * __`~`__ - A single `~` is expected in the input.

  * __`d`__ - A decimal integer is expected.

  * __`u`__ - An unsigned integer in base 2-36 is expected. The field width parameter is used to specify base. Leading whitespace characters are not skipped.

  * __`-`__ - An optional sign character is expected. A sign character `-` gives return value `-1`. Sign character `+` or none gives `1`. The field width parameter is ignored. Leading whitespace characters are not skipped.

  * __`#`__ - An integer in base 2-36 with Erlang-style base prefix (for example, `"16#ffff"`) is expected.

  * __`f`__ - A floating point number is expected. It must follow the Erlang floating point number syntax.

  * __`s`__ - A string of non-whitespace characters is read. If a field width has been specified, this number of characters are read and all trailing whitespace characters are stripped. An Erlang string (list of characters) is returned.
  ...
  * __`a`__ - Similar to `s`, but the resulting string is converted into an atom.

  * __`c`__ - The number of characters equal to the field width are read (default is 1) and returned as an Erlang string. However, leading and trailing whitespace characters are not omitted as they are with `s`. All characters are returned.
  ...
  * __`l`__ - Returns the number of characters that have been scanned up to that point, including whitespace characters.

  The function returns:

  * __`{ok, Terms}`__ - The read was successful and `Terms` is the list of successfully matched and read items.

  * __`eof`__ - End of file was encountered.

  * __`{error, FreadError}`__ - The reading failed and `FreadError` gives a hint about the error.

  * __`{error, ErrorDescription}`__ - The read operation failed and parameter `ErrorDescription` gives a hint about the error.
...
* __`{ok, Tokens, EndLocation}`__ - The tokenization succeeded.

* __`{eof, EndLocation}`__ - End of file was encountered by the tokenizer.

* __`eof`__ - End of file was encountered by the I/O server.

* __`{error, ErrorInfo, ErrorLocation}`__ - An error occurred while tokenizing.
...
The return values are the same as for [`scan_erl_exprs/1,2,3,4`](`scan_erl_exprs/1`).
""".
-doc(#{since => <<"OTP R16B">>}).
...
Reads data from the standard input (`IoDevice`), prompting it with `Prompt`. Starts reading at location `StartLocation` (`1`). Argument `Options` is passed on as argument `Options` of function `erl_scan:tokens/4`. The data is tokenized and parsed as if it was a sequence of Erlang expressions until a final dot (`.`) is reached.

The function returns:

* __`{ok, ExprList, EndLocation}`__ - The parsing was successful.

* __`{eof, EndLocation}`__ - End of file was encountered by the tokenizer.

* __`eof`__ - End of file was encountered by the I/O server.

* __`{error, ErrorInfo, ErrorLocation}`__ - An error occurred while tokenizing or parsing.
...
* __`{ok, AbsForm, EndLocation}`__ - The parsing was successful.

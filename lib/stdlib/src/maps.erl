...
This module contains functions for maps processing. The Efficiency Guide contains a chapter that describes [how to use maps efficiently](`p:system:maps.md`).
""".
...
Created using [`maps:iterator/1`](`iterator/1`) or [`maps:iterator/2`](`iterator/2`).
...
-doc """
Key-based iterator order option that can be one of `undefined` (default for [`maps:iterator/1`](`iterator/1`)), `ordered` (sorted in map-key order), `reversed`, or a custom sorting function.

Used by [`maps:iterator/2`](`iterator/2`).

The [Expressions section](`p:system:expressions.md#term-comparisons`) contains descriptions of how terms are ordered.
""".
...
-doc """
Returns a map iterator `Iterator` that can be used by [`maps:next/1`](`next/1`) to traverse the key-value associations in a map. When iterating over a map, the memory usage is guaranteed to be bounded no matter the size of the map.
...
-doc """
Returns a map iterator `Iterator` that can be used by [`maps:next/1`](`next/1`) to traverse the key-value associations in a map sorted by key using the given `Order`.

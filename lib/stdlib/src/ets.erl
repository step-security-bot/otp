...
* Two Erlang terms `match` if they are of the same type and have the same value, so that `1` matches `1`, but not `1.0` (as `1.0` is a `t:float()` and not an `t:integer()`).
...
* The `ordered_set` works on the *Erlang term order* and no defined order exists between an `t:integer()` and a `t:float()` that extends to the same value. Hence the key `1` and the key `1.0` are regarded as equal in an `ordered_set` table.
...
* __`badarg`__ - If any argument has the wrong format.

* __`badarg`__ - If the table identifier is invalid.

* __`badarg`__ - If the operation is denied because of table access rights ([protected](`m:ets#protected`) or [private](`m:ets#private`)).

* __`system_limit`__ - Modification of a value causes it to not be representable internally in the VM. For example, incrementation of a counter past the largest integer representable.

* __`system_limit`__ - If a match specification passed as argument has excessive nesting which causes scheduler stack exhaustion for the scheduler that the calling process is executing on. [Scheduler stack size](`p:erts:erl_cmd.md#sched_thread_stack_size`) can be configured when starting the runtime system.
...
* Search with simple *match patterns*, using [`match/1/2/3`](`match/1`), `match_delete/2` and [`match_object/1/2/3`](`match_object/1`).
* Search with more powerful *match specifications*, using [`select/1/2/3`](`select/1`), `select_count/2`, `select_delete/2`, `select_replace/2` and [`select_reverse/1/2/3`](`select_reverse/1`).
* *Table conversions*, using [`tab2file/2/3`](`tab2file/2`) and `tab2list/1`.
...
>
> Effects like this are improbable but possible. The probability will further be reduced (if not vanish) if table option [`write_concurrency`](`m:ets#new_2_write_concurrency`) is not enabled. This can also only be a potential concern for `ordered_set` where the traversal order is defined.
...
Some of the functions use a *match specification*, `match_spec`. For a brief explanation, see `select/2`. For a detailed description, see section [Match Specifications in Erlang](`p:erts:match_spec.md`) in ERTS User's Guide.

A match specifications with excessive nesting will cause a [`system_limit`](`m:ets#ets_failures`) error exception to be raised.
""".
...
-doc "Opaque continuation used by [`select/1,3`](`select/1`), [`select_reverse/1,3`](`select_reverse/1`), [`match/1,3`](`match/1`), and [`match_object/1,3`](`match_object/1`).".
...
-doc "Deletes all objects with key `Key` from table `Table`. This function succeeds even if no objects with key `Key` exist.".
...
-doc "Delete all objects in the ETS table `Table`. The operation is guaranteed to be [atomic and isolated](`m:ets#concurrency`).".
...
-doc "Delete the exact object `Object` from the ETS table, leaving objects with the same key but other differences (useful for type `bag`). In a `duplicate_bag` table, all instances of the object are deleted.".
...
Notice that this function does not affect option [`heir`](`m:ets#heir`) of the table. A table owner can, for example, set `heir` to itself, give the table away, and then get it back if the receiver terminates.
""".
...
* __`{compressed, boolean()}`__ - Indicates if the table is compressed.

* __`{decentralized_counters, boolean()}`__ - Indicates whether the table uses `decentralized_counters`.

* __`{heir, pid() | none}`__ - The pid of the heir of the table, or `none` if no heir is set.

* __`{id,`[ `tid()`](`t:tid/0`)`}`__ - The table identifier.

* __`{keypos, integer() >= 1}`__ - The key position.

* __`{memory, integer() >= 0}`__ - The number of words allocated to the table.

* __`{name, atom()}`__ - The table name.

* __`{named_table, boolean()}`__ - Indicates if the table is named.

* __`{node, node()}`__ - The node where the table is stored. This field is no longer meaningful, as tables cannot be accessed from other nodes.

* __`{owner, pid()}`__ - The pid of the owner of the table.

* __`{protection,` [`access()`](`t:table_access/0`)`}`__ - The table access rights.

* __`{size, integer() >= 0}`__ - The number of objects inserted in the table.

* __`{type,` [`type()`](`t:table_type/0`)`}`__ - The table type.

* __`{read_concurrency, boolean()}`__ - Indicates whether the table uses `read_concurrency` or not.

* __`{write_concurrency, WriteConcurrencyAlternative}`__ - Indicates which `write_concurrency` option the table uses.

> #### Note {: class=info }
> The execution time of this function is affected by the [`decentralized_counters`](`m:ets#new_2_decentralized_counters`) table option. The execution time is much longer when the `decentralized_counters` option is set to `true` than when the `decentralized_counters` option is set to `false`.
""".
...
* [](){: id=info_2_safe_fixed_monotonic_time }
  ...
  * __`safe_fixed`__ - `FixationTime` corresponds to the result returned by `erlang:timestamp/0` at the time of fixation. Notice that when the system uses single or multi [time warp modes](`p:erts:time_correction.md#time_warp_modes`) this can produce strange results, as the use of `safe_fixed` is not [time warp safe](`p:erts:time_correction.md#time_warp_safe_code`). Time warp safe code must use `safe_fixed_monotonic_time` instead.

  * __`safe_fixed_monotonic_time`__ - `FixationTime` corresponds to the result returned by `erlang:monotonic_time/0` at the time of fixation. The use of `safe_fixed_monotonic_time` is [time warp safe](`p:erts:time_correction.md#time_warp_safe_code`).
  ...
  Table fixations are not limited to `safe_fixtable/2`. Temporary fixations may also be done by for example [traversing functions](`m:ets#traversal`) like `select` and `match`. Such table fixations are automatically released before the corresponding functions returns, but they may be seen by a concurrent call to `ets:info(T,safe_fixed|safe_fixed_monotonic_time)`.
...
The entire operation is guaranteed to be [atomic and isolated](`m:ets#concurrency`), even when a list of objects is inserted.

[](){: id=insert_list_order }
For `bag` and `duplicate_bag`, objects in the list with identical keys will be inserted in list order (from head to tail). That is, a subsequent call to [`lookup(T,Key)`](`lookup/2`) will return them in that inserted order.
...
If `ObjectOrObjects` is a list, the function checks *every* key before inserting anything. Nothing is inserted unless *all* keys present in the list are absent from the table. Like `insert/2`, the entire operation is guaranteed to be [atomic and isolated](`m:ets#concurrency`).
""".
...
-doc """
Checks if a term represent a valid compiled [match specification](`m:ets#match_spec`). A compiled match specification is only valid on the Erlang node where it was compiled by calling `match_spec_compile/1`.
...
As an example, one can insert an object with `t:integer()` `1` as a key in an `ordered_set` and get the object returned as a result of doing a `lookup/2` with `t:float()` `1.0` as the key to search for.
...
Notice that the sequential order of object insertions is preserved; the first object inserted with the specified key is the first in the resulting list, and so on. See also the note about [list insertion order](`m:ets#insert_list_order`).
""".
...
Use `safe_fixtable/2` to guarantee [safe traversal](`m:ets#traversal`) for subsequent calls to `match/1`.
""".
...
Use `safe_fixtable/2` to guarantee [safe traversal](`m:ets#traversal`) for subsequent calls to `match_object/1`.
""".
...
-doc """
Transforms a [match specification](`m:ets#match_spec`) into an internal representation that can be used in subsequent calls to `match_spec_run/2`. The internal representation is opaque. To check the validity of a compiled match specification, use `is_compiled_ms/1`.

If term `MatchSpec` does not represent a valid match specification, a `badarg` exception is raised.

> #### Note {: class=info }
> This function has limited use in normal code. It is used by the `m:dets` module to perform the `dets:select/1` operations.
""".
...
-doc "Works like `lookup/2`, but does not return the objects. Returns `true` if one or more elements in the table has key `Key`, otherwise `false`.".
...
* __`set`__ - The table is a `set` table: one key, one object, no order among objects. This is the default table type.

* __`ordered_set`__ - The table is a `ordered_set` table: one key, one object, ordered in Erlang term order, which is the order implied by the < and > operators. Tables of this type have a somewhat different behavior in some situations than tables of other types. Most notably, the `ordered_set` tables regard keys as equal when they *compare equal*, not only when they match. This means that to an `ordered_set` table, `t:integer()` `1` and `t:float()` `1.0` are regarded as equal. This also means that the key used to lookup an element does not necessarily *match* the key in the returned elements, if `t:float()`'s and `t:integer()`'s are mixed in keys of a table.

* __`bag`__ - The table is a `bag` table, which can have many objects, but only one instance of each object, per key.

* __`duplicate_bag`__ - The table is a `duplicate_bag` table, which can have many objects, including multiple copies of the same object, per key.

* __`public`__ - Any process can read or write to the table.

  [](){: id=protected }

* __`protected`__ - The owner process can read and write to the table. Other processes can only read the table. This is the default setting for the access rights.

  [](){: id=private }

* __`private`__ - Only the owner process can read or write to the table.

* __`named_table`__ - If this option is present, the table is registered under its `Name` which can then be used instead of the table identifier in subsequent operations.
...
* __`{keypos,Pos}`__ - Specifies which element in the stored tuples to use as key. By default, it is the first element, that is, `Pos=1`. However, this is not always appropriate. In particular, we do not want the first element to be the key if we want to store Erlang records in a table.

  Notice that any tuple stored in the table must have at least `Pos` number of elements.

  [](){: id=heir }

* __`{heir,Pid,HeirData} | {heir,none}`__ - Set a process as heir. The heir inherits the table if the owner terminates. Message `{'ETS-TRANSFER',tid(),FromPid,HeirData}` is sent to the heir when that occurs. The heir must be a local process. Default heir is `none`, which destroys the table when the owner terminates.

  [](){: id=new_2_write_concurrency }

* __`{write_concurrency,WriteConcurrencyAlternative}`__ - Performance tuning. Defaults to `false`, in which case an operation that mutates (writes to) the table obtains exclusive access, blocking any concurrent access of the same table until finished. If set to `true`, the table is optimized for concurrent write access. Different objects of the same table can be mutated (and read) by concurrent processes. This is achieved to some degree at the expense of memory consumption and the performance of sequential access and concurrent reading.
  ...
  The `write_concurrency` option can be combined with the options [`read_concurrency`](`m:ets#new_2_read_concurrency`) and [`decentralized_counters`](`m:ets#new_2_decentralized_counters`). You typically want to combine `write_concurrency` with `read_concurrency` when large concurrent read bursts and large concurrent write bursts are common; for more information, see option [`read_concurrency`](`m:ets#new_2_read_concurrency`). It is almost always a good idea to combine the `write_concurrency` option with the [`decentralized_counters`](`m:ets#new_2_decentralized_counters`) option.

  Notice that this option does not change any guarantees about [atomicity and isolation](`m:ets#concurrency`). Functions that makes such promises over many objects (like `insert/2`) gain less (or nothing) from this option.
...
* __`{read_concurrency,boolean()}`__(Since OTP R14B)  
  ...
  Option `read_concurrency` can be combined with option [`write_concurrency`](`m:ets#new_2_write_concurrency`). You typically want to combine these when large concurrent read bursts and large concurrent write bursts are common.

  [](){: id=new_2_decentralized_counters }

* __`{decentralized_counters,boolean()}`__(Since OTP 23.0)  
  ...
  When this option is enabled the counters for the table size and memory consumption are distributed over several cache lines and the scheduling threads are mapped to one of those cache lines. The `erl` option [`+dcg`](`p:erts:erl_cmd.md#+dcg`) can be used to control the number of cache lines that the counters are distributed over.

  [](){: id=new_2_compressed }

* __`compressed`__(Since OTP R14B01)  
...
-doc "Renames the named table `Table` to the new name `Name`. Afterwards, the old name cannot be used to access the table. Renaming an unnamed table has no effect.".
...
-doc """
Fixes a table of type `set`, `bag`, or `duplicate_bag` for [safe traversal](`m:ets#traversal`) using `first/1` & `next/2`, `match/3` & `match/1`, `match_object/3` & `match_object/1`, or `select/3` & `select/1`.
...
To retrieve information about which processes have fixed which tables, use [`info(Table, safe_fixed_monotonic_time)`](`m:ets#info_2_safe_fixed_monotonic_time`). A system with many processes fixing tables can need a monitor that sends alarms when tables have been fixed for too long.
...
-doc """
Matches the objects in table `Table` using a [match specification](`m:ets#match_spec`). This is a more general call than `match/2` and `match_object/2` calls. In its simplest form, the match specification is as follows:
...
This syntax is equivalent to the syntax used in the trace patterns (see the `m:dbg`) module in Runtime_Tools.

The `Guard`s are constructed as tuples, where the first element is the test name and the remaining elements are the test parameters. To check for a specific type (say a list) of the element bound to the match variable `'$1'`, one would write the test as `{is_list, '$1'}`. If the test fails, the object in the table does not match and the next `MatchFunction` (if any) is tried. Most guard tests present in Erlang can be used, but only the new versions prefixed `is_` are allowed (`is_float`, `is_atom`, and so on).
...
Use `safe_fixtable/2` to guarantee [safe traversal](`m:ets#traversal`) for subsequent calls to `select/1`.
""".
...
-doc """
Matches the objects in table `Table` using a [match specification](`m:ets#match_spec`). If the match specification returns `true` for an object, that object considered a match and is counted. For any other result from the match specification the object is not considered a match and is therefore not counted.
...
-doc """
Matches the objects in the table `Table` using a [match specification](`m:ets#match_spec`). For each matched object, the existing object is replaced with the match specification result.

The match-and-replace operation for each individual object is guaranteed to be [atomic and isolated](`m:ets#concurrency`). The `select_replace` table traversal as a whole, like all other select functions, does not give such guarantees.
...
-doc "Works like `select/2`, but returns the list in reverse order for table type `ordered_set`. For all other table types, the return value is identical to that of `select/2`.".
-doc(#{since => <<"OTP R14B">>}).
...
-doc "Sets table options. The only allowed option to be set after the table has been created is [`heir`](`m:ets#heir`). The calling process must be the table owner.".
...
-doc """
This function provides an efficient way to update one or more counters, without the trouble of having to look up an object, update the object by incrementing an element, and insert the resulting object into the table again. The operation is guaranteed to be [atomic and isolated](`m:ets#concurrency`).
...
-doc """
This function returns the [`tid()`](`t:tid/0`) of the named table identified by `TableName`, or `undefined` if no such table exists. The `t:tid()` can be used in place of the table name in all operations, which is slightly faster since the name does not have to be resolved on each call.

If the table is deleted, the `t:tid()` will be invalid even if another named table is created with the same name.
""".
-doc(#{since => <<"OTP 21.0">>}).
...
-doc """
Executes the matching specified in a compiled [match specification](`m:ets#match_spec`) on a list of terms. Term `CompiledMatchSpec` is to be the result of a call to `match_spec_compile/1` and is hence the internal representation of the match specification one wants to use.
...
> #### Note {: class=info }
> This function has limited use in normal code. It is used by the `m:dets` module to perform the `dets:select/1` operations and by Mnesia during transactions.
""".
...
-doc """
Pseudo function that by a `parse_transform` translates `LiteralFun` typed as parameter in the function call to a [match specification](`m:ets#match_spec`). With "literal" is meant that the fun must textually be written as the parameter of the function, it cannot be held in a variable that in turn is passed to the function.
...
For more information, see [`ms_transform(3)`](`m:ms_transform#top`).
""".
...
-doc "Fills an already created/opened Dets table with the objects in the already opened ETS table named `Table`. The Dets table is emptied before the objects are inserted.".
...
-doc """
This function is a utility to test a [match specification](`m:ets#match_spec`) used in calls to `select/2`. The function both tests `MatchSpec` for "syntactic" correctness and runs the match specification against object `Tuple`.
...
-doc "Deletes all objects that match pattern `Pattern` from table `Table`. For a description of patterns, see `match/2`.".
...
* __`object_count`__ - The number of objects written to the file is noted in the file footer, so file truncation can be verified even if the file was updated during dump.

* __`md5sum`__ - The header and objects in the file are checksummed using the built-in MD5 functions. The MD5 sum of all objects is written in the file footer, so that verification while reading detects the slightest bitflip in the file data. Using this costs a fair amount of CPU time.
...
* __`name`__ - The name of the dumped table. If the table was a named table, a table with the same name cannot exist when the table is loaded from file with `file2tab/2`. If the table is not saved as a named table, this field has no significance when loading the table from file.

* __`type`__ - The ETS type of the dumped table (that is, `set`, `bag`, `duplicate_bag`, or `ordered_set`). This type is used when loading the table again.

* __`protection`__ - The protection of the dumped table (that is, `private`, `protected`, or `public`). A table loaded from the file gets the same protection.

* __`named_table`__ - `true` if the table was a named table when dumped to file, otherwise `false`. Notice that when a named table is loaded from a file, there cannot exist a table in the system with the same name.

* __`keypos`__ - The `keypos` of the table dumped to file, which is used when loading the table again.

* __`size`__ - The number of objects in the table when the table dump to file started. For a `public` table, this number does not need to correspond to the number of objects saved to the file, as objects can have been added or deleted by another process during table dump.

* __`extended_info`__ - The extended information written in the file footer to allow stronger verification during table loading from file, as specified to `tab2file/3`. Notice that this function only tells *which* information is present, not the values in the file footer. The value is a list containing one or more of the atoms `object_count` and `md5sum`.

* __`version`__ - A tuple `{Major,Minor}` containing the major and minor version of the file format for ETS table dumps. This version field was added beginning with STDLIB 1.5.1. Files dumped with older versions return `{0,0}` in this field.
...
-doc """
Returns a Query List Comprehension (QLC) query handle. The `m:qlc` module provides a query language aimed mainly at Mnesia, but ETS tables, Dets tables, and lists are also recognized by QLC as sources of data. Calling `table/1,2` is the means to make the ETS table `Table` usable to QLC.
...
* __`first_next`__ - The table is traversed one key at a time by calling `first/1` and `next/2`.

* __`last_prev`__ - The table is traversed one key at a time by calling `last/1` and `prev/2`.

* __`select`__ - The table is traversed by calling `select/3` and `select/1`. Option `n_objects` determines the number of objects returned (the third argument of `select/3`); the default is to return `100` objects at a time. The [match specification](`m:ets#match_spec`) (the second argument of `select/3`) is assembled by QLC: simple filters are translated into equivalent match specifications while more complicated filters must be applied to all objects returned by `select/3` given a match specification that matches all objects.

* __`{select, MatchSpec}`__ - As for `select`, the table is traversed by calling `select/3` and `select/1`. The difference is that the match specification is explicitly specified. This is how to state match specifications that cannot easily be expressed within the syntax provided by QLC.

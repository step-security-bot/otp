...
The slave nodes are started with the `m:slave` module. This effects terminal I/O, file I/O, and code loading.
...
-doc """
Starts a new pool. The file `.hosts.erlang` is read to find host names where the pool nodes can be started; see section [Files](`m:pool#files`). The startup procedure fails if the file is not found.

The slave nodes are started with [`slave:start/2,3`](`slave:start/2`), passing along `Name` and, if provided, `Args`. `Name` is used as the first part of the node names, `Args` is used to specify command-line arguments.
...
-doc "Ensures that a pool master is running and includes `Node` in the pool master's pool of nodes.".
...
-doc "Spawns a process on the pool node that is expected to have the lowest future load.".
...
-doc "Spawns and links to a process on the pool node that is expected to have the lowest future load.".

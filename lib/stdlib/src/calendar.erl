...
-doc "Year cannot be abbreviated. For example, 93 denotes year 93, not 1993. The valid range depends on the underlying operating system. The date tuple must denote a valid date.".
...
-doc """
> #### Note {: class=info }
> The `native` time unit was added to `t:rfc3339_time_unit()` in OTP 25.0.
""".
...
%% January 1st.
%%
-doc "Computes the number of gregorian seconds starting with year 0 and ending at the specified date and time.".
...
%%
-doc "Computes the day of the week from the specified `Year`, `Month`, and `Day`. Returns the day of the week as `1`: Monday, `2`: Tuesday, and so on.".
...
%%
-doc "Returns tuple `{Year, WeekNum}` representing the ISO week number for the actual date. To determine the actual date, use function `local_time/0`.".
...
%%
-doc "Returns tuple `{Year, WeekNum}` representing the ISO week number for the specified date.".
-doc(#{since => <<"OTP R14B02">>}).
...
""".
-doc(#{deprecated =>
           <<"calendar:local_time_to_universal_time/1 is deprecated; use calendar:local_time_to_universal_time_dst/1 instead">>}).
...
* __`[]`__ - For a local `{Date1, Time1}` during the period that is skipped when switching *to* daylight saving time, there is no corresponding UTC, as the local time is illegal (it has never occured).

* __`[DstDateTimeUTC, DateTimeUTC]`__ - For a local `{Date1, Time1}` during the period that is repeated when switching *from* daylight saving time, two corresponding UTCs exist; one for the first instance of the period when daylight saving time is still active, and one for the second instance.

* __`[DateTimeUTC]`__ - For all other local times only one corresponding UTC exists.
""".
...
%% Returns: {date(), time()}, date() = {Y, M, D}, time() = {H, M, S}.
%% 
-doc "Returns Universal Coordinated Time (UTC) converted from the return value from `erlang:timestamp/0`.".
...
%% Args: Now = now()
%%
-doc "Returns local date and time converted from the return value from `erlang:timestamp/0`.".
...
Converts an RFC 3339 timestamp into system time. The data format of RFC 3339 timestamps is described by [RFC 3339](https://www.ietf.org/rfc/rfc3339.txt). Starting from OTP 25.1, the minutes part of the time zone is optional.

Valid option:

* __`{unit, Unit}`__ - The time unit of the return value. The default is `second`.
...
%%
-doc "Converts a specified number of seconds into days, hours, minutes, and seconds. `Time` is always non-negative, but `Days` is negative if argument `Seconds` is.".
...
Converts a system time into an RFC 3339 timestamp. The data format of RFC 3339 timestamps is described by [RFC 3339](https://www.ietf.org/rfc/rfc3339.txt). The data format of offsets is also described by RFC 3339.

Valid options:

* __`{offset, Offset}`__ - The offset, either a string or an integer, to be included in the formatted string. An empty string, which is the default, is interpreted as local time. A non-empty string is included as is. The time unit of the integer is the same as the one of `Time`.

* __`{time_designator, Character}`__ - The character used as time designator, that is, the date and time separator. The default is `$T`.

* __`{unit, Unit}`__ - The time unit of `Time`. The default is `second`. If some other unit is given (`millisecond`, `microsecond`, `nanosecond`, or `native`), the formatted string includes a fraction of a second. The number of fractional second digits is three, six, or nine depending on what time unit is chosen. For `native` three fractional digits are included. Notice that trailing zeros are not removed from the fraction.
...
%%
%% Returns: {date(), time()}, date() = {Y, M, D}, time() = {H, M, S}.
-doc "Returns the Universal Coordinated Time (UTC) reported by the underlying operating system. Returns local time if universal time is unavailable.".
...
%%
-doc "Converts from Universal Coordinated Time (UTC) to local time. `DateTime` must refer to a date after Jan 1, 1970.".

...
* __`{port_policy, Policy}`__ - `Policy = random | Port | {range, MinPort, MaxPort}`
...
* __`{udp, Options}`__ - `Options = [Opt]`, see [gen_udp:open/2](`gen_udp:open/1`).

* __`{use_tsize, Bool}`__ - `Bool = bool()`
...
* __`{TftpKey, TftpVal}`__ - `TftpKey = string()`  
  `TftpVal = string()`

  Name and value of a TFTP option.

* __`{reject, Feature}`__ - `Feature = Mode | TftpKey`  
  ` Mode = read | write`  
  ` TftpKey = string()`
...
* __`{callback, {RegExp, Module, State}}`__ - `RegExp = string()`  
  `Module = atom()`  
  `State = term()`
  ...
  The callback module must implement the `tftp` behavior, see [CALLBACK FUNCTIONS](`m:tftp#tftp_callback`).

* __`{logger, Module}`__ - `Module = module()`

  Callback module for customized logging of errors, warnings, and info messages. The callback module must implement the `m:tftp_logger` behavior. The default module is `tftp_logger`.

* __`{max_retries, MaxRetries}`__ - `MaxRetries = int()`

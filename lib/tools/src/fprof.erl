...
* __`1`__ - Tracing; to file, as mentioned in the previous paragraph. The trace contains entries for function calls, returns to function, process scheduling, other process related (spawn, etc) events, and garbage collection. All trace entries are timestamped.

* __`2`__ - Profiling; the trace file is read, the execution call stack is simulated, and raw profile data is calculated from the simulated call stack and the trace timestamps. The profile data is stored in the `fprof` server state. During this step the trace data may be dumped in text format to file or console.

* __`3`__ - Analysing; the raw profile data is sorted, filtered and dumped in text format either to file or console. The text format intended to be both readable for a human reader, as well as parsable with the standard erlang parsing tools.
...
`PidSpec` and `Tracer` are used in calls to `erlang:trace(PidSpec, true, [{tracer, Tracer} | Flags])`, and `Filename` is used to call `dbg:trace_port(file, Filename)`. Please see `erlang:trace/3` and `dbg:trace_port/2`.

Option description:

* __`stop`__ - Stops a running `fprof` trace and clears all tracing from the node. Either option `stop` or `start` must be specified, but not both.

* __`start`__ - Clears all tracing from the node and starts a new `fprof` trace. Either option `start` or `stop` must be specified, but not both.

* __`verbose` | `{verbose, boolean()}`__ - The options `verbose` or `{verbose, true}` adds some trace flags that `fprof` does not need, but that may be interesting for general debugging purposes. This option is only allowed with the `start` option.

* __`cpu_time` | `{cpu_time, boolean()}`__ - The options `cpu_time` or `{cpu_time, true}` makes the timestamps in the trace be in CPU time instead of wallclock time which is the default. This option is only allowed with the `start` option.
...
* __`{procs, PidSpec}` | `{procs, [PidSpec]}`__ - Specifies which processes that shall be traced. If this option is not given, the calling process is traced. All processes spawned by the traced processes are also traced. This option is only allowed with the `start` option.

* __`file` | `{file, Filename}`__ - Specifies the filename of the trace. If the option `file` is given, or none of these options are given, the file `"fprof.trace"` is used. This option is only allowed with the `start` option, but not with the `{tracer, Tracer}` option.

* __`{tracer, Tracer}`__ - Specifies that trace to process or port shall be done instead of trace to file. This option is only allowed with the `start` option, but not with the `{file, Filename}` option.
""".
...
-doc "Same as `profile([{OptionName, OptionValue}])`.".
...
`Dumpfile` is used to call `file:open/2`, and `Filename` is used to call `dbg:trace_port(file, Filename)`. Please see `file:open/2` and `dbg:trace_port/2`.

Option description:

* __`file` | `{file, Filename}`__ - Reads the file `Filename` and creates raw profile data that is stored in RAM by the `fprof` server. If the option `file` is given, or none of these options are given, the file `"fprof.trace"` is read. The call will return when the whole trace has been read with the return value `ok` if successful. This option is not allowed with the `start` or `stop` options.

* __`dump` | `{dump, Dump}`__ - Specifies the destination for the trace text dump. If this option is not given, no dump is generated, if it is `dump` the destination will be the caller's group leader, otherwise the destination `Dump` is either the pid of an I/O device or a filename. And, finally, if the filename is `[]` \- `"fprof.dump"` is used instead. This option is not allowed with the `stop` option.

* __`append`__ - Causes the trace text dump to be appended to the destination file. This option is only allowed with the `{dump, Dumpfile}` option.

* __`start`__ - Starts a tracer process that profiles trace data in runtime. The call will return immediately with the return value `{ok, Tracer}` if successful. This option is not allowed with the `stop`, `file` or `{file, Filename}` options.

* __`stop`__ - Stops the tracer process that profiles trace data in runtime. The return value will be value `ok` if successful. This option is not allowed with the `start`, `file` or `{file, Filename}` options.
""".
...
-doc "Same as `analyse([{OptionName, OptionValue}])`.".
...
* __`dest` | `{dest, Dest}`__ - Specifies the destination for the analysis. If this option is not given or it is `dest`, the destination will be the caller's group leader, otherwise the destination `Dest` is either the `t:pid()` of an I/O device or a filename. And, finally, if the filename is `[]` \- `"fprof.analysis"` is used instead.

* __`append`__ - Causes the analysis to be appended to the destination file. This option is only allowed with the `{dest, Destfile}` option.

* __`{cols, Cols}`__ - Specifies the number of columns in the analysis text. If this option is not given the number of columns is set to 80.

* __`callers` | `{callers, true}`__ - Prints callers and called information in the analysis. This is the default.

* __`{callers, false}` | `no_callers`__ - Suppresses the printing of callers and called information in the analysis.

* __`{sort, SortSpec}`__ - Specifies if the analysis should be sorted according to the ACC column, which is the default, or the OWN column. See [Analysis Format](`m:fprof#analysis`) below.

* __`totals` | `{totals, true}`__ - Includes a section containing call statistics for all calls regardless of process, in the analysis.

* __`{totals, false}`__ - Suppresses the totals section in the analysis, which is the default.

* __`details` | `{details, true}`__ - Prints call statistics for each process in the analysis. This is the default.

* __`{details, false}` | `no_details`__ - Suppresses the call statistics for each process from the analysis.
""".

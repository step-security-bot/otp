...
Return the number of matching functions that can have call count breakpoints, the same as [`start/0..3`](`start/0`) with the same arguments would have returned.
""".
...
-doc """
```erlang
-type func_analysis_list() ::
          [{mfa(), FuncCallCount :: non_neg_integer()}].
```
```erlang
-type mod_analysis() ::
          {Mod :: module(),
...
* __`FuncAnalysisList`__ - A list of tuples, one for each function in a module, in decreasing `FuncCallCount` order.

* __`ModCallCount`__ - The sum of `FuncCallCount` values for all functions in module `Mod`.

* __`AllCallCount`__ - The sum of `ModCallCount` values for all modules concerned in `ModAnalysisList`.

* __`ModAnalysisList`__ - A list of tuples, one for each module except `cprof`, in decreasing `ModCallCount` order.

If call counters are still running while `analyse/0..2` is executing, you might get an inconsistent result. This happens if the process executing `analyse/0..2` gets scheduled out so some other process can increment the counters that are being analysed, Calling [`pause()`](`pause/0`) before analysing takes care of the problem.

If the `Mod` argument is given, the result contains a `ModAnalysis` tuple for module `Mod` only, otherwise the result contains one `ModAnalysis` tuple for all modules returned from [`code:all_loaded()`](`code:all_loaded/0`) except `cprof` itself.

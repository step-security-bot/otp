...
[`make:all/0,1`](`all/0`) and [`make:files/1,2`](`files/1`) first looks for `{emake, Emake}` in options, then in the current working directory for a file named `Emakefile`. If present `Emake` should contain elements like this:
...
-doc """
`files/1,2` does exactly the same thing as [`all/0,1`](`all/0`) but for the specified `ModFiles`, which is a list of module or file names. The file extension `.erl` may be omitted.

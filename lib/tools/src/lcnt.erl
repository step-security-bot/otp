...
-doc "Starts the lock profiler server. The server only act as a medium for the user and performs filtering and printing of data collected by `lcnt:collect/1`.".
...
This will fail if the `copy_save` option is enabled; see [`lcnt:rt_opt/2`](`rt_opt/2`).
...
* __`{copy_save, boolean()}`__ - Retains the statistics of destroyed locks.  
  Default: `false`
...
* __`{process_locks, boolean()}`__ - Profile process locks, equal to adding `process` to the lock category mask; see `lcnt:rt_mask/2`  
  Default: `true`
""".
-doc(#{title => <<"Internal runtime lock counter controllers">>,
       since => <<"OTP R13B04">>}).
...
-doc "Same as `rt_opt(node(), {Type, Value})`.".
-doc(#{title => <<"Internal runtime lock counter controllers">>,
       since => <<"OTP R13B04">>}).
...
-doc "Same as `clear(node())`.".
-doc(#{since => <<"OTP R13B04">>}).
-spec clear() -> 'ok'.

clear() -> rt_clear().

-doc "Clears the internal lock statistics from the runtime system. This does not clear the data on the server only on runtime system. All counters for static locks are zeroed, all dynamic locks currently alive are zeroed and all saved locks now destroyed are removed. It also resets the duration timer.".
-doc(#{since => <<"OTP R13B04">>}).
...
-doc "Same as `collect(node())`.".
-doc(#{since => <<"OTP R13B04">>}).
-spec collect() -> 'ok'.

collect() -> call({collect, rt_collect()}).

-doc "Collects lock statistics from the runtime system. The function starts a server if it is not already started. It then populates the server with lock statistics. If the server held any lock statistics data before the collect then that data is lost.".
-doc(#{since => <<"OTP R13B04">>}).
...
Lock `Name` and `Id` for ports and processes are interchangeable with the use of [`lcnt:swap_pid_keys/0`](`swap_pid_keys/0`) and is the reason why `t:pid()` and `t:port()` options can be used in both `Name` and `Id` space. Both pids and ports are special identifiers with stripped creation and can be recreated with [`lcnt:pid/2,3`](`pid/3`) and [`lcnt:port/1,2`](`port/2`).

Option description:

* __`{combine, boolean()}`__ - Combine the statistics from different instances of a lock class.  
  Default: `true`

* __`{locations, boolean()}`__ - Print the statistics by source file and line numbers.  
  Default: `false`

* __`{max_locks, MaxLocks}`__ - Maximum number of locks printed or no limit with `none`.  
  Default: `20`

* __`{print, PrintOptions}`__ - Printing options:
  * __`name`__ - Named lock or named set of locks (classes). The same name used for initializing the lock in the VM.

  * __`id`__ - Internal id for set of locks, not always unique. This could be table name for ets tables (db_tab), port id for ports, integer identifiers for allocators, etc.

  * __`type`__ - Type of lock: `rw_mutex`, `mutex`, `spinlock`, `rw_spinlock` or `proclock`.

  * __`entry`__ - In combination with `{locations, true}` this option prints the lock operations source file and line number entry-points along with statistics for each entry.

  * __`tries`__ - Number of acquisitions of this lock.

  * __`colls`__ - Number of collisions when a thread tried to acquire this lock. This is when a trylock is EBUSY, a write try on read held rw_lock, a try read on write held rw_lock, a thread tries to lock an already locked lock. (Internal states supervises this).

  * __`ratio`__ - The ratio between the number of collisions and the number of tries (acquisitions) in percentage.

  * __`time`__ - Accumulated waiting time for this lock. This could be greater than actual wall clock time, it is accumulated for all threads. Trylock conflicts does not accumulate time.

  * __`duration`__ - Percentage of accumulated waiting time of wall clock time. This percentage can be higher than 100% since accumulated time is from all threads.

  Default: `[name,id,tries,colls,ratio,time,duration]`

* __`{reverse, boolean()}`__ - Reverses the order of sorting.  
  Default: `false`

* __`{sort, Sort}`__ - Column sorting orders.  
  Default: `time`

* __`{thresholds, Thresholds}`__ - Filtering thresholds. Anything values above the threshold value are passed through.  
...
-doc "Prints lcnt server state and generic information about collected lock statistics.".
-doc(#{since => <<"OTP R13B04">>}).
-spec information() -> 'ok'.
...
-doc "Same as `apply(fun() -> erlang:apply(Module, Function, Args) end)`.".
...
-doc "Same as `pid(node(), Id, Serial)`.".
-doc(#{title => <<"Convenience functions">>,since => <<"OTP R13B04">>}).

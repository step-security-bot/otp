...
overhead(Config) ->
    ...
    {FProfDrvTime,{ok,sofs_copy,_}} = fprof:apply(timer, tc, [TC], [{file_drv, LogFile}]),
    ct:pal("FProfDrv: ~p Norm: ~p Ratio: ~p",[FProfDrvTime, NormTime, NormTime / FProfDrvTime * 100]),
    ...
    ct_event:notify(#event{name = benchmark_data,
                           data = [{name, fprof_drv_overhead},
                                   {value, NormTime / FProfDrvTime * 100}]}),
    ct_event:notify(#event{name = benchmark_data,

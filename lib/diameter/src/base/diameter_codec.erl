...
Incoming Diameter messages are decoded from binary() before being communicated to `m:diameter_app` callbacks. Similarly, outgoing Diameter messages are encoded into binary() before being passed to the appropriate `m:diameter_transport` module for transmission. The functions documented here implement the default encode/decode.

> #### Warning {: class=warning }
> The diameter user does not need to call functions here explicitly when sending and receiving messages using `diameter:call/4` and the callback interface documented in `m:diameter_app`: diameter itself provides encode/decode as a consequence of configuration passed to `diameter:start_service/2`, and the results may differ from those returned by the functions documented here, depending on configuration.

The [header()](`m:diameter_codec#header`) and [packet()](`m:diameter_codec#packet`) records below are defined in diameter.hrl, which can be included as follows.
...
* __`uint8()  = 0..255`{: id=integers }__  

* __`uint24() = 0..16777215`__  

* __`uint32() = 0..4294967295`__ - 8-bit, 24-bit and 32-bit integers occurring in Diameter and AVP headers.

* __`avp() = #diameter_avp{}`{: id=avp }__ - The application-neutral representation of an AVP. Primarily intended for use by relay applications that need to handle arbitrary Diameter applications. A service implementing a specific Diameter application (for which it configures a dictionary) can manipulate values of type [message()](`m:diameter_codec#message`) instead.

  Fields have the following types.

  * __`code = uint32()`__  

  * __`is_mandatory = boolean()`__  

  * __`need_encryption = boolean()`__  

  * __`vendor_id = uint32() | undefined`__ - Values in the AVP header, corresponding to AVP Code, the M flag, P flags and Vendor-ID respectively. A Vendor-ID other than `undefined` implies a set V flag.

  * __`data = iolist()`__ - The data bytes of the AVP.

  * __`name = atom()`__ - The name of the AVP as defined in the dictionary file in question, or `undefined` if the AVP is unknown to the dictionary file in question.

  * __`value = term()`__ - The decoded value of an AVP. Will be `undefined` on decode if the data bytes could not be decoded, the AVP is unknown, or if the [decode format](`m:diameter#decode_format`) is `none`. The type of a decoded value is as document in [diameter_dict(4)](diameter_dict.md#data_types).

  * __`type = atom()`__ - The type of the AVP as specified in the dictionary file in question (or one it inherits). Possible types are `undefined` and the Diameter types: `OctetString`, `Integer32`, `Integer64`, `Unsigned32`, `Unsigned64`, `Float32`, `Float64`, `Grouped`, `Enumerated`, `Address`, `Time`, `UTF8String`, `DiameterIdentity`, `DiameterURI`, `IPFilterRule` and `QoSFilterRule`.

  

* __`dictionary() = module()`{: id=dictionary }__ - The name of a generated dictionary module as generated by [diameterc(1)](diameterc_cmd.md) or `diameter_make:codec/2`. The interface provided by a dictionary module is an implementation detail that may change.

* __`header() = #diameter_header{}`{: id=header }__ - The record representation of the Diameter header. Values in a [packet()](`m:diameter_codec#packet`) returned by `decode/2` are as extracted from the incoming message. Values set in an [packet()](`m:diameter_codec#packet`) passed to `encode/2` are preserved in the encoded binary(), with the exception of `length`, `cmd_code` and `application_id`, all of which are determined by the [dictionary()](`m:diameter_codec#dictionary`) in question.
  ...
  * __`cmd_code = uint24()`__  

  * __`application_id = uint32()`__  

  * __`hop_by_hop_id = uint32()`__  

  * __`end_to_end_id = uint32()`__ - Values of the Version, Message Length, Command-Code, Application-ID, Hop-by-Hop Identifier and End-to-End Identifier fields of the Diameter header.

  * __`is_request = boolean()`__  

  * __`is_proxiable = boolean()`__  

  * __`is_error = boolean()`__  

  * __`is_retransmitted = boolean()`__ - Values corresponding to the R(equest), P(roxiable), E(rror) and T(Potentially re-transmitted message) flags of the Diameter header.

  

* __`message() = record() | maybe_improper_list()`{: id=message }__ - The representation of a Diameter message as passed to `diameter:call/4` or returned from a [handle_request/3](`c:diameter_app:handle_request/3`) callback. The record representation is as outlined in [diameter_dict(4)](diameter_dict.md#message_records): a message as defined in a dictionary file is encoded as a record with one field for each component AVP. Equivalently, a message can also be encoded as a list whose head is the atom-valued message name (as specified in the relevant dictionary file) and whose tail is either a list of AVP name/values pairs or a map with values keyed on AVP names. The format at decode is determined by [diameter:service_opt()](`m:diameter#service_opt`) [decode_format](`m:diameter#decode_format`). Any of the formats is accepted at encode.
...
* __`packet() = #diameter_packet{}`{: id=packet }__ - A container for incoming and outgoing Diameter messages. Fields have the following types.

  * __`header = `[`header()`](`m:diameter_codec#header`)` | undefined`__ - The Diameter header of the message. Can be (and typically should be) `undefined` for an outgoing message in a non-relay application, in which case diameter provides appropriate values.

  * __`avps = [`[`avp()`](`m:diameter_codec#avp`)`] | undefined`__ - The AVPs of the message. Ignored for an outgoing message if the `msg` field is set to a value other than `undefined`.

  * __`msg = `[`message()`](`m:diameter_codec#message`)` | undefined`__ - The incoming/outgoing message. For an incoming message, a term corresponding to the configured [decode format](`m:diameter#decode_format`) if the message can be decoded in a non-relay application, `undefined` otherwise. For an outgoing message, setting a `[`[`header()`](`m:diameter_codec#header`)` | `[`avp()`](`m:diameter_codec#avp`)`]` list is equivalent to setting the `header` and `avps` fields to the corresponding values.

    > #### Warning {: class=warning }
    > A value in the `msg` field does *not* imply an absence of decode errors. The `errors` field should also be examined.

  * __`bin = binary()`__ - The incoming message prior to encode or the outgoing message after encode.

  * __`errors = [5000..5999 | {5000..5999, avp()}]`__ - Errors detected at decode of an incoming message, as identified by a corresponding 5xxx series Result-Code (Permanent Failures). For an incoming request, these should be used to formulate an appropriate answer as documented for the [handle_request/3](`c:diameter_app:handle_request/3`) callback in `m:diameter_app`. For an incoming answer, the [diameter:application_opt()](`m:diameter#application_opt`) `answer_errors` determines the behaviour.

  * __`transport_data = term()`__ - An arbitrary term of meaning only to the transport process in question, as documented in `m:diameter_transport`.

  

## SEE ALSO

[diameterc(1)](diameterc_cmd.md), `m:diameter_app`, [diameter_dict(4)](diameter_dict.md), `m:diameter_make`
""".
...
-doc """
Mod = [dictionary()](`m:diameter_codec#dictionary`)  
Msg = [message()](`m:diameter_codec#message`) | [packet()](`m:diameter_codec#packet`)  
Pkt = [packet()](`m:diameter_codec#packet`)  

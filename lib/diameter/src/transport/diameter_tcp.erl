...
This module implements diameter transport over TCP using `m:gen_tcp`. It can be specified as the value of a `transport_module` option to `diameter:add_transport/2` and implements the behaviour documented in `m:diameter_transport`. TLS security is supported, either as an upgrade following capabilities exchange or at connection establishment.
...
Reason = term()  
OwnOpt = \{raddr, [inet:ip_address()](`t:inet:ip_address/0`)\} | \{rport, integer()\} | \{accept, Match\} | \{port, integer()\} | \{fragment_timer, infinity | 0..16#FFFFFFFF\} | \{message_cb, [diameter:eval()](`m:diameter#eval`)\} | \{sender, boolean()\}  
...
Option `ssl_options` must be specified for a transport that should support TLS: a value of `true` results in a TLS handshake immediately upon connection establishment while `t:list()` specifies options to be passed to `ssl:connect/2` or `ssl:handshake/2` after capabilities exchange if TLS is negotiated.

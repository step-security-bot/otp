...
* __`message() = binary() | `{: id=message }[`diameter_codec:packet()`{: id=message }](`m:diameter_codec#packet`)__ - A Diameter message as passed over the transport interface.

  For an inbound message from a transport process, a [diameter_codec:packet()](`m:diameter_codec#packet`) must contain the received message in its `bin` field. In the case of an inbound request, any value set in the `transport_data` field will passed back to the transport module in the corresponding answer message, unless the sender supplies another value.

  For an outbound message to a transport process, a [diameter_codec:packet()](`m:diameter_codec#packet`) has a value other than `undefined` in its `transport_data` field and has the binary() to send in its `bin` field.
...
* __`{diameter, {send, `[`message()`](`m:diameter_transport#message`)` | false}}`__ - An outbound Diameter message. The atom `false` can only be received when request acknowledgements have been requests: see the `ack` message below.

* __`{diameter, {close, Pid}}`__ - A request to terminate the transport process after having received DPA in response to DPR. The transport process should exit. `Pid` is the pid() of the parent process.

* __`{diameter, {tls, Ref, Type, Bool}}`__ - Indication of whether or not capabilities exchange has selected inband security using TLS. `Ref` is a reference() that must be included in the `{diameter, {tls, Ref}}` reply message to the transport's parent process (see below). `Type` is either `connect` or `accept` depending on whether the process has been started for a connecting or listening transport respectively. `Bool` is a boolean() indicating whether or not the transport connection should be upgraded to TLS.
...
* __`{diameter, {self(), connected}}`__ - Inform the parent that the transport process with `Type=accept` has established a connection with the peer. Not sent if the transport process has `Type=connect`.

* __`{diameter, {self(), connected, Remote}}`__  

* __`{diameter, {self(), connected, Remote, [LocalAddr]}}`__ - Inform the parent that the transport process with `Type=connect` has established a connection with a peer. Not sent if the transport process has `Type=accept`.  `Remote` is an arbitrary term that uniquely identifies the remote endpoint to which the transport has connected. A `LocalAddr` list has the same semantics as one returned from `c:start/3`.

* __`{diameter, ack}`__ - Request acknowledgements of unanswered requests. A transport process should send this once before passing incoming Diameter messages into diameter. As a result, every Diameter request passed into diameter with a `recv` message (below) will be answered with a `send` message (above), either a [message()](`m:diameter_transport#message`) for the transport process to send or the atom `false` if the request has been discarded or otherwise not answered.
...
* __`{diameter, {recv, `[`message()`](`m:diameter_transport#message`)`}}`__ - An inbound Diameter message.

* __`{diameter, {tls, Ref}}`__ - Acknowledgment of a successful TLS handshake. `Ref` is the reference() received in the `{diameter, {tls, Ref, Type, Bool}}` message in response to which the reply is sent. A transport must exit if a handshake is not successful.
...
-doc """
Type = connect | accept  
Ref = [diameter:transport_ref()](`m:diameter#transport_ref`)  
Svc = #diameter_service\{\}  
Config = term()  
Pid = pid()  
LAddrs = [[inet:ip_address()](`t:inet:ip_address/0`)]  
Reason = term()  

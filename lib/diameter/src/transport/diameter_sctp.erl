...
This module implements diameter transport over SCTP using `m:gen_sctp`. It can be specified as the value of a transport_module option to `diameter:add_transport/2` and implements the behaviour documented in `m:diameter_transport`.
...
Opt = OwnOpt | SctpOpt  
Pid = pid()  
LAddr = [inet:ip_address()](`t:inet:ip_address/0`)  
Reason = term()  
OwnOpt = \{raddr, [inet:ip_address()](`t:inet:ip_address/0`)\} | \{rport, integer()\} | \{accept, Match\} | \{unordered, boolean() | pos_integer()\} | \{packet, boolean() | raw\} | \{message_cb, [diameter:eval()](`m:diameter#eval`)\} | \{sender, boolean()\}  
SctpOpt = term()  
Match = [inet:ip_address()](`t:inet:ip_address/0`) | string() | \[Match]  
...
Options `message_cb` and `sender` have semantics identical to those documented in [diameter_tcp(3)](`m:diameter_tcp#sender`), but with the message argument to a `recv` callback being as directed by the `packet` option.
...
> An small receive buffer may result in a peer having to resend incoming messages: set the `m:inet` option `recbuf` to increase the buffer size.
>
> An small send buffer may result in outgoing messages being discarded: set the `m:inet` option `sndbuf` to increase the buffer size.
""".

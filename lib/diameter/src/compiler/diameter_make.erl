...
The function `codec/2` is used to compile a diameter [dictionary file](diameter_dict.md) into Erlang source. The resulting source implements the interface diameter requires to encode and decode the dictionary's messages and AVPs.

The utility [diameterc(1)](diameterc_cmd.md) provides an alternate compilation interface.
...
* __`parse | forms | erl | hrl`__ - Specifies an output format. Whether the output is returned or written to file depends on whether or not option `return` is specified. When written to file, the resulting file(s) will have extensions `.D`, `.F`, `.erl`, and `.hrl` respectively, basenames defaulting to `dictionary` if the input dictionary is literal and does not specify [`@name`](diameter_dict.md#name). When returned, results are in the order of the corresponding format options. Format options default to `erl` and `hrl` (in this order) if unspecified.
...
* __`{include, string()}`__ - Prepend the specified directory to the code path. Use to point at beam files compiled from inherited dictionaries, [`@inherits`](diameter_dict.md#inherits) in a dictionary file creating a beam dependency, not an erl/hrl dependency.

  Multiple `include` options can be specified.

* __`{outdir, string()}`__ - Write generated source to the specified directory. Defaults to the current working directory. Has no effect if option `return` is specified.

* __`return`__ - Return results in a `{ok, [Out]}` tuple instead of writing to file and returning `ok`.

* __`{name|prefix, string()}`__ - Transform the input dictionary before compilation, setting [`@name`](diameter_dict.md#name) or [`@prefix`](diameter_dict.md#prefix) to the specified string.

* __`{inherits, string()}`__ - Transform the input dictionary before compilation, appending [`@inherits`](diameter_dict.md#inherits) of the specified string.
...
Note that a dictionary's [`@name`](diameter_dict.md#name), together with the `outdir` option, determine the output paths when the `return` option is not specified. The [`@name`](diameter_dict.md#name) of a literal input dictionary defaults to `dictionary`.
...
-doc "Turns a parsed dictionary, as returned by `codec/2`, back into the dictionary format.".
...
-doc "Reconstitute a parsed dictionary, as returned by `codec/2`, without using [`@inherits`](diameter_dict.md#inherits). That is, construct an equivalent dictionary in which all AVP's are definined in the dictionary itself. The return value is also a parsed dictionary.".
-doc(#{since => <<"OTP R16B03">>}).
-spec flatten(parsed())
   -> parsed().
...
-doc "Turn an error reason returned by `codec/2` into a readable string.".
-doc(#{since => <<"OTP 17.0">>}).
format_error(T) ->

...
This function is called at the end of stage 1 of the discovery process. It should return either the atom `ignore` or `{ok, usm_entry() | [usm_entry()]}`. See [usm_entry()](snmp_agent_config_files.md#usm) and [usm_entry/13](`m:snmpa_conf#usm_entry`) for more info.
...
In either case, the agent will do nothing, but return the retrieved ManagerEngineID (see [discovery](`m:snmpa#discovery`) for more info) and possible continue with stage 2 of the discovery process.

The `ExtraInfo` argument is passed on from the [discovery](`m:snmpa#discovery`) function.

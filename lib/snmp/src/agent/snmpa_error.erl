...
Two simple implementation(s) is provided with the toolkit; the modules `m:snmpa_error_logger` which is the default and `m:snmpa_error_io`.

The error report module is configured using the directive `error_report_mod`, see [configuration parameters](snmp_config.md#configuration_params).

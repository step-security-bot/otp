...
A network interface filter can e.g. be used during testing or for load regulation. If the intended use is load regulation, see also [req_limit](snmp_config.md#agent_ni_req_limit) and the function [register_notification_filter](`m:snmpa#register_notification_filter`).

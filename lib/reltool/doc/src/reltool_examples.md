...
The main process in Reltool is the server. It can be used as it is or be used via the GUI frontend process. When the GUI is started, a server process will automatically be started. The GUI process is started with `reltool:start/0`, `reltool:start/1` or `reltool:start_link/1`. The pid of its server can be obtained with [`reltool:get_server/1`](`reltool:start_link/1`)

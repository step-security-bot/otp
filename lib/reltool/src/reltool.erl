...
* __`config`__ - This is the main option and it controls the configuration of Reltool. It can either be a `sys` tuple or a name of a `file` containing a sys tuple.

* __`trap_exit`__ - This option controls the error handling behavior of Reltool. By default the window processes traps exit, but this behavior can altered by setting `trap_exit` to `false`.

* __`wx_debug`__ - This option controls the debug level of `wx`. As its name indicates it is only useful for debugging. See `wx:debug/1` for more info.
...
* __`incl_sys_filters`__ - This parameter normally contains a list of regular expressions that controls which files in the system should be included. Each file in the target system must match at least one of the listed regular expressions in order to be included. Further the files may not match any filter in `excl_sys_filters` in order to be included. Which application files should be included is controlled with the parameters `incl_app_filters` and `excl_app_filters`. This parameter defaults to `[".*"]`.

* __`excl_sys_filters`__ - This parameter normally contains a list of regular expressions that controls which files in the system should not be included in the target system. In order to be included, a file must match some filter in `incl_sys_filters` but not any filter in `excl_sys_filters`. This parameter defaults to `[]`.

* __`incl_app_filters`__ - This parameter normally contains a list of regular expressions that controls which application specific files that should be included. Each file in the application must match at least one of the listed regular expressions in order to be included. Further the files may not match any filter in `excl_app_filters` in order to be included. This parameter defaults to `[".*"]`.

* __`excl_app_filters`__ - This parameter normally contains a list of regular expressions that controls which application specific files should not be included in the target system. In order to be included, a file must match some filter in `incl_app_filters` but not any filter in `excl_app_filters`. This parameter defaults to `[]`.

On application (`escript`) level, the following options are supported:

* __`incl_cond`__ - The value of this parameter overrides the parameter with the same name on system level.

On application (`app`) level, the following options are supported:

* __`vsn`__ - The version of the application. In an installed system there may exist several versions of an application. The `vsn` parameter controls which version of the application will be chosen.
...
* __`lib_dir`__ - The directory to read the application from. This parameter can be used to point out a specific location to fetch the application from. This is useful for instance if the parent directory for some reason is no good as a library directory on system level.

  This parameter is mutual exclusive with `vsn`. If `vsn` and `lib_dir` are both omitted, the latest version will be chosen.

  Note that in order for reltool to sort application versions and thereby be able to select the latest, it is required that the version id for the application consists of integers and dots only, for example `1`, `2.0` or `3.17.1`.

* __`mod`__ - Module specific configuration. A module has a mandatory name and module level options that are described below.

* __`mod_cond`__ - The value of this parameter overrides the parameter with the same name on system level.

* __`incl_cond`__ - The value of this parameter overrides the parameter with the same name on system level.

* __`app_file`__ - The value of this parameter overrides the parameter with the same name on system level.

* __`debug_info`__ - The value of this parameter overrides the parameter with the same name on system level.

* __`incl_app_filters`__ - The value of this parameter overrides the parameter with the same name on system level.

* __`excl_app_filters`__ - The value of this parameter overrides the parameter with the same name on system level.
...
* __`debug_info`__ - The value of this parameter overrides the parameter with the same name on application level.

...
-doc "The socket is supposed to be result of a [gen_tcp:connect](`gen_tcp:connect/3`) or a [gen_tcp:accept](`gen_tcp:accept/1`). The socket must be in passive mode (that is, opened with the option `{active,false})`.".
-doc(#{title => <<"Other data types">>}).
...
The `channel_callback` is the module that implements the `m:ssh_server_channel` (replaces ssh_daemon_channel) behaviour in the daemon. See the section [Creating a Subsystem](using_ssh.md#usersguide_creating_a_subsystem) in the User's Guide for more information and an example.
...
-doc """
List of algorithms to use in the algorithm negotiation. The default `t:algs_list()` can be obtained from `default_algorithms/0`.
...
For background and more examples see the [User's Guide](configure_algos.md#introduction).
...
* Input is the `t:modify_algs_list()` and a set of algorithms `A` obtained from the `preferred_algorithms` option if existing, or else from the [ssh:default_algorithms/0](`default_algorithms/0`).
* The head of the `t:modify_algs_list()` modifies `A` giving the result `A'`.
...
* Repeat the modification step with the tail of `t:modify_algs_list()` and the resulting `A'`.

If an unsupported algorithm is in the `t:modify_algs_list()`, it will be silently ignored
...
-doc """
Options for [clients](`connect/3`). The individual options are further explained below or by following the hyperlinks.

Note that not every [gen_tcp:connect_option()](`t:gen_tcp:connect_option/0`) is accepted. See `set_sock_opts/2` for a list of prohibited options.

Also note that setting a [gen_tcp:connect_option()](`t:gen_tcp:connect_option/0`) could change the socket in a way that impacts the ssh client's behaviour negatively. You use it on your own risk.
""".
-doc(#{title => <<"Client Options">>}).
-type client_options()        :: [ client_option() ] .
-doc """
Options for [daemons](`daemon/1`). The individual options are further explained below or by following the hyperlinks.

Note that not every [gen_tcp:listen_option()](`t:gen_tcp:listen_option/0`) is accepted. See `set_sock_opts/2` for a list of prohibited options.

Also note that setting a [gen_tcp:listen_option()](`t:gen_tcp:listen_option/0`) could change the socket in a way that impacts the ssh deamon's behaviour negatively. You use it on your own risk.
""".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
-type daemon_options()        :: [ daemon_option() ].
                              

-doc "The options above can be used both in clients and in daemons (servers). They are further explained below.".
-doc(#{title => <<"Options common to clients and daemons">>}).
...
For more information about timeouts, see the [Timeouts section ](hardening.md#timeouts)in the User's Guide [Hardening](hardening.md) chapter.
""".
-doc(#{title => <<"Options common to clients and daemons">>}).
...
-doc "Sets a limit for the size of a logged item excluding a header. The unit is bytes and the value defaults to 500.".
-doc(#{title => <<"Options common to clients and daemons">>}).
...
-doc """
Module implementing the behaviour `m:ssh_client_key_api` and/or `m:ssh_server_key_api`. Can be used to customize the handling of public keys. If callback options are provided along with the module name, they are made available to the callback module via the options passed to it under the key 'key_cb_private'.

The `Opts` defaults to `[]` when only the `Module` is specified.

The default value of this option is `{ssh_file, []}`. See also the manpage of `m:ssh_file`.
...
where `...` are arguments to `F` as in `m:ssh_client_key_api` and/or `m:ssh_server_key_api`. The `UserOptions` are the options given to [ssh:connect](`connect/3`), [ssh:shell](`shell/1`) or [ssh:daemon](`daemon/2`).
""".
-doc(#{title => <<"Options common to clients and daemons">>}).
...
-doc "Provides a fun to implement your own logging or other action when an unexpected message arrives. If the fun returns `report` the usual info report is issued but if `skip` is returned no report is generated.".
-doc(#{title => <<"Options common to clients and daemons">>}).
-type unexpectedfun_common_option()     ::
        {unexpectedfun, fun((Message::term(),{Host::term(),Port::term()}) -> report | skip ) }.
-doc """
Provide a fun to implement your own logging of the SSH message SSH_MSG_DEBUG. The last three parameters are from the message, see [RFC 4253, section 11.3](https://tools.ietf.org/html/rfc4253#section-11.3). The [`connection_ref()`](`t:connection_ref/0`) is the reference to the connection on which the message arrived. The return value from the fun is not checked.
...
The default value is the `public_key` entry in the list returned by [ssh:default_algorithms/0](`default_algorithms/0`).
...
See also the option [`user_dir`](`t:ssh_file:user_dir_common_option/0`) for specifying the path to the user's keys.
""".
-doc(#{title => <<"Options common to clients and daemons">>}).
...
-doc "Allows an existing file-descriptor to be used (passed on to the transport protocol).".
-doc(#{title => <<"Options common to clients and daemons">>}).
-type fd_common_option() :: {fd, gen_tcp:socket()} .


-doc "Opaque types that define experimental options that are not to be used in products.".
-doc(#{title => <<"Other data types">>}).
...
-doc """
* __`silently_accept_hosts`{: id=hardening_client_options-silently_accept_hosts }__ - This option guides the `connect` function on how to act when the connected server presents a Host Key that the client has not seen before. The default is to ask the user with a question on stdio of whether to accept or reject the new Host Key. See the option [`user_dir`](`t:ssh_file:user_dir_common_option/0`) for specifying the path to the file `known_hosts` where previously accepted Host Keys are recorded. See also the option [key_cb](`t:key_cb_common_option/0`) for the general way to handle keys.

  The option can be given in three different forms as seen [above](`t:accept_hosts/0`):

  * The value is a `t:boolean()`. The value `true` will make the client accept any unknown Host Key without any user interaction. The value `false` preserves the default behaviour of asking the user on stdio.
  * An `t:accept_callback()` will be called and the boolean return value `true` will make the client accept the Host Key. A return value of `false` will make the client to reject the Host Key and as a result the connection will be closed. The arguments to the fun are:
  ...
  * A tuple `{HashAlgoSpec, accept_callback}`. The `HashAlgoSpec` specifies which hash algorithm shall be used to calculate the fingerprint used in the call of the `t:accept_callback()`. The `HashALgoSpec` is either an atom or a list of atoms as the first argument in `hostkey_fingerprint/2`. If it is a list of hash algorithm names, the `FingerPrint` argument in the `t:accept_callback()` will be a list of fingerprints in the same order as the corresponding name in the `HashAlgoSpec` list.

* __`user_interaction`__ - If `false`, disables the client to connect to the server if any user interaction is needed, such as accepting the server to be added to the `known_hosts` file, or supplying a password.
...
* __`save_accepted_host`__ - If `true`, the client saves an accepted host key to avoid the accept question the next time the same host is connected. If the option [`key_cb`](`t:key_cb_common_option/0`) is not present, the key is saved in the file "known_hosts". See option [`user_dir`](`t:ssh_file:user_dir_common_option/0`) for the location of that file.

  If `false`, the key is not saved and the key will still be unknown at the next access of the same host.

  Defaults to `true`

* __`quiet_mode`__ - If `true`, the client does not print anything on authorization.
...
-doc """
* __`user`__ - Provides the username. If this option is not given, `ssh` reads from the environment (`LOGNAME` or `USER` on UNIX, `USERNAME` on Windows).

* __`password`__ - Provides a password for password authentication. If this option is not given, the user is asked for a password, if the password authentication method is attempted.
""".
-doc(#{title => <<"Client Options">>}).
...
-doc "Sets the three diffie-hellman-group-exchange parameters that guides the connected server in choosing a group. See [RFC 4419](https://tools.ietf.org/html/rfc4419) for the details. The default value is `{1024, 6144, 8192}`.".
-doc(#{title => <<"Client Options">>}).
-type diffie_hellman_group_exchange_client_option() ::
        {dh_gex_limits,        {Min::pos_integer(), I::pos_integer(), Max::pos_integer()} } .

-doc """
Sets a timeout on the transport layer connect time. For `m:gen_tcp` the time is in milli-seconds and the default value is `infinity`.
...
-doc """
Make the client tell the server that the client accepts extension negotiation, that is, include `ext-info-c` in the kexinit message sent. See [RFC 8308](https://tools.ietf.org/html/rfc8308) for details and [ssh(6)](ssh_app.md#supported-ext-info) for a list of currently implemented extensions.
...
See the option [`exec-option`](`t:exec_daemon_option/0`) for a description of how the daemon executes shell-requests and exec-requests depending on the shell- and exec-options.
""".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
...
See the User's Guide section on [One-Time Execution](using_ssh.md#one-time-execution) for examples.
...
The option cooperates with the daemon-option [`shell`](`t:shell_daemon_option/0`) in the following way:

* __1\. If neither the [`exec-option`](`t:exec_daemon_option/0`) nor the [`shell-option`](`t:shell_daemon_option/0`) is present:__ - The default Erlang evaluator is used both for exec and shell requests. The result is returned to the client.

* __2\. If the [`exec_spec`](`t:exec_daemon_option/0`)'s value is `disabled` (the [`shell-option`](`t:shell_daemon_option/0`) may or may not be present):__ - No exec-requests are executed but shell-requests are not affected, they follow the [`shell_spec`](`t:shell_daemon_option/0`)'s value.

* __3\. If the [`exec-option`](`t:exec_daemon_option/0`) is present and the [`exec_spec`](`t:exec_daemon_option/0`) value =/= `disabled` (the [`shell-option`](`t:shell_daemon_option/0`) may or may not be present):__ - The [`exec_spec`](`t:exec_daemon_option/0`) `fun()` is called with the same number of parameters as the arity of the fun, and the result is returned to the client. Shell-requests are not affected, they follow the [`shell_spec`](`t:shell_daemon_option/0`)'s value.

* __4\. If the [`exec-option`](`t:exec_daemon_option/0`) is absent, and the [`shell-option`](`t:shell_daemon_option/0`) is present with the default Erlang shell as the [`shell_spec`](`t:shell_daemon_option/0`)'s value:__ - The default Erlang evaluator is used both for exec and shell requests. The result is returned to the client.

* __5\. If the [`exec-option`](`t:exec_daemon_option/0`) is absent, and the [`shell-option`](`t:shell_daemon_option/0`) is present with a value that is neither the default Erlang shell nor the value `disabled`:__ - The exec-request is not evaluated and an error message is returned to the client. Shell-requests are executed according to the value of the [`shell_spec`](`t:shell_daemon_option/0`).

* __6\. If the [`exec-option`](`t:exec_daemon_option/0`) is absent, and the [`shell_spec`](`t:shell_daemon_option/0`)'s value is `disabled`:__ - Exec requests are executed by the default shell, but shell-requests are not executed.

If a custom CLI is installed (see the option [`ssh_cli`](`t:ssh_cli_daemon_option/0`)) the rules above are replaced by thoose implied by the custom CLI.

> #### Note {: class=info }
> The [`exec-option`](`t:exec_daemon_option/0`) has existed for a long time but has not previously been documented. The old definition and behaviour are retained but obey the rules 1-6 above if conflicting. The old and undocumented style should not be used in new programs.
""".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
-type exec_result()  :: {ok,Result::term()} | {error,Reason::term()} .
-doc "Old-style exec specification that are kept for compatibility, but should not be used in new programs".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
...
It is a channel callback module that implements a shell and command execution. The shell's read-eval-print loop can be customized, using the option [`shell`](`t:shell_daemon_option/0`). This means less work than implementing an own CLI channel. If `ssh_cli` is set to `no_cli`, the CLI channels like [`shell`](`t:shell_daemon_option/0`) and [`exec`](`t:exec_daemon_option/0`) are disabled and only subsystem channels are allowed.
""".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
-type ssh_cli_daemon_option()   :: {ssh_cli, mod_args() | no_cli }.

-doc "Enables (`true`) or disables (`false`) the possibility to tunnel a TCP/IP connection out of a [server](`daemon/2`). Disabled per default.".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
-type tcpip_tunnel_out_daemon_option() :: {tcpip_tunnel_out, boolean()} .
-doc "Enables (`true`) or disables (`false`) the possibility to tunnel a TCP/IP connection in to a [server](`daemon/2`). Disabled per default.".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
-type tcpip_tunnel_in_daemon_option() :: {tcpip_tunnel_in, boolean()} .

-doc """
Make the server (daemon) tell the client that the server accepts extension negotiation, that is, include `ext-info-s` in the kexinit message sent. See [RFC 8308](https://tools.ietf.org/html/rfc8308) for details and [ssh(6)](ssh_app.md#supported-ext-info) for a list of currently implemented extensions.
...
-doc """
* __`auth_method_kb_interactive_data`__ - Sets the text strings that the daemon sends to the client for presentation to the user when using `keyboard-interactive` authentication.
...
* __`user_passwords`{: id=option-user_passwords }__ - Provides passwords for password authentication. The passwords are used when someone tries to connect to the server and public key user-authentication fails. The option provides a list of valid usernames and the corresponding passwords.

  > #### Warning {: class=warning }
  > Note that this is very insecure due to the plain-text passwords; it is intended for test purposes. Use the [`pwdfun`](`m:ssh#option-pwdfun`) option to handle the password checking instead.

* __`pk_check_user`{: id=option-pk_check_user }__ - Enables checking of the [client's user name](`t:authentication_client_options/0`) in the server when doing public key authentication. It is disabled by default.

  The term "user" is used differently in OpenSSH and SSH in Erlang/OTP: see more in the [User's Guide](terminology.md#the-term-user).

  If the option is enabled, and no [`pwdfun`](`m:ssh#option-pwdfun`) is present, the user name must present in the [user_passwords](`m:ssh#option-user_passwords`) for the check to succeed but the value of the password is not checked.

  In case of a [`pwdfun`](`m:ssh#option-pwdfun`) checking the user, the atom `pubkey` is put in the password argument.

* __`password`{: id=option-password }__ - Provides a global password that authenticates any user.
...
* __`pwdfun`{: id=option-pwdfun } with [`pwdfun_4()`](`t:pwdfun_4/0`)__ - Provides a function for password validation. This could used for calling an external system or handling passwords stored as hash values.
  ...
  * `{false, NewState:any()}` if the user or password is invalid
  ...
  In case of the [`pk_check_user`](`m:ssh#option-pk_check_user`) is set, the atom `pubkey` is put in the password argument when validating a public key login. The pwdfun is then responsible to check that the user name is valid.

* __`pwdfun` with [`pwdfun_2()`](`t:pwdfun_2/0`)__ - Provides a function for password validation. This function is called with user and password as strings, and returns:
...
* __`no_auth_needed`{: id=option-no_auth_needed }__ - If `true`, a client is authenticated without any need of providing any password or key.
...
-doc """
* __`dh_gex_groups`__ - Defines the groups the server may choose among when diffie-hellman-group-exchange is negotiated. See [RFC 4419](https://tools.ietf.org/html/rfc4419) for details. The three variants of this option are:

  * __`{Size=integer(),G=integer(),P=integer()}`__ - The groups are given explicitly in this list. There may be several elements with the same `Size`. In such a case, the server will choose one randomly in the negotiated Size.

  * __`{file,filename()}`__ - The file must have one or more three-tuples `{Size=integer(),G=integer(),P=integer()}` terminated by a dot. The file is read when the daemon starts.

  * __`{ssh_moduli_file,filename()}`__ - The file must be in [ssh-keygen moduli file format](`public_key:dh_gex_group/4`). The file is read when the daemon starts.

  The default list is fetched from the [public_key](`public_key:dh_gex_group/4`) application.

* __`dh_gex_limits`__ - Limits what a client can ask for in diffie-hellman-group-exchange. The limits will be `{MaxUsed = min(MaxClient,Max), MinUsed = max(MinClient,Min)}` where `MaxClient` and `MinClient` are the values proposed by a connecting client.

  The default value is `{0,infinity}`.

  If `MaxUsed < MinUsed` in a key exchange, it will fail with a disconnect.

  See [RFC 4419](https://tools.ietf.org/html/rfc4419) for the function of the Max and Min values.
""".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
...
* __`max_channels`{: id=hardening_daemon_options-max_channels }__ - The maximum number of channels with active remote subsystem that are accepted for each connection to this daemon

  By default, this option is not set. This means that the number is not limited.

* __`parallel_login`{: id=hardening_daemon_options-parallel_login }__ - If set to false (the default value), only one login is handled at a time. If set to true, an unlimited number of login attempts are allowed simultaneously.
...
* __`minimal_remote_max_packet_size`{: id=hardening_daemon_options-minimal_remote_max_packet_size }__ - The least maximum packet size that the daemon will accept in channel open requests from the client. The default value is 0.
""".
-doc(#{title => <<"Daemon Options (Server Options)">>}).
...
-doc """
* __`connectfun`__ - Provides a fun to implement your own logging when a user authenticates to the server.

* __`failfun`__ - Provides a fun to implement your own logging when a user fails to authenticate.
""".
-doc(#{title => <<"Daemon Options (Server Options)">>}).

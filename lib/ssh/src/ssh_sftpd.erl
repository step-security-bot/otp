...
The `Name` is `"sftp"` and `CbMod` is the name of the Erlang module implementing the subsystem using the `m:ssh_server_channel` (replaces ssh_daemon_channel) behaviour.

Options:

* __`cwd`__ - Sets the initial current working directory for the server.

* __`file_handler`__ - Determines which module to call for accessing the file server. The default value is `ssh_sftpd_file`, which uses the `m:file` and `m:filelib` APIs to access the standard OTP file server. This option can be used to plug in other file servers.

* __`max_files`__ - The default value is `0`, which means that there is no upper limit. If supplied, the number of filenames returned to the SFTP client per `READDIR` request is limited to at most the given value.

* __`root`__ - Sets the SFTP root directory. Then the user cannot see any files above this root. If, for example, the root directory is set to `/tmp`, then the user sees this directory as `/`. If the user then writes `cd /etc`, the user moves to `/tmp/etc`.

* __`sftpd_vsn`__ - Sets the SFTP version to use. Defaults to 5. Version 6 is under development and limited.
""".

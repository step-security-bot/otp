...
The intention is to be compatible with the [OpenSSH](http://www.openssh.com) storage in files. Therefore it mimics directories and filenames of [OpenSSH](http://www.openssh.com).

Ssh_file implements the `m:ssh_server_key_api` and the `m:ssh_client_key_api`. This enables the user to make an own interface using for example a database handler.

Such another callback module could be used by setting the option [`key_cb`](`t:ssh:key_cb_common_option/0`) when starting a client or a server (with for example [ssh:connect](`ssh:connect/3`), [ssh:daemon](`ssh:daemon/2`) of [ssh:shell](`ssh:shell/1`) ).
...
Optionally, in case of `publickey` authorization, one or more of the remote user's public keys in the [USERDIR](`m:ssh_file#userdir`) directory are used. See the files [`USERDIR/authorized_keys`](`m:ssh_file#file-authorized_keys`) and [`USERDIR/authorized_keys2`](`m:ssh_file#file-authorized_keys2`).
...
* __[](){: id=SYSDIR }
  ...
  * [](){: id=FILE-ssh_host_STAR_key }
    `ssh_host_dsa_key`{: id=FILE-ssh_host_dsa_key } \- private dss host key (optional)
  * `ssh_host_rsa_key`{: id=FILE-ssh_host_rsa_key } \- private rsa host key (optional)
  * `ssh_host_ecdsa_key`{: id=FILE-ssh_host_ecdsa_key } \- private ecdsa host key (optional)
  * `ssh_host_ed25519_key`{: id=FILE-ssh_host_ed25519_key } \- private eddsa host key for curve 25519 (optional)
  * `ssh_host_ed448_key`{: id=FILE-ssh_host_ed448_key } \- private eddsa host key for curve 448 (optional)

  The key files could be generated with OpenSSH's ssh-keygen command.

  At least one host key must be defined. The default value of SYSDIR is `/etc/ssh`{: id=#/etc/ssh }.

  For security reasons, this directory is normally accessible only to the root user.

  To change the SYSDIR, see the [system_dir](`t:system_dir_daemon_option/0`) option.

* __[](){: id=USERDIR }
  USERDIR__  
  This is the directory holding the files:

  * `authorized_keys`{: id=FILE-authorized_keys } and, as second alternative `authorized_keys2`{: id=FILE-authorized_keys2 } \- the user's public keys are stored concatenated in one of those files.

    It is composed of lines as for [OpenSSH](https://man.openbsd.org/sshd#AUTHORIZED_KEYS_FILE_FORMAT):
    ...
    ```
  * `known_hosts`{: id=FILE-known_hosts } \- host keys from hosts visited concatenated. The file is created and used by the client.

    It is composed of lines as for [OpenSSH](https://man.openbsd.org/sshd#SSH_KNOWN_HOSTS_FILE_FORMAT):
    ...
    ```
  * [](){: id=FILE-id_STAR }
    `id_dsa`{: id=FILE-id_dsa } \- private dss user key (optional)
  * `id_rsa`{: id=FILE-id_rsa } \- private rsa user key (optional)
  * `id_ecdsa`{: id=FILE-id_ecdsa } \- private ecdsa user key (optional)
  * `id_ed25519`{: id=FILE-id_ed25519 } \- private eddsa user key for curve 25519 (optional)
  * `id_ed448`{: id=FILE-id_ed448 } \- private eddsa user key for curve 448 (optional)

  The key files could be generated with OpenSSH's ssh-keygen command.

  The default value of USERDIR is `/home/`[`LOCALUSER`](`m:ssh_file#localuser`)`/.ssh`.
...
-doc "Sets the [user directory](`m:ssh_file#userdir`) dynamically by evaluating the `user2dir` function.".
-doc(#{title => <<"Options for the default ssh_file callback module">>}).
...
To set it, set the option `{key_cb, {ssh_file, [{optimize,TimeOrSpace}]}` in the call of ["ssh:connect/3](`ssh:connect/3`), `ssh:daemon/2` or similar function call that initiates an ssh connection.
""".
-doc(#{title => <<"Options for the default ssh_file callback module">>}).
...
See the api description in [ssh_server_key_api, Module:host_key/2](`c:ssh_server_key_api:host_key/2`).
...
See the api description in [ssh_server_key_api: Module:is_auth_key/3](`c:ssh_server_key_api:is_auth_key/3`).
...
See the api description in [ssh_client_key_api, Module:is_host_key/5](`c:ssh_client_key_api:is_host_key/5`).

[](){: id=is_host_key-4 }
Note that the alternative, the old [Module:is_host_key/4](`c:ssh_client_key_api:is_host_key/4`) is no longer supported by `ssh_file`.
...
See the api description in [ssh_client_key_api, Module:add_host_key/4](`c:ssh_client_key_api:add_host_key/4`).

[](){: id=add_host_key-3 }
Note that the alternative, the old [Module:add_host_key/3](`c:ssh_client_key_api:add_host_key/3`) is no longer supported by `ssh_file`.

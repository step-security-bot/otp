...
The [SSH Connection Protocol](http://www.ietf.org/rfc/rfc4254.txt) is used by clients and servers, that is, SSH channels, to communicate over the SSH connection. The API functions in this module send SSH Connection Protocol events, which are received as messages by the remote channel handling the remote channel. The Erlang format of thoose messages is (see also [below](`t:event/0`)):

`{ssh_cm, `[ssh:connection_ref()](`t:ssh:connection_ref/0`)`, `[`channel_msg()`](`t:channel_msg/0`)`}`

If the `m:ssh_client_channel` behavior is used to implement the channel process, these messages are handled by [handle_ssh_msg/2](`c:ssh_client_channel:handle_ssh_msg/2`).
""".
...
-doc "The status of a request. Corresponds to the `SSH_MSG_CHANNEL_SUCCESS` and `SSH_MSG_CHANNEL_FAILURE` values in [RFC 4254, Section 5.4](https://tools.ietf.org/html/rfc4254#section-5.4).".
-type req_status() :: success | failure .
-doc """
The result of a call.

If the request reached the peer, was handled and the response reached the requesting node the [req_status()](`t:req_status/0`) is the status reported from the peer.

If not, the [reason()](`t:reason/0`) indicates what went wrong:

* __`closed`__ - indicates that the channel or connection was closed when trying to send the request

* __`timeout`__ - indicates that the operation exceeded a time limit
""".
...
-doc """
The valid values are `0` ("normal") and `1` ("stderr"), see [RFC 4254, Section 5.2](https://tools.ietf.org/html/rfc4254#page/8).
""".
...
-doc "As mentioned in the introduction, the [SSH Connection Protocol](https://tools.ietf.org/html/rfc4254) events are handled as messages. When writing a channel handling process without using the support by the `m:ssh_client_channel` behavior the process must handle thoose messages.".
...
-doc "Messages that include a `WantReply` expect the channel handling process to call [ssh_connection:reply_request/4](`reply_request/4`) with the boolean value of `WantReply` as the second argument.".
-doc(#{title => <<"SSH Connection Protocol: General">>}).
-type want_reply() :: boolean().

-doc """
Data has arrived on the channel. This event is sent as a result of calling [ssh_connection:send/\[3,4,5]](`send/3`).
""".
...
-doc "Indicates that the other side sends no more data. This event is sent as a result of calling [ssh_connection:send_eof/2](`send_eof/2`).".
-doc(#{title => <<"Closing a Channel (RFC 4254, section 5.3)">>}).
-type eof_ch_msg() :: {eof,
                       ssh:channel_id()
                      } .
-doc "A signal can be delivered to the remote process/service using the following message. Some systems do not support signals, in which case they are to ignore this message. There is currently no function to generate this event as the signals referred to are on OS-level and not something generated by an Erlang program.".
-doc(#{title => <<"Signals (RFC 4254, section 6.9)">>}).
-type signal_ch_msg() :: {signal,
                          ssh:channel_id(),
                          SignalName :: string()
                         } .
-doc "A remote execution can terminate violently because of a signal. Then this message can be received. For details on valid string values, see [RFC 4254](https://tools.ietf.org/html/rfc4254#section-6.10) Section 6.10, which shows a special case of these signals.".
-doc(#{title => <<"Returning Exit Status (RFC 4254, section 6.10)">>}).
-type exit_signal_ch_msg() :: {exit_signal, ssh:channel_id(),
                               ExitSignal :: string(),
                               ErrorMsg :: string(),
                               LanguageString :: string()} .
-doc "When the command running at the other end terminates, the following message can be sent to return the exit status of the command. A zero `exit_status` usually means that the command terminated successfully. This event is sent as a result of calling [ssh_connection:exit_status/3](`exit_status/3`).".
-doc(#{title => <<"Returning Exit Status (RFC 4254, section 6.10)">>}).
-type exit_status_ch_msg() :: {exit_status,
                               ssh:channel_id(),
                               ExitStatus :: non_neg_integer()
                              } .
-doc "This event is sent as a result of calling [ssh_connection:close/2](`close/2`). Both the handling of this event and sending it are taken care of by the `m:ssh_client_channel` behavior.".
-doc(#{title => <<"Closing a Channel (RFC 4254, section 5.3)">>}).
-type closed_ch_msg() :: {closed,
                          ssh:channel_id()
                         } .
-doc "Environment variables can be passed to the shell/command to be started later. This event is sent as a result of calling [ssh_connection:setenv/5](`setenv/5`).".
...
-doc "A pseudo-terminal has been requested for the session. `Terminal` is the value of the TERM environment variable value, that is, `vt100`. Zero dimension parameters must be ignored. The character/row dimensions override the pixel dimensions (when non-zero). Pixel dimensions refer to the drawable area of the window. `Opcode` in the `TerminalModes` list is the mnemonic name, represented as a lowercase Erlang atom, defined in [RFC 4254](https://tools.ietf.org/html/rfc4254#section/8), Section 8. It can also be an `Opcode` if the mnemonic name is not listed in the RFC. Example: `OP code: 53, mnemonic name ECHO erlang atom: echo`. This event is sent as a result of calling [ssh_connection:ptty_alloc/4](`ptty_alloc/4`).".
...
-doc "This message requests that the user default shell is started at the other end. This event is sent as a result of calling [ssh_connection:shell/2](`shell/2`).".
-doc(#{title => <<"Starting a Shell or Command (RFC 4254, section 6.5)">>}).
-type shell_ch_msg() :: {shell,
                         ssh:channel_id(),
                         want_reply()
                        } .
-doc "When the window (terminal) size changes on the client side, it *can* send a message to the server side to inform it of the new dimensions. No API function generates this event.".
...
-doc "This message requests that the server starts execution of the given command. This event is sent as a result of calling [ssh_connection:exec/4 ](`exec/4`).".
-doc(#{title => <<"Starting a Shell or Command (RFC 4254, section 6.5)">>}).
...
-doc "Opens a channel for an SSH session. The channel id returned from this function is the id used as input to the other functions in this module.".
...
* __N x [data message(s)](`t:data_ch_msg/0`)__ - The result of executing the command can be only one line or thousands of lines depending on the command.

* __0 or 1 x [eof message](`t:eof_ch_msg/0`)__ - Indicates that no more data is to be sent.

* __0 or 1 x [exit signal message](`t:exit_signal_ch_msg/0`)__ - Not all systems send signals. For details on valid string values, see RFC 4254, Section 6.10

* __0 or 1 x [exit status message](`t:exit_status_ch_msg/0`)__ - It is recommended by the SSH Connection Protocol to send this message, but that is not always the case.

* __1 x [closed status message](`t:closed_ch_msg/0`)__ - Indicates that the `ssh_client_channel` started for the execution of the command has now been shut down.
...
The function `subsystem/4` and subsequent calls of [send/3,4,5](`send/3`) must be executed in the same process.
""".
...
-doc """
ConnectionRef = [ssh:connection_ref()](`t:ssh:connection_ref/0`)  
ChannelId = [ssh:channel_id()](`t:ssh:channel_id/0`)  
Data = iodata()  
Type = [ssh_data_type_code()](`t:ssh_data_type_code/0`)  
Timeout = timeout()  
Error = \{error, [reason()](`t:reason/0`)\}  
...
> #### Note {: class=info }
> Channels implemented with the `m:ssh_client_channel` behavior do not normally need to call this function as flow control is handled by the behavior. The behavior adjusts the window every time the callback [handle_ssh_msg/2](`c:ssh_client_channel:handle_ssh_msg/2`) returns after processing channel data.
""".
...
%%--------------------------------------------------------------------
-doc "Environment variables can be passed before starting the shell/command. Is to be called by a client channel processes.".
...
> #### Note {: class=info }
> This function is called by the `ssh_client_channel` behavior when the channel is terminated, see `m:ssh_client_channel`. Thus, channels implemented with the behavior are not to call this function explicitly.
""".
...
* __\{term, string()\}__ - Defaults to *os:getenv("TERM")* or *vt100* if it is undefined.

* __\{width, integer()\}__ - Defaults to 80 if `pixel_width` is not defined.

* __\{height, integer()\}__ - Defaults to 24 if `pixel_height` is not defined.

* __\{pixel_width, integer()\}__ - Is disregarded if `width` is defined.

* __\{pixel_height, integer()\}__ - Is disregarded if `height` is defined.

* __\{pty_opts, \[\{posix_atom(), integer()\}]\}__ - Option can be an empty list. Otherwise, see possible *POSIX* names in Section 8 in [RFC 4254](http://www.ietf.org/rfc/rfc4254.txt).
""".
-doc(#{since => <<"OTP 17.4,OTP 17.5">>}).
...
-doc "Is to be called by a server-channel process to send the exit status of a command to the client.".

...
The `t:atom()` value is formed from the sftp error codes in the protocol-level responses as defined in [draft-ietf-secsh-filexfer-13](https://tools.ietf.org/html/draft-ietf-secsh-filexfer-13#page-49) section 9.1. The codes are named as `SSH_FX_*` which are transformed into lowercase of the star-part. E.g. the error code `SSH_FX_NO_SUCH_FILE` will cause the `t:reason()` to be `no_such_file`.

The `t:string()` reason is the error information from the server in case of an exit-signal. If that information is empty, the reason is the exit signal name.

The `t:tuple()` reason are other errors like for example `{exit_status,1}`.
""".
...
-doc """
Host = [ssh:host()](`t:ssh:host/0`)  
Port = [inet:port_number()](`t:inet:port_number/0`)  
TcpSocket = [ssh:open_socket()](`t:ssh:open_socket/0`)  
Options = [ [sftp_option()](`t:sftp_option/0`) | [ssh:client_option()](`t:ssh:client_option/0`) ]  
SftpOptions = [ [sftp_option()](`t:sftp_option/0`) ]  
ChannelPid = pid()  
...
* __`{timeout, timeout()}`__ - There are two ways to set a timeout for the underlying ssh connection:
...
* __`{sftp_vsn, integer()}`__ - Desired SFTP protocol version. The actual version is the minimum of the desired version and the maximum supported versions by the SFTP server.

All other options are directly passed to [ssh:connect/3](`m:ssh`) or ignored if a connection is already provided.
""".
...
%%%----------------------------------------------------------------
-doc "Stops an SFTP channel. Does not close the SSH connection. Use `ssh:close/1` to close it.".
...
-doc "Opens a file on the server and returns a handle, which can be used for reading or writing.".
...
For code examples see Section [Example with encryption](using_ssh.md#example-with-encryption) in the ssh Users Guide.
""".
...
-doc """
The `t:init_fun()` in the [tar_crypto_spec](`t:tar_crypto_spec/0`) is applied once prior to any other `crypto` operation. The intention is that this function initiates the encryption or decryption for example by calling `crypto:crypto_init/4` or similar. The `t:crypto_state()` is the state such a function may return.

If the selected cipher needs to have the input data partitioned into blocks of a certain size, the `t:init_fun()` should return the second form of return value with the `t:chunk_size()` set to the block size. If the `t:chunk_size()` is `undefined`, the size of the `PlainBin`s varies, because this is intended for stream crypto, whereas a fixed `t:chunk_size()` is intended for block crypto. A `t:chunk_size()` can be changed in the return from the `t:crypto_fun()`. The value can be changed between `t:pos_integer()` and `undefined`.
""".
-doc(#{title => <<"Crypto operations for open_tar">>}).
...
-doc """
The initial `t:crypto_state()` returned from the [init_fun()](`t:init_fun/0`) is folded into repeated applications of the `t:crypto_fun()` in the [tar_crypto_spec](`t:tar_crypto_spec/0`). The binary returned from that fun is sent to the remote SFTP server and the new `t:crypto_state()` is used in the next call of the `t:crypto_fun()`.

If the `t:crypto_fun()` reurns a `t:chunk_size()`, that value is as block size for further blocks in calls to `t:crypto_fun()`.
""".
-doc(#{title => <<"Crypto operations for open_tar">>}).
...
-doc "If doing encryption, the `t:final_fun()` in the [tar_crypto_spec](`t:tar_crypto_spec/0`) is applied to the last piece of data. The `t:final_fun()` is responsible for padding (if needed) and encryption of that last piece.".
-doc(#{title => <<"Crypto operations for open_tar">>}).
...
-doc """
Opens a handle to a tar file on the server, associated with `ChannelPid`. The handle can be used for remote tar creation and extraction. The actual writing and reading is performed by calls to [erl_tar:add/3,4](`erl_tar:add/3`) and `erl_tar:extract/2`. Note: The `erl_tar:init/3` function should not be called, that one is called by this open_tar function.

For code examples see Section [SFTP Client with TAR Compression](using_ssh.md#sftp-client-with-tar-compression) in the ssh Users Guide.

The `crypto` mode option is explained in the data types section above, see [Crypto operations for open_tar](`m:ssh_sftp#crypto-operations-for-open_tar`). Encryption is assumed if the `Mode` contains `write`, and decryption if the `Mode` contains `read`.
""".
-doc(#{since => <<"OTP 17.4">>}).
...
-doc "Opens a handle to a directory on the server. The handle can be used for reading directory contents.".
...
-doc "The `pread/3,4` function reads from a specified position, combining the `position/3` and [`read/3,4`](`read/3`) functions.".
...
-doc "The `apread/4` function reads from a specified position, combining the `position/3` and `aread/3` functions.".
...
-doc "The `pwrite/3,4` function writes to a specified position, combining the `position/3` and [`write/3,4`](`write/3`) functions.".
...
-doc "Writes `data` to the file referenced by `Handle`. The file is to be opened with `write` or `append` flag. Returns `ok` if successful or `{error, reason()}` otherwise.".
...
%% TODO this ought to be a cast! Is so in all practical meaning
%% even if it is obscure!
-doc "The `apwrite/4` function writes to a specified position, combining the `position/3` and `awrite/3` functions.".
...
* __`bof | cur | eof`__ - The same as eariler with `Offset` 0, that is, `{bof, 0} | {cur, 0} | {eof, 0}`.
""".
...
-doc """
Writes file information from a `file_info` record to the file specified by `Name`. See [file:write_file_info/\[2,3]](`file:write_file_info/2`) for information about the record.
""".
...
-doc "Returns a `file_info` record from the symbolic link specified by `Name` or `Handle`. See `file:read_link_info/2` for information about the record.".
...
-doc "Creates a directory specified by `Name`. `Name` must be a full path to a new directory. The directory can only be created in an existing directory.".
...
-doc "Deletes a directory specified by `Name`. The directory must be empty before it can be successfully deleted.".
...
-doc "Lists the given directory on the server, returning the filenames as a list of strings.".
...
-doc "Writes a file to the server. The file is created if it does not exist but overwritten if it exists.".

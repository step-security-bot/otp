...
This is the interface module for the `SSH` application. The Secure Shell (SSH) Protocol is a protocol for secure remote login and other secure network services over an insecure network. See [ssh(6)](ssh_app.md#supported) for details of supported RFCs, versions, algorithms and unicode handling.

With the SSH application it is possible to start *clients* and to start *daemons* (servers).

Clients are started with `connect/2`, `connect/3` or `connect/4`. They open an encrypted connection on top of TCP/IP. In that encrypted connection one or more channels could be opened with [ssh_connection:session_channel/2,4](`ssh_connection:session_channel/2`).
...
A server (daemon) is started with [daemon/1](`daemon/2`), `daemon/2` or [daemon/3](`daemon/2`). Possible channel handlers (subsystems) are declared with the [subsystem](`t:subsystem_daemon_option/0`) option when the daemon is started.

To just run a shell on a remote machine, there are functions that bundles the needed three steps needed into one: [shell/1,2,3](`shell/1`). Similarly, to just open an sftp (file transfer) connection to a remote machine, the simplest way is to use [ssh_sftp:start_channel/1,2,3](`ssh_sftp:start_channel/1`).

To write your own client channel handler, use the behaviour `m:ssh_client_channel`. For server channel handlers use `m:ssh_server_channel` behaviour (replaces ssh_daemon_channel).

Both clients and daemons accepts options that controls the exact behaviour. Some options are common to both. The three sets are called [Client Options](`t:client_options/0`), [Daemon Options](`t:daemon_options/0`) and [Common Options](`t:common_options/0`).

The descriptions of the options uses the [Erlang Type Language](`p:system:typespec.md`) with explaining text.

> #### Note {: class=info }
> The [User's Guide](index.html) has examples and a [Getting Started](using_ssh.md) section.

## Keys and files

A number of objects must be present for the SSH application to work. Those objects are per default stored in files. The default names, paths and file formats are the same as for [OpenSSH](http://www.openssh.com). Keys could be generated with the `ssh-keygen` program from OpenSSH. See the [User's Guide](using_ssh.md#running-an-erlang-ssh-daemon).

The paths could easily be changed by options: [`user_dir`](`t:ssh_file:user_dir_common_option/0`) and [`system_dir`](`t:ssh_file:system_dir_daemon_option/0`).

A completely different storage could be interfaced by writing call-back modules using the behaviours `m:ssh_client_key_api` and/or `m:ssh_server_key_api`. A callback module is installed with the option [`key_cb`](`t:key_cb_common_option/0`) to the client and/or the daemon.
...
* Mandatory: one or more *Host key(s)*, both private and public. Default is to store them in the directory `/etc/ssh` in the files
  ...
  * `ssh_host_ecdsa_key` and `ssh_host_ecdsa_key.pub`
  ...
  The host keys directory could be changed with the option [`system_dir`](`t:ssh_file:system_dir_daemon_option/0`).
* Optional: one or more *User's public key* in case of `publickey` authorization. Default is to store them concatenated in the file `.ssh/authorized_keys` in the user's home directory.
  ...
  The user keys directory could be changed with the option [`user_dir`](`t:ssh_file:user_dir_common_option/0`).

### Clients

The keys and some other data are by default stored in files in the directory `.ssh` in the user's home directory.

The directory could be changed with the option [`user_dir`](`t:ssh_file:user_dir_common_option/0`).
...
* Optional: a list of *Host public key(s)* for previously connected hosts. This list is handled by the SSH application without any need of user assistance. The default is to store them in the file `known_hosts`.
  ...
  The [host_accepting_client_options()](`t:host_accepting_client_options/0`) are associated with this list of keys.
...
Returned by the functions [ssh_connection:session_channel/2,4](`ssh_connection:session_channel/2`).
""".
-doc(#{title => <<"Other data types">>}).
...
Returned by the functions [`connect/2,3,4`](`connect/3`) and [`ssh_sftp:start_channel/2,3`](`ssh_sftp:start_channel/2`).
""".
-doc(#{title => <<"Other data types">>}).
...
-doc "Utility function that starts the applications `crypto`, `public_key`, and `ssh`. Default type is `temporary`. For more information, see the `m:application` manual page in Kernel.".
...
%%--------------------------------------------------------------------
-doc "Stops the `ssh` application. For more information, see the `m:application` manual page in Kernel.".
...
Options = [client_options()](`t:client_options/0`)  
TcpSocket = [open_socket()](`t:open_socket/0`)  
NegotiationTimeout = timeout()  
Result = \{ok, [connection_ref()](`t:connection_ref/0`)\} | \{error, term()\}  
...
No channel is started. This is done by calling [ssh_connection:session_channel/\[2, 4]](`ssh_connection:session_channel/2`).

The `NegotiationTimeout` is in milli-seconds. The default value is `infinity` or the value of the [`connect_timeout`](`t:connect_timeout_client_option/0`) option, if present. For connection timeout, use the option [`connect_timeout`](`t:connect_timeout_client_option/0`).
""".
-doc(#{since => <<"OTP 19.0">>}).
...
Port = integer()  
TcpSocket = [open_socket()](`t:open_socket/0`)  
Options = [daemon_options()](`t:daemon_options/0`)  
HostAddress = [host()](`t:host/0`) | any  
Result = \{ok, [daemon_ref()](`t:daemon_ref/0`)\} | \{error, atom()\}  
...
Please note that by historical reasons both the `HostAddress` argument and the [gen_tcp connect_option() `{ip,Address}`](`t:gen_tcp:connect_option/0`) set the listening address. This is a source of possible inconsistent settings.
...
The handling of Erlang configurations is described in the User's Guide; see chapters [Configuration in SSH](configurations.md) and [Configuring algorithms in SSH](configure_algos.md).
""".
-doc(#{since => <<"OTP 25.1">>}).
...
-doc "Stops the listener, but leaves existing connections started by the listener operational.".
-doc(#{since => <<"OTP 21.0">>}).
...
See the [User's Guide](configure_algos.md#example_default_algorithms) for an example.
""".
-doc(#{since => <<"OTP 18.0">>}).
...
This function calls the `inet:setopts/2`, read that documentation and for [gen_tcp:option()](`t:gen_tcp:option/0`).
...
Note that in case of an Erlang/OTP SSH server (daemon) as peer, that server must have been started with the option [tcpip_tunnel_in](`t:tcpip_tunnel_in_daemon_option/0`) to allow the connection.
""".
-doc(#{since => <<"OTP 23.0">>}).
-spec tcpip_tunnel_to_server(ConnectionRef,
                             ListenHost, ListenPort,
...
Note that in case of an Erlang/OTP SSH server (daemon) as peer, that server must have been started with the option [tcpip_tunnel_out](`t:tcpip_tunnel_out_daemon_option/0`) to allow the connection.
""".
-doc(#{since => <<"OTP 23.0">>}).
-spec tcpip_tunnel_from_server(ConnectionRef,
                               ListenHost, ListenPort,
...
-doc """
HostKey = [public_key:public_key()](`t:public_key:public_key/0`)  
DigestType = [public_key:digest_type()](`t:public_key:digest_type/0`)  

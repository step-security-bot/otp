...
All functions in this Reference Manual can be used in any combination with queries using the list comprehension notation. For information about the query notation, see the `m:qlc` manual page in STDLIB.
...
* `-mnesia debug Level`. Controls the debug level of Mnesia. The possible values are as follows:
  ...
  * __`verbose`__ - Activates tracing of important debug events. These events generate `{mnesia_info, Format, Args}` system events. Processes can subscribe to these events with `mnesia:subscribe/1`. The events are always sent to the Mnesia event handler.
...
* [](){: id=dump_log_write_threshold }
  `-mnesia dump_log_write_threshold Max`. `Max` is an integer that specifies the maximum number of writes allowed to the transaction log before a new dump of the log is performed. Default is `1000` log writes.
* [](){: id=dump_log_time_threshold }
...
* `-mnesia max_transfer_size Number`. Specifies the estimated size in bytes of a single packet of data to be used when copying a table from the local node to another one. Default is `64000`.
* `-mnesia schema_location Loc`. Controls where Mnesia looks for its schema. Parameter `Loc` can be one of the following atoms:

  * __`disc`__ - Mandatory disc. The schema is assumed to be located in the Mnesia directory. If the schema cannot be found, Mnesia refuses to start. This is the old behavior.

  * __`ram`__ - Mandatory RAM. The schema resides in RAM only. At startup, a tiny new schema is generated. This default schema only contains the definition of the schema table and only resides on the local node. Since no other nodes are found in the default schema, configuration parameter `extra_db_nodes` must be used to let the node share its table definitions with other nodes.

    Parameter `extra_db_nodes` can also be used on disc based nodes.

  * __`opt_disc`__ - Optional disc. The schema can reside on disc or in RAM. If the schema is found on disc, Mnesia starts as a disc-based node and the storage type of the schema table is `disc_copies`. If no schema is found on disc, Mnesia starts as a disc-less node and the storage type of the schema table is `ram_copies`. Default value for the application parameter is `opt_disc`.
...
* __`extra_db_nodes`__ - `Value` is a list of nodes that Mnesia is to try to connect to. `ReturnValue` is those nodes in `Value` that Mnesia is connected to.
...
* __`dc_dump_limit`__ - `Value` is a number. See the description in [Section Configuration Parameters](`m:mnesia#configuration_parameters`). `ReturnValue` is the new value. Notice that this configuration parameter is not persistent. It is lost when Mnesia has stopped.
""".
...
-doc """
[](){: id=set_debug_level }
Changes the internal debug level of Mnesia. For details, see [Section Configuration Parameters](`m:mnesia#configuration_parameters`).
""".
...
-doc "Makes the transaction silently return the tuple `{aborted, Reason}`. Termination of a Mnesia transaction means that an exception is thrown to an enclosing `catch`. Thus, the expression `catch mnesia:abort(x)` does not terminate the transaction.".
...
* __`transaction`__ - Short for `{transaction, infinity}`

* __`{transaction, Retries}`__ - Calls `mnesia:transaction(Fun, Args, Retries)`. Notice that the result from `Fun` is returned if the transaction is successful (atomic), otherwise the function exits with an abort reason.

* __`sync_transaction`__ - Short for `{sync_transaction, infinity}`

* __`{sync_transaction, Retries}`__ - Calls `mnesia:sync_transaction(Fun, Args, Retries)`. Notice that the result from `Fun` is returned if the transaction is successful (atomic), otherwise the function exits with an abort reason.

* __`async_dirty`__ - Calls `mnesia:async_dirty(Fun, Args)`.

* __`sync_dirty`__ - Calls `mnesia:sync_dirty(Fun, Args)`.

* __`ets`__ - Calls `mnesia:ets(Fun, Args)`.
...
* __`write`__ - Write locks are exclusive. This means that if one transaction manages to acquire a write lock on an item, no other transaction can acquire any kind of lock on the same item.

* __`read`__ - Read locks can be shared. This means that if one transaction manages to acquire a read lock on an item, other transactions can also acquire a read lock on the same item. However, if someone has a read lock, no one can acquire a write lock at the same item. If someone has a write lock, no one can acquire either a read lock or a write lock at the same item.
...
* __`sticky_write`__ - Sticky write locks are a mechanism that can be used to optimize write lock acquisition. If your application uses replicated tables mainly for fault tolerance (as opposed to read access optimization purpose), sticky locks can be the best option available.
...
* __`{table, Tab}`__ - This acquires a lock of type `LockKind` on the entire table `Tab`.

* __`{global, GlobalKey, Nodes}`__ - This acquires a lock of type `LockKind` on the global resource `GlobalKey`. The lock is acquired on all active nodes in the `Nodes` list.
...
-doc "Works exactly like `mnesia:first/1`, but returns the last object in Erlang term order for the `ordered_set` table type. For all other table types, `mnesia:first/1` and `mnesia:last/1` are synonyms.".
...
-doc "Works exactly like `mnesia:next/2`, but returns the previous object in Erlang term order for the `ordered_set` table type. For all other table types, `mnesia:next/2` and `mnesia:prev/2` are synonyms.".
...
For a complete description of `select`, see the [ERTS](`p:erts:index.html`) User's Guide and the `m:ets` manual page in STDLIB.
...
-doc """
[](){: id=select_4 }
Matches the objects in table `Tab` using a `match_spec` as described in the [ERTS](`p:erts:index.html`) User's Guide, and returns a chunk of terms and a continuation. The wanted number of returned terms is specified by argument `NObjects`. The lock argument can be `read` or `write`. The continuation is to be used as argument to `mnesia:select/1`, if more or all answers are needed.
...
* `all`. Returns a list of all local table information. Each element is a `{InfoKey, ItemVal}` tuple.
  ...
  New `InfoItem`s can be added and old undocumented `InfoItem`s can be removed without notice.
...
* The function [mnesia:create_table(bar, \[\{attributes, 3.14\}])](`create_table/2`) returns the tuple `{aborted,Reason}`, where `Reason` is the tuple `{bad_type,bar,3.14000}`.
* The function [mnesia:error_description(Reason)](`error_description/1`) returns the term `{"Bad type on some provided arguments",bar,3.14000}`, which is an error description suitable for display.
""".
...
* `all`. Returns a list of all local system information. Each element is a `{InfoKey, InfoVal}` tuple.
  ...
  New `InfoKey`s can be added and old undocumented `InfoKey`s can be removed without notice.
...
* `{default_op, Operation}`, where `Operation` is either of the operations `skip_tables`, `clear_tables`, `keep_tables`, or `recreate_tables`. The default operation specifies which operation that is to be used on tables from the backup that is not specified in any of the mentioned lists. If omitted, operation `clear_tables` is used.
...
-doc "Calls `mnesia:transform_table(Tab, Fun, NewAttributeList, RecName)`, where `RecName` is `mnesia:table_info(Tab, record_name)`.".
...
-doc "`Majority` must be a boolean. Default is `false`. When `true`, a majority of the table replicas must be available for an update to succeed. When used on fragmented tables, `Tab` must be the base table name. Directly changing the majority setting on individual fragments is not allowed.".
...
-doc """
[](){: id=dump_log }
Performs a user-initiated dump of the local log file. This is usually not necessary, as Mnesia by default manages this automatically. See configuration parameters [dump_log_time_threshold](`m:mnesia#dump_log_time_threshold`) and [dump_log_write_threshold](`m:mnesia#dump_log_write_threshold`).
""".
-spec dump_log() -> 'dumped'.
dump_log() ->
    mnesia_controller:sync_dump_log(user).

-doc "Ensures that the local transaction log file is synced to disk. On a single node system, data written to disk tables since the last dump can be lost if there is a power outage. See `dump_log/0`.".
-doc(#{since => <<"OTP 17.0">>}).
...
-doc """
[](){: id=subscribe }
Ensures that a copy of all events of type `EventCategory` is sent to the caller. The available event types are described in the [User's Guide](mnesia_chap5.md#event_handling).
""".
...
-doc """
[](){: id=table }
Returns a Query List Comprehension (QLC) query handle, see the `m:qlc` manual page in STDLIB. The module `qlc` implements a query language that can use Mnesia tables as sources of data. Calling `mnesia:table/1,2` is the means to make the `mnesia` table `Tab` usable to QLC.

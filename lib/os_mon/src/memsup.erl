...
`memsup` is a process which supervises the memory usage for the system and for individual processes. It is part of the OS_Mon application, see [os_mon(6)](os_mon_app.md). Available for Unix and Windows.
...
Alarms are reported to the SASL alarm handler, see `m:alarm_handler`. To set an alarm, `alarm_handler:set_alarm(Alarm)` is called where `Alarm` is either of the alarms specified above.

The alarms are cleared automatically when the alarm cause is no longer valid.

The function [get_memory_data()](`get_memory_data/0`) can be used to retrieve the result of the latest periodic memory check.

There is also a interface to system dependent memory data, [get_system_memory_data()](`get_system_memory_data/0`). The result is highly dependent on the underlying operating system and the interface is targeted primarily for systems without virtual memory. However, the output on other systems is still valid, although sparse.
...
* __`memory_check_interval = int()>0`__ - The time interval, in minutes, for the periodic memory check. The default is one minute.

* __`system_memory_high_watermark = float()`__ - The threshold, as percentage of system memory, for how much system memory can be allocated before the corresponding alarm is set. The default is 0.80 (80%).

* __`process_memory_high_watermark = float()`__ - The threshold, as percentage of system memory, for how much system memory can be allocated by one Erlang process before the corresponding alarm is set. The default is 0.05 (5%).

* __`memsup_helper_timeout = int()>0`__ - A timeout, in seconds, for how long the `memsup` process should wait for a result from a memory check. If the timeout expires, a warning message `"OS_MON (memsup) timeout"` is issued via `error_logger` and any pending, synchronous client calls will return a dummy value. Normally, this situation should not occur. There have been cases on Linux, however, where the pseudo file from which system data is read is temporarily unavailable when the system is heavily loaded.

  The default is 30 seconds.

* __`memsup_system_only = bool()`__ - Specifies whether the `memsup` process should only check system memory usage (`true`) or not. The default is `false`, meaning that information regarding both system memory usage and Erlang process memory usage is collected.
...
* __`total_memory`__ - The total amount of memory available to the Erlang emulator, allocated and free. May or may not be equal to the amount of memory configured in the system.

* __`available_memory`__ - Informs about the amount memory that is available for increased usage if there is an increased memory need. This value is not based on a calculation of the other provided values and should give a better value of the amount of memory that actually is available than calculating a value based on the other values reported. This value is currently only present on newer Linux kernels. If this value is not available on Linux, you can use the sum of `cached_memory`, `buffered_memory`, and `free_memory` as an approximation.

* __`free_memory`__ - The amount of free memory available to the Erlang emulator for allocation.

* __`system_total_memory`__ - The amount of memory available to the whole operating system. This may well be equal to `total_memory` but not necessarily.

* __`buffered_memory`__ - The amount of memory the system uses for temporary storing raw disk blocks.

* __`cached_memory`__ - The amount of memory the system uses for cached files read from disk. On Linux, also memory marked as reclaimable in the kernel slab allocator will be added to this value.

* __`total_swap`__ - The amount of total amount of memory the system has available for disk swap.

* __`free_swap`__ - The amount of memory the system has available for disk swap.
...
The change will take effect after the next memory check and is non-persistent. That is, in case of a process restart, this value is forgotten and the default value will be used. See [Configuration](`m:memsup#config`) above.
""".
...
-doc "Returns the threshold, in percent, for process memory allocation.".
...
The change will take effect during the next periodic memory check and is non-persistent. That is, in case of a process restart, this value is forgotten and the default value will be used. See [Configuration](`m:memsup#config`) above.
""".
...
-doc "Returns the threshold, in percent, for system memory allocation.".
...
The change will take effect for the next memory check and is non-persistent. That is, in the case of a process restart, this value is forgotten and the default value will be used. See [Configuration](`m:memsup#config`) above.
""".

...
`os_sup` is a process providing a message passing service from the operating system to the error logger in the Erlang runtime system. It is part of the OS_Mon application, see [os_mon(6)](os_mon_app.md). Available for Solaris and Windows.
...
The `nteventlog` module is used to implement `os_sup`. See `m:nteventlog`. Note that the start functions of `nteventlog` does not need to be used, as in this case the process is started automatically as part of the OS_Mon supervision tree.

OS messages are formatted as a tuple `{Time, Category, Facility, Severity, Message}`:

* __`Time = {MegaSecs, Secs, MicroSecs}`__ - A time stamp as returned by the BIF `now()`.

* __`Category = string()`__ - Usually one of `"System"`, `"Application"` or `"Security"`. Note that the NT eventlog viewer has another notion of category, which in most cases is totally meaningless and therefore not imported into Erlang. What is called a category here is one of the main three types of events occurring in a normal NT system.

* __`Facility = string()`__ - The source of the message, usually the name of the application that generated it. This could be almost any string. When matching messages from certain applications, the version number of the application may have to be accounted for. This is what the NT event viewer calls "source".

* __`Severity = string()`__ - One of `"Error"`, `"Warning"`, `"Informational"`, `"Audit_Success"`, `"Audit_Faulure"` or, in case of a currently unknown Windows NT version `"Severity_Unknown"`.

* __`Message = string()`__ - Formatted exactly as it would be in the NT eventlog viewer. Binary data is not imported into Erlang.

[](){: id=config }
## Configuration

* __`os_sup_mfa = {Module, Function, Args}`__ - The callback function to use. `Module` and `Function` are atoms and `Args` is a list of terms. When an OS message `Msg` is received, this function is called as `apply(Module, Function, [Msg | Args])`.

  Default is `{os_sup, error_report, [Tag]}` which will send the event to the error logger using [error_logger:error_report(Tag, Msg)](`error_logger:error_report/2`). `Tag` is the value of `os_sup_errortag`, see below.

* __`os_sup_errortag = atom()`__ - This parameter defines the error report type used when messages are sent to error logger using the default callback function. Default is `std_error`, which means the events are handled by the standard event handler.

* __`os_sup_enable = bool()`__ - Solaris only. Defines if the service should be enabled (and disabled) inside (`true`) or outside (`false`) `os_sup`. For backwards compatibility reasons, the default is `true`. The recommended value is `false`, as the Erlang emulator should normally not be run with `root` privileges, as is required for enabling the service.

* __`os_sup_own = string()`__ - Solaris only. Defines the directory which contains the backup copy and the Erlang specific configuration files for `syslogd`, and a named pipe to receive the messages from `syslogd`. Default is `"/etc"`.

* __`os_sup_syslogconf = string()`__ - Solaris only. Defines the full name of the configuration file for `syslogd`. Default is `"/etc/syslog.conf"`.

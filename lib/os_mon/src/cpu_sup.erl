...
`cpu_sup` is a process which supervises the CPU load and CPU utilization. It is part of the OS_Mon application, see [os_mon(6)](os_mon_app.md). Available for Unix, although CPU utilization values (`util/0,1`) are only available for Solaris, Linux, FreeBSD and OpenBSD.
...
* __`detailed`__ - The returned `UtilDesc`(s) will be even more detailed.

* __`per_cpu`__ - Each CPU will be specified separately (assuming this information can be retrieved from the operating system), that is, a list with one `UtilDesc` per CPU will be returned.

Description of `UtilDesc = {Cpus, Busy, NonBusy, Misc}`:

* __`Cpus`__ - If the `detailed` and/or `per_cpu` option is given, this is the CPU number, or a list of the CPU numbers.

  If not, this is the atom `all` which implies that the `UtilDesc` contains information about all CPUs.

* __`Busy`__ - If the `detailed` option is given, this is a list of `{State, Share}` tuples, where each tuple contains information about a processor state that has been identified as a busy processor state (see below). The atom `State` is the name of the state, and the float `Share` represents the percentage share of the CPU cycles spent in this state since the last call to `util/0` or `util/1`.
...
* __`NonBusy`__ - Similar to `Busy`, but for processor states that have been identified as non-busy (see below).

* __`Misc`__ - Currently unused; reserved for future use.

Currently these processor states are identified as busy:

* __`user`__ - Executing code in user mode.

* __`nice_user`__ - Executing code in low priority (nice) user mode. This state is currently only identified on Linux.

* __`kernel`__ - Executing code in kernel mode.

Currently these processor states are identified as non-busy:

* __`wait`__ - Waiting. This state is currently only identified on Solaris.

...
for app in $(ls -d lib/${APPS}/ebin | awk -F "/" '{print $2}'); do
    ...
    APP="lib/$app" ex_doc -f html $app "${VSN}" "lib/$app/ebin" -o "docs/$app" -c $EX_DOCS_EXS || exit
done
...
if [ "$APPS" = '*' ] || [ $APPS = 'erts' ]; then
    ...
    APP=$app ex_doc -f html $app "${VSN}" "$app/preloaded/ebin" -o "docs/$app" -c $EX_DOCS_EXS || exit
fi
...
if [ "$APPS" = '*' ] || [ $APPS = 'system' ]; then
    ...
    APP=$app ex_doc -f html "Erlang System Documentation" "${VSN}" "lib/erl_interface/ebin" -o "docs/system" -c system/doc/ex_doc.exs || exit
fi

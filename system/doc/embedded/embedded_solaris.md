...
File `/export/home/otpuser/otp/bin/start` referred to in the above script is precisely the `start` script described in *Starting Erlang*. The script variable `$OTPROOT` in that `start` script corresponds to the following example path used in this section:
...
To take Erlang down gracefully, see the [`erl_call(1)`](`p:erl_interface:erl_call_cmd.md`) manual page in `erl_interface` for details on the use of `erl_call`. However, that requires that Erlang runs as a distributed node, which is not always the case.
...
See the `m:os_mon` application, the `m:application` manual page in Kernel, and the [`erl(1)`](`p:erts:erl_cmd.md`) manual page in ERTS.
...
The `<RELDIR>` directory is where new release packets are installed, and where the release handler keeps information about releases. For more information, see the `m:release_handler` manual page in SASL.

...
void *beamasm_codegen(void *ba, BeamCodeHeader *in_hdr, BeamCodeHeader **out_hdr);
...
void beamasm_embed_rodata(void *ba, char *labelName, const char* buff, size_t size);
...
enum erts_asm_bp_flag {
    ...
    ERTS_ASM_BP_FLAG_BP_NIF_CALL_NIF_EARLY = ERTS_ASM_BP_FLAG_CALL_NIF_EARLY | ERTS_ASM_BP_FLAG_BP
};

static inline void
erts_asm_bp_set_flag(ErtsCodeInfo *ci, enum erts_asm_bp_flag flag) {
...
static inline enum erts_asm_bp_flag
erts_asm_bp_get_flags(ErtsCodeInfo *ci) {
    byte *codebytes = (byte*)erts_codeinfo_to_code(ci);
    return (enum erts_asm_bp_flag)codebytes[2];
}

static inline void
erts_asm_bp_unset_flag(ErtsCodeInfo *ci, enum erts_asm_bp_flag flag) {
    BeamInstr volatile *code_ptr = erts_codeinfo_to_code(ci);
    BeamInstr code = *code_ptr;
    byte *codebytes = (byte*)&code;
    Uint32 *code32 = (Uint32*)(codebytes+4);

	...
	case TTBEncode:
		...
		return_normal:
			...
			erts_bin_free(context_b);
                     ...
                     *
                     * Note that we do not bump refc or link it into the
                     * off-heap list until we know it will be used. */
                    ...
                    if (referenced_cbin) {
                        ...
                        erts_refc_inc(&result_bin->intern.refc, 1);
                    } else {
	...
	case BITSTRING_DEF:
            {
                ...
                ERTS_PIN_BITSTRING(obj, br, base, offset, size);
...
#ifdef HARD_DEBUG
    /* This is incorrect as the base of the binary can point elsewhere in rare
     * cases, e.g. for NIF resource binaries. Nevertheless, this assertion is
     * very effective at finding errors in their absence. */
    ASSERT(ErtsInArea(ptr, (byte *)&bin->orig_bytes[0], bin->orig_size));
#endif

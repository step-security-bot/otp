...
#define ERTS_BR_OVERHEAD(oh, pb)                                              \
    do {                                                                      \
        (oh)->overhead += ((pb)->val)->orig_size / sizeof(Eterm);             \
    } while(0)
...
#define ERTS_PIN_BITSTRING(Bin, Ref, Base, Offset, Size)                      \
    do {                                                                      \
...
#define ERTS_GET_BITSTRING_REF(Bin, Ref, Base, Offset, Size)                  \
    do {                                                                      \
        const Eterm *_unboxed = bitstring_val(Bin);                           \
        Size = _unboxed[1];                                                   \
        if (*_unboxed == HEADER_SUB_BITS) {                                   \

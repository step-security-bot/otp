...
void BeamGlobalAssembler::emit_bs_get_tail_shared() {
    ...
    /* HACK: erts_build_sub_bitstring expects an untagged pointer instead of a
     * term. This is in preparation of the upcoming match context rewrite. */
    a.and_(ARG2, imm(~TAG_PTR_MASK__));
    runtime_call<5>(erts_build_sub_bitstring);
...
void BeamModuleAssembler::emit_i_bs_match_test_heap(ArgLabel const &Fail,
    ...
    for (int i = 0; i < count; i++) {
        ...
        case BsmSegment::action::GET_BITSTRING: {
            ...
            /* HACK: erts_build_sub_bitstring expects an untagged pointer
             * instead of a term. This is in preparation of the upcoming match
             * context rewrite. */
            a.and_(ARG2, imm(~TAG_PTR_MASK__));
            runtime_call<5>(erts_build_sub_bitstring);

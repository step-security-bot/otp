...
static Eterm do_split_single_result(Process *p, Eterm subject, BinaryFindContext **ctxp)
{
    ...
    ERTS_GET_BITSTRING_REF(subject, br, base, subject_offset, subject_size);
    ...
    if ((ctx->flags & (BF_FLAG_SPLIT_TRIM | BF_FLAG_SPLIT_TRIM_ALL)) &&
        ...
        if ((ctx->flags & BF_FLAG_SPLIT_TRIM_ALL) && (pos == 0)) {
            ...
            first = erts_build_sub_bitstring(&hp, br, base,
                                             subject_offset,
                                             NBITS(pos));
        }

        rest = erts_build_sub_bitstring(&hp, br, base,
...
static Eterm do_split_global_result(Process *p, Eterm subject, BinaryFindContext **ctxp)
{
    ...
    ERTS_GET_BITSTRING_REF(subject, br, base, offset, size);
...
BIF_RETTYPE erts_binary_part(Process *p, Eterm binary, Eterm epos, Eterm elen)
{
    ...
    ERTS_GET_BITSTRING_REF(binary, br, base, offset, size);
    ...
    result = erts_build_sub_bitstring(&hp, br, base,
                                      offset + NBITS(pos),
                                      NBITS(len));

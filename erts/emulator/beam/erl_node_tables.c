...
static int insert_monitor(ErtsMonitor *mon, void *idp)
{
    Eterm id = *((Eterm *) idp);
...
static int clear_visited_monitor(ErtsMonitor *mon, void *p)
{
    ErtsMonitorData *mdp = erts_monitor_to_data(mon);
    mdp->origin.flags &= ~ERTS_ML_FLG_DBG_VISITED;
    return 1;
...
static int insert_link(ErtsLink *lnk, void *idp)
{
    Eterm id = *((Eterm *) idp);
...
static int clear_visited_link(ErtsLink *lnk, void *p)
{
    ErtsLinkData *ldp = erts_link_to_data(lnk);
    ldp->a.flags &= ~ERTS_ML_FLG_DBG_VISITED;
    return 1;

	...
	L_iter_list:
		...
		*buf++ = unsigned_val(obj);
		...
		goto L_type_error;
            ...
            ERTS_DECLARE_DUMMY(Eterm underlying_binary);
            Uint size_in_bytes;
            Uint offset, size;
            byte *base;
            BinRef *br;
                  ...
                  Uint blimit)
{
    ...
    ERTS_GET_BITSTRING_REF(obj, br, base, offset, size);
...
static void iol2v_init(iol2v_state_t *state, Process *process, Eterm input) {
    ...
    state->off_heap.first = MSO(process).first;
...
static void iol2v_finish_acc(iol2v_state_t *state) {
...
static Uint iol2v_expand_acc(iol2v_state_t *state, UWord extra) {
    ...
    br->next = state->off_heap.first;
    state->off_heap.first = (struct erl_off_heap_header*)br;
...
static int iol2v_append_binary(iol2v_state_t *state, Eterm bin_term) {
    ...
    ERTS_PIN_BITSTRING(bin_term, br, base, offset, size);

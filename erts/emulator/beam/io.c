		  ...
		  Eterm *retvalp)
{
    ...
    if (is_bitstring(data)) {
        ...
        ERTS_PIN_BITSTRING(data, br, basep, offset, size);
	...
	if (!try_call) {
		...
		if (r == ERTS_IOLIST_TO_BUF_TYPE_ERROR) { /* Type error */
		    erts_free(ERTS_ALC_T_TMP, basep);
		    return ERTS_PORT_OP_BADARG;
		}
		else {
    ...
    ASSERT(is_bitstring(data) || (!try_call ^ tmp_alloced));
    ...
    if (copy) {
        ...
        if (tmp_alloced) {
            erts_free(ERTS_ALC_T_TMP, old_basep);
        }
    }

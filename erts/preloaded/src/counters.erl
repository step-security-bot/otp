...
* __`atomics` (Default)__ - Counters will be sequentially consistent. If write operation A is done sequentially before write operation B, then a concurrent reader may see the result of none of them, only A, or both A and B. It cannot see the result of only B.

* __`write_concurrency`__ - This is an optimization to achieve very efficient concurrent [`add`](`add/3`) and [`sub`](`sub/3`) operations at the expense of potential read inconsistency and memory consumption per counter.
  ...
  The typical use case for `write_concurrency` is when concurrent calls to [`add`](`add/3`) and [`sub`](`sub/3`) toward the same counters are very frequent, while calls to [`get` ](`get/2`)and [`put`](`put/3`) are much less frequent. The lack of absolute read consistency must also be acceptable.
...
> #### Note {: class=info }
> Despite its name, the `write_concurrency` optimization does not improve `put`. A call to `put` is a relatively heavy operation compared to the very lightweight and scalable [`add`](`add/3`) and [`sub`](`sub/3`). The cost for a `put` with `write_concurrency` is like a [`get` ](`get/2`)plus a `put` without `write_concurrency`.
""".
-doc(#{since => <<"OTP 21.2">>}).
...
* __`size`__ - The number of counters in the array.

* __`memory`__ - Approximate memory consumption for the array in bytes.
""".
-doc(#{since => <<"OTP 21.2">>}).

...
* __`{signed, boolean()}`__ - Indicate if the elements of the array will be treated as signed or unsigned integers. Default is `true` (signed).
...
-doc "Atomically replaces the value of the atomic with `Desired` and returns the value it held previously.".
-doc(#{since => <<"OTP 21.2">>}).
...
-doc "Atomically compares the atomic with `Expected`, and if those are equal, set atomic to `Desired`. Returns `ok` if `Desired` was written. Returns the actual atomic value if not equal to `Expected`.".
-doc(#{since => <<"OTP 21.2">>}).
...
* __`size`__ - The number of atomics in the array.

* __`max`__ - The highest possible value an atomic in this array can hold.

* __`min`__ - The lowest possible value an atomic in this array can hold.

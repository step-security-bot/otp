...
Term lookup (using `get/1`) is done in constant time and without taking any locks, and the term is __not__ copied to the heap (as is the case with terms stored in ETS tables).
...
The current implementation of persistent terms uses the literal [allocator](erts_alloc.md) also used for literals (constant terms) in BEAM code. By default, 1 GB of virtual address space is reserved for literals in BEAM code and persistent terms. The amount of virtual address space reserved for literals can be changed by using the [`+MIscs option`](erts_alloc.md#miscs) when starting the emulator.
...
If there existed a previous persistent term associated with key `Key`, a global GC has been initiated when `erase/1` returns. See [Description](`m:persistent_term`).
""".
-doc(#{since => <<"OTP 21.2">>}).
...
-doc "Retrieve the keys and values for all persistent terms. The keys will be copied to the heap for the process calling `get/0`, but the values will not.".
-doc(#{since => <<"OTP 21.2">>}).
-spec get() -> List when
      List :: [{key(),value()}].
get() ->
...
* __`count`__ - The number of persistent terms.

* __`memory`__ - The total amount of memory (measured in bytes) used by all persistent terms.
""".
-doc(#{since => <<"OTP 21.2">>}).
-spec info() -> Info when
...
If there existed a previous persistent term associated with key `Key`, a global GC has been initiated when `put/2` returns. See [Description](`m:persistent_term`).
""".
-doc(#{since => <<"OTP 21.2">>}).

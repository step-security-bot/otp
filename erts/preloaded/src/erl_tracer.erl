...
> #### Note {: class=info }
> All functions in this behavior must be implemented as NIFs. This limitation can be removed in a future releases. An [example tracer module NIF](`m:erl_tracer#example`) implementation is provided at the end of this page.
...
-doc "The different trace tags that the tracer is called with. Each trace tag is described in detail in [`Module:trace/5`](`c:trace/5`).".
...
* __`timestamp`__ - If set the tracer has been requested to include a time stamp.

* __`extra`__ - If set the tracepoint has included additional data about the trace event. What the additional data is depends on which `TraceTag` has been triggered. The `extra` trace data corresponds to the fifth element in the trace tuples described in [erlang:trace/3](`m:erlang#trace_3_trace_messages`).

* __`match_spec_result`__ - If set the tracer has been requested to include the output of a match specification that was run.

* __`scheduler_id`__ - If set the scheduler id is to be included by the tracer.
""".
...
-doc "The state specified when calling [`erlang:trace(PidPortSpec,true,[{tracer,Module,TracerState}])`](`erlang:trace/3`). The tracer state is an immutable value that is passed to `erl_tracer` callbacks and is to contain all the data that is needed to generate the trace event.".
-type tracer_state() :: term().

-doc """
TraceTag = [trace_tag()](`t:trace_tag/0`) | trace_status  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
...
-callback enabled(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_call()](`t:trace_tag_call/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`call | return_to`](`erlang:trace/3`) is triggered.

If `c:enabled_call/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback enabled_call(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_gc()](`t:trace_tag_gc/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`garbage_collection`](`erlang:trace/3`) is triggered.

If `c:enabled_garbage_collection/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback enabled_garbage_collection(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_ports()](`t:trace_tag_ports/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`ports`](`erlang:trace/3`) is triggered.

If `c:enabled_ports/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback enabled_ports(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_procs()](`t:trace_tag_procs/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`procs`](`erlang:trace/3`) is triggered.

If `c:enabled_procs/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback enabled_procs(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_receive()](`t:trace_tag_receive/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`'receive'`](`erlang:trace/3`) is triggered.

If `c:enabled_receive/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback enabled_receive(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_running_ports()](`t:trace_tag_running_ports/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`running_ports`](`erlang:trace/3`) is triggered.

If `c:enabled_running_ports/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback enabled_running_ports(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_running_procs()](`t:trace_tag_running_procs/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`running_procs | running`](`erlang:trace/3`) is triggered.

If `c:enabled_running_procs/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback enabled_running_procs(TraceTag, TracerState, Tracee) -> Result when
      ...
      Result :: trace | discard | remove.
-doc """
TraceTag = [trace_tag_send()](`t:trace_tag_send/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
Result = trace | discard | remove  

This callback is called whenever a tracepoint with trace flag [`send`](`erlang:trace/3`) is triggered.

If `c:enabled_send/3` is undefined, [`Module:enabled/3`](`c:enabled/3`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
-doc """
TraceTag = [trace_tag()](`t:trace_tag/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
TraceTerm = term()  
Opts = [trace_opts()](`t:trace_opts/0`)  
Result = ok  

This callback is called when a tracepoint is triggered and the [`Module:enabled/3`](`c:enabled/3`) callback returned `trace`. In it any side effects needed by the tracer are to be done. The tracepoint payload is located in the `TraceTerm`. The content of the `TraceTerm` depends on which `TraceTag` is triggered. `TraceTerm` corresponds to the fourth element in the trace tuples described in [`erlang:trace/3`](`m:erlang#trace_3_trace_messages`).
...
For more information on what `Label` and `SeqTraceInfo` can be, see `m:seq_trace`.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_call/3`](`c:enabled_call/3`) callback returned `trace`.

If `c:trace_call/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_garbage_collection/3`](`c:enabled_garbage_collection/3`) callback returned `trace`.

If `c:trace_garbage_collection/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback trace_garbage_collection(TraceTag, TracerState, Tracee, TraceTerm, Opts) -> Result when
      ...
      Result :: ok.
-doc """
TraceTag = [trace_tag()](`t:trace_tag_ports/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
TraceTerm = term()  
Opts = [trace_opts()](`t:trace_opts/0`)  
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_ports/3`](`c:enabled_ports/3`) callback returned `trace`.

If `c:trace_ports/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
-callback trace_ports(TraceTag, TracerState, Tracee, TraceTerm, Opts) -> Result when
      ...
      Result :: ok.
-doc """
TraceTag = [trace_tag()](`t:trace_tag_procs/0`)  
TracerState = term()  
Tracee = [tracee()](`t:tracee/0`)  
TraceTerm = term()  
Opts = [trace_opts()](`t:trace_opts/0`)  
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_procs/3`](`c:enabled_procs/3`) callback returned `trace`.

If `c:trace_procs/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_receive/3`](`c:enabled_receive/3`) callback returned `trace`.

If `c:trace_receive/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_running_ports/3`](`c:enabled_running_ports/3`) callback returned `trace`.

If `c:trace_running_ports/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_running_procs/3`](`c:enabled_running_procs/3`) callback returned `trace`.

If `c:trace_running_procs/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).
...
This callback is called when a tracepoint is triggered and the [`Module:enabled_send/3`](`c:enabled_send/3`) callback returned `trace`.

If `c:trace_send/5` is undefined, [`Module:trace/5`](`c:trace/5`) is called instead.
""".
-doc(#{title => <<"Callback Functions">>,since => <<"OTP 19.0">>}).

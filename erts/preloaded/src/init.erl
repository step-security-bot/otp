...
* __`--`__ - Everything following `--` up to the next flag is considered plain arguments and can be retrieved using `get_plain_arguments/0`.

* __`-code_path_choice Choice`__ - Can be set to `strict` or `relaxed`. It controls how each directory in the code path is to be interpreted:
  ...
  * `init` is to be more relaxed and try to find a suitable directory if it can choose from a regular `ebin` directory and an `ebin` directory in an archive file.
  ...
  This flag is particular useful when you want to elaborate with code loading from archives without editing the `boot script`. For more information about interpretation of boot scripts, see [`script(4)`](`p:sasl:script.md`). The flag has also a similar effect on how the code server works; see `m:code`.

* __`-epmd_module Module`__ - Specifies the module to use for registration and lookup of node names. Defaults to `erl_epmd`.

* __`-eval Expr`__ - Scans, parses, and evaluates an arbitrary expression `Expr` during system initialization. If any of these steps fail (syntax error, parse error, or exception during evaluation), Erlang stops with an error message. In the following example Erlang is used as a hexadecimal calculator:
...
* __`-extra`__ - Everything following `-extra` is considered plain arguments and can be retrieved using `get_plain_arguments/0`.
...
* __`-S Mod [Func [Arg1, Arg2, ...]]`__ - Evaluates the specified function call during system initialization. `Func` defaults to `start`. If no arguments are provided, the function is assumed to be of arity 0. Otherwise it is assumed to be of arity 1, taking the list `[Arg1,Arg2,...]` as argument. All arguments are passed as strings. If an exception is raised, Erlang stops with an error message.
...
-doc "Returns all command-line flags and the system-defined flags, see `get_argument/1`.".
...
* __`root`__ - The installation directory of Erlang/OTP, `$ROOT`:
...
* __`progname`__ - The name of the program which started Erlang:
...
* __`home`{: id=home }__ - The home directory (on Unix, the value of $HOME):
...
-doc "Gets the identity of the boot script used to boot the system. `Id` can be any Erlang term. In the delivered boot scripts, `Id` is `{Name, Vsn}`. `Name` and `Vsn` are strings.".
...
-doc """
All applications are taken down smoothly, all code is unloaded, and all ports are closed before the system terminates. If command-line flag `-heart` was specified, the `heart` program tries to reboot the system. For more information, see `m:heart`.
...
-doc """
All applications are taken down smoothly, all code is unloaded, and all ports are closed before the system terminates by calling `halt(Status)`. If command-line flag `-heart` was specified, the `heart` program is terminated before the Erlang node terminates. For more information, see `m:heart`.
...
`BootArgs` are all command-line arguments except the emulator flags, that is, flags and plain arguments; see [`erl(1)`](erl_cmd.md).

`init` interprets some of the flags, see section [Command-Line Flags](`m:init#flags`) below. The remaining flags ("user flags") and plain arguments are passed to the `init` loop and can be retrieved by calling `get_arguments/0` and `get_plain_arguments/0`, respectively.
""".

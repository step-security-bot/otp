...
* __`badarg`__ - Bad argument.

* __`not_initialized`__ - The stream hasn't been initialized, eg. if `inflateInit/1` wasn't called prior to a call to `inflate/2`.

* __`not_on_controlling_process`__ - The stream was used by a process that doesn't control it. Use `set_controlling_process/2` if you need to transfer a stream to a different process.

* __`data_error`__ - The data contains errors.

* __`stream_error`__ - Inconsistent stream state.

* __`{need_dictionary,Adler32}`__ - See `inflate/2`.
""".
...
* __`Level`__ - Compression level to use:
...
* __`MemLevel`__ - Specifies how much memory is to be allocated for the internal compression state: `MemLevel`=1 uses minimum memory but is slow and reduces compression ratio; `MemLevel`=9 uses maximum memory for optimal speed. Defaults to 8.

* __`Strategy`__ - Tunes the compression algorithm. Use the following values:
...
This function must be called immediately after [`deflateInit/1,2,6`](`deflateInit/1`) or `deflateReset/1`, before any call of `deflate/3`.
...
-doc "Equivalent to `deflateEnd/1` followed by [`deflateInit/1,2,6`](`deflateInit/1`), but does not free and reallocate all the internal compression state. The stream keeps the same compression level and any other attributes.".
...
-doc "Same as `deflate(Z, Data, none)`.".
...
`Flush` can be set to `finish` immediately after [`deflateInit`](`deflateInit/1`) if all compression is to be done in one step.
...
-doc "Ends the deflate session and cleans all data used. Notice that this function throws a `data_error` exception if the last call to `deflate/3` was not called with `Flush` set to `finish`.".
...
-doc """
Returns the decompression dictionary currently in use by the stream. This function must be called between [`inflateInit/1,2`](`inflateInit/1`) and [`inflateEnd`](`inflateEnd/1`).
...
-doc "Equivalent to `inflateEnd/1` followed by `inflateInit/1`, but does not free and reallocate all the internal decompression state. The stream will keep attributes that could have been set by `inflateInit/1,2`.".
...
-doc """
> #### Warning {: class=warning }
> This function is deprecated and will be removed in a future release. Use [`safeInflate/2` ](`safeInflate/2`)instead.

Like `inflate/2`, but decompresses no more data than will fit in the buffer configured through [`setBufSize/2` ](`setBufSize/2`). Is is useful when decompressing a stream with a high compression ratio, such that a small amount of compressed input can expand up to 1000 times.
...
""".
-doc(#{deprecated =>
           <<"zlib:inflateChunk/2 is deprecated and will be removed in OTP 27; use safeInflate/2 instead">>,
       since => <<"OTP 18.0">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:inflateChunk/1 is deprecated and will be removed in OTP 27; use safeInflate/2 instead">>,
       since => <<"OTP 18.0">>}).
...
-doc "Ends the inflate session and cleans all data used. Notice that this function throws a `data_error` exception if no end of stream was found (meaning that not all data has been uncompressed).".
...
""".
-doc(#{deprecated =>
           <<"zlib:setBufSize/2 is deprecated and will be removed in OTP 27; this function will be removed in a future release">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:getBufSize/1 is deprecated and will be removed in OTP 27; this function will be removed in a future release">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:crc32/1 is deprecated and will be removed in OTP 27; use erlang:crc32/1 on the uncompressed data instead">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:crc32/2 is deprecated and will be removed in OTP 27; use erlang:crc32/1 instead">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:crc32/3 is deprecated and will be removed in OTP 27; use erlang:crc32/2 instead">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:crc32_combine/4 is deprecated and will be removed in OTP 27; use erlang:crc32_combine/3 instead">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:adler32/2 is deprecated and will be removed in OTP 27; use erlang:adler32/1 instead">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:adler32/3 is deprecated and will be removed in OTP 27; use erlang:adler32/2 instead">>}).
...
""".
-doc(#{deprecated =>
           <<"zlib:adler32_combine/4 is deprecated and will be removed in OTP 27; use erlang:adler_combine/3 instead">>}).

...
Predefined types are listed in the [Predefined datatypes](`m:erlang#predefined-datatypes`) section of this reference manual and in the [Types and Function Specifications](`p:system:typespec.md`) section of the Erlang Reference Manual.
...
%% Built-in datatypes
-doc "All possible Erlang terms. Synonym for `t:term()`.".
-doc(#{title => <<"Predefined datatypes">>}).
-type any() :: any().
-doc "The arity of a function or type.".
-doc(#{title => <<"Predefined datatypes">>}).
-type arity() :: arity().
-doc "An Erlang [atom](`p:system:data_types.md#atom`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type atom() :: atom().
-doc "An Erlang [binary](`p:system:data_types.md#bit-strings-and-binaries`), that is, a bitstring with a size divisible by 8.".
-doc(#{title => <<"Predefined datatypes">>}).
-type binary() :: <<_:_*8>>.
-doc "An Erlang [bitstring](`p:system:data_types.md#bit-strings-and-binaries`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type bitstring() :: <<_:_*1>>.
-type bool() :: boolean().
-doc "A [boolean](`p:system:data_types.md#boolean`) value.".
-doc(#{title => <<"Predefined datatypes">>}).
...
-doc "The [dynamic](`p:system:typespec.md#dynamic`) type.".
-doc(#{title => <<"Predefined datatypes">>}).
-type dynamic() :: dynamic().
-doc "An Erlang [float](`p:system:data_types.md#number`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type float() :: float().
-doc "An Erlang [fun](`p:system:data_types.md#fun`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type function() :: fun().
-doc "An unique identifier for some entity, for example a [process](`p:system:ref_man_processes.md`), [port](`p:system:ports.md#ports`) or [monitor](`monitor/2`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type identifier() :: pid() | port() | reference().
-doc "An Erlang [integer](`p:system:data_types.md#number`).".
-doc(#{title => <<"Predefined datatypes">>}).
...
To convert an iodata() term to binary() you can use [iolist_to_binary/2](`iolist_to_binary/1`). To transcode a [string()](`t:string/0`) or [unicode:chardata()](`t:unicode:chardata/0`) to iodata() you can use `unicode:characters_to_binary/1`.
""".
-doc(#{title => <<"Predefined datatypes">>}).
...
In most use cases you want to use [iodata()](`t:iodata/0`) instead of this type.
""".
-doc(#{title => <<"Predefined datatypes">>}).
-type iolist() :: maybe_improper_list(byte() | binary() | iolist(), binary() | []).
-doc "An Erlang [list](`p:system:data_types.md#list`) containing terms of any type.".
-doc(#{title => <<"Predefined datatypes">>}).
-type list() :: [any()].
-doc "An Erlang [list](`p:system:data_types.md#list`) containing terms of the type `ContentType`.".
-doc(#{title => <<"Predefined datatypes">>}).
-type list(ContentType) :: [ContentType].
-doc "An Erlang [map](`p:system:data_types.md#map`) containing any number of key and value associations.".
-doc(#{title => <<"Predefined datatypes">>}).
-type map() :: #{ any() => any() }.
-doc "An Erlang [list](`p:system:data_types.md#list`) that is not guaranteed to end with a [`[]`](`t:nil/0`), and where the list elements can be of any type.".
-doc(#{title => <<"Predefined datatypes">>}).
-type maybe_improper_list() :: maybe_improper_list(any(), any()).
-doc "An Erlang [list](`p:system:data_types.md#list`), that is not guaranteed to end with a [`[]`](`t:nil/0`), and where the list elements are of the type `ContentType`.".
-doc(#{title => <<"Predefined datatypes">>}).
...
-doc "The empty [list()](`t:list/0`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type nil() :: [].
-doc "The type used to show that a function will *never* return a value, that is it will *always* throw an exception.".
-doc(#{title => <<"Predefined datatypes">>}).
-type no_return() :: none().
-doc "An Erlang [node](`p:system:distributed.md#nodes`) represented by an atom.".
-doc(#{title => <<"Predefined datatypes">>}).
-type node() :: atom().
-doc "A non-negative integer, that is any positive integer or 0.".
-doc(#{title => <<"Predefined datatypes">>}).
-type non_neg_integer() :: non_neg_integer().
-doc "This type is used to show that a function will *never* return a value; that is it will *always* throw an exception. In a spec, use `t:no_return()` for the sake of clarity.".
-doc(#{title => <<"Predefined datatypes">>}).
-type none() :: none().
-doc "A [binary()](`t:binary/0`) that contains some data.".
-doc(#{title => <<"Predefined datatypes">>}).
-type nonempty_binary() :: <<_:8, _:_*8>>.
-doc "A [bitstring()](`t:bitstring/0`) that contains some data.".
-doc(#{title => <<"Predefined datatypes">>}).
...
-doc "A [list()](`t:list/0`) that contains some items.".
-doc(#{title => <<"Predefined datatypes">>}).
...
-doc "A [maybe_improper_list()](`t:maybe_improper_list/0`) that contains some items.".
-doc(#{title => <<"Predefined datatypes">>}).
-type nonempty_maybe_improper_list() :: nonempty_maybe_improper_list(any(), any()).
-doc "A [maybe_improper_list(ContentType, TerminationType)](`t:maybe_improper_list/0`) that contains some items.".
-doc(#{title => <<"Predefined datatypes">>}).
-type nonempty_maybe_improper_list(ContentType, TerminationType) :: nonempty_maybe_improper_list(ContentType, TerminationType).
-doc "A [string()](`t:string/0`) that contains some characters.".
-doc(#{title => <<"Predefined datatypes">>}).
-type nonempty_string() :: nonempty_list(char()).
-doc "An Erlang [number](`p:system:data_types.md#number`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type number() :: integer() | float().
-doc "An Erlang [process identifier](`p:system:data_types.md#pid`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type pid() :: pid().
-doc "An Erlang [port identifier](`p:system:data_types.md#port-identifier`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type port() :: port().
-doc "An integer greater than zero.".
-doc(#{title => <<"Predefined datatypes">>}).
-type pos_integer() :: pos_integer().
-doc "An Erlang [reference](`p:system:data_types.md#reference`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type reference() :: reference().
-doc "A character string represented by a list of ASCII characters or unicode codepoints.".
-doc(#{title => <<"Predefined datatypes">>}).
-type string() :: [char()].
-doc "All possible Erlang terms. Synonym for `t:any()`.".
-doc(#{title => <<"Predefined datatypes">>}).
-type term() :: any().
-doc "A timeout value that can be passed to a [receive expression](`p:system:expressions.md#receive`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type timeout() :: 'infinity' | non_neg_integer().
-doc "An Erlang [tuple](`p:system:data_types.md#tuple`).".
-doc(#{title => <<"Predefined datatypes">>}).
-type tuple() :: tuple().
...
-doc "An Erlang stacktrace as described by [Errors and Error Handling](`p:system:errors.md#stacktrace`) section in the Erlang Reference Manual.".
...
-doc "A term of type [`iovec()`](`t:iovec/0`), structured according to the Erlang external term format.".
...
* __`PartsPerSecond :: integer() >= 1`__ - Time unit expressed in parts per second. That is, the time unit equals `1/PartsPerSecond` second.

* __`second`__ - Symbolic representation of the time unit represented by the integer `1`.

* __`millisecond`__ - Symbolic representation of the time unit represented by the integer `1000`.

* __`microsecond`__ - Symbolic representation of the time unit represented by the integer `1000_000`.

* __`nanosecond`__ - Symbolic representation of the time unit represented by the integer `1000_000_000`.

* __`native`__ - Symbolic representation of the native time unit used by the Erlang runtime system.
  ...
  One can get an approximation of the `native` time unit by calling [`erlang:convert_time_unit(1, second, native)`](`convert_time_unit/3`). The result equals the number of whole `native` time units per second. If the number of `native` time units per second does not add up to a whole number, the result is rounded downwards.

  > #### Note {: class=info }
  > The value of the `native` time unit gives you more or less no information about the quality of time values. It sets a limit for the [resolution](time_correction.md#time_resolution) and for the [precision](time_correction.md#time_precision) of time values, but it gives no information about the [accuracy](time_correction.md#time_accuracy) of time values. The resolution of the `native` time unit and the resolution of time values can differ significantly.

* __`perf_counter`__ - Symbolic representation of the performance counter time unit used by the Erlang runtime system.
...
* __[`deprecated_time_unit()`](`m:erlang#type_deprecated_time_unit`)__ - Deprecated symbolic representations kept for backwards-compatibility.

The `t:time_unit/0` type can be extended. To convert time values between time units, use [`erlang:convert_time_unit/3`](`convert_time_unit/3`).
""".
-type time_unit() ::
	pos_integer()
      | 'second'
...
-doc """
[](){: id=type_deprecated_time_unit }
The [`time_unit()`](`m:erlang#type_time_unit`) type also consist of the following *deprecated* symbolic time units:

* __`seconds`__ - Same as [`second`](`m:erlang#type_time_unit`).

* __`milli_seconds`__ - Same as [`millisecond`](`m:erlang#type_time_unit`).

* __`micro_seconds`__ - Same as [`microsecond`](`m:erlang#type_time_unit`).

* __`nano_seconds`__ - Same as [`nanosecond`](`m:erlang#type_time_unit`).
""".
...
-doc "An opaque handle identifying a [NIF resource object ](erl_nif.md#resource_objects).".
...
-doc "A list of binaries. This datatype is useful to use together with [`enif_inspect_iovec`](erl_nif.md#enif_inspect_iovec).".
-type iovec() :: [binary()].
-export_type([iovec/0]).
...
> #### Note {: class=info }
> The number of characters that are permitted in an atom name is limited. The default limits can be found in the [efficiency guide (section Advanced)](`p:system:advanced.md`).

> #### Note {: class=info }
> There is configurable limit on how many atoms that can exist and atoms are not garbage collected. Therefore, it is recommended to consider whether `binary_to_existing_atom/2` is a better option than `binary_to_atom/2`. The default limits can be found in [efficiency guide (section Advanced)](`p:system:advanced.md`).
...
-doc "The same as [`binary_to_existing_atom`](`binary_to_existing_atom/2`) `(Binary, utf8)`.".
-doc(#{since => <<"OTP 23.0">>}).
...
The Erlang system has a [configurable limit](`p:system:advanced.md`) for the total number of atoms that can exist, and atoms are not garbage collected. Therefore, it is not safe to create many atoms from binaries that come from an untrusted source (for example, a file fetched from the Internet), for example, using `binary_to_atom/2`. This function is thus the appropriate option when the input binary comes from an untrusted source.
...
The float string format is the same as the format for [Erlang float literals](`p:system:data_types.md`) except for that underscores are not permitted.
...
%% binary_to_term/1
-doc """
Returns an Erlang term that is the result of decoding binary object `Binary`, which must be encoded according to the [Erlang external term format](erl_ext_dist.md).
...
* __`safe`__ - Use this option when receiving binaries from an untrusted source.
...
* __`used`__ - Changes the return value to `{Term, Used}` where `Used` is the number of bytes actually read from `Binary`.
...
%% cancel_timer/1
-doc "Cancels a timer. The same as calling [`erlang:cancel_timer(TimerRef, [])`](`cancel_timer/2`).".
...
%% cancel_timer/2
-doc """
Cancels a timer that has been created by [`erlang:start_timer`](`start_timer/4`) or [`erlang:send_after`](`send_after/4`). `TimerRef` identifies the timer, and was returned by the BIF that created the timer.

`Option`s:

* __`{async, Async}`__ - Asynchronous request for cancellation. `Async` defaults to `false`, which causes the cancellation to be performed synchronously. When `Async` is set to `true`, the cancel operation is performed asynchronously. That is, `cancel_timer()` sends an asynchronous request for cancellation to the timer service that manages the timer, and then returns `ok`.

* __`{info, Info}`__ - Requests information about the `Result` of the cancellation. `Info` defaults to `true`, which means the `Result` is given. When `Info` is set to `false`, no information about the result of the cancellation is given.
...
See also [`erlang:send_after/4`](`send_after/4`), [`erlang:start_timer/4`](`start_timer/4`), and [`erlang:read_timer/2`](`read_timer/2`).
""".
-doc(#{since => <<"OTP 18.0">>}).
...
%% check_old_code/1
-doc """
Returns `true` if `Module` has [old code](`p:system:code_loading.md#code-replacement`), otherwise `false`.
...
* __`{allow_gc, boolean()}`__ - Determines if garbage collection is allowed when performing the operation. If `{allow_gc, false}` is passed, and a garbage collection is needed to determine the result of the operation, the operation is aborted (see information on `CheckResult` below). The default is to allow garbage collection, that is, `{allow_gc, true}`.

* __`{async, RequestId}`__ - The function `check_process_code/3` returns the value `async` immediately after the request has been sent. When the request has been processed, the process that called this function is passed a message on the form `{check_process_code, RequestId, CheckResult}`.
...
* __`true`__ - The process identified by `Pid` executes old code for `Module`. That is, the current call of the process executes old code for this module, or the process has references to old code for this module, or the process contains funs that references old code for this module.

* __`false`__ - The process identified by `Pid` does not execute old code for `Module`.

* __`aborted`__ - The operation was aborted, as the process needed to be garbage collected to determine the operation result, and the operation was requested by passing option `{allow_gc, false}`.
...
>
> As of ERTS version 9.0, the check process code operation only checks for direct references to the code. Indirect references via `fun`s will be ignored. If such `fun`s exist and are used after a purge of the old code, an exception will be raised upon usage (same as the case when the `fun` is received by the process after the purge). Literals will be taken care of (copied) at a later stage. This behavior can as of ERTS version 8.1 be enabled when [building OTP](`p:system:install.md#advanced-configuration-and-build-of-erlangotp_configuring`), and will automatically be enabled if dirty scheduler support is enabled.

See also `m:code`.

Failures:

* __`badarg`__ - If `Pid` is not a node local process identifier.

* __`badarg`__ - If `Module` is not an atom.

* __`badarg`__ - If `OptionList` is an invalid list of options.
""".
-doc(#{since => <<"OTP 17.0">>}).
...
The time zone and Daylight Saving Time correction depend on the underlying OS. The return value is based on the [OS System Time](time_correction.md#os-system-time). Example:
...
* __`raw | 0`__ - No packet handling is done. The entire binary is returned unless it is empty.

* __`1 | 2 | 4`__ - Packets consist of a header specifying the number of bytes in the packet, followed by that number of bytes. The length of the header can be one, two, or four bytes; the order of the bytes is big-endian. The header is stripped off when the packet is returned.

* __`line`__ - A packet is a line-terminated by a delimiter byte, default is the latin-1 newline character. The delimiter byte is included in the returned packet unless the line was truncated according to option `line_length`.

* __`asn1 | cdr | sunrm | fcgi | tpkt`__ - The header is *not* stripped off.

  The meanings of the packet types are as follows:

  * __`asn1` \- ASN.1 BER__  

  * __`sunrm` \- Sun's RPC encoding__  

  * __`cdr` \- CORBA (GIOP 1.1)__  

  * __`fcgi` \- Fast CGI__  

  * __`tpkt` \- TPKT format \[RFC1006]__  

  

* __`http | httph | http_bin | httph_bin`__ - The Hypertext Transfer Protocol. The packets are returned with the format according to `HttpPacket` described earlier. A packet is either a request, a response, a header, or an end of header mark. Invalid lines are returned as `HttpError`.
  ...
  Since OTP 26.0, `Host` may be an IPv6 address enclosed in `[]`, as defined in [RFC2732 ](https://www.ietf.org/rfc/rfc2732.txt).

Options:

* __`{packet_size, integer() >= 0}`__ - Sets the maximum allowed size of the packet body. If the packet header indicates that the length of the packet is longer than the maximum allowed length, the packet is considered invalid. Defaults to 0, which means no size limit.

* __`{line_length, integer() >= 0}`__ - For packet type `line`, lines longer than the indicated length are truncated.
...
* __`{line_delimiter, 0 =< byte() =< 255}`__ - For packet type `line`, sets the delimiting byte. Default is the latin-1 character `$\n`.
...
> #### Warning {: class=warning }
> This BIF is intended for the code server (see `m:code`) and is not to be used elsewhere.
...
`demonitor(MonitorRef, [])` is equivalent to [`demonitor(MonitorRef)`](`demonitor/1`).

`Option`s:

* __`flush`__ - Removes (one) `{_, MonitorRef, _, _, _}` message, if there is one, from the caller message queue after monitoring has been stopped.
...
* __`info`__ - The returned value is one of the following:
  ...
  * __`false`__ - The monitor was not found and could not be removed. This probably because someone already has placed a `'DOWN'` message corresponding to this monitor in the caller message queue.
...
* __`badarg`__ - If `OptionList` is not a list.

* __`badarg`__ - If `Option` is an invalid option.

* __`badarg`__ - The same failure as for `demonitor/1`.
""".
...
* __`explicit_unalias`__ - The alias can only be deactivated via a call to `unalias/1`. This is also the default behaviour if no options are passed or if `alias/0` is called.

* __`reply`__ - The alias will be automatically deactivated when a reply message sent via the alias is received. The alias can also still be deactivated via a call to `unalias/1`.
...
For more information on process aliases see the [*Process Aliases*](`p:system:ref_man_processes.md#process-aliases`) section of the *Erlang Reference Manual*.
""".
-doc(#{since => <<"OTP 24.0">>}).
...
The intent of the exception class `error` is to signal that an unexpected error has happened (for example, a function is called with a parameter that has an incorrect type). See the guide about [errors and error handling](`p:system:errors.md`) for additional information. Example:
...
If the `error_info` option is given, the `ErrorInfoMap` will be inserted into the stacktrace. The information given in the `ErrorInfoMap` is to be used by error formatters such as [`erl_error`](`erl_error:format_exception/4`) to provide more context around an error.

The default `module` of the `ErrorInfoMap` is the module that the call to `error/3` is made. The default `function` is `format_error`. See [`format_error/2`](`m:erl_error#format_error`) for more details on how this Module:Function/2 is to be used

The intent of the exception class `error` is to signal that an unexpected error has happened (for example, a function is called with a parameter that has an incorrect type). See the guide about [errors and error handling](`p:system:errors.md`) for additional information.
""".
-doc(#{since => <<"OTP 24.0">>}).
...
This function differ from [`error/1,2,3`](`error/1`) by causing an exception of a different class and by having a reason that does not include the list of functions from the call stack.

See the guide about [errors and error handling](`p:system:errors.md`) for additional information.
...
* If `P` is not [trapping exits](`process_flag/2`), `P` exits with exit reason `Reason`.
* If `P` is [trapping exits](`process_flag/2`), the exit signal is transformed into a message `{'EXIT', From, Reason}`, where `From` is the process identifier of the process that sent the exit signal, and delivered to the message queue of `P`.
...
* If `P` is [trapping exits](`process_flag/2`), the exit signal is transformed into a message `{'EXIT', From, normal}`, where `From` is the process identifier of the process that sent the exit signal, and delivered to `P`'s message queue.
...
>
> * If `P` is [trapping exits](`process_flag/2`), the exit signal is transformed into a message `{'EXIT', From, normal}`, where `From` is `P`'s process identifier, and delivered to `P`'s message queue.
...
%% float_to_binary/1
-doc "The same as `float_to_binary(Float,[{scientific,20}])`.".
-doc(#{since => <<"OTP R16B">>}).
...
%% float_to_list/1
-doc "The same as `float_to_list(Float,[{scientific,20}])`.".
...
In the last example, `float_to_list(0.1+0.2)` evaluates to `"3.00000000000000044409e-01"`. The reason for this is explained in [Representation of Floating Point Numbers](`p:system:data_types.md#float_representation_problem`).
""".
-doc(#{since => <<"OTP R16B">>}).
...
`String` has the following form, if `Fun` was created by a [fun expression](`p:system:expressions.md#fun-expressions`) of the form `fun ModuleName:FuncName/Arity`:

`"fun ModuleName:FuncName/Arity"`

The form of `String` when `Fun` is created from other types of [fun expressions](`p:system:expressions.md#fun-expressions`) differs depending on if the fun expression was executed while executing compiled code or if the fun expression was executed while executing uncompiled code (uncompiled escripts, the Erlang shell, and other code executed by the erl_eval module):

* __compiled code__ - `"#Fun<M.I.U>"`, where M, I and U correspond to the values named `module`, `index` and `uniq` in the result of [`erlang:fun_info(Fun)`](`fun_info/1`).

* __uncompiled code__ - All funs created from fun expressions in uncompiled code with the same arity are mapped to the same list by `fun_to_list/1`.

> #### Note {: class=info }
> Generally, one can not use `fun_to_list/1` to check if two funs are equal as `fun_to_list/1` does not take the fun's environment into account. See [`erlang:fun_info/1`](`fun_info/1`) for how to get the environment of a fun.
...
%% function_exported/3
-doc "Returns `true` if the module `Module` is [current](`p:system:code_loading.md#code-replacement`) and contains an exported function `Function/Arity`, or if there is a BIF (a built-in function implemented in C) with the specified name, otherwise returns `false`.".
...
* __`{async, RequestId}`__ - The function `garbage_collect/2` returns the value `async` immediately after the request has been sent. When the request has been processed, the process that called this function is passed a message on the form `{garbage_collect, RequestId, GCResult}`.

* __`{type, 'major' | 'minor'}`__ - Triggers garbage collection of requested type. Default value is `'major'`, which would trigger a fullsweep GC. The option `'minor'` is considered a hint and may lead to either minor or major GC run.
...
* __`true`__ - The process identified by `Pid` has been garbage collected.

* __`false`__ - No garbage collection was performed, as the process identified by `Pid` terminated before the request could be satisfied.
...
* When running tests, both [`common_test`](`p:common_test:index.html`) and `m:eunit` set the group leader in order to capture any I/O from the testcase.
...
Setting the group leader follows the signal ordering guarantees described in the [Processes Chapter](`p:system:ref_man_processes.md#signals`) in the *Erlang Reference Manual*.

See also `group_leader/0` and [OTP design principles](`p:system:applications.md#stopping`) related to starting and stopping applications.
...
* __`{flush, EnableFlushing}`{: id=halt_flush }__ - If `EnableFlushing` equals `true`, which also is the default behavior, the runtime system will perform the following operations before terminating:
  ...
  * Call all installed [NIF *on halt* callbacks](erl_nif.md#on_halt).
  * Wait for all ongoing [NIF calls with the *delay halt* setting](erl_nif.md#delay_halt) enabled to return.
...
> #### Note {: class=info }
> The [`{flush, boolean()}`](`m:erlang#halt_flush`) option will be ignored, and flushing will be disabled.

[](){: id=halt_crash_dump_2 }
```erlang
-type halt_options() :: [{flush, boolean()}].
```

Halt the Erlang runtime system and generate an [Erlang crash dump](crash_dump.md). The string `CrashDumpSlogan` will be used as slogan in the Erlang crash dump created. The slogan will be trunkated if `CrashDumpSlogan` is longer than 1023 characters.
...
%% iolist_to_iovec/1
-doc """
Returns an [iovec](`t:iovec/0`) that is made from the integers and binaries in `IoListOrBinary`. This function is useful when you want to flatten an iolist but you do not need a single binary. This can be useful for passing the data to nif functions such as [`enif_inspect_iovec`](erl_nif.md#enif_inspect_iovec) or do more efficient message passing. The advantage of using this function over `iolist_to_binary/1` is that it does not have to copy [off-heap binaries](`p:system:binaryhandling.md#refc_binary`). Example:
...
See the documentation about [signals](`p:system:ref_man_processes.md#signals`) and [erlang:exit/2](`exit/2`) for more information about signals and exit signals.
""".
...
If one of the participants of a link terminates, it will [send an exit signal](`p:system:ref_man_processes.md#sending_exit_signals`) to the other participant. The exit signal will contain the [exit reason](`p:system:ref_man_processes.md#link_exit_signal_reason`) of the terminated participant. Other cases when exit signals are triggered due to a link are when no linkee exist (`noproc` exit reason) and when the connection between linked processes on different nodes is lost or cannot be established (`noconnection` exit reason).
...
* [Links](`p:system:ref_man_processes.md#links`)
* [Sending Exit Signals](`p:system:ref_man_processes.md#sending_exit_signals`)
* [Receiving Exit Signals](`p:system:ref_man_processes.md#receiving_exit_signals`)

For historical reasons, `link/1` has a strange semi-synchronous behavior when it is "cheap" to check if the linkee exists or not, and the caller does not [trap exits](`m:erlang#process_flag_trap_exit`). If the above is true and the linkee does not exist, `link/1` will raise a `noproc` error *exception*. The expected behavior would instead have been that `link/1` returned `true`, and the caller later was sent an exit signal with `noproc` exit reason, but this is unfortunately not the case. The `noproc` [exception](`p:system:errors.md#exceptions`) is not to be confused with an [exit signal](`p:system:ref_man_processes.md#sending_exit_signals`) with exit reason `noproc`. Currently it is "cheap" to check if the linkee exists when it is supposed to reside on the same node as the calling process.

The link setup and activation is performed asynchronously. If the link already exists, or if the caller attempts to create a link to itself, nothing is done. A detailed description of the [link protocol](erl_dist_protocol.md#link_protocol) can be found in the *Distribution Protocol* chapter of the *ERTS User's Guide*.
...
> #### Note {: class=info }
> There is configurable limit on how many atoms that can exist and atoms are not garbage collected. Therefore, it is recommended to consider if `list_to_existing_atom/1` is a better option than `list_to_atom/1`. The default limits can be found in the [efficiency guide (section Advanced)](`p:system:advanced.md`).
...
The time zone and Daylight Saving Time correction depend on the underlying OS. The return value is based on the [OS System Time](time_correction.md#os-system-time).
""".
...
%% make_ref/0
-doc """
Returns a [unique reference](`p:system:advanced.md#unique_references`). The reference is unique among connected nodes.
...
For more information about MD5, see [RFC 1321 - The MD5 Message-Digest Algorithm](https://www.ietf.org/rfc/rfc1321.txt).
...
%% md5_final/1
-doc "Finishes the update of an MD5 `Context` and returns the computed `MD5` message digest.".
...
%% md5_init/0
-doc "Creates an MD5 context, to be used in the following calls to `md5_update/2`.".
...
%% module_loaded/1
-doc "Returns `true` if the module `Module` is loaded as [*current code*](`p:system:code_loading.md#code-replacement`); otherwise, `false`. It does not attempt to load the module.".
...
A `process` or `port` monitor by name resolves the `RegisteredName` to `t:pid()` or `t:port()` only once at the moment of monitor instantiation, later changes to the name registration will not affect the existing monitor.
...
* __`Object`__ - The monitored entity, which triggered the event. When monitoring a process or a local port, `Object` will be equal to the `t:pid()` or `t:port()` that was being monitored. When monitoring process or port by name, `Object` will have format `{RegisteredName, Node}` where `RegisteredName` is the name which has been used with `monitor/2` call and `Node` is local or remote node name (for ports monitored by name, `Node` is always local node name).

* __`Info`__ - Either the exit reason of the process, `noproc` (process or port did not exist at the time of monitor creation), or `noconnection` (no connection to the node where the monitored process resides).

* __Monitoring a `process`{: id=monitor_process }__ - Creates monitor between the current process and another process identified by `Item`, which can be a `t:pid()` (local or remote), an atom `RegisteredName` or a tuple `{RegisteredName, Node}` for a registered process, located elsewhere.
...
* __Monitoring a `port`{: id=monitor_port }__ - Creates monitor between the current process and a port identified by `Item`, which can be a `t:port()` (only local), an atom `RegisteredName` or a tuple `{RegisteredName, Node}` for a registered port, located on this node. Note, that attempt to monitor a remote port will result in `badarg`.

* __Monitoring a `time_offset`{: id=monitor_time_offset }__ - Monitors changes in [`time offset`](`time_offset/0`) between [Erlang monotonic time](time_correction.md#erlang_monotonic_time) and [Erlang system time](time_correction.md#erlang_system_time). One valid `Item` exists in combination with the `time_offset Type`, namely the atom `clock_service`. Notice that the atom `clock_service` is *not* the registered name of a process. In this case it serves as an identifier of the runtime system internal clock service at current runtime system instance.

  The monitor is triggered when the time offset is changed. This either if the time offset value is changed, or if the offset is changed from preliminary to final during [finalization of the time offset](`m:erlang#system_flag_time_offset`) when the [single time warp mode](time_correction.md#single_time_warp_mode) is used. When a change from preliminary to final time offset is made, the monitor is triggered once regardless of whether the time offset value was changed or not.

  If the runtime system is in [multi time warp mode](time_correction.md#multi_time_warp_mode), the time offset is changed when the runtime system detects that the [OS system time](time_correction.md#os_system_time) has changed. The runtime system does, however, not detect this immediately when it occurs. A task checking the time offset is scheduled to execute at least once a minute, so under normal operation this is to be detected within a minute, but during heavy load it can take longer time.
  ...
  When the `'CHANGE'` message has been received you are guaranteed not to retrieve the old time offset when calling [`erlang:time_offset()`](`time_offset/0`). Notice that you can observe the change of the time offset when calling `erlang:time_offset()` before you get the `'CHANGE'` message.
...
* __`{alias, UnaliasOpt}`__ - The returned monitor reference will also become an alias for the calling process. That is, the returned reference can be used for sending messages to the calling process. See also `alias/0`. The `UnaliasOpt` determines how the alias should be deactivated.

  * __`explicit_unalias`__ - Only an explicit call to `unalias/1` will deactivate the alias.

  * __`demonitor`__ - The alias will be automatically deactivated when the monitor is removed. This either via an explicit call to `demonitor/1` or when it is automatically removed at the same time as a `'DOWN'` message is delivered due to the monitor. The alias can also still be deactivated via a call to `unalias/1`.

  * __`reply_demonitor`__ - The alias will be automatically deactivated when the monitor is removed (see `demonitor` option above) or a reply message sent via the alias is received. When a reply message is received via the alias the monitor will also be automatically removed. This is useful in client/server scenarios when a client monitors the server and will get the reply via the alias. Once the response is received both the alias and the monitor will be automatically removed regardless of whether the response is a reply or a `'DOWN'` message. The alias can also still be deactivated via a call to `unalias/1`. Note that if the alias is removed using the `unalias/1` BIF, the monitor will still be left active.
...
* __`{tag, UserDefinedTag}`__ - Replace the default `Tag` with `UserDefinedTag` in the [monitor message](`m:erlang#monitor_message`) delivered when the monitor is triggered. For example, when monitoring a process, the `'DOWN'` tag in the down message will be replaced by `UserDefinedTag`.

  An example of how the `{tag, UserDefinedTag}` option can be used in order to enable the new [selective receive optimization](`p:system:eff_guide_processes.md#receiving-messages`), introduced in OTP 24, when making multiple requests to different servers:
...
-doc "Works exactly like `error/1`, but Dialyzer thinks that this BIF will return an arbitrary term. When used in a stub function for a NIF to generate an exception when the NIF library is not loaded, Dialyzer does not generate false warnings.".
-doc(#{since => <<"OTP R14B">>}).
...
-doc "Works exactly like `error/2`, but Dialyzer thinks that this BIF will return an arbitrary term. When used in a stub function for a NIF to generate an exception when the NIF library is not loaded, Dialyzer does not generate false warnings.".
-doc(#{since => <<"OTP R14B">>}).
...
>
> For more information, see section [Time and Time Correction](time_correction.md) in the User's Guide. Specifically, section [Dos and Dont's](time_correction.md#dos_and_donts) describes what to use instead of `erlang:now/0`.
...
-doc """
> #### Warning {: class=warning }
> This function is deprecated as [`erlang:phash2/2`](`phash2/2`) should be used for new code. Note that `erlang:phash(X,N)` is not necessary equal to `erlang:phash2(X,N)`
...
> #### Note {: class=info }
> The [creation](erl_dist_protocol.md) for the node is not included in the list representation of `Pid`. This means that processes in different incarnations of a node with a specific name can get the same list representation.
""".
...
%% port_to_list/1
-doc "Returns a string corresponding to the text representation of the port identifier `Port`.".
...
-doc """
Generates and returns an [integer unique on current runtime system instance](`p:system:advanced.md#unique_integers`). The integer is unique in the sense that this BIF, using the same set of modifiers, does not return the same integer more than once on the current runtime system instance. Each integer value can of course be constructed by other means.
...
* __positive__ - Returns only positive integers.

  Notice that by passing the `positive` modifier you will get heap allocated integers (bignums) quicker.

* __monotonic__ - Returns [strictly monotonically increasing](time_correction.md#strictly_monotonically_increasing) integers corresponding to creation time. That is, the integer returned is always larger than previously returned integers on the current runtime system instance.
...
* __`badarg`__ - if `ModifierList` is not a proper list.

* __`badarg`__ - if `Modifier` is not a valid modifier.
""".
-doc(#{since => <<"OTP 18.0">>}).
...
-doc "Generates and returns an [integer unique on current runtime system instance](`p:system:advanced.md#unique_integers`). The same as calling [`erlang:unique_integer([])`](`unique_integer/1`).".
-doc(#{since => <<"OTP 18.0">>}).
...
-doc """
Returns the current [Erlang monotonic time](time_correction.md#erlang_monotonic_time) in `native` [time unit](`m:erlang#type_time_unit`). This is a monotonically increasing time since some unspecified point in time.

> #### Note {: class=info }
> This is a [monotonically increasing](time_correction.md#monotonically_increasing) time, but *not* a [strictly monotonically increasing](time_correction.md#strictly_monotonically_increasing) time. That is, consecutive calls to `erlang:monotonic_time/0` can produce the same result.
>
> Different runtime system instances will use different unspecified points in time as base for their Erlang monotonic clocks. That is, it is *pointless* comparing monotonic times from different runtime system instances. Different runtime system instances can also place this unspecified point in time different relative runtime system start. It can be placed in the future (time at start is a negative value), the past (time at start is a positive value), or the runtime system start (time at start is zero). The monotonic time at runtime system start can be retrieved by calling [`erlang:system_info(start_time)`](`m:erlang#system_info_start_time`).
""".
-doc(#{since => <<"OTP 18.0">>}).
...
-doc """
Returns the current [Erlang monotonic time](time_correction.md#erlang_monotonic_time) converted into the `Unit` passed as argument.

Same as calling [`erlang:convert_time_unit`](`convert_time_unit/3`)`(`[ `erlang:monotonic_time()`](`monotonic_time/0`)`, native, Unit)`, however optimized for commonly used `Unit`s.
""".
-doc(#{since => <<"OTP 18.0">>}).
...
-doc """
Returns current [Erlang system time](time_correction.md#erlang_system_time) in `native` [time unit](`m:erlang#type_time_unit`).

Calling `erlang:system_time()` is equivalent to [`erlang:monotonic_time()`](`monotonic_time/0`)` + `[`erlang:time_offset()`](`time_offset/0`).

> #### Note {: class=info }
> This time is *not* a monotonically increasing time in the general case. For more information, see the documentation of [time warp modes](time_correction.md#time_warp_modes) in the User's Guide.
""".
-doc(#{since => <<"OTP 18.0">>}).
...
-doc """
Returns current [Erlang system time](time_correction.md#erlang_system_time) converted into the `Unit` passed as argument.

Calling `erlang:system_time(Unit)` is equivalent to [`erlang:convert_time_unit`](`convert_time_unit/3`)`(`[`erlang:system_time()`](`system_time/0`)`, native, Unit)`.
...
-doc """
Returns the current time offset between [Erlang monotonic time](time_correction.md#erlang_monotonic_time) and [Erlang system time](time_correction.md#erlang_system_time) in `native` [time unit](`m:erlang#type_time_unit`). Current time offset added to an Erlang monotonic time gives corresponding Erlang system time.

The time offset may or may not change during operation depending on the [time warp mode](time_correction.md#time_warp_modes) used.

> #### Note {: class=info }
> A change in time offset can be observed at slightly different points in time by different processes.
>
> If the runtime system is in [multi-time warp mode](time_correction.md#multi_time_warp_mode), the time offset is changed when the runtime system detects that the [OS system time](time_correction.md#os_system_time) has changed. The runtime system will, however, not detect this immediately when it occurs. A task checking the time offset is scheduled to execute at least once a minute; so, under normal operation this is to be detected within a minute, but during heavy load it can take longer time.
""".
-doc(#{since => <<"OTP 18.0">>}).
...
-doc """
Returns the current time offset between [Erlang monotonic time](time_correction.md#erlang_monotonic_time) and [Erlang system time](time_correction.md#erlang_system_time) converted into the `Unit` passed as argument.

Same as calling [`erlang:convert_time_unit`](`convert_time_unit/3`)`(`[ `erlang:time_offset()`](`time_offset/0`)`, native, Unit)` however optimized for commonly used `Unit`s.
""".
-doc(#{since => <<"OTP 18.0">>}).
...
Returns current [Erlang system time](time_correction.md#erlang_system_time) on the format `{MegaSecs, Secs, MicroSecs}`. This format is the same as `os:timestamp/0` and the deprecated [`erlang:now/0`](`now/0`) use. The reason for the existence of `erlang:timestamp()` is purely to simplify use for existing code that assumes this time stamp format. Current Erlang system time can more efficiently be retrieved in the time unit of your choice using [`erlang:system_time/1`](`system_time/1`).
...
%% pre_loaded/0
-doc "Returns a list of Erlang modules that are preloaded in the run-time system. Pre-loaded modules are Erlang modules that are needed to bootstrap the system to load the first Erlang modules from either disk or by using `m:erl_boot_server`.".
-spec pre_loaded() -> [module()].
pre_loaded() ->
    erlang:nif_error(undefined).

%% process_display/2
-doc "Writes information about the local process `Pid` on standard error. The only allowed value for the atom `Type` is `backtrace`, which shows the contents of the call stack, including information about the call chain, with the current function printed first. The format of the output is not further defined.".
...
See the reference manual about [errors and error handling](`p:system:errors.md`) for more information about exception classes and how to catch exceptions.
""".
...
%% read_timer/1
-doc "Reads the state of a timer. The same as calling [`erlang:read_timer(TimerRef, [])`](`read_timer/2`).".
...
%% read_timer/2
-doc """
Reads the state of a timer that has been created by either [`erlang:start_timer`](`start_timer/4`) or [`erlang:send_after`](`send_after/4`). `TimerRef` identifies the timer, and was returned by the BIF that created the timer.

`Options`:

* __`{async, Async}`__ - Asynchronous request for state information. `Async` defaults to `false`, which causes the operation to be performed synchronously. In this case, the `Result` is returned by `erlang:read_timer`. When `Async` is `true`, `erlang:read_timer` sends an asynchronous request for the state information to the timer service that manages the timer, and then returns `ok`. A message on the format `{read_timer, TimerRef, Result}` is sent to the caller of `erlang:read_timer` when the operation has been processed.
...
See also [`erlang:send_after/4`](`send_after/4`), [`erlang:start_timer/4`](`start_timer/4`), and [`erlang:cancel_timer/2`](`cancel_timer/2`).
""".
-doc(#{since => <<"OTP 18.0">>}).
...
%% register/2
-doc """
Registers the name `RegName` with a process identifier (pid) or a port identifier in the [`name registry`](`p:system:ref_man_processes.md#runtime-service`). `RegName`, which must be an atom, can be used instead of the pid or port identifier in send operator (`RegName ! Message`) and most other BIFs that take a pid or port identifies as an argument. Example:
...
The registered name is considered a [Directly Visible Erlang Resource](`p:system:ref_man_processes.md#visible-resources`) and is automatically unregistered when the process terminates.

Failures:

* __`badarg`__ - If `PidOrPort` is not an existing local process or port.

* __`badarg`__ - If `RegName` is already in use.

* __`badarg`__ - If the process or port is already registered (already has a name).

* __`badarg`__ - If `RegName` is the atom `undefined`.
""".
...
%% resume_process/1
-doc """
Decreases the suspend count on the process identified by `Suspendee`. `Suspendee` is previously to have been suspended through [`erlang:suspend_process/2`](`suspend_process/2`) or [`erlang:suspend_process/1`](`suspend_process/1`) by the process calling `erlang:resume_process(Suspendee)`. When the suspend count on `Suspendee` reaches zero, `Suspendee` is resumed, that is, its state is changed from suspended into the state it had before it was suspended.
...
* __`badarg`__ - If `Suspendee` is not a process identifier.

* __`badarg`__ - If the process calling `erlang:resume_process/1` had not previously increased the suspend count on the process identified by `Suspendee`.

* __`badarg`__ - If the process identified by `Suspendee` is not alive.
""".
...
%% send_after/3
-doc "Starts a timer. The same as calling [`erlang:send_after(Time, Dest, Msg, [])`](`send_after/4`).".
...
%% send_after/4
-doc "Starts a timer. When the timer expires, the message `Msg` is sent to the process identified by `Dest`. Apart from the format of the time-out message, this function works exactly as [`erlang:start_timer/4`](`start_timer/4`).".
-doc(#{since => <<"OTP 18.0">>}).
...
%% spawn_link/3
-doc "Returns the process identifier of a new process started by the application of `Module:Function` to `Args`. A link is created between the calling process and the new process, atomically. Otherwise works like `spawn/3`.".
...
%% start_timer/3
-doc "Starts a timer. The same as calling [`erlang:start_timer(Time, Dest, Msg, [])`](`start_timer/4`).".
...
* __`{abs, false}`__ - This is the default. It means the `Time` value is interpreted as a time in milliseconds *relative* current [Erlang monotonic time](time_correction.md#erlang_monotonic_time).

* __`{abs, true}`__ - Absolute `Time` value. The `Time` value is interpreted as an absolute Erlang monotonic time in milliseconds.

More `Option`s can be added in the future.

The absolute point in time, the timer is set to expire on, must be in the interval `[ `[erlang:convert_time_unit](`convert_time_unit/3`)`(`[erlang:system_info](`m:erlang#system_info_start_time`)`(start_time), native, millisecond), `[erlang:convert_time_unit](`convert_time_unit/3`)`(`[erlang:system_info](`m:erlang#system_info_end_time`)`(end_time), native, millisecond) ]`. If a relative time is specified, the `Time` value is not allowed to be negative.

If `Dest` is a `t:pid()`, it must be a `t:pid()` of a process created on the current runtime system instance. This process has either terminated or not. If `Dest` is an `t:atom()`, it is interpreted as the name of a locally registered process. The process referred to by the name is looked up at the time of timer expiration. No error is returned if the name does not refer to a process.

If `Dest` is a `t:pid()`, the timer is automatically canceled if the process referred to by the `t:pid()` is not alive, or if the process exits. This feature was introduced in ERTS 5.4.11. Notice that timers are not automatically canceled when `Dest` is an `t:atom()`.

See also [`erlang:send_after/4`](`send_after/4`), [`erlang:cancel_timer/2`](`cancel_timer/2`), and [`erlang:read_timer/2`](`read_timer/2`).
...
* __`badarg`__ - If the process identified by `Suspendee` is the same process as the process calling `erlang:suspend_process/2`.

* __`badarg`__ - If the process identified by `Suspendee` is not alive.

* __`badarg`__ - If the process identified by `Suspendee` resides on another node.

* __`badarg`__ - If `OptList` is not a proper list of valid `Opt`s.

* __`system_limit`__ - If the process identified by `Suspendee` has been suspended more times by the calling process than can be represented by the currently used internal data structures. The system limit is greater than 2,000,000,000 suspends and will never be lower.
""".
...
-doc """
Suspends the process identified by `Suspendee`. The same as calling [`erlang:suspend_process(Suspendee, [])`](`suspend_process/2`).
...
Returns the current system monitoring settings set by [`erlang:system_monitor/2`](`system_monitor/2`) as `{MonitorPid, Options}`, or `undefined` if no settings exist. The order of the options can be different from the one that was set.
""".
...
Calling the function with `{MonitorPid, Options}` as argument is the same as calling [`erlang:system_monitor(MonitorPid, Options)`](`system_monitor/2`).

Returns the previous system monitor settings just like [`erlang:system_monitor/0`](`system_monitor/0`).
""".
...
* __`{long_schedule, Time}`__ - If a process or port in the system runs uninterrupted for at least `Time` wall clock milliseconds, a message `{monitor, PidOrPort, long_schedule, Info}` is sent to `MonitorPid`. `PidOrPort` is the process or port that was running. `Info` is a list of two-element tuples describing the event.

  If a `t:pid()`, the tuples `{timeout, Millis}`, `{in, Location}`, and `{out, Location}` are present, where `Location` is either an MFA (`{Module, Function, Arity}`) describing the function where the process was scheduled in/out, or the atom `undefined`.

  If a `t:port()`, the tuples `{timeout, Millis}` and `{port_op,Op}` are present. `Op` is one of `proc_sig`, `timeout`, `input`, `output`, `event`, or `dist_cmd`, depending on which driver callback was executing.
...
* __`{large_heap, Size}`__ - If a garbage collection in the system results in the allocated size of a heap being at least `Size` words, a message `{monitor, GcPid, large_heap, Info}` is sent to `MonitorPid`. `GcPid` and `Info` are the same as for `long_gc` earlier, except that the tuple tagged with `timeout` is not present.
  ...
  When a process is killed by [`max_heap_size`](`m:erlang#process_flag_max_heap_size`), it is killed before the garbage collection is complete and thus no large heap message is sent.

* __`busy_port`__ - If a process in the system gets suspended because it sends to a busy port, a message `{monitor, SusPid, busy_port, Port}` is sent to `MonitorPid`. `SusPid` is the pid that got suspended when sending to `Port`.

* __`busy_dist_port`[](){: id=busy_dist_port }
__  
...
* __`badarg`__ - If `MonitorPid` does not exist.

* __`badarg`__ - If `MonitorPid` is not a local process.
""".
...
Returns the current system profiling settings set by [`erlang:system_profile/2`](`system_profile/2`) as `{ProfilerPid, Options}`, or `undefined` if there are no settings. The order of the options can be different from the one that was set.
""".
...
* __`exclusive`__ - If a synchronous call to a port from a process is done, the calling process is considered not runnable during the call runtime to the port. The calling process is notified as `inactive`, and later `active` when the port callback returns.

* __`monotonic_timestamp`__ - Time stamps in profile messages use [Erlang monotonic time](time_correction.md#erlang_monotonic_time). The time stamp (Ts) has the same format and value as produced by `erlang:monotonic_time(nanosecond)`.

* __`runnable_procs`__ - If a process is put into or removed from the run queue, a message, `{profile, Pid, State, Mfa, Ts}`, is sent to `ProfilerPid`. Running processes that are reinserted into the run queue after having been pre-empted do not trigger this message.

* __`runnable_ports`__ - If a port is put into or removed from the run queue, a message, `{profile, Port, State, 0, Ts}`, is sent to `ProfilerPid`.

* __`scheduler`__ - If a scheduler is put to sleep or awoken, a message, `{profile, scheduler, Id, State, NoScheds, Ts}`, is sent to `ProfilerPid`.

* __`strict_monotonic_timestamp`__ - Time stamps in profile messages consist of [Erlang monotonic time](time_correction.md#erlang_monotonic_time) and a monotonically increasing integer. The time stamp (Ts) has the same format and value as produced by `{erlang:monotonic_time(nanosecond), erlang:unique_integer([monotonic])}`.

* __`timestamp`__ - Time stamps in profile messages include a time stamp (Ts) that has the same form as returned by `erlang:now()`. This is also the default if no time stamp flag is specified. If `cpu_timestamp` has been enabled through [`erlang:trace/3`](`trace/3`), this also effects the time stamp produced in profiling messages when flag `timestamp` is enabled.
...
If evaluated within a [catch expression](`p:system:expressions.md#catch`), the catch expression returns value `Any`. Example:
...
If evaluated within a `try`\-block of a [try expression](`p:system:expressions.md#try`), the value `Any` can be caught within the catch block. Example:
...
* __`all`__ - All currently existing processes and ports and all that will be created in the future.

* __`processes`__ - All currently existing processes and all that will be created in the future.

* __`ports`__ - All currently existing ports and all that will be created in the future.

* __`existing`__ - All currently existing processes and ports.

* __`existing_processes`__ - All currently existing processes.

* __`existing_ports`__ - All currently existing ports.

* __`new`__ - All processes and ports that will be created in the future.

* __`new_processes`__ - All processes that will be created in the future.

* __`new_ports`__ - All ports that will be created in the future.

`FlagList` can contain any number of the following flags (the "message tags" refers to the list of [`trace messages`](`m:erlang#trace_3_trace_messages`)):

* __`all`__ - Sets all trace flags except `tracer` and `cpu_timestamp`, which are in their nature different than the others.

* __`send`__ - Traces sending of messages.

  Message tags: [`send`](`m:erlang#trace_3_trace_messages_send`) and [`send_to_non_existing_process`](`m:erlang#trace_3_trace_messages_send_to_non_existing_process`).

* __`'receive'`__ - Traces receiving of messages.

  Message tags: [`'receive'`](`m:erlang#trace_3_trace_messages_receive`).

* __`call`__ - Traces certain function calls. Specify which function calls to trace by calling [`erlang:trace_pattern/3`](`trace_pattern/3`).

  Message tags: [`call`](`m:erlang#trace_3_trace_messages_call`) and [`return_from`](`m:erlang#trace_3_trace_messages_return_from`).

* __`silent`__ - Used with the `call` trace flag. The `call`, `return_from`, and `return_to` trace messages are inhibited if this flag is set, but they are executed as normal if there are match specifications.
  ...
  Message tags: [`call`](`m:erlang#trace_3_trace_messages_call`), [`return_from`](`m:erlang#trace_3_trace_messages_return_from`), and [`return_to`](`m:erlang#trace_3_trace_messages_return_to`). Or rather, the absence of.

* __`return_to`__ - Used with the `call` trace flag. Traces the return from a traced function back to its caller. Only works for functions traced with option `local` to [`erlang:trace_pattern/3`](`trace_pattern/3`).
...
* __`procs`__ - Traces process-related events.

  Message tags: [`spawn`](`m:erlang#trace_3_trace_messages_spawn`), [`spawned`](`m:erlang#trace_3_trace_messages_spawned`), [`exit`](`m:erlang#trace_3_trace_messages_exit`), [`register`](`m:erlang#trace_3_trace_messages_register`), [`unregister`](`m:erlang#trace_3_trace_messages_unregister`), [`link`](`m:erlang#trace_3_trace_messages_link`), [`unlink`](`m:erlang#trace_3_trace_messages_unlink`), [`getting_linked`](`m:erlang#trace_3_trace_messages_getting_linked`), and [`getting_unlinked`](`m:erlang#trace_3_trace_messages_getting_unlinked`).

* __`ports`__ - Traces port-related events.

  Message tags: [`open`](`m:erlang#trace_3_trace_messages_open`), [`closed`](`m:erlang#trace_3_trace_messages_closed`), [`register`](`m:erlang#trace_3_trace_messages_register`), [`unregister`](`m:erlang#trace_3_trace_messages_unregister`), [`getting_linked`](`m:erlang#trace_3_trace_messages_getting_linked`), and [`getting_unlinked`](`m:erlang#trace_3_trace_messages_getting_unlinked`).

* __`running`__ - Traces scheduling of processes.

  Message tags: [`in`](`m:erlang#trace_3_trace_messages_in_proc`) and [`out`](`m:erlang#trace_3_trace_messages_out_proc`).

* __`exiting`__ - Traces scheduling of exiting processes.

  Message tags: [`in_exiting`](`m:erlang#trace_3_trace_messages_in_exiting_proc`), [`out_exiting`](`m:erlang#trace_3_trace_messages_out_exiting_proc`), and [`out_exited`](`m:erlang#trace_3_trace_messages_out_exited_proc`).

* __`running_procs`__ - Traces scheduling of processes just like `running`. However, this option also includes schedule events when the process executes within the context of a port without being scheduled out itself.

  Message tags: [`in`](`m:erlang#trace_3_trace_messages_in_proc`) and [`out`](`m:erlang#trace_3_trace_messages_out_proc`).

* __`running_ports`__ - Traces scheduling of ports.

  Message tags: [`in`](`m:erlang#trace_3_trace_messages_in_port`) and [`out`](`m:erlang#trace_3_trace_messages_out_port`).

* __`garbage_collection`__ - Traces garbage collections of processes.

  Message tags: [`gc_minor_start`](`m:erlang#trace_3_trace_messages_gc_minor_start`), [`gc_max_heap_size`](`m:erlang#trace_3_trace_messages_gc_max_heap_size`), and [`gc_minor_end`](`m:erlang#trace_3_trace_messages_gc_minor_end`).

* __`timestamp`__ - Includes a time stamp in all trace messages. The time stamp (Ts) has the same form as returned by `erlang:now()`.

* __`cpu_timestamp`__ - A global trace flag for the Erlang node that makes all trace time stamps using flag `timestamp` to be in CPU time, not wall clock time. That is, `cpu_timestamp` is not be used if `monotonic_timestamp` or `strict_monotonic_timestamp` is enabled. Only allowed with `PidPortSpec==all`. If the host machine OS does not support high-resolution CPU time measurements, `trace/3` exits with `badarg`. Notice that most OS do not synchronize this value across cores, so be prepared that time can seem to go backwards when using this option.

* __`monotonic_timestamp`__ - Includes an [Erlang monotonic time](time_correction.md#erlang_monotonic_time) time stamp in all trace messages. The time stamp (Ts) has the same format and value as produced by [`erlang:monotonic_time(nanosecond)`](`monotonic_time/1`). This flag overrides flag `cpu_timestamp`.

* __`strict_monotonic_timestamp`__ - Includes an time stamp consisting of [Erlang monotonic time](time_correction.md#erlang_monotonic_time) and a monotonically increasing integer in all trace messages. The time stamp (Ts) has the same format and value as produced by `{` [`erlang:monotonic_time(nanosecond)`](`monotonic_time/1`)`,` [`erlang:unique_integer([monotonic])`](`unique_integer/1`)`}`. This flag overrides flag `cpu_timestamp`.

* __`arity`__ - Used with the `call` trace flag. `{M, F, Arity}` is specified instead of `{M, F, Args}` in call trace messages.

* __`set_on_spawn`__ - Makes any process created by a traced process inherit its trace flags, including flag `set_on_spawn`.

* __`set_on_first_spawn`__ - Makes the first process created by a traced process inherit its trace flags, excluding flag `set_on_first_spawn`.

* __`set_on_link`__ - Makes any process linked by a traced process inherit its trace flags, including flag `set_on_link`.

* __`set_on_first_link`__ - Makes the first process linked to by a traced process inherit its trace flags, excluding flag `set_on_first_link`.

* __`{tracer, Tracer}`__ - Specifies where to send the trace messages. `Tracer` must be the process identifier of a local process or the port identifier of a local port.

* __`{tracer, TracerModule, TracerState}`__ - Specifies that a tracer module is to be called instead of sending a trace message. The tracer module can then ignore or change the trace message. For more details on how to write a tracer module, see `m:erl_tracer`.
...
[](){: id=trace_3_trace_messages }
* __`{trace, PidPort, send, Msg, To}`{: id=trace_3_trace_messages_send }__ - When `PidPort` sends message `Msg` to process `To`.

* __`{trace, PidPort, send_to_non_existing_process, Msg, To}`{: id=trace_3_trace_messages_send_to_non_existing_process }__ - When `PidPort` sends message `Msg` to the non-existing process `To`.

* __`{trace, PidPort, 'receive', Msg}`{: id=trace_3_trace_messages_receive }__ - When `PidPort` receives message `Msg`. If `Msg` is set to time-out, a receive statement can have timed out, or the process received a message with the payload `timeout`.

* __`{trace, Pid, call, {M, F, Args}}`{: id=trace_3_trace_messages_call }__ - When `Pid` calls a traced function. The return values of calls are never supplied, only the call and its arguments.

  Trace flag `arity` can be used to change the contents of this message, so that `Arity` is specified instead of `Args`.

* __`{trace, Pid, return_to, {M, F, Arity}}`{: id=trace_3_trace_messages_return_to }__ - When `Pid` returns *to* the specified function. This trace message is sent if both the flags `call` and `return_to` are set, and the function is set to be traced on *local* function calls. The message is only sent when returning from a chain of tail recursive function calls, where at least one call generated a `call` trace message (that is, the functions match specification matched, and `{message, false}` was not an action).

* __`{trace, Pid, return_from, {M, F, Arity}, ReturnValue}`{: id=trace_3_trace_messages_return_from }__ - When `Pid` returns *from* the specified function. This trace message is sent if flag `call` is set, and the function has a match specification with a `return_trace` or `exception_trace` action.

* __`{trace, Pid, exception_from, {M, F, Arity}, {Class, Value}}`{: id=trace_3_trace_messages_exception_from }__ - When `Pid` exits *from* the specified function because of an exception. This trace message is sent if flag `call` is set, and the function has a match specification with an `exception_trace` action.

* __`{trace, Pid, spawn, Pid2, {M, F, Args}}`{: id=trace_3_trace_messages_spawn }__ - When `Pid` spawns a new process `Pid2` with the specified function call as entry point.

  `Args` is supposed to be the argument list, but can be any term if the spawn is erroneous.

* __`{trace, Pid, spawned, Pid2, {M, F, Args}}`{: id=trace_3_trace_messages_spawned }__ - When `Pid` is spawned by process `Pid2` with the specified function call as entry point.

  `Args` is supposed to be the argument list, but can be any term if the spawn is erroneous.

* __`{trace, Pid, exit, Reason}`{: id=trace_3_trace_messages_exit }__ - When `Pid` exits with reason `Reason`.

* __`{trace, PidPort, register, RegName}`{: id=trace_3_trace_messages_register }__ - When `PidPort` gets the name `RegName` registered.

* __`{trace, PidPort, unregister, RegName}`{: id=trace_3_trace_messages_unregister }__ - When `PidPort` gets the name `RegName` unregistered. This is done automatically when a registered process or port exits.

* __`{trace, Pid, link, Pid2}`{: id=trace_3_trace_messages_link }__ - When `Pid` links to a process `Pid2`.

* __`{trace, Pid, unlink, Pid2}`{: id=trace_3_trace_messages_unlink }__ - When `Pid` removes the link from a process `Pid2`.

* __`{trace, PidPort, getting_linked, Pid2}`{: id=trace_3_trace_messages_getting_linked }__ - When `PidPort` gets linked to a process `Pid2`.

* __`{trace, PidPort, getting_unlinked, Pid2}`{: id=trace_3_trace_messages_getting_unlinked }__ - When `PidPort` gets unlinked from a process `Pid2`.

* __`{trace, Port, open, Pid, Driver}`{: id=trace_3_trace_messages_open }__ - When `Pid` opens a new port `Port` with the running `Driver`.

  `Driver` is the name of the driver as an atom.

* __`{trace, Port, closed, Reason}`{: id=trace_3_trace_messages_closed }__ - When `Port` closes with `Reason`.

* __[](){: id=trace_3_trace_messages_in_proc }
  `{trace, Pid, in | in_exiting, {M, F, Arity} | 0}`{: id=trace_3_trace_messages_in_exiting_proc }__  
...
* __[](){: id=trace_3_trace_messages_out_proc }
  [](){: id=trace_3_trace_messages_out_exiting_proc }
  `{trace, Pid, out | out_exiting | out_exited, {M, F, Arity} | 0}`{: id=trace_3_trace_messages_out_exited_proc }__  
...
* __`{trace, Port, in, Command | 0}`{: id=trace_3_trace_messages_in_port }__ - When `Port` is scheduled to run. `Command` is the first thing the port will execute, it can however run several commands before being scheduled out. On some rare occasions, the current function cannot be determined, then the last element is `0`.

  The possible commands are `call`, `close`, `command`, `connect`, `control`, `flush`, `info`, `link`, `open`, and `unlink`.

* __`{trace, Port, out, Command | 0}`{: id=trace_3_trace_messages_out_port }__ - When `Port` is scheduled out. The last command run was `Command`. On some rare occasions, the current function cannot be determined, then the last element is `0`. `Command` can contain the same commands as `in`

* __`{trace, Pid, gc_minor_start, Info}`{: id=trace_3_trace_messages_gc_minor_start }__ - [](){: id=gc_minor_start }
  ...
  * __`heap_size`__ - The size of the used part of the heap.

  * __`heap_block_size`__ - The size of the memory block used for storing the heap and the stack.

  * __`old_heap_size`__ - The size of the used part of the old heap.

  * __`old_heap_block_size`__ - The size of the memory block used for storing the old heap.

  * __`stack_size`__ - The size of the stack.

  * __`recent_size`__ - The size of the data that survived the previous garbage collection.

  * __`mbuf_size`__ - The combined size of message buffers associated with the process.

  * __`bin_vheap_size`__ - The total size of unique off-heap binaries referenced from the process heap.

  * __`bin_vheap_block_size`__ - The total size of binaries allowed in the virtual heap in the process before doing a garbage collection.

  * __`bin_old_vheap_size`__ - The total size of unique off-heap binaries referenced from the process old heap.

  * __`bin_old_vheap_block_size`__ - The total size of binaries allowed in the virtual old heap in the process before doing a garbage collection.

  * __`wordsize`__ - For the `gc_minor_start` event it is the size of the need that triggered the GC. For the corresponding `gc_minor_end` event it is the size of reclaimed memory = start `heap_size` \- end `heap_size`.

  All sizes are in words.

* __`{trace, Pid, gc_max_heap_size, Info}`{: id=trace_3_trace_messages_gc_max_heap_size }__ - Sent when the [`max_heap_size`](`m:erlang#process_flag_max_heap_size`) is reached during garbage collection. `Info` contains the same kind of list as in message `gc_start`, but the sizes reflect the sizes that triggered `max_heap_size` to be reached.

* __`{trace, Pid, gc_minor_end, Info}`{: id=trace_3_trace_messages_gc_minor_end }__ - Sent when young garbage collection is finished. `Info` contains the same kind of list as in message `gc_minor_start`, but the sizes reflect the new sizes after garbage collection.

* __`{trace, Pid, gc_major_start, Info}`{: id=trace_3_trace_messages_gc_major_start }__ - Sent when fullsweep garbage collection is about to be started. `Info` contains the same kind of list as in message `gc_minor_start`.

* __`{trace, Pid, gc_major_end, Info}`{: id=trace_3_trace_messages_gc_major_end }__ - Sent when fullsweep garbage collection is finished. `Info` contains the same kind of list as in message `gc_minor_start`, but the sizes reflect the new sizes after a fullsweep garbage collection.
...
When used together with a [Tracer Module](`m:erl_tracer`), any message sent in the trace callback is guaranteed to have reached its recipient before the `trace_delivered` message is sent.
...
* __`traced`__ - Returns `global` if this function is traced on global function calls, `local` if this function is traced on local function calls (that is, local and global function calls), and `false` if local or global function calls are not traced.

* __`match_spec`__ - Returns the match specification for this function, if it has one. If the function is locally or globally traced but has no match specification defined, the returned value is `[]`.

* __`meta`__ - Returns the meta-trace tracer process, port, or trace module for this function, if it has one. If the function is not meta-traced, the returned value is `false`. If the function is meta-traced but has once detected that the tracer process is invalid, the returned value is `[]`.

* __`meta_match_spec`__ - Returns the meta-trace match specification for this function, if it has one. If the function is meta-traced but has no match specification defined, the returned value is `[]`.

* __`call_count`__ - Returns the call count value for this function or `true` for the pseudo function `on_load` if call count tracing is active. Otherwise `false` is returned.

  See also [`erlang:trace_pattern/3`](`trace_pattern/3`).

* __`call_time`__ - Returns the call time values for this function or `true` for the pseudo function `on_load` if call time tracing is active. Otherwise `false` is returned. The call time values returned, `[{Pid, Count, S, Us}]`, is a list of each process that executed the function and its specific counters.

  See also [`erlang:trace_pattern/3`](`trace_pattern/3`).

* __`call_memory`__ - Returns the accumulated number of words allocated by this function. Accumulation stops at the next memory traced function: if there are `outer`, `middle` and `inner` functions each allocating 3 words, but only `outer` is traced, it will report 9 allocated words. If `outer` and `inner` are traced, 6 words are reported for `outer` and 3 for `inner`. When function is not traced, `false` is returned. Returned tuple is `[{Pid, Count, Words}]`, for each process that executed the function.

  See also [`erlang:trace_pattern/3`](`trace_pattern/3`).

* __`all`__ - Returns a list containing the `{Item, Value}` tuples for all other items, or returns `false` if no tracing is active for this function.
...
* __`match_spec`__ - Returns the match specification for this event, if it has one, or `true` if no match specification has been set.
...
The link removal is performed asynchronously. If such a link does not exist, nothing is done. A detailed description of the [link protocol](erl_dist_protocol.md#link_protocol) can be found in the *Distribution Protocol* chapter of the *ERTS User's Guide*.
...
%% unregister/1
-doc """
Removes the [`registered name`](`register/2`) `RegName` associated with a process identifier or a port identifier from the [`name registry`](`p:system:ref_man_processes.md#runtime-service`). For example:
...
%% whereis/1
-doc """
Returns the process identifier or port identifier with the [`registered name`](`register/2`) `RegName` from the [`name registry`](`p:system:ref_man_processes.md#runtime-service`). Returns `undefined` if the name is not registered. Example:
...
* __`badfile`__ - The object code in `Binary` has an incorrect format *or* the object code contains code for another module than `Module`.

* __`not_purged`__ - `Binary` contains a module that cannot be loaded because old code for this module already exists.

* __`on_load`__ - The code in `Binary` contains an `on_load` declaration that must be executed before `Binary` can become the current code. Any previous current code for `Module` will remain until the `on_load` call has finished.

* __not_allowed__ - The code in `Binary` has been compiled with features that are currently not enabled in the runtime system.
...
* __`load_failed`__ - The OS failed to load the NIF library.

* __`bad_lib`__ - The library did not fulfill the requirements as a NIF library of the calling module.

* __`load | upgrade`__ - The corresponding library callback was unsuccessful.

* __`reload`__ - A NIF library is already loaded for this module instance. The previously deprecated `reload` feature was removed in OTP 20.

* __`old_code`__ - The call to `load_nif/2` was made from the old code of a module that has been upgraded; this is not allowed.

If the [`-nifs()`](`p:system:modules.md#nifs_attribute`) attribute is used (which is recommended), all NIFs in the dynamic library much be declared as such for `load_nif/2` to succeed. On the other hand, all functions declared with the `-nifs()` attribute do not have to be implemented by the dynamic library. This allows a target independent Erlang file to contain fallback implementations for functions that may lack NIF support depending on target OS/hardware platform.
""".
...
* __`{spawn, Command}`__ - Starts an external program. `Command` is the name of the external program to be run. `Command` runs outside the Erlang work space unless an Erlang driver with the name `Command` is found. If found, that driver is started. A driver runs in the Erlang work space, which means that it is linked with the Erlang runtime system.
...
* __`{spawn_driver, Command}`__ - Works like `{spawn, Command}`, but demands the first (space-separated) token of the command to be the name of a loaded driver. If no driver with that name is loaded, a `badarg` error is raised.

* __`{spawn_executable, FileName}`__ - Works like `{spawn, FileName}`, but only runs external executables. `FileName` in its whole is used as the name of the executable, including any spaces. If arguments are to be passed, the `PortSettings` `args` and `arg0` can be used.
...
* __`{fd, In, Out}`__ - Allows an Erlang process to access any currently opened file descriptors used by Erlang. The file descriptor `In` can be used for standard input, and the file descriptor `Out` for standard output. It is only used for various servers in the Erlang OS (`shell` and `user`). Hence, its use is limited.

`PortSettings` is a list of settings for the port. The valid settings are as follows:

* __`{packet, N}`__ - Messages are preceded by their length, sent in `N` bytes, with the most significant byte first. The valid values for `N` are 1, 2, and 4.

* __`stream`__ - Output messages are sent without packet lengths. A user-defined protocol must be used between the Erlang process and the external object.

* __`{line, L}`__ - Messages are delivered on a per line basis. Each line (delimited by the OS-dependent newline sequence) is delivered in a single message. The message data format is `{Flag, Line}`, where `Flag` is `eol` or `noeol`, and `Line` is the data delivered (without the newline sequence).
...
* __`{cd, Dir}`__ - Only valid for `{spawn, Command}` and `{spawn_executable, FileName}`. The external program starts using `Dir` as its working directory. `Dir` must be a string.

* __`{env, Env}`__ - Types:  
    `Name = `[`os:env_var_name()`](`t:os:env_var_name/0`)  
    `Val = `[`os:env_var_value()`](`t:os:env_var_value/0`)` | false`  
    `Env = [{Name, Val}]`
...
* __`{args, [ string() | binary() ]}`__ - Only valid for `{spawn_executable, FileName}` and specifies arguments to the executable. Each argument is specified as a separate string and (on Unix) eventually ends up as one element each in the argument vector. On other platforms, a similar behavior is mimicked.
...
* __`{arg0, string() | binary()}`__ - Only valid for `{spawn_executable, FileName}` and explicitly specifies the program name argument when running an executable. This can in some circumstances, on some OSs, be desirable. How the program responds to this is highly system-dependent and no specific effect is guaranteed.

* __`exit_status`__ - Only valid for `{spawn, Command}`, where `Command` refers to an external program, and for `{spawn_executable, FileName}`.
...
* __`use_stdio`__ - Only valid for `{spawn, Command}` and `{spawn_executable, FileName}`. It allows the standard input and output (file descriptors 0 and 1) of the spawned (Unix) process for communication with Erlang.

* __`nouse_stdio`__ - The opposite of `use_stdio`. It uses file descriptors 3 and 4 for communication with Erlang.

* __`stderr_to_stdout`__ - Affects ports to external programs. The executed program gets its standard error file redirected to its standard output file. `stderr_to_stdout` and `nouse_stdio` are mutually exclusive.

* __`overlapped_io`__ - Affects ports to external programs on Windows only. The standard input and standard output handles of the port program are, if this option is supplied, opened with flag `FILE_FLAG_OVERLAPPED`, so that the port program can (and must) do overlapped I/O on its standard handles. This is not normally the case for simple port programs, but an option of value for the experienced Windows programmer. *On all other platforms, this option is silently discarded.*

* __`in`__ - The port can only be used for input.

* __`out`__ - The port can only be used for output.

* __`binary`__ - All I/O from the port is binary data objects as opposed to lists of bytes.

* __`eof`__ - The port is not closed at the end of the file and does not produce an exit signal. Instead, it remains open and a `{Port, eof}` message is sent to the process holding the port.

* __`hide`__ - When running on Windows, suppresses creation of a new console window when spawning the port program. (This option has no effect on other platforms.)

* __`{parallelism, Boolean}`__ - [](){: id=open_port_parallelism }
  Sets scheduler hint for port parallelism. If set to `true`, the virtual machine schedules port tasks; when doing so, it improves parallelism in the system. If set to `false`, the virtual machine tries to perform port tasks immediately, improving latency at the expense of parallelism. The default can be set at system startup by passing command-line argument [`+spp`](erl_cmd.md#+spp) to `p:erl_cmd.md`.

* __`{busy_limits_port, {Low, High} | disabled}`__ - Sets limits that will be used for controlling the busy state of the port.
...
* __`{busy_limits_msgq, {Low, High} | disabled}`__ - Sets limits that will be used for controlling the busy state of the port message queue.
  ...
  *Note* that the driver might fail if it also adjust these limits by itself and you have disabled this feature.
  ...
  For more information see the documentation [`erl_drv_busy_msgq_limits()`](erl_driver.md#erl_drv_busy_msgq_limits).
...
* __`badarg`__ - Bad input arguments to `open_port`.

* __`system_limit`__ - All available ports in the Erlang emulator are in use.

* __`enomem`__ - Not enough memory to create the port.

* __`eagain`__ - No more available OS processes.

* __`enametoolong`__ - Too long external command.

* __`emfile`__ - No more available file descriptors (for the OS process that the Erlang emulator runs in).

* __`enfile`__ - Full file table (for the entire OS).

* __`eacces`__ - `Command` specified in `{spawn_executable, Command}` does not point out an executable file.

* __`enoent`__ - `FileName` specified in `{spawn_executable, FileName}` does not point out an existing file.

During use of a port opened using `{spawn, Name}`, `{spawn_driver, Name}`, or `{spawn_executable, Name}`, errors arising when sending messages to it are reported to the owning process using signals of the form `{'EXIT', Port, PosixCode}`. For the possible values of `PosixCode`, see `m:file`.

The maximum number of ports that can be open at the same time can be configured by passing command-line flag [`+Q`](erl_cmd.md#max_ports) to `p:erl_cmd.md`.
""".
...
-doc "Process priority level. For more info see [`process_flag(priority, Level)`](`m:erlang#process_flag_priority`)".
-type priority_level() ::
      low | normal | high | max.

-doc """
See [`process_flag(message_queue_data, MQD)`](`m:erlang#process_flag_message_queue_data`).

Process message queue data configuration. For more information, see [`process_flag(message_queue_data, MQD)`](`m:erlang#process_flag_message_queue_data`)
""".
-type message_queue_data() ::
	off_heap | on_heap.

-doc """
[](){: id=process_flag_async_dist }
Enable or disable *fully asynchronous distributed signaling* for the calling process. When disabled, which is the default, the process sending a distributed signal will block in the send operation if the buffer for the distribution channel reach the [distribution buffer busy limit](erl_cmd.md#+zdbbl). The process will remain blocked until the buffer shrinks enough. This might in some cases take a substantial amount of time. When `async_dist` is enabled, send operations of distributed signals will always buffer the signal on the outgoing distribution channel and then immediately return. That is, these send operations will *never* block the sending process.
...
Blocking due to disabled `async_dist` can be monitored by [`erlang:system_monitor()`](`system_monitor/2`) using the [`busy_dist_port`](`m:erlang#busy_dist_port`) option. Only data buffered by processes which (at the time of sending a signal) have disabled `async_dist` will be counted when determining whether or not an operation should block the caller.

The `async_dist` flag can also be set on a new process when spawning it using the [`spawn_opt()`](`spawn_opt/4`) BIF with the option [`{async_dist, Enable}`](`m:erlang#spawn_opt_async_dist`). The default `async_dist` flag to use on newly spawned processes can be set by passing the command line argument [`+pad <boolean>`](erl_cmd.md#+pad) when starting the runtime system. If the `+pad <boolean>` command line argument is not passed, the default value of the `async_dist` flag will be `false`.

You can inspect the state of the `async_dist` process flag of a process by calling [`process_info(Pid, async_dist)`](`m:erlang#process_info_async_dist`).

Returns the old value of the `async_dist` flag.

[](){: id=process_flag_trap_exit }
...
Used by a process to redefine the error handler for undefined function calls and undefined registered processes. Use this flag with substantial caution, as code auto-loading depends on the correct operation of the error handling module.
...
For details on how the heap grows, see [Sizing the heap](garbagecollection.md#sizing-the-heap) in the ERTS internal documentation.

* __`size`__ - The maximum size in words of the process. If set to zero, the heap size limit is disabled. `badarg` is be thrown if the value is smaller than [`min_heap_size`](`m:erlang#process_flag_min_heap_size`). The size check is only done when a garbage collection is triggered.

  `size` is the entire heap of the process when garbage collection is triggered. This includes all generational heaps, the process stack, any [messages that are considered to be part of the heap](`m:erlang#process_flag_message_queue_data`), and any extra memory that the garbage collector needs during collection.

  `size` is the same as can be retrieved using [`erlang:process_info(Pid, total_heap_size)`](`m:erlang#process_info_total_heap_size`), or by adding `heap_block_size`, `old_heap_block_size` and `mbuf_size` from [`erlang:process_info(Pid, garbage_collection_info)`](`m:erlang#process_info_garbage_collection_info`).

* __`kill`__ - When set to `true`, the runtime system sends an untrappable exit signal with reason `kill` to the process if the maximum heap size is reached. The garbage collection that triggered the `kill` is not completed, instead the process exits as soon as possible. When set to `false`, no exit signal is sent to the process, instead it continues executing.

  If `kill` is not defined in the map, the system default will be used. The default system default is `true`. It can be changed by either option [\+hmaxk](erl_cmd.md#+hmaxk) in `p:erl_cmd.md`, or [`erlang:system_flag(max_heap_size, MaxHeapSize)`](`m:erlang#system_flag_max_heap_size`).

* __`error_logger`__ - When set to `true`, the runtime system logs an error event via `m:logger`, containing details about the process when the maximum heap size is reached. One log event is sent each time the limit is reached.

  If `error_logger` is not defined in the map, the system default is used. The default system default is `true`. It can be changed by either the option [\+hmaxel](erl_cmd.md#+hmaxel) int `p:erl_cmd.md`, or [`erlang:system_flag(max_heap_size, MaxHeapSize)`](`m:erlang#system_flag_max_heap_size`).

* __`include_shared_binaries`__ - When set to `true`, off-heap binaries are included in the total sum compared against the `size` limit. Off-heap binaries are typically larger binaries that may be shared between processes. The size of a shared binary is included by all processes that are referring it. Also, the entire size of a large binary may be included even if only a smaller part of it is referred by the process.

  If `include_shared_binaries` is not defined in the map, the system default is used. The default system default is `false`. It can be changed by either the option [\+hmaxib](erl_cmd.md#+hmaxib) in `p:erl_cmd.md`, or [`erlang:system_flag(max_heap_size, MaxHeapSize)`](`m:erlang#system_flag_max_heap_size`).
...
[](){: id=process_flag_message_queue_data }
```erlang
-type message_queue_data() :: off_heap | on_heap.
```
...
* __`off_heap`__ - *All* messages in the message queue will be stored outside the process heap. This implies that *no* messages in the message queue will be part of a garbage collection of the process.

* __`on_heap`__ - All messages in the message queue will eventually be placed on the process heap. They can, however, be temporarily stored off the heap. This is how messages have always been stored up until ERTS 8.0.

The default value of the `message_queue_data` process flag is determined by the command-line argument [`+hmqd`](erl_cmd.md#+hmqd) in `p:erl_cmd.md`.
...
Getting process information follows the signal ordering guarantees described in the [Processes Chapter](`p:system:ref_man_processes.md#signals`) in the *Erlang Reference Manual*.
...
* __`{async_dist, Enabled}`{: id=process_info_async_dist }__ - Since: OTP 25.3

  Current value of the [`async_dist`](`m:erlang#process_flag_async_dist`) process flag.

* __`{backtrace, Bin}`__ - Binary `Bin` contains the same information as the output from `erlang:process_display(Pid, backtrace)`. Use `binary_to_list/1` to obtain the string of characters from the binary.

* __`{binary, BinInfo}`__ - `BinInfo` is a list containing miscellaneous information about binaries on the heap of this process. This `InfoTuple` can be changed or removed without prior notice. In the current implementation `BinInfo` is a list of tuples. The tuples contain; `BinaryId`, `BinarySize`, `BinaryRefcCount`.

  Depending on the value of the [`message_queue_data`](`m:erlang#process_flag_message_queue_data`) process flag the message queue may be stored on the heap.

* __`{catchlevel, CatchLevel}`__ - `CatchLevel` is the number of currently active catches in this process. This `InfoTuple` can be changed or removed without prior notice.

* __`{current_function, {Module, Function, Arity} | undefined}`__ - `Module`, `Function`, `Arity` is the current function call of the process. The value `undefined` can be returned if the process is currently executing native compiled code.

* __`{current_location, {Module, Function, Arity, Location}}`__ - `Module`, `Function`, `Arity` is the current function call of the process. `Location` is a list of two-tuples describing the location in the source code.

* __`{current_stacktrace, Stack}`__ - Returns the current call stack back-trace (*stacktrace*) of the process. The stack has the same format as in the `catch` part of a `try`. See [The call-stack back trace (stacktrace)](`p:system:errors.md#stacktrace`). The depth of the stacktrace is truncated according to the `backtrace_depth` system flag setting.

* __`{dictionary, Dictionary}`__ - `Dictionary` is the process dictionary.

* __`{{dictionary, Key}, Value}`__ - `Value` associated with `Key` in the process dictionary.

* __`{error_handler, Module}`__ - `Module` is the error handler module used by the process (for undefined function calls, for example).

* __`{garbage_collection, GCInfo}`__ - `GCInfo` is a list containing miscellaneous information about garbage collection for this process. The content of `GCInfo` can be changed without prior notice.

* __`{garbage_collection_info, GCInfo}`{: id=process_info_garbage_collection_info }__ - `GCInfo` is a list containing miscellaneous detailed information about garbage collection for this process. The content of `GCInfo` can be changed without prior notice. For details about the meaning of each item, see [`gc_minor_start`](`m:erlang#gc_minor_start`) in [`erlang:trace/3`](`trace/3`).

* __`{group_leader, GroupLeader}`__ - `GroupLeader` is the group leader for the I/O of the process.

* __`{heap_size, Size}`__ - `Size` is the size in words of the youngest heap generation of the process. This generation includes the process stack. This information is highly implementation-dependent, and can change if the implementation changes.

* __`{initial_call, {Module, Function, Arity}}`__ - `Module`, `Function`, `Arity` is the initial function call with which the process was spawned.

* __`{links, PidsAndPorts}`__ - `PidsAndPorts` is a list of process identifiers and port identifiers, with processes or ports to which the process has a link.

* __`{last_calls, false|Calls}`__ - The value is `false` if call saving is not active for the process (see `process_flag/3`). If call saving is active, a list is returned, in which the last element is the most recent called.

* __`{memory, Size}`__ - [](){: id=process_info_memory }
  `Size` is the size in bytes of the process. This includes call stack, heap, and internal structures.

* __`{message_queue_len, MessageQueueLen}`__ - `MessageQueueLen` is the number of messages currently in the message queue of the process. This is the length of the list `MessageQueue` returned as the information item `messages` (see below).

* __`{messages, MessageQueue}`__ - `MessageQueue` is a list of the messages to the process, which have not yet been processed.

* __`{min_heap_size, MinHeapSize}`__ - `MinHeapSize` is the minimum heap size for the process.

* __`{min_bin_vheap_size, MinBinVHeapSize}`__ - `MinBinVHeapSize` is the minimum binary virtual heap size for the process.

* __`{monitored_by, MonitoredBy}`__ - A list of identifiers for all the processes, ports and NIF resources, that are monitoring the process.

* __`{monitors, Monitors}`__ - A list of monitors (started by `monitor/2`) that are active for the process. For a local process monitor or a remote process monitor by a process identifier, the list consists of:

  * __`{process, Pid}`__ - Process is monitored by pid.

  * __`{process, {RegName, Node}}`__ - Local or remote process is monitored by name.

  * __`{port, PortId}`__ - Local port is monitored by port id.

  * __`{port, {RegName, Node}}`__ - Local port is monitored by name. Please note, that remote port monitors are not supported, so `Node` will always be the local node name.

  

* __`{message_queue_data, MQD}`__ - `MQD` is the current value of the `message_queue_data` process flag, which can be either `off_heap` or `on_heap`. For more information, see the documentation of [`process_flag(message_queue_data, MQD)`](`m:erlang#process_flag_message_queue_data`).

* __`{parent, Pid}`__ - `Pid` is the identifier of the parent process, the one that spawned current process. When the process does not have a parent `undefined` is returned. Only the initial process (`init`) on a node lacks a parent, though.

* __`{priority, Level}`__ - `Level` is the current priority level for the process. For more information on priorities, see [`process_flag(priority, Level)`](`m:erlang#process_flag_priority`).

* __`{reductions, Number}`__ - `Number` is the number of reductions executed by the process.

* __`{registered_name, Atom}`__ - `Atom` is the registered process name. If the process has no registered name, this tuple is not present in the list.

* __`{sequential_trace_token, [] | SequentialTraceToken}`__ - `SequentialTraceToken` is the sequential trace token for the process. This `InfoTuple` can be changed or removed without prior notice.

* __`{stack_size, Size}`__ - `Size` is the stack size, in words, of the process.

* __`{status, Status}`__ - `Status` is the status of the process and is one of the following:
...
* __`{suspending, SuspendeeList}`__ - `SuspendeeList` is a list of `{Suspendee, ActiveSuspendCount, OutstandingSuspendCount}` tuples. `Suspendee` is the process identifier of a process that has been, or is to be, suspended by the process identified by `Pid` through the BIF [`erlang:suspend_process/2`](`suspend_process/2`) or [`erlang:suspend_process/1`](`suspend_process/1`).
...
* __`{total_heap_size, Size}`{: id=process_info_total_heap_size }__ - `Size` is the total size, in words, of all heap fragments of the process. This includes the process stack and any unreceived messages that are considered to be part of the heap.

* __`{trace, InternalTraceFlags}`__ - `InternalTraceFlags` is an integer representing the internal trace flag for this process. This `InfoTuple` can be changed or removed without prior notice.

* __`{trap_exit, Boolean}`__ - `Boolean` is `true` if the process is trapping exits, otherwise `false`.

Notice that not all implementations support all these `Item`s.

Failures:

* __`badarg`__ - If `Pid` is not a local process.

* __`badarg`__ - If `Item` is an invalid item.
""".
...
-doc """
Sends a message and returns `Msg`. This is the same as using the [send operator](`p:system:expressions.md#send`): `Dest ! Msg`.
...
-doc """
Either sends a message and returns `ok`, or does not send the message but returns something else (see below). Otherwise the same as [`erlang:send/2`](`send/2`). For more detailed explanation and warnings, see [`erlang:send_nosuspend/2,3`](`send_nosuspend/2`).

Options:

* __`nosuspend`__ - If the sender would have to be suspended to do the send, `nosuspend` is returned instead.

* __`noconnect`__ - If the destination node would have to be auto-connected to do the send, `noconnect` is returned instead.
...
-doc """
[](){: id=statistics_active_tasks }
Returns the same as [`statistics(active_tasks_all)`](`m:erlang#statistics_active_tasks_all`) with the exception that no information about the dirty IO run queue and its associated schedulers is part of the result. That is, only tasks that are expected to be CPU bound are part of the result.

[](){: id=statistics_active_tasks_all }
...
See also [`statistics(total_active_tasks)`](`m:erlang#statistics_total_active_tasks`), [`statistics(run_queue_lengths)`](`m:erlang#statistics_run_queue_lengths`), [`statistics(run_queue_lengths_all)`](`m:erlang#statistics_run_queue_lengths_all`), [`statistics(total_run_queue_lengths)`](`m:erlang#statistics_total_run_queue_lengths`), and [`statistics(total_run_queue_lengths_all)`](`m:erlang#statistics_total_run_queue_lengths_all`).

Returns the total number of context switches since the system started.

[](){: id=statistics_exact_reductions }
Returns the number of exact reductions.

> #### Note {: class=info }
> `statistics(exact_reductions)` is a more expensive operation than [statistics(reductions)](`m:erlang#statistics_reductions`).
...
* __`scheduler`__ - The main execution threads that do most of the work. See [erl +S](erl_cmd.md#+s) for more details.

* __`dirty_cpu_scheduler`__ - The threads for long running cpu intensive work. See [erl +SDcpu](erl_cmd.md#+sdcpu) for more details.

* __`dirty_io_scheduler`__ - The threads for long running I/O work. See [erl +SDio](erl_cmd.md#+sdio) for more details.

* __`async`__ - Async threads are used by various linked-in drivers (mainly the file drivers) do offload non-CPU intensive work. See [erl +A](erl_cmd.md#async_thread_pool_size) for more details.

* __`aux`__ - Takes care of any work that is not specifically assigned to a scheduler.

* __`poll`__ - Does the IO polling for the emulator. See [erl +IOt](erl_cmd.md#+iot) for more details.
...
* __`aux`__ - Time spent handling auxiliary jobs.

* __`check_io`__ - Time spent checking for new I/O events.

* __`emulator`__ - Time spent executing Erlang processes.

* __`gc`__ - Time spent doing garbage collection. When extra states are enabled this is the time spent doing non-fullsweep garbage collections.

* __`other`__ - Time spent doing unaccounted things.

* __`port`__ - Time spent executing ports.

* __`sleep`__ - Time spent sleeping.
...
* __`alloc`__ - Time spent managing memory. Without extra states this time is spread out over all other states.

* __`bif`__ - Time spent in BIFs. Without extra states this time is part of the `emulator` state.

* __`busy_wait`__ - Time spent busy waiting. This is also the state where a scheduler no longer reports that it is active when using [`statistics(scheduler_wall_time)`](`m:erlang#statistics_scheduler_wall_time`). So, if you add all other states but this and sleep, and then divide that by all time in the thread, you should get something very similar to the `scheduler_wall_time` fraction. Without extra states this time is part of the `other` state.

* __`ets`__ - Time spent executing ETS BIFs. Without extra states this time is part of the `emulator` state.

* __`gc_full`__ - Time spent doing fullsweep garbage collection. Without extra states this time is part of the `gc` state.

* __`nif`__ - Time spent in NIFs. Without extra states this time is part of the `emulator` state.

* __`send`__ - Time spent sending messages (processes only). Without extra states this time is part of the `emulator` state.

* __`timers`__ - Time spent managing timers. Without extra states this time is part of the `other` state.

The utility module `m:msacc` can be used to more easily analyse these statistics.

Returns `undefined` if system flag [`microstate_accounting`](`m:erlang#system_flag_microstate_accounting`) is turned off.
...
> #### Change {: class=neutral }
> As from ERTS 5.5 (Erlang/OTP R11B), this value does not include reductions performed in current time slices of currently scheduled processes. If an exact value is wanted, use [`statistics(exact_reductions)`](`m:erlang#statistics_exact_reductions`).

[](){: id=statistics_run_queue }
Returns the total length of all normal and dirty CPU run queues. That is, queued work that is expected to be CPU bound. The information is gathered atomically. That is, the result is a consistent snapshot of the state, but this operation is much more expensive compared to [`statistics(total_run_queue_lengths)`](`m:erlang#statistics_total_run_queue_lengths`), especially when a large amount of schedulers is used.

[](){: id=statistics_run_queue_lengths }
Returns the same as [`statistics(run_queue_lengths_all)`](`m:erlang#statistics_run_queue_lengths_all`) with the exception that no information about the dirty IO run queue is part of the result. That is, only run queues with work that is expected to be CPU bound is part of the result.

[](){: id=statistics_run_queue_lengths_all }
...
See also [`statistics(run_queue_lengths)`](`m:erlang#statistics_run_queue_lengths`), [`statistics(total_run_queue_lengths_all)`](`m:erlang#statistics_total_run_queue_lengths_all`), [`statistics(total_run_queue_lengths)`](`m:erlang#statistics_total_run_queue_lengths`), [`statistics(active_tasks)`](`m:erlang#statistics_active_tasks`), [`statistics(active_tasks_all)`](`m:erlang#statistics_active_tasks_all`), and [`statistics(total_active_tasks)`](`m:erlang#statistics_total_active_tasks`), [`statistics(total_active_tasks_all)`](`m:erlang#statistics_total_active_tasks_all`).
...
[](){: id=statistics_scheduler_wall_time }
Returns information describing how much time [normal](`m:erlang#system_info_schedulers`) and [dirty CPU](`m:erlang#system_info_dirty_cpu_schedulers`) schedulers in the system have been busy. This value is normally a better indicator of how much load an Erlang node is under instead of looking at the CPU utilization provided by tools such as `top` or `sysstat`. This is because `scheduler_wall_time` also includes time where the scheduler is waiting for some other reasource (such as an internal mutex) to be available but does not use the CPU. In order to better understand what a scheduler is busy doing you can use [microstate accounting](`m:erlang#statistics_microstate_accounting`).

The definition of a busy scheduler is when it is not idle and not [busy waiting](erl_cmd.md#+sbwt) for new work, that is:
...
> #### Note {: class=info }
> It is recommended to use the module `m:scheduler` instead of this function directly as it provides an easier way to get the information that you usually want.

If [enabled](`m:erlang#system_flag_scheduler_wall_time`) this function returns a list of tuples with `{SchedulerId, ActiveTime, TotalTime}`, where `SchedulerId` is an integer ID of the scheduler, `ActiveTime` is the duration the scheduler has been busy, and `TotalTime` is the total time duration since [`scheduler_wall_time`](`m:erlang#system_flag_scheduler_wall_time`) activation for the specific scheduler. The time unit returned is undefined and can be subject to change between releases, OSs, and system restarts. `scheduler_wall_time` is only to be used to calculate relative values for scheduler utilization. The `ActiveTime` can never exceed `TotalTime`. The list of scheduler information is unsorted and can appear in different order between calls.

The [disabled](`m:erlang#system_flag_scheduler_wall_time`) this function returns `undefined`.
...
Only information about schedulers that are expected to handle CPU bound work is included in the return values from this function. If you also want information about [dirty I/O schedulers](`m:erlang#system_info_dirty_io_schedulers`), use [`statistics(scheduler_wall_time_all)`](`m:erlang#statistics_scheduler_wall_time_all`) instead.

Normal schedulers will have scheduler identifiers in the range `1 =< SchedulerId =< `[`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`). Dirty CPU schedulers will have scheduler identifiers in the range `erlang:system_info(schedulers) < SchedulerId =< erlang:system_info(schedulers) + `[`erlang:system_info(dirty_cpu_schedulers)`](`m:erlang#system_info_dirty_cpu_schedulers`).
...
> #### Note {: class=info }
> `scheduler_wall_time` is by default disabled. To enable it, use [`erlang:system_flag(scheduler_wall_time, true)`](`m:erlang#system_flag_scheduler_wall_time`).

[](){: id=statistics_scheduler_wall_time_all }
The same as [`statistics(scheduler_wall_time)`](`m:erlang#statistics_scheduler_wall_time`), except that it also include information about all dirty I/O schedulers.

Dirty IO schedulers will have scheduler identifiers in the range [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`)` + `[`erlang:system_info(dirty_cpu_schedulers)`](`m:erlang#system_info_dirty_cpu_schedulers`)` < SchedulerId =< erlang:system_info(schedulers) + erlang:system_info(dirty_cpu_schedulers) + `[`erlang:system_info(dirty_io_schedulers)`](`m:erlang#system_info_dirty_io_schedulers`).
...
[](){: id=statistics_total_active_tasks }
The same as calling `lists:sum(`[`statistics(active_tasks)`](`m:erlang#statistics_active_tasks`)`)`, but more efficient.

[](){: id=statistics_total_active_tasks_all }
The same as calling `lists:sum(`[`statistics(active_tasks_all)`](`m:erlang#statistics_active_tasks_all`)`)`, but more efficient.

[](){: id=statistics_total_run_queue_lengths }
The same as calling `lists:sum(`[`statistics(run_queue_lengths)`](`m:erlang#statistics_run_queue_lengths`)`)`, but more efficient.

[](){: id=statistics_total_run_queue_lengths_all }
The same as calling `lists:sum(`[`statistics(run_queue_lengths_all)`](`m:erlang#statistics_run_queue_lengths_all`)`)`, but more efficient.
...
> #### Warning {: class=warning }
> *This argument is deprecated.* Instead of using this argument, use command-line argument [`+sct`](erl_cmd.md#+sct) in `p:erl_cmd.md`.
>
...
The user-defined CPU topology can also be set by passing command-line argument [`+sct`](erl_cmd.md#+sct) to `p:erl_cmd.md`.

For information on type `CpuTopology` and more, see [`erlang:system_info(cpu_topology)`](`m:erlang#system_info_cpu_topology`) as well as command-line flags [`+sct`](erl_cmd.md#+sct) and [`+sbt`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

[](){: id=system_flag_dirty_cpu_schedulers_online }
...
For more information, see [`erlang:system_info(dirty_cpu_schedulers)`](`m:erlang#system_info_dirty_cpu_schedulers`) and [`erlang:system_info(dirty_cpu_schedulers_online)`](`m:erlang#system_info_dirty_cpu_schedulers_online`).

Sets system flags for [`erts_alloc(3)`](erts_alloc.md). `Alloc` is the allocator to affect, for example `binary_alloc`. `F` is the flag to change and `V` is the new value.

Only a subset of all `erts_alloc` flags can be changed at run time. This subset is currently only the flag [`sbct`](erts_alloc.md#m_sbct).
...
[](){: id=system_flag_microstate_accounting }
Turns on/off microstate accounting measurements. When passing reset, all counters are reset to 0.

For more information see [`statistics(microstate_accounting)`](`m:erlang#statistics_microstate_accounting`).
...
Sets the default minimum binary virtual heap size for processes. The size is specified in words. The new `min_bin_vhheap_size` effects only processes spawned after the change of `min_bin_vheap_size` has been made. `min_bin_vheap_size` can be set for individual processes by using [`spawn_opt/2,3,4`](`spawn_opt/4`) or `process_flag/2`.

Returns the old value of the flag.

[](){: id=system_flag_max_heap_size }
```erlang
-type max_heap_size() ::
          Size ::
              non_neg_integer() |
...
Sets the default maximum heap size settings for processes. The size is specified in words. The new `max_heap_size` effects only processes spawned after the change has been made. `max_heap_size` can be set for individual processes using [`spawn_opt/2,3,4`](`spawn_opt/4`) or [`process_flag/2`](`m:erlang#process_flag_max_heap_size`).

Returns the old value of the flag.

For details on how the heap grows, see [Sizing the heap](garbagecollection.md#sizing-the-heap) in the ERTS internal documentation.

[](){: id=system_flag_multi_scheduling }
...
The return values are `disabled`, `blocked`, `blocked_normal`, or `enabled`. The returned value describes the state just after the call to `erlang:system_flag(multi_scheduling, BlockState)` has been made. For information about the return values, see [`erlang:system_info(multi_scheduling)`](`m:erlang#system_info_multi_scheduling`).
...
See also [`erlang:system_info(multi_scheduling)`](`m:erlang#system_info_multi_scheduling`), [`erlang:system_info(normal_multi_scheduling_blockers)`](`m:erlang#system_info_normal_multi_scheduling_blockers`), [`erlang:system_info(multi_scheduling_blockers)`](`m:erlang#system_info_multi_scheduling_blockers`), and [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`).

[](){: id=system_flag_outstanding_system_requests_limit }
...
* __The Code Purger__ - The code purger orchestrates checking of references to old code before old code is removed from the system.

* __The Literal Area Collector__ - The literal area collector orchestrates copying of references from old literal areas before removal of such areas from the system.

Each of these processes are allowed to have as many outstanding requests as this limit is set to. By default this limit is set to twice the amount of [schedulers](`m:erlang#system_info_schedulers`) on the system. This will ensure that schedulers will have enough work scheduled to perform these operations as quickly as possible at the same time as other work will be interleaved with this work. Currently used limit can be checked by calling [`erlang:system_info(outstanding_system_requests_limit)`](`m:erlang#system_info_outstanding_system_requests_limit`).

This limit can also be set by passing the command line argument [`+zosrl <Limit>`](erl_cmd.md#+zosrl) to `erl`.

[](){: id=system_flag_scheduler_bind_type }
```erlang
-type scheduler_bind_type() ::
...
> #### Warning {: class=warning }
> *This argument is deprecated.* Instead of using this argument, use command-line argument [`+sbt`](erl_cmd.md#+sbt) in `p:erl_cmd.md`. When this argument is removed, a final scheduler bind type to use is determined at emulator boot time.
...
> #### Note {: class=info }
> If a scheduler fails to bind, this is often silently ignored, as it is not always possible to verify valid logical processor identifiers. If an error is reported, an error event is logged. To verify that the schedulers have bound as requested, call [`erlang:system_info(scheduler_bindings)`](`m:erlang#system_info_scheduler_bindings`).
...
In order for the runtime system to be able to bind schedulers, the CPU topology must be known. If the runtime system fails to detect the CPU topology automatically, it can be defined. For more information on how to define the CPU topology, see command-line flag [`+sct`](erl_cmd.md#+sct) in `p:erl_cmd.md`.
...
* __`unbound`__ - Same as command-line argument [`+sbt u`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`no_spread`__ - Same as command-line argument [`+sbt ns`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`thread_spread`__ - Same as command-line argument [`+sbt ts`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`processor_spread`__ - Same as command-line argument [`+sbt ps`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`spread`__ - Same as command-line argument [`+sbt s`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`no_node_thread_spread`__ - Same as command-line argument [`+sbt nnts`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`no_node_processor_spread`__ - Same as command-line argument [`+sbt nnps`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`thread_no_node_processor_spread`__ - Same as command-line argument [`+sbt tnnps`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

* __`default_bind`__ - Same as command-line argument [`+sbt db`](erl_cmd.md#+sbt) in `p:erl_cmd.md`.

The returned value equals `How` before flag `scheduler_bind_type` was changed.

Failures:

* __`notsup`__ - If binding of schedulers is not supported.

* __`badarg`__ - If `How` is not one of the documented alternatives.

* __`badarg`__ - If CPU topology information is unavailable.

The scheduler bind type can also be set by passing command-line argument [`+sbt`](erl_cmd.md#+sbt) to `p:erl_cmd.md`.

For more information, see [`erlang:system_info(scheduler_bind_type)`](`m:erlang#system_info_scheduler_bind_type`), [`erlang:system_info(scheduler_bindings)`](`m:erlang#system_info_scheduler_bindings`), as well as command-line flags [`+sbt`](erl_cmd.md#+sbt) and [`+sct`](erl_cmd.md#+sct) in `p:erl_cmd.md`.

[](){: id=system_flag_scheduler_wall_time }
Try enable or disable scheduler wall time measurements by passing `Boolean` as either `true` or `false`.

For more information about how to use scheduler wall time measurements, see [`statistics(scheduler_wall_time)`](`m:erlang#statistics_scheduler_wall_time`).
...
[](){: id=system_flag_schedulers_online }
Sets the number of schedulers online. Range is `1 <= SchedulersOnline <= erlang:system_info(schedulers)`.

Returns the old value of the flag.

If the emulator was built with support for [dirty schedulers](`m:erlang#system_flag_dirty_cpu_schedulers_online`), changing the number of schedulers online can also change the number of dirty CPU schedulers online. For example, if 12 schedulers and 6 dirty CPU schedulers are online, and `system_flag/2` is used to set the number of schedulers online to 6, then the number of dirty CPU schedulers online is automatically decreased by half as well, down to 3. Similarly, the number of dirty CPU schedulers online increases proportionally to increases in the number of schedulers online.

For more information, see [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`) and [`erlang:system_info(schedulers_online)`](`m:erlang#system_info_schedulers_online`).
...
> #### Note {: class=info }
> This function is designed to be used by the KERNEL `m:logger`. Be careful if you change it to something else as log messages may be lost. If you want to intercept emulator log messages, do it by adding a specialized handler to the KERNEL logger.

Sets the value of the node trace control word to `TCW`, which is to be an unsigned integer. For more information, see function [`set_tcw`](match_spec.md#set_tcw) in section "Match Specifications in Erlang" in the User's Guide.

Returns the old value of the flag.

[](){: id=system_flag_time_offset }
Finalizes the [time offset](`time_offset/0`) when [single time warp mode](time_correction.md#single_time_warp_mode) is used. If another time warp mode is used, the time offset state is left unchanged.
...
* If `final` is returned, the time offset was already in the final state. This either because another `erlang:system_flag(time_offset, finalize)` call or because [no time warp mode](time_correction.md#no_time_warp_mode) is used.
* If `volatile` is returned, the time offset cannot be finalized because [multi-time warp mode](time_correction.md#multi_time_warp_mode) is used.
""".
-doc(#{since =>
...
-doc """
Returns a binary data object that is the result of encoding `Term` according to the [Erlang external term format.](erl_ext_dist.md)
...
* __`compressed`__ - Compress the external term format. The compressed format is automatically recognized by `binary_to_term/1` as from Erlang/OTP R7B.

* __`{compressed, Level}`__ - Compress the external term format to a given level. The compression level is specified by `Level` which is an integer in the range 0..9, where:

  * __`0`__ - No compression is done (it is the same as giving no `compressed` option).

  * __`1`__ - Takes least time but may not compress as well as the higher levels.

  * __`6`__ - Default level when option `compressed` is provided.

  * __`9`__ - Takes most time and tries to produce a smaller result. Notice "tries" in the preceding sentence; depending on the input term, level 9 compression either does or does not produce a smaller result than level 1 compression.

  

* __`{minor_version, Version}`__(Since R11B-4)  
  The option can be used to control some encoding details. Valid values for `Version` are:

  * __`0`__ - Floats are encoded using a textual representation.
  ...
  * __`1`__ - Floats are encoded in a more space-efficient and exact way (namely in the 64-bit IEEE format, rather than converted to a textual representation). As from Erlang/OTP R11B-4, `binary_to_term/1` can decode this representation.

    Atoms that can be represented by a latin1 string are encoded using latin1 while only atoms that cannot be represented by latin1 are encoded using utf8.

  * __`2`__ - This is as of Erlang/OTP 26.0 the *default*. Atoms are unconditionally encoded using utf8. Erlang/OTP systems as of R16B can decode this representation.

  

* __`deterministic`__(Since OTP 24.1)  
...
* __`local`[](){: id=term_to_binary_local }
__(Since OTP 26.0)  
  This option will cause encoding of `Term` to an alternative local version of the external term format which when decoded by the same runtime system instance will produce a term identical to the encoded term even when the node name and/or [creation](`m:erlang#system_info_creation`) of the current runtime system instance have changed between encoding and decoding. When encoding without the `local` option, local identifiers such as [pids](`t:pid/0`), [ports](`t:port/0`) and [references](`t:reference/0`) will not be the same if node name and/or creation of the current runtime system instance changed between encoding and decoding. This since such identifiers refer to a specific node by node name and creation.

  Node name and creation of a runtime system instance change when the distribution is started or stopped. The distribution is started when the runtime system is started using the [`-name`](erl_cmd.md#name) or [`-sname`](erl_cmd.md#sname) command line arguments. Note that the actual start of the distribution happens after other code in the startup phase has begun executing. The distribution can also be started by calling `net_kernel:start/2` and stopped by calling [`net_kernel:stop/1`](`net_kernel:stop/0`) if it has not been started via the command line.

  The decoding of a term encoded with the `local` option, using for example [`binary_to_term()`](`term_to_binary/1`), will try to verify that the term actually was encoded by the same runtime system instance, and will in the vast majority of cases fail if the encoding was performed by another runtime system instance. You should however *not* trust that this verification will work in all cases. You *should* make sure to *only* decode terms encoded with the `local` option on the same Erlang runtime system instance as the one that encoded the terms.

  Since it is only the runtime system that encoded a term using the `local` option that can decode it, the local encoding is typically pieced together with something else to produce a reply to where the `local` encoding originates from. If a term encoded using the `local` option is stripped of its leading version number, it can be added as part of a larger term (for example as an element in a tuple) when encoding on the external term format using, for example, [ei](`p:erl_interface:ei.md`). In the `ei` case, you would strip it of the version number using [`ei_decode_version()`](`p:erl_interface:ei.md#ei_decode_version`) and then add the remaining local encoding to what you are encoding using for example [`ei_x_append_buf()`](`p:erl_interface:ei.md#ei_x_append_buf`).

  A good example of when you want to use the `local` option, is when you want to make a request from a process to a port [driver](erl_driver.md) and utilize the [selective receive optimization](`p:system:eff_guide_processes.md#receiving-messages`) when receiving the reply. In this scenario you want to create a reference, serialize the reference on the external term format using the `local` option, pass this to the driver in the request, and then wait for the reply message in a selective receive matching on the reference. The driver should send the reply using either [`erl_drv_output_term()`](erl_driver.md#erl_drv_output_term) or [`erl_drv_send_term()`](erl_driver.md#erl_drv_send_term) using the term type [`ERL_DRV_EXT2TERM`](erl_driver.md#erl_drv_ext2term) for the, in the request, previously received reference on the external term format. Note that you should not strip the leading version number from the local encoding when using the term type `ERL_DRV_EXT2TERM` of this functionality. If you in this example do not encode the reference using the `local` option, and the distribution is started or stopped while the request is ongoing, the process that made the request will hang indefinitely since the reference in the reply message will never match.

  This option cannot be combined with the `deterministic` option.

  For more information see the [`LOCAL_EXT`](erl_ext_dist.md#local_ext) tag in the documentation of the external term format.
...
-doc """
Returns the encoding of `Term` according to the Erlang external term format as [`ext_iovec()`](`t:ext_iovec/0`).
...
The same as [`erlang:trace_pattern(Event, MatchSpec, [])`](`trace_pattern/3`), retained for backward compatibility.
""".
...
Sets trace pattern for *message sending*. Must be combined with [`erlang:trace/3`](`trace/3`) to set the `send` trace flag for one or more processes. By default all messages sent from `send` traced processes are traced. To limit traced send events based on the message content, the sender and/or the receiver, use `erlang:trace_pattern/3`.

Argument `MatchSpec` can take the following forms:

* __`MatchSpecList`__ - A list of match specifications. The matching is done on the list `[Receiver, Msg]`. `Receiver` is the process or port identity of the receiver and `Msg` is the message term. The pid of the sending process can be accessed with the guard function `self/0`. An empty list is the same as `true`. For more information, see section [Match Specifications in Erlang](match_spec.md) in the User's Guide.

* __`true`__ - Enables tracing for all sent messages (from `send` traced processes). Any match specification is removed. *This is the default*.

* __`false`__ - Disables tracing for all sent messages. Any match specification is removed.
...
* __`badarg`__ - If an argument is invalid.

* __`system_limit`__ - If a match specification passed as argument has excessive nesting which causes scheduler stack exhaustion for the scheduler that the calling process is executing on. [Scheduler stack size](erl_cmd.md#sched_thread_stack_size) can be configured when starting the runtime system.
...
Sets trace pattern for *message receiving*. Must be combined with [`erlang:trace/3`](`trace/3`) to set the `'receive'` trace flag for one or more processes. By default all messages received by `'receive'` traced processes are traced. To limit traced receive events based on the message content, the sender and/or the receiver, use `erlang:trace_pattern/3`.

Argument `MatchSpec` can take the following forms:

* __`MatchSpecList`__ - A list of match specifications. The matching is done on the list `[Node, Sender, Msg]`. `Node` is the node name of the sender. `Sender` is the process or port identity of the sender, or the atom `undefined` if the sender is not known (which can be the case for remote senders). `Msg` is the message term. The pid of the receiving process can be accessed with the guard function `self/0`. An empty list is the same as `true`. For more information, see section [Match Specifications in Erlang](match_spec.md) in the User's Guide.

* __`true`__ - Enables tracing for all received messages (to `'receive'` traced processes). Any match specification is removed. *This is the default*.

* __`false`__ - Disables tracing for all received messages. Any match specification is removed.
...
Enables or disables *call tracing* for one or more functions. Must be combined with [`erlang:trace/3`](`trace/3`) to set the `call` trace flag for one or more processes.
...
To add or remove one or more processes to the set of traced processes, use [`erlang:trace/3`](`trace/3`).
...
* __`{Module,Function,'_'}`__ - All functions of any arity named `Function` in module `Module`.

* __`{Module,'_','_'}`__ - All functions in module `Module`.

* __`{'_','_','_'}`__ - All functions in all loaded modules.
...
* __`false`__ - Disables tracing for the matching functions. Any match specification is removed.

* __`true`__ - Enables tracing for the matching functions. Any match specification is removed.

* __`MatchSpecList`__ - A list of match specifications. An empty list is equivalent to `true`. For a description of match specifications, see section [Match Specifications in Erlang](match_spec.md) in the User's Guide.

* __`restart`__ - For the `FlagList` options `call_count`, `call_time` and `call_memory`: restarts the existing counters. The behavior is undefined for other `FlagList` options.

* __`pause`__ - For the `FlagList` options `call_count`, `call_time` and `call_memory`: pauses the existing counters. The behavior is undefined for other `FlagList` options.

Parameter `FlagList` is a list of options. The following are the valid options:

* __`global`__ - Turns on or off call tracing for global function calls (that is, calls specifying the module explicitly). Only exported functions match and only global calls generate trace messages. *This is the default*.

* __`local`__ - Turns on or off call tracing for all types of function calls. Trace messages are sent whenever any of the specified functions are called, regardless of how they are called. If flag `return_to` is set for the process, a `return_to` message is also sent when this function returns to its caller.

* __`meta | {meta, Pid} | {meta, TracerModule, TracerState}`__ - Turns on or off meta-tracing for all types of function calls. Trace messages are sent to the tracer whenever any of the specified functions are called. If no tracer is specified, `self()` is used as a default tracer process.
...
* __`call_count`__ - Starts (`MatchSpec == true`) or stops (`MatchSpec == false`) call count tracing for all types of function calls. For every function, a counter is incremented when the function is called, in any process. No process trace flags need to be activated.
...
* __`call_memory`__ - Starts (`MatchSpec == true`) or stops (`MatchSpec == false`) call memory tracing for all types of function calls.
...
Part of a match specification list cannot be changed directly. If a function has a match specification, it can be replaced with a new one. To change an existing match specification, use the BIF [`erlang:trace_info/2`](`trace_info/2`) to retrieve the existing match specification.
...
* __[`Memory Allocation`](`m:erlang#system_info_allocator`)__ - [`allocated_areas`](`m:erlang#system_info_allocated_areas`), [`allocator`](`m:erlang#system_info_allocator`), [`alloc_util_allocators`](`m:erlang#system_info_alloc_util_allocators`), [`allocator_sizes`](`m:erlang#system_info_allocator_sizes`)

* __[`CPU Topology`](`m:erlang#system_info_cpu_topology`)__ - [`cpu_topology`](`m:erlang#system_info_cpu_topology`), [`logical_processors`](`m:erlang#system_info_logical_processors`), [`cpu_quota`](`m:erlang#system_info_update_cpu_info`), [`update_cpu_info`](`m:erlang#system_info_update_cpu_info`)

* __[`Process Information`](`m:erlang#system_info_process`)__ - [`fullsweep_after`](`m:erlang#system_info_fullsweep_after`), [`garbage_collection`](`m:erlang#system_info_garbage_collection`), [`heap_sizes`](`m:erlang#system_info_heap_sizes`), [`heap_type`](`m:erlang#system_info_heap_type`), [`max_heap_size`](`m:erlang#system_info_max_heap_size`), [`message_queue_data`](`m:erlang#system_info_message_queue_data`), [`min_heap_size`](`m:erlang#system_info_min_heap_size`), [`min_bin_vheap_size`](`m:erlang#system_info_min_bin_vheap_size`), [`procs`](`m:erlang#system_info_procs`)

* __[`System Limits`](`m:erlang#system_info_limits`)__ - [`atom_count`](`m:erlang#system_info_atom_count`), [`atom_limit`](`m:erlang#system_info_atom_limit`), [`ets_count`](`m:erlang#system_info_ets_count`), [`ets_limit`](`m:erlang#system_info_ets_limit`), [`port_count`](`m:erlang#system_info_port_count`), [`port_limit`](`m:erlang#system_info_port_limit`), [`process_count`](`m:erlang#system_info_process_count`), [`process_limit`](`m:erlang#system_info_process_limit`)

* __[`System Time`](`m:erlang#system_info_time`)__ - [`end_time`](`m:erlang#system_info_end_time`), [`os_monotonic_time_source`](`m:erlang#system_info_os_monotonic_time_source`), [`os_system_time_source`](`m:erlang#system_info_os_system_time_source`), [`start_time`](`m:erlang#system_info_start_time`), [`time_correction`](`m:erlang#system_info_time_correction`), [`time_offset`](`m:erlang#system_info_time_offset`), [`time_warp_mode`](`m:erlang#system_info_time_warp_mode`), [`tolerant_timeofday`](`m:erlang#system_info_tolerant_timeofday`)

* __[`Scheduler Information`](`m:erlang#system_info_scheduler`)__ - [`dirty_cpu_schedulers`](`m:erlang#system_info_dirty_cpu_schedulers`), [`dirty_cpu_schedulers_online`](`m:erlang#system_info_dirty_cpu_schedulers_online`), [`dirty_io_schedulers`](`m:erlang#system_info_dirty_io_schedulers`), [`multi_scheduling`](`m:erlang#system_info_multi_scheduling`), [`multi_scheduling_blockers`](`m:erlang#system_info_multi_scheduling_blockers`), [`normal_multi_scheduling_blockers`](`m:erlang#system_info_normal_multi_scheduling_blockers`), [`scheduler_bind_type`](`m:erlang#system_info_scheduler_bind_type`), [`scheduler_bindings`](`m:erlang#system_info_scheduler_bindings`), [`scheduler_id`](`m:erlang#system_info_scheduler_id`), [`schedulers`](`m:erlang#system_info_schedulers`), [`smp_support`](`m:erlang#system_info_smp_support`), [`threads`](`m:erlang#system_info_threads`), [`thread_pool_size`](`m:erlang#system_info_thread_pool_size`)

* __[`Distribution Information`](`m:erlang#system_info_dist`)__ - [`async_dist`](`m:erlang#system_info_async_dist`), [`creation`](`m:erlang#system_info_creation`), [`delayed_node_table_gc`](`m:erlang#system_info_delayed_node_table_gc`), [`dist`](`m:erlang#system_info_dist`), [`dist_buf_busy_limit`](`m:erlang#system_info_dist_buf_busy_limit`), [`dist_ctrl`](`m:erlang#system_info_dist_ctrl`)

* __[`System Information`](`m:erlang#system_info_misc`)__ - [`c_compiler_used`](`m:erlang#system_info_c_compiler_used`), [`check_io`](`m:erlang#system_info_check_io`), [`compat_rel`](`m:erlang#system_info_compat_rel`), [`debug_compiled`](`m:erlang#system_info_debug_compiled`), [`driver_version`](`m:erlang#system_info_driver_version`), [`dynamic_trace`](`m:erlang#system_info_dynamic_trace`), [`dynamic_trace_probes`](`m:erlang#system_info_dynamic_trace_probes`), [`emu_flavor`](`m:erlang#system_info_emu_flavor`), [`emu_type`](`m:erlang#system_info_emu_type`), [`info`](`m:erlang#system_info_info`), [`kernel_poll`](`m:erlang#system_info_kernel_poll`), [`loaded`](`m:erlang#system_info_loaded`), [`machine`](`m:erlang#system_info_machine`), [`modified_timing_level`](`m:erlang#system_info_modified_timing_level`), [`nif_version`](`m:erlang#system_info_nif_version`), [`otp_release`](`m:erlang#system_info_otp_release`), [`outstanding_system_requests_limit`](`m:erlang#system_info_outstanding_system_requests_limit`), [`port_parallelism`](`m:erlang#system_info_port_parallelism`), [`system_architecture`](`m:erlang#system_info_system_architecture`), [`system_logger`](`m:erlang#system_info_system_logger`), [`system_version`](`m:erlang#system_info_system_version`), [`trace_control_word`](`m:erlang#system_info_trace_control_word`), [`version`](`m:erlang#system_info_version`), [`wordsize`](`m:erlang#system_info_wordsize`)

[](){: id=system_info_allocator }
[](){: id=system_info_allocator_tags }
Returns various information about the memory allocators of the current system (emulator) as specified by `Item`:

[](){: id=system_info_allocated_areas }
* __`allocated_areas`{: id=system_info_allocated_areas }__ - Returns a list of tuples with information about miscellaneous allocated memory areas.
  ...
  Notice that the sum of these values is *not* the total amount of memory allocated by the emulator. Some values are part of other values, and some memory areas are not part of the result. For information about the total amount of memory allocated by the emulator, see [`erlang:memory/0,1`](`memory/0`).

* __`allocator`{: id=system_info_allocator }__ - Returns `{Allocator, Version, Features, Settings`\}, where:
...
* __`alloc_util_allocators`{: id=system_info_alloc_util_allocators }__ - Returns a list of the names of all allocators using the ERTS internal `alloc_util` framework as atoms. For more information, see section [The alloc_util framework](erts_alloc.md#alloc_util) in `m:erts_alloc`.

* __`{allocator_sizes, Alloc}`{: id=system_info_allocator_sizes }__ - Returns various size information for the specified allocator. The information returned is a subset of the information returned by [`erlang:system_info({allocator, Alloc})`](`m:erlang#system_info_allocator_tuple`).

[](){: id=system_info_cpu_topology }
```erlang
-type cpu_topology() :: [LevelEntry :: level_entry()] | undefined.
```
  All `LevelEntry`s of a list must contain the same `LevelTag`, except on the top level where both `node` and `processor``LevelTag`s can coexist.  
```erlang
-type info_list() :: [].
```
...
[](){: id=system_info_cpu_topology_tags }
Returns various information about the CPU topology of the current system (emulator) as specified by `Item`:

* __`cpu_topology`{: id=system_info_cpu_topology }__ - Returns the `CpuTopology` currently used by the emulator. The CPU topology is used when binding schedulers to logical processors. The CPU topology used is the [user-defined CPU topology](`m:erlang#system_info_cpu_topology_defined`), if such exists, otherwise the [automatically detected CPU topology](`m:erlang#system_info_cpu_topology_detected`), if such exists. If no CPU topology exists, `undefined` is returned.
...
* __`{cpu_topology, defined}`__ - [](){: id=system_info_cpu_topology_defined }
  Returns the user-defined `CpuTopology`. For more information, see command-line flag [`+sct`](erl_cmd.md#+sct) in `p:erl_cmd.md` and argument [`cpu_topology`](`m:erlang#system_info_cpu_topology`).

* __`{cpu_topology, detected}`__ - [](){: id=system_info_cpu_topology_detected }
  ...
  For more information, see argument [`cpu_topology`](`m:erlang#system_info_cpu_topology`).

* __`{cpu_topology, used}`__ - Returns `CpuTopology` used by the emulator. For more information, see argument [`cpu_topology`](`m:erlang#system_info_cpu_topology`).

* __`logical_processors`{: id=system_info_logical_processors }__ - Returns the detected number of logical processors configured in the system. The return value is either an integer, or the atom `unknown` if the emulator cannot detect the configured logical processors.

* __`logical_processors_available`{: id=system_info_logical_processors_available }__ - Returns the detected number of logical processors available to the Erlang runtime system. The return value is either an integer, or the atom `unknown` if the emulator cannot detect the available logical processors. The number of available logical processors is less than or equal to the number of [logical processors online](`m:erlang#system_info_logical_processors_online`).

* __`logical_processors_online`{: id=system_info_logical_processors_online }__ - Returns the detected number of logical processors online on the system. The return value is either an integer, or the atom `unknown` if the emulator cannot detect logical processors online. The number of logical processors online is less than or equal to the number of [logical processors configured](`m:erlang#system_info_logical_processors`).

* __`cpu_quota`{: id=system_info_cpu_quota }__ - Returns the detected CPU quota the emulator is limited by. The return value is an integer saying how many processors' worth of runtime we get (between 1 and the number of logical processors), or the atom `unknown` if the emulator cannot detect a quota.

* __`update_cpu_info`{: id=system_info_update_cpu_info }__ - The runtime system rereads the CPU information available and updates its internally stored information about the [detected CPU topology](`m:erlang#system_info_cpu_topology_detected`) and the number of logical processors [configured](`m:erlang#system_info_logical_processors`), [online](`m:erlang#system_info_logical_processors_online`), [available](`m:erlang#system_info_logical_processors_available`), and [cpu quota](`m:erlang#system_info_cpu_quota`).

  If the CPU information has changed since the last time it was read, the atom `changed` is returned, otherwise the atom `unchanged`. If the CPU information has changed, you probably want to [adjust the number of schedulers online](`m:erlang#system_flag_schedulers_online`). You typically want to have as many schedulers online as [logical processors available](`m:erlang#system_info_logical_processors_available`).

[](){: id=system_info_process }
```erlang
...
[](){: id=system_info_process_tags }
Returns information about the default process heap settings:

* __`fullsweep_after`{: id=system_info_fullsweep_after }__ - Returns `{fullsweep_after, integer() >= 0}`, which is the `fullsweep_after` garbage collection setting used by default. For more information, see `garbage_collection` described below.

* __`garbage_collection`{: id=system_info_garbage_collection }__ - Returns a list describing the default garbage collection settings. A process spawned on the local node by a `spawn` or `spawn_link` uses these garbage collection settings. The default settings can be changed by using [`erlang:system_flag/2`](`system_flag/2`). [`spawn_opt/2,3,4`](`spawn_opt/4`) can spawn a process that does not use the default settings.

* __`heap_sizes`{: id=system_info_heap_sizes }__ - Returns a list of integers representing valid heap sizes in words. All Erlang heaps are sized from sizes in this list.

* __`heap_type`{: id=system_info_heap_type }__ - Returns the heap type used by the current emulator. One heap type exists:

  * __`private`__ - Each process has a heap reserved for its use and no references between heaps of different processes are allowed. Messages passed between processes are copied between heaps.

  

* __`max_heap_size`{: id=system_info_max_heap_size }__ - Returns `{max_heap_size, MaxHeapSize}`, where `MaxHeapSize` is the current system-wide maximum heap size settings for spawned processes. This setting can be set using the command-line flags [`+hmax`](erl_cmd.md#+hmax), [`+hmaxk`](erl_cmd.md#+hmaxk), [`+hmaxel`](erl_cmd.md#+hmaxel) and [`+hmaxibl`](erl_cmd.md#+hmaxib) in `p:erl_cmd.md`. It can also be changed at runtime using [`erlang:system_flag(max_heap_size, MaxHeapSize)`](`m:erlang#system_flag_max_heap_size`). For more details about the `max_heap_size` process flag, see [`process_flag(max_heap_size, MaxHeapSize)`](`m:erlang#process_flag_max_heap_size`).

* __`message_queue_data`{: id=system_info_message_queue_data }__ - Returns the default value of the `message_queue_data` process flag, which can be either `off_heap` or `on_heap`. The default value is set by the command-line argument [`+hmqd`](erl_cmd.md#+hmqd) in `p:erl_cmd.md`. For more information, see the documentation of [`process_flag(message_queue_data, MQD)`](`m:erlang#process_flag_message_queue_data`).

* __`min_heap_size`{: id=system_info_min_heap_size }__ - Returns `{min_heap_size, MinHeapSize}`, where `MinHeapSize` is the current system-wide minimum heap size for spawned processes.

* __`min_bin_vheap_size`{: id=system_info_min_bin_vheap_size }__ - Returns `{min_bin_vheap_size, MinBinVHeapSize}`, where `MinBinVHeapSize` is the current system-wide minimum binary virtual heap size for spawned processes.

* __`procs`{: id=system_info_procs }__ - Returns a binary containing a string of process and port information formatted as in Erlang crash dumps. For more information, see section [How to interpret the Erlang crash dumps](crash_dump.md) in the User's Guide.

[](){: id=system_info_limits }
[](){: id=system_info_limits }
Returns information about the current system (emulator) limits as specified by `Item`:

* __`atom_count`{: id=system_info_atom_count }__ - Returns the number of atoms currently existing at the local node. The value is given as an integer.

* __`atom_limit`{: id=system_info_atom_limit }__ - Returns the maximum number of atoms allowed. This limit can be increased at startup by passing command-line flag [`+t`](erl_cmd.md#+t) to `p:erl_cmd.md`.

* __`ets_count`{: id=system_info_ets_count }__ - Returns the number of ETS tables currently existing at the local node.

* __`ets_limit`{: id=system_info_ets_limit }__ - Returns the limit for number of ETS tables. This limit is [partially obsolete](`m:ets#max_ets_tables`) and number of tables are only limited by available memory.

* __`port_count`{: id=system_info_port_count }__ - Returns the number of ports currently existing at the local node. The value is given as an integer. This is the same value as returned by `length(erlang:ports())`, but more efficient.

* __`port_limit`{: id=system_info_port_limit }__ - Returns the maximum number of simultaneously existing ports at the local node as an integer. This limit can be configured at startup by using command-line flag [`+Q`](erl_cmd.md#+q) in `p:erl_cmd.md`.

* __`process_count`{: id=system_info_process_count }__ - Returns the number of processes currently existing at the local node. The value is given as an integer. This is the same value as returned by `length(processes())`, but more efficient.

* __`process_limit`{: id=system_info_process_limit }__ - Returns the maximum number of simultaneously existing processes at the local node. The value is given as an integer. This limit can be configured at startup by using command-line flag [`+P`](erl_cmd.md#+p) in `p:erl_cmd.md`.

[](){: id=system_info_time }
[](){: id=system_info_time_tags }
Returns information about the current system (emulator) time as specified by `Item`:

* __`end_time`{: id=system_info_end_time }__ - The last [Erlang monotonic time](`monotonic_time/0`) in `native` [time unit](`m:erlang#type_time_unit`) that can be represented internally in the current Erlang runtime system instance. The time between the [start time](`m:erlang#system_info_start_time`) and the end time is at least a quarter of a millennium.

* __`os_monotonic_time_source`{: id=system_info_os_monotonic_time_source }__ - Returns a list containing information about the source of [OS monotonic time](time_correction.md#os_monotonic_time) that is used by the runtime system.
  ...
  * __`{function, Function}`__ - `Function` is the name of the function used. This tuple always exists if OS monotonic time is available to the runtime system.

  * __`{clock_id, ClockId}`__ - This tuple only exists if `Function` can be used with different clocks. `ClockId` corresponds to the clock identifier used when calling `Function`.

  * __`{resolution, OsMonotonicTimeResolution}`__ - Highest possible [resolution](time_correction.md#time_resolution) of current OS monotonic time source as parts per second. If no resolution information can be retrieved from the OS, `OsMonotonicTimeResolution` is set to the resolution of the time unit of `Function`s return value. That is, the actual resolution can be lower than `OsMonotonicTimeResolution`. Notice that the resolution does not say anything about the [accuracy](time_correction.md#time_accuracy) or whether the [precision](time_correction.md#time_precision) aligns with the resolution. You do, however, know that the precision is not better than `OsMonotonicTimeResolution`.

  * __`{extended, Extended}`__ - `Extended` equals `yes` if the range of time values has been extended; otherwise `Extended` equals `no`. The range must be extended if `Function` returns values that wrap fast. This typically is the case when the return value is a 32-bit value.

  * __`{parallel, Parallel}`__ - `Parallel` equals `yes` if `Function` is called in parallel from multiple threads. If it is not called in parallel, because calls must be serialized, `Parallel` equals `no`.

  * __`{time, OsMonotonicTime}`__ - `OsMonotonicTime` equals current OS monotonic time in `native` [time unit](`m:erlang#type_time_unit`).

  

* __`os_system_time_source`{: id=system_info_os_system_time_source }__ - Returns a list containing information about the source of [OS system time](time_correction.md#os_system_time) that is used by the runtime system.
  ...
  * __`{function, Function}`__ - `Function` is the name of the function used.

  * __`{clock_id, ClockId}`__ - Exists only if `Function` can be used with different clocks. `ClockId` corresponds to the clock identifier used when calling `Function`.

  * __`{resolution, OsSystemTimeResolution}`__ - Highest possible [resolution](time_correction.md#time_resolution) of current OS system time source as parts per second. If no resolution information can be retrieved from the OS, `OsSystemTimeResolution` is set to the resolution of the time unit of `Function`s return value. That is, the actual resolution can be lower than `OsSystemTimeResolution`. Notice that the resolution does not say anything about the [accuracy](time_correction.md#time_accuracy) or whether the [precision](time_correction.md#time_precision) do align with the resolution. You do, however, know that the precision is not better than `OsSystemTimeResolution`.

  * __`{parallel, Parallel}`__ - `Parallel` equals `yes` if `Function` is called in parallel from multiple threads. If it is not called in parallel, because calls needs to be serialized, `Parallel` equals `no`.

  * __`{time, OsSystemTime}`__ - `OsSystemTime` equals current OS system time in `native` [time unit](`m:erlang#type_time_unit`).

  

* __`start_time`{: id=system_info_start_time }__ - The [Erlang monotonic time](`monotonic_time/0`) in `native` [time unit](`m:erlang#type_time_unit`) at the time when current Erlang runtime system instance started.

  See also [`erlang:system_info(end_time)`](`m:erlang#system_info_end_time`).

* __`time_correction`{: id=system_info_time_correction }__ - Returns a boolean value indicating whether [time correction](time_correction.md#time_correction) is enabled or not.

* __`time_offset`{: id=system_info_time_offset }__ - Returns the state of the time offset:

  * __`preliminary`__ - The time offset is preliminary, and will be changed and finalized later. The preliminary time offset is used during the preliminary phase of the [single time warp mode](time_correction.md#single_time_warp_mode).

  * __`final`__ - The time offset is final. This either because [no time warp mode](time_correction.md#no_time_warp_mode) is used, or because the time offset have been finalized when [single time warp mode](time_correction.md#single_time_warp_mode) is used.

  * __`volatile`__ - The time offset is volatile. That is, it can change at any time. This is because [multi-time warp mode](time_correction.md#multi_time_warp_mode) is used.

  

* __`time_warp_mode`{: id=system_info_time_warp_mode }__ - Returns a value identifying the [time warp mode](time_correction.md#time_warp_modes) that is used:

  * __`no_time_warp`__ - The [no time warp mode](time_correction.md#no_time_warp_mode) is used.

  * __`single_time_warp`__ - The [single time warp mode](time_correction.md#single_time_warp_mode) is used.

  * __`multi_time_warp`__ - The [multi-time warp mode](time_correction.md#multi_time_warp_mode) is used.

  

* __`tolerant_timeofday`{: id=system_info_tolerant_timeofday }__ - Returns whether a pre ERTS 7.0 backwards compatible compensation for sudden changes of system time is `enabled` or `disabled`. Such compensation is `enabled` when the [time offset](`m:erlang#system_info_time_offset`) is `final`, and [time correction](`m:erlang#system_info_time_correction`) is enabled.

[](){: id=system_info_scheduler }
```erlang
-type scheduler_bind_type() ::
          no_node_processor_spread | no_node_thread_spread | no_spread |
          processor_spread | spread | thread_spread |
...
[](){: id=system_info_scheduler_tags }
Returns information about schedulers, scheduling and threads in the current system as specified by `Item`:

* __`dirty_cpu_schedulers`{: id=system_info_dirty_cpu_schedulers }__ - Returns the number of dirty CPU scheduler threads used by the emulator. Dirty CPU schedulers execute CPU-bound native functions, such as NIFs, linked-in driver code, and BIFs that cannot be managed cleanly by the normal emulator schedulers.

  The number of dirty CPU scheduler threads is determined at emulator boot time and cannot be changed after that. However, the number of dirty CPU scheduler threads online can be changed at any time. The number of dirty CPU schedulers can be set at startup by passing command-line flag [`+SDcpu`](erl_cmd.md#+sdcpu) or [`+SDPcpu`](erl_cmd.md#+sdpcpu) in `p:erl_cmd.md`.

  See also [`erlang:system_flag(dirty_cpu_schedulers_online, DirtyCPUSchedulersOnline)`](`m:erlang#system_flag_dirty_cpu_schedulers_online`), [`erlang:system_info(dirty_cpu_schedulers_online)`](`m:erlang#system_info_dirty_cpu_schedulers_online`), [`erlang:system_info(dirty_io_schedulers)`](`m:erlang#system_info_dirty_io_schedulers`), [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`), [`erlang:system_info(schedulers_online)`](`m:erlang#system_info_schedulers_online`), and [`erlang:system_flag(schedulers_online, SchedulersOnline)`](`m:erlang#system_flag_schedulers_online`).

* __`dirty_cpu_schedulers_online`{: id=system_info_dirty_cpu_schedulers_online }__ - Returns the number of dirty CPU schedulers online. The return value satisfies `1 <= DirtyCPUSchedulersOnline <= N`, where `N` is the smallest of the return values of `erlang:system_info(dirty_cpu_schedulers)` and `erlang:system_info(schedulers_online)`.

  The number of dirty CPU schedulers online can be set at startup by passing command-line flag [`+SDcpu`](erl_cmd.md#+sdcpu) in `p:erl_cmd.md`.

  For more information, see [`erlang:system_info(dirty_cpu_schedulers)`](`m:erlang#system_info_dirty_cpu_schedulers`), [`erlang:system_info(dirty_io_schedulers)`](`m:erlang#system_info_dirty_io_schedulers`), [`erlang:system_info(schedulers_online)`](`m:erlang#system_info_schedulers_online`), and [`erlang:system_flag(dirty_cpu_schedulers_online, DirtyCPUSchedulersOnline)`](`m:erlang#system_flag_dirty_cpu_schedulers_online`).

* __`dirty_io_schedulers`{: id=system_info_dirty_io_schedulers }__ - Returns the number of dirty I/O schedulers as an integer. Dirty I/O schedulers execute I/O-bound native functions, such as NIFs and linked-in driver code, which cannot be managed cleanly by the normal emulator schedulers.

  This value can be set at startup by passing command-line argument [`+SDio`](erl_cmd.md#+sdio) in `p:erl_cmd.md`.

  For more information, see [`erlang:system_info(dirty_cpu_schedulers)`](`m:erlang#system_info_dirty_cpu_schedulers`), [`erlang:system_info(dirty_cpu_schedulers_online)`](`m:erlang#system_info_dirty_cpu_schedulers_online`), and [`erlang:system_flag(dirty_cpu_schedulers_online, DirtyCPUSchedulersOnline)`](`m:erlang#system_flag_dirty_cpu_schedulers_online`).

* __`multi_scheduling`{: id=system_info_multi_scheduling }__ - Returns one of the following:

  * __`disabled`__ - The emulator has been started with only one scheduler thread.

  * __`blocked`__ - The emulator has more than one scheduler thread, but all scheduler threads except one are blocked. That is, only one scheduler thread schedules Erlang processes and executes Erlang code.

  * __`blocked_normal`__ - The emulator has more than one scheduler thread, but all normal scheduler threads except one are blocked. Notice that dirty schedulers are not blocked, and can schedule Erlang processes and execute native code.

  * __`enabled`__ - The emulator has more than one scheduler thread, and no scheduler threads are blocked. That is, all available scheduler threads schedule Erlang processes and execute Erlang code.

  See also [`erlang:system_flag(multi_scheduling, BlockState)`](`m:erlang#system_flag_multi_scheduling`), [`erlang:system_info(multi_scheduling_blockers)`](`m:erlang#system_info_multi_scheduling_blockers`), [`erlang:system_info(normal_multi_scheduling_blockers)`](`m:erlang#system_info_normal_multi_scheduling_blockers`), and [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`).

* __`multi_scheduling_blockers`{: id=system_info_multi_scheduling_blockers }__ - Returns a list of `Pid`s when multi-scheduling is blocked, otherwise the empty list is returned. The `Pid`s in the list represent all the processes currently blocking multi-scheduling. A `Pid` occurs only once in the list, even if the corresponding process has blocked multiple times.

  See also [`erlang:system_flag(multi_scheduling, BlockState)`](`m:erlang#system_flag_multi_scheduling`), [`erlang:system_info(multi_scheduling)`](`m:erlang#system_info_multi_scheduling`), [`erlang:system_info(normal_multi_scheduling_blockers)`](`m:erlang#system_info_normal_multi_scheduling_blockers`), and [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`).

* __`normal_multi_scheduling_blockers`{: id=system_info_normal_multi_scheduling_blockers }__ - Returns a list of `Pid`s when normal multi-scheduling is blocked (that is, all normal schedulers but one is blocked), otherwise the empty list is returned. The `Pid`s in the list represent all the processes currently blocking normal multi-scheduling. A `Pid` occurs only once in the list, even if the corresponding process has blocked multiple times.

  See also [`erlang:system_flag(multi_scheduling, BlockState)`](`m:erlang#system_flag_multi_scheduling`), [`erlang:system_info(multi_scheduling)`](`m:erlang#system_info_multi_scheduling`), [`erlang:system_info(multi_scheduling_blockers)`](`m:erlang#system_info_multi_scheduling_blockers`), and [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`).

* __`scheduler_bind_type`{: id=system_info_scheduler_bind_type }__ - Returns information about how the user has requested schedulers to be bound or not bound.

  Notice that although a user has requested schedulers to be bound, they can silently have failed to bind. To inspect the scheduler bindings, call [`erlang:system_info(scheduler_bindings)`](`m:erlang#system_info_scheduler_bindings`).

  For more information, see command-line argument [`+sbt`](erl_cmd.md#+sbt) in `p:erl_cmd.md` and [`erlang:system_info(scheduler_bindings)`](`m:erlang#system_info_scheduler_bindings`).

* __`scheduler_bindings`{: id=system_info_scheduler_bindings }__ - Returns information about the currently used scheduler bindings.

  A tuple of a size equal to [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`) is returned. The tuple elements are integers or the atom `unbound`. Logical processor identifiers are represented as integers. The `N`th element of the tuple equals the current binding for the scheduler with the scheduler identifier equal to `N`. For example, if the schedulers are bound, `element(erlang:system_info(scheduler_id), erlang:system_info(scheduler_bindings))` returns the identifier of the logical processor that the calling process is executing on.

  Notice that only schedulers online can be bound to logical processors.

  For more information, see command-line argument [`+sbt`](erl_cmd.md#+sbt) in `p:erl_cmd.md` and [`erlang:system_info(schedulers_online)`](`m:erlang#system_info_schedulers_online`).

* __`scheduler_id`{: id=system_info_scheduler_id }__ - Returns the scheduler ID (`SchedulerId`) of the scheduler thread that the calling process is executing on. `SchedulerId` is a positive integer, where `1 <= SchedulerId <= erlang:system_info(schedulers)`.

  See also [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`).

* __`schedulers`{: id=system_info_schedulers }__ - Returns the number of scheduler threads used by the emulator. Scheduler threads online schedules Erlang processes and Erlang ports, and execute Erlang code and Erlang linked-in driver code.
  ...
  See also [`erlang:system_flag(schedulers_online, SchedulersOnline)`](`m:erlang#system_flag_schedulers_online`), [`erlang:system_info(schedulers_online)`](`m:erlang#system_info_schedulers_online`), [`erlang:system_info(scheduler_id)`](`m:erlang#system_info_scheduler_id`), [`erlang:system_flag(multi_scheduling, BlockState)`](`m:erlang#system_flag_multi_scheduling`), [`erlang:system_info(multi_scheduling)`](`m:erlang#system_info_multi_scheduling`), [`erlang:system_info(normal_multi_scheduling_blockers)`](`m:erlang#system_info_normal_multi_scheduling_blockers`) and [`erlang:system_info(multi_scheduling_blockers)`](`m:erlang#system_info_multi_scheduling_blockers`).

* __`schedulers_online`{: id=system_info_schedulers_online }__ - Returns the number of schedulers online. The scheduler identifiers of schedulers online satisfy the relationship `1 <= SchedulerId <= erlang:system_info(schedulers_online)`.

  For more information, see [`erlang:system_info(schedulers)`](`m:erlang#system_info_schedulers`) and [`erlang:system_flag(schedulers_online, SchedulersOnline)`](`m:erlang#system_flag_schedulers_online`).

* __`smp_support`{: id=system_info_smp_support }__ - Returns `true`.

* __`threads`{: id=system_info_threads }__ - Returns `true`.

* __`thread_pool_size`{: id=system_info_thread_pool_size }__ - [](){: id=system_info_thread_pool_size }
  Returns the number of async threads in the async thread pool used for asynchronous driver calls ([ `erl_driver:driver_async()`](erl_driver.md#driver_async)). The value is given as an integer.

[](){: id=system_info_async_dist }
[](){: id=system_info_dist }
[](){: id=system_info_dist_tags }
Returns information about Erlang Distribution in the current system as specified by `Item`:

* __`async_dist`{: id=system_info_async_dist }__ - Since: OTP 25.3

  Returns the value of the command line argument [\+pad <boolean>](erl_cmd.md#+pad) which the runtime system use. This value determines the default [`async_dist`](`m:erlang#process_flag_async_dist`) value for newly spawned processes.

* __`creation`{: id=system_info_creation }__ - Returns the "creation" value of the local node as an integer. The creation is changed when a node is restarted. The creation of a node is stored in process identifiers, port identifiers, and references. This makes it possible to distinguish between identifiers from different incarnations of a node. Creation values are currently 32-bit positive integers, but this may change in future releases. If the node is not alive, `0` is returned.

* __`delayed_node_table_gc`{: id=system_info_delayed_node_table_gc }__ - Returns the amount of time in seconds garbage collection of an entry in a node table is delayed. This limit can be set on startup by passing command-line flag [`+zdntgc`](erl_cmd.md#+zdntgc) to `p:erl_cmd.md`. For more information, see the documentation of the command-line flag.

* __`dist`{: id=system_info_dist }__ - Returns a binary containing a string of distribution information formatted as in Erlang crash dumps. For more information, see section [How to interpret the Erlang crash dumps](crash_dump.md) in the User's Guide.

* __`dist_buf_busy_limit`{: id=system_info_dist_buf_busy_limit }__ - Returns the value of the distribution buffer busy limit in bytes. This limit can be set at startup by passing command-line flag [`+zdbbl`](erl_cmd.md#+zdbbl) to `p:erl_cmd.md`.

* __`dist_ctrl`{: id=system_info_dist_ctrl }__ - Returns a list of tuples `{Node, ControllingEntity}`, one entry for each connected remote node. `Node` is the node name and `ControllingEntity` is the port or process identifier responsible for the communication to that node. More specifically, `ControllingEntity` for nodes connected through TCP/IP (the normal case) is the socket used in communication with the specific node.

[](){: id=system_info_misc }
[](){: id=system_info_misc_tags }
Returns various information about the current system (emulator) as specified by `Item`:

* __`build_type`{: id=system_info_build_type }__ - Deprecated, use [emu_type](`m:erlang#system_info_emu_type`) instead

* __`c_compiler_used`{: id=system_info_c_compiler_used }__ - Returns a two-tuple describing the C compiler used when compiling the runtime system. The first element is an atom describing the name of the compiler, or `undefined` if unknown. The second element is a term describing the version of the compiler, or `undefined` if unknown.

* __`check_io`{: id=system_info_check_io }__ - Returns a list containing miscellaneous information about the emulators internal I/O checking. Notice that the content of the returned list can vary between platforms and over time. It is only guaranteed that a list is returned.

* __`compat_rel`{: id=system_info_compat_rel }__ - Returns the compatibility mode of the local node as an integer. The integer returned represents the Erlang/OTP release that the current emulator has been set to be backward compatible with. The compatibility mode can be configured at startup by using command-line flag [`+R`](erl_cmd.md#compat_rel) in `p:erl_cmd.md`.

* __`debug_compiled`{: id=system_info_debug_compiled }__ - Returns `true` if the emulator has been debug-compiled, otherwise `false`.

* __`driver_version`{: id=system_info_driver_version }__ - Returns a string containing the Erlang driver version used by the runtime system. It has the form ["<major ver>.<minor ver>"](erl_driver.md#version_management).

* __`dynamic_trace`{: id=system_info_dynamic_trace }__ - Returns an atom describing the dynamic trace framework compiled into the virtual machine. It can be `dtrace`, `systemtap`, or `none`. For a commercial or standard build, it is always `none`. The other return values indicate a custom configuration (for example, `./configure --with-dynamic-trace=dtrace`). For more information about dynamic tracing, see `m:dyntrace` manual page and the `README.dtrace`/`README.systemtap` files in the Erlang source code top directory.

* __`dynamic_trace_probes`{: id=system_info_dynamic_trace_probes }__ - Returns a `t:boolean()` indicating if dynamic trace probes (`dtrace` or `systemtap`) are built into the emulator. This can only be `true` if the virtual machine was built for dynamic tracing (that is, `system_info(dynamic_trace)` returns `dtrace` or `systemtap`).

* __`emu_flavor`{: id=system_info_emu_flavor }__ - Returns an atom describing the flavor of the runtime system. This will be either `emu` or `jit`. Possible return values can be added or removed at any time without prior notice.

* __`emu_type`{: id=system_info_emu_type }__ - Returns an atom describing the build type of the runtime system. This is normally the atom `opt` for optimized. Other possible return values are `debug`, `gcov`, `valgrind`, `gprof`, and `lcnt`. Possible return values can be added or removed at any time without prior notice.

* __`info`{: id=system_info_info }__ - Returns a binary containing a string of miscellaneous system information formatted as in Erlang crash dumps. For more information, see section [How to interpret the Erlang crash dumps](crash_dump.md) in the User's Guide.

* __`kernel_poll`{: id=system_info_kernel_poll }__ - Returns `true` if the emulator uses some kind of kernel-poll implementation, otherwise `false`.

* __`loaded`{: id=system_info_loaded }__ - Returns a binary containing a string of loaded module information formatted as in Erlang crash dumps. For more information, see section [How to interpret the Erlang crash dumps](crash_dump.md) in the User's Guide.

* __`machine`{: id=system_info_machine }__ - Returns a string containing the Erlang machine name.

* __`modified_timing_level`{: id=system_info_modified_timing_level }__ - Returns the modified timing-level (an integer) if modified timing is enabled, otherwise `undefined`. For more information about modified timing, see command-line flag [`+T`](erl_cmd.md#+t) in `p:erl_cmd.md`

* __`nif_version`{: id=system_info_nif_version }__ - Returns a string containing the version of the Erlang NIF interface used by the runtime system. It is on the form "<major ver>.<minor ver>".

* __`otp_release`{: id=system_info_otp_release }__ - [](){: id=system_info_otp_release }
  ...
  As from Erlang/OTP 17, the OTP release number corresponds to the major OTP version number. No `erlang:system_info()` argument gives the exact OTP version. This is because the exact OTP version in the general case is difficult to determine. For more information, see the description of versions in [System principles](`p:system:versions.md`) in System Documentation.

* __`outstanding_system_requests_limit`{: id=system_info_outstanding_system_requests_limit }__ - Returns the limit on the amount of outstanding requests made by a system process orchestrating system wide changes. See [`erlang:system_flag(outstanding_system_requests_limit, Limit)`](`m:erlang#system_flag_outstanding_system_requests_limit`) for more information.

* __`port_parallelism`{: id=system_info_port_parallelism }__ - Returns the default port parallelism scheduling hint used. For more information, see command-line argument [`+spp`](erl_cmd.md#+spp) in `p:erl_cmd.md`.

* __`system_architecture`{: id=system_info_system_architecture }__ - Returns a string containing the processor and OS architecture the emulator is built for.

* __`system_logger`{: id=system_info_system_logger }__ - Returns the current `system_logger` as set by [`erlang:system_flag(system_logger, _)`](`system_flag/2`).

* __`system_version`{: id=system_info_system_version }__ - Returns a string containing version number and some important properties, such as the number of schedulers.

* __`trace_control_word`{: id=system_info_trace_control_word }__ - Returns the value of the node trace control word. For more information, see function `get_tcw` in section [Match Specifications in Erlang](match_spec.md#get_tcw) in the User's Guide.

* __`version`{: id=system_info_version }__ - Returns a string containing the version number of the emulator.

* __`wordsize`{: id=system_info_wordsize }__ - Same as `{wordsize, internal}`.

* __`{wordsize, internal}`__ - Returns the size of Erlang term words in bytes as an integer, that is, 4 is returned on a 32-bit architecture, and 8 is returned on a 64-bit architecture.

* __`{wordsize, external}`__ - Returns the true word size of the emulator, that is, the size of a pointer. The value is given in bytes as an integer. On a pure 32-bit architecture, 4 is returned. On a 64-bit architecture, 8 is returned.
""".
-doc(#{since =>
           <<"OTP 17.0, OTP 17.1, OTP 17.4, OTP 18.0, OTP 19.0, OTP 20.0, OTP 21.1, OTP 21.3, OTP 24.0, OTP 24.2, OTP 25.3, OTP R13B04, OTP R14B, OTP R14B01, OTP R15B01, OTP R16B, OTP R16B03">>}).
...
-doc "Returns the process identifier of a new process started by the application of `Fun` to the empty list `[]`. Otherwise works like `spawn/3`.".
...
-doc "Returns the process identifier of a new process started by the application of `Fun` to the empty list `[]` on `Node`. If `Node` does not exist, a useless pid is returned. Otherwise works like `spawn/3`.".
...
-doc "Returns the process identifier of a new process started by the application of `Fun` to the empty list `[]`. A link is created between the calling process and the new process, atomically. Otherwise works like `spawn/3`.".
...
-doc "Returns the process identifier (pid) of a new process started by the application of `Fun` to the empty list `[]` on `Node`. A link is created between the calling process and the new process, atomically. If `Node` does not exist, a useless pid is returned and an exit signal with reason `noconnection` is sent to the calling process. Otherwise works like `spawn/3`.".
...
-doc "Returns the process identifier of a new process, started by the application of `Fun` to the empty list `[]`, and a reference for a monitor created to the new process. Otherwise works like `spawn/3`.".
...
-doc "A new process is started by the application of `Module:Function` to `Args`. The process is monitored at the same time. Returns the process identifier and a reference for the monitor. Otherwise works like `spawn/3`.".
...
-doc "Process max heap size configuration. For more info see [`process_flag(max_heap_size, MaxHeapSize)`](`m:erlang#process_flag_max_heap_size`)".
-type max_heap_size() ::
        Size :: non_neg_integer()
...
-doc "Returns the process identifier (pid) of a new process started by the application of `Module:Function` to `Args` on `Node`. If `Node` does not exist, a useless pid is returned. Otherwise works like `spawn/3`.".
...
-doc "Returns the process identifier (pid) of a new process started by the application of `Module:Function` to `Args` on `Node`. A link is created between the calling process and the new process, atomically. If `Node` does not exist, a useless pid is returned and an exit signal with reason `noconnection` is sent to the calling process. Otherwise works like `spawn/3`.".
...
* __`link`__ - Sets a link to the parent process (like `spawn_link/3` does).

* __`monitor`__ - Monitors the new process (like [`monitor(process, Pid)`](`monitor/2`) does). A `{Pid, MonitorRef}` tuple will be returned instead of just a `Pid`.

* __`{monitor, MonitorOpts}`__ - Monitors the new process with options (like [`monitor(process, Pid, MonitorOpts)`](`monitor/3`) does). A `{Pid, MonitorRef}` tuple will be returned instead of just a `Pid`.

* __`{priority, Level}`__ - Sets the priority of the new process. Equivalent to executing [`process_flag(priority, Level)`](`m:erlang#process_flag_priority`) in the start function of the new process, except that the priority is set before the process is selected for execution for the first time. For more information on priorities, see [`process_flag(priority, Level)`](`m:erlang#process_flag_priority`).

* __`{fullsweep_after, Number}`__ - Useful only for performance tuning. Do not use this option unless you know that there is problem with execution times or memory consumption, and ensure that the option improves matters.
  ...
  * In embedded systems with a limited amount of RAM and no virtual memory, you might want to preserve memory by setting `Number` to zero. (The value can be set globally, see [`erlang:system_flag/2`](`system_flag/2`).)

* __`{min_heap_size, Size}`__ - Useful only for performance tuning. Do not use this option unless you know that there is problem with execution times or memory consumption, and ensure that the option improves matters.
...
* __`{min_bin_vheap_size, VSize}`__ - Useful only for performance tuning. Do not use this option unless you know that there is problem with execution times or memory consumption, and ensure that the option improves matters.
...
* __`{max_heap_size, Size}`__ - Sets the `max_heap_size` process flag. The default `max_heap_size` is determined by command-line argument [`+hmax`](erl_cmd.md#+hmax) in `p:erl_cmd.md`. For more information, see the documentation of [`process_flag(max_heap_size, Size)`](`m:erlang#process_flag_max_heap_size`).

* __`{message_queue_data, MQD}`__ - Sets the value of the `message_queue_data` process flag. `MQD` can be either `off_heap` or `on_heap`. The default value of the `message_queue_data` process flag is determined by the command-line argument [`+hmqd`](erl_cmd.md#+hmqd) in `p:erl_cmd.md`. For more information, see the documentation of [`process_flag(message_queue_data, MQD)`](`m:erlang#process_flag_message_queue_data`).

* __`{async_dist, Enabled}`{: id=spawn_opt_async_dist }__ - Since: OTP 25.3

  Set the [`async_dist`](`m:erlang#process_flag_async_dist`) process flag of the spawned process. This option will override the default value set by the command line argument [`+pad <boolean>`](erl_cmd.md#+pad).
""".
...
-doc "The same as the call [`spawn_request(node(),Fun,[])`](`spawn_request/3`). That is, a spawn request on the local node with no options.".
-doc(#{since => <<"OTP 23.0">>}).
...
-doc """
The same as the call [`spawn_request(node(),Fun,Options)`](`spawn_request/3`). That is, a spawn request on the local node.

The same as the call [`spawn_request(Node,Fun,[])`](`spawn_request/3`). That is, a spawn request with no options.
""".
-doc(#{since => <<"OTP 23.0">>}).
...
-doc """
The same as [`spawn_request(Node,erlang,apply,[Fun,[]],Options)`](`spawn_request/5`). That is, a spawn request using the fun `Fun` of arity zero as entry point.
...
The same as the call [`spawn_request(node(),Module,Function,Args,[])`](`spawn_request/5`). That is, a spawn request on the local node with no options.
""".
-doc(#{since => <<"OTP 23.0">>}).
...
-doc """
The same as the call [`spawn_request(Node,Module,Function,Args,[])`](`spawn_request/5`). That is, a spawn request with no options.

The same as the call [`spawn_request(node(),Module,Function,Args,Options)`](`spawn_request/5`). That is, a spawn request on the local node.
""".
-doc(#{since => <<"OTP 23.0">>}).
...
[](){: id=spawn_request_success_message }
...
[](){: id=spawn_request_error_message }
...
* __`badopt`__ - An invalid `Option` was passed as argument. Note that different runtime systems may support different options.

* __`notsup`__ - The node identified by `Node` does not support spawn operations issued by `spawn_request()`.

* __`noconnection`__ - Failure to set up a connection to the node identified by `Node` or the connection to that node was lost during the spawn operation. In the case the connection was lost, a process may or may not have been created.

* __`system_limit`__ - Could not create a new process due to that some system limit was reached. Typically the process table was full.

Valid `Option`s:

* __`monitor`__ - In the absence of spawn operation failures, atomically sets up a monitor to the newly created process. That is, as if the calling process had called [`monitor(process, Pid)`](`monitor/2`) where `Pid` is the process identifier of the newly created process. The `ReqId` returned by `spawn_request()` is also used as monitor reference as if it was returned from `monitor(process, Pid)`.

  The monitor will not be activated for the calling process until the spawn operation has succeeded. The monitor can not be [demonitored](`demonitor/1`) before the operation has succeeded. A `'DOWN'` message for the corresponding monitor is guaranteed not to be delivered before a [*success message*](`m:erlang#spawn_request_success_message`) that corresponds to the spawn operation. If the spawn operation fails, no `'DOWN'` message will be delivered.
...
* __`{monitor, MonitorOpts}`__ - In the absence of spawn operation failures, atomically sets up a monitor to the newly created process. That is, as if the calling process had called [`monitor(process, Pid, MonitorOpts)`](`monitor/2`) where `Pid` is the process identifier of the newly created process. See the `monitor` option above for more information.
...
* __`link`__ - In absence of spawn operation failures, atomically sets up a link between the calling process and the newly created process. That is, as if the calling process had called [`link(Pid)`](`link/1`) where `Pid` is the process identifier of the newly created process.

  The link will not be activated for the calling process until the spawn operation has succeeded. The link can not be removed before the operation has succeeded. An exit signal due to the link is guaranteed not to be delivered before a [*success message*](`m:erlang#spawn_request_success_message`) that corresponds to the spawn operation. If the spawn operation fails, no exit signal due to the link will be delivered to the caller of `spawn_request()`.
...
* __`{reply, Reply}`__ - Valid `Reply` values:

  * __`yes`__ - A spawn reply message will be sent to the caller regardless of whether the operation succeeds or not. If the call to `spawn_request()` returns without raising an exception and the `reply` option is set to `yes`, the caller is guaranteed to be delivered either a [*success message*](`m:erlang#spawn_request_success_message`) or an [*error message*](`m:erlang#spawn_request_error_message`). The `reply` option is by default set to `yes`.

  * __`no`__ - No spawn reply message will be sent to the caller when the spawn operation completes. This regardless of whether the operation succeeds or not.

  * __`error_only`__ - No spawn reply message will be sent to the caller if the spawn operation succeeds, but an [*error message*](`m:erlang#spawn_request_error_message`) will be sent to the caller if the operation fails.

  * __`success_only`__ - No spawn reply message will be sent to the caller if the spawn operation fails, but a [*success message*](`m:erlang#spawn_request_success_message`) will be sent to the caller if the operation succeeds.

  

* __`{reply_tag, ReplyTag}`__ - Sets the reply tag to `ReplyTag` in the reply message. That is, in the [*success*](`m:erlang#spawn_request_success_message`) or [*error*](`m:erlang#spawn_request_error_message`) message that is sent to the caller due to the spawn operation. The default reply tag is the atom `spawn_reply`.

* __`OtherOption`__ - Other valid options depends on what options are supported by the node identified by `Node`. A description of other valid `Option`s for the local node of current OTP version can be found in the documentation of `spawn_opt/4`.
...
-doc """
Abandon a previously issued spawn request. `ReqId` corresponds to a request identifier previously returned by [`spawn_request()`](`spawn_request/5`) in a call from current process. That is, only the process that has made the request can abandon the request.
...
* __`true`__ - The spawn request was successfully abandoned.

* __`false`__ - No spawn request was abandoned. The `ReqId` request identifier did not correspond to an outstanding spawn request issued by the calling process. The reason for this is either:
...
See the documentation for [`receive-after` expressions](`p:system:expressions.md#receive`) for how to make the current process sleep for a specific number of milliseconds.
...
-doc "Returns a list of all nodes connected to this node through normal connections (that is, [hidden nodes](`p:system:distributed.md#hidden-nodes`) are not listed). Same as [nodes(visible)](`m:erlang#nodes_visible`).".
...
* __`visible`{: id=nodes_visible }__ - Nodes connected to this node through normal connections.

* __`hidden`__ - Nodes connected to this node through hidden connections.

* __`connected`__ - All nodes connected to this node.

* __`this`__ - This node.

* __`known`__ - Nodes that are known to this node. That is, connected nodes and nodes referred to by process identifiers, port identifiers, and references located on this node. The set of known nodes is garbage collected. Notice that this garbage collection can be delayed. For more information, see [`erlang:system_info(delayed_node_table_gc)`](`m:erlang#system_info_delayed_node_table_gc`).
...
* __`node_type => boolean()`__ - If the value of the association equals `true`, the `Info` map in the returned result will contain the key `node_type` associated with the value `NodeTypeInfo`. Currently the following node types exist:

  * __`visible`__ - The node is connected to the node of the calling process through an ordinary visible connection. That is, the node name would appear in the result returned by `nodes/0`.

  * __`hidden`__ - The node is connected to the node of the calling process through a hidden connection. That is, the node name would *not* appear in the result returned by `nodes/0`.

  * __`this`__ - This is the node of the calling process.

  * __`known`__ - The node is not connected but known to the node of the calling process.
...
> #### Note {: class=info }
> This function may return before [`nodedown` messages](`monitor_node/2`) have been delivered.
""".
...
* __`{type, Type}`__ - `Type` is `local` or `external`.

* __`{module, Module}`__ - `Module` (an atom) is the module name.
...
* __`{name, Name}`__ - `Name` (an atom) is a function name.
...
* __`{arity, Arity}`__ - `Arity` is the number of arguments that the fun is to be called with.

* __`{env, Env}`__ - `Env` (a list) is the environment or free variables for the fun. For external funs, the returned list is always empty.

The following elements are only present in the list if `Fun` is local:

* __`{pid, Pid}`__ - `Pid` is the process identifier of the process that originally created the fun.
  ...
  > #### Change {: class=neutral }
  > In Erlang/OTP 27, we plan to change the return value so that it always points to the local `init` process, regardless of which process or node the fun was originally created on. See [Upcoming Potential Incompatibilities ](`p:system:upcoming_incompatibilities.md#fun-creator-pid-will-always-be-local-init-process`).

* __`{index, Index}`__ - `Index` (an integer) is an index into the module fun table.

* __`{new_index, Index}`__ - `Index` (an integer) is an index into the module fun table.

* __`{new_uniq, Uniq}`__ - `Uniq` (a binary) is a unique value for this fun. It is calculated from the compiled code for the entire module.

* __`{uniq, Uniq}`__ - `Uniq` (an integer) is a unique value for this fun. As from Erlang/OTP R15, this integer is calculated from the compiled code for the entire module. Before Erlang/OTP R15, this integer was based on only the body of the fun.
""".
...
-doc "The destination for a send operation, can be a remote or local process identifier, a (local) port, a reference denoting a process alias, a locally registered name, or a tuple `{RegName, Node}` for a registered name at another node.".
...
-doc """
The same as [`erlang:send(Dest, Msg, [nosuspend])`](`send/3`), but returns `true` if the message was sent and `false` if the message was not sent because the sender would have had to be suspended.
...
-doc """
The same as [`erlang:send(Dest, Msg, [nosuspend | Options])`](`send/3`), but with a Boolean return value.

This function behaves like [`erlang:send_nosuspend/2`](`send_nosuspend/2`), but takes a third parameter, a list of options. The only option is `noconnect`, which makes the function return `false` if the remote node is not currently reachable by the local node. The normal behavior is to try to connect to the node, which can stall the process during a short period. The use of option `noconnect` makes it possible to be sure not to get the slightest delay when sending to a remote process. This is especially useful when communicating with nodes that expect to always be the connecting part (that is, nodes written in C or Java).
...
* __`badarg`__ - If `Port` is not an identifier of an open port, or the registered name of an open port. If the calling process was previously linked to the closed port, identified by `Port`, the exit signal from the port is guaranteed to be delivered before this `badarg` exception occurs.

* __`badarg`__ - If `Data` is an invalid I/O list.
...
* __`force`__ - The calling process is not suspended if the port is busy, instead the port command is forced through. The call fails with a `notsup` exception if the driver of the port does not support this. For more information, see driver flag [`ERL_DRV_FLAG_SOFT_BUSY`](driver_entry.md#driver_flags).

* __`nosuspend`__ - The calling process is not suspended if the port is busy, instead the port command is aborted and `false` is returned.
...
* __`badarg`__ - If `OptionList` is an invalid option list.

* __`notsup`__ - If option `force` has been passed, but the driver of the port does not allow forcing through a busy port.
...
* __`badarg`__ - If the process identified by `Pid` is not an existing local process.
""".
...
* __`badarg`__ - If `Port` is not an open port or the registered name of an open port.

* __`badarg`__ - If `Operation` cannot fit in a 32-bit integer.

* __`badarg`__ - If the port driver does not support synchronous control operations.

* __`badarg`__ - If the port driver so decides for any reason (probably something wrong with `Operation` or `Data`).
  > #### Warning {: class=warning }
...
* __`badarg`__ - If `Operation` does not fit in a 32-bit integer.
...
`OsPid` is the process identifier (or equivalent) of an OS process created with [`open_port({spawn | spawn_executable, Command}, Options)`](`open_port/2`). If the port is not the result of spawning an OS process, the value is `undefined`.
...
`Boolean` corresponds to the port parallelism hint used by this port. For more information, see option [`parallelism`](`m:erlang#open_port_parallelism`) of `open_port/2`.
...
-doc """
Register an alternate input handler process for the distribution channel identified by `DHandle`. Once this function has been called, `InputHandler` is the only process allowed to call [`erlang:dist_ctrl_put_data(DHandle, Data)`](`dist_ctrl_put_data/2`) with the `DHandle` identifying this distribution channel.

> #### Note {: class=info }
> When the distribution controller for the distribution channel identified by `DHandle` is a process, it is the only process allowed to call this function. This function is also allowed to be called when the distribution controller for the distribution channel identified by `DHandle` is a port. The data received by the port should in this case be delivered to the process identified by `InputHandler` which in turn should call [`erlang:dist_ctrl_put_data/2`](`dist_ctrl_put_data/2`).

This function is used when implementing an alternative distribution carrier. `DHandle` is retrieved via the callback [`f_handshake_complete`](alt_dist.md#hs_data_f_handshake_complete). More information can be found in the documentation of [ERTS User's Guide ➜ How to implement an Alternative Carrier for the Erlang Distribution ➜ Distribution Module](alt_dist.md#distribution_module).
""".
-doc(#{since => <<"OTP 21.0">>}).
...
-doc """
Get distribution channel data from the local node that is to be passed to the remote node. The distribution channel is identified by `DHandle`. If no data is available, the atom `none` is returned. One can request to be informed by a message when more data is available by calling [`erlang:dist_ctrl_get_data_notification(DHandle)`](`dist_ctrl_get_data_notification/1`).

The returned value when there are data available depends on the value of the `get_size` option configured on the distribution channel identified by `DHandle`. For more information see the documentation of the `get_size` option for the [`erlang:dist_ctrl_set_opt/3`](`dist_ctrl_set_opt/3`) function.
...
This function is used when implementing an alternative distribution carrier using processes as distribution controllers. `DHandle` is retrieved via the callback [`f_handshake_complete`](alt_dist.md#hs_data_f_handshake_complete). More information can be found in the documentation of [ERTS User's Guide ➜ How to implement an Alternative Carrier for the Erlang Distribution ➜ Distribution Module](alt_dist.md#distribution_module).
""".
-doc(#{since => <<"OTP 21.0">>}).
...
-doc """
Sets the value of the `get_size` option on the distribution channel identified by `DHandle`. This option controls the return value of calls to [erlang:dist_ctrl_get_data(DHandle)](`dist_ctrl_get_data/1`) where `DHandle` equals `DHandle` used when setting this option. When the `get_size` option is:

* __`false`__ - and there are distribution data available, a call to `erlang:dist_ctrl_get_data(DHandle)` will just return `Data` to pass over the channel. This is the default value of the `get_size` option.

* __`true`__ - and there are distribution data available, a call to `erlang:dist_ctrl_get_data(DHandle)` will return `Data` to pass over the channel as well as the `Size` of `Data` in bytes. This is returned as a tuple on the form `{Size, Data}`.
...
-doc """
Returns the value of the `get_size` option on the distribution channel identified by `DHandle`. For more information see the documentation of the `get_size` option for the [`erlang:dist_ctrl_set_opt/3`](`dist_ctrl_set_opt/3`) function.
...
-doc """
Sets the magic cookie of the local node to the atom `Cookie`, which is also the cookie for all nodes that have no explicit cookie set with `set_cookie/2` `Cookie` (see section [Distributed Erlang](`p:system:distributed.md`) in the Erlang Reference Manual in System Documentation).
...
-doc """
Sets the magic cookie for `Node` to the atom `Cookie`. If `Node` is the local node, the function sets the cookie of all other nodes (that have no explicit cookie set with this function) to `Cookie` (see section [Distributed Erlang](`p:system:distributed.md`) in the Erlang Reference Manual in System Documentation).
...
-doc "Returns the magic cookie of the local node if the node is alive, otherwise the atom `nocookie`. This value is set by `set_cookie/1`.".
-spec erlang:get_cookie() -> Cookie | nocookie when
      Cookie :: atom().
get_cookie() ->
    auth:get_cookie().

-doc "Returns the magic cookie for node `Node` if the local node is alive, otherwise the atom `nocookie`. This value is set by `set_cookie/2`.".
-doc(#{since => <<"OTP 24.1">>}).
...
The [Expressions section](`p:system:expressions.md#term-comparisons`) contains descriptions of the `==` operator and how terms are ordered.
...
* __`total`__ - The total amount of memory currently allocated. This is the same as the sum of the memory size for `processes` and `system`.

* __`processes`__ - The total amount of memory currently allocated for the Erlang processes.

* __`processes_used`__ - The total amount of memory currently used by the Erlang processes. This is part of the memory presented as `processes` memory.

* __`system`__ - The total amount of memory currently allocated for the emulator that is not directly related to any Erlang process. Memory presented as `processes` is not included in this memory. `m:instrument` can be used to get a more detailed breakdown of what memory is part of this type.

* __`atom`__ - The total amount of memory currently allocated for atoms. This memory is part of the memory presented as `system` memory.

* __`atom_used`__ - The total amount of memory currently used for atoms. This memory is part of the memory presented as `atom` memory.

* __`binary`__ - The total amount of memory currently allocated for binaries. This memory is part of the memory presented as `system` memory.

* __`code`__ - The total amount of memory currently allocated for Erlang code. This memory is part of the memory presented as `system` memory.

* __`ets`__ - The total amount of memory currently allocated for ETS tables. This memory is part of the memory presented as `system` memory.

* __`maximum`__ - The maximum total amount of memory allocated since the emulator was started. This tuple is only present when the emulator is run with instrumentation.

  For information on how to run the emulator with instrumentation, see `m:instrument` and/or [`erl(1)`](erl_cmd.md).
...
> #### Change {: class=neutral }
> As from ERTS 5.6.4, `erlang:memory/0` requires that all [`erts_alloc(3)`](erts_alloc.md) allocators are enabled (default behavior).

Failure: `notsup` if an [`erts_alloc(3)`](erts_alloc.md) allocator has been disabled.
""".
...
> #### Change {: class=neutral }
> As from ERTS 5.6.4, `erlang:memory/1` requires that all [`erts_alloc(3)`](erts_alloc.md) allocators are enabled (default behavior).

Failures:

* __`badarg`__ - If `Type` is not one of the memory types listed in the description of [`erlang:memory/0`](`memory/0`).

* __`badarg`__ - If `maximum` is passed as `Type` and the emulator is not run in instrumented mode.

* __`notsup`__ - If an [`erts_alloc(3)`](erts_alloc.md) allocator has been disabled.

%% ===============================================================
%% Generic pretty printer: a strict-style context passing
%% implementation of John Hughes algorithm, described in ``The
%% design of a Pretty-printing Library''. The "paragraph-style"
%% formatting, empty documents and "floating" documents are my own
%% additions to the algorithm. A note on hacking: it's fairly easy
%% to break this thing (in particular, to muck up the complexity)
...
-vsn('$Id: prettypr.erl,v 1.3 2003/04/18 11:41:00 richardc Exp $').
...
main([]) ->
   case get(prettypr_data) of
       undefined ->
           {ok, _Dev, Fullname} =
               file:path_open(code:get_path(), "prettypr.input", [read]),
           {ok,[X]} = file:consult(Fullname),
           put(prettypr_data, X),
           X;
       X -> X
   end,
...
prettyprint() ->
   Data =
     case get(prettypr_data) of
       undefined ->
       exit({no_prefetched_data});
       X -> X
     end,
   aprettypr:format(Data),
   ok.

%% ===============================================================
%% text(string()) -> document()
%%
%%	Yields a document representing a fixed sequence of
...
%%	(`$\t') is interpreted as padding of 1-8 space characters
%%	to the next column of 8 characters *within the string*.
...
%%	    Doc = Doc1 = document()
%%
%%	`empty' yields the empty document, which has neither
%%	height nor width (it is thus different from an empty
%%	`text' string, which has zero width but height 1). Empty
%%	documents are occasionally useful; in particular, they
%%	have the property that |above(X, empty())| will force a
%%	new line after `X' without leaving an empty line below it;
%%	since this is a common idiom, the utility function `break'
...
%%	    Doc = Doc1 = document()
%%
%%	`Doc1' is the document `Doc' with an additional
%%	indentation of `N' character positions to the right. Note
%%	that N may be negative, shifting the text to the left, or
%%	zero, in which case D is not affected.
...
%%
%%	`Doc' is a document representing the horizontal
%%	concatenation of the documents `Doc1' and `Doc2' such that
%%	the last character of `Doc1' is directly adjacent to the
%%	first character of `Doc2', in all possible layouts of
%%	`Doc'.
%%
...
%%	    Doc1 = Doc2 = Doc = document()
%%
%%	`Doc' is a document representing the vertical
%%	concatenation of the documents `Doc1' and `Doc2' such that
%%	the first line of `Doc2' follows directly below the last
%%	line of `Doc1', and the first character of `Doc1' is in
%%	the same horizontal column as the first character of
%%	`Doc2', in all possible layouts of `Doc'.
...
%%	    Doc = document()
%%
%%	`Doc' is a document representing two alternative layouts
%%	of the (nonempty) sequence `Docs' of documents, such that
%%	either all elements in `Docs' are concatenated
%%	horizontally, and separated by a space character, or all
%%	elements are concatenated vertically (without extra
%%	separation).
%%
%%	Note: If some document in `Docs' contains a line break,
%%	the vertical layout will always be selected.
...
%%	sequence `Docs' of documents. Elements in `Docs' are
%%	separated horizontally by a single space character and
%%	vertically with a single line break. All new lines are
%%	indented to the same left column. The optional `Offset'
%%	parameter specifies the indentation of the following lines
%%	(if any) relative to the position of the first element in
%%	`Docs'. For example, with an offset of -4, the following
...
%%		15 16 17
%%
%%	Note: whenever a document in `Docs' contains a line break,
%%	it will be placed on a separate line. Thus, neither
%%
%%		(foo) (bar
%%		       baz)
%%	nor
...
%%		 baz) (foo)
%%
%%	will be generated. However, a useful idiom for making the
%%	former variant possible for two documents `D1' and `D2'
%%	is: |beside(par([D1, text("")], N), D2)|. This will break
%%	the line between `D1' and `D2' if `D1' contains a line
%%	break, or otherwise if necessary, and (optionally) further
%%	indent `D2' by `N' character positions; i.e.:
%%
%%		D1 D2
%%	or
%%		D1
...
%%	    HPri = VPri = integer()
%%
%%	`Doc1' is a "floating" document representing the same set
%%	of layouts as `Doc'. A floating document may be moved
%%	relative to other floating documents immediately beside or
%%	above it, according to their relative horizontal and
%%	vertical priorities. These priorities are set with the
%%	`HPri' and `VPri' parameters; if left out, both default to
%%	zero.
%%
%%	Note: floating documents seem to be working well, but have
%%	not been thoroughly mained yet. It is possible to nest
%%	"floating" operators (even with different priorities), but
%%	the effects may be difficult to predict. In any case, note
%%	that the way the algorithm reorders floating documents
%%	amounts to a "bubblesort", so don't expect it to be able
%%	to sort large sequences of floating documents quickly.
...
%%
%%	`Document' specifies a set of possible layouts of a text;
%%	see the corresponding constructor functions for details.
%%	`format' selects a "best" layout for the document (cf.
%%	`best') and returns the corresponding text.
%%
%%	`PaperWidth' specifies the width (in number of characters)
%%	of the field for which the text is to be laid out.
%%	`LineWidth' specifies the desired maximum width (in number
%%	of characters) of the text printed on any single line,
%%	disregarding leading and trailing white space. For
%%	instance, formatting with a `PaperWidth' of 120, it could
%%	be desirable to use a `LineWidth' of no greater than 70 in
%%	order to avoid getting too much text on individual lines.
%%	By default, `PaperWidth' is 80 and `LineWidth' is 65.
...
%%			  p = bool()}
%%
%% A `text' node simply represents a string (which should not
%% contain linefeed or carriage return characters). A `nest' node
%% specifies a relative indentation (in number of character
%% positions) of a document. The indentation could be a negative
%% number. A `beside' node specifies a horizontal composition of
%% two documents, and an `above' node a vertical composition. A
%% `sep' node specifies a list of alternative documents; the `i'
%% field holds the extra indentation of all documents but the
%% first in `ds', and if the `p' field is `true' then the list is
%% typeset in paragraph mode.
%%
%% The function `best/3' yields a representation of a "best
%% layout", suitable for direct conversion to text, having the
...
%%		 | {nest, int(), layout()}
%%
%% the function `layout/1' performs the final transformation to a
%% single flat string.
...
%%	`format' for more information. This function selects a
%%	"best" layout for the document and returns the
%%	corresponding `Doc1' representing that single layout, or
%%	the atom `empty' if no layout could be produced. For
%%	information on `PaperWidth' and `LineWidth', see `format'.
%%
...
%%		 W, L)
%%
%%	will include `D' as a displayed-text example indented by 8
%%	and with a right margin indentation of 4, the maximum
%%	length of individual lines shorter by 6 than in the
%%	surrounding document.

%% Recall that a document represents a set of possible layouts.
%% `best' selects the "best" layout of a document, returning a
%% simplified representation that can be given directly to
%% `layout', unless the returned value is `empty', signaling that
%% no layout could be produced. In addition, documents on the form
%% `#union{d1 = D1, d2 = D2}' and `#fit{d = D}' are used
%% internally.
%%
%% Note: It is vital for this algorithm to maintain the invariant
%% on unions that the left argument has a longer first line than
%% the right argument!
	...
	#c_sep_nest{ds = Ds, i = N, c = C1} ->
		...
		false ->
		    ...
		    rewrite(beside(mktext(S),
			    ...
			    C1);
		true ->
		    %% This is the only place where we are forced
		    %% to introduce a union. Recall the invariant
		    %% that the left argument must have a longer
		    %% first line than the right argument; also
		    %% recall that `Ds' is always nonempty here.
		    %% Now, since [D | Ds] contains at least two
		    %% elements, the first line of the horizontal
		    %% layout will always contain at least one
		    %% space character more than the first line of
		    %% the vertical layout.
...
rewrite(#nest{n = N, d = D}, C) ->
	...
	#c_best_nest{w = W, r = R, i = N1} ->
	    %% Note that we simplify by not creating an actual
	    %% `nest' node, but instead just modifying the
	    %% context: rewrite(nest(N1, nest(N, D))) =
	    %% rewrite(nest(N1 + N, D)).
...
%% not perform list concatenation until the text is requested. We use
%% cons cells for the structure, mainly to minimise space usage.
...
%% We need to use `strwidth' instead of list `length', in case the
%% user places Tab characters in the text segments. Note that the
%% width of tabs is hard-coded as 8 character positions, and that
%% strings are individually considered to be aligned at column 0;
%% Tab characters are not really nice to give to a prettyprinter,
%% and this seems to be the best interpretation.

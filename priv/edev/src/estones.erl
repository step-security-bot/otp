...
-export([lists/1,
	 ...
	 timer/1,
	 links/1,lproc/1,
	 run_micro/2,p1/1,ppp/3,macro/1,micros/0]).
...
benchmark_unit() -> "estones".

test(Type) -> 
    Stats = run_micro(micro(Type)),
    proplists:get_value(estones, Stats).
...
go() ->
    L = macro(micros()),
    {_, Stones} = sum_micros(L, 0, 0),
    [{"Total", Stones}] ++ make_values(L).

make_values(Items) -> make_values(Items, []).
make_values([], Out) -> Out;
make_values([Item|Items],Out) ->
    Key       = ks(title, Item),
    Stones    = ks(estones, Item),
    %Time      = ks(Item, microsecs),
    %KeyStones = Key ++ "_stones",
    %KeyTime   = Key ++ "_ms",
    make_values(Items, [{Key,Stones}|Out]).
    %make_values(Items, [{KeyTime,Time},{KeyStones,Stones}|Out]).
...
run_micro(Top, M) -> Top ! {self(),  apply_micro(M)}.
...
disp() ->
    Tup = {a},
    L = [x],
    _P1 = self(), _P2 = self(), _P3 = self(), _P4 = self(), _P5 = self(),
    _P6 = self(), _P7 = self(), _P8 = self(), _P9 = self(),
    _ = make_ref(),
    _ = atom_to_list(''),
    _X = list_to_atom([]),
    _ = tuple_to_list({}),
    ...
    _ = element(1, Tup),
    _ = element(1, Tup),
    _Elem = element(1, Tup), _ = element(1, Tup),_ = element(1, Tup),_ = element(1, Tup),
    _ = element(1, Tup),_ = element(1, Tup),_ = element(1, Tup),_ = element(1, Tup),
    _ = element(1, Tup),_ = element(1, Tup),_ = element(1, Tup),_ = element(1, Tup),
    _ = element(1, Tup),_ = element(1, Tup),_ = element(1, Tup),_ = element(1, Tup),
    _ = setelement(1, Tup,k),
    _ = setelement(1, Tup,k),
    _ = setelement(1, Tup,k),_ = setelement(1, Tup,k),_ = setelement(1, Tup,k),
    _ = setelement(1, Tup,k),_ = setelement(1, Tup,k),_ = setelement(1, Tup,k),
    _ = setelement(1, Tup,k),
    _ = setelement(1, Tup,k),
    _ = setelement(1, Tup,k),
    ...
    _ = hd(L),
    _ = tl(L),
    _Len = length(L),_ = length(L),_ = length(L),_ = length(L),
    _ = node(),_ = node(),_ = node(),_ = node(),_ = node(),_ = node(),_ = node(),_ = node(),
    S=self(),
    _ = node(S),_ = node(S),_ = node(S),
    _ = size(Tup),
    _W = whereis(code_server),_ = whereis(code_server),
    _ = whereis(code_server),_ = whereis(code_server),
    _ = whereis(code_server),_ = whereis(code_server),

...
-module(netio).
-export([consumer/0, producer/0, test/0, test/2]).
...
benchmark_arguments() ->
    [{test, [[],[{connections, Nc}, {packet_size, Psz}, {ack, Ack}, {packets, Np}]]} ||
    	Nc <- [200],
	Psz <- [1000, 10000],
	Np <- [1000],
	Ack <- [false]
...
consumer(Opts) ->
    S = #state{
    	port       = proplists:get_value(port, Opts, 8760),
...
binary(N) -> erlang:list_to_binary(lists:duplicate(N, 77)).

...
-export([go/0, go/1, go/2, test/3]).
-export([benchmark_arguments/0, benchmark_unit/0]).

benchmark_arguments() ->
    [{test, [Drivers, Accounts, Branches]} || Drivers <- [20,40], Accounts <- [20,40], Branches <- [2000]].  
...
test(Drivers, Accounts, Branches) ->
    Nodes = [node()],
    S     = 20,
    Config = [
    	{db_nodes, Nodes},
...
driver_loop(DS) ->
	...
	{From, report} ->
	    ...
	    driver_loop(DS3);
	{From, stop} ->
	    _Acc = add_time(DS#driver_state.time, DS#driver_state.acc_time),

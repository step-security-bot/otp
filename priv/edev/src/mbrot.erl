...
benchmark_arguments() ->
    [{go, [N, Np]} || N <- [250, 500, 750], Np <- [8, 16, 32]].
...
%%% go(N, Np)
%%% N  :: size of image in pixels (N x N)
%%% Np :: #processes running the an instance of the mandelbrot
...
start_workers(N, Np) -> start_workers(N, Np, self(), []).

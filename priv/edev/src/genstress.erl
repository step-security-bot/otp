...
benchmark_arguments() ->
    [{Type, [Np, N, Cqueue]} || Type <- [proc_call, gen_call], Np <- [10,1000], N <- [3000], Cqueue <- [500, 5000]].
...
gen_call(Np, N, Cqueue) ->
    test(gen_call, Np, N, Cqueue).
proc_call(Np, N, Cqueue) ->
    test(proc_call, Np, N, Cqueue).

test(Type, Np, N, Cqueue) ->
